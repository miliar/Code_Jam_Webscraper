...
inFile = open('C:/Users/quentin/Downloads/B-small-attempt10.in', 'r')
...
def doCase(case):
	...
	L = []
	if D > 1:
		L = sorted([int(x) for x in inFile.readline().split()], reverse = True)
	else:
	...
	while swap < 20:
		s, x = (Q.get())
		x, n = x
		best = min(swap - s , best)
		#print(best)
		if s == -1:
			break
		n += 1
		swap+=1
		Q.put((-math.ceil(x/n), (x,n)))
