...
def get_flip_count(pancakes, desired_direction):
    """
    pancakes: STR current config
    desired_direction: STR desired config
    return: minimum # of flips to get from pancakes -> desired
    """
    if all(pancake == desired_direction for pancake in pancakes):
        # Everything is in the direction we want.
        # This may be vacuously true if there are no pancakes left in the stack.
        return 0
    elif len(pancakes) == 1:
        # There's one pancake left, but it's not in the direction we want. Flip that and we're done.
        return 1

    # If there are any pancakes in the desired direction at the bottom (right), we can ignore those
    pancakes_to_configure = pancakes.rstrip(desired_direction)

    # Now, we're left with N pancakes at the end that aren't in the direction we want.
    # We can get everything else to be in THAT direction, and then flip the whole stack up through here.
    opposite_direction = get_opposite_direction(desired_direction)
    pancakes_to_flip_first = pancakes_to_configure.rstrip(opposite_direction)
    return 1 + get_flip_count(pancakes_to_flip_first, opposite_direction)
...
if __name__ == "__main__":
    ...
    for case_num in range(1, t + 1):
        pancakes = raw_input()
        print "Case #{}: {}".format(case_num, get_flip_count(pancakes, "+"))
