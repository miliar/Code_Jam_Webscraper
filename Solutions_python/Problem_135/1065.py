...
def reader(inFile):
    print "New Case:"
    firstdraw = inFile.getInt()
    for k in xrange(4):
        if k == firstdraw - 1:
            firstrow = inFile.getInts()
        else:
            inFile.readline()

    seconddraw = inFile.getInt()
    for k in xrange(4):
        if k == seconddraw - 1:
            secondrow = inFile.getInts()
        else:
            inFile.readline()
    return [firstrow, secondrow]

def solver(case):
    intersect = set(case[0]).intersection(case[1])
    if len(intersect)==1:
        sln = list(intersect)[0]
    elif len(intersect)==0:
        sln = "Volunteer cheated!"
    else:
        sln = "Bad magician!"
    return sln
...
if __name__ == "__main__":
    from GCJ import GCJ
    GCJ(reader, solver, "C:/Users/Robin/PycharmProjects/GCJ2014/", "a").run()
