...
""" functions """
def solve(rnum1, rows1, rnum2, rows2):
    intersect = set(rows1[rnum1 - 1]).intersection(set(rows2[rnum2 - 1]))
...
for questionindex in xrange(numquestions):
    ### parse input ###
    rnum1, = read_ints()
    rows1 = [read_ints() for i in range(4)]
    rnum2, = read_ints()
    rows2 = [read_ints() for i in range(4)]

    ### calculate answer ###
    answer = solve(rnum1, rows1, rnum2, rows2)
