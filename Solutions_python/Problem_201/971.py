...
def solve_case(next):
    ...
    N, K = [int(n) for n in next().split()]
    high_val, num_high, num_splits, max_splits = N, 1, 0, int(math.log(K, 2))
    while num_splits < max_splits:
        if high_val % 2 == 1:
            num_high += 2 ** num_splits
        high_val /= 2
        num_splits += 1
    K_left = K - 2 ** max_splits + 1
    if K_left <= num_high:
        split_num = high_val
    else:
        split_num = high_val - 1
    return str(split_num / 2) + ' ' + str((split_num - 1) / 2)
