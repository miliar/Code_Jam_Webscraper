	...
	with file(filepath, 'rb') as f_in:
		...
		for line_index, line in enumerate(f_in):
			...
			if new_case:
				...
				lines_left = int(line.strip().split(' ')[0])
...
def calc_result(case):
    ...
    for r in xrange(R):
        ...
        if '?' not in map[r]:
            print 'full'
            continue
        if all(map[r][c] == '?' for c in xrange(C)):
            print 'all ?'
            if r == 0:
                for j in xrange(R):
                    if any(map[j][c] != '?' for c in xrange(C)):
                        break
                map[r] = map[j]
                print map[r],
            else:
