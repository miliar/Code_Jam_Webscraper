...
for cases in xrange(1,T+1): # loop over cases
    ...
    graph = map(lambda x: [0]*N, [0]*N)

    # find max appart points and store dist
    max = -1
    ...
    for i in xrange(N):
        ...
        d1 = graph[i][u]#dist(pi[0], pu[0]) + pu[1]
        d2 = graph[i][v]#dist(pi[0], pv[0]) + pv[1]

        if d1 < d2: cu.append(i)
        else: cv.append(i)
                
    #print cu, cv

    # find max dist within each cluster
    maxu, maxv = -1, -1
    for i in cu:
        for j in cu:
            d = graph[i][j]
    ...
    if maxu > maxv: ans = maxu / 2.0
    else: ans = maxv / 2.0

    # main
    print "Case #%d: %.7f"%( cases, ans ) # answer output
