...
Then she starts naming N, 2 × N, 3 × N, and so on. Whenever she names a number, she thinks about all of the digits in that number.
...
class Sheep:
    def __init__(self, caseNum, inputNum):
        self.resultNum = "INSOMNIA"
    ...
    def __repr__(self):
        return "Case #{0}: {1}".format(self.caseNum, self.resultNum)

    def add_ch(self, ch):
        if ch in self.p:
            return False
        self.p.append(ch)
        #print("key = {0} {1} {2} {3}".format(ch,self.p,len(self.p),len(self.p)==10))
