...
def get_divisors(nb):
    ...
    for base in range(2, 11):
        test_composite = int(nb, base)
        for divisor in range(2, 100):
            if test_composite % divisor == 0:
                divisors.append(divisor)
                break
        else:
            # we didn't find a divison under 100: try another nb
            return
    return divisors

def get_jamcoins(n, j):
    found = {}
    for pre_nb in range(2**(n-2)):
        pre_nb_bin = bin(pre_nb).lstrip('0b')
        padding = '0' * (n - 2 - len(pre_nb_bin))
        nb = ''.join(('1', padding, pre_nb_bin , '1'))
        candidate_divisors = get_divisors(nb)
        if candidate_divisors:
            found[nb] = candidate_divisors
        if len(found) >= j:
            return found
    assert False, 'failed to find enough jamcoins'
...
def do_job(in_file, out_file=sys.stdout):
    ...
    for testcase in range(T):
        ...
        # pylint: enable=invalid-name
        # for integer input
        N, J = [int(x) for x in in_file.readline().split()]
        # for other inputs
#        values = in_file.readline().rstrip('\n')
        # assert len(values) == N
        result = get_jamcoins(N, J)
        # print_output(out_file, testcase, result)
        print('Case #1:', file=out_file)
        for k in result:
            print(k, ' '.join(map(str, result[k])), file=out_file)
