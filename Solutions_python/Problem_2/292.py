...
def open_file(module_name, inputfile_setting = 3):
    ...
    for fin_name, fout_name in reversed(map(None, in_names[:], out_names[:])):
...
def problem_solution(fin , fout, base_name):
    ...
    from B import problem_testcase
...
def check_sample_solution(filename_out, filename_val):
    ...
    validated = True
    for out_l, val_l in map(None, file(filename_out, 'rt').readlines(), file(filename_val, 'rt').readlines()):
        out_items = clean_items(out_l)
        val_items = clean_items(val_l)
        difference = False
        for out_item, val_item in map(None, out_items, val_items):
