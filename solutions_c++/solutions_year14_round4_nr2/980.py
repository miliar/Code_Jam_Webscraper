...
/*int updown(vector<int> v,int i, int mode, int jst){
	if(i == -1) return updown(v,0,mode,0);
	if(i == v.size()-1) return 0;
	if(mode == 1){
		if(v[i+1] > v[i])
			return min(updown(v,i+1,1,0),updown(v,i+1,2,1));
		else{
			vector<int> u = v;
			int a = u[i+1];
			u[i+1] = u[i];
			u[i] = a;
			return min(1 + updown(u,i-1,1,0), updown(v,i+1,2,1));
		}
	}
	else{
		if(v[i+1] < v[i])
			return updown(v,i+1,2,0);
		else{
			vector<int> u = v;
			int a = u[i+1];
			u[i+1] = u[i];
			u[i] = a;
			if(jst == 1)
				return 1 + updown(v,i+1,2,1);
			else 
				return 1 + updown(u,i-1,2,0);
		}
	}
}*/

int doup(vector<int> &v,int &a, int &b){
	int minint  = 1000000001;
	int minpos = 0;
	for(int i = a; i < b; i++){
		if(v[i] < minint){
			minpos = i;
	...
	if(minpos - a <= b - 1 - minpos){
		//shiftleft
		for(int j = minpos; j > a; j--){
			int t  = v[j-1];
			v[j-1] =v[j];
			v[j] = t;
		}
