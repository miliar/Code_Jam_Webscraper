 ...
 * ccmp(圧縮後) => 元の座標
 */
template<class T> struct ccmp: map<T, int>{ //{{{
    vector<T> r;
    void add(T x){
        this->insert(mp(x, -1));
    }
...
template<typename T> struct BIT{//{{{
    int max;
    vector<T> bit;
    BIT(int max):max(max){bit = vector<T>(max+1, 0);}
    T sum(int i){// [0, i) //{{{
        T s=0;
        while(i>0){
            s += bit[i];
...
inline void solve2(){
    ...
    BIT<int> tree(n+10);
    rep(i, n) tree.add(i, 1LL);
    int res = 0;
    for(auto xi : b){
        int x = xi.fst;
        int idx = xi.snd;
        tree.add(idx, -1);
        int l = tree.sum(0, idx);
        int r = tree.sum(idx, n);
        res += min(l, r);
    }
    cout << res << endl;
}
