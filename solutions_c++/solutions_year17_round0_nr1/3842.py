...
// const unsigned long POW2 [32]
// ={1ul,2ul,4ul,8ul,16ul,32ul,64ul,128ul,256ul,512ul,1024ul,2048ul,4096ul,8192ul,16384ul,32768ul,65536ul,131072ul,262144ul,524288ul,1048576ul,2097152ul,4194304ul,8388608ul,16777216ul,33554432ul,67108864ul,134217728ul,268435456ul,536870912ul,1073741824ul,2147483648ul};
// const unsigned long POW2 [64] =
...
void flip(string& a, int i, int s)
{
    int end = i + s;
    for (; i < end; ++i) {
        if (a[i] == '+') a[i] = '-';
...
int main()
{
    ...
    for (int t = 1; t <= T; ++t) {
        ...
        int flips = 0;
        for (int i = 0; i <= size(a) - s; ++i) {
            if (a[i] == '-') {
                flip(a, i, s);
                ++flips;
            }
        }
        if (ranges::all_of(a, [](char c) { return c == '+'; }))
            cout << "Case #" << t << ": " << flips << "\n";
        else
