...
public:
  ...
  void Run(uint32 T) {
    auto v = as_vector(map(generate(text), [](char c) { return (c == '+')? 0 : 1; }));
    int32 count = 0;
    std::set<int32> flippedTill;
    for (auto j: range<int32>(0, text.size() - K + 1)) {
      flippedTill.erase(j);
      if ((v[j] + flippedTill.size()) % 2 == 1) {
        flippedTill.insert(j + K);
        count++;
      }
    }
    for (auto j: range<int32>(text.size() - K + 1, text.size())) {
      flippedTill.erase(j);
      if ((v[j] + flippedTill.size()) % 2 == 1) {
        print("Case #%0: %1", T + 1, "IMPOSSIBLE");
        return;
      }
    }
    print("Case #%0: %1", T + 1, count);
  }
