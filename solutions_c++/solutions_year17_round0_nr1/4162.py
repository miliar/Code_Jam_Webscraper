...
string flip(string flipMe) {
	string result = "";
	for(int i=0; i < flipMe.size(); i++) {
		if(flipMe[i]=='-') result+='+';
		else if(flipMe[i]=='+') result+='-';
	}
	return result;
}

typedef ll otype;
otype calcFunction(string input, ll K) {
	otype result=0;
	for (ll i = 0; i<input.size(); i++) {
		if (input[i]=='-') {
			if (i > input.size()-K) return -1;
			input = input.replace(i, K, flip(input.substr(i, K)));
			result++;
		}
	}
	return result;
...
int main() {
	...
	fore(test, 1, tests){
		...
		otype result=calcFunction(input, K);
		outfile << "Case #" << test << ": ";
		cout << "Case #" << test << ": ";
#ifndef IFSTRUCT
		outfile << result << endl;
