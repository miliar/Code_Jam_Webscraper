...
int main()
{
    ...
    while(test--){
        ...
        for(int i = 0;i<=len;i++) revStr[i] = str[i];
        reverse(revStr,revStr+len);


        int flag1 = func(str,k);
        int flag2 = func(revStr,k);

        printf("Case #%d: ",cas++);
        if(flag1==-1 && flag2==-1)
            pf("IMPOSSIBLE\n");
        else{
            if(flag1==-1) flag1 = 100000000;
            if(flag2==-1) flag2 = 100000000;
            pf("%d\n",min(flag1,flag2));
