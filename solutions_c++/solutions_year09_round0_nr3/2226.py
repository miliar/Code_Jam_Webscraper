...
const int MAX_DIGITS = 1000;	// The maximum digits that can be stored in BigInt
...
int main()
{
	...
	BigInt* table = new BigInt[600*20];
	...
	for (int i = 0; i < N; i++)
	{
		...
		for (int j = 1; j <= len; j++)
		{
			for (int k = 1; k <= 19; k++)
			{
				if (str[j-1] == token[k-1])
				{
					...
					table[j*20+k] = table[(j-1)*20+k-1] + table[(j-1)*20+k];
				}
				else
				{
					//cout << "j: " << j << "   k: " << k << "   token: " << token[k-1] << "   str: " << str[j-1] << endl;
					table[j*20+k] = table[(j-1)*20+k];
				}
			}
		}
		...
		/*if (table[len*20+19] > 2147483647)
		{
			cout << "Warning: " << table[len][19] << " might overflow\n";
		}*/
		//fout << "Case #" << i+1 << ": ";
		fprintf(fp, "Case #%d: ", i+1);
		int nD = table[len*20+19].numDigits;
		cout << table[len*20+19] << endl;
		for (int i = 3; i >= 0; i--)
		{
			if (i >= nD)
				fprintf(fp, "0");
			else
				fprintf(fp, "%d", table[len*20+19].digits[i]);
		}
		fprintf(fp, "\n");
