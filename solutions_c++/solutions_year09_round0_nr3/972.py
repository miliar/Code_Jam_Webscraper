...
int main() {
    ...
    while (k--) {
        ...
        vector<BI> v(S.size(), cero), aux(S.size());
        for (int i = 0; i < S.size(); ++i) if (S[i] == T[0]) v[i] = uno;
        ...
        res%=mod;
        BI diez = 10, cien = 100, mil = 1000;
        cout << "Case #" << caso++ << ": ";
        if (res < diez) cout << "000";
        else if (res < cien) cout << "00";
        else if (res < mil) cout << "0";
        cout << res << endl;
    }
}
