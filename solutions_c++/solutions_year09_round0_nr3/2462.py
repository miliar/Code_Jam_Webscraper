	...
	while(N--)
	{
		...
		for(int i=start+1; i<strlen(cc); i++)
		{
			...
			else if(cc[i] != ' ')
			{
				...
				{
					a[i][b[cc[i]-'a'][j]] = (a[i-1][b[cc[i]-'a'][j]-1] + a[i][b[cc[i]-'a'][j]])%10000;
				//a[i][b[cc[i]-'a'][j]] = (a[i-1][b[cc[i]-'a'][j]-1] + a[i][b[cc[i]-'a'][j]]);
				}
			}
			else
			{
				...
				{
					a[i][b[26][j]] = (a[i-1][b[26][j]-1] + a[i][b[26][j]])%10000;
					//a[i][b[26][j]] = (a[i-1][b[26][j]-1] + a[i][b[26][j]]);
				}
			}
		}
