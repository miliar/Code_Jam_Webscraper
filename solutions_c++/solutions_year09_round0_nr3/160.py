...
void solveCase(int cnum) {
  ...
  FOR(pos,0,Size(t)+1) FOR(len,0,Size(s)+1) tab[pos][len] = 0;
  tab[0][0] = 1;

  FOR(pos,0,Size(t)+1) FOR(len,0,Size(s)+1) {
    tab[pos][len] %= 10000;
    if(pos < Size(t)) tab[pos+1][len] += tab[pos][len];
    if(pos < Size(t) && len < Size(s) && t[pos] == s[len])
      tab[pos+1][len+1] += tab[pos][len];
    }
  
  printf("Case #%d: %04d\n", cnum+1, tab[Size(t)][Size(s)]);
  }
