...
//}
//
//bool isAnyNeighbourLower(int i,int j,int H,int W,vector<vector<int> >& arr)
//{
//	int selfHt = arr[i][j];
//	int lft,rght,top,bottom;
//	lft=rght=top=bottom=100000;
//	if (j > 0)
//		lft=arr[i][j-1];
//
//	if (j<W-1) 
...
//		bottom= arr[i+1][j];
//
//	if (lft < selfHt  ||  rght < selfHt  ||  top  < selfHt  || bottom < selfHt)
//	{
//		return true;	
//	}
//	return false;
//}
//
//void GetAllNeighbours(int i,int j,vector<pair<int,int> >&neighs,int H,int W)
//{
//	neighs.clear();
//	//north
//	if (i>0)
...
//}
//
//void FindSinks(vector<vector<int> >& arr, vector<pair<int,int> >& sinkLocs, int H, int W)
//{
//	for (int i=0;i<H;++i)
//	{
//		for (int j=0;j<W;++j)
//		{
//			if(!isAnyNeighbourLower(i,j,H,W,arr))
//			{
//				sinkLocs.push_back(make_pair(i,j));
//			}
//		}
//	}
//}
//
//pair<int,int> FindBestNbr(int i,int j, int H,int W, vector<vector<int> >& arr)
//{
//	vector<pair<int,int> >neighs;
//	GetAllNeighbours(i,j,neighs,H,W);
//	for (int i=0;i<neighs.size();++i)
//	{
//		for (int j=i+1;j<neighs.size();++j)
//		{
//			int iAlt = arr[neighs[i].first][neighs[i].second];
//			int jAlt = arr[neighs[j].first][neighs[j].second];
//			if (jAlt < iAlt)
//			{
//				pair<int,int> tmp = neighs[i];
//				neighs[i]=neighs[j];
//				neighs[j]=tmp;
//			}
//		}
...
//}
//
//void RemoveAllSinks(vector<pair<int,int> >& neighs,vector<pair<int,int> >& sinkLoc,vector<pair<int,int> >& tempVec)
//{
//	tempVec.clear();
//	for (int i=0;i<neighs.size();++i)
//	{
//		bool flag = false;
//		for (int j=0;j<sinkLoc.size();++j)
//		{
//			if (neighs[i]==sinkLoc[j])
//			{
//				flag = true;
//				break;
//			}
...
//		if (flag == false)
//		{
//			pair<int,int> tPair = neighs[i];
//			tempVec.push_back(tPair);
//		}
//	}
//}
//
...
//		}
//		sinkInd = arr;
//		vector<pair<int,int> > sinkLoc; 
//		FindSinks(arr,sinkLoc,H,W);
//		vector<vector<bool> >  flagMAt;
//		vector<vector<int> >  sinkIndMat;
//		for (int temp2=0;temp2<H;++temp2)
//		{
//			vector<bool> bVec;
//			vector<int> iVec;
...
//				iVec.push_back(-1);
//			}
//			flagMAt.push_back(bVec);
//			sinkIndMat.push_back(iVec);
//		}	
//		vector<pair<int,int> >::iterator it;
//		//all sinks are done..
//		int index = 0;
//		for (it=sinkLoc.begin();it!=sinkLoc.end();++it,++index)
//		{
//			flagMAt[it->first][it->second] = true;
//			sinkIndMat[it->first][it->second] = index;
//		}
//
//		queue<pair<int,int> > myQ;
//		for (int zz=0;zz<sinkLoc.size();++zz)
//		{
//			myQ.push(sinkLoc[zz]);
//			while (!myQ.empty())
//			{
...
//				myQ.pop();
//				vector<pair<int,int> > neighs,tempVec;
//				GetAllNeighbours(curr.first,curr.second,neighs,H,W);
//
//				RemoveAllSinks(neighs,sinkLoc,tempVec);
//				neighs.clear();
//				neighs = tempVec;
//
//				vector<pair<int,int> >::iterator it;
...
//				{
//					pair<int,int> bNbr = FindBestNbr(it->first,it->second,H,W,arr);
//					if (bNbr == curr)
//					{
//						sinkIndMat[it->first][it->second] = sinkIndMat[curr.first][curr.second];
//						flagMAt[it->first][it->second]= flagMAt[curr.first][curr.second];
//						myQ.push(*it);
//					}
//				}
//			}
...
//		}
//		cout << "Case #"<<tcase+1<<":\n";
//		vector<char>  fVec(sinkLoc.size(),'_');
//		char ch = 'a';
//		for (int tk=0;tk<H;++tk)
//		{
//			for (int vp=0;vp<W;++vp)
//			{
//				if (fVec[sinkIndMat[tk][vp]] == '_')
//				{
//					fVec[sinkIndMat[tk][vp]]=ch;
//					cout << ch << " "; 
//					ch++;
//				}
//				else
//				{
//					cout << fVec[sinkIndMat[tk][vp]] << " ";
//				}
//			}
//			cout << "\n";
//		}
...
//}
//
//vector<vector<LONG64> > countMat;
//
//void PruneText(string& textStr,string& searchStr)
//{
//	string str;
//	for (int i=0;i<textStr.size();++i)
//	{
//		if (searchStr.find_first_of(textStr[i]) != string::npos)
//		{
//			str += textStr[i]; 
//		}
//	}
...
//}
//
//LONG64 GetCount(const char* text, const char*search, int i, int j)
//{
//	if (countMat[i][j]!=-1)
//		return countMat[i][j];
//
...
//	else
//	{
//		LONG64 k1 = GetCount(text,search,i+1,pch-text+1);
//		LONG64 k2 = GetCount(text,search,i,pch-text+1);
//		countMat[i][j] = ((k1%10000) + (k2%10000))%10000;
//		return countMat[i][j];
//	}
//}
//
//void GetLast4Digits(int count,char *tmp)
//{
//	int i=3;
//	while(count != 0)
//	{
...
//void QualRound3()
//{
//	string searchStr("welcome to code jam");
...
//	{
//		getline(cin,textStr);
//		PruneText(textStr,searchStr);
//		countMat.clear();
//		for (int t=0;t<=searchStr.size();++t)
//		{
//			vector<LONG64> tmp(textStr.size()+1,-1);
//			countMat.push_back(tmp);
//		}
//		int count = GetCount(textStr.c_str(),searchStr.c_str(),0,0);
//		char tmp[] = {'0','0','0','0','\0'};
//		GetLast4Digits(count%10000,tmp);
//		cout << "Case #"<<tCase+1<<": "<<tmp<<"\n";
//	}
//}
//
//int main()
...
char WelcomeString[] = "welcome to code jam";
int indx, stringlen;

int NoOfSubstring(char A[], int inputindex, int seqindex);
int F[500][500];
int Countchar(char A[], int inputindex, char searchchar);
int main()
{
	char inputstring[1000];
	unsigned int testcases, z, j, k;
	int count = 0;
	scanf("%d", &testcases);
	//printf("%d\n", testcases);
	stringlen = strlen(WelcomeString);
	getchar();
	//scanf("%s", inputstring);
	z = 1;
	while(z <= testcases)
	{
		...
		getchar();
		//printf("z:%d\n", z);
		count = NoOfSubstring(inputstring, 0, 0);
		//count = count % 10000;
		printf("Case #%d: %04d\n", z, count);
		count = 0;
		z = z + 1;
...
int Countchar(char A[], int inputindex, char searchchar)
{
	//printf("A:%s, searchchar: %c", A, searchchar);
	char *p = A + inputindex;
	int count = 0;
	while(1)
	{
		p = strchr(p, searchchar);
		if(p)
		{
			//printf("p: %s\n", p);
			count++;
...
int NoOfSubstring(char A[], int inputindex, int seqindex)
{
	if(WelcomeString[seqindex] == '\0')return 1;
	if(A[inputindex] == '\0')return 0;
	/*if( WelcomeString[seqindex + 1] == '\0')
	{
	F[inputindex][seqindex] = Countchar(A, inputindex, WelcomeString[seqindex]);
	return F[inputindex][seqindex];
	}*/
	if(F[inputindex][seqindex] >= 0)return F[inputindex][seqindex];

	if(A[inputindex] == WelcomeString[seqindex])
	{	
		F[inputindex + 1][seqindex + 1] = NoOfSubstring(A, inputindex + 1, seqindex + 1);	
		F[inputindex + 1][seqindex] = NoOfSubstring(A, inputindex + 1, seqindex);
		return (F[inputindex + 1][seqindex + 1] % 10000 + F[inputindex + 1][seqindex] % 10000) % 10000;
	}
	else
	{
		F[inputindex + 1][seqindex]  = NoOfSubstring(A, inputindex + 1, seqindex);
		return F[inputindex + 1][seqindex];
	}
}