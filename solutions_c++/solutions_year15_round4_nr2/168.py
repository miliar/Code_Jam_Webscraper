...
bool check(double t, double V, double VX) {
  REP(i,n) mv[i] = s[i].f(t);
  {
    double vx = 0;
    double sum = 0;
    REP(i,n) {
      double v = min(mv[i], (VX-vx)/s[i].x);
      vx += v * s[i].x;
      sum += v;
    }
    if (myless(vx, VX) || myless(V,sum)) {
      return 0;
    }
  }
  {
