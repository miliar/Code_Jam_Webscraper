...
#define maxERR 0.000000005

typedef long long ll;
typedef pair<long,long>pos;
typedef pair<long,pos>weightedPos;
...
int main(void){
	...
	for(long T=0;T<test_cases;T++){
        ...
        for(long i=0;i<N;i++)
            cin>>R[i]>>C[i];
        if(N==1){
            if(fabs(C[0]-X)>maxERR)
                cout<<"IMPOSSIBLE"<<endl;
            else
                cout<<V/R[0]<<endl;
        }
        else{
            if(C[0]>maxERR+X&&C[1]>maxERR+X)
                cout<<"IMPOSSIBLE"<<endl;
            else if(C[0]+maxERR<X&&C[1]+maxERR<X)
                cout<<"IMPOSSIBLE"<<endl;
            else {
                double diff1=fabs(X-C[0]);
                double diff2=fabs(X-C[1]);
                if(fabs(diff1)<maxERR&&diff2<maxERR)
                    cout<<V/(R[0]+R[1])<<endl;
                else if(diff1<maxERR)
                    cout<<V/R[0]<<endl;
                else if(diff2<maxERR)
                    cout<<V/R[1]<<endl;
                else {
                    double V1=(diff2*V)/(diff1+diff2);
                    double V2=(diff1*V)/(diff1+diff2);
                    double T1=V1/R[0];
                    double T2=V2/R[1];
                    cout<<max(T1,T2)<<endl;
                }
