...
void Runner::Run()
{
	...
	for (map<double, double>::iterator iter = Map.begin(); iter != Map.end(); ++iter)
	{
		A.push_back(MP(iter->second, iter->first));
	}
	...
	if (N == 1)
	{
		if (X == A[0].second)
		{
			mOut << "Case #" << mCase << ": " << fixed << setprecision(9) << (V / A[0].first) << endl;
		}
		else
		{
	...
	double V1 = V *(A[0].second - X) / (A[0].second - A[1].second);
	double V0 = V - V1;
	if (V1+eps < 0. || V0+eps < 0.)
	{
		mOut << "Case #" << mCase << ": " << "IMPOSSIBLE" << endl;
		return;
	}

	double T1 = V1 / A[1].first;
	double T0 = V0 / A[0].first;
	res = max(T1, T0);
