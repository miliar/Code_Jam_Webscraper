...
struct Sol {
  ...
  void Test() {
    ...
    if (maxc + kEps < need_y || minc - kEps > need_y) {
      cout<<"IMPOSSIBLE\n";
      //cout<<minc<<" "<<maxc<<" "<<need_y<<"\n";
      return;
    }
    if (prt) {
      debug(Point(x, y));
    ...
    Line l{{0, 0}, {x, y}};
    LD res = 1e9;
    for (int i = 1; i < SZ(pol.pts) - 1; i++) {
      Line k{pol.pts[i], pol.pts[(i + 1) % SZ(pol.pts)]};
      
      Point heh = Utils::InterLineLine(k, l);
      if (Utils::PtBelongToSeg(k, heh)) {
        mini(res, x / heh.x);//<<"\n";
        //return;
      }
      
//       vector<Point> in = Utils::InterSegs(k, l);
...
//       }
    ...
    Line beg[2] = {{pol.pts[0], pol.pts[1]}, {pol.pts[0], pol.pts.back()}};
    REP (tr, 2) {
      if (Utils::AreParallel(beg[tr], l)) {
        //debug2(x, beg[tr][1].x);
        mini(res, x / beg[tr][1].x);
        //cout<<x / beg[tr][1].x<<"\n";
        //return;
      }
    }
    assert(maxc + kEps > need_y && minc < need_y + kEps);
//     if (res < 1e9 / 2) {
//       if (!(maxc + kEps > need_y && minc < need_y + kEps)) {
//         cout<<"ZONK\n";
//       }
      cout<<res<<"\n";
//     } else {
//       assert(maxc < need_y || minc > need_y);
//       cout<<"IMPOSSIBLE\n";
//     }
