...
class Main{
	...
	void run(){
		...
		for(int q:range(T)){
			...
			if(N==1){
				if(X == ps[0].second){
					cout << make_tuple("Case","#"+to_string(q+1)+":",V/ps[0].first)<<endl;
				}else{
					cout << make_tuple("Case","#"+to_string(q+1)+":","IMPOSSIBLE")<<endl;
				}
			}else if(N==2){
				if(X == ps[0].second && X==ps[1].second){
					cout << make_tuple("Case","#"+to_string(q+1)+":",V/(ps[0].first+ps[1].first))<<endl;
				}else if(X==ps[0].second){
					cout << make_tuple("Case","#"+to_string(q+1)+":",V/(ps[0].first))<<endl;
				}else if(X==ps[1].second){
					cout << make_tuple("Case","#"+to_string(q+1)+":",V/(ps[1].first))<<endl;
				}else if(min(ps[0].second,ps[1].second) < X && X < max(ps[0].second,ps[1].second)){
					//一意
					cout << make_tuple("Case","#"+to_string(q+1)+":",
						max(V*(X-ps[0].second)/(ps[1].first*(ps[1].second-ps[0].second)),
							V*(X-ps[1].second)/(ps[0].first*(ps[0].second-ps[1].second))))<<endl;
				}else{
					cout << make_tuple("Case","#"+to_string(q+1)+":","IMPOSSIBLE")<<endl;
				}
			}else{
