/** Micro Mezz Macro Flation -- Overheated Economy ., ll Update: Nov. 7th 2014 **/ //{
...
LL ll_ans; int Case; template<class T> inline void OT(const T &x){
    //printf("Case #%d: ", ++Case);
    //printf("%lld\n", x);
    //printf("%.9f\n", x);
    printf("%d\n", x);
	...
	Rush{
		...
		if (n == 1) {
			...
			if (b[0] > B || b[1] < B) {
				...
				DB b0 = B - b[0], b1 = b[1] - B;
				DB tt = (sgn(b0) ? 0 : a[0]) + (sgn(b1) ? 0 : a[1]);

				if (tt > 0) printf("%.9f\n", A / tt);
				else printf("%.9f\n", A / (min(a[0] / b1, a[1] / b0) * (b0 + b1)));
			}
		}
	}
}
