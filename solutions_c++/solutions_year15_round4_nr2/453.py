...
inline int dcmp(const long double x, const long double y){ if(x-y>eps)return 1;else return x-y<-eps; }
...
int main(){
	...
	for(int _T=1;_T<=T;_T++){
		printf("Case #%d: ", _T);
		double buf_v, buf_x;
		scanf("%d%lf%lf", &n, &buf_v, &buf_x);
		v=buf_v; x=buf_x;
		int flag=1;
		if(n<=2){
			for(int i=0; i<n; i++){
				double buf_r, buf_c;
				scanf("%lf%lf", &buf_r, &buf_c);
				r[i]=buf_r; c[i]=buf_c;
			}
			if(n==1){
				c[1]=c[0];
			...
			if(!dcmp(c[0], c[1])){
				...
				long double v0=(v*x-v*c[1])/(c[0]-c[1]);
				long double v1=v-v0;
				if(v0>-eps && v1>-eps){
