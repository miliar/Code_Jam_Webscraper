...
struct problem{
  ...
  int solve(){
    ...
    double ans = 0;
    sort(all(v));
    if(v[0].temp>temp)return pr("IMPOSSIBLE"),0;
    if(v.back().temp<temp)return pr("IMPOSSIBLE"),0;
    if(l<r){
      ff(i,0,n){
        pipe p = v[i];
        if(p.temp<=temp)ans+=p.vol;
        else{
          double tt = hq(p);
          if(tt>l){
            ans+=l/tt*p.vol;
