...
DB v , x;
int n;
bool check(DB t , DB & Maxc , DB & Minc){
    DB vv = v;
    Maxc = 0, Minc = 0;
    for (int i = 0 ; i < n ; ++i){
        if (dblcmp(A[i].r * t - vv) <= 0){
            Minc += A[i].r * t * A[i].c;
            vv -= A[i].r * t;
        }
        else{
    ...
    Maxc /= v;
    if (dblcmp(Maxc - x) < 0 || dblcmp(x - Minc) < 0){
        return false;
    }
    return true;
}
void solve(){
    ...
    sort(A , A + n);
    DB low = 0 , high = 1e15 , mid;
    DB ans = high;
    DB epss = 1e-14;
    Maxc = -1 , Minc = 1e15;
    DB nowMaxc = -1, nowMinc = 1e15;
    do{
        nowMaxc = Maxc , nowMinc = Minc;
        mid = (low + high) / 2;
        if (check(mid , Maxc , Minc)){
            checkMin(ans , mid);
            high = mid - epss;
        }
        else low = mid + epss;
    }while(dblcmp(nowMaxc - Maxc) || dblcmp(nowMinc - Minc));
    cerr << Case <<  ' ' << ans << endl;
