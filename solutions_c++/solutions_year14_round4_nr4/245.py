...
const int N = 4, M = 8, L = 50;
char str[M][L]; int n, m; int F[N+1];
VI P[4]; int ans, cnt;

namespace ACM{
    const int N = 100009, Z = 26;
    int trans[N][Z], fail[N],  Q[N], cz, op, tt;

    int new_node(){
        RST(trans[tt]), fail[tt] = 0;
        return tt++;
    }
...
void stat(){
    int res = 0; REP(i, n) res += ACM::Init(P[i]);
    if (res > ans) ans =res, cnt = 1;
    else if (res == ans) ++cnt;
}

void dfs(int k = 0, int nn = 0){
...
int main(){
    ...
    F[0] = 1; FOR(i, 1, N+1) F[i] = F[i-1] * i;

    Rush{
        RD(m, n); REP(i, m) RS(str[i]);
        //cout << m << " " << n << endl;
        //assert(n <= m);
