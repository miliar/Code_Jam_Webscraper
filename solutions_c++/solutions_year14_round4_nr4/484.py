...
void gao(){
	for (int i=0; i<1<<m; ++i) node[i] = cal(i);
	mem(dp, 0);
	mem(way, 0);
	for (int i=1; i<1<<m; ++i){
		for (int j=1; j<=n; ++j){
			if (j == 1){
				dp[i][j] = node[i], way[i][j] = 1;
			}else {
				for (int k=i; k!=0; k=(k-1)&i){
					if (k == i) continue;
					int t = dp[k][j-1] + node[i^k];
					if (t > dp[i][j]){
						dp[i][j] = t;
						way[i][j] = way[k][j-1];
					}else if (t == dp[i][j]){
	...
	cout << dp[(1<<m)-1][n] << " " << way[(1<<m)-1][n] << endl;
}
