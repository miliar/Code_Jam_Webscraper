...
void dfs(int now)
{
	...
	{
		int tot = 0;
		REP(i,n) if(SIZE(input[i]) > 0)
		{
			set<string> haha;
			REP(j,SIZE(input[i]))
			{
				string s = "";
				REP(k,SIZE(input[i][j]))
				{
					haha.insert(s);
					s += input[i][j][k];
				}
		...
		if (tot > maks.fi) maks = mp(tot,1);
		else if (tot == maks.fi) maks.se = (maks.se + 1) % MOD;
		return;
	}
	REP(i,n)
	{
