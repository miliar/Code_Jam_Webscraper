...
int ret = 0, ways = 0;
void dfs(vs& a, vi& assign, int ng, int upto){
  if(sz(assign) == sz(a)){
    int cost = 0;
    vector< set<string> > st(ng);
    fi(sz(a))
      fj(sz(a[i])+1)
        st[assign[i]].insert(a[i].substr(0, j));
    fi(ng)
      cost += sz(st[i]);
    if(cost > ret){
      ret = cost;
    ...
    assign.pb(i);
    dfs(a, assign, ng, upto);
    assign.pop_back();
  }
/*
  fi(min(upto+1, ng)){
    assign.pb(i);
    dfs(a, assign, ng, max(i+1, upto));
    assign.pop_back();
  }
*/
}
