...
int check(){
	...
	for(int i=0;i<n;i++)vst[g[i]]++;
	for(int i=0;i<m;i++)if(!vst[i])return 0;
	return 1;
}
...
int count(){
	int ret=0;
	for(int k=0;k<m;k++){
		memset(rot,0,sizeof(rot));num=0;
		for(int i=0;i<n;i++)if(g[i]==k)insert(a[i]);
		ret+=num+1;
	}
	return ret;
}
...
void core(const int &k){
	if(k==n){
		if(check())
			update();
		return;
	}
	for(g[k]=0;g[k]<m;g[k]++)core(k+1);
}
