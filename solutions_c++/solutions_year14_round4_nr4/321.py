...
bool next_set(vector<int> &v, int n){
...
int triecnt(const vector<string> &S){
    set<string> s;
    rep(i, sz(S)){
        string t = "";
        rep(j, sz(S[i])){
...
inline void solve(){
    int m, n; cin >> m >> n;
    vector<string> in(m); rep(i, m) cin >> in[i];
    vector<int> a(m);
    int res = 0;
    do{
        vector<int> used(n, 100);
        rep(i, m) mineq(used[a[i]], i+1);
        rep(i, n-1) if(used[i] > used[i+1]) continue;
        if(*max_element(all(used)) == 100) continue;
        vector<vector<string>> S(n);
        rep(i, m) S[a[i]].eb(in[i]);
        int tmp = 0;
        rep(i, n) tmp += triecnt(S[i]);
        res = max(res, tmp);
    }while(next_set(a, n));
    int res2 = 0;
    do{
