...
void dfs(int depth, vector<set<string> > servers) {
  if(depth==M) {
    int count = 0;
    REP(i, N) {
      if(servers[i].size()==0) return;
      count += servers[i].size();
    }
    if(count > ans) {
      ans = count;
      ways = 1;
  ...
  vector<set<string> > s = servers;
  REP(i, N) {
    for(auto v: pre[depth]) {
      s[i].insert(v);
    }
    dfs(depth+1, s);
    s[i] = servers[i];
