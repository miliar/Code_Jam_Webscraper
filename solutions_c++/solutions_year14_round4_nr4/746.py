...
void palabrachange(char buff[LEN+1])
{   
  int t=root;
      ...
      for( char *c = buff; *c; ++c )//recorre todas las letras de la palabra
      {
         if( dict[t].p[*c-'A'] == -1 )//si no existe un nodo empezando en "t", que tenga como adyacente a la letra "*c-a" entonces
            dict[t].p[*c-'A'] = todosnodos();//crea nuevo nodo
...
int  go(vector <int> indi) {
   ...
   root = todosnodos();//tendra la raiz
  
  for(int i=0;i<indi.size();i++) {
    palabrachange(arr[indi[i]]);
  }
  return indice_nodos;
}
...
int main(){
  ...
  for(int t=1;t<=cases;t++){
    ...
    if(NN == 1){
      vector <int> T;
      for(int i=0;i<MM;i++)T.push_back(i);
       cout<<"Case #"<<t<<": ";
       cout<<go(T)<<" "<<1<<endl;
       continue;
    }
