#include <cstdio>
#include <vector>
#include <iostream>
#include <string>
#include <set>
#include <map>
#include <queue>
#include <algorithm>
#include <utility>
#include <stdlib.h>
#include <time.h>       
using namespace std;
unsigned const int PRIME = 100000009;
unsigned int POWER = 47;
char vzor[1002];
char kmp[1002];
char text[10002];
int dynamika[1002];
int Ndynamika[1002];
int i,j,k,n,m,K,zaciatok;
unsigned int hash2(char * s, int dlzka){
    unsigned int h = 0;
    while(dlzka--){
        h*=POWER;
        h+=(s[dlzka]);
    }
    return h;
}
unsigned int hash3(char * s, int dlzka){
    unsigned int h = 0;
    while(dlzka--){
        h*=POWER;
        h+=(s[0]);
        s=s+1001;
    }
    return h;
}
int max(int a, int b){
    if(a>=b) return a;
    return b;
}
int min(int a, int b){
    if(a<=b) return a;
    return b;
}

vector<int> cisla1, cisla2;

int CONST = -1000000001;
double C = 5, F = 5, X = 5;
int N, L;
		vector<double> Naomi, Ken;

bool otestuj(int kde){
	if(kde == N)
		return true;
	for(int i = 0; i < N - kde; i++){
		if(Naomi[N - i - 1] < Ken[N - kde - 1 - i])
			return false;
	}
	return true;
}

vector<vector<int> > hrany;
vector<int> pocet, binary;
int vys = 1500;

void poc(int vrchol, int z){
	for(int i = 0; i < hrany[vrchol].size(); i++){
		if(hrany[vrchol][i]!=z)
			poc(hrany[vrchol][i], vrchol);
	}

	int spol = 1;
	for(int i = 0; i < hrany[vrchol].size(); i++){
		if(hrany[vrchol][i]!=z)
			spol+=pocet[hrany[vrchol][i]];
	}

	pocet[vrchol] = spol;
}

void bin(int vrchol, int z){
	for(int i = 0; i < hrany[vrchol].size(); i++){
		if(hrany[vrchol][i]!=z)
			bin(hrany[vrchol][i], vrchol);
	}

	int max1 = 0;
	int max2 = 0;
	int sucet = 0;
	for(int i = 0; i < hrany[vrchol].size(); i++){
		if(hrany[vrchol][i]!=z){
			sucet+=pocet[hrany[vrchol][i]];
			int rozdiel = pocet[hrany[vrchol][i]] - binary[hrany[vrchol][i]];
			if(rozdiel > max1){
				max2 = max1;
				max1 = rozdiel;
			}
			else if(rozdiel > max2){
				max2 = rozdiel;
			}
		}
	}

	int hran = hrany[vrchol].size();
	if(z != -1) hran--;

	if(hran == 0)
		binary[vrchol] = 0;
	else if(hran == 1)
		binary[vrchol] = sucet;
	else if(hran >= 2)
		binary[vrchol] = sucet - max1 - max2;
}

void skus(int vrchol){
	poc(vrchol, -1);
	bin(vrchol, -1);
	vys = min(vys, binary[vrchol]);
}

vector<int> co;
int x;

int binar(int po, int od){
	//cout<<po<<" "<<od<<endl;
	if(abs(po-od) == 1)
		return (co.size()-po)/2 + (co.size()-po)%2 + po;
	int s = od + po; s/=2;
	if(s==od)
		s++;
	int preskocim = s;
	int dlzka = co.size() - preskocim;
	bool mozem = true;
	for(int i = dlzka - 1; i >= 0; i--){
		if(co[i] + co[dlzka-i-1] > x)
		{
			mozem = false;
			break;
		}
	}
	if(mozem == false)
		return binar(po,s);
	else
		return binar(s,od);
}

vector<string> slova;

vector<int> aktua;

int totalMax = 0, poccet = 0;

int skusame(){
	vector<set<string> > vsetky;
	vsetky.resize(n);
	for(int i = 0; i < m; i++){
		string nov = "";
		for(int j = 0; j < slova[i].size(); j++){
			nov+=slova[i][j];
			vsetky[aktua[i]].insert(nov);
		}
	}

	int maxx = 0;

	for(int i = 0; i < n; i++){
		if(vsetky[i].size() == 0)
			return 0;
			maxx += vsetky[i].size();
	}

	if(maxx == totalMax)
		poccet++;
	else if(maxx > totalMax){
		totalMax = maxx;
		poccet=1;
	}
	return 0;
}

int test(int kolko){
	if(kolko == 0){
		skusame();
		return 0;
	
	}

	for(int i = 0; i < n; i++){
		aktua[kolko-1] = i;
		test(kolko-1);
	}
	return 0;

}

int main(void)
{
	int a;
	scanf("%d",&a);
	int pocetP = 0, pocetD = 0, pocetS = 0, pocet = 0;
	for(int t = 1; t <= a; t++){
		/*co.clear();
		int n;
		scanf("%d%d",&n,&x);
		int c;
		while(n--){
			scanf("%d",&c);
			co.push_back(c);
		}
		vector<int> co2 = co;
		sort(co2.begin(), co2.end());
		map<int,int> obraz;
		for(int i = 0; i < co2.size(); i++){
			obraz[co2[i]]=i;
		}
		for(int i = 0; i < co2.size(); i++){
			co[i] = obraz[co[i]];
		}*/

		scanf("%d%d",&m, &n);
		string sl;
		slova.clear();
		aktua.clear(); aktua.resize(m,0);
		int mm = m;
		while(m--){
			cin>>sl;
			slova.push_back(sl);
		}
		m = mm;
		totalMax = 0; poccet = 0;
		test(m);

		cout<<"Case #"<<t<<": "<<totalMax + n<<" "<<poccet%1000000007<<endl;
	}
	 return 0;
}