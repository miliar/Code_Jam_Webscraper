...
void solve() { db(1);
    ...
    dp[0][0] = 1;
    dp2[0][0] = 1;
    rep (j, (1<<M)) ca[j] = cost(j);
    cerr<<"XX " << ca[9]<<" "<<ca[6] << endl;

    rep (i, N) {
        rep (j, (1<<M)) if (dp2[i][j]) {
            rep (k, (1<<M)) if ((k&j) == j && k != j) {
                int nsum = dp2[i][j] + ca[k^j];

                if (dp2[i+1][k] == 0 || dp2[i+1][k] < nsum) {
                    dp2[i+1][k] = nsum;
                    dp[i+1][k] = 0;
                }
                if (dp2[i+1][k] == nsum) {
