...
int lcp(const string & s, const string & t) {
    int r = 0;
    while( r < s.length() && r < t.length() && s[r] == t[r] ) ++r;
    return r;
}
int get(vector<string>& t) {
    int r = 1;
    sort(all(t));
    forn(i, t.size() ) {
        r += t[i].length();
        if( i ) r -= lcp(t[i], t[i-1]);
    } return r;
...
void f(int i) {
    if( i == m ) {
        int s = 0; bool good = true;
        forn(j, n) {
            vector<string> t;
            forn(z, m) if( dest[z] == j ) t.pb(v[z]);
            s += get(t);
            if( t.empty() ) good =false;
        }
        if( !good ) return ;
        if( s == a ) {
            r += 1;
