...
void solveTest(istream& in, ostream& out) {
    string n = next<string>(in);
    auto din = make_vec<int64_t>(n.length() + 1, 10, 2);
    din.fill(-1LL);
    din(0, 0, 1) = 0;
    forn (int, i, n.length()) {
        forn (int, d, 10) {
            foran (int, d1, d, 10) {
                if (din(i, d, 0) != -1) {
                    rmx(din(i + 1, d1, 0), 10 * din(i, d, 0) + d1);
                }
                if (din(i, d, 1) != -1 && d1 <= n[i] - '0') {
                    rmx(din(i + 1, d1, d1 == n[i] - '0' ? 1 : 0), 10 * din(i, d, 1) + d1);
                }
            }
        }
    }
    int64_t ans = -1;
    forn (int, d, 10) {
        rmx(ans, din(n.length(), d, 0), din(n.length(), d, 1));
    }
    out << ans << "\n";
}
