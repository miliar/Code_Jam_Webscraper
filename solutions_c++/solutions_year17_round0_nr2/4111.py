...
int main()
{
    ...
    rep(ppll, 1, T)
    {
       ...
       if( N<=9 )
       {
          cout << "Case #" << ppll << ": " << N << endl;
       }
       else
       {
          string stop = con_ll_to_str( N ), ress = "";
          rep(i, 1, (int)stop.length()-1)
          {
             ress += "9";
          }

          anss = con_str_to_ll( ress );

          ll star = 0, prev = 1;
          bool flag = 0;
          while( star<(int)stop.length() )
          {
              sttemp = stop;
              rep(i, prev, 9)
              {
                 char ch = i + '0';
                 sttemp[star] = ch;
                 rep(j, star+1, (int)stop.length()-1)
                 {
                    sttemp[j] = ch;
                 }
                 if( con_str_to_ll(sttemp)<=N )
                 {
                     flag = 1;
                     prev = i;
                 }
          ...
          if( flag )
          {
              anss = max(anss, con_str_to_ll( stop ) );
          }
