...
ll rec(int pos,int last,int flag)
{
    if(pos==0) {
        return 1;
    }
    ...
    int end = flag ? 9 : num[pos];
    ll ans = 0;

    for(int i = last;i<=end;i++){
        ans += rec(pos - 1,i,flag || i < end);
    }
    if(flag) ret = ans;
    return ans;
...
void path(int pos,int last,int flag,ll k)
{
    if(pos==0) {
        return ;
    }
...
void solve(ll a){
    ...
    zeroFlag = 1;
    return path(len,0,0,rec(len,0,0));
