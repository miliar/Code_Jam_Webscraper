...
bool ok(ll crnt, ll limit, int len)
{
	int d = crnt % 10;
	if (d == 0)
	{
...
ll rec(ll crnt, ll limit, int len)
{
	if (len == 0)
	{
		return crnt;
	...
	for (int d = 9; d >= 1; d--)
	{
		if (ok(crnt * 10 + d, limit, len - 1))
		{
			return rec(crnt * 10 + d, limit, len - 1);
		}
	}
	assert(false);
}
...
int main()
{
	...
	ji(t)
	{
		...
		printf("Case #%d: ", i+1);
		cerr << n << endl;
		for (int len = dlen(n); len >= 1; len--)
		{
			if (ok(0, n, len))
			{
				cout << rec(0, n, len) << endl;
				break;
			}
		}
	}	
