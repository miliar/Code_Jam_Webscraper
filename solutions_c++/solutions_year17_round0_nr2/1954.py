...
vector<ll>digits;
ll mx;
void findTidyNumber(ll idx,ll lastDigit,ll isSmall, ll number)
{
    ll i;
    if(isSmall==1||idx<0)
    {
    ...
    for(i=lastDigit;i<=digits[idx];i++)
    {
        findTidyNumber(idx-1,i,i<digits[idx],number*10+i);
    }
    findTidyNumber(idx-1,lastDigit,1,number);
}
int main()
{
   ...
   for(x=1;x<=t;x++)
   {
       ...
       findTidyNumber(digits.size()-1,1,0,0);
       pf("Case #%lld: %lld\n",x,mx);
   }
