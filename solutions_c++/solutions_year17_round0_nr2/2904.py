...
string s;
string target;
bool check(i64 pos, i64 full)
{
  if(pos == sz(target)) return true;
  if(!full){
    s[pos] = '9';
    return check(pos + 1, full);
  }
  for(i64 i = 9; 0 <= i; --i){
    s[pos] = '0' + i;
    if(0 < pos && s[pos] < s[pos - 1]) continue;
    if(target[pos] < s[pos]) continue;
    if(!check(pos + 1, full && (s[pos] == target[pos]))) continue;
    return true;
  }
  return false;
}
...
int main()
{
  ...
  rep(test_id, test_n){
    cin >> target;
    s = string(sz(target), '9');
    check(0, true);
    while(s[0] == '0'){
      s = s.substr(1, sz(s));
    }
    cout << "Case #" << test_id + 1 << ": " << s << endl;
  }
}
