...
int findpos(int x,int left,int right)
{
    while(right-left>1)
    {
        ...
        if(a[median]>x)
            return findpos(x,left,median-1);
        else
            return findpos(x,median,right);
    };
    if(x>=a[right])
        return right;
    else
