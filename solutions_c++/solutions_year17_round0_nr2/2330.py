...
bool Check(int pos, int digit, int less) {
    if (pos == int(s.size())) {
        return true;
    }

    int& ans = d[pos][digit][less];
    if (ans != -1) return (ans == 1);

    ans = 0;
    const int maxDigit = (less == 0) ? (s[pos] - '0') : 9;
    for (int d = maxDigit; d >= digit; --d) {
        const int newLess = less || (d != s[pos] - '0');
        if (Check(pos + 1, d, newLess)) {
            ans = 1;
            break;
        }
    }
...
void FindAns(int pos, int digit, int less, int64& answer) {
    if (pos == int(s.size())) {
        return;
    }

    const int ans = Check(pos, digit, less);
    assert(ans == 1);
    ...
    for (int d = maxDigit; d >= digit; --d) {
        ...
        if (Check(pos + 1, d, newLess)) {
            answer = answer * 10 + d;
            return FindAns(pos + 1, d, newLess, answer);
        }
    }
}

bool IsTidy(int64 x) {
    const string s = toStr(x);
    forn(i, s.size()) {
        if (i > 0 && s[i] < s[i - 1]) {
            return false;
        }
...
void solve(bool skipThisTest) {
    ...
    if (N <= 10000000) {
        const int64 stupidAnswer = stupid();
