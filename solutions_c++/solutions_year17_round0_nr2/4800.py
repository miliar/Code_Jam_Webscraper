...
void rec(i64 val , int l , bool ready )
{
    if( c1 == l )
    {
        if( val <= num )
    ...
    if(ready)
    {
        for( int A = 1 ; A <= st ; A++ )
        {
            rec(val*10 + (i64)A , l + 1 , 0 );
        }
    }
    else
    {
        for( int A = (val%10) ; A <= 9  ; A++ )
        {
            rec(val*10 + (i64)A , l + 1 , 0 );
        }
    }
...
int main()
{
    ...
    ///I_LOVE_YOU_TANYA_ROMANOVA <3
    //freopen("U:/in.txt","r",stdin);
    //freopen("U:/out2.txt","w",stdout);
