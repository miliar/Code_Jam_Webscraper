...
char ss[20];int N;
ll dp(int i=0,int s=1,bool c=1,int p=0)
{
    if(i==N) return c;
    if(~mem[i][s][c][p])return mem[i][s][c][p];
    ll ans=0;int x;
    if(s)
    {
        for(x=0;x<ss[i]-'0';x++)ans+=dp(i+1,0,c && x>=p,x);
        ans+=dp(i+1,1,c&& x>=p,x);
    }else for(x=0;x<10;x++)ans+=dp(i+1,0,c&& x>=p,x);

    return mem[i][s][c][p]=ans;
}
int i,j,k,m,n,a,b,c,tc,o;
ll ans,kk,lo,hi,mid;
main()
{
    input;output;
