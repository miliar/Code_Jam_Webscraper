...
string SolveSmall(const string& s)
{
	unsigned __int64 d = _strtoui64(s.c_str(), NULL, 10);
	char buf[100];
	...
	for (unsigned __int64 i = d; i >= 0; --i)
	{
		_ui64toa_s(i, buf, sizeof(buf), 10);
		int l1 = (int)strlen(buf);
		bool bad = false;
		FOR(j, 1, l1)
		{
			if (buf[j] < buf[j - 1])
...
string SolveLarge(const string& s)
{
	string res;
