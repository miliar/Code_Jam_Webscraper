...
inline int findInvPos(const string & s) {
    _forn(i, 0, clen(s) - 1) {
        int val = (int) (s [i] - '0');
        int nextVal = (int) (s [i + 1] - '0');
        if (val > nextVal) {
            return i;
        }
    }
...
string solve(string s) {
    int invPos = findInvPos(s);
    while (invPos >= 0) {
        int invVal = (int) (s [invPos] - '0');
        -- invVal;
        s [invPos] = (char) (invVal + '0');
        _forn(i, invPos + 1, clen(s)) {
            s [i] = '9';
        }
        if ('0' == s [0]) {
            s = s.substr (1);
