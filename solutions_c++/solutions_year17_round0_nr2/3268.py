...
template<class T> string i2s(T x) {ostringstream o; if (floor(x) == x) o << (LL)x; else o << x; return o.str();}
...
bool ok(LL x) {
	string s = i2s(x);
	FOR(i, 1, s.S) if (s[i] < s[i-1]) return false;
	return true;
}
...
LL find(LL x) {
	...
	for (int i = s.S-1; ;i--) {
		if (s[i] < s[i-1]) {
			string r = i2s(atoll(s.substr(0, i).c_str()) - 1) + string(s.S - i, '9');
			return find(atoll(r.c_str()));
		}
	}
	return -1;
}
