...
int ans_me(int id, int ls, int big)
{
    if(id == n) return true;
    int &ret = dp[id][ls][big];
    if(ret != -1) return ret;
    ret = 0;
    int lm = big ? in[id] - '0' : 9;
    for(int i = 0; i <= lm; i++) {
        if(ls <= i) ret |= ans_me(id + 1, i, big && i == lm);
    }
    return ret;
}


void path(int id, int ls, int big)
{
    if(id == n) return ;
    int lm = big ? in[id] - '0' : 9;
    for(int i = lm; i >= 0; i--) {
        if(ls <= i && ans_me(id + 1, i, big && i == lm)) {
            Ans = Ans * 10 + i;
            path(id + 1, i, big && i == lm);
            return ;
        }
    }
}
