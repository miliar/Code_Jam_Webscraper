...
string dp2[25][2][10];
bool visit[25][2][10];
int solve(int pos,int is_small, int prev)
{
    if(pos==NUM.size())
    {
        return 1;
    }
    int &ret=dp[pos][is_small][prev];
    if(ret!=-1)return ret;
    int limit = is_small ? 9:NUM[pos]-'0';
    int cnt=0;
    for(int i=prev;i<=limit;i++)
    {
        cnt|=solve(pos+1, is_small|(i<NUM[pos]-'0'), i);
    }
    return ret=cnt;
}

string solve2(int pos,int is_small, int prev)
{
    if(pos==NUM.size())return "";
    string &ret=dp2[pos][is_small][prev];
    if(visit[pos][is_small][prev])return ret;
    visit[pos][is_small][prev]=true;
    ret="";
    int limit = is_small ? 9 : NUM[pos]-'0';
    for(int i=prev;i<=limit;i++)
    {
        if(solve(pos+1, is_small|(i<NUM[pos]-'0'), i))
        {
            char c = '0'+i;
            ret=max(ret,c+solve2(pos+1, is_small|(i<NUM[pos]-'0'), i));
...
int main()
{
    ...
    for(int t=1;t<=test;t++)
    {
        ...
        mem(visit,false);
        solve(0,0,0);
        string ans = solve2(0,0,0);
        LLD a = atoll(ans.c_str());
        printf("Case #%d: %lld\n",t,a);
    }
