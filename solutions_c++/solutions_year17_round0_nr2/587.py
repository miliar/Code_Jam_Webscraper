...
void solve(int casenum, outboth rout) {
  ...
  reverse(All(digits));
  // cout << db(N) << endl;
  // cout << db(sz(digits)) << endl;
  // cout << "digits: " << show(digits) << endl;

  ans = 0;
  Loop(k, sz(digits)-1) ans = ans*10 + 9;

  ll nondesc = true;
  Loop(k, sz(digits)-1) if (digits[k+1] < digits[k]) nondesc = false;
  if (nondesc) ans = max(ans, N);
  // cout << db(ans) << endl;

  ll prefix = 0;
  for (int k = 0; k < sz(digits); ++k) {
    if (k > 0 && digits[k] < digits[k-1]) break;
    if (digits[k] > (k > 0 ? digits[k-1] : 0)) {
      ll x = 10*prefix + digits[k] - 1;
      for (int j = k+1; j < sz(digits); ++j)
        x = 10*x + 9;
      cout << db(k) << db(prefix) << db(x) << endl;
      ans = max(ans, x);
    }
    prefix = 10*prefix+digits[k];
  }
  assert(ans <= N);
  // cout << ans << endl;
  // cout << N << endl;
...
// clang-format off
// Local variables:
// compile-command: "/opt/clang/bin/clang++ -fsanitize={address,undefined} -fmodules -std=c++11 -Wall -Wextra -g -isysroot /Applications/Xcode.app/Contents/Developer/Platforms/MacOSX.platform/Developer/SDKs/MacOSX10.11.sdk -I/opt/brew/include -L/opt/brew/lib -o B B.cc -lgmp && ./B B.test.in"
