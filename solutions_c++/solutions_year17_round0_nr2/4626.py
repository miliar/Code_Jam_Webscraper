...
ll solve(){
	vi v(find_length(n, 10));
	rep(position, 0, v.size()){
		rep(digit, 0, 10){
			rep(i, position, v.size()) v[i] = digit;
			if(vec_to_num(v, 10) > n){
				rep(i, position, v.size()) v[i] = digit-1;
				break;
			} 
		}
	}
	return vec_to_num(v, 10);
}
