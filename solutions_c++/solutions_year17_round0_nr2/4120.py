...
ll getDigit(ll number, ll pos) {
	return (number % (powll(10, pos+1))) / (powll(10, pos));
}

typedef ll otype;
otype calcFunction(ll number) {
	ll startPos=30,
	//endPos,
	curNum = 0;
	bool isTidy = true;
	for (ll i=18; i>=0; i--) {
		if (getDigit(number, i) > curNum) {
			startPos = i;
			//endPos = i;
			curNum = getDigit(number, i);
		}
		/*if (getDigit(number, i) == curNum) {
			endPos = i;
		}*/
	...
	if(isTidy) return number;
	otype result=0;
	for (ll i=0; i<=18; i++) {
		if(i<startPos) result+=9*powll(10, i);
		if(i==startPos) result+=(getDigit(number, i)-1)*powll(10,i);
		if(i>startPos) result+=getDigit(number, i)*powll(10,i);
	}
	return result;
}
...
int main() {
	...
	fore(test, 1, tests){
		...
		otype result=calcFunction(number);
		outfile << "Case #" << test << ": ";
		cout << "Case #" << test << ": ";
#ifndef IFSTRUCT
		outfile << result << endl;
