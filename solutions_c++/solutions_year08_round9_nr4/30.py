...
void solve_case(int casen)
{
	...
	for(int i = 0; i < n; ++i)
	{
		for(int j = 0; j < m; ++j)
		{
			...
			if(c == 'T')
			{
				forests.push_back(make_pair(i, j));
			}
		}	
	}
	...
	for(int i = 0; i < n; ++i)
	{
		for(int j = 0; j < m; ++j)
		{
			...
			int cur = 0;
			s[cur].push_back(make_pair(i, j));
			sp[i][j][i][j] = 1;
			for(int d = 1; !s[cur].empty(); ++d)
			{
				s[!cur].clear();
				while(!s[cur].empty())
				{
					int x = s[cur].back().first;
					int y = s[cur].back().second;
					s[cur].pop_back();
					...
					for(int o = 0; o < 4; ++o)
					{
						int nx = x + xd[o];
						int ny = y + yd[o];
						if(0 <= nx && 0 <= ny && nx < n && ny < m && tab[nx][ny] != '.' && !sp[i][j][nx][ny])
						{
							sp[i][j][nx][ny] = d;
							s[!cur].push_back(make_pair(nx, ny));
						}
					}
				}
				cur = !cur;
	...
	if(forests.size() > 1)
	{
		...
		int fx = forests[1].first;
		int fy = forests[1].second;
		...
		while(x != fx && y != fy)
		{
			...
			for(int o = 0; o < 4; ++o)
			{
				int nx = x + xd[o];
				int ny = y + yd[o];
				if(0 <= nx && 0 <= ny && nx < n && ny < m && sp[nx][ny][fx][fy] == d)
				{
					x = nx;
					y = ny;
					break;
...
*/
	...
	for(int i = 0; i < n; ++i)
	{
		for(int j = (i == 0); j < m; ++j)
		{
			...
			{
				v = min(v, sp[i][j][forests[f].first][forests[f].second]);
			}
		...
		for(int d = 1; !s[cur].empty(); ++d)
		{
			...
			while(!s[cur].empty())
			{
				...
				for(int o = 0; o < 4; ++o)
				{
					int nx = x + xd[o];
					int ny = y + yd[o];
					if(tab[nx][ny] != '.' && !sp[i][j][nx][ny])
					{
						sp[i][j][nx][ny] = d;
						s[!cur].push(make_pair(nx, ny));
					}
				}
			}
			cur = !cur;
