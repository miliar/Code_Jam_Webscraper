...
__int64 doit( filein& fin )
{
   ...
   //   if ( a == 0 || b == 0 ) w[a][b] = 1;
   //   //trace << (w[a][b]?'.':'X');
   //   //if ( b && w[a][b] && !w[a][b-1] ) trace << a << " " << b-1 << " " << int(a*((1+sqrt(5.))/2)) << endl;
   //   if ( b && !w[a][b] && w[a][b-1] ) trace << a << " " << b-1 << " " << int(a/((1+sqrt(5.))/2)) << endl;
   //   bool ww = w[a][b] ? true : false;
   //   //if ( ww != isWinner(a,b) ) 
   //   //   trace << "error " << a << " " << b << endl;
   //   if ( w[a][b] ) continue;
   //   for ( int a2 = a+b; a2 < 1000; a2 += b ) w[a2][b] = 1;
   //   for ( int b2 = a+b; b2 < 1000; b2 += a ) w[a][b2] = 1;
   //}
   //   //trace << endl;
   //}
   int x0 = fin.readInt();
