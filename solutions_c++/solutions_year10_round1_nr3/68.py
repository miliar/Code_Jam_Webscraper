...
int A1, A2, B1, B2;
LL counti(int r, int a) {
	if (a <= 0) return 0;
	if (a < S[r][0]) return a;
	if (a <= S[r][1]) return S[r][0] - 1;

	return a - (S[r][1] - S[r][0] + 1);
}
LL counti(int r, int a, int b) {
	return counti(r, b) - counti(r, a - 1);
}
void solve() {
	cin >> A1 >> A2 >> B1 >> B2;

	LL ret = 0;
	for (int i = A1; i <= A2; ++i) ret += counti(i, B1, B2);
	cout << ret << '\n';
}
