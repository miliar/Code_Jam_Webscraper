...
int main()
{
   ...
   for (int a=1; a<=numT; a++)
   {
      ...
      vvi nextI;
      nextI.push_back(pos);
      cleared.push_back(pos);
      int minMoves = -1;
      bool done = false;
      while (!done)
      {
         minMoves++;
         vvi currI(nextI.begin(), nextI.end());
         nextI.clear();
         for (vvi::iterator i=currI.begin();i != currI.end(); ++i)
         {
            if (test(*i))
            {
               done = true;
            ...
            else
            {
               for (c=0; c<b-1; c++)
               {
                  if ((*i)[c+1] > c) continue;
                  vi curr = swap(*i,c);
                  if (find(cleared.begin(), cleared.end(), curr) == cleared.end())
                  {
                     cleared.push_back(curr);
                     nextI.push_back(curr);
                  }
               }
            }
         }
