...
llint dp[1000001];
void DP(llint dp[],llint up,vector<llint>& all)
{
	assert(up<sizeof(::dp));
	for(int i=0;i<=up;i++)dp[i]=inf;
	dp[0]=0;
	for(int i=0;i<all.size();i++)
	{
		llint add=all[i];
		for(int x=add;x<=up;x++)
			dp[x]=min(dp[x],dp[x-add]+1);
	}
}
int main()
{
	...
	for(cin>>tt;tt--;ca++)
	{
		...
		if(n==1)
		{
			ans=(s%all[0])?inf:s/all[0];
		}
		else
		{
		const llint up=all[0]*all[1];
		debug(up);
		DP(dp,up,all);
		for(llint i=0;i<=up;i++)
		{
			if(s>=i&&0==((s-i)%all[0])&&dp[i]<inf)
			{
				ans=min(ans,(s-i)/all[0]+dp[i]);
			}
		}
		}
		debug(ans);
