...
int main() {
    ...
    char ch;
      ...
      while(true) {
        ch = cin.get();
        if(ch >= '0') {
          L = BigUnsigned(10)*L + BigUnsigned((int) (ch - '0'));
        } else {
          break;
        }
      }
      ...
      ans = L / maxB;
      remains = L - BigUnsigned(maxB)*ans;
      ans = ans - (maxB - 1);
      remains = remains + maxB*(maxB-1);
      r = remains.toInt();
      for(int i=0; i<=r; i++) {
        mincount[i] = -1;
      }
      ...
      while(true) {
        for(int i=0; i<=r; i++) {
          if(mincount[i] == count) {
            for(int j=0; j<N; j++) {
              if(i + B[j] <= r) {
                if(mincount[i+B[j]] == -1) mincount[i+B[j]] = count+1;
              }
            }
          }
        }
        if(mincount[r] > -1) break;
        count++;
        if(count >= 200) break;
      }
