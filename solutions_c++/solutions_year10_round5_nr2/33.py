...
ll exactchange(vec<ll>& b, ll target, ll M) {
  costs[0] = 0;
  cerr << "M is " << M << endl;
  ...
  upto(i,0,N-1) {
	...
	ll p = ((w - bi) + M) % M;
	do {
	  ll thcost = costs[p] + (p < w);
	  //	  cerr << "thcost[" << p << "] = " << thcost << endl;
	  //	  cerr << "costs[" << w << "] = " << costs[w] << endl;
	  if (costs[p] != -1 && (costs[w] == -1 || costs[w] > thcost)) {
	    costs[w] = thcost;
	    changes ++;
	  }
...
void run(int casenum) {
  ...
  if (L % bgcd != 0) {
    ...
    ll chainlength = L / b[i];
    ll chainrem = L % b[i];

    ll cst = exactchange(b, chainrem, b[i]);
    
    solution = cst + chainlength;
