...
{
	return a.r + b.r + sqrt((a.x-b.x)*(a.x-b.x) + (a.y-b.y)*(a.y-b.y));
}
...
int main()
{
	...
	for(int icas = 1; icas <= cas; icas++)
	{
		...
		{
			printf("Case #%d: %.6lf\n", icas, max(lis[0].r,lis[1].r)*1.0);
		}
		else
		{
			double ma = 0x7fffffff;

			ma = min(ma, max(dis(lis[0], lis[1])/2, lis[2].r));
			ma = min(ma, max(dis(lis[0], lis[2])/2, lis[1].r));
			ma = min(ma, max(dis(lis[2], lis[1])/2, lis[0].r));
			printf("Case #%d: %.6lf\n", icas, ma*1.0);
		}
	}
	return 0;
