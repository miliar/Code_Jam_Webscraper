...
int main(){
  ...
  FOR(i,0,_T){
    cout << "Case #" << (i+1) << ": ";
    cin >> N; x.resize(N), y.resize(N),r.resize(N);
    FOR(j,0,N) cin >> x[j] >> y[j] >> r[j];

    if(N == 1) cout << r[0];
    else if(N == 2) cout << max(r[0],r[1]);
    else if(N == 3){
       long double best = INF;
       FOR(i,0,3) FOR(j,i+1,3){
             long double dist = (sqrt(distanceSquared(make_pair(x[i],y[i]),make_pair(x[j],y[j]))) + r[i] + r[j])/2.0;
//	     cout << "DIST: " << dist << endl;
	     best = min(best,max(dist,(long double)r[lowestBit(7-SHIFT(i)-SHIFT(j))]));
       }
       cout << best;
    }
    cout << endl;
