...
double go(double a, double b, double r1, double c, double d, double r2) {
  double ret = (a - c) * (a - c) + (b - d) * (b - d);
  ret = sqrt(ret);
  return (ret + r1 + r2)/2.0;
}
void solve() {
  //your code here
  int n;
  ...
  for (int i = 1; i <= n; ++i) {
    ...
    yy.PB(b);
    rr.PB(c);
    if (1.0 * c > abi) abi = 1.0 * c;
  }
  double ret = -1.0;
  for (int i = 0; i < n; ++i)
    for (int j = i + 1; j < n; ++j) {
      double dist = go(xx[i], yy[i], rr[i], xx[j], yy[j], rr[j]);
      dist >?= abi; 
      if (ret == -1 || dist < ret) ret = dist;
    }
  if (n == 2 && abi < ret) ret = abi; 
  printf("%.06llf\n", ret);
}
