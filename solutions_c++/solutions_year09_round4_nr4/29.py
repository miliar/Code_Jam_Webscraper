...
{	return sqrtl(sqr(D(p1.X-p2.X))+sqr(D(p1.Y-p2.Y))); }
...
inline bool ins(okrag a,okrag b)
{
  D x=d(a.FI,b.FI);
  return x <= b.SE-a.SE+1e-10;
}
...
bool dasie(double R)
{
  REP(i,n) { t1[i]=t[i]; t1[i].SE=max(R-t[i].SE,(D)0.0); }
  vector<PDD> kand;
  REP(i,n) FOR(j,i+1,n-1)
  {
    pair<int,pair<PDD,PDD> > p=przec(t1[i],t1[j]);
    if (p.FI>=1) kand.PB(p.SE.FI);
    if (p.FI>=2) kand.PB(p.SE.SE);
  }
  REP(i,n) kand.PB(t1[i].FI);
  REP(i,SIZE(kand)) FOR(j,i+1,SIZE(kand))
  {
    okrag o1=MP(kand[i],R), o2=MP(kand[j],R);
    bool ok=1;
    REP(k,n) if (!ins(t[k],o1) && !ins(t[k],o2)) { ok=0; break; }
    if (ok) return true;
  }
  return false;
}
