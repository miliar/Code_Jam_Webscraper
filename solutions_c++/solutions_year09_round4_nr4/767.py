...
void docase(int64 case_number) {
    ...
    if (n == 3) {
        double sum[3]; // choosing i as the independent one
        sum[0] = max(r[0], (dist(x[1], y[1], x[2], y[2]) + r[1] + r[2])/2);
        sum[1] = max(r[1], (dist(x[0], y[0], x[2], y[2]) + r[0] + r[2])/2);
        sum[2] = max(r[2], (dist(x[0], y[0], x[1], y[1]) + r[0] + r[1])/2);
        
        ans = min(sum[0], min(sum[1], sum[2]));
