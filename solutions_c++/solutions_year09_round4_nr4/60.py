...
template <typename T> double Point<T>::Len() { return sqrt((double)((*this)*(*this))); }
...
double gettwo(Circle a, Circle b)
{
	return ((a.center()-b.center()).Len()+a.r+b.r)/2.0;
}
...
int main()
{
	...
	REP(tst,tests)
	{
		...
		else
		{
			res = 1.0e30;
			REP(i,3)
				res = min(res, max((double)a[i].r, gettwo(a[(i+1)%3], a[(i+2)%3])));
		}
		printf("Case #%d: %.9lf\n",tst+1,res);
	}
	return 0;
