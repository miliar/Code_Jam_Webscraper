...
int main(void) {
	...
	REP(__number_of_case, __number_of_cases) {
		...
		double res;
		cin >> N;
		vector<vector<double> > coord(N, vector<double> (3));
		cin >> coord;
		switch (N) {
			case 1:
				res = coord[0][2];
				break;
			case 2:
				res = max(coord[0][2], coord[1][2]);
				break;
			case 3:
				res = min(
						max(dist(coord[0][0], coord[0][1], coord[1][0], coord[1][1]) + coord[0][2] + coord[1][2], coord[2][2]),
						min(
							max(dist(coord[2][0], coord[2][1], coord[1][0], coord[1][1]) + coord[2][2] + coord[1][2], coord[0][2]),
							max(dist(coord[0][0], coord[0][1], coord[2][0], coord[2][1]) + coord[2][2] + coord[0][2], coord[1][2])
						   )
						)/2;
				break;
		}
