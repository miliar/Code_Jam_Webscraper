...
template<class T> inline void chkmin(T &a,T b){if(b<a) a=b;}//NOTES:checkmin(
template<class T> inline void chkmax(T &a,T b){if(b>a) a=b;}//NOTES:checkmax(
    ...
    for(int Cas=1;Cas<=CAS;++Cas){
        ...
        for(i=0;i<n;++i){
            ...
            for(j=i+1;j<n;++j){
                ...
                double sx=x[i]-r[i]*(xx/sqrt(xx*xx+yy*yy)),tx=x[j]+r[j]*(xx/sqrt(xx*xx+yy*yy));
                double sy=x[i]-r[i]*(yy/sqrt(xx*xx+yy*yy)),ty=x[j]+r[j]*(yy/sqrt(xx*xx+yy*yy));
                V.pb(mp((sx+tx)/2,(sy+ty)/2));
            }
        }
        ...
        for(i=0;i<V.size();++i){
            for(j=i+1;j<V.size();++j){
                ...
                double res=0;
                for(int k=0;k<n;++k){
                    double r1=dist(V[i].first,V[i].second,x[k],y[k])+r[k];
                    double r2=dist(V[j].first,V[j].second,x[k],y[k])+r[k];
                    chkmax(res,min(r1,r2));
                }
                chkmin(ans,res);
            }
        }
