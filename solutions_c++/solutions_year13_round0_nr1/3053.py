...
bool ok(char c) {
    REP(i, 4) {
        if(get(a[i][0], c) == get(a[i][1], c)
           && get(a[i][1], c) == get(a[i][2], c)
           && get(a[i][2], c) == get(a[i][3], c)
           && get(a[i][3], c) == c) {
            return true;
        }
        if(get(a[0][i], c) == get(a[1][i], c)
           && get(a[1][i], c) == get(a[2][i], c)
           && get(a[2][i], c) == get(a[3][i], c)
           && get(a[3][i], c) == c) {
            return true;
        }
    }
    if(get(a[0][0], c) == get(a[1][1], c)
       && get(a[1][1], c) == get(a[2][2], c)
       && get(a[2][2], c) == get(a[3][3], c)
       && get(a[3][3], c) == c) {
        return true;
    }
    if(get(a[0][3], c) == get(a[1][2], c)
       && get(a[1][2], c) == get(a[2][1], c)
       && get(a[2][1], c) == get(a[3][0], c)
       && get(a[3][0], c) == c) {
        return true;
    }
    return false;
...
int main(){
    ...
    while(t--) {
        ...
        REP(i, 4)   fin >> a[i];
        bool filled = true;
        REP(i, 4)   REP(j, 4)   if(a[i][j] == '.')  filled = false;
        fout << "Case #" << kase << ": ";
        if(ok('X')) fout << "X won\n";
        else if(ok('O'))    fout << "O won\n";
        else if(filled) fout << "Draw\n";
