...
template<typename key,typename value>class ext_a:public __gnu_pbds::tree<key,value,less<key>,__gnu_pbds::rb_tree_tag,__gnu_pbds::tree_order_statistics_node_update>{};
...
template<typename key,typename value>class ext_a:public __gnu_pbds::tree<key,value,less<key>,__gnu_pbds::rb_tree_tag,__gnu_pbds::tree_order_statistics_node_update>{};
template<typename key>class ext_set:public __gnu_pbds::tree<key,__gnu_pbds::null_aped_type,less<key>,__gnu_pbds::rb_tree_tag,__gnu_pbds::tree_order_statistics_node_update>{};
#endif
...
string check(char a,char b,char c,char d) {
  ...
  if (sz(s)==1) {
    if (s[0]=='.') return "";
    return s.substr(0,1)+" won";
  }
  if (sz(s)> 2) return "";

  if (s[0]=='T') swap(s[0],s[1]);
  if (s[0]=='O'&&s[1]=='T') return "O won";
  if (s[0]=='X'&&s[1]=='T') return "X won";
  return "";
}
...
int main() {
  ...
  ft(t,1,T) {
    ...
    string res="";
    ft(i,1,4) {
      if (res=="") res=check(a[i][1],a[i][2],a[i][3],a[i][4]);
      if (res=="") res=check(a[1][i],a[2][i],a[3][i],a[4][i]);
    }
    if (res=="") res=check(a[1][1],a[2][2],a[3][3],a[4][4]);
    if (res=="") res=check(a[1][4],a[2][3],a[3][2],a[4][1]);
    if (res=="") res=flag?"Game has not completed":"Draw";
