...
int main(){
	...
	REP(i,t){
		REP(j,4) {
			scanf("%c",&c);
			REP(k,4) scanf("%c",&tab[j][k]);
		}
		scanf("%c",&c);
		
		bool dot = 0;		
		REP(j,4) REP(k,4) if(tab[j][k] == '.') dot = 1;

		REP(j,4) {
			REP(k,4) tt[k] = tab[j][k];
			res = check(tt);
			if(res > 0) break;
			REP(k,4) tt[k] = tab[k][j];
			res = check(tt);
			if(res > 0) break;
		}
		if(res == 0){
			REP(i,4) tt[i] = tab[i][i];
			res = check(tt);
		}
		if(res == 0){
			REP(i,4) tt[i] = tab[i][3-i];
			res = check(tt);
		}
		
		if(res == 2) printf("Case #%d: X won\n", i+1);
		if(res == 1) printf("Case #%d: O won\n", i+1);	
		if(res == 0 && dot == 0) printf("Case #%d: Draw\n", i+1);	
		if(res == 0 && dot == 1) printf("Case #%d: Game has not completed\n", i+1);	
