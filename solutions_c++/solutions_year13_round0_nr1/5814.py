...
int Check( char c)
{
	...
	for(int k=0;k<4;k++)
	{
		if(CheckC(A[k][0], c)==1 && CheckC(A[i+k][j+1],c)==1 && CheckC(A[i+k][j+2],c)==1 && CheckC(A[i+k][j+3],c)==1) {
			return 1;
		}
	}
	...
	for(int k=0;k<4;k++)
	{
		if(CheckC(A[i][j+k],c) == 1 && CheckC(A[i+1][j+k],c)==1 && CheckC(A[i+2][j+k],c)==1 && CheckC(A[i+3][j+k],c)==1) return 1;
	}

	if(CheckC(A[i][j],c)==1 && CheckC(A[i+1][j+1],c) == 1 && CheckC(A[i+2][j+2],c)==1 && CheckC(A[i+2][j+2],c) == 1) return 1;

	if(CheckC(A[0][3],c)==1 && CheckC(A[1][2],c) == 1 && CheckC(A[2][1],c)==1 && CheckC(A[3][0],c) == 1) return 1;
