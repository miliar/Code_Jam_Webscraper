...
//vector<tuple<char,int,double>> car;

int main(int argc,char **args)
{

	printf("Hello World!\n");
//	freopen("A-example.in","r",stdin);freopen("A-example.out","w",stdout);
	...
	for (int caseId=1;caseId<=testcase;caseId++)
	{
        ...
        forn(i,N)
        {
            forn(j,N)
            {
                scanf("%c\n",&C);
                if(C=='O')table[i][j]=-11;
                else if(C=='X')table[i][j]=10;
                else if(C=='T')table[i][j]=2;
                else {table[i][j]=0;flag=false;}
            }
        }
...
*/
        ...
        forn(i,N)
        {
            ...
            if(checkmax(maxSumAbs,abs(sum)))tmpSum=sum;

            //if(maxSumAbs>=3){flag1=true;break;}
        }
        ...
        sum=0;
        forn(i,N)sum+=table[i][i];
        if(checkmax(maxSumAbs,abs(sum)))tmpSum=sum;

        sum=0;
        forn(j,N)sum+=table[N-j-1][j];
        cerr<<sum<<endl;
        if(checkmax(maxSumAbs,abs(sum)))tmpSum=sum;

        if(maxSumAbs>30)
        {
            if(tmpSum>0)cout<<" X won"<<endl;
