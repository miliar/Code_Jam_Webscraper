...
{
	fori(4) if (check(0,i,1,0,c)) return true;
	fori(4) if (check(i,0,0,1,c)) return true;
	if (check(0,0,1,1,c)) return true;
	if (check(3,0,-1,1,c)) return true;
	return false;
}
...
int main()
{
	...
	fort
	{
		...
		{
			bool ok = false;
			fori(4) if (count(mp[i], mp[i]+4, '.')) ok = true;
			if (ok)
