...
//#define INS(A, P, A.insert(A.begin() + P,
...
{
    set<char>S;
    for(int i=0; i<4; i++)S.insert(s[n][i]);
    if(S.size()>2)return 0;
    if(S.size()==2)
    {
        if(S.find('X')!=S.end() && S.find('T')!=S.end())return 1;
        if(S.find('O')!=S.end() && S.find('T')!=S.end())return 2;
        return 0;
    }
    if(S.find('X')!=S.end())return 1;
    if(S.find('O')!=S.end())return 2;
    return 0;
}
...
{
    set<char>S;
    for(int i=0; i<4; i++)S.insert(s[i][n]);
    if(S.size()>2)return 0;
    if(S.size()==2)
    {
        if(S.find('X')!=S.end() && S.find('T')!=S.end())return 1;
...
int main()
{
    ...
    while(kase--)
    {
        ...
        getchar();
        for(int i=0;i<4;i++)for(int j=0;j<4;j++)dot+=(s[i][j]=='.');
/*
        for(int i=0;i<4;i++)
        {
            for(int j=0;j<4;j++)
        ...
        for(int i=0; i<4; i++)if(!res)res=row(i);
        if(!res)for(int i=0; i<4; i++)if(!res)res=col(i);
        if(!res)res=dia(0);
        if(!res)res=dia(3);

        printf("Case #%d: ",++ks);
        if(!res && dot)puts("Game has not completed");
        else if(!res)puts("Draw");
        else if(res==1)puts("X won");
        else puts("O won");
    }
    return 0;
}
