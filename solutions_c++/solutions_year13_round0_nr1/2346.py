...
string f(char x){
    return x == 'X' ? "X won" : "O won";
}

string gao(){
    bool draw = 1; int x = -1, y; int t = 0;
    ...
    REP(i, 4){
        if (A[i][0] == A[i][1] && A[i][0] == A[i][2] && A[i][0] == A[i][3]) return f(A[i][0]);
        if (A[0][i] == A[1][i] && A[0][i] == A[2][i] && A[0][i] == A[3][i]) return f(A[0][i]);
    }

    if (A[0][0] == A[1][1] && A[0][0] == A[2][2] && A[0][0] == A[3][3]) return f(A[0][0]);
    if (A[0][3] == A[1][2] && A[0][3] == A[2][1] && A[0][3] == A[3][0]) return f(A[0][3]);
