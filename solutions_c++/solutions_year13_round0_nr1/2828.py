...
bool isfill(const matrix<char>& m)
{
	for(int i = 0; i < 4; ++i)
	{
		for(int j = 0; j < 4; ++j)
...
inline bool isot(char c) { return c == O || c == T; }
inline bool isxt(char c) { return c == X || c == T; }
st evl(char a, char b, char c, char d)
{
	if(isot(a) && isot(b) && isot(c) && isot(d)) return st_o;
	if(isxt(a) && isxt(b) && isxt(c) && isxt(d)) return st_x;
	return st_n;
}

st ev(const matrix<char>& m)
{
	st s;
	for(int i = 0; i < 4; ++i)
	{
		s = evl(m(i,0), m(i,1), m(i,2), m(i,3)); if(s == st_x) return st_x; if(s == st_o) return st_o;
		s = evl(m(0,i), m(1,i), m(2,i), m(3,i)); if(s == st_x) return st_x; if(s == st_o) return st_o;
	}

	s = evl(m(0,0), m(1,1), m(2,2), m(3,3)); if(s == st_x) return st_x; if(s == st_o) return st_o;
	s = evl(m(0,3), m(1,2), m(2,1), m(3,0)); if(s == st_x) return st_x; if(s == st_o) return st_o;
...
void solve(istream& in, ostream& fout, int caseIdx)
{
	...
	switch(ev(m))
	{
	case st_x : OUT( << CaseHead(caseIdx+1) << "X won" << endl ); return;
	case st_o : OUT( << CaseHead(caseIdx+1) << "O won" << endl ); return;
	case st_n : OUT( << CaseHead(caseIdx+1) << "Game has not completed" << endl ); return;
	case st_d : OUT( << CaseHead(caseIdx+1) << "Draw" << endl ); return;
	}
}
...
int main(int argc, char* argv[])
{
	...
	proc_file( "../CJ/in.txt" , "../CJ/out.txt" );
