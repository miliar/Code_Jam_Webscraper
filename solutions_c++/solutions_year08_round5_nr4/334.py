...
bool rocks[100][100];
long long H, W, R, rr, rc;

long long solve(int r, int c){
    if(r==0 && c==0) return 1;
    if(r < 0 || c < 0 || rocks[r][c]) return 0;
    if(dp[r][c]!=-1) return dp[r][c];
    dp[r][c] = 0;
    long long A = solve(r-2,c-1), B=solve(r-1,c-2);
    if(A==-2) dp[r][c] = B;
    else if(B==-2) dp[r][c] = A;
    else dp[r][c] = (A + B)%MOD;
    return dp[r][c];
}
...
int main(){
  cin >> _times;
  FOR(_t,1,_times+1){
    MEMSET(dp,-1); MEMSET(rocks,false);
    cin >> H >> W >> R;
    FOR(i,0,R) { cin >> rr >> rc; rocks[rr-1][rc-1] = true;}
    cout << "Case #" << _t << ": " << solve(H-1,W-1) << endl;
  }
  return 0;
}
