...
RealCase::RealCase()
{
    ...
    Rr = VI(R);
    Rc = VI(R);
    REP(n, R) { cin >> Rr[n] >> Rc[n]; Rr[n] -= 1; Rc[n] -= 1; }
}

int memo[100][100];

int RealCase::calc(int r, int c)
{
    int &m = memo[r][c];
    if (m >= 0) return m;
    REP(n, R) if (Rr[n] == r && Rc[n] == c) return m = 0;
    if (r == 0 && c == 0) return m = 1;
    int res = 0;
    if (r >= 1 && c >= 2) res += calc(r - 1, c - 2);
    if (r >= 2 && c >= 1) res += calc(r - 2, c - 1);
    return m = (res%M);
}
