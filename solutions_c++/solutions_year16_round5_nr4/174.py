...
string q[MAXN], s;
//int n, l;//, d[MAXN][MAXN][MAXN][2];

//bool solve(int v1, int v2, int v3, int lst, const string& s1, const string& s2, const string& c) {
//    if (v1 == int(s1.size()) && v2 == int(s2.size()) && vc == int(c.size())) return true;
//    if (v1 == int(s1.size()) || v2 == int(s2.size()) || vc == int(c.size())) return true;
//
//    int &ans = d[v1][v2][v3][lst];
//    if (ans != -1) return ans;
//
//    ans = 0;
//
...
bool check(const string& s1, const string& s2, const string& c) {
//    memset(d, 255, sizeof d);
    return true; // solve(0, 0, 0, 0, s1, s2, c);
}
