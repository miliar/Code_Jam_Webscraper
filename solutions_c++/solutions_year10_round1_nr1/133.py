...
char grid[100][100],rot[100][100];
// Code review is very important!!
bool find(char rot[100][100],char name,int n,int k)
{
	for(int r=0;r<n;r++)
	for(int c=0;c<n;c++)
	for(int d=0;d<8;d++)
	{
		int cnt=0,i=r,j=c;
		while(cnt<k&&i>=0&&i<n&&j>=0&&j<n&&rot[i][j]==name)
...
//			debug(grid[i]);
		...
		for(int r=0;r<n;r++)
		for(int c=0;c<n;c++)
			rot[c][n-1-r]=grid[r][c];
//		hline();
///		for(int i=0;i<n;i++)cerr<<rot[i]<<endl;

		for(int r=n-1;r>=0;r--)
		for(int c=0;c<n;c++)
		{
			if(rot[r][c]=='.')continue;
			for(int i=r;i+1<n&&rot[i+1][c]=='.';i++)
				rot[i+1][c]=rot[i][c],rot[i][c]='.';
		}
//		hline();
//		for(int i=0;i<n;i++)cerr<<rot[i]<<endl;
