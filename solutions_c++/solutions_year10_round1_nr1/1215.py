...
//	freopen("C-large.in","r",stdin);freopen("C-large.out","w",stdout);
	...
	for (int caseId=1;caseId<=testcase;caseId++)
	{
		...
		for (int i=0; i< size; i++) for (int j=0; j<size; j++) {
			if (G[i][j] != '.') {
				int count=0;int k;
				for ( k=j,count=0; k<size && G[i][j] == G[i][k]; k++) count++;
				if (count >= cnt) {
					if (G[i][j] == 'B') blue = true; else red = true;
				}
				for ( k=i,count=0; k<size && G[i][j] == G[k][j]; k++) count++;
				if (count >= cnt) {
					if (G[i][j] == 'B') blue = true; else red = true;
				}
				for ( k=i,count=0; i<size && i+k<size && j+k < size && G[i][j] == G[i+k][j+k]; k++) count++;
				if (count >= cnt) {
					if (G[i][j] == 'B') blue = true; else red = true;
				}
				for ( k=0,count=0; i+k<size && j-k>=0 && G[i][j] == G[i+k][j-k]; k++) count++;
				if (count >= cnt) {
					if (G[i][j] == 'B') blue = true; else red = true;
				}
			}
