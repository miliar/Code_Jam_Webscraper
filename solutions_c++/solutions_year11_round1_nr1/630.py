...
int main() {
	gcase {
		...
		if(G == 100 && D != 100) gout << "Broken" << endl;
		else if(G == 0 && D != 0) gout << "Broken" << endl;
		else if(D == 0) gout << "Possible" << endl;
		else if(D == 100) gout << "Possible" << endl;
		else {
			long o = 100;
			long id = 100-D;
			bool possible = false;
			for(long m = 1; o*m <= N*id; m++) {
				long x = o*m;
				if(x % id == 0) {
					possible = (x/id <= N);
					if(possible) break;
			...
			if(possible) gout << "Possible" << endl;
			else gout << "Broken" << endl;
		}
	}
}
