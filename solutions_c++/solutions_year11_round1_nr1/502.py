#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <math.h>

#define MAX_LEN_LINE	100

int GetInt(char *str, int *len, bool *bEndLine)
{
	int ipos = 0;
	while (str[ipos] != ' ' && str[ipos] != '\n')
		ipos++;
	char strZahl[20];
	strncpy(strZahl, str, ipos);
	strZahl[ipos] = 0;
	int iResult = atoi(strZahl);
	*len = ipos;
	if (str[ipos]=='\n')
		*bEndLine = true;
	else
		*bEndLine = false;
	return iResult;
}
__int64 GetInt64(char *str, int *len, bool *bEndLine)
{
	int ipos = 0;
	while (str[ipos] != ' ' && str[ipos] != '\n')
		ipos++;
	char strZahl[50];
	strncpy(strZahl, str, ipos);
	strZahl[ipos] = 0;
	__int64 iResult = _atoi64(strZahl);
	*len = ipos;
	if (str[ipos]=='\n')
		*bEndLine = true;
	else
		*bEndLine = false;
	return iResult;
}
double GetDouble(char *str, int *len, bool *bEndLine)
{
	int ipos = 0;
	while (str[ipos] != ' ' && str[ipos] != '\n')
		ipos++;
	char strZahl[50];
	strncpy(strZahl, str, ipos);
	strZahl[ipos] = 0;
	double dResult = atof(strZahl);
	*len = ipos;
	if (str[ipos]=='\n')
		*bEndLine = true;
	else
		*bEndLine = false;
	return dResult;
}
void GetString(char *str,char *strOut, int *len, bool *bEndLine)
{
	int ipos = 0;
	while (str[ipos] != ' ' && str[ipos] != '\n')
		ipos++;
	strncpy(strOut, str, ipos);
	strOut[ipos] = 0;
	*len = ipos;
	if (str[ipos]=='\n')
		*bEndLine = true;
	else
		*bEndLine = false;
}

int ggT(int a, int b)
{
	if (a == b)
	{
		return a;
	}
	if (a > b)
	{
		return ggT(b, (a - b));
	}
	return ggT(b, (b - a));
}

int main(int argv, char *argc[])
{
	if (argv < 2)
		return -1;
	

	int iCountCases = 0;
	
	// read Input
	FILE *fInput = fopen(argc[1], "r");
	if (fInput == NULL)
		return -1;

	// output
	FILE *fOutput = fopen("output.txt", "w");
	if (fOutput == NULL)
	{
		fclose(fInput);
		return -1;
	}


	char strLine[MAX_LEN_LINE];
	// read first line
	if (fgets(strLine, MAX_LEN_LINE, fInput)==NULL)
	{
		// Fehler beim Lesen
		return -1;
	}

	iCountCases = atoi(strLine);

	int x1;

	int iCase;
	for (int i=0; i<iCountCases; i++)
	{
		iCase = i+1;

		if (fgets(strLine, MAX_LEN_LINE, fInput) == NULL)
			return 0;
		
		int ipos = 0;
		int ilen;
		bool bEndLine;

		__int64 iN = GetInt64(&strLine[ipos], &ilen, &bEndLine);
		ipos += (ilen+1);
		int iPToday = GetInt(&strLine[ipos], &ilen, &bEndLine);
		ipos += (ilen+1);
		int iPAll = GetInt(&strLine[ipos], &ilen, &bEndLine);
		ipos += (ilen+1);
		
		// algorithm
		int iGGT = ggT(100, iPToday);
		int iMinToday = 100/iGGT;
		
		bool bPossible;
		if (iMinToday<=iN)
		{
			if (iPAll==100 && iPToday<100)
				bPossible = false;
			else if (iPAll==0 && iPToday>0)
				bPossible = false;
			else
				bPossible = true;
		}
		else
			bPossible = false;
		
		// Ausgabe
		if (bPossible)
			fprintf(fOutput, "Case #%d: Possible\n", iCase);
		else
			fprintf(fOutput, "Case #%d: Broken\n", iCase);


		iCase++;
	}

	fclose(fInput);
	fclose(fOutput);

	return 0;
}