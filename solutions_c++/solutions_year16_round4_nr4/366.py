...
int n;
bool test(int** mat,int *worker,bool *mach,int depth) {
    if(depth == 0) {
        for(int i = 0;i < n;i++) if(!mach[i]) return false;
        return true;
    }
    int check = 0;
    for(int i = 0;i < n;i++) {
        if(worker[i] == -1) {
            for(int j = 0;j < n;j++) {
                if(mat[i][j] == 1 && !mach[j]) {
                    ...
                    worker[i] = j;
                    mach[j] = true;
                    if(!test(mat,worker,mach,depth-1)) {
                        return false;
                    }
                    worker[i] = -1;
                    mach[j] = false;
...
void solve()
{
    ...
    int *worker = carray<int>(n);
    bool *mach = carray<bool>(n);
    int **cpy = carray<int>(n,n);
    int ans = 9999;
    for(int i = 0;i < (1<<unk);i++) {
        int ii = i;
        int cost = 0;
        forn(i,n) forn(j,n) {
            if(mat[i][j] == 1) cpy[i][j] = 1;
            else {
                if(ii%2 == 1) {
                    cpy[i][j] = 1;
                    cost++;
        ...
        setArray(worker,n,-1);
        setArray(mach,n,false);
        bool res = test(cpy,worker,mach,n);
        if(res) {
            //printf("%d ",i);
            ans = min(ans,cost);
        }
    ...
    for(int j = 0;j < n;j++) {
        //printf("%d %d\n",needed[j],have[j]);
        if(have[j] >= needed[j]+1) continue;
        ans += needed[j]+1 - have[j];
    }
    printf("%d\n",ans);
    freeMatr(mat,n,n);*/
}
