...
bool safe() { //Try all assignment orders
	bool finished = true;
	for (int i=0; i<N; i++) {
		...
		for (int j=0; j<N; j++) {
			if (know[i][j] && !taken[j]) {
				...
				taken[j] = true;
				occupied[i] = true;
				if (!safe()) return false; //This order would possibly cause problems
				taken[j] = false;
				occupied[i] = false;
			}
		}
	...
	return true; //Either no more workers to assign, or no problems down the decision tree
}

void trythis(int cost) {
	//Test all possible decision arcs
	for (int i=0; i<N; i++) taken[i] = occupied[i] = false;
	/*err("trying this at cost %d:\n",cost);
	for (int i=0; i<N; i++) {
		printf("\t");
		for (int j=0; j<N; j++) printf("%d",know[i][j]);
		printf("\n");
	}*/
	if (safe()) {
		//err("it is safe.\n");
...
void tryall(int id = 0, int cost = 0) {
	if (id == N*N) {
		trythis(cost);
		return;
	}
	int i = id%N;
	int j = id/N;
	tryall(id+1,cost); //Don't train this worker in this skill
...
void doCase(int case_number) {
	...
	for (int i=0; i<N; i++) {
		string s; cin >> s;
		for (int j=0; j<N; j++) know[i][j] = (s[j] == '1');
	}
