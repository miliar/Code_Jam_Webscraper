...
i64 solve(i64 n, vector<string> known)
{
  i64 best = bit(50);
  rep(mask, bit(n * n)){
    bool ok = true;
    rep(i, n)rep(j, n)if(~mask & bit(i * n + j))if(known[i][j] == '1') ok = false;
    map<i64, i64> knowledge_cnt;
    rep(i, n) knowledge_cnt[(mask >> (n * i)) & (bit(n) - 1)] += 1;
    for(auto p: knowledge_cnt)if(__builtin_popcountll(p.first) != p.second) ok = false;
    for(auto p0: knowledge_cnt)for(auto p1: knowledge_cnt){
        if(p0.first != p1.first && (p0.first & p1.first) != 0) ok = false;
      }
    i64 cost = __builtin_popcountll(mask);
    rep(i, n)rep(j, n) cost -= known[i][j] - '0';

    //rep(i, n){ rep(j, n) cout << ((mask >> (i * n + j)) & 1); cout << endl; }
    //cout << cost << ": " << ok << endl;
