...
public:
  ...
  void solve() {
    int s = 0;
    REP(i, n) REP(j, n) if (o[i][j]) s |= 1<<(i*n+j);
    int ans = INF;
    REP(u, 1<<(n*n)) if ((u & s) == s) {
      REP(i, n) REP(j, n) o[i][j] = u>>(i*n+j) & 1;
      bool ok = true;
      REP(i, n) {
        int m = 0;
        REP(j, n) if (o[i][j]) m |= 1<<j;
        ok = false;
        for (int v = m; v; v = (v-1)&m) {
          int x = 0;
          REP(jj, n) if (m>>jj&1) REP(ii, n) if (ii != i && o[ii][jj]) x |= 1<<ii;
          if (__builtin_popcount(x) < __builtin_popcount(v)) {
            ok = true;
            break;
          }
        }
      ...
      if (ok) smin(ans, __builtin_popcount(u));
    }
    print("%d", ans - __builtin_popcount(s));
  }
...
int main() {
  //randomTest();
  //maxTest();
  freopen("d-small-attempt0.in", "r", stdin);  // -small-attempt0.in
  freopen("d-small-attempt0.txt", "w", stdout);  // -large
  testIndex = 0;
  scanf("%d", &testCount);
