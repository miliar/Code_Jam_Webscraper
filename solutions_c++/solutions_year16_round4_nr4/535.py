...
void solve(){
  ...
  for(int t=0; t<(1<<(n*n)); t++){
    ...
    do{
      ...
      function<bool(int, int)> dfs = [&](int d, int used) -> bool{
        if(d==n) return used==((1<<n)-1);

        bool nx = false;
        bool ok = true;
        int p = ord[d];
        for(int i=0; i<n; i++){
          if( k_bit(t, n*p+i ) && !k_bit(used, i) ){
            nx = true;
            ok = ok && dfs(d+1, used|(1<<i));
          }
        }
    ...
    if(ok) ans = min(ans, bit_count(t^s));
  }
