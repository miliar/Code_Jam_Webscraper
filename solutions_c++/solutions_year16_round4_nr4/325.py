...
public:
	void solve(std::istream& in, std::ostream& out) {
		...
		function<bool()> check = [&]() {
			...
			for (int i: range(n)) {
				for (int j: range(n)) {
					bool ok = (mask[i] == mask[j]) || ((mask[i] & mask[j]) == 0);
					if (!ok) {
						//cerr << i << ' ' << j << endl;
						return false;
					}
			...
			for (auto p: cnt) {
				if (__builtin_popcount(p.first) != p.second) {
					return false;
				}
			}
			return true;
		...
		function<void(int, int)> rec = [&](int x, int curAns) {
			if (x == n) {
				if (check()) {
					ans = min(ans, curAns);
				}
			...
			for (int i: range(1 << n)) {
				if ((i & initial[x]) != initial[x]) {
					continue;
				}
				mask[x] = i;
				rec(x + 1, curAns + __builtin_popcount(i));
			}
		};
