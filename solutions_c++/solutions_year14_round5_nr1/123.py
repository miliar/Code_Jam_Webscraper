...
int main()
{
	...
	fort
	{
		...
		vll prefix(n + 1, 0);
		forin prefix[i+1] = prefix[i] + v[i];

		ll sumOpt = prefix.back();
		...
		for (int posi = 0; posi < n; posi++)
		{
			while (posj < posi)
				++posj;
			while (posj < n && max(prefix[posj] - prefix[posi], prefix[n] - prefix[posj]) > max(prefix[posj+1] - prefix[posi], prefix[n] - prefix[posj+1])) //ranges: [0; posi); [posi; posj); [posj; n)
				++posj;
			sumOpt = min(sumOpt, max(max(prefix[posi] - prefix[0], prefix[posj] - prefix[posi]), prefix[n] - prefix[posj]));
		}
		ans = ld(prefix.back() - sumOpt) / prefix.back();
