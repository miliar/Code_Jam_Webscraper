...
double solve()
{
    ...
    const ll all_sum = accumulate(all(a), 0LL);
    ...
    rep(second_end, n)
    {
        ...
        while (first_end < second_end && first < second)
        {
            first += a[first_end];
            second -= a[first_end];
            ++first_end;
        ...
        upmin(min_solv, max(max(first, second), third));
        if (first_end > 0)
            upmin(min_solv, max(max(first - a[first_end - 1], second + a[first_end - 1]), third));
    }
    return double(all_sum - min_solv) / all_sum;
}
int main()
{
    int T;
