...
long long s[MAXN];
long long sum[MAXN];
long long maxof[MAXN];

inline long long segsum(int i,int j) {
    return sum[j+1]-sum[i];
}

inline long long fr2(int a,int b) {
    return max(segsum(a,b),segsum(b+1,n-1));
}

inline long long f3(int a) {
    return max(segsum(0,a-1),maxof[a]);
}
...
double solve() {
    ...
    for(int a=0;a<n;a++)
        opt=min(opt,f3(a));
    double sol = (long double)opt/sum[n];
    sol=1-sol;
    return sol;
}
