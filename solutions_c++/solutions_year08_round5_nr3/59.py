...
struct RealCase : public Case
{
    ...
    int calc(int r, int c, int cur, int last);
...
RealCase::RealCase()
{
    ...
    REP(r, H) {
        ...
        cin >> word;
        assert(SZ(word) == W);
        REP(c, W) X[r][c] = (word[c] == 'x');
    }
}

short memo[10][10][1<<10][1<<10];

int RealCase::calc(int r, int c, int cur, int last)
{
    if (c >= W) return calc(r + 1, 0, 0, cur);
    if (r == H) return 0;
    assert(r >= 0 && c >= 0 && r < 10 && c < 10 && cur >= 0 && cur < (1<<10) && last >= 0 && last < (1<<10));
    short &m = memo[r][c][cur][last];
    if (m >= 0) return m;
    if (X[r][c] || (((last<<1)>>c)&5)) return m = calc(r, c + 1, cur, last);
    return m = max( calc(r, c + 1, cur, last),
                    1 + calc(r, c + 2, cur|(1<<c), last) );
}

void RealCase::solve(Result &result)
{
    SET(memo, -1);
    result.set(STR(calc(0, 0, 0, 0)));
}
