...
int ROWS,COLS;
int f(int row, int col,int curMask, int prevMask) {
	pair<int,pair<int,int> > key=make_pair(row*100+col, make_pair(curMask,prevMask));
	if (mm.find(key)!=mm.end()) {
		return mm[key];
	}
	if (row==ROWS) return 0;
	if (col==COLS) return mm[key]=f(row+1,0,0,curMask);
	int can=board[row][col]=='.';
	if (col==0) {
		can = can && (1<<(col+1) & prevMask)==0;
	} else {
		can = can && (1<<(col-1) & prevMask)==0 &&
				(1<<(col+1) & prevMask)==0 &&
					(1<<(col-1) & curMask)==0 ;
	}
	int resNo = f(row,col+1,curMask,prevMask);
	if (!can) {
		return mm[key] = resNo;
	} else {
		int resYes =1+ f(row,col+1,curMask|(1<<col),prevMask);
		return mm[key] = max(resNo,resYes);
	}
}
...
for (int ca=0; ca<CA; ca++) {
	board.clear();mm.clear();
	READ(int,R);READ(int,C);
	ROWS=R; COLS=C;
	FN(i,R) {
		READ(string,s); board.push_back(s);
	}
	int ans = f(0,0,0,0);
