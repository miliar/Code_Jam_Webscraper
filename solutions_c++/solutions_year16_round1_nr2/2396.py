...
int main(int, char *[])
{
    freopen("c:/Users/Infinity/Downloads/" file ".in", "r", stdin);
    freopen("c:/Users/Infinity/Downloads/" file ".out", "w", stdout);
    int ct = clock();
    for (int te = getint(), ti = 1; ti <= te; ti++)
    {
            ...
            [&]()
        {
            ...
            for (int i = 1; i <= n; i++)
            {
                if (l[i] != -1)
                    for (int j = 1; j <= n; j++)
                        if (c[j] != -1)
                            if (v[l[i]][j - 1] != v[c[j]][i - 1])
                                return false;
                if (c[i] != -1)
                    for (int j = 1; j <= n; j++)
                        if (l[j] != -1)
                            if (v[l[j]][i - 1] != v[c[i]][j - 1])
                                return false;
            }
            return true;
        };
        vector<char> up(n + 1), un(n + 1);
        ...
        catch (bool)
        {
            ...
            catch (int k)
            {
                for (int i = 0; i < 2 * n - 1; i++)
                    if (sgn(b[i]) != sgn(k))
                        write(SP, v[i][abs(k) - 1]);
            }
            writeln();
        }
    }
    clog << "Time elapsed: " << clock() - ct << " ms" << endl;
    return 0;
}
