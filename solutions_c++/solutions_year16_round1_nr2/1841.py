...
void Solve1B(int (*rank_matrix)[50], const int &n, std::ostream &out_file) {    
  std::vector<int> result;
  int all_elements[2500];
  for (int i = 0; i < 2500; i++) all_elements[i] = 0;
  for (int i = 0; i < 2 * n -1 ; i++) {
    for (int j = 0; j < n; j++) {
	  all_elements[rank_matrix[i][j] - 1]++;
	}
  }
  for (int i = 0; i < 2500; i++) {
    if ((all_elements[i] % 2) == 1) result.push_back(i + 1);
  }
...
void GCJ1B() {
  std::ifstream in_file("B-large.in");
  std::ofstream out_file("B-large.out");
  std::string in_line;
  // Get Test Case T 
  std::getline(in_file, in_line);
  int num_test_case;
  ...
  int cnt = 0;
  int n = 0;
  int rank_matrix[200][50];
  while (std::getline(in_file, in_line)) {
	out_file << "Case #" << ++cnt << ": ";	
    GetTestCases(in_line, n);
	for (int i = 0; i < 2 * n - 1; i++) { 
	  std::getline(in_file, in_line);
	  GetArrayParams(in_line ,n, rank_matrix[i]);
	}    	
	Solve1B(rank_matrix, n, out_file);
  }
  in_file.close();
  out_file.close();
  return;  
