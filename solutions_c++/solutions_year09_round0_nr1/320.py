...
int solve(){
  int ret = 0;
  FOR(i,0,words.size()){
    bool all = true; FOR(j,0,L) all = all && dp[j][words[i][j]-'a'];
    if(all) ret++;
  }
  return ret;
}
...
int main(){
  cin >> L >> D >> _T;
  words = vector<string>(D);
  FOR(i,0,D) cin >> words[i];
  
  FOR(i,0,_T){
    cout << "Case #" << (i+1) << ": ";
    MEMSET(dp,false);
    string str; cin >> str;
    int p = 0, cp = 0; while(p < str.length()){
	if(str[p]=='('){ p++; while(str[p]!=')') dp[cp][str[p++]-'a'] = true; }
        else dp[cp][str[p]-'a'] = true; 
	cp++, p++;
    }
    cout << solve();
    cout << endl;
