...
int main()
{
	...
	for(int n_counter=0; n_counter < n;n_counter ++)
	{
		...
		for(int d_counter=0; d_counter < d;d_counter ++){
			int word_checker = 0;
			int between_squares = 0;
			int word_found = 1;
			for(int l_counter=0;l_counter<l;l_counter++)
			{	
				found_array[l_counter]=0;
			}
			
			for(int guess_counter=0;guess[guess_counter]!='\0';guess_counter++){
				if(guess[guess_counter] == '(')
				{
					between_squares = 1;
				} else if(guess[guess_counter] == ')')
				{
					between_squares = 0;
					word_checker++;
				} else 
				{
					if(guess[guess_counter] == D[d_counter][word_checker])
					{
						found_array[word_checker] = 1;
					}
					if(between_squares == 0){
						word_checker++;
					}
				}
			}
		...
		//Case #1: 2
		printf("Case #%d: %d",n_counter+1,match_counter);
		if(n_counter+1<n){
			printf("\n");
		}
		fflush(stdout);
