...
//	long long int multi=1;
   ...
   // memset(lookupChar,0,L*sizeof(char))
    ...
    tempString.clear();
    //Í³¼Æ¸÷¸öÊý×éÖÐµÄ×ÖÄ¸£¬ÔÚ×ÖµäÖÐµÄÏàÓ¦Î»ÖÃºÏ·¨µÄÇé¿ö
    k=0;
   for(i=0;i<L;i++)
   {
	    k=0;
...
int main(int argc, char **argv){
 ...
 //scanf("%d%d%d", &L,&D,&N);
  ...
  //ÔÚ×ÖµäµÄ¶ÔÓ¦Î»ÖÃ£¬¼ÇÂ¼Ã¿¸ö×ÖÄ¸³öÏÖµÄ´ÎÊý
  ...
  for(j=0; j<D; j++) //construct the dictionary
	...
	//¶Ô×ÖµäµÄÃ¿Ò»ÐÐ½øÐÐÅÅÐò,Í³¼ÆÔÚÃ¿Ò»¸öÎ»ÖÃ£¬Ã¿Ò»¸ö×ÖÄ¸¿ÉÄÜ³öÏÖµÄ´ÎÊý
 /* for(j =0; j<L; j++)
  {
	//sort(&array[0][j],&array[D-1][j]);
	for(i=0;i<D;i++)
	{
		Index[j][array[j][i]-97]++;
	}
  }*/
  for(int C=1; C<=N; C++) {
