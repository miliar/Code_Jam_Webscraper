...
int solve(int pos, int prev, int itype){
    if(pos == nums.size() || pos == nums.size()-1 && prev == BLANK) return 0;
    int &ret = dp[pos][prev][itype]; if(ret != -1) return ret;
    // cerr << "Solve: " << pos << " " << prev << " " << itype << endl;

    ret = D + solve(pos+1,prev,NONE);

    if(prev == BLANK){
	FOR(i,0,256){
	    ret = min(ret,I + solve(pos,i,NONE));
	    ret = min(ret,(int)abs(nums[pos]-i) + solve(pos+1,i,NONE));
        }
	ret = min(ret,solve(pos+1,nums[pos],NONE));
    }
    else{
      if(abs(nums[pos]-prev) <= M) ret = min(ret,solve(pos+1,nums[pos],NONE));

      ret = min(ret,abs(nums[pos]-prev) + solve(pos+1,prev,NONE));

      FOR(i,max(0,prev-M),prev){
          if(itype != UP) ret = min(ret,I + solve(pos,i,DOWN));
	  ret = min(ret,(int)abs(nums[pos]-i) + solve(pos+1,i,NONE));
      }

      FOR(i,prev+1,min(255,prev+M)+1){
          if(itype != DOWN) ret = min(ret,I + solve(pos,i,UP));
	  ret = min(ret,(int)abs(nums[pos]-i) + solve(pos+1,i,NONE));
      }
    }
    // cerr << "Solve: " << pos << " " << prev << " " << itype << " -> " << ret << endl;
    return ret;
}
...
int main(){
  ...
  FOR(i,0,_T){
    ...
    cout << "Case #" << (i+1) << ": ";
    cin >> D >> I >> M >> N;
    nums.resize(N); FOR(j,0,N) cin >> nums[j];
    cout << solve(0,BLANK,NONE);
    cout << endl;
  }
  return 0;
}
