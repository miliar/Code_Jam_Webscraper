...
int Solve(bool print){
    ...
    FORV(i,a){
        ...
        REP(j, 256)//  change
            REP(k, 256)
                if( abs(j-k) <= M )
                    C[i+1][j] <?= C[i][k] + abs( j - a[i] );

        if( M )
            REP(j, 256)
                REP(k, 256){
                    int n = max( ( abs(j-k) - 1 ) / M, 0);
                    C[i+1][j] <?= C[i][k] + n * I + abs( j - a[i] );
                }
        
        
    }
    return *min_element(C[N], C[N]+256);
}
...
freopen("output.txt","w",stdout);
    ...
    cin>>Tc;
    REP(tc, Tc){
        cout<<"Case #"<<tc+1<<": "<<Solve(tc == 89)<<endl;
    }
    return 0;
}
