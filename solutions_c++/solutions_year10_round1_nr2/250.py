...
int a[101];
int mem[102][256][2];
int get(int i,int p,bool isFirst)
{
	if(i==n)
		return 0;
	int &r=mem[i][p][isFirst];
	if(r!=-1)
		return r;
	r=1e9;
	if(!isFirst)
	{
		rep2(nc,max(p-m,0),min(p+m,255))
		{
			if(p!=nc)
				r=min(r,get(i,nc,0)+I);
			r=min(r,get(i+1,nc,0)+abs(a[i]-nc));
		}
		r=min(r,get(i+1,p,0)+d);
	}
	else
	{
		rep2(nc,0,255)
