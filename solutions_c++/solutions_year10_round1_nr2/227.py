...
int main( int argc, char* argv[] ) {	
	...
	for(int testCase=1;testCase<=testCases;testCase++){
		...
		rep(i,n){
			vi d1(256);
			rep(j,256)d1[j]=d[j]+del;
			if(m){
				//rep(j,256)checkmin(d1[a[i]],d[j]+max(0,abs(j-a[i])-1)/m*ins);
				rep(j,256)rep(k,256)checkmin(d1[k],d[j] + abs(k-a[i]) + max(0,abs(k-j)-1)/m*ins);
			}
			rep(j,256)rep(k,256)if(abs(j-k)<=m)checkmin(d1[k],d[j]+abs(k-a[i]));
