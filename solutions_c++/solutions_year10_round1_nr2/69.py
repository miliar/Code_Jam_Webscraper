...
int cost(int a, int b) {
	if (a > b) return cost(b, a);
	int ret = 0;

	if (a == b) return 0;
	if (M == 0) return 1000 * 2500;
	while (b - a > M) {
		a+=M;
		++ret;
	}
...
int solve(int pas, int lst) {
	if (pas == N) return 0;

	if (was[pas][lst]) return memo[pas][lst];

	int best = 1000 * 250;
	// delete
	best = min(best, D + solve(pas + 1, lst));
	for (int i = 0; i <= 255; ++i) best = min(best, cost(lst, i) +
			abs(v[pas] - i) + solve(pas + 1, i));
	
	was[pas][lst] = 1;
	return memo[pas][lst] = best;
}
void solve() {
	cin >> D >> I >> M >> N;
	...
	int ans = 1000 * 250;
	for (int i = 0; i <= 255; ++i) ans = min(ans, solve(0, i));
