...
int num[1000];
int del,ins,step;
int DP(int num[],int idx,int n,int pre)
{
	if(idx==n)return 0;
	if(dp[idx].find(pre)!=dp[idx].end())return dp[idx][pre];
	int ret=DP(num,idx+1,n,pre)+del;
	int cur=num[idx];
	if(step==0)
	{
		ret=min(ret,abs(pre-cur)+DP(num,idx+1,n,pre));
	}
	else
	{
		int ins_cost=(abs(cur-pre)-1)/step*ins;
		if(ins_cost<0)ins_cost=0;
		ret=min(ret,DP(num,idx+1,n,cur)+ins_cost);

		for(int k=0;(abs(k)-1)*ins<ret;k++)
		{
			int des=k*step+pre;
			int mid=abs(k)>1?(abs(k)-1)*ins:0;
			ret=min(ret,DP(num,idx+1,n,des)+abs(des-cur)+mid);
		}

		for(int k=-1;(abs(k)-1)*ins<ret;k--)
		{
			int des=k*step+pre;
			int mid=abs(k)>1?(abs(k)-1)*ins:0;
			ret=min(ret,DP(num,idx+1,n,des)+abs(des-cur)+mid);
...
int main()
{
	...
	for(cin>>tt;tt--;ca++)
	{
		...
		for(int i=0;i<n;i++)
		{
			for(int j=0;j<n;j++)num[j]=all[j];
			reverse(num,num+i);
			clear();
//			hline()
			int ret=DP(num,0,i,all[i]);
			clear();
//			hline();
			ret+=DP(num+i+1,0,n-i-1,all[i]);
	//		cerr<<i<<" "<<all[i]<<"="<<ret<<endl;
			ans=min(ans,ret);
		}
