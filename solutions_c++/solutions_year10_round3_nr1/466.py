...
readIn( ifstream& ifs ){
   ...
   for ( unsigned i = 0 ; i < nCases ; ++i ){
      ...
      string s1 = c1;
      N.push_back(str2Int(s1));
      wins[i].resize(str2Int(s1));
      for ( unsigned x = 0 ; x < str2Int(s1) ; ++x ){
         ...
         vector<int> ww;
         for ( unsigned y = 0 ; y < secondline.size() ; ++y ){
            ww.push_back(str2Int(secondline[y]));
         }
         wins[i][x]=ww;
      }
   }
