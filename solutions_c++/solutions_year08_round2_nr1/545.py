...
#define dlong signed long long int  /* 64b for unix    : %llu  */
...
int main () {
  ...
  FORU(i,0,N-1) {
    ...
    FORU(t1,0,T-1) {
      FORU(t2,t1+1,T-1) {
        FORU(t3,t2+1,T-1) {
          double cx = (TT[t1].x+TT[t2].x+TT[t3].x)/3.0;
          double cy = (TT[t1].y+TT[t2].y+TT[t3].y)/3.0;
          if (((int)cx == cx) && ((int)cy == cy)) ++res;
        }
      }
    }
