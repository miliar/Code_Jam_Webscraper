...
bool isinrange(matrix<char>& mat, int x, int y)
{
	if (x < 0)
		return false;
	if (y < 0)
...
bool ismine(matrix<char>& mat, int x, int y)
{
	if (!isinrange(mat, x, y))
		return false;

	return mat(x,y) == CM;
}

bool ishole(matrix<char>& mat, int x, int y)
{
	if (!isinrange(mat, x, y))
		return false;
...
int aroundmines(matrix<char>& mat, int x, int y)
{
	int i = 0;
	if (ismine(mat, x+1, y+0))i++;
	if (ismine(mat, x+1, y+1))i++;
	if (ismine(mat, x+0, y+1))i++;
	if (ismine(mat, x-1, y+1))i++;
	if (ismine(mat, x-1, y+0))i++;
	if (ismine(mat, x-1, y-1))i++;
	if (ismine(mat, x+0, y-1))i++;
...
bool iszero(matrix<char>& mat, int x, int y)
{
	if (!isinrange(mat, x, y))
		return false;

	return aroundmines(mat, x, y) == 0;
}

bool linkzero(matrix<char>& mat, int x, int y)
{
	if (iszero(mat, x+1, y+0))return true;
	if (iszero(mat, x+1, y+1))return true;
	if (iszero(mat, x+0, y+1))return true;
	if (iszero(mat, x-1, y+1))return true;
	if (iszero(mat, x-1, y+0))return true;
	if (iszero(mat, x-1, y-1))return true;
	if (iszero(mat, x+0, y-1))return true;
...
bool putmine(matrix<char>& mat, int& rest, int x, int y)
{
	if (x < 0 || x >= mat.size_x())
		return false;
	if (y < 0 || y >= mat.size_y())
...
bool fillmat(matrix<char>& mat, int M)
{
	...
	int holes = mat.size() - M;

	if (mat.size_x() == 1 || mat.size_y() == 1)
	{
		for (int i = 0; i < M; ++i)
		{
			mat[mat.size() - 1 - i] = CM;
	...
	for (int x = mat.size_x() - 1; x >= 2; --x)
	{
		for (int y = mat.size_y() - 1; y >= 2; --y)
		{
			if (putmine(mat, rest, x, y))
			{
				mat(0,0) = CC;
				return true;
			}
	...
	for (int x = mat.size_x() - 1; x >= 3; --x)
	{
		if (putmine(mat, rest, x, 0)) { assert(false); }
		if (putmine(mat, rest, x, 1))
		{
			mat(0,0) = CC;
			return true;
		}
	...
	for (int y = mat.size_y() - 1; y >= 3; --y)
	{
		if (putmine(mat, rest, 0, y)) { assert(false); }
		if (putmine(mat, rest, 1, y))
		{
			mat(0,0) = CC;
			return true;
		}
...
void printmat(matrix<char>& mat, ostream& fout)
{
...
void solve(istream& in, ostream& fout, int caseIdx)
{
	...
	OUTANS( << CaseHead(caseIdx+1) << endl );

	matrix<char> mat(W, H, CD);
	if (fillmat(mat, M))
	{
		printmat(mat, fout);
	}
