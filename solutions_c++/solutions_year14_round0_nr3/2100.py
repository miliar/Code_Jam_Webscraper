...
/*
const int N = 209, M = int(109 * 109 * 2);

int e[M], mm;

int D[N], hd[N], suc[M], to[M], cap[M], cst[M];
int n, m, s, t; LL flow, cost; int mask = 1023;

inline void add_edge(int x, int y, int c, int w){
...
inline void add_edgeec(int x, int y, int c, int w){
    add_edgee(x, y, c, 0), add_edgee(x, y, INF, w);
}
...
bool spfa(){
    cz = 0, op = 1; fill(inQ, inQ + n, 0), fill(D, D + n, INF);
...
pair<LL, LL> min_cost_max_flow(){
    to[m] = s; flow = 0, cost = 0; while (spfa()){
        add_path();
        //cout << flow << " " << cost << endl;
        if (check()) break;
        //cout << endl;
...
const int N = 109;
bool A[N][N], vis[N][N]; int cnt; int cc;

bool inGrid(int xx, int yy){
    return (xx >= 0 && yy >= 0 && xx < r && yy < cc);
}
...
void dfs(int x = 0, int y = 0){
    ...
    vis[x][y] = 1; ++cnt; if (C(x, y)) return;
    if (x+1 != r) dfs(x+1, y);
    if (y+1 != c) dfs(x, y+1);
    if (x+1 != r && y+1 != c) dfs(x+1, y+1);
...
bool ck(int t){
    ...
    int ii, jj; REP(i, r){
        REP(j, cc) if (bb < b) A[i][j] = 1, ++bb, ii = i, jj = j;
    }
    if (bb % cc == 1){
        A[ii][jj+1] = 1;
        A[ii-1][cc-1] = 0;
