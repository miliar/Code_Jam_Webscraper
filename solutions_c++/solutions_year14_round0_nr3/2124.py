...
bool recursive(int row,int col,int faka)
{
    int ans=0;
    kingloop ()
    {
        int newr = row + dx[li] , newc = col + dy [li];
        if(valid(newr,newc)&&matrix[newr][newc]==' ') ans++;
    }
    if(ans+faka<fix)
    {
        vector <int> RR , CC ;
        ...
        {
            int newr = row + dx[li] , newc = col + dy [li];
            if(valid(newr,newc)&&matrix[newr][newc]==' ') {RR.push_back(newr);CC.push_back(newc);matrix[newr][newc]='.';}
        }

        for(int ii=0;ii<RR.size();ii++)
        {
            bool h = false;
            int newr = RR[ii] , newc = CC[ii];
            h=recursive(newr,newc,ans+faka);
    ...
    else if(ans+faka==fix)
    {
        kingloop()
        {
            int newr = row + dx[li] , newc = col + dy [li];
            if(valid(newr,newc)&&matrix[newr][newc]==' ') matrix[newr][newc]='.';
        }
        for(int i=1;i<=R;i++)
        {
            for(int j=1;j<=C;j++)
...
int main()
{
    ...
    for(int cas=1;cas<=test;cas++)
    {
        ...
        for(int i=1;i<=R;i++)
        {
            ...
            {
                matrix[i][j]='c';
                if(recursive(i,j,1)) {f=true;break;}
                matrix[i][j]=' ';
            }
            if(f==true) break;
        }
