...
string solve(int r,int c,int m)
{
	...
	else if (min(r,c)==1)
	{
		...
		{
			out<<endl;
			rep(j,c) out<<(cnt==0?'c':cnt<m?'.':'*'),cnt++;
		}
	}
	else
	{
		vec<vvb> dp(r,vvb(c+1,vb(r*c+1)));
		vec<vvi> pre(r,vvi(c+1,vi(r*c+1)));
		ft(i,0,c) dp[0][i][i]=(i!=1);
		repf(i,1,r)
		{
			ft(j,0,c) if (j!=1)
			{
		...
		bool ans=false;
		ft(j,0,c) if (dp[r-1][j][m])
		{
			int i=r-1,s=m;
			while (i>=0)
			{
				out<<endl;
				rep(k,j) out<<(i==r-1&&k==0?"c":".");
				repf(k,j,c) out<<"*";
				int ss=s-j;
				int jj=pre[i][j][s];
				int ii=i-1;
