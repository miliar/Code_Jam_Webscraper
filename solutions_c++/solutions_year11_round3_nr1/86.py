...
inline bool ok(int i,int j){
	REP(k,4)	if(!(valid(i+di[k],j+dj[k])&&g[i+di[k]][j+dj[k]]==1))	return false;
	return true;
}
...
int main(){
	...
	while(t--){
		...
		REP(i,m){
			cin>>s;
			REP(j,n)	g[i][j]=(s[j]=='.')?0:1;
		}
		REP(i,m)	REP(j,n){
			if(ok(i,j)){
				int v=2;
				REP(k,4)	g[i+di[k]][j+dj[k]]=v++;
			}
		}
		bool possible=true;
		REP(i,m)	REP(j,n)	if(g[i][j]==1)	{possible=false;break;}
		printf("Case #%d:\n",kase);
		if(!possible){
			printf("Impossible\n");
		}
