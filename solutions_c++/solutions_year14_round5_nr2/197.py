...
//    cout << p << " " << q << endl;
    ...
    REP(i, SZ(I)){
        ...
        if (i == p && i == q){
            if (I[i].fi > P + Q){
                II.PB(MP(I[i].fi-P-Q, I[i].se));
            }
            else if (I[i].fi <= P) ++q;
        }

        else if (i == p){
            if (I[i].fi > P) II.PB(MP(I[i].fi-P, I[i].se));
        }

        else if (i == q){
            if (I[i].fi > Q) II.PB(MP(I[i].fi-Q, I[i].se));
        }
        else {
            II.PB(MP(I[i].fi, I[i].se));
        }
        cout << i << " " << p << " " <<q << endl;
...
int main(){
    ...
    Rush{
        I.clear(); RD(P, Q, n); REP(i, n){
            RD(h[i], g[i]);
            I.PB(MP(h[i], g[i]));
        }
        p = 0, q = 1; H[p].clear(); H[p][I] = 0;

        int res = 0; while (!H[p].empty()){
            swap(p, q); H[p].clear();
            ...
            ECH(it, H[q]){
                ...
                I.clear(); FOR(i, 0, SZ(it->fi)) I.PB(it->fi[i]);
                ...
                if ((it->fi).empty()) continue;
                FOR(i, -1, SZ(it->fi)){
                    int vv = it->se + ( (i != -1 && I[i].fi <= P) ? I[i].se : 0);
                    //cout << " " << i << " " << vv << endl;


                    //cout << '?' << endl;
                    checkMax(H[p][s(i, 0)], vv); checkMax(res, vv);
...
int main() {
	...
	Rush{
		RD(p, q, n);
		REP(i, n){
			RD(x, g[i]); ot[i] = (x + q - 1) / q, rr[i] = ot[i] - 1;
			md[i] = (x - rr[i] * q + p - 1) / p;
		}
		...
		REP(i, n){
			REP(j, M) if (~dp[i][j]){
				...
				if (jj >= 2005) jj = 2004;
				// 1
				if (jj >= 0)  checkMax(dp[i + 1][jj], dp[i + 1][jj], dp[i][j] + g[i]);
				// 0
				jj = j + ot[i]; if (jj >= 2005) jj = 2004;
                checkMax(dp[i + 1][jj], dp[i + 1][jj], dp[i][j]);
			}
		}

		int ans = 0; REP(j, M) checkMax(ans, dp[n][j]);
		OT(ans);
	}
}
