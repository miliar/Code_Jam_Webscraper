...
void initializer(){
    fact[0] = 1;
    rep(i,100) fact[i+1] = (i+1)*fact[i]%mod;
}
//////////////////////////////////////////////////////////////////////////////////////////////////
...
void solve(){
    ...
    rep(i,N){
        ...
        have[i] = 0;
        rep(j,seq[i].size())
            have[i] |= 1LL << (seq[i][j]-'a');
    }
    rep(i,N) rep(j,N) if(i != j){
        int c = __builtin_popcountll(have[i] & have[j]);
        if(c == 0 or (seq[i][seq[i].size()-1] == seq[j][0] and c == 1))
            G[i][j] = 1;
    }
    do{
        used = 0;
