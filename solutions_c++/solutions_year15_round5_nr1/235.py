...
//    int id;
//    int s;
//    bool operator < (const Manager & A) const{
//        return s > A.s;
//    }
//};
//priority_queue<Manager> q;
int ans , now;
void remove(int x){
//    if (!inside[x]) return;
    if (inside[x]) now--;
...
void solve(){
    ...
    salaryList.PB(s[0]);
    for (int i = 0 ; i < n - 1 ; ++i){
        s[i + 1] = (1ll * as *s[i] + cs) % rs;
        salaryList.PB(s[i + 1]);
        manager[i + 1] = (1ll * manager[i] * am + cm) % rm;
    ...
    for (int i = 1 ; i < n ; ++i)
        manager[i] %= i;
//    for (int i = 1 ; i < n ; ++i){
//        cerr << "E " << s[i] << ' ' << manager[i] << endl;
//    }
    UNQ(salaryList);
    for (int i = 0 ; i < n ; ++i){
        s[i] = lower_bound(ALL(salaryList) , s[i]) - salaryList.begin();
        adds(s[i] , i);
        if (i) addedge(manager[i] , i);
    }
    int m = SZ(salaryList);
    int tail = -1;
    ans = 0;
...
//        cerr << "i " << i << endl;
        if (i > 0){
            int de = i - 1;
            for (int j = headsa[de] ; j != -1 ; j = sa[j].next)
            if (inside[sa[j].to]){
                remove(sa[j].to);
            }
        }
        while(tail + 1 < m && salaryList[tail + 1] - salaryList[i] <= d){
//                cerr << "tail " << tail << endl;
            ++tail;
            for (int j = headsa[tail] ; j != -1 ; j = sa[j].next){
//                    cerr << "check " << sa[j].to << ' ' << manager[sa[j].to] << ' ' << inside[manager[sa[j].to]] << endl;
                if (sa[j].to == 0 || inside[manager[sa[j].to]]){
                    add(sa[j].to , i , tail);
                }
            }
        }
        if (s[0] <= tail) checkMax(ans , now);
    }
    printf("Case #%d: %d\n" , ++Case , ans);
}
int main(){
