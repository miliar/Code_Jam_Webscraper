...
bool marked[1005000];
ll s[1005000];
ll open[1005000], close[1005000];
int n;


void dfs(int x, ll o, ll c)
{
	o = min(o, s[x]);
	c = max(c, s[x]);
	open[x] = o;
	close[x] = c;
	foria(graph[x]) dfs(graph[x][i], o, c);
}
...
//	ofstream cout("output.txt");
	...
	fort
	{
		...
		forin if (i)
		{
			s[i] = (s[i-1] * as + cs) % rs;
			m[i] = (m[i-1] * am + cm) % rm;
			graph[m[i] % i].push_back(i);
		}
		dfs(0, s[0], s[0]);

		vector<pair<ll, bool> > vp;
		forin if (close[i] - open[i] <= d)
		{
			vp.push_back(make_pair(close[i] - d, false));
			vp.push_back(make_pair(open[i], true));
		}
		...
		cout << "Case #" << gett << ": " << result << endl;
		std::cout << "Case #" << gett << ": " << result << endl;
	}
