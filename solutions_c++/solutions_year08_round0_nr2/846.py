...
{ 
  ...
  REP(ctest, ntest)
  {
    ...
    VI events[2] = {VI(26 * 60, 0), VI(26 * 60, 0)};

    REP(i, na)
    {
      int hfroma, mfroma, htob, mtob;
      scanf("%d:%d %d:%d", &hfroma, &mfroma, &htob, &mtob);

      int froma = hfroma * 60 + mfroma;
      int tob = htob * 60 + mtob;
    ...
    REP(i, nb)
    {
      int hfromb, mfromb, htoa, mtoa;
      scanf("%d:%d %d:%d", &hfromb, &mfromb, &htoa, &mtoa);

      int fromb = hfromb * 60 + mfromb;
      int toa = htoa * 60 + mtoa;

      events[1][fromb] -= 1;
      events[0][toa + t] += 1;
    }
    
    printf("Case #%d:", ctest + 1);
