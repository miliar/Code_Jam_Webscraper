...
float rec(char a[3],char b[3],int t)
{
	int x =u(b)+t;
	if(x>=60)
		return(u(a)+1+(((float)(x-60))/100));
	else
	return(u(a)+(((float)x)/100));
}
int main()
{
	...
	for(int z =0;z<N;z++)
	{
		...
		for(int i=0;i<NA;i++)
		{
			...
			tA.push_back(rec(a,b,0));
			fscanf(fin,"%c%c:%c%c\n",&a[0],&a[1],&b[0],&b[1]);
			eB.push_back((rec(a,b,turn)));
			
		}
		for(int i=0;i<NB;i++)
		{
			fscanf(fin,"%c%c:%c%c ",&a[0],&a[1],&b[0],&b[1]);
			tB.push_back(rec(a,b,0));
			fscanf(fin,"%c%c:%c%c\n",&a[0],&a[1],&b[0],&b[1]);
			eA.push_back((rec(a,b,turn)));
		}
