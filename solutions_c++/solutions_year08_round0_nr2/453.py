...
unsigned long long lrand(){ return ((((unsigned long long)rand())<<32) | rand());}

long long LSB(int num, long long data){
   assert(num==-1); return data;
}
...
unsigned long long lrand(int seed){
  if(!_seeded) _seeded=true,srand((seed==-1)?time(0):seed);
  return ((((unsigned long long)rand())<<32) | rand());
}

unsigned long long toHexLong(string str){
   unsigned long long num = 0;
   FOR(i,0,str.length()) { num<<=4; if(isalpha(str[i])) num += (str[i]-'A'+10); else num += (str[i]-'0');}
   return num;
}

string toHexString(unsigned long long num) {
  string ret;
  FOR(k,0,16) { ret += _hexdigits[((num>>(k<<2))&0xF)]; }
...
struct bigint{
private:
   int size; long long *data;   // in reverse order
   bool negative;

   int abscmp(const bigint &rhs) const{
         int mn = min(rhs.size,size);
         for(int j=size-1;j>=mn;j--) if(data[j]!=0) return 1;
         for(int j=rhs.size-1;j>=mn;j--) if(rhs.data[j]!=0) return -1;
         for(int j=mn-1;j>=0;j--) if(data[j]!=rhs.data[j]) return ((unsigned long long)(data[j])>(unsigned long long)(rhs.data[j]))?1:-1;
         return 0;
   }

   bigint add(const bigint &rhs){
        bigint mx = (rhs.size>=size)?rhs:(*this), mn = (rhs.size>=size)?(*this):rhs; int c = 0;
        FOR(i,0,mn.size) { mx.data[i] += mn.data[i] + c; c = (mx.data[i]>=0 && (rhs.data[i]<0||data[i]<0)) ? 1 : 0;}
        FOR(i,mn.size,mx.size){ mx.data[i] += c; c = (c==1&&mx.data[i]==0)?1:0;}
        if(c) { bigint nx(0,mx.size+1); memcpy(nx.data,mx.data,sizeof(mx.data)*mx.size); nx.data[nx.size-1] = 1; return nx;}
        return mx;
   }

   bigint sub(const bigint &rhs){
          int borrow = 0; bigint ret = (*this); ret.negative = false;
          int end = rhs.size-1; while(end > 0 && data[end]==0) end--;
          FOR(i,0,end+1){
             unsigned long long A = ret.data[i]-borrow, B = rhs.data[i]; int nborrow = (borrow && ret.data[i]==0)?1:0;
             if(A >= B) ret.data[i] = A-B; else nborrow = 1, ret.data[i] = 0-B+A; borrow = nborrow;
          }
          return ret;
   }
...
public:
   ...
   bigint(string str){
        negative = false; if(str[0] == '-') negative = true, str = str.substr(1); bool zero = true;
        assert((str.length()+1)%17==0); size = (str.length()+1)/17; data = new long long[size];
        FOR(i,0,size) { data[size-i-1] = toHexLong(str.substr(i*17,16)); zero = zero && data[size-i-1]==0;}
        if(zero) negative = false;
   }

   ~bigint() { delete [] data; data = NULL;}
   bigint operator=(const bigint &rhs){ size = max(size,rhs.size); delete [] data; data = new long long[size]; negative = rhs.negative;
                                        memset(data,0,sizeof(data)*size); memcpy(data,rhs.data,sizeof(rhs.data)*rhs.size); return (*this);}
   ...
   bigint operator<<(int num){
      ...
      bigint ret(0,size+added+1);
      int dest = added, mask = 0;

      for(int src = 0; src<size; src++, dest++){
            ret.data[dest] = (data[src]<<num) + mask;
            mask = data[src] - LSB(64-num,data[src]);
        }
   ...
   bigint operator>>(int num){
      ...
      bigint ret(0,max(1,size - removed));
      int dest = ret.size-1;
      unsigned long long mask = 0;
      for(int src = size-1; src>=0,dest>=0; src--, dest--){
            ret.data[dest] = LSB(64-num,(data[src]>>num)) | (mask<<(64-num));
            mask = LSB(num,data[src]);
        }
      return ret;
   }

   bigint operator+(const bigint &rhs){
        if(negative!=rhs.negative) switch(abscmp(rhs)){ case 0: return 0; case 1: return (abs(*this) - abs(rhs)) * (negative?-1:1); case -1: return (abs(rhs) - abs(*this)) * (rhs.negative?-1:1);}
        bigint ret = add(rhs); ret.negative = negative; return ret;
   }

   bigint operator-(const bigint &rhs){
        if(negative != rhs.negative) return (abs(*this) + abs(rhs)) * (negative?-1:1);
        switch(abscmp(rhs)){
                case 0: return 0;
                case 1: return abs(*this).sub(abs(rhs)) * (negative?-1:1);
                case -1: return abs(rhs).sub(abs(*this)) * (negative?1:-1);
        }
        return 0;
   }

   bigint operator*(const bigint &rhs){
        int sz = max(size,rhs.size);
        long long *tempA = new long long[2*sz], *tempB = new long long[2*sz], *tempC = new long long[4*sz];
        memset(tempA,0,sizeof(tempA)*2*sz); memset(tempB,0,sizeof(tempB)*2*sz); memset(tempC,0,sizeof(tempC)*4*sz);

        FOR(i,0,size) tempA[i*2] = LSB(32,data[i]), tempA[i*2+1]=LSB(32,(data[i]>>32));
        FOR(i,0,rhs.size) tempB[i*2] = LSB(32,rhs.data[i]), tempB[i*2+1]=LSB(32,(rhs.data[i]>>32));

        for(int d=0;d<sz*4-1;d++) for(int i=max(d-sz*2+1,0);i<sz*2;i++){
            int j = d-i; if(j >= 2*sz || j < 0) continue; long long res = tempA[i]*tempB[j]; tempC[i+j] += LSB(32,res), tempC[i+j+1] += LSB(32,(res>>32));}

        long long carry = 0; FOR(i,0,sz*4){ tempC[i] += carry; carry = LSB(32,(tempC[i]>>32)); tempC[i] = LSB(32,tempC[i]);}
        int j; for(j=sz*4-1;j>0&&tempC[j]==0;j--);


        bigint ret(0,(j/2)+1); FOR(i,0,ret.size) ret.data[i] = tempC[i*2] + (tempC[i*2+1]<<32);
        delete [] tempA; delete [] tempB; delete [] tempC; ret.negative = (negative!=rhs.negative);
        return ret;
        }
   ...
   void div(const bigint &rhs, bigint &D, bigint &R){
            assert(rhs!=0); D = R = 0;
            switch(abscmp(rhs)){
                case -1: R = (*this); break; case 0: D = 1; break;
                case 1: { bigint low = 0, high = (*this);
                    while(true){
                       bigint k = (high-low+1); bigint k2 = low + (k>>1);
                       D = low + ((high - low + 1)>>1);
                       bigint mult = D*rhs;
                       R = (*this)-(D*rhs);
                       if(R < 0) high = D; else if(R >= rhs) low = D; else return;
                    }
                }
            }
        }
   ...
   bigint operator/(const bigint &rhs){
        bigint d, r;
        abs(*this).div(abs(rhs),d,r);
        if(negative!=rhs.negative && d!=0) d.negative = true;
        return d;
   }
   ...
   static bigint rand(const bigint &upperInc){
         ...
         unsigned long long first = LSB(k,LLMAX); bigint ret(0,upperInc.size);

         for(int j=s;j>=0;j--){ unsigned long long lr = lrand();
   ...
   string toString() const{ int j; string ret;
          for(j=size-1;j>0&&(data[j]==0);j--);
          for(;j>=0;j--) ret += ":" + toHexString(data[j]);
          ret = ret.substr(1);
          if(negative) ret = "-" + ret;
          return ret;
        }
...
bigint bigint_choose(long long T, long long B){
  bigint ret = 1;
  vector<long long> top, bottom; if(T<B || B<0) return -1; B = min(B,T-B);
  FOR(i,0,B){ top.push_back(T-i); bottom.push_back(i+1); }
  FOR(i,0,top.size()) FOR(j,0,bottom.size()){
      if(top[i]==1) break; long long g = gcd(top[i],bottom[j]);
      top[i]/=g; bottom[j]/=g; if(bottom[j]==1){ swap(bottom[bottom.size()-1],bottom[j--]); bottom.pop_back();} }
  FOR(i,0,top.size()) ret*=top[i];
  return ret;
}
...
int flood(int r, int c, int nR, int nC, bool (*invalid)(int,int)){ if(r<0 || c<0 || r>=nR || c>=nC || floodmatrix[r][c] || invalid(r,c)) return 0; floodmatrix[r][c] = true; int ret =1;      FOR(i,0,4) ret += flood(r+fdr[i],c+fdr[i],nR,nC,invalid); return ret;}
// END CUT - Flood Fill
...
struct route{
   int start, end;
   bool beginA;
   bool operator<(const route &rhs) const{ return start < rhs.start;}
};

int toNum(string str){ return atoi(str.substr(0,2).c_str()) * 60 + atoi(str.substr(3,2).c_str()); }
vector<bool> done;
vector<route> routes;
int numleft;
...
bool assignTrain(){
  ...
  bool ret = routes[pos].beginA;

  bool last; int lTime; 
  do{
    last = routes[pos].beginA; lTime = routes[pos].end + T;
    done[pos] = true; numleft--;

    while(pos < routes.size() && !(!done[pos] && routes[pos].beginA != last && routes[pos].start >= lTime)) pos++;
...
int main(){
   ...
   FOR(i,0,N){
	   cin >> T;
	   cin >> NA >> NB;
	   done.clear(); routes.clear();

	   string str;
	   numleft = NA + NB;
	   int cntA = 0, cntB = 0;

	   FOR(j,0,NA){ route r; cin >> str; r.beginA=true; r.start = toNum(str); cin >> str; r.end=toNum(str); routes.push_back(r); done.push_back(false);}
	   FOR(j,0,NB){ route r; cin >> str; r.beginA=false; r.start = toNum(str); cin >> str; r.end=toNum(str); routes.push_back(r); done.push_back(false);}
	   sort(BEGEND(routes));

	   while(numleft>0){
		   if(assignTrain()) cntA++; else cntB++;
		}
	   cout << "Case #" << (i+1) << ": " << cntA << " " << cntB << endl;
	}
   return 0;
}
