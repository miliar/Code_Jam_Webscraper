...
void read(int n, vector<int>&dep, vector<int>&arr, int delay) {
	FN(i,n) {
		int h,m; char p;
		cin>>h>>p>>m;
		dep.push_back(h*60+m);
//cout<<"(dep: "<<h<<p<<m<<" "<<(h*60+m)<<")";
		cin>>h>>p>>m;
//cout<<"  arr: "<<h<<p<<m<<endl;
		arr.push_back(h*60+m+delay);
	}
}
int sim(vector<int> dep, vector<int> arr) {
	int t=0, d=0, a=0, need=0;
	while (d<SZ(dep)) {
//cout<<"-"<<a<<SZ(arr)<<endl;
		int ta=a<SZ(arr) ? arr[a] : 1000000, td = dep[d];
		if (ta<=td) {
			t++;
			a++;
		} else {
...
int main(int argc, char* argv[])
{
	...
	for (int ca=0; ca<N; ca++) {
		...
		read(ab, depA, arrB, T);
		read(ba, depB, arrA, T);
		sort(all(depA));sort(all(depB));sort(all(arrA));sort(all(arrB));
		int na=sim(depA,arrA);
		int nb=sim(depB,arrB);
		cout<<"Case #"<<ca+1<<": "<<na<<" "<<nb<<endl;
	}
	return 0;
}
// END CUT HERE 
