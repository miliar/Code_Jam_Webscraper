...
{
	int x,y,tempval,eartempval,earyval,tmp;
	
	int incrflag=0,thistime=0;
	...
	for(x=0;x<na;x++)
	{
		...
		for(y=0;y<nb;y++)
		{
			...
			if((tmp+tat)>=60)
			{
				tempval=(checkarr2[y])/100;
				tempval=tempval++;
				tempval=tempval*100;
				tempval=tempval+(tmp+tat-60);
			}
			else
			{
...
void main()
{
	...
	fp=fopen("E:\\Amol\\CodeJam\\Train\\B-large.txt","r");
	if(fp==NULL)
		printf("error");

	fp2=fopen("E:\\Amol\\CodeJam\\Train\\B-large-out.txt","w");
	if(fp==NULL)
		printf("error2");
