...
pii calc(deque<int>& depa,deque<int>& arra,deque<int>& depb,deque<int>& arrb){
...
*/
    ...
    for(int tt=0;tt<24*60;++tt){
        while(!arra.empty() && arra.front()==tt){
            arra.pop_front();
            ++curb;
        }
        while(!arrb.empty() && arrb.front()==tt){
            arrb.pop_front();
            ++cura;
        }
        while(!depa.empty() && depa.front()==tt){
            depa.pop_front();
            if(cura==0)++cura,++tra;
            --cura;
        }
        while(!depb.empty() && depb.front()==tt){
            depb.pop_front();
            if(curb==0)++curb,++trb;
            --curb;
        }
...
void solve(){
    ...
    pii p=calc(depa,arra,depb,arrb);
    printf("%d %d\n",p.first,p.second);
}
