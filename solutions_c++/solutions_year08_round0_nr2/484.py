...
int main(){
	int t = io.readInt<int>(), tt, i;
	io.setWhite(":", true);
	FUP(tt,1,t){
		int sa, sb;
		sa=sb=0;
		int T = io.readInt<int>();
		int na = io.readInt<int>();
		int nb = io.readInt<int>();
		p.clear();
		a1.clear();
		a2.clear();
		FUP(i,0,na-1){
			int t1 = io.readInt<int>();
			t1 = t1*60 + io.readInt<int>();
			int t2 = io.readInt<int>();
			t2 = t2*60 + io.readInt<int>();
			p.insert(mp(t1, mp(t2, 0)));
		}
		FUP(i,0,nb-1){
			int t1 = io.readInt<int>();
			t1 = t1*60 + io.readInt<int>();
			int t2 = io.readInt<int>();
			t2 = t2*60 + io.readInt<int>();
			p.insert(mp(t1, mp(t2, 1)));
		}
		ITERATE(it, p){
			int t1 = it->first;
			int t2 = it->second.first;
			if (it->second.second == 0){
				if (a1.size() && *a1.begin()<= t1)
					a1.erase(a1.begin());
				else
					sa++;
