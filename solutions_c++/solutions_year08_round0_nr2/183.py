...
void solve()
{
	...
	while(NA--)
	{
		int partida = readtime();
		int chegada = readtime() + turntime;
	...
	for(int i=1; i<size(vecA); i++) vecA[i] += vecA[i-1];
	for(int i=1; i<size(vecB); i++) vecB[i] += vecB[i-1];

	cout << -*min_element( all(vecA) ) << " " << -*min_element( all(vecB) ) << endl;
}
