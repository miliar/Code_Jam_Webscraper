...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	FILE* file;
	FILE* file2;
	char* filename = "D:\\B-large.txt";
    char* filename2 = "D:\\B-large result.txt";
	int turnaround_time;
	int fromA;
	int fromB;
	...
	for(int i=0;i<n;i++)
	{
	   ...
	   int arrangebuffer;
	   for(int time = 0; time<fromA-1;time++ )  // arrange NA
	   {
		  exhangeflag = false;
		  for(int rangenum=0;rangenum<fromA-1-time; rangenum++)
	      {
              if(NA[rangenum][1]>NA[rangenum+1][1])  // arrange by arrive time 
		      {
	   ...
	   for(int time = 0; time<fromA-1;time++ )  // arrange NA2
	   {
		  exhangeflag = false;
		  for(int rangenum=0;rangenum<fromA-1-time; rangenum++)
	      {
              if(NA2[rangenum][0]>NA2[rangenum+1][0])   // arrange by departure time 
		      {
	   ...
	   for(int time = 0; time<fromB-1;time++ )  // arrange NB
	   {
		  exhangeflag = false;
		  for(int rangenum=0;rangenum<fromB-1-time; rangenum++)
	      {
              if(NB[rangenum][1]>NB[rangenum+1][1])   // arrange by arrive time 
		      {
	   ...
	   for(int time = 0; time<fromB-1;time++ )  // arrange NB2
	   {
		  exhangeflag = false;
		  for(int rangenum=0;rangenum<fromB-1-time; rangenum++)
	      {
              if(NB2[rangenum][0]>NB2[rangenum+1][0])  // arrange by departure time
		      {
