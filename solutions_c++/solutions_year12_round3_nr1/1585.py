...
typedef std::map<int, int>  Knoop;      // beeldt knoopnummer (van buur) af op verbindingsnummer
	...
	/** Omslachtige methode, kan VEEL korter
	void draaiGraafOm(const Graaf<RT> &origineel) {
		this->wis();
		//Zorgen dat we gelijk aantal knopen hebben.
		while (this->aantalKnopen() < origineel.aantalKnopen())
			this->voegKnoopToe();


		vector<bool> gezien(origineel.aantalKnopen());
		for (int i = 0; i < origineel.aantalKnopen(); i++) {
			if (gezien[i] == false) {
				stack<int> stapel;
				stapel.push(i);
				gezien[i] = true;
				while (!stapel.empty()) {
					...
					Knoop buren = origineel[nr];
					for (Knoop::iterator it = buren.begin(); it != buren.end(); it++) {
						this->voegVerbindingToe(it->first, nr);
						if (!gezien[it->first]) {
							stapel.push(it->first);
	...
	//Kortere methode:
	void draaiGraafOm2(const Graaf<RT> &origineel) {
		while (this->aantalKnopen() < origineel.aantalKnopen())
			this->voegKnoopToe();

		for (int van=0;van<origineel.aantalKnopen();van++) {
			Knoop k;
			for (std::map<int, int>::const_iterator it=origineel.knopen[van].begin(); it!=origineel.knopen[van].end(); it++){
				assert(it->first < origineel.aantalKnopen());
				this->voegVerbindingToe(it->first, van);
			}
		}
	}


	void bepaalDiepteEerstNummerHulp(std::stack<int> &prio, int knoopNummer, std::vector<bool> &gezien) {
		Knoop buren = (*this)[knoopNummer];
		for (Knoop::iterator it = buren.begin(); it != buren.end(); it++) {
			if (!gezien[it->first]) {
				gezien[it->first] = true;
				bepaalDiepteEerstNummerHulp(prio, it->first, gezien);
			}
		}
		prio.push(knoopNummer);
	}

	void bepaalDiepteEerstNummer(std::stack<int> &prio) {
		vector<bool> gezien(this->aantalKnopen());
		for (int i = 0; i < this->aantalKnopen(); i++) {
			if (!gezien[i]) {
				gezien[i] = true;
				bepaalDiepteEerstNummerHulp(prio, i, gezien);
			}
		}
	}

	//Kan dit korter?
	void nummerComponenten(std::stack<int> &prio) {
		componentnummer.clear();
		componentnummer.resize(this->aantalKnopen() );

		int componentTeller = 0; //Nummer 0 == reserved voor de 0 v/d initialisatie van componentennummer
		while (!prio.empty()) {
			int knoopNummer = prio.top();
			prio.pop();

			if (componentnummer[knoopNummer] == 0) {
				//Nog maar is diepte eerst toepassen.
				//Nummer alle knopen met huidig componenten nummer die nog geen componenten nummer hebben.
				componentTeller++;
				stack<int> stapel;
				stapel.push(knoopNummer);
				componentnummer[knoopNummer] = componentTeller;
				...
				while (!stapel.empty()) {
					...
					Knoop buren = (*this)[nr];
					for (Knoop::iterator it = buren.begin(); it != buren.end(); it++) {
						if (componentnummer[it->first] == 0  ) {
							componentnummer[it->first] = componentTeller;
							stapel.push(it->first);
						}
					}
				}
		...
		std::cout<<"Het aantal componenten dat we hebben: "<<componentTeller<<std::endl;
	}

	void bepaalComponentNummer() {
		//Graaf omdraaiien.
		Graaf<RT> gedraaid;
		gedraaid.draaiGraafOm2(*this);
		stack<int> prio;
		gedraaid.bepaalDiepteEerstNummer(prio);
		this->nummerComponenten(prio);
	...
	//Kan het efficienter ? Zonder bepalen van aantal samenhangende comp.
	void wordt_componentengraaf_van(const Graaf& origineel) {
		this->wis();

		//Bepaal #Samenhangende comp.
		int aantalSamenComp = origineel.componentnummer[0];
		for (int i = 0; i < origineel.aantalKnopen(); i++) {
			aantalSamenComp = max(aantalSamenComp, origineel.componentnummer[i]);
		}

		while (this->aantalKnopen() <= aantalSamenComp)
			this->voegKnoopToe();

		for (int i = 0; i < origineel.aantalKnopen(); i++) {
			Knoop buur = origineel[i];
			for (Knoop::iterator it = buur.begin(); it != buur.end(); it++) {
				int van = origineel.componentnummer[i];
				int naar = origineel.componentnummer[it->first];
				if (van != naar) {
					//Controlleer of we verbinding nog niet bestaat.
					if ((*this)[van].count(naar) == 0)
						this->voegVerbindingToe(van, naar);//Toevoegen.
				}
			}
		}
...
protected:
	...
	std::vector<int>	componentnummer;
};
