/*
Date: 2017/04/15 09:15:32 Saturday
*/
#include<bits/stdc++.h>
using namespace std;
#define X first
...
int main(){
	...
	scanf("%d",&C);rep(_,1,C){
		...
		rep(i,0,100)rep(j,0,100){
			int cnt=0,HD=hd,HK=hk,AD=ad,AK=ak;
			rep(_,1,j){
				int tmp=AK;
				AK=max(AK-D,0);++cnt;
				if(HD<=AK)HD=hd-tmp-AK,++cnt;
				else HD-=AK;
				if(HD<=0){cnt=inf;break;}
			}
			if(cnt==inf)continue;
			rep(_,1,i){
				AD+=B;++cnt;
				if(HD<=AK)HD=hd-AK-AK,++cnt;
				else HD-=AK;
				if(HD<=0){cnt=inf;break;}
			}
			if(cnt==inf)continue;
			while(1){
				if(AD==0){cnt=inf;break;}
				HK-=AD;++cnt;if(HK<=0)break;
				if(HD<=AK)HD=hd-AK-AK,++cnt;
				else HD-=AK;
				if(HD<=0){cnt=inf;break;}
			}
