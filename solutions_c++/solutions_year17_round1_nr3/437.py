...
struct Node {
  ...
  bool operator<(const Node& x) const {
    return make_pair(make_pair(h1,h2),make_pair(a1,a2)) < make_pair(make_pair(x.h1,x.h2),make_pair(x.a1,x.a2));
  }
};

int main(){
  SF(out/C-small-attempt0.in);
  PF
  int T, hd, ad, hk, ak, b, d, res;
  S(T);
  while (T--) {
    S(hd, ad, hk, ak, b, d);
    map<Node, int> mp;
