...
const int len = 500;
long long icombs[len + 1][len + 1];
void inchoosem()
{		
	memset(icombs, 0, (len  + 1) * (len + 1));
	for (int i = 0; i <= len; ++i)
	{
		icombs[i][0] = 1;			 
	}	 
	...
	{
		for (int j = 1; j <= i; ++j)
			icombs[i][j] = (icombs[i - 1][j - 1] + icombs[i - 1][j]) % 100003;
	}
}
...
void gcj3()
{
	...
	for (int n = 3; n < sz; ++n)
	{
		...
		for (int m = 2; m < n; ++m)
		{
			...
			long long tmp = 0;
			for (int k = max(2 * m - n, 1); k < m; ++k)
			{
				tmp = (tmp + dp[m][k] * icombs[n - m - 1][m - k - 1]) % 100003;
			}
			dp[n][m] = tmp;
			num[n] = (num[n] + tmp) % 100003;
		}		
	}
