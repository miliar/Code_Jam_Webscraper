...
{
	const int MAX = 1000;
	static std::vector< std::vector<ll> > cache(MAX, std::vector<ll>(MAX, -1));
	if (k == 0) return 1;
	if (n == 0) return 0;
	if (cache[n][k] == -1)
	{
		cache[n][k] = choose(n-1, k-1) + choose(n-1, k);
		if (mod != 0) cache[n][k] %= mod;
	}
	return cache[n][k];
}
using namespace std;
...
// # of pure subsets of {2,3...n} with c elements.
int solve(int n, int c)
{
	...
	// Now need # of subsets of size c containing c and n.
	// {{L}, c, {R}, n}
	int r = 0;
	for (int nl = 0; nl <= c - 2; ++nl)
	{
		int nr = c - 2 - nl;
		int rc = n - c - 1;
		r += solve(c, nl + 1) * choose(rc, nr, mod);
		r %= mod;
	}
