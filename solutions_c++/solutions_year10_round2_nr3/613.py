...
const int CHN = 500;
vector<vector<ll> > Ch(CHN, vector<ll>(CHN,0));
void ini_ch()
{
    Ch[0][0]=1;
    FOR( i, 1, CHN)
...
int doing ( int tail, int length)
{
    if( his[tail][length] != -1)
        return his[tail][length];
    int sum = 0;
    FOR( i, 1, length)
    {
        int p = doing( length, i );
       int mm = tail - length -  1;
       int nn = length-i-1;
        int q = (nn<=mm) ?   ( Ch[mm][nn]) : 0;
        if(0)
        {
            db(tail);
            db(length);
    ...
    cout<< "   in doning " << tail <<" " << length <<" " << sum <<endl;
    return (his[tail][length] = sum);
}
