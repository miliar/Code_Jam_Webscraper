...
#define close      freopen ("B-small.out","w",stdout)
...
//                cout << i << endl;
                ...
                for (int j=0;j<n;j++){
                    for (int k=0;k<m;k++){
                        if (!Check(i,id[j][k]))    continue;
                        int x,y,chk = 0;

                        x = j;
                        y = k+1;

                        if (OK(n,m,x,y) && Check(i,id[x][y]))       chk++;      x = j+1;y = k;
                        if (OK(n,m,x,y) && Check(i,id[x][y]))       chk++;      x = j;y = k-1;
                        if (OK(n,m,x,y) && Check(i,id[x][y]))       chk++;      x = j-1;y = k;
                        if (OK(n,m,x,y) && Check(i,id[x][y]))       chk++;
