...
static double memo[1<<20];

double Case::calc(unsigned free)  // memoize!
{
...
{
    SET(memo, -1);
    unsigned free = (1u<<N) - 1;
    REP(i, N) if (S[i] == 'X') free -= 1u<<i;
    cout << calc(free) << endl;
    result.set(STR(calc(free)));
}
