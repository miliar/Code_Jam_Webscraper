...
inline DB getTime(){
    ...
    timeval tv;
    gettimeofday(&tv, 0);
    return tv.tv_sec + tv.tv_inQc * 1e-6;
#endif
}
...
void dfs(int u){
	inQ[u] = 1, ++D[u];
	ECH(it, adj[u]) if (!inQ[v] && D[u] == D[v]) dfs(v);
}

void Bellman_Ford(){

    deque<int> Q(1, n); RST(inQ); inQ[n]=1;
	while(!Q.empty()){
		int u = Q.front(); Q.pop_front();

		ECH(it, adj[u]) if (D[v] < D[u] + 1){
            D[v] = D[u] + 1; if (!inQ[v]) inQ[v] = 1, Q.PB(v);
        }
        inQ[u] = 0;
	}
...
/*#ifndef ONLINE_JUDGE
    ...
    Rush{
        ...
        RD(n); REP(i, n+1) CLR(adj[i]);

        REP(i, n){

            RD(A[i]), adj[n].PB(i);

            DWN(j, i, 0) if (A[i] == A[j]){
                adj[i].PB(j);
                break;
            }
            DWN(j, i, 0) if (A[i] == A[j] + 1){
                adj[j].PB(i);
                break;
            }
        }
