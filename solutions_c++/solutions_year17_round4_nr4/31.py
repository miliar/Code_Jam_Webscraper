...
public:
    ...
    void solve() {
        vector<pii> sol, tur;
        REP(i, r) REP(j, c) {
                if (f[i][j] == 'S') sol.eb(i, j);
                if (f[i][j] == 'T') tur.eb(i, j);
            }
        int ns = sz(sol), nt = sz(tur);
        vector<vector<bool>> reach;
        REP(mask, 1 << nt) {
            vector<int> shoot[N][N];
            REP(i, r) REP(j, c) shoot[i][j] = {};
            REP(u, nt) if (mask >> u & 1) {
                    int x0 = tur[u].X, y0 = tur[u].Y;
                    ...
                    while (y + 1 < c && f[x0][y + 1] != '#') {
                        ++y;
                        shoot[x0][y].pb(u);
                    }
                }

            vector<bool> res(ns * nt);
            REP(u, ns) {
                vector<pii> q = {sol[u]};
                int dist[N][N];
                CL(dist, -1);
                dist[sol[u].X][sol[u].Y] = 0;
                for (int a = 0; a < sz(q); ++a) {
                    int x0 = q[a].X, y0 = q[a].Y;
                    if (!shoot[x0][y0].empty()) {
                        for (int t : shoot[x0][y0]) {
                            res[u + ns * t] = true;
                        }
                        continue;
                    }
                    if (dist[x0][y0] >= m) continue;
        ...
        int opt = mx - 1;
        for (int a = 0; a < sz(q); ++a) {
            if (__builtin_popcount(q[a]) < __builtin_popcount(opt)) {
                opt = q[a];
            }
            int mask = q[a] >> ns;
            REP(i, ns) if (q[a] >> i & 1) {
                    REP(j, nt) if (mask >> j & 1) {
                            int e = (1 << i) | (1 << (j + ns));
                            if (reach[mask][i + ns * j] && vis[q[a] ^ e] == 0) {
                                q.pb(q[a] ^ e);
                                vis[q[a] ^ e] = e;
                            }
                        }
...
int main() {
    //randomTest();
    //maxTest();
    input.reset(new InputFile("d-small-attempt0.in"));  // -small-attempt0.in
