...
int f(int x, int want)
{
  ...
  else
  {
    ...
    if (g[x] == 1) // and gate
      if (want == 1)
        r = FMIN(r, FMIN(INF, f(x * 2, 1) + f(x * 2 + 1, 1)));
      else 
        r = FMIN(r, FMIN(f(x * 2, 0), f(x * 2 + 1, 0)));
    else // or gate
      if (want == 1)
        r = FMIN(r, FMIN(f(x * 2, 1), f(x * 2 + 1, 1)));
      else
        r = FMIN(r, FMIN(INF, f(x * 2, 0) + f(x * 2 + 1, 0)));

    if (c[x] == 1)
    {
      if (g[x] == 0) // and gate
        if (want == 1)
          r = FMIN(r, 1 + FMIN(INF, f(x * 2, 1) + f(x * 2 + 1, 1)));
        else 
          r = FMIN(r, 1 + FMIN(f(x * 2, 0), f(x * 2 + 1, 0)));
      else // or gate
        if (want == 1)
          r = FMIN(r, 1 + FMIN(f(x * 2, 1), f(x * 2 + 1, 1)));
        else
          r = FMIN(r, 1 + FMIN(INF, f(x * 2, 0) + f(x * 2 + 1, 0)));
    }
...
{ 
  ...
  REP(ctest, ntest)
  {
    ...
    int res = f(1, v);
    if (res == INF)
      cout << "Case #" << ctest + 1 << ": " << "IMPOSSIBLE" << endl;
    else
      cout << "Case #" << ctest + 1 << ": " << res << endl;
