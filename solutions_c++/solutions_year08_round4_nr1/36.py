...
int go(int v){
	...
	if (type[v] < 2){
		if (type[v] == 0)
			ret = min(go(v*2), go(v*2+1));
		else{
			ret = go(v*2) + go(v*2+1);
			if (chg[v])
				minn(ret, min(go(v*2), go(v*2+1)) + 1);
		}
	}else
		ret = (type[v]==2)?1000000:0;
	return ret++;
}
int main(){
	int t = io.readInt<int>(), tt, i, j,k;
	FUP(tt,1,t){
		...
		FUP(i,1,M){
			type[i] = io.readInt<int>();
			if (!V)type[i] = 1-type[i];
			if (i <= (M-1)/2)
				chg[i] = io.readInt<int>();
			else
				type[i]+=2;
		}
		printf("Case #%d:", tt);
