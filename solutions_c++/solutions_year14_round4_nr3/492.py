...
/* http://webdocs.cs.ualberta.ca/~contest/code/Old/Graph_Theory/maxflow.c
...
graph_add_dummy(head_t *g, int v, int w) {
  ...
  g[v].l = (edge_t*)realloc(g[v].l, (g[v].n+1)*sizeof(g[v].l[0]));
  p = &g[v].l[ g[v].n ];
...
maxflow( int source, int sink, int N ) {
  ...
  queue = (int*)malloc((N+1)* sizeof(queue[0]));
  pred  = (int*)malloc(N * sizeof(pred[0]));
  maxcap= (int*)malloc(N * sizeof(maxcap[0]));
  if( S ) free(S);
  S     = (int*)malloc(N * sizeof(S[0]));
...
int main() {
  ...
  for (int t=1;t<=T;t++) {
		...
		g = (head_t*)malloc( (W*H*2+2) * sizeof(g[0]));
		...
		for (int x=0;x<W;x++) {
			for (int y=0;y<H;y++) {
				...
				graph_addedge(g,W*H+x*H+y,x*H+y,1);
				if (y<H-1) graph_addedge(g,x*H+y,W*H+x*H+y+1,1);
				if (y>0) graph_addedge(g,x*H+y,W*H+x*H+y-1,1);
				if (x<W-1) graph_addedge(g,x*H+y,W*H+x*H+y+H,1);
				if (x>0) graph_addedge(g,x*H+y,W*H+x*H+y-H,1);
				if (y==0) graph_addedge(g,2*W*H,W*H+x*H+y,1);
				if (y==H-1) graph_addedge(g,x*H+y,2*W*H+1,1);
			}
		}
		int flow = maxflow(2*W*H, 2*W*H+1, W*H*2+2);
		/*
