...
struct rect{
    ...
    void in(){
        RD(x0, y0, x1, y1);
        /*P.clear(); P.PB(x0); P.PB(y0);P.PB(x1);P.PB(y1);*/
        /*Py.PB(y0), Py.PB(y1);
        if (y0 != h-1) Py.PB(y0+1);
        if (y1) Py.PB(y1-1);
        if (y0) Py.PB(y0-1);
        if (y1 != h-1) Py.PB(y1+1);*/
    }
} R[N];
...
namespace Network{
    ...
    const int N = 100*500*2*3+9, M = 6*N;
...
#define f cap[i^1]
    ...
    int V(int x, int y, int lv){
        return y+1 + x*w + lv*h*w;
    }

    int Solve(){

        s = 0, t = h*w*2+1, n = t + 1, m = 2, fill(hd, hd+n, 0);
        ...
        int i = 0; REP(j, w){
            add_edge(s, V(i,j,0), 1);
            if (j != w-1){
                add_edge(V(i,j,1), V(i,j+1,0), 1);
                add_edge(V(i,j+1,1), V(i,j,0), 1);
            }
        }

        REP(i, h-1) REP(j, w){
            add_edge(V(i,j,1), V(i+1,j,0), 1);
            add_edge(V(i+1,j,1), V(i,j,0), 1);
            if (j != w-1){
                add_edge(V(i,j,1), V(i,j+1,0), 1);
                add_edge(V(i,j+1,1), V(i,j,0), 1);
            }
        ...
        REP_2(i, j, h, w) if (!C[i][j]){
            //cout << i << " " << j << endl;
            add_edge(V(i,j,0), V(i,j,1), 1)
;        }
        assert(m < M);
        return Dinitz();
    }
...
int main(){
    ...
    Rush{
        printf("Case #%d: ", ++Case);
        RD(w, h, b); //Py.clear();
        REP(i, b) R[i].in(); // UNQ(Py);
        RST(C);

        //ECH(it, Py) cout << *it; puts("");
