            ...
            for (int j = 0; j < w; ++j) {
                for (int dx = -1; dx <= +1; ++dx)
                    for (int dy = -1; dy <= +1; ++dy) {
                        ...
                        if (plan[i][j] == '*')
                            cost = 0;
                        al2[ni*w+nj].push_back(pii(i*w+j, cost));
                        //al2[i*w+j].push_back(pii(ni*w+nj, cost));
                    }
            }
        for (int i = 0; i < h; ++i) {
            if (plan[i][0] == '.') {
                al2[h*w].push_back(pii(i*w, 1));
            }
            if (plan[i][0] == '*') {
                al2[h*w].push_back(pii(i*w, 0));
            }
            if (plan[i][w-1] == '*') {
                al2[i*w +w -1].push_back(pii(h*w+1, 0));
            }
            if (plan[i][w-1] == '.') {
                al2[i*w +w -1].push_back(pii(h*w+1, 0));
            }
