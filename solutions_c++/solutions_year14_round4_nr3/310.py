...
int main() {
    ...
    rep (ri, Tc) {
        ...
        rep (i, w) rep (j, h) {
            if (!a[i][j]) {
                net.add_edge(i * h + j, i * h + j + w * h, 1);
            }
            rep (o, 4) {
                int x = i + dx[o];
                int y = j + dy[o];
                if (0 <= x && x < w && 0 <= y && y < h) {
                    net.add_edge(i * h + j + w * h, x * h + y, 1);
                }
            }
        }
        rep (i, w) net.add_edge(S, coor(i, 0), 1);
        rep (i, w) net.add_edge(coor(i, h - 1) + w * h, T, 1);
        printf("%d\n", net.flow(S, T));
    }
}
