...
int inside(int a, int b, int w,int h) {
    return 0 <= a && a < w && 0 <= b && b < h;
}
    ...
    for (int tt=1; tt<=ts; tt++) {
        ...
        for (int i = 0; i < w; ++i)
            for (int j = 0; j < h; ++j) {
                if (f[i][j]) continue;
                int A = i * h + j + 1;
                x[A].push_back(make_pair(B + A, 1));
                for (int l = 0; l < 4; ++l) {
                    if (!inside(i + MX[l], j + MY[l], w,h)) continue;
                    int C = (i + MX[l]) * h + j + MY[l] + 1;
                    x[B + A].push_back(make_pair(C, 1));
                }
            }
        for (int i = 0; i < w; ++i) {
            int A = i*h + 1;
        ...
        cout<< "Case #"<<tt<<": "<<max_flow(s, t, t)<<endl;
    }
