...
const int W = 102 , H = 502;
bool a[H][W];
int w , h , b;
int V(int x , int y , int v){
    return x * w + y + v * h * w + 2;
}
...
void solve(){
    ...
    InitNetwork(V(h , w , 1) + 1 , 0 , 1);
    for (int i = 0 ; i < w; ++i) if (!a[0][i])
        AddEdge(0 , V(0 , i , 0) , 1);
    for (int i = 0 ; i < h; ++i)
        for (int j = 0 ; j < w; ++j) if (!a[i][j])
            AddEdge(V(i , j , 0) , V(i , j , 1) , 1);
    for (int i = 0 ; i < w; ++i) if (!a[h - 1][i])
        AddEdge(V(h - 1 , i , 1) , 1 , 1);
    for (int i = 0 ; i < h; ++i)
        for (int j = 0 ; j < w; ++j) if (!a[i][j]){
            for (int d = 0 ; d < 4 ; ++d){
                int p = i + dx4[d] , q = j + dy4[d];
                if (!inmap(p , q) || a[p][q]) continue;
                AddEdge(V(i , j , 1) , V(p , q , 0) , 1);
            }
        }
    printf("Case #%d: %d\n" , ++Case , Dinic());
}
