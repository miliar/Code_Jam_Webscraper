...
int legal(const int &x, const int &y){
...
int main(){
	...
	for(int i=0;i<w;i++)for(int j=0;j<h;j++)
			if(!dg[i][j])AddEdge(c[i][j],DST+c[i][j],1);

		scanf("%d%d%d",&w,&h,&b);
		for(int i=0;i<w;i++)for(int j=0;j<h;j++)dg[i][j]=0;
		for(int i=0;i<b;i++){
			int x0,y0,x1,y1;
			scanf("%d%d%d%d",&x0,&y0,&x1,&y1);
			for(int i=x0;i<=x1;i++)for(int j=y0;j<=y1;j++)dg[i][j]=1;
		}

		SRC=0;DST=1;for(int i=0;i<w;i++)for(int j=0;j<h;j++){
			if(dg[i][j])continue;
			c[i][j]=DST++;
		}
		...
		for(int i=0;i<w;i++){
			if(!dg[i][0])AddEdge(SRC,c[i][0],1);
			if(!dg[i][h-1])AddEdge(DST+c[i][h-1],DST,1);
		}
		for(int i=0;i<w;i++)for(int j=0;j<h;j++){
			if(dg[i][j])continue;
			for(int k=0;k<K;k++){
				int x=i+dx[k],y=j+dy[k];
				if(!legal(x,y) || dg[x][y])continue;
				AddEdge(DST+c[i][j],c[x][y],1);
			}
		}
