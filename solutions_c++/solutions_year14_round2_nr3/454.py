...
struct snode {
    int city;
    int code;
    snode(int cy, int ce) : city(cy), code(ce) {
    }
};
bool snode_sorter (snode const &a, snode const &b) {
...
inline void foreachTest(uns testNum) {
    ...
    for (nn=0; nn<N; ++nn) {
        cin >> ccode;
        codes.push_back(snode(nn, ccode));
    }
    int ii, jj;
    for (ii=0; ii<N; ++ii) {
        for (jj=0; jj<N; ++jj) {
    ...
    // 2. SOLVE test
    sort(codes.begin(), codes.end(), snode_sorter);

    vector<int> order;
    for (nn=0; nn<N; ++nn) {
        order.push_back(nn);
        //cerr << codes[nn].code << ", " << codes[nn].city << endl;
    }
    bool found = false;
    do {
        /*cerr << "curr_order = ";
        ...
        set<int> visited;
        nn = 0;
        visited.insert(order[nn]);
        visitOrder.push(order[nn]);
        ++nn;
        found = true;
        for (; nn<N; ++nn) {
            bool visitable = false;
            while (!visitOrder.empty()) {
                int cvi = visitOrder.top();
                if (adj[codes[cvi].city][codes[order[nn]].city]) {
                    visitable = true;
                    visitOrder.push(order[nn]);
                    //cerr << " visitable: " << codes[cvi].city+1 << ", " << codes[order[nn]].city+1 << endl;
                    break;
                } else {
                    visitOrder.pop();
                }
            ...
            if (visitable) {
                ...
                found = false;
                //cerr << " not visitable to " << codes[order[nn]].city << endl;
                //cerr << " found = false" << endl;
                break;
            }
        }
    } while (!found && next_permutation(order.begin(), order.end()));
    
    // 3.  WRITE outputs
    cout << "Case #" << testNum << ": ";
    for (nn=0; nn<N; ++nn) {
        //cerr << order[nn] << endl;
        cout << codes[order[nn]].code;
    }
