...
int go(int maska, int ost, int pierw){
	if (maska == (1<<K)-1)
		return end[pierw][ost];
	int& ret = dp[maska][ost][pierw];
	if (ret)return ret-1;
	int i;
	ret = 1000000;
	FUP(i,0,K-1)if (((1<<i)&maska) == 0){
		if (maska)
			minn(ret, go(maska | (1<<i), i, pierw) + mid[i][ost]);
		else
			minn(ret, go(maska | (1<<i), i, i));
	}
	return ret++;
}

int main(){
	int t = io.readInt<int>(), tt, i, j,k,a,b,c;
	vector<int> p;
	FUP(tt,1,t){
		K = io.readInt<int>();
		io.readStringLow(s);

		int l=strlen(s), L = l/K;

		cleara(end,0);
		cleara(mid,0);
		cleara(dp,0);

		FUP(i,0,L-2)FUP(a,0,K-1)FUP(b,0,K-1)
			end[a][b] += (s[i*K + b] != s[(i+1)*K + a]);
		FUP(i,0,L-1)FUP(a,0,K-1)FUP(b,0,K-1)
			mid[a][b] += (s[i*K + b] != s[i*K + a]);
		printf("Case #%d: %d\n", tt, 1+go(0, 0, 0));
	}
	return 0;
}
