...
unsigned long long lrand(int seed){ if(!_seeded) _seeded=true,srand((seed==-1)?time(0):seed); return ((((unsigned long long)rand())<<32) | rand());}
...
public:
   ...
   static bigint rand(const bigint &upperInc){ assert(upperInc>=0); int s, k; bool onBounds = true; 
for(s=upperInc.size-1;s>0&&upperInc.data[s]==0;s--); for(k=63;k>0;k--)if(SHIFT(k) & upperInc.data[s]) break; k++; unsigned long long first = LSB(k,LLMAX); bigint ret(0,upperInc.size); for(int j=s;j>=0;j--){ unsigned long long lr = lrand(0); if(j==s){ lr&=first; if(lr > upperInc.data[j]){ j = s+1; onBounds = true; continue;} ret.data[j] = lr; if(lr < upperInc.data[j]) onBounds = false;} else if(onBounds){ if(lr > upperInc.data[j]) { j = s+1; onBounds = true; continue;} ret.data[j] = lr; if(lr < upperInc.data[j]) onBounds = false;} else ret.data[j] = lr;} return ret;}
...
int simulate(vector<int> &order, string S){
   int times = S.length()/k, cnt = 0;
   char last = '?';
   FOR(i,0,times) FOR(j,0,k) if(S[k*i+order[j]]!=last) {cnt++; last = S[k*i+order[j]];}
   return cnt;
}
...
int main(){
  ...
  FOR(i,0,N){
    cin >> k >> S;
    vector<int> order;
    FOR(j,0,k) order.push_back(j);
    int ret = S.length();
    do{
   	ret = min(ret,simulate(order,S));
    }while(next_permutation(BEGEND(order)));
