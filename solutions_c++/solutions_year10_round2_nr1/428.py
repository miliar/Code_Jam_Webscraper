...
int main(int argc, char* argv[])
{
	...
	{
		vecsz v = readvec<sz>();
		vecstr exx = readvec_ml<str>(v[0]);
		vecstr crr = readvec_ml<str>(v[1]);

		set<vecstr> ex;
		for (size_t i = 0; i < exx.size(); ++i)
		{
			exx[i].erase(0, 1);
			ex.insert(dl::split(exx[i], '/'));
		}
		...
		{
			crr[i].erase(0, 1);
			vecstr s = dl::split(crr[i], '/');

			for (sz j = 1; j <= s.size(); ++j)
			{
				vecstr cur(s.begin(), s.begin() + j);
				if (ex.find(cur) == ex.end())
				{
					ex.insert(cur);
					++cnt;
				}
		...
		PRINT("Case #" << current_test <<": " << cnt << endl);
	}
