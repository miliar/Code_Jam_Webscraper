...
struct Dir
{
	...
	Dir* GetChild(const string& name, bool& bNew)
	{
		map<string, Dir*>::iterator it = children.find(name);
		if ( it == children.end() ) {
			bNew = true;
			Dir* pd = new Dir;
			children.insert( make_pair(name, pd) );
			return pd;
		} else {
			bNew = false;
			return it->second;
	...
	void Clear()
	{
		for ( map<string, Dir*>::iterator it = children.begin(); it != children.end(); ++it )
			delete it->second;
		children.clear();
	}
};
...
void main()
{
	...
	for ( int t = 0; t < T; ++t )
	{
		...
		for ( i = 0; i < N; ++i )
		{
			gets(buf);
			pd = &root;
			for ( pc = strtok(buf, "/"); pc; pc = strtok(NULL, "/") )
				pd = pd->GetChild(pc, b);
		}
		for ( i = 0; i < M; ++i )
		{
