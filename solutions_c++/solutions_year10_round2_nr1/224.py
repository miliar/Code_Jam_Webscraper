...
typedef map<string,int> TreeNode;
...
int add(const string& str) {
  path = splitString<string>(str,"/");
  int ret = 0;
  int ti = 0;
  FORS (p,path) {
    if (tree[ti].find(path[p]) == tree[ti].end()) {
      tree.push_back(TreeNode());
      tree[ti][path[p]] = SIZE(tree)-1;
      ++ret;
    }
    ti = tree[ti][path[p]];
  }
...
int solve() {
  ...
  REP (i,ne) add(exist[i]);
  int ret = 0;
  REP (i,nc) {
    int cur = add(create[i]);
    ret += cur;
  }
  return ret;
}
...
void main()
{
  ...
  FOR (test,1,ntests+1) {
    ...
    ifs >> ne >> nc;
    getline(ifs, string());
    exist.resize(ne); create.resize(nc);
    REP (i,ne) ifs >> exist[i];
    REP (i,nc) ifs >> create[i];
    int res = solve();
    ofs << " " << res;
    //-------------------------------------------------------------
    ofs << endl;
