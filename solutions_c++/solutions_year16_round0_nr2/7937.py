...
inline bool isValidStack(string st) {
    int i, len = (int)st.length();
    F(i,0,len){
        if (st[i] != HAPPY) return false;
    }
    return true;
}

inline string flip(string st, int n) {
    int i, len = (int)st.length();
    string newSt(st);

    F(i,0,n) {
        newSt[i] = st[n-i-1] == HAPPY? BLANK : HAPPY;
    }

    DBG("n=%d old %s new %s with len=%d\n", n, st.c_str(), newSt.c_str(), len);
...
int bfs(string st) {
    ...
    auto s = MP(st, 0);
    queue<pair<string, int>> q; q.push(s);                  // start from source

    states.insert(s.first);

    while (!q.empty()) {
        auto u = q.front(); q.pop();                   // queue: layer by layer!

        if (isValidStack(u.first)) return u.second;

        int i, len = (int)u.first.length();
        // try different flips
        FE(i,1,len) {
            string newSt = flip(u.first, i);
            if (states.count(newSt) == 0) {     // do not flip to previous state
                states.insert(newSt);
                q.push(MP(newSt, u.second + 1));
            }
        }
    }
}
...
int main() {
    CASES{
        ...
        int i, result = 0, len = (int)strlen(st);
        char good = HAPPY;
...
// }
//
// int dp(string st, unordered_set<string> &states){
//     // terminal state
//     if (isValidStack(st)) {
//         DBG("Stack is valid %s\n", st.c_str());
//         return 0;
//     }
//
//     // memo
//     auto got = memo.find(st);
//     if (got != memo.end()) return got->second;
//
//     // states.insert(st);
//
//     int minFlips = INF, i, len = (int)st.length();
//     // try different flips
//     FE(i,1,len) {
//         string newSt = flip(st, i);
//         if (states.count(newSt) == 0) {
//             states.insert(newSt);
//             REMIN(minFlips, 1 + dp(newSt, states));
//             states.erase(newSt);
//         }
//     }
//
