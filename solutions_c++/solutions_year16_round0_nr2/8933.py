...
int getPositionToRotate(const std::string& currStack)
{
				////////////////std::cout<<"Poistion as of now is: " << currStack <<std::endl;
				std::size_t iPos = std::string::npos;
				std::size_t iPlus = std::string::npos;
				std::size_t iMinus = std::string::npos;
				while(iPlus <= iMinus && iPos != 0 )
				{
								iMinus = currStack.find_last_of('-',iPos);
								if(iMinus == std::string::npos)
								{
												iPos = iPlus;
												break;
								}
								...
								{
												iPlus = currStack.find_last_of('+',iMinus);
												iPos = iPlus == std::string::npos ? iMinus : iPlus;
												if( iPlus == std::string::npos || iMinus == std::string::npos)
																break;
								}
				}
				return iPos;
}
bool doManeuver(const int& pos,std::string& currStack)
{
				const char* workingStack = currStack.c_str();
				int len = currStack.length();
				char* outputStack = new char[len+1];
				for(int i = pos+1; i < len ; ++i)
				{
								outputStack[i] = workingStack[i];
				}
				for(int i = pos, j = 0; i >= 0; --i,++j)
				{
								if(workingStack[i] == '+')
												outputStack[j] = '-';
								else if(workingStack[i] == '-')
												outputStack[j] = '+';
								else
												std::cerr<<"Whats this!!!!"<<std::endl;
				}
				outputStack[len] = '\0';
				currStack.assign(outputStack);
				delete [] outputStack;
				return (currStack.find_first_of('-') != std::string::npos);
}

void checkStack(const int& iTC, std::string& currStack)
{
				//std::cout << "\n\nComputing for Case#" <<iTC<<": " << currStack<<std::endl;
				int iRotationReqd = 0;
				if(currStack.find_first_of('-') != std::string::npos)
				{
								iRotationReqd++;
								while(doManeuver(getPositionToRotate(currStack),currStack))
								{
												iRotationReqd++;
								}
				}
				std::cout<<"Case #"<<iTC<<": " <<iRotationReqd <<std::endl;
}
int main()
{
				int iNumTCs;
				std::cin>>iNumTCs;
				std::string* cakeStacks = new std::string[iNumTCs];
				for(int i = 0 ; i < iNumTCs;++i)
				{
								std::cin>>cakeStacks[i];
				}
				for(int i = 0 ; i < iNumTCs;++i)
								checkStack(i+1,cakeStacks[i]);
				delete [] cakeStacks;
				return 0;
}
