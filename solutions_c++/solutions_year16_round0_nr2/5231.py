...
string flip(string s, int size)
{
	for(int i=0; i < size/2; i++)
	{
		s[i] = comp(s[i]);
		s[size-i-1] = comp(s[size-i-1]);

		swap(s[i], s[size-i-1]);
	}

	if(size%2==1)
		s[size/2] = comp(s[size/2]);
...
/*int brute(string s, int idx)
{
	...
	int res = brute(s, idx+1);
	res = min(res, brute(flip(s, idx+1), idx+1)+1);
...
pair<int,int> GetNums(string s)
{
	...
	return pair<int,int>(pos, neg);
}

string setPositive(string s, int size)
{
...
int greedy(string s)
{
	...
	while (true)
	{
		...
		s = setPositive(s, stats.first + stats.second);
	}
