...
public:
	...
	void Run(uint32 number) {
		...
		for (char c : text) {
			if (c == '+') {
				uint32 new_last_minus = last_plus + 1;
				uint32 new_last_plus = last_plus;

				last_minus = new_last_minus;
				last_plus = new_last_plus;
			}
			else {
				uint32 new_last_minus = last_minus;
				uint32 new_last_plus = last_minus + 1;

				last_minus = new_last_minus;
				last_plus = new_last_plus;
			}
		}
		cout << last_plus << newline;
	}
