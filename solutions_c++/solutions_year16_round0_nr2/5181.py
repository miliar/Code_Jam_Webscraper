...
int gen(string s, int move_id) {
	...
	for (int i = 1; i < len(s) + 1; i++) {
		string new_s = preffer(turn(s, i));
		if (s != new_s) {
			int x = gen(new_s, move_id + 1);
			if (dp[len(s)] == -1 || (dp[len(s)] > x + 1)) {
				dp[len(s)] = x + 1;
			}
		}
	}
...
int main(int argc, char *argv[]) {
	...
	for (int i = 0; i < n; i++) {
		...
		gen(query, 0);
		printf("Case #%d: ", i + 1);
		pii pos = get_pos(preffer(query));
		print dp[len(preffer(query))], nl;
	}
}