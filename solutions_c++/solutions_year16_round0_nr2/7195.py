...
public:
	...
	void solve()
	{
		...
		bool p[128]; int N = (int)str.size();
		for (int i = 0; i < N; ++i)
			p[i] = str[i] == '+';
		
		int rr = 0;
		while (count(p, p + N, true) != N) {
			int m = 0, fl = N-1;
			for (int i = N-2; i >= 0; --i) {
				if (p[i] == p[i+1] || p[i+1] == p[0]) continue;
				int t = (find(p, p+i+1, !p[0]) - p) + (find(p+i+1, p+N, !p[i+1]) - (p+i+1));
				if (t > m) {
					fl = i;
					m = t;
				}
			...
			rr++;
			reverse(p, p+fl+1);
			transform(p, p+fl+1, p, [](bool b) { return !b; });
		}
		printf("Case #%d: %d\n", getCaseID()+1, rr);
	}
};
