...
inline int index_of(InIt first1, InIt last1, const T &val)
{
	...
	return f != last1 ? distance(first1, f) : -1;
}
...
std::string reverse(std::string s, int p)
{
	int n = s.size();
	string ret;
	reverse_copy(partof(s, 0, p), back_inserter(ret));
	REP(i, p) { ret[i] = ret[i] == '-' ? '+' : '-'; }
	copy(partof(s, p, n - p), back_inserter(ret));
	//fprintf(stderr, "reverse %d %s -> %s\n", p, s.c_str(), ret.c_str());
	return ret;
}
...
void solve()
{
	...
	REP(testcase, testcases)
	{
		...
		int count = 0;
		while (any_of(allof(s), PREDICATE(char, c, c == '-')))
		{
			int last = s.find_last_of('-');
			if (s[0] == '+')
			{
				s = reverse(s, s.find_last_of('+', last) + 1);
				count++;
			}
			s = reverse(s, last + 1);
			count++;
