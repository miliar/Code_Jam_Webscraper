...
int main(){
	freopen("codejamibl.in","r",stdin);freopen("codejamobl.out","w",stdout);
	ll z=0;
	jkm(){
string s;
cin>>s;
...
ll c=0;
ll pos=0;
if(count(s.begin(),s.end(),'-')==s.length()) c=1;
else if(count(s.begin(),s.end(),'+')==s.length()) c=0;
else {
	c=1;
	for(int i=l(s)-1;i>=0;i--) if(s[i]=='-') {pos=i; break;}
		//debug(pos);
	for(int i=1;i<=pos;i++){
		if(s[i]!=s[i-1]) c++;
	}
