...
static int DR[3] = {  0,  0, +1 } ;
static int DC[3] = { -1, +1,  0 };
...
{
    cin >> H_in >> W_in;
    grid_in = vector<string>(H_in);
    REP(r, H_in) cin >> grid_in[r];
}

static int H, W;
static char grid[100][100];
static bool reachable[100][100];
static set<pair<int,int> > all_reachable;

static int dfs_reach(int r, int c)
{
    all_reachable.insert(make_pair(r, c));
    reachable[r][c] = true;
    int res = 1;
    REP(d, 3) {
        int nr = r - DR[d], nc = c - DC[d];
        if (grid[nr][nc] != '#' && !reachable[nr][nc]) res += dfs_reach(nr, nc);
    }
    return res;
}

static bool is_lucky(int r, int c)
{
    deque<set<pair<int,int> > > queue;
    set<set<pair<int,int> > > seen;
    queue.push_back(all_reachable);
    seen.insert(all_reachable);
    while (!queue.empty())
    {
        set<pair<int,int> > locs = queue.front();
        // FORIT(it, locs) cout << it->first << ',' << it->second << ' '; cout << endl;
        queue.pop_front();
        if (locs.size() == 1 && *locs.begin() == make_pair(r,c)) return true;

        REP(d, 3) {
            bool allowed = true;
            FORIT(it, locs) {
                int nr = it->first + DR[d], nc = it->second + DC[d];
                if (grid[nr][nc] != '#' && !reachable[nr][nc]) { allowed = false; break; }
            }
            if (allowed) {
                set<pair<int,int> > new_locs;
                FORIT(it, locs) {
                    int nr = it->first + DR[d], nc = it->second + DC[d];
                    new_locs.insert( (grid[nr][nc] == '#') ? *it : make_pair(nr, nc) );
                }
                if (seen.insert(new_locs).second) queue.push_back(new_locs);
            }
        }
    }
    return false;
...
void RealCase::solve(Result &result)
{
    H = H_in;
    W = W_in;
    REP(r, H) REP(c, W) grid[r][c] = grid_in[r][c];

    REP(d, 10) {
        REP(r, H) REP(c, W) if (grid[r][c] == char('0' + d)) {
            // cout << "d=" << d << endl;
            SET(reachable, 0);
            all_reachable.clear();
            int nreach = dfs_reach(r, c);
            bool lucky = is_lucky(r, c);
            result.add(STR(d) + ": " + STR(nreach) + (lucky ? " Lucky" : " Unlucky"));
        }
    }
}
