...
long bff[1100];
long inq[1010];
long dfs(long startPoint,long curPoint){
...
int main(void){
    ...
    for(long T=0;T<test_cases;T++){
        ...
        long Ans=0;
         do {
            if(bff[perMutation[0]]==perMutation[1]){
                for(long i=1;i<N;i++){
                    if(bff[perMutation[i]]==perMutation[0]||bff[perMutation[i]]==perMutation[i-1]){
                        if(i+1>Ans)
                            Ans=i+1;
                    }
                    if(bff[perMutation[i]]!=perMutation[(i+1)%N]&&bff[perMutation[i]]!=perMutation[(i+N-1)%N]){
                        i=N+1;
                        break;
                    }
                }
