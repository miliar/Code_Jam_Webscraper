...
int next[N],ans=0,mx=0,n;
VC<int> p[N];
bool vst[N];
void dfs(int f,int u,int pre,int k,int dir)
{
    int i,v;
    vst[u]=true;
    //PI3(u,k,ans);//PF("..");PI(k);
        ...
        if(v==pre){
            ...
            FOR(i,1,n){
                if(!vst[i]){
                    //PI3(i,k+1,ans);
                    vst[i]=true;
                    if(!vst[next[i]]) dfs(i,next[i],i,ans+2,2);
                }
            }
        }
        else if(vst[v]){
            if(v==f&&dir==0) ans=max(ans,k);
        }
        else if(!vst[v]) dfs(f,v,u,k+1,dir);
    }
    else if(dir==1){
        ans=max(ans,k);
        for(i=0;i<p[u].size();i++){
