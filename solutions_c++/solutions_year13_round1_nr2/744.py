...
int main(int argc, _TCHAR* argv[])
{
	...
	fout.open("C:\\Users\\ravill\\Documents\\Visual Studio 2010\\Projects\\Algo\\GCJ\\GCJ13\\testfiles\\B-small-attempt0.out",ios::out);
	if(!fin.good())
	{
		cout<<"Input file not opened"<<endl;
		exit(-1);
		...
		FR(i,0,N) {
			...
			FRE(j,0,E) {
				FRE(k,0,j) {
					int index = (j-k+R);
					if(index > E) index = E;
					D[cur][index] = max(D[cur][index], D[prev][j]+k*v[i]);
				}
			}
		}
		long long res=-1;
