...
ll e, r, n, x[1<<17];
ll gt(int lp, int rp, ll ep, ll k)
{
	int mi=-1, mx=0;
	FOR(i, lp, rp) if (x[i]>mx) mx=x[i], mi=i;
	if (mi==-1) return 0;
	ll ne=min(e, ep+(mi-lp)*r);
	ll c=max(0LL, min(ne, ne+(rp-mi)*r-k));
	return gt(lp, mi, ep, ne)+gt(mi+1, rp, min(e, ne-c+r), k)+x[mi]*c;
}
int main()
{
	FIN;
