...
bool isvalid(matrix<int>& m)
{
	//vector<int> minhx(m.size_x(), (numeric_limits<int>::max)());
	//vector<int> minhy(m.size_x(), (numeric_limits<int>::max)());
	vector<int> maxhx(m.size_x(), 0);
	vector<int> maxhy(m.size_y(), 0);
	...
	for(int i = 0; i < m.size_x(); ++i)
	{
		...
		{
			int h = m(i, j);
			//minhx[i] = (std::min)(minhx[i], h);
			//minhy[j] = (std::min)(minhy[j], h);
			maxhx[i] = (std::max)(maxhx[i], h);
			maxhy[j] = (std::max)(maxhy[j], h);
		}
	}
...
void solve(istream& in, ostream& fout, int caseIdx)
{
	...
	if(isvalid(m))
	{
		OUT( << CaseHead(caseIdx+1) << "YES" << endl );
	}
	else
	{
		OUT( << CaseHead(caseIdx+1) << "NO" << endl );
	}
}
