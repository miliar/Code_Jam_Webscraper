...
bool go()
{
    scanf("%d%d",&n,&m);
    int i,j;
    rii(n)  rij(m)  scanf("%d",&bo[i][j]);

    rii(n)
    {
        r[i] = true;
        rij(m)  if(bo[i][j]>1)  {r[i]=false;break;}
    }

    rii(m)
    {
        c[i] = true;
        rij(n)  if(bo[j][i]>1)  {c[i]=false;break;}
    }

    rii(n)rij(m)    if(bo[i][j]==1 && !(r[i]||c[j]))    return false;
