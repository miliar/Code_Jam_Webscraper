...
int main(){
	...
	for(int i1 = 0; i1 < ncase; i1++){
		...
		for(int i = 0; i < n; i++) for(int j = 0; j < m; j++) ans[i][j] = 100;
		for(int i = 0; i < n; i++) {
			int maxh = 0;
			for(int j = 0; j < m; j++) if(mp[i][j] > maxh) maxh = mp[i][j];
			for(int j = 0; j < m; j++) ans[i][j] = min1(maxh, ans[i][j]);
		}
		for(int i = 0; i < m; i++) {
			int maxh = 0;
			for(int j = 0; j < n; j++) if(mp[j][i] > maxh) maxh = mp[j][i];
			for(int j = 0; j < n; j++) ans[j][i] = min1(maxh, ans[j][i]);
		}
		bool f = 1;
		for(int i = 0; i < n; i++) for(int j = 0; j < m; j++) if(mp[i][j] != ans[i][j]) f = 0;
