...
template<class T> inline int getbit(T n,int pos){return n & (1<<pos);}//NOTES:countbit(
template<class T> inline int setbit(T& n,int pos){return n |= (1<<pos);}//NOTES:countbit(
//Numberic Functions
...
{
	T den = lcm(p.b,q.b);
	return Fraction<T>( p.a*(den/p.b) + q.a*(den/q.b) ,den);
}
...
template<class T> Fraction<T> operator*(Fraction<T> p,T q){return Fraction<T>(p.a*q,p.b);}
template<class T> bool operator==(Fraction<T> p,Fraction<T> q){return p.a==q.a && p.b==q.b;}
...
public:
	...
	bigNum operator/(int n/*const bigNum& a*/)const
...
template<class T> 
inline int numFac(T num)
{
	vector<pair<T,int> > f = factorize(num);
	int fac=1;
	rep(i,f.size())
		fac*=f[i].second+1;
	return fac;
...
bool RemoveRow(int arr[200][200], int &n, int &m, int r)
{
	int v = arr[r][0];
	bool possible = true;
	For(i,1,m-1)
...
bool RemoveCol(int arr[200][200], int &n, int &m, int c)
{
	int v = arr[0][c];
	bool possible = true;
	For(i,1,n-1)
...
int main()
{
	...
	rep(cc,T)
	{
		...
		while(n>0 && m>0)
		{
			...
			bool possible = false;
			possible = RemoveRow(arr,n,m,minRow);
			if(!possible)
				possible = RemoveCol(arr,n,m,minCol);
			if(!possible)
				break;
		}
		fout<<"Case #"<<cc+1<<": ";
