...
bool verificar_fila_columna (vector< vector<int>  > &v, int m, int n, int i, int j,int height) {
	bool fila = true;
	bool columna = true;
	if(v[i][j]!=height)
		return true;
...
bool reeplace (vector< vector<int>  > &v,int m,int n,int height) {
	fori(a,m){
		fori(b,n){
			if(v[b][a] == height)
				v[b][a]++;
	...
	fori1(i,t){
		...
		if(n>2 || m>2){
			fori(p,max){
				fori(j,n){
					fori(k,m){
						if(!verificar_fila_columna(v,m,n,j,k,p)){
