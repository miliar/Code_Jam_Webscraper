...
Board initialize(int M, int N, int max_height)
{
  Board board;
  board.reserve(M);
  fori(M) {
    Line line(N, max_height);
    board.push_back(line);
  }
  return board;
}

int maxHeight(const Board& board)
{
  int ret = 0;
  FOREACH(line, board) {
    FOREACH(c, (*line)) {
...
void cutRow(Board& board, const int i, const int h)
{
...
void cutColumn(Board& board, const int i, const int h)
{
...
bool check(const Board& board, const Board& target)
{
  const int M = board.size();
  const int N = board[0].size();
...
void solve(Board board, const Board& target, int M, int N)
{
  // rows
  fori(M) {
    int m = target[i][0];
...
int main(int argc, char** argv)
{
  ...
  // the first line
  {
    ...
    for (int t=0; t<T; ++t) {
      ...
      Board init = initialize(M,N, maxHeight(board));
      cout << "Case #" << t+1 << ": ";
      solve(init, board, M, N);
    }
  }
