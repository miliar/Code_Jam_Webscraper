...
const int ht_limit = 100;

bool isvalid(int x,int y,int fi[N_MAX][M_MAX],int N,int M){
...
int main(){
	...
	for(int case_id=0;case_id<caseno;case_id++){
		...
		for(int i=0;i<N;i++){
			for(int j=0;j<M;j++){
				if(field[i][j] < ht_limit){ //the lawn has mowed the field in this square
