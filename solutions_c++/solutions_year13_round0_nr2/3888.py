...
int main() {
  ...
  static int a[101][101],p[101],q[101];
  int T,n,m;
  scanf("%d",&T);
  ft(t,1,T) {
    scanf("%d%d",&n,&m);
    ft(i,1,n) ft(j,1,m) scanf("%d",&a[i][j]);

    memset(p,0xf0,sizeof(p));
    memset(q,0xf0,sizeof(q));
    ...
    string res="YES";
    ft(k,1,100) ft(i,1,n) ft(j,1,m) {
      if (res=="NO") break;
      if (a[i][j]==k && k<min(p[i],q[j])) res="NO";
    }
