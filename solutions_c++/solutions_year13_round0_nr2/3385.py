...
ll vagerMod(ll a, ll b, ll mm)
{
    return ((a%mm)*(BigMod(b,mm-2,mm)))%mm;
}
...
void process()
{
    ...
    while(T--)
    {
        ...
        for(ll i=0; i<n; i++)
        {
            for(ll j=0; j<m; j++)
            {
                if(arr[i][j]==rowmax[i] || arr[i][j]==colmax[j])continue;
                else
                {
                    f=1;
                }
