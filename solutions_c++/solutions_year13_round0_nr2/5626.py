...
int nor, noc;
char cutToHeight(int stX, int stY, int rw, int cl, int ht)
{
...
int main(int argc, char** argv)
{
    ...
    for(int eachCase = 0; eachCase<cases; eachCase++){
        init();
        cin>>nor>>noc;
        vector<int> rowNums(noc);
        vector<int> maxRow(nor, 0);
        vector<int> maxCol(noc, 0);
        vector<vector<int> > trial;
        vector<int> dumRow(noc, 100);
        
        for(int i=0;i<nor;i++){
            for(int j=0;j<noc;j++)
                {
                    cin>>rowNums[j];
                    maxRow[i] = max(maxRow[i], rowNums[j]);
                    maxCol[j] = max(maxCol[j], rowNums[j]);
                }
            field.PB(rowNums);
            trial.PB(dumRow);
        }
        ...
        bool flag = true;
        for(int i=0;i<nor && flag;i++)
        {
            for(int j=0;j<noc;j++)
                {
                    trial[i][j] = min(maxRow[i], trial[i][j]);
                    trial[i][j] = min(maxCol[j], trial[i][j]);
                    if(trial[i][j] != field[i][j])
                        {
                            flag = false;
                            break;
                        }
        ...
        if(flag)
            cout<<"Case #"<<eachCase+1<<": YES"<<endl;
        else             
            cout<<"Case #"<<eachCase+1<<": NO"<<endl;
    }
}
