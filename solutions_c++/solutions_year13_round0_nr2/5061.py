...
int main(int argc,char **args)
{
	...
	printf("Hello World!\n");
//	freopen("B-example.in","r",stdin);freopen("B-example.out","w",stdout);
...
*/
        ...
        memset(H_max,0,sizeof(H_max));
        memset(V_max,0,sizeof(V_max));
        forn(i,N)
        {
            forn(j,M)
            {
                checkmax(H_max[i],table[i][j]);
                checkmax(V_max[j],table[i][j]);
            }
        }
        ...
        forn(i,N)
        {
            forn(j,M)
            {
                ...
                if(H_max[i]!=table[i][j] && V_max[j]!=table[i][j])
                {
                    flag=false;
                    break;
                }
