...
namespace NInput
{
  inline char PK() {return cin.peek();}
  inline char PKC(istream& cin) {return cin.peek();}

  inline void Readln() {static STR tmp_s; getline(cin, tmp_s);}
  inline bool IsLatin(char c) {c = toupper(c); return 'A' <= c && c <= 'Z';}
  ...
  inline void SkipSpacesEOLN(istream& cin)
  {
    for (; PKC(cin) == ' ' || PKC(cin) == 10 || PKC(cin) == 13; cin.get());
  }
  inline void SkipSpacesEOLN() {SkipSpacesEOLN(cin);}
}
