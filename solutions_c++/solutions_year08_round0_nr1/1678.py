// contest1.cpp : Defines the entry point for the console application.
//
...
int _tmain(int argc, _TCHAR* argv[])
{
	  ...
	  FILE* file;
	  FILE* file2;
	  char* filename = "D:\\A-large.txt";
      char* filename2 = "D:\\A-large result.txt";
	  char tempbuffer[100];
	  char *temp_buffer = tempbuffer;
	  int search_num,queries_num;
	  char searchbuffer[100][100];
	  char queriesbuffer[1000][100];             // need to be revised when large
	  char *search_buffer[100];              
	  char *queries_buffer[1000];
	  int searchbufferflag[100];
	  int count = 0;
	  int change = 0;
	  for(int t = 0; t<100; t++)                 // need to be revised when large
	  {
	...
	for(int i=0;i<n;i++)
	{
		...
		for(int s=0;s<search_num;s++)  // get the engines
		{
			fgets(temp_buffer, 100, file);
		...
		for(int t = 0; t<100; t++)    // clear buffer
		{
           searchbufferflag[t] = 0;
		}
		count = 0;
		change = 0;
        for(int q = 0; q<queries_num; q++)   // get the queries
		{
            fgets(temp_buffer, 100, file);
			strcpy(queries_buffer[q],temp_buffer);
			for(int s = 0; s<search_num;s++)  // cmp with the engines
			{
