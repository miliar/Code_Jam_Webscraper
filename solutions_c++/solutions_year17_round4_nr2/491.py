...
bool oc[N][N];
int now[N];
int person[N] , position[N];
bool check(int co , int &pr){
    RST(oc);
    pr = 0;
    RST(now);
    for (int i = 0 ; i < n ; ++i) if (position[i]){
        for (int j = 0 ; j < position[i] ; ++j){
            bool ok = false;
            for (int k = 0 ; k < co ; ++k) if (!oc[k][i]){
                ok = true;
                oc[k][i] = true;
                break;
            }
            if (!ok){
                ++pr;
                for (int k = 0 ; k < co ; ++k){
                    while(now[k] < n && oc[k][now[k]]) ++ now[k];
                    if (now[k] < i){
                        oc[k][now[k]] = true;
                        ok = true;
                        break;
...
void solve(){
    ...
    for (int i = 0 ; i < m ; ++i){
        RD(a[i].fi , a[i].se);
        a[i].fi--;
        checkMax(maxRen , ++person[a[i].se]);
        ++position[a[i].fi];
    }
    int low = maxRen , ans = m , high = m;
    int ans2 = 0;
    do{
        int mid = low + high >> 1;
        int pp;
