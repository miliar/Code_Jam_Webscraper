...
pair<int, int> solve() {
	next(int, n_seats);
	next(int, n_customers);
	next(int, n_tickets);

	vector<int> count_by_customer(n_customers);
	vector<vector<int>> tickets(n_seats);
	FOR (i, 0, n_tickets) {
		next(int, p); p--;
		next(int, c); c--;
		count_by_customer[c]++;
	...
	int least_rides = accumulate(all(count_by_customer), 0, [](int a, int b) { return max(a, b); });

	int delta = 0;
	FOR (row, 0, n_seats) {
		delta += least_rides;
		delta -= tickets[row].size();
		while (delta < 0) {
			delta += row + 1;
			least_rides++;
		}
	...
	int no_shift = 0;
	FOR (row, 0, n_seats) {
		no_shift += min(least_rides, (int) tickets[row].size());

	}

	return { least_rides , n_tickets - no_shift };
}
