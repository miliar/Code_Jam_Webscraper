/*
Date: 2017/05/13 21:25:40 Saturday
*/
#include<bits/stdc++.h>
using namespace std;
#define X first
...
int main(){
	...
	scanf("%d",&C);rep(_,1,C){
		printf("Case #%d: ",_);
		scanf("%d%d%d",&n,&m,&t);
		rep(i,0,1000)A[i]=B[i]=suma[i]=0;
		rep(i,1,t)scanf("%d%d",&a[i],&b[i]),A[a[i]]++,B[b[i]]++;
		int ans=0,res=0;
		rep(i,1,m)MAX(ans,B[i]);
		rep(i,1,n){
			suma[i]=suma[i-1]+A[i];
			MAX(ans,(suma[i]-1)/i+1);
		}
		printf("%d ",ans);
		rep(i,1,n)if(A[i]>ans)res+=A[i]-ans;
		printf("%d\n",res);
	}
	return 0;
}
