...
int main()
{
  ...
  for (int tn = 1; tn <= test_cnt; ++tn)
  {
    ...
    auto c = ni();
    auto m = ni();
    vector<int> tickets[1024];
    for (int i = 0; i < m; ++i) {
      auto pos = ni();
      auto id = ni();
      tickets[id-1].push_back(pos);
    }
    ...
    if (c == 2) {
      int ans = 0;
      int prs = 0;
      if (tickets[0].size() < tickets[1].size())
        swap(tickets[1], tickets[0]);
      for (int i = 0; i < tickets[0].size(); ++i) {
        auto f = false;
        for (int j = 0; !f && j < tickets[1].size(); ++j) {
          if (tickets[1][j] != tickets[0][i]) {
            if (binary_search(tickets[0].begin() + i, tickets[0].end(), tickets[1][j])) {              
              tickets[1].erase(tickets[1].begin() + j);
              f = true;
              break;
            }
          }
      ...
      for (int i = 0; i < tickets[0].size(); ++i) if (tickets[0][i] >= 0) {
        auto f = false;
        for (int j = 0; !f && j < tickets[1].size(); ++j) {
          if (tickets[1][j] == tickets[0][i]) {
            if (tickets[1][j] == 1)
