...
struct Sol{
  ...
  bool spr(int roz){
    ...
    R(i,m){
      flow.add_edge(ed[i].SE, ed[i].FI + c, 1, 0);
    }
    R(i,n){
      flow.add_edge(c + i, c + n + i, inf, 1);
      flow.add_edge(c + n + i, c + i, inf, 0);
      if(i != n-1)
        flow.add_edge(c + n + i + 1, c + n + i, inf, 0);
      flow.add_edge(c + i, t, roz, 0);
    }
    PII x = flow.get_min_cost_flow(s,t);
    pom = x.SE;
    return x.FI == m;
  }
  vector<int> il;
