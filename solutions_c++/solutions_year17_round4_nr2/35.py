...
int solve(const VI& posns, int days)
{
	int avail = 0;
	int promo = 0;
	ji(posns.size())
...
int main()
{
	...
	{
		printf("Case #%d: ", k+1);
		map<int, int> posnFreq;
		map<int, int> ticketsBought;
		int n, c, m;
		rint(n);
		rint(c);
		rint(m);
		...
		ji(n)
		{
			posns[i] = posnFreq[i+1];
		}
		int lo = 0;
		int hi = m;
		for (auto pr : ticketsBought)
		{
			lo = max(lo, pr.second);
		}
		while (lo < hi)
		...
		int promotions = solve(posns, lo);
		cerr << n << endl << c << endl << m << endl << lo << endl << promotions << endl << endl;
		assert(promotions != -1);
		printf("%d %d\n", lo, promotions);
	}
}
