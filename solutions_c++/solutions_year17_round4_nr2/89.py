...
public:
    ...
    void solve() {
        ...
        REP(i, m) {
            bool ok = false;
            REP(j, sz(r)) if (sz(r[j]) < t[i].X) {
                    ok = true;
                    for (int v : r[j]) if (v == t[i].Y) {
                            ok = false;
                            break;
                        }
                    if (ok) {
...
int main() {
    //randomTest();
    //maxTest();
    input.reset(new InputFile("b-small-attempt0.in"));  // -small-attempt0.in
