...
void calcFunction() {
    ll minrides = *max_element(all(crides));
    ll counter = 0;
    forn(i,n){
        counter += ptickets[i];
        minrides = max(minrides,(i+counter)/(i+1));
    }
    ll promos =0;
    counter =0;
    forn(i,n){
        counter += ptickets[i];
        promos += max(0ll, ptickets[i]-minrides);
    }
    result = mp(minrides,promos);
}
