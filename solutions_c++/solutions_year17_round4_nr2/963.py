...
int main(void){
	...
	REP(t, T){
		...
		REP(i, 1010){
			if(i > 0)
				sum[i] += sum[i-1];
			latte = max(latte, bn[i]);
			dp[i] = {0, 0};
		}
		dp[0] = {latte, 0};
		FOR(i, 1, N+1){
			dp[i] = dp[i-1];
			if(dp[i].first >= sum[i]-sum[i-1]) continue;
			else if(dp[i].first < sum[i]-sum[i-1]){
				if(dp[i].first * i >= sum[i]){
					dp[i].second += sum[i]-sum[i-1]-dp[i].first;
				}else{
					while(dp[i].first *i < sum[i]){
						dp[i].first++;
					}
					dp[i].second += sum[i]-sum[i-1]-dp[i].first;
				}
		...
		cout << "Case #" << t+1 << ": " << dp[N].first << " " << dp[N].second << endl;
	}
}
