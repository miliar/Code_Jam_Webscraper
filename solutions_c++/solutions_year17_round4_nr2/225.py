...
public:
	void solve(std::istream& in, std::ostream& out) {
		...
		out << "Case #" << ++test << ": ";
		cerr << test << endl;
		int seats, cust, tickets;
		in >> seats >> cust >> tickets;

		auto cnt = make_vector<int>(2, seats, 0);

		for (int i: range(tickets)) {
			int pos, b;
			in >> pos >> b;
			--pos, --b;
			if (cust == 2) {
		...
		int ans = max(prefixSums(cnt[0]).back(), prefixSums(cnt[1]).back());
		updateMax(ans, cnt[0][0] + cnt[1][0]);

		out << ans << " ";
		for (int i: range(seats)) {
			if (cnt[0][i] + cnt[1][i] > ans) {
				out << cnt[0][i] + cnt[1][i] - ans << "\n";
				return;
			}
		}
