...
int main() {
	...
	FOR(tt, 1, tc+1) {
		...
		map<int, vl> customers;
		vector<pair<int, int>> posses(n);
		FOR(i,0,m) {
			int p, b;
			cin >> p >> b;
			p--;
			b--;
			customers[b].pb(p);
			if (b == 0) posses[p].first++;
			if (b == 1) posses[p].second++;
		}
		int rides = posses[0].first + posses[0].second;
		rides = max(rides, (int)max(customers[0].size(), customers[1].size()));
		int count = 0;
		cout << rides << " ";
		FOR(pp, 0, n) if (posses[pp].first + posses[pp].second > rides) count += posses[pp].first + posses[pp].second - rides;
		cout << count << endl;
	}
}
