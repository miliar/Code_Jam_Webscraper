...
int n, k, m, ret;
string input, words[100];
vector<string> expr;

int wval[100][26], val[26];

void go(int x) {
  if (x == m) {
    for (int i = 0; i < sz(expr); ++i) {
      int c = 1;
      for (int j = 0; j < sz(expr[i]); ++j) c *= val[expr[i][j] - 'a'];
      ret = (ret + c) % mod;
    }
    return;
  }
  ...
  for (int i = 0; i < n; ++i) {
    for (int j = 0; j < 26; ++j) val[j] += wval[i][j];
    go(x + 1);
    for (int j = 0; j < 26; ++j) val[j] -= wval[i][j];
  }
}
...
void solve_case() {
  ...
  expr = split<string>(input, "+");

  for (int i = 0; i < n; ++i) cin >> words[i];
  for (int i = 0; i < n; ++i) {
    for (int j = 0; j < 26; ++j) wval[i][j] = 0;
    for (int j = 0; j < sz(words[i]); ++j) wval[i][words[i][j] - 'a']++;
  }
