...
#define myabs(x)  ((x<0)?-(x):(x))
	llint a,b;
	void simplify(){while(b==0);assert(b);llint g=__gcd(myabs(a),myabs(b));a/=g;b/=g;if(b<0)b=-b,a=-a;};
public:
...
int word[100][26];
const int mod=10009;
map<string,int> mem[11];
int bino[1001][1001];
vector<int> get_sum(string poly,int n,int m)
{
	assert(m<=10);
	for(int i=0;i<=m;i++)mem[i].clear();
	mem[0][""]=1;
	int len=poly.length();
	vector<string> all_sub;
	for(int mk=1;mk<two(len);mk++)
	{
		string sub;
		for(int i=0;i<len;i++)
			if(two(i)&mk)sub.push_back(poly[i]);
		mem[0][sub]=0;
		all_sub.push_back(sub);
	}
	all_sub=unique(all_sub);
	for(int i=1;i<=n;i++)
	{
//		debug(i);

		for(int k=m;k;k--)
		for(int j=0;j<all_sub.size();j++)
		{
			const string term=all_sub[j];
			int len=term.length();
			assert(len);
			for(int in=1;in<=k;in++)
			{
				...
				for(int mk=0;mk<two(len);mk++)
				{
					...
					for(int x=0;x<len;x++)
					{
						if(two(x)&mk)sub.push_back(term[x]);
						else mul*=(word[i-1][term[x]-'a']*in)%mod,mul%=mod;
					}
					mul*=kk;mul%=mod;
					mem[k][term]+=(((mul*mem[k-in][sub])%mod))%mod;
					mem[k][term]%=mod;
	//				if(k==2)cerr<<mul<<" "<<in<<" "<<sub<<" "<<mem[k-in][sub]<<endl;
				}
			}
//			cerr<<k<<" "<<term<<" "<<mem[k][term]<<endl;
		}
		for(int j=1;j<=m;j++)mem[j][""]=i*mem[j-1][""],mem[j][""]%=mod;

	}
	vector<int> res(m,0);
	for(int i=1;i<=m;i++)res[i-1]=mem[i][poly];
	return res;
}
...
int main()
{
	...
	{
		bino[i][0]=bino[i][i]=1;
		for(int j=1;j<i;j++)bino[i][j]=(bino[i-1][j]+bino[i-1][j-1])%mod;
	}
	debug(mod*mod);
	int ca=1,tt;
	for(cin>>tt;tt--;ca++)
	{
		...
		string tmp;
		cin>>tmp;
		vector<string> poly=parse<string>(tmp,"+");
	//	cerr<<poly<<endl;
		int m;
		cin>>m;
		debug(m);
		...
		{
			cin>>tmp;
			for(int j=0;j<tmp.length();j++)word[i][tmp[j]-'a']++;
		}
		vector<int> ans(m,0);
		for(int i=0;i<poly.size();i++)
		{
			vector<int> tmp=get_sum(poly[i],n,m);
	//		debug(poly[i]);
//			cerr<<tmp<<endl;
			for(int j=0;j<m;j++)ans[j]+=tmp[j],ans[j]%=mod;
		}
		//printf("Case #%d: %.6lf\n",ca,ans);
		cout<<"Case #"<<ca<<":";
