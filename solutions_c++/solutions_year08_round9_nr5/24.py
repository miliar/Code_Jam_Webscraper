...
//		cout << mand[i] << ' ' << meq[i] << endl;
	...
	{
		popc[c] = __builtin_popcount(c);
	}
	for(int c = 0; c < 32768; ++c)
	{
		value[c] = __builtin_popcount(c ^ (c << 1));
	}
...
/*
		for(int c = 0; c < (1 << m); ++c)
			if(mat[cur][c] > 0)
				cerr << i << ' ' << c << ' ' << mat[cur][c] << endl;
*/
	}

	for(int c = 0; c < (1 << m); ++c)
	{
		res = max(res, (int)mat[cur][c] + popc[c]);
	}
