...
long long getLastFlip(long long num){
    long long len=getlen(num);
    long long halflen=len/2,temp=num,ans=1;
...
long long getminMove(long long num){
    if(num<21)
        return num;
    else if(num<100)
    {
        if(num%10==0)
            return 1+getminMove(num-1);
//        if(num/10<num%10)
         return (num%10)+(num/10)+10;
  //       else
    //        return getminMove(num-1-num%10)+1+num%10;
    }
    long len=getlen(num);
    long halflen=len/2,temp=num;
    long long lFlip=getLastFlip(num);
    long long lpoint=flipNum(lFlip);
    if(lpoint>num)
        return 1+getminMove(num-1);
    else if(lpoint==num&&lFlip<num)
        return 1+getminMove(lFlip);
    if(lFlip<num&&lFlip<flipNum(lFlip))
        return num-lpoint+lFlip%pow10[halflen]+10+getminMove(lFlip-(lFlip%pow10[halflen]+9));
    return num-flipNum(lFlip)+lFlip%pow10[halflen]+9+getminMove(lFlip-(lFlip%pow10[halflen]+9));
}
void buildDP(){
    for(long i=0;i<21;i++)
        dp[i]=i;
    for(long i=21;i<1000002;i++){
        ...
        if(fn<i&&i%10)
            ...
            dp[i]=1+dp[i-1];
        if(getminMove(i)!=dp[i]){
            cout<<i<<" "<<dp[i]<<" "<<getminMove(i)<<endl;
        }
    }
}
...
int main(void){
    ...
    freopen ("E:/outputSmall4.txt","w",stdout);
	long test_cases;
	cin>>test_cases;
	string s;
    pow10[0]=1;
