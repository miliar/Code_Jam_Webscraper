...
int aStar(int N)
{
  // A* with admissible heuristics (our is even monotonous)
  priority_queue<Entry> H;
  unordered_map<int, int> best = { {1, 1} };
  H.push(Entry{1, 1});
  ...
  while (!H.empty()) {
    ...
    if (best[entry.num] < entry.length)
      continue;
    ...
    // push all neighbors to the heap
    Entry next;
    next = Entry{entry.num + 1, entry.length + 1};
    if (!best.count(next.num) || best[next.num] > next.length) {
      best[next.num] = next.length;
      H.push(next);
    }

    next = Entry{rev(entry.num), entry.length + 1};
    if (!best.count(next.num) || best[next.num] > next.length) {
      best[next.num] = next.length;
      H.push(next);
    }
