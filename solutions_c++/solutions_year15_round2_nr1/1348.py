...
int main()
{
	...
	rep(i,1,1000000)
	{
	    dp[i+1]=min(dp[i+1],dp[i]+1); //for immediate next
	    vec.clear();

	    int dum=i;
	    while(dum!=0) {vec.pb(dum%10); dum=dum/10;}
        dum=0;
        tr(vec,it) dum=dum*10+*it;
        dp[dum]=min(dp[i]+1,dp[dum]);
       // if(i==12) cout<<dum<<endl;
	}
