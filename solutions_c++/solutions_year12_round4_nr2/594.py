...
public:
		...
		do{
			...
			for(int i=0; i<size; ++i){
				for(int j=0; j<size; ++j){
					double x = W * (i / (double)size);
					double y = L * (j / (double)size);
					for(int n=0; n<N; ++n){
						if(set[n]){
							continue;
						}
						...
						for(int k=0; k<N; ++k){
							if(set[k]){
								double dx = result[k].first - x,
									dy = result[k].second - y,
									d = sqrt(dx * dx + dy * dy);
								if(d < r + R[k]){
									ok = false;
									break;
	...
	void OutputResult(ostream& out) const{
		for(int i=0; i<result.size(); ++i){
			out << " " << std::setprecision(20) << result[i].first << " " << std::setprecision(20) << result[i].second;
		}
		out << endl;
	}
};
