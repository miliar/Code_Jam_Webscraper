...
void Case::solve(Result &result)
{
    ...
    REP(mask, 1u<<M) {
        VVI dist(N, VI(N, inf));
        REP(i, M) edges[i].w = (mask&(1u<<i)) ? edges[i].min_w : edges[i].max_w;
        REP(i, N) dist[i][i] = 0;
        REP(i, M) dist[edges[i].i][edges[i].j] = edges[i].w;
        REP(k, N) REP(i, N) REP(j, N) dist[i][j] = min(dist[i][j], dist[i][k] + dist[k][j]);
/*
REP(i, N) {
    REP(j, N) printf("%10d", dist[i][j]);
    printf("\n");
}
*/
        int k = 0, sofar = 0;
        while (k < P) {
            sofar += edges[path[k]].w;
            if (sofar + dist[edges[path[k]].j][1] > dist[0][1]) break;
            ++k;
        }
        max_k = max(max_k, k);
    }
    if (max_k == P) result.set("Looks Good To Me"); else result.set(STR(path[max_k] + 1));
}
