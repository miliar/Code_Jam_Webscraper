...
int flood(int r, int c){
   if(r < 0 || c < 0 || r>= R || c >= C || tspace[r][c] != '.') return 0;
   int ret = 1; tspace[r][c] = '#';
   FOR(i,0,4) ret += flood(r+dr4[i],c+dc4[i]);
   return ret;
}

bool isDangerous(vector<int> &state){
   MEMSET(tspace,'#'); FOR(i,0,state.size()) tspace[state[i]/12][state[i]%12] = '.';
//   FOR(i,0,5) {FOR(j,0,4) cout << tspace[i][j]; cout << endl;}
   int sc = flood(state[0]/12,state[0]%12);
//   cout << "SCORE: " << sc << endl;
   return sc != state.size();
}

bool canMove(vector<int> &state, int ind, int dir){
   int r = state[ind]/12, c = state[ind]%12;
   int nr, nc, orr, oc;

   nr = r + dr4[dir], nc = c + dc4[dir];
   orr = r - dr4[dir], oc = c - dc4[dir];

   if(nr < 0 || nc < 0 || nr >= R || nc >= C ||
      oc < 0 || orr < 0 || oc >= C || orr >= R || 
      space[orr][oc] == '#' || space[nr][nc]=='#') 
	return false;

   bitset<144> used = 0;
   FOR(i,0,state.size()) used[state[i]] = 1;
   if(used[nr*12 + nc] || used[orr*12 + oc]) return false;
   return true;
}

int renew(int val, int j){
   int r = val/12, c = val%12;
   r += dr4[j], c += dc4[j];
   return r*12+c;
}

int solve(vector<int> &boxes, const vector<int> &goal){
  dp[boxes] = 0;
  queue<vector<int> > vq; vq.push(boxes);
  if(boxes == goal) return 0;

  while(!vq.empty()){
       boxes = vq.front(); vq.pop();
       int d = dp[boxes];
       bool forced = isDangerous(boxes);
//       cout << d << " " << forced << " -> "; printArray(boxes);

       FOR(i,0,boxes.size()) FOR(j,0,4) if(canMove(boxes,i,j)){
	   vector<int> tboxes = boxes; tboxes[i] = renew(tboxes[i],j); sort(BEGEND(tboxes));
           if(dp.count(tboxes) || forced && isDangerous(tboxes)) continue;
	   if(goal == tboxes) return d+1; dp[tboxes] = d+1;
	   vq.push(tboxes);
       }
  }
  return -1;
...
int main(){
  ...
  FOR(i,0,_T){
    cout << "Case #" << (i+1) << ": ";
    cin >> R >> C;
    vector<int> boxes, dest;
    dp.clear(); N = 0;

    FOR(r,0,R) FOR(c,0,C){
	cin >> space[r][c];
	if(space[r][c] == 'x' || space[r][c] == 'w') dest.push_back(r*12+c);
	if(space[r][c] == 'o' || space[r][c] == 'w') boxes.push_back(r*12+c), N++;
    }
    int sc = solve(boxes,dest);
    cout << sc;
//    FOR(i,0,R){ FOR(j,0,C) cerr << space[i][j]; cerr << endl;} cerr << "SCORE: " << sc << endl;
    cout << endl;
  }
  return 0;
}
