...
struct Node {
	...
	inline ll len() const { return r-l+1; }
	bool operator>(const Node &rhs) const {
		if(len()==rhs.len()) return l>rhs.l;
		return len()<rhs.len();
	}
};

int t,n,k;
ll mid,ans1,ans2,dist1,dist2;
...
int main(){
	...
	for(int z=1;z<=t;z++){
		...
		q1.push(Node(1,n));
		while(!q1.empty()&&k--){
			mid=(q1.top().l+q1.top().r)/2,dist1=mid-q1.top().l,dist2=q1.top().r-mid;
			ans1=max(dist1,dist2),ans2=min(dist1,dist2);
			if(mid-1>=q1.top().l) q1.push(Node(q1.top().l,mid-1));
			if(q1.top().r>=mid+1) q1.push(Node(mid+1,q1.top().r));
			q1.pop();
		}
