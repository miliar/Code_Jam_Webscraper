...
#ifdef small
#ifdef comp
#define  fileName  problem "-small-1-attempt" small_number 
#else
...
int main() {
	...
	FOR1(test_number, T) {
		...
		int width;
		long long int people, stalls;
		fscanf(in, "%lld %lld\n", &stalls, &people);

		int old_stalls = stalls;

		unsigned long divs;
		BitScanReverse64(&divs, people);
		int large_half = ~(people - 1-((1 << divs) - 1));
		for (int i = 0; i < divs; i++) {
			int lh = (large_half>>i) & 1;
			stalls = (stalls + lh - 1) / 2;
		}

		int max = (stalls) / 2;
		int min = (stalls - 1) / 2;

		pr("\t[%d %d]\t ->\t", old_stalls, people);
		pr("%d %d\n", max, min);

		int min_2;
		int max_2;
		{//brute force
			bool *arr = (bool *)malloc(old_stalls);
			memset(arr, 0, old_stalls);
			FOR(i, people) {
				int start_range = -1;
				int len = -1;
				int start_range_tmp, tmp_len;
				start_range_tmp = -1;
				len = -1;

				FOR(i, old_stalls) {
					if (start_range_tmp == -1) {
						if (arr[i] == false) {
							start_range_tmp = i;
							tmp_len = 1;
						}
				...
				if (len >= 0 && start_range >= 0) {
					if (arr[(start_range + (len - 1) / 2)])__debugbreak();
					arr[(start_range + (len - 1) / 2)] = true;
				} else __debugbreak();
				max_2 = len / 2;
				min_2 = (len - 1) / 2;
			}
			if (!(min_2 == min && max_2 == max)) __debugbreak();
			free(arr);
		}
