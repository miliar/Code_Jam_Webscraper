...
int T, n, k, minv, maxv, ming, maxg, v1, v2, vc, lastmax, lastmin, minplace;
set<int> s;
    ...
    FOR(t,1, T+1) {
        ...
        RR(j,k) {
            ...
            for (; nn!=s.end(); ++it, ++nn) {
                ...
                if (v2>v1+1) {
                    //printf("test %d %d %d\n", v1, v2, vc);
                    minv = min(vc-v1,v2-vc) - 1;
                    maxv = max(vc-v1,v2-vc) - 1;
                    if (minv > ming || (minv == ming && maxv > maxg)) {
                        maxg=maxv;
                        ming=minv;
                        minleft=it;
                        minright=nn;
            ...
            s.insert(minplace);
            lastmin = min(minplace-*minleft,*minright-minplace) - 1;
            lastmax = max(minplace-*minleft,*minright-minplace) - 1;
            //printf("place %d %d %d\n", minplace, lastmin, lastmax);
        }
