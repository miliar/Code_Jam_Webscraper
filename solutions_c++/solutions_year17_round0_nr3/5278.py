...
void sov(){
    ...
    while(1){
        ...
        k -= (nula+nusm);
        getnum(sm,t[1],t[2]);getnum(la,t[3],t[4]);
//        for(int i =1; i <= 4; i++)
//            cout <<"t["<<i<<"] =  "<<t[i]<<endl;
        ...
        tsm = tla = 0;
        for(int i = 1 ; i <= 4; i++){
            if(t[i] == sm && i < 3)  tsm += nusm;
            if(t[i] == sm && i >= 3)  tsm += nula;
            if(t[i] == la && i >= 3)  tla += nula;
            if(t[i] == la && i < 3)  tla += nusm;
        }
        swap(nula,tla);swap(nusm,tsm);
