...
We only fall to rise even higher to heights that we never knew existed!
...
int main()
{
    ...
    test_case
    {
    	...
    	map< ll,ll,greater<int> > mapping , next_mapping;
    	mapping[n] = 1;
    	...
    	while(true)
	    {
	    	...
	    	for( auto m: mapping)
	    	{
	    		...
	    		next_mapping[ m.first/2 ] += m.second;
	    		next_mapping[ (m.first-1)/2 ] += m.second;
	    	}
