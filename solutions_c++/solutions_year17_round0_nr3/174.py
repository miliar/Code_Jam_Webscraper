...
struct Pile {
    ll len;
    ll num;
    Pile(ll len, ll num): len(len), num(num) {}
};
...
void push(ll len, ll num) {
    if (len < v[v.size()-1].len) {
        v.push_back(Pile(len,num));
    } else {
        int i = v.size()-1;
        while (i >= 0 && v[i].len != len) i--;
        assert(i >= 0);
        v[i].num += num;
    }
}
...
void doCase(int case_number) {
    cin >> n >> k;
    v.clear();
    v.push_back(Pile(n,1));
    int i = 0;
    while (k > 0) {
        if (v[i].num == 0) i++;
        assert(i < v.size() && v[i].num > 0);
        //debug(i); debug(v.size()); cerr << endl;
        ll h = min(k,v[i].num);
        v[i].num -= h;
        push(v[i].len/2,h);
        push((v[i].len-1)/2,h);
        //err("Taking %d of length %d\n",h,v[i].len);
        k -= h;
    }

    printf("Case #%d: %lld %lld\n",case_number, v[i].len/2, (v[i].len-1)/2);
}
