...
pair<unsigned __int64, unsigned __int64> SolveSmall(unsigned __int64 n, unsigned __int64 k)
{
	set<Node, NodeCmp> m;
	...
	for (int i = 0; i < k; ++i)
	{
		...
		if (c > 2)
		{
			m.insert(Node(p.s, r / 2));
		}

		if(c > 1)
		{
			m.insert(Node(p.s + r / 2 + 1, (r + 1) / 2));
		}
	}
...
pair<unsigned __int64, unsigned __int64> SolveLarge(unsigned __int64 n, unsigned __int64 k)
{
	pair<unsigned __int64, unsigned __int64> res;
	map<ULL, ULL, SegmentCmp> m;
...
VOID CALLBACK MyWorkCallback(PTP_CALLBACK_INSTANCE Instance, PVOID Parameter, PTP_WORK Work)
{
	...
	fprintf(g_fout, "Case #%d: %I64u %I64u\n", t, res.second, res.first);
