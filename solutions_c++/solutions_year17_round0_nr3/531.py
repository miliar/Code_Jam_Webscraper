...
void tc(int tci) {
	...
	// The slots will be lowslot and lowslot + 1.
	// There will be highcount of the latter.
	ll lowslot = n + 1;
	ll highcount = 0;
	...
	for (int round = 0; ; round++) {
		// In round, there will be 2^round slots at the start.
		// 2^round people will sit down.
		ll curPeople = 1LL << round;
		ll curSlots  = 1LL << round;
		if (curPeople < peopleLeft) {
			peopleLeft -= curPeople;
			if (lowslot & 1LL) {
				lowslot = lowslot / 2;
				highcount = curSlots + highcount;
			} else {
				lowslot = lowslot / 2;
			}
		} else {
			// last step
			if (peopleLeft <= highcount) {
				finalSlot = lowslot + 1;
			} else {
				assert(peopleLeft <= curPeople);
				finalSlot = lowslot;
			}
			break;
		}
	...
	printf("Case #%d: %" PRId64 " %" PRId64 "\n", tci, (finalSlot - 1) / 2, (finalSlot - 2) / 2);
}
