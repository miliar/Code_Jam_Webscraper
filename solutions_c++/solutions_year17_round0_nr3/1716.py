...
int main() {
  ...
  while(t--)
  {
    ...
    if((n-suml)%lev_sum==0)
    {
      ...
      if(len%2LL==0)
        out << "Case #"<< ++kase << ": "<< len/2 <<" "<< len/2-1 << endl;
      else
    ...
    else
    {
      ...
      {
        if(big_num%2==0)
          out << "Case #"<< ++kase << ": "<< big_num/2 <<" "<< big_num/2-1 << endl;
        else
          out << "Case #"<< ++kase << ": "<< big_num/2 <<" "<< big_num/2 << endl;
      }
      else
      {
        if(sml_num%2==0)
          out << "Case #"<< ++kase << ": "<< sml_num/2 <<" "<< sml_num/2-1 << endl;
        else
          out << "Case #"<< ++kase << ": "<< sml_num/2 <<" "<< sml_num/2 << endl;
      }
    }
  }
