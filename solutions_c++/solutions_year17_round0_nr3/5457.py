...
struct TreeNode {
	...
	TreeNodePtr par;
	TreeNode(TreeNodePtr p, long long m):
		max(m),
		left(nullptr),
		right(nullptr),
		par(p){}
...
void calculate(int t) {
	long long N, K;
	inp >> N >> K;
	TreeNodePtr root = make_shared<TreeNode>(nullptr, N);
	TreeNodePtr cur = root;
	for (long long k = 0; k < K; k++) {
		cur = insertNode(root);
	}
	long long max = cur->left->max > cur->right->max ? cur->left->max : cur->right->max;
	long long min = cur->left->max < cur->right->max ? cur->left->max : cur->right->max;
	out << "Case #" << t + 1 << ": ";
	out << max << " " << min;
	out << endl;
}
