...
pair<long long, long long> solve(long long n, long long k)
{
	...
	q[make_pair(-((n - 1) / 2), -(n - 1 - (n - 1) / 2))] = 1;
	pair<long long, long long> curr;
	while(k > 0)
	{
		curr = q.begin()->first;
		long long cnt = q.begin()->second;
		q.erase(q.begin());
		curr.first *= -1;
		curr.second *= -1;
		if (cnt >= k)
		...
		k -= cnt;
		if (curr.first != 0)
			q[make_pair(-((curr.first - 1) / 2), -(curr.first - 1 - (curr.first - 1) / 2))] += cnt;
		if(curr.second != 0)
			q[make_pair(-((curr.second - 1) / 2), -(curr.second - 1 - (curr.second - 1) / 2))] += cnt;
	}
	return curr;
}
