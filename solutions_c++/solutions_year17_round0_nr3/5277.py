...
pair<long long, long long> sizeToPair(long long size) {
    long long first = size / 2;
    long long second = size & 1 ? size / 2 : size / 2 - 1;
    return make_pair(first, second);
}

void process(long long num, long long count, priority_queue<long long> &q, unordered_map<long long,long long> &sizeToCount) {
    if (sizeToCount.count(num) == 0) {
        q.push(num);
        sizeToCount[num] = count;
    } else {
        sizeToCount[num] += count;
    }
}

pair<long long,long long> getLastBathroomStall(long long N, long long K) {
    long long occupied = 0;
    priority_queue<long long> q;
    unordered_map<long long, long long> sizeToCount;
    q.push(N);
    sizeToCount[N] = 1;
    while (true) {
        long long current = q.top();
        q.pop();
        occupied += sizeToCount[current];
        pair<long long,long long> split = sizeToPair(current);
        // Check for ending condition
        if (occupied >= K) {
            return split;
        }

        process(split.first, sizeToCount[current], q, sizeToCount);
        process(split.second, sizeToCount[current], q, sizeToCount);
    }
}
...
int main() {
    ...
    for (int i=1; i<=T; ++i) {
        cin >> N >> K;
        cout << "Case #" << i << ": ";
        pair<long long,long long> last = getLastBathroomStall(N, K);
