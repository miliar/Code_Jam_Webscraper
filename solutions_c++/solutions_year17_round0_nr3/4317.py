...
struct lazy {
	...
	bool operator<(const lazy& rhs) const{
		if(v-u+1 == rhs.v - rhs.u + 1){
            return (rhs.u < u);
		}
		return (rhs.v - rhs.u + 1 > v-u+1);
	}
};
...
void solve(){
    ...
    for(int i = 1;i<=K;i++){
        lazy cur = pq.top();
        pq.pop();
        int mid = (cur.u + cur.v)/2;
        int s1 = cur.u, e1 = mid - 1;
        int s2 = mid+1, e2 = cur.v;
        if(s1<=e1) pq.push(lazy(s1, e1));
        if(s2<=e2) pq.push(lazy(s2, e2));
        int L = e1 - s1 + 1, R = e2 - s2 + 1;
        if(i == K) printf("%d %d\n",max(L,R),min(L,R));
        //printf("At %2d = (%2d - %2d)\n",i,min(L,R),max(L,R));
    }
}
...
int main() {
    ...
    freopen ( "outputCsmall2.txt", "w", stdout );
    #endif // forthright48
