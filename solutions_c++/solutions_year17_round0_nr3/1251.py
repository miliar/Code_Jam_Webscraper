...
struct Rec {
	long long l; 
	long long num; 
	Rec(long long al, long long an) : l(al), num(an) {}
};
bool operator< (Rec& rec1, Rec& rec2) {
	return rec1.l < rec2.l; 
}

void formatVec(vector<Rec> &v) {
	sort(v.begin(), v.end()); 
...
int main(int argc, char* argv[]) {
	...
	int T = 0; inf >> T;
	for (int t = 0; t < T; t++) {
		long long n, k; inf >> n >> k; 
		vector<Rec> recs; 

		Rec rec(n, 1); 
		recs.push_back(rec); 

		while (k > recs.back().num) {
			k -= recs.back().num; 
			if (recs.back().l % 2 == 1) {
				Rec rec(recs.back().l / 2, recs.back().num * 2); 
				recs.erase(recs.begin() + recs.size() - 1);
				recs.push_back(rec); 
			}
			else {
				Rec rec1(recs.back().l / 2, recs.back().num); 
				Rec rec2(recs.back().l / 2 - 1, recs.back().num); 
				recs.erase(recs.begin() + recs.size() - 1); 
				recs.push_back(rec1);
				recs.push_back(rec2); 
			}
		...
		long long lmax, lmin; 
		lmin = lmax = recs.back().l / 2; 
		if (recs.back().l % 2 == 0) lmin--; 

		cout << "Case #" << t + 1 << ": " << lmax << " " << lmin << endl; 
	}
