...
pair<ll, ll> testcase_itr(ll N, ll K)
{
    // solution start
    ll large_val = N;
    ll large_num = 1;
    ...
    while(true){
        if(K<=large_num){
            return make_pair(large_val/2, (large_val-1)/2);
        }
        K-=large_num;
        if(K<=small_num){
            return make_pair(small_val/2, (small_val-1)/2);
        }
        K-=small_num;
        
        ll next_large_num = 0, next_small_num = 0;
        ll next_large_val = 0, next_small_val = 0;
        if(large_val%2==0){
            next_large_num += large_num;
            next_small_num += large_num + small_num * 2;
            next_large_val = large_val / 2;
            next_small_val = (large_val / 2) - 1;
        }else{
            next_large_num += large_num * 2 + small_num;
            next_small_num += small_num;
            next_large_val = small_val / 2;
            next_small_val = (small_val / 2) - 1;
        }
        large_num = next_large_num;
        small_num = next_small_num;
        large_val = next_large_val;
        small_val = next_small_val;
    }
