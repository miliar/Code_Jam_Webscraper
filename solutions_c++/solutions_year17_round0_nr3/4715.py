...
int main()
{
    ...
    freopen("c_in_small2.in","r",stdin);
    freopen("c_out_small2.out","w",stdout);
    ...
    while(test--)
    {
    	...
    	{
    		int t=PQ.top(); PQ.pop();
    		if(t%2) PQ.push(t/2), PQ.push(t/2);
    		else PQ.push(t/2), PQ.push(t/2-1);
    	}

    	printf("Case #%d: ", cases++);
    	if(PQ.top()%2) cout<<PQ.top()/2<<" "<<PQ.top()/2<<endl;
    	else cout<<PQ.top()/2<<" "<<PQ.top()/2-1<<endl;
