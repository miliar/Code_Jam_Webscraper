...
void rec()
{
    if(k>(1LL<<cl))
    {
        ...
        for(auto it:pre)
        {
            ll x = it.fs.fs-1LL;
            ll y = it.fs.sc-1LL;
            if(x>=0LL)now[{flr(x),cil(x)}]+=it.sc;
            if(y>=0LL)now[{flr(y),cil(y)}]+=it.sc;
        }
        pre.clear();
        pre = now;
        k-=(1LL<<cl);
    ...
    else
    {
        for(auto it: pre)
        {
            cal.push_back({{it.fs.fs,it.fs.sc},it.sc});
        }
        for(int i=cal.size()-1LL;i>=0LL;--i)
        {
            if(k<=cal[i].sc)
            {
                ll x = cal[i].fs.fs;
