...
struct problem_t {
  ...
  struct stalls_t {
    ...
    void add(LL l, LL r, LL c) {
      cnt[{min(l, r), max(l, r)}] += c;
    }

    void add(LL len, LL c) {
      LL l = (len - 1) / 2;
      LL r = len / 2;
      add(l, r, c);
    }

    pair<pair<LL, LL>, LL> extract() {
      assert(!cnt.empty());
  ...
  void solve(int test) {
    ...
    while (k > 0) {
      ...
      s.add(it.first.first, it.second);
      s.add(it.first.second, it.second);
      k -= it.second;
    ...
    assert(last.first != -1);
    cout << T(test) << " " << last.second << " " << last.first << endl;
  }
};
