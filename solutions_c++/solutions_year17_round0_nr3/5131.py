...
int test(){
	...
	int remain=K,n_min=0,n_max=1,val=N,a,b;
	bool cont=true;
	while(cont) {
		//cout<<"remain "<<remain<<endl<<n_max<<" spots at value "<<val<<" and "<<n_min<<" spots at value"<<val-1<<endl;
		if(remain<=n_max) {
			a=val/2;
			if(val%2==0) {
				b=a-1;
...
int init(){
	FILE* gg;
	gg= freopen("C-small-2-attempt0.in","r",stdin);
	if(gg==NULL) { cout<<"prob"; }
	freopen("outputbis.txt","w",stdout);
	return 0;
}
