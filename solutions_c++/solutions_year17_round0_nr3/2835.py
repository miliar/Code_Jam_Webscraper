...
int main(int argc, const char* argv[]) {
  ...
  for (auto t: range<uint32>(0, T)) {
    ...
    std::map<uint64, uint64, std::greater<uint64>> counts;
    counts.emplace(N, 1);
    uint64 rMax, rMin;
    while (K > 0) {
      auto front = *counts.begin();
      counts.erase(counts.begin());
      auto P = std::min(K, front.second);
      front.second -= P;
      K -= P;
      if (front.second > 0) {
        counts.insert(front);
      ...
      rMin = (front.first - 1) / 2;
      rMax = front.first - 1 - (front.first - 1) / 2;
      counts[rMin] += P;
      counts[rMax] += P;
    }
    print("Case #%0: %1 %2", t + 1, rMax, rMin);
  }
  return 0;
}
