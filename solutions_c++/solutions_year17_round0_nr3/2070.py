...
string SolveProblem(LL N, LL K) {
    K--;
    map<LL, LL> spaces;
    spaces[N] = 1;
    while (1) {
        LL space, num;
        tie(space, num) = *spaces.rbegin();
        if (K < num) break;
        K -= num;
        spaces.erase(space);
        spaces[space/2ll] += num;
        spaces[(space-1ll)/2ll] += num;
    }

    LL curr_space = spaces.rbegin()->first;
    LL MinorSpace = curr_space / 2ll;
    LL MajorSpace = (curr_space - 1) / 2ll;
    return to_string(MinorSpace) + " " + to_string(MajorSpace);
}
