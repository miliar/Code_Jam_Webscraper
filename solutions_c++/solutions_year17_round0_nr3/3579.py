...
namespace {
    ...
    void solve(ll n, ll kei) {

        priority_queue<vector<ll>> pq; // ,greater<ll>

        pq.push(vector<ll>{n, 0ll});



        while (!pq.empty()) {
            const vector<ll> &v = pq.top();
            ll w = v[0];
            ll p = -v[1];
            pq.pop();
            if (--kei == 0) {
            ...
            if (w > 2)
                pq.push(vector<ll>{(w - 1) / 2, -p});
            if (w > 1)
                pq.push(vector<ll>{w / 2, -(p + (w + 1) / 2)});
        }
    }
