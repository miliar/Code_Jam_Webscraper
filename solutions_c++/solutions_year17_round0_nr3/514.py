...
void task(int ti) {
    using Map64 = std::map<uint64_t, uint64_t>;
    uint64_t n;
    uint64_t k;
    std::cin >> n >> k;
    ...
    while (true) {
        auto it = --bb.end();
        uint64_t curr_dist = it->first;
        uint64_t curr_count = it->second;
        bb.erase(it);

        uint64_t l = curr_dist / 2;
        uint64_t r = curr_dist / 2;
        if (l + r == curr_dist) {
            --r;
        }
        if (curr_count >= k) {
            std::cout << "Case #" << ti << ": " << l << " " << r << "\n";
            return;
        }
        k -= curr_count;
        bb[l] += curr_count;
