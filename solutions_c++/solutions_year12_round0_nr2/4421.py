...
int calcValue(int given)
{
    return DIVUP(given,3);
}

int calcSurprise(int given)
{
    if (given < 2)
        return calcValue(given);
    if (given%3 == 1)
        return calcValue(given);
    return calcValue(given)+1;
}
...
int main(void)
{
  ...
  cout.rdbuf(out->rdbuf());
    ...
    FORCASE(n,cases)
    {
        ...
        FOR(i,0,N)
        {
            int val  = calcValue(googlers[i]);
            int val2 = calcSurprise(googlers[i]);
            if (val >= p)
                higher++;
            else if (val2 >= p)
                surprisers++;
        }
        cout << higher + min(surprisers,S) << endl;
    }
#endif
    return 0;
}
