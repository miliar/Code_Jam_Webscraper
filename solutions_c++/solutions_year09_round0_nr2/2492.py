...
char flood(int x, int y, char z){
	//printf("flood %d %d %c\n", x, y, z);
	if (res[x][y])return res[x][y];
	int mx = -1,my=-1;
	if (x>0 && cell[x-1][y] < cell[x][y]){mx=x-1;my=y;}
	else if (y>0 && cell[x][y-1] < cell[x][y]){mx=x;my=y-1;}
	else if (y+1<W && cell[x][y+1] < cell[x][y]){mx=x;my=y+1;}
	else if (x+1<H && cell[x+1][y] < cell[x][y]){mx=x+1;my=y;}

	if (mx != -1){

		if (x-1>=0 && cell[x-1][y]<cell[mx][my]){mx=x-1;my=y;}
		if (y-1>=0 && cell[x][y-1]<cell[mx][my]){mx=x;my=y-1;}
		if (y+1<W  && cell[x][y+1]<cell[mx][my]){mx=x;my=y+1;}
		if (x+1<H && cell[x+1][y]<cell[mx][my]){mx=x+1;my=y;}

		char t = flood(mx, my, z);
		if (t < z)z=t;
	}
