...
inline pair<int, int> find_ShortestN(int x, int y);


void	main(void)
{
		int N;
		string ip,num, lang, dest_lang;
		...
		REP(i, N){
			...
			REP(j, rows){
				REP(k, cols){
					if(color[j][k] == -1){
						pair<int, int> SN = find_ShortestN(j, k);
						if(SN.first != -1){
							if(color[SN.first][SN.second] == -1){
						...
						if(SN.first != -1){
							if(color[SN.first][SN.second] == -1){
								...
								if(color[SN.first][SN.second] != color[j][k]){
									int ll = min(color[SN.first][SN.second],color[j][k]);
									int lh = max(color[SN.first][SN.second],color[j][k]);
