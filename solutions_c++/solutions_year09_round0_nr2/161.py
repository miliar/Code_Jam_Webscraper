...
void solveCase(int cnum) {
  ...
  scanf("%d%d", &H, &W);
  FOR(h,0,H) FOR(w,0,W) scanf("%d", &map[h][w]);
  
  FOR(h,0,H) FOR(w,0,W) bid[h][w] = 0;
  
  printf("Case #%d:\n", cnum+1);

  char next = 'a';
  FOR(oh,0,H) FOR(ow,0,W) {
    ...
    while(true) {
      ...
      int dirid = -1, high = map[h][w];

      FOR(dir,0,4) {
        int dh = h + dy[dir];
        int dw = w + dx[dir];
        if(dh < 0 || dw < 0 || dw >= W || dh >= H) continue;
      
        if(map[dh][dw] < high) high = map[dh][dw], dirid = dir;
        }
      
      if(dirid == -1) break;

      h += dy[dirid]; w += dx[dirid];
      }
    
    if(bid[h][w] == 0) bid[h][w] = next++;
    printf("%c ", bid[h][w]);
    if(ow+1==W) printf("\n");
    }
  }
