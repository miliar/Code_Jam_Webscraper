...
Array * flowsInto(int r, int c){
	...
	if((alt <= arr[r-1][c].alt) && (alt <= arr[r+1][c].alt) && (alt <= arr[r][c-1].alt) && (alt <= arr[r][c+1].alt))
		return 0;
	else if((arr[r-1][c].alt <= arr[r][c-1].alt) && (arr[r-1][c].alt <= arr[r][c+1].alt) && (arr[r-1][c].alt <= arr[r+1][c].alt) )
		return &arr[r-1][c];
...
int main(){
	...
	for(tc=0; tc<TC; ++tc){
		...
		for (pos = coll.begin(); pos != coll.end(); ++pos) {
			Array *cell;
			cell = flowsInto(pos->second/(Col+2), pos->second % (Col+2));
			if(!cell)
				arr[pos->second/(Col+2)][pos->second % (Col+2)].basin = nxtBasin++;
			} else {
				arr[pos->second/(Col+2)][pos->second % (Col+2)].basin = cell->basin;
			}
		}
