...
# include<stdlib.h>
using namespace std;
int land[102][102],q[10010],top,basin[102][102],h,w,nob,saket;
int alok[100];
...
int main()
	...
	while(zo<=no)
		...
		//alok = (char *)calloc(nob,sizeof(char));
		//for(i = 0;i<nob;i++)
		//	alok[i] = 0;
		
		saket = (int)'a';
		//cout<<basin[0][0] - 1<<" "<<saket<<endl;
		alok[basin[0][0] - 1] = saket;
	...
	printf("Case #%d:\n",zo);
		for(i=0;i<h;i++)
			...
			for(j=0;j<w-1;j++)
				{
				if((int)alok[basin[i][j]- 1] != 0)
					{
					printf("%c ",(char)alok[basin[i][j]- 1]);
					}
				else
					{
					saket++;
					alok[basin[i][j]- 1] = (char)saket;
					printf("%c ",(char)alok[basin[i][j]- 1]);
					}
				}
				if((int)alok[basin[i][j]- 1] != 0)
