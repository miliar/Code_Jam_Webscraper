/* 
 * SOUR:pku 1408 && 计算几何模板
 * DATE: 2009年 08月 28日 星期五 00:54:28 CST
 * */
#include<iostream>
#include<cstdio>
#include<cmath>
...
{
	return dcmp(cross_mul(l.e, p, l.s)) == 0 && (p.x - l.s.x) * (p.x - l.e.x) <= 0
...
bool intersect_ne(const lineseg_t & a, const lineseg_t & b)
{
	return
		intersect_e(a, b) &&
		!online(a, b.s) && !online(a, b.e) && !online(b, a.e) && !online(b, a.s);
}
...
lineseg_t line[N][maxn];
point_t p[maxn][maxn];

int g[N][N];

bool ifcross(int a,int b)
{
	int i,j;
	for(i = 1;i < m;i++) {
		for(j = 1;j < m;j++) {
			if(intersect_e(line[a][i],line[b][j]))
				return true;
		}
	}
	return false;
...
int vis[N], clique[N], group[N], res;
...
int main()
{
	...
	for(int id = 1;id <= C;id++) {
		...
		for(i = 0;i < n;i++) {
			...
			for(j = 1;j < m;j++) {
				scanf("%d",&nw); 
				line[i][j] = lineseg_t(point_t(j - 1,pre),point_t(j,nw));
				pre = nw;
			}
		}
