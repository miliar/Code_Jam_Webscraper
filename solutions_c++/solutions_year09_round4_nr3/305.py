...
bool overlap[100][100];
bool used[100];

bool hasOverlap(int A, int B){
   if(vects[A][0] == vects[B][0]) return true;
   // if(A == 0 && B == 2) printArray(vects[A]), printArray(vects[B]);
   if(vects[A][0] > vects[B][0]) swap(A,B);
   FOR(i,1,K) if(vects[A][i] >= vects[B][i]) return true;
   return false;
}

int dp[SHIFT(17)][17];
int solve(int mask, int cur){
   if(mask == 0 && cur==0) return 0;
   int &ret = dp[mask][cur]; if(ret!=-1) return ret; ret = INF;
   //cout << bitset<16>(mask) << " " << cur << endl;

   if(cur == 0){  FOR(i,0,N) if(mask & SHIFT(i)) ret = min(ret,solve(mask - SHIFT(i),i+1));}
   else{
      ret = min(ret,1 + solve(mask,0));
      FOR(i,cur,N) if((mask & SHIFT(i)) && !overlap[cur-1][i]) ret = min(ret,solve(mask - SHIFT(i),i+1));
   }
   //cout << bitset<16>(mask) << " " << cur << " -> " << ret << endl;
...
int main(){
  ...
  FOR(i,0,_T){
    cout << "Case #" << (i+1) << ": ";
    cin >> N >> K;
    MEMSET(overlap,false); MEMSET(used,false); MEMSET(dp,-1); vects.clear();
    FOR(j,0,N){ vector<int> cur(K); FOR(k,0,K) cin >> cur[k]; vects.push_back(cur);}
    sort(BEGEND(vects));
    FOR(j,0,N) FOR(k,j+1,N) if(hasOverlap(j,k)){
	  //cout << "OVERLAP: " << j << " " << k << endl;
	  overlap[j][k] = overlap[k][j] = true;
    }
//    FOR(j,0,N) { FOR(k,0,N) cout << overlap[j][k] << " "; cout << endl;}
    cout << solve(SHIFT(N)-1,0);
    cout << endl;
  }
  return 0;
}
