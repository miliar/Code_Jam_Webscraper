...
public:
	...
	inline void AddEdge(int p1, int p2)	{a[p1][m[p1]++] = c1+p2;}
	inline int operator [](int v) {return mate[v];}
	bool dfs(int v)
	{
		int ver;
		static int vv;
		mark[v] = true;
		for(int i = 0; i < m[v]; i++)
			if (!mark[ver=a[v][i]] && ver != mate[v])
			{
				mark[ver] = true;
				if ((vv=mate[ver])==-1 || !mark[vv] && dfs(vv))
				{
					mate[ver] = v;
					mate[v] = ver;
					return true;
...
bool can(int i1, int i2)
{
	if (a[i1][0] >= a[i2][0])
		return false;
	int s = sign(a[i1][0]-a[i2][0]);
	if (s==0) return false;
	REP(i,k)
		if (sign(a[i1][i]-a[i2][i])!=s)
			return false;
	return true;
}
