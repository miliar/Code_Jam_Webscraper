...
int n,k,price[maxn][maxn],maxc;
LL N[maxn],C[maxn];

void backtrack(int c,int yy,int i) {
  if(i==n) {
    maxc = c;
    return;
  }
  for(int j=1; j<maxc && j<=yy+1; j++) {
    if(!(C[j]&N[i])) {
      C[j]|=1LL<<i;
      backtrack(max(c,j),max(yy,j),i+1);
      C[j]&=~(1LL<<i);
    }
...
int main() {
  ...
  FOR(K,1,T) {
    ...
    memset(N,0,sizeof(N));
    memset(C,0,sizeof(C));
    REP(t,k-1) REP(i,n) FOR(j,i+1,n-1) {
      point A(t,price[i][t]), B(t+1,price[i][t+1]), 
	U(t,price[j][t]), V(t+1,price[j][t+1]);
      if(intersect_in(A,B,U,V)) N[i]|=1<<j, N[j]|=1<<i;
    }
    maxc = inf;
    backtrack(0,0,0);
