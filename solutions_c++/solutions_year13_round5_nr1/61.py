...
ld test(vll v, ll b, int k, ll sum)
{
	int cur = sz(v) - 1;
	ll actual = 0;
	ll bb = b;
...
int main()
{
	...
	fort
	{
		...
		for (int k = 1; k <= 36; ++k)
		{
			ll l = v[sz(v) - k], r = v[sz(v) - k] + b;
			while (r - l > 9)
			{
				ll t1 = (l + l + r) / 3;
				ll t2 = (l + r + r) / 3;
				ld test2 = test(v, b, k, t1);
				ld test3 = test(v, b, k, t2);
				if (test2 < 0)
					r = t1;
				else if (test3 < 0)
					r = t2;
			...
			for (ll i = l; i <= r; i++)
				a1 = max(a1, test(v,b,k,i));
		}


		cout.setf(ios::fixed);
		cout.precision(20);
		write "Case #" << gett << ": " << a1  << endl;

		std::cout.setf(ios::fixed);
		std::cout.precision(20);
		std::cout << "Case #" << gett << ": " << a1 << endl;
	}
}