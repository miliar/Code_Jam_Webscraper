...
void Case::solve(Result &result)
{
    double answer = 0;
    VLL their_bets(37);
    REP(i, N) their_bets[i] = X[i];
    SORT(their_bets);
    FOR(k, 1, 37) if (their_bets[k] > their_bets[k - 1]) {
        VLL my_bets(37);
        REP(i, k) my_bets[i] = their_bets[k - 1] - their_bets[i];
        LL spent = std::accumulate(ALL(my_bets), 0LL);
        if (spent > B) continue;
        REP(j, k) if (spent + j <= B) {
            LL extra_rows = min((B - spent - j)/k, their_bets[k] - their_bets[k - 1] - 1);
            double profit = 36.0*(accumulate(my_bets.begin(), my_bets.begin() + k - j, 0LL) + extra_rows*(k - j))/(k - j)
                            - (spent + j + extra_rows*k);
