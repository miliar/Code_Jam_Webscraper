...
int mainf()
{
	...
	ft(win,1,36)
	{
		ft(i,win,36)
		{
			lli sum=b+win;
			rep(j,i) sum+=x[j];
			lli put=min(sum/i,x[i]-1);
			if (put-1>=x[win-1]&&put>=x[i-1]&&put<=x[i])
			{
				db get=0;
				prt(win),prt(i),prt(put),prt(sum-win);
				rep(j,win) get+=(db(36)/db(win)-db(1))*db(put-1-x[j]);
				//prt(get);
				repf(j,win,i) get-=db(put-x[j]);
				//prt(get);
				cmax(ans,get);
			}
		}
