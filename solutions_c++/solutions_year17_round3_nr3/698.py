...
int CodeJamMain()
{
    ...
    for (int caseNum = 0; caseNum < T; caseNum++)
    {
        ...
        for (int i = 1; i < N; i++)
        {
            ...
            long long delta = target - P[0];
            if (U >= delta * i)
            {
                for (int j = 0; j < i; j++) P[j] += delta, U -= delta;
            }
            else
            {
                long long inc = U / i;
                for (int j = 0; j < i; j++) P[j] += inc, U -= inc;
                break;
            }
        }
        long long  delta = 10000 - P[0];
        long long inc = U / N;
        for (int i = 0; i < N; i++) P[i] += inc, U -= inc;
        for (int i = 0; i < N; i++) if (U > 0) P[i]++, U--;
