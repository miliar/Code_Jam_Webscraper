...
void Solve()
{
	...
	if (!bDoIt)
		return;
	int pasadas = 0;
	while (U > 0 && CanImprove() && pasadas < 100000)
	{
		++pasadas;
		int minVal = Min(-1);
		double minProb = P[minVal];
		assert(minVal != -1);
		int minCount = ValCount(P[minVal]);
		assert(minCount >= 1);
		int secondMin = Min(P[minVal]);
		double toRep = 0;
		if (secondMin == -1)
		{///Todos iguales
			double allVal = minProb;
			toRep = U / N;
			double maxToRep = 1.0 - allVal;
			if (toRep > maxToRep)
				toRep = maxToRep;
			U = 0;
		}
		...
		{
			double secondMinProb = P[secondMin];
			assert(minProb < secondMinProb);
			double delta = secondMinProb - minProb;
			toRep = U / minCount;
			if (toRep > delta)
				toRep = delta;
			U -= toRep*minCount;
