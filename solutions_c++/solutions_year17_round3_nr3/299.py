...
void Solver::read(std::istream &in) {
    in >> n_ >> k_ >> u_;
    p_ = readVector<double>(in, n_);
}


void Solver::saw(std::ostream &out) {
    std::sort(all(p_), std::greater<double>());
    vector<double> room(k_);
    vector<double> acc(k_);
    vector<double> take(k_);
    for (int i = 0; i < k_; ++i) {
        room[i] = 1.0 - p_[i];
        acc[i] = room[i] + (i == 0 ? 0.0 : room[i - 1]);
    }
    for (int i = 0; i < k_; ++i) {
        int j = k_ - i - 1;
        double available = u_ / (i + 1);
        double prevp = j == 0 ? 1.0 : p_[j - 1];
        double need = prevp - p_[j];
        take[j] = min(available, need);
        u_ -= (i + 1) * take[j];
        if (u_ < 0)
            u_ = 0;
    }
