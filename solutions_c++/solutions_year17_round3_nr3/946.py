...
const int N = 1e6 + 5, M = 1e9 + 7, NIL = -1;
...
double go(int pos, int tak) {
    if(pos == n) return (tak >= k);
    double &ret = dp[pos][tak];
    if(vis[pos][tak]) return ret;
    vis[pos][tak] = 1;
    ret = 0;
    ret += p[pos] * go(pos + 1, tak + 1);
    ret += (1 - p[pos]) * go(pos + 1, tak);
    return ret;
}

int main() {
    READ("C-small-1-attempt0.in");
    WRITE("C-small-1-attempt0.out");
    CASE() {
        cin >> n >> k;
        cin >> u;
        FOR(i, 0, n) cin >> p[i];
