...
template <typename T> T get_in() { T t; cin >> t; return t; }

int main() {
    int T;
    cin >> T;
    for (int C = 1; C <= T; ++C) {
        const int n = get_in<int>();
        const int k = get_in<int>();
        assert(n == k);

        const double u = get_in<double>();
        ...
        rep(_, 10000) {
            ...
            double needed_u = 0;
            rep(i, n) {
                needed_u += max<double>(0.0, mid - p[i]);
            }
            if (u >= needed_u) {
                low = mid;
            }
