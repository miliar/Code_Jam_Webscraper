...
int main() {
	...
	rep(ii, T) {
		...
		int W = id.size();
		int src = W * 2 + N, dst = src + 1;
		MaximumFlow mf; mf.init(dst + 1);
		mf.add(src, W * 2 + 0, INF);	//En[0]
		mf.add(W * 2 + 1, dst, INF);	//Fr[0]
		rep(i, N) each(j, words[i]) {
			mf.add(W * 2 + i, *j, INF);		//En -> en
			mf.add(W + *j, W * 2 + i, INF);	//!fr -> En <-> Fr -> fr
		}
		rep(j, W) {
			mf.add(j, dst, 1);		//en: cost
			mf.add(src, W + j, 1);	//fr: cost
		}
		int ans = mf.maximumFlow(src, dst) - W;
		printf("Case #%d: %d\n", ii+1, ans);
	}
	return 0;
}
