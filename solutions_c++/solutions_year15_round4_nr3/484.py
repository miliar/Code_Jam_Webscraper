/* My Template for the Google Code Jam.
 *
 * Compile: g++ -std=c++11 -lgmp -lgmpxx
 *  - I'm probably using some C++11 features.
 *  - I might use GMP (GNU Multiple Precision Arighmetic Library) so
 *    I'm including the library in the template even if they
 *    won't be used.
 *
 * This code is ugly but it works - otherwise you wouldn't be reading
 * it, right?
 */
#include <map>
#include <cassert>
//#define NDEBUG

#include <cstdlib>
#include <cstdint>
#include <cmath>

#include <iomanip>
#include <iostream>
#include <algorithm>
#include <string>
#include <vector>
#include <queue>
#include <sstream>
#include <set>

#include <gmpxx.h>

using namespace std;
typedef size_t szt;

typedef long long ll;
typedef unsigned long long ull;

typedef mpz_class mpz;

template<typename T>
void read_to_vector(size_t N, std::vector<T> &v) {
  for(size_t i=0; i<N; i++) {
    T tmp;
    std::cin >> tmp;
    v.push_back(tmp);
  }
}
template<typename T>
void print_vector(const std::vector<T> &v) {
  for(auto it = v.begin(); it<v.end(); it++) {
    if(it!=v.begin())
      std::cout << " ";
    std::cout << (*it);
  }
}

//Copy functions from TCR here.



//Solution:

ll solve() {
  int N;
  cin >> N;

  string tmp;
  getline(cin, tmp);

  vector<vector<string> > sentences (N);
  for(int i=0; i<N; i++) {
    string line;
    getline(cin, line);
    istringstream iss(line);
    while(iss) {
      string s;
      iss >> s;
      if(s.size()>0)
	sentences[i].push_back(s);
    }
    //    cout << "Read:" << line << "\n";
  }

  map<string, int> strmasks;
  for(int i=0; i<N; i++) {
    for(auto s : sentences[i]) {
      auto it = strmasks.find(s);
      if(it == strmasks.end()) {
	strmasks.insert(std::pair<string,int> (s, 0));
	it = strmasks.find(s);
      }
      strmasks[s] |= 1<<(N-1-i);
    }
  }


  ll count = 1e9;
  //  cerr << N;
  for(int mask=1<<(N-1); mask < 3<<(N-2); mask++) {
    ll thiscount = 0;
    for(auto pair : strmasks) {
      //      cout << pair.first << ":" << pair.second << "\n";
      if ((pair.second & mask) && (pair.second & ~mask))
	thiscount++;
    }

    count = min(count, thiscount);
  }

  return count;


}

//This is executed before any input is read.
void pre_compute() {
  
}

int main() {
  std::cout << std::setprecision(15);
  pre_compute();
  size_t T;
  std::cin >> T;
  for(size_t i=1; i<=T; i++) {
    //CHOOSE A
    auto res = solve();
    std::cout << "Case #" << i << ": " << res << "\n";
  }

  return 0;
}
