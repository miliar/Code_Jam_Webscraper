...
so far as part of any number she has named. Once she has seen each of the ten digits at least once, 
...
In Case #1, since 2 × 0 = 0, 3 × 0 = 0, and so on, Bleatrix will never see any digit other than 0, 
...
In Case #3, Bleatrix will name 2, 4, 6... and so on. She will not see the digit 9 in any number until 90, 
at which point she will fall asleep. By that point, she will have already seen the digits 0, 1, 2, 3, 4, 5, 6, 7, and 8, 
which will have appeared for the first time in the numbers 10, 10, 2, 30, 4, 50, 6, 70, and 8, respectively.

In Case #4, Bleatrix will name 11, 22, 33, 44, 55, 66, 77, 88, 99, 110 and then fall asleep.

Case #5 is the one described in the problem statement. Note that it would only show up in the Large dataset, 
and not in the Small dataset.
*/
...
int FindDigits(size_t num, unordered_map<int, bool>& found_digits, int found_num)
{
	while(num != 0)
	{
		int d = num % 10;
...
int main()
{
    ...
    while(T--)
    {
    	...
    	if(N == 0)
    	{
        	cout << case_str << count++ << colon << "INSOMNIA\n";
    	}
    	else
    	{
    		...
    		while(has_found != 10)
    		{
    			i++;
    			size_t num = i*N;
    			has_found = FindDigits(num, digits, has_found);
			}
        	cout << case_str << count++ << colon << i*N << std::endl;
    	}
    }
    return 0;
}
