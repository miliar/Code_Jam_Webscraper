...
void doCompuatation(const int& iTC, const int& iSleep)
{
				//std::cout << "\n\nComputing for Case#" <<iTC<<": " << iSleep<<std::endl;
				bool digitsMarker[10] = {false};
				long int currNumber = iSleep;
				int iMultiplier = 2;
				if(currNumber == 0)
				{
								std::cout << "Case #" <<iTC<<": " << "INSOMNIA"<<std::endl;
				}
				else
				{
								while (iMultiplier != INT_MAX)
								{
												int div = currNumber;
												while(div > 0)
												{
																digitsMarker[div%10] = true;
																div = div/10;
												}
												int j = 0;
												//std::cout<<"Reached so far for " << currNumber<<"\t"<<std::endl;
												//for( ;j<10,digitsMarker[j]; ++j) 
												bool found = true;
												for( ;j<10; ++j) 
												{
																if(digitsMarker[j] == true)
																{
														//						std::cout<<j<<"\t";
																}
																found &= digitsMarker[j];
												}
											//	std::cout<<std::endl;
												//if(j == 10)
												if(found)
												{
																std::cout << "Case #" <<iTC<<": " << currNumber<<std::endl;
																return;
												}
												else
												{
																currNumber = iMultiplier*iSleep;
//																std::cout << "Next number is: "<<currNumber<<"\t";
																iMultiplier++;
												}
								}
								std::cout << "Case #" <<iTC<<": " << "INSOMNIA"<<std::endl;
...
int main()
{
				int iNumTCs;
				std::cin>>iNumTCs;
				int* sleepNumbers = new int[iNumTCs];
				int* results = new int[iNumTCs];
				for(int i = 0 ; i < iNumTCs;++i)
				{
								std::cin>>sleepNumbers[i];
				}
				for(int i = 0 ; i < iNumTCs;++i)
								doCompuatation(i+1,sleepNumbers[i]);
				delete [] sleepNumbers;
				delete [] results;
				return 0;
}
