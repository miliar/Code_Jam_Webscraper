...
{
private:
  std::set<uint64_t> digits_;

	void add_digits(uint64_t num)
		{
			std::cout <<"Adding :"<<num<<std::endl;
			uint64_t ten_pows = 10;
			while(num)
			{
				uint64_t digit = num % ten_pows;
				digits_.insert(digit);
				std::cout<<"Inserting digit:"<<digit<<std::endl;
				num = num / ten_pows;
				ten_pows * 10;
			}
		}
...
public:
	TestCaseResult<Out> operator()(TestCase<In> &test_case)
	{
		...
		if ( test_case.number_ == 0 )
		{
			test_case_result.addResult("INSOMNIA");
			return test_case_result;
		}
	    uint64_t current_number = 0;
		while (digits_.size() < 10 )
		{
			current_number += test_case.number_;
			add_digits(current_number);
		}

		test_case_result.addResult(std::to_string(current_number));
...
int main(int argc, char* argv[])
{
	...
	ProblemSolver<uint64_t, std::string> solver(argv[1], "output.txt",
												CountingSheep<uint64_t, std::string>());
