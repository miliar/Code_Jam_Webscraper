...
void main(){
	...
	for (n = 0; n < T; n++){
		...
		if (N > 0){
			zIntArray z;
			for (i = 0; i < 10; i++)z.Add() = 0;
			for (k = 1; true; k++){
				for (i = N*k; i > 0; i /= 10)z[i % 10]++;
				for (i = 0; i < 10; i++)if (z[i] == 0)break;
				if (i == 10)break;
			}
		}

		res.Add() = "Case #" + to_string(n + 1) + ": ";
		if (N > 0)res[-1] += to_string(N*k);
		else res[-1] += "INSOMNIA";
		std::cout << (n + 1) << endl;
	}	
	...
	for (i = 0; i < 10000; i++){
		...
		b = true;
		for (j = 0; j < 10; j++)if (z[j] == 0)b = false;
		if (!b)res.Add() = "!!! " + to_string(t);
	}*/
