...
int main() {
	...
	repInit(caseIdx, 1, caseCount + 1) {
		...
		if (inputNum == 0) {
			...
			int tempDigit;
			bool seenAllDigits;
			LongLong currResult, tempInputNum;
			bool seenStatus[10] = {false, false, false, false, false, false, false, false, false, false};
			int idxMultiplier = 1;
			
			do {
				seenAllDigits = true;
				tempDigit = -1;
				result = idxMultiplier * inputNum;
				// Get digits
				tempInputNum = result;
				while(tempInputNum > 0) {
					tempDigit = tempInputNum % 10;
					seenStatus[tempDigit] = true;
					tempInputNum = tempInputNum / 10;
				}
				// Check digits seen status
				repInit(idxDigit, 0, 10) {
					if (seenStatus[idxDigit] == false) {
						seenAllDigits = false;
						break;
					}
				}
