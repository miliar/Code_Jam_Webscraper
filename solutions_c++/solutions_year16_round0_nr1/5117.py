...
int main(int argc, char *argv[])
{
    ...
    QFile file_map("d:\\Downloads\\t1.db");
    if (!file_map.open(QIODevice::ReadOnly | QIODevice::Text)) {
        return 2;
    }

    QTextStream map(&file_map);

    QMap<QString, QString> map_r;
    QStringList lst_s;
    while (!map.atEnd()) {
        line = map.readLine();
        lst_s = line.split(" ");
        map_r[lst_s.at(0)] = lst_s.at(1);
    }
/*
    int r, idx, last_res_idx, num;
    QList<int> lst;
    QString s;
    bool added;
    for (int i = 0; i <= 1000000; ++i) {
        ...
        while(true) {
            added = false;
            s = QString::number(r * idx);
            for (int j = 0; j < s.length(); ++j) {
                num = QString(s.at(j)).toInt();
                if (lst.indexOf(num) == -1) {
                    added = true;
                    lst.append(num);
                    last_res_idx = idx;
                }
    ...
    while (!in.atEnd()) {
        ...
        //sl = line.split(" ");
            answer = "Case #" + QString::number(++CaseNum) + ": " + map_r[line];
            out << answer;
            endl(out);
            if (is_debug) {
                qDebug() << line << answer;
