...
struct RealCase : public Case
{
    ...
    LL calc(int cur, int par, int av);
...
vector<int> primes;
map<int, vector<pair<int,int> > > factors;

const vector<pair<int,int> > &factorize(int n)
{
    map<int, vector<pair<int,int> > >::const_iterator it = factors.find(n);
    ...
    FORIT(it2, primes) {
        ...
        if (cnt > 0) facts.push_back(make_pair(p, n));
    }
    return factors[n] = facts;
}

void add(vector<pair<int,int> > &a, vector<pair<int,int > > b)
{
    vector<pair<int,int > > c;
    vector<pair<int,int > >::iterator i = a.begin(), j = b.begin();
    ...
    while (i != a.end()) { c.push_back(*i); ++i; }
    while (j != b.end()) { c.push_back(*j); ++j; }
    a = c;
}

void remove(vector<pair<int,int > > &a, vector<pair<int,int > > b)
{
    vector<pair<int,int > >::iterator i = a.begin(), j = b.begin();
    while (i != a.end() && j != b.end())
    {
...
LL mult(const vector<pair<int,int> > &a)
{
    LL res = 1;
    FORIT(it, a) {
        REP(n, it->second) res = (res * it->first)%MOD;
    }
    return res;
}

LL ncr(int tot, int tak)
{
    LL res = 1;
    REP(n, tak) res = res*(tot - n)%MOD;
    //REP(n, tak) res /= tak;
    return res;
/*    
    vector<pair<int,int> > a;
    REP(n, tak) add(a, factorize(tot - n));
    REP(n, tak) remove(a, factorize(tak));
    return mult(a);
*/}
...
LL RealCase::calc(int cur, int par, int av)
{
    P[cur] = par;
    ...
    int C = SZ(adj[cur]) - (par >= 0);
    if (C == 0) return 1;
    if (av < C) return 0;
    LL res = 1;
    FORIT(it, adj[cur]) if (*it != par) {
        res = (res * calc(*it, cur, av - C))%MOD;
    }
    printf("%d  %d,%d %d\n", (int)res, av, C, (int)ncr(av, C));
    res = res*ncr(av, C)%MOD;
    return res;
}
