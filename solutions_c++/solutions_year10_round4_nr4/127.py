...
double calc(int *p1, int *p2, int *q)
{
	double r1 = dist(p1[0], p1[1], q[0], q[1]);
	double r2 = dist(p2[0], p2[1], q[0], q[1]);

	CIRCLE a(p1[0], p1[1], r1);
	CIRCLE b(p2[0], p2[1], r2);

	return CommonArea(a, b);
...
{
	//static const char * const config[] = {"D-sample.txt", NULL};
	static const char * const config[] = {"D-small-attempt0.in", "D_out_small.txt"};
