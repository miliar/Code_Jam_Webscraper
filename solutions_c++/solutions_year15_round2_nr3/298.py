...
int main()
{
	...
	while(T--)
	{
		test++;
		cin>>n;
		vector <pair<double,double> > hiker;
		FOR(i,n){
			double d,h,m;
			cin>>d>>h>>m;
			FOR(j,h){
				hiker.PB(make_pair(m,d));
				m++;
			}
		}
		sort(hiker.begin(),hiker.end());
		double m1 = (1.0 - (double)hiker[0].Y/360.0)*(double)hiker[0].X + (double)hiker[0].X;
		sh(m1);
		double m2 = (1.0 - (double)hiker[1].Y/360.0)*(double)hiker[1].X;
		sh(m2);
		int ans;
		if(m1<=m2)
			ans = 1;
