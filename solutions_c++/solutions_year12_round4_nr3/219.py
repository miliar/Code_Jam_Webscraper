...
bool legal(){
    ...
    REP(i, n-1){
        ...
        if (t < h[i]){
            FOR(j, i+1, n) if (h[j] > t || h[j] == t && j > a[i]) return false;
        }
        else {
            DB d = (t - h[i]) / (a[i] - i);
            //if (i == 1) printf("%d %d!: %.2lf\n", i, a[i], d);
            FOR(j, i+1, n) if ( sgn(h[j], d * (j-i) + h[i]) > 0 || sgn(h[j], t * (j-i) + h[i]) == 0 && j < a[i]) return false;
        }
    }
