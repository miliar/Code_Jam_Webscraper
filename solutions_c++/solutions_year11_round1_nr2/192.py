#include <cstdio>
#include <iostream>
#include <algorithm>
#include <vector>
#include <queue>
#include <stack>
#include <set>
#include <map>
#include <cstring>
#include <cstdlib>
#include <cmath>
#include <string>
#include <memory.h>
#include <sstream>

#define oo 1000000005
#define eps 1e-11

#define REP(i,n) for(int i = 0, _n = (n); i < _n; i++)
#define REPD(i,n) for(int i = (n) - 1; i >= 0; i--)
#define FOR(i,a,b) for (int i = (a), _b = (b); i <= _b; i++)
#define FORD(i,a,b) for (int i = (a), _b = (b); i >= _b; i--)
#define FOREACH(it,c) for (__typeof ((c).begin()) it = (c).begin(); it != (c).end(); it++)
#define RESET(c,x) memset (c, x, sizeof (c))

#define sqr(x) ((x) * (x))
#define PB push_back
#define MP make_pair
#define F first
#define S second
#define ALL(c) (c).begin(), (c).end()
#define SIZE(c) (c).size()

using namespace std;

const double PI = 2.0 * acos (0.0);

typedef long long LL;
typedef pair <int, int> PII;

inline int getInt () { int x; scanf ("%d", &x); return x; }
inline LL getLL () { LL x; scanf ("%lld", &x); return x; };
inline int NUM (char c) { return (int)c - 48; }
inline char CHR (int n) { return (char)(n + 48); }
template <class T> inline T MAX (T a, T b) { if (a > b) return a; return b; }
template <class T> inline T MIN (T a, T b) { if (a < b) return a; return b; }

inline int BPM (int a, int b, int MOD) {
    if (a == 0)
        return 0;
    if (b == 0)
        return 1 % MOD;
    if (b % 2 == 1)
        return (int)((LL) BPM (a, b - 1, MOD) * a % MOD);
    int tmp = BPM (a, b / 2, MOD);
    return (int)((LL) tmp * tmp % MOD);
}

inline int phi (int N) {
    int ret = N;
    for (int i = 2; N > 1; i++) {
        if (i * i > N) {
            ret -= ret / N;
            break;
        }
        if ( N % i == 0 ) {
            ret -= ret / i;
            while (N % i == 0)
                N /= i;
        }
    }
    return ret;
}

#define MAXROW1 1000
#define MAXCOL1 1000
#define MAXROW2 100
#define MAXCOL2 100

int shift[MAXROW1 + 5];
int pi[MAXROW2 + 5];

void compute_prefix(char *P, int m, int *pi) { 
	int k = pi[0] = -1; 
	for (int q = 1; q<m; ++q) { 
		while (k >= 0 && P[k + 1] != P[q]) k = pi[k]; 
		if (P[k + 1] == P[q]) ++k; 
		pi[q] = k;
	}
}

int kmp_match(char *T, char *P, int *shift) { 
	int n, m, q = -1, shifts = 0; n = strlen(T); m = strlen(P); 
	compute_prefix(P, m, pi);
	REP(i,n) { 
		while (q > -1 && P[q + 1] != T[i]) q = pi[q]; 
		if (P[q + 1] == T[i]) ++q; 
		if (q == m - 1) { 
			shift[shifts++] = i - m + 1; 
			q = pi[q]; 
		} 
	} 
	return shifts; 
}

inline void OPEN (string s) {
    freopen ((s + ".in").c_str (), "r", stdin);
    freopen ((s + ".out").c_str (), "w", stdout);
}

// ptrrsn_1's template

#define MAXWLENGTH 10
#define MAXN 10000
#define MAXALPHA 26

char words[MAXN + 5][MAXWLENGTH + 5];
char qstring[MAXALPHA + 5];
int N, M;
char pattern[MAXWLENGTH + 5];
bool ada[500];
bool guesswrong[500];

int main () {
	OPEN("prog");
	int nTC;
	scanf("%d", &nTC);
	FOR (tc, 1, nTC) {
		scanf("%d%d", &N, &M);
		REP (i, N) {
			scanf("%s", words[i]);
		}
		cout << "Case #" << tc << ":";
		REP (i, M) {
			scanf("%s", qstring);
			int mx = -1;
			int idx = -1;
			REP (j, N) {
				RESET(ada, 0);
				RESET(guesswrong, 0);
			//	printf("Checking %s\n", words[j]);
				int underscore = 0;
				for (int k = 0; words[j][k]; k++) {
					pattern[k] = '_';
					underscore++;
				}
				pattern[underscore] = 0;
				int miss = 0;
				REP (q, MAXALPHA) {
					bool qboleh = false;
					REP (p, N) {
						if (strlen(words[p]) == strlen(words[j])) {
							//printf("(%s)\n", words[p]);
							bool flag = true, found = false;
							for (int k = 0; words[p][k]; k++) {
								if (guesswrong[words[p][k]]) {
									flag = false;
									break;
								}
								if (pattern[k] != '_' && pattern[k] != words[p][k]) {
									flag = false;
									break;
								}
								if (pattern[k] == '_' && ada[words[p][k]]) {
									flag = false;
									break;
								}
								if (words[p][k] == qstring[q]) {
									found = true;
								}
							}
							if (flag && found) {
								qboleh = true;
							}
						}
					//	printf (" - %s : %d\n", words[p], qboleh);
					}
					if (qboleh) {
					//	printf("guess! %c\n", qstring[q]);
						bool flag = false;
						for (int k = 0; words[j][k]; k++) {
							if (words[j][k] == qstring[q]) {
								pattern[k] = qstring[q];
								ada[qstring[q]] = true;
								flag = true;
								underscore--;
							}
						}
						if (!flag) {
							guesswrong[qstring[q]] = true;
							miss++;
						} else if (underscore == 0) {
							break;	
						}	
					//	printf ("pattern: ");
					//	puts(pattern);
					}
				}
				if (miss > mx) {
					mx = miss;
					idx = j;
				}
			//	printf("qstring = %s, guess = %s, miss = %d, pattern = %s\n", qstring, words[j], miss, pattern);
			}
			printf (" %s", words[idx]);
		}
		puts("");
	}
    return 0;
}
