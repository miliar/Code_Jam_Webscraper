...
ostream & operator<<(ostream &out, const pair<T,S> &p){ return out << "(" << p.first << "," << p.second << ")";}
...
string branchit(char ch, string str){
   FOR(i,0,str.length()) if(str[i] != ch) str[i] = '.';
   return str;
}

PAIR solve(int pos, const string &list, const vector<int> & valid){
    if(valid.size() == 0) return make_pair(INF,INF); 
    if(valid.size() == 1 || pos == list.length()) return make_pair(0,valid[0]);


    map<string, vector<int> > branches;
    FOR(i,0,valid.size()) branches[branchit(list[pos],words[valid[i]])].push_back(valid[i]);

    PAIR ret = make_pair(INF,valid[0]); 
    ITER(it,branches){
//	if(it->second.size() == 0){ cerr << "PROBLEM" << endl;}
        PAIR cur = solve(pos+1,list,it->second);
//	cerr << cur << " " << ret << endl;
	if(branches.size() != 1 && it->first.find(list[pos])==string::npos) cur.first--;
	if(cur < ret) ret = cur;
    }
//    cerr << "SOLVE: " << ret << endl;
    return ret;
...
PAIR solve(int length, string list){
    vector<int> valid = parts[length];
    return solve(0,list,valid);
}

PAIR solve(string list){
//   cerr << "SOLVING: " << list << endl;
   FOR(i,0,11) parts[i].clear();
   FOR(i,0,words.size()) parts[words[i].length()].push_back(i);
   PAIR ret = make_pair(0,0);
   FOR(i,0,11) if(parts[i].size()>0){
	PAIR cur = solve(i,list);
//	cerr << i << " " << cur.first << " " << cur.second << " " << words[cur.second] << endl;
	if(cur < ret) ret = cur;
   }
   return ret;
}
...
int main(){
  ...
  FOR(i,0,_T){
    ...
    cin >> N >> M;
    words.resize(N);
    FOR(j,0,N) cin >> words[j]; 
    FOR(j,0,M){
        string list;
        cin >> list;
        cout << " " << words[solve(list).second];
    }
    cout << endl;
  }
  return 0;
