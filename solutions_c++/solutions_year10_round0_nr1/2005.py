...
int main() {
	...
	int kases, n, k; scanf("%d",&kases);
	int rep[32];
	REP(t,kases) {
		scanf("%d%d",&n,&k); int cnt = 0; memset(rep, 0, sizeof rep);
		while(k) rep[cnt++] = k%2, k >>= 1;
		bool solved = true;
		REP(i,n) solved &= rep[i];
		printf("Case #%d: ",t+1); puts(solved ? "ON" : "OFF");
	}
	return 0;
}
