...
public:
	void solve(std::istream& in, std::ostream& out) {
		...
		vector<vector<int>> have = make_vector<int>(n, p, 0);
		for (int i: range(n)) {
			for (int j: range(p)) {
				in >> have[i][j];
			}
		...
		for (int count = 1; count <= 1100000; ++count) {
			for (int i: range(n)) {
				int64_t cur_need_mn = divideCeil(need[i] * 9LL * count, 10LL);
				int64_t cur_need_mx = divideFloor(need[i] * 11LL * count, 10LL);
				while (pos[i] != p && have[i][pos[i]] < cur_need_mn) {
					++pos[i];
				}
			}
			bool stop = false;
			while (true) {
				...
				for (int i: range(n)) {
					...
					if (pos[i] == p) {
						stop = true;
						ok = false;
					} else if (have[i][pos[i]] > cur_need_mx) {
						ok = false;
					}
				}
				if (ok) {
