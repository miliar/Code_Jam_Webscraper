...
int main(){
  ...
  rep(kase,T){
    ...
    int re=0;
    rep(i,n) sort(v[i],v[i]+m);
    vector<int> inds(n);
    ll t=1;
    while(*max_element(all(inds))<m){
      int f=1;
      rep(i,n){
	while(inds[i]<m && 10*v[i][inds[i]]<9*t*a[i]) ++inds[i];
	if(inds[i]==m || 10*v[i][inds[i]]>11*t*a[i]) f=0;
      }
      if(f){
	rep(i,n) ++inds[i];
	++re;
	...
	++t;
	if(inds[0]<m){
	  while(10*v[0][inds[0]]>11*t*a[0]) ++t;
	}
      }
    }
