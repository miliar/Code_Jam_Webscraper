...
int main()
{
	...
	jk(t)
	{
		...
		auto valid = [&](int s, int i, int j) {
			ll lo = (ll(s) * 9 * R[i] + 9) / 10;
			ll hi = (ll(s) * 11 * R[i]) / 10;
			return lo <= Q[i][j] && Q[i][j] <= hi;
		};
		set<int> servings;
		jij(n, p)
		{
			int z = (int) (Q[i][j] / 0.9 / R[i]);
			servings.insert(z);
			servings.insert(z + 1);
			int a = z;
			...
			if ((valid(a, i, j) || valid(a+1, i, j)) != (valid(b, i, j) || valid(b+1, i, j)))
			{
				cerr << a << endl;
				cerr << b << endl;
				cerr << R[i] << endl;
		...
		while (!servings.empty())
		{
			...
			ji(n)
			{
				...
				if (Q[i][0] > hi)
				{
					servings.erase(servings.begin());
					goto retr;
				}
			}
			++ans;
			ji(n) Q[i].erase(Q[i].begin());
			retr:;
		}
		en:
		cout << ans << endl;
