...
//createDummyTestCases();
    freopen ("E:/outputSmall1.txt","w",stdout);
    ll test_cases=1,seed=1,f,ans=0,g;
  cout << setprecision(15) << fixed;
  //createDummyTestCase();
    cin>>test_cases;
    string s,s1,s2[1010];
    vector<long>sherlock;
    vector<long>moriarty;
    //printV();
    ll reqWeight[100];
    ll packageMatrix[55][55];
    ll pos[55];
    ll p;
    for(long T=0;T<test_cases;T++){
        ...
        for(long i=0;i<n;i++){
            for(long j=0;j<p;j++){
                cin>>packageMatrix[i][j];
                long maxpack=(10*packageMatrix[i][j])/(reqWeight[i]*9);
                if((packageMatrix[i][j]*10)>(maxpack*reqWeight[i]*11))
                    packageMatrix[i][j]=-1;
             //   if(maxpack*reqWeight[i]<packageMatrix[i][j])
            }
            sort(packageMatrix[i],packageMatrix[i]+p);
            while(pos[i]<p&&packageMatrix[i][pos[i]]<0)
                pos[i]++;
        }
        long maxpos=0,kits=0;
        while(maxpos<p){
            ll maxVal=0,maxNum=0;
            for(long i=0;i<n;i++){
              ll num= ((packageMatrix[i][pos[i]]*10)/(reqWeight[i]*11));
            if(num*reqWeight[i]*11<(packageMatrix[i][pos[i]]*10))
                num++;
            if(maxNum<num)
                maxNum=num;
              /*ll val= (packageMatrix[i][pos[i]]*10000000)/reqWeight[i];
              if(val>maxVal){
                maxVal=val;
                maxpos=i;
                maxNum=packageMatrix[i]
              } */
            }
            for(long i=0;i<n;i++){
               while(pos[i]<p&&(packageMatrix[i][pos[i]]*10)<(maxNum*reqWeight[i]*9))
                pos[i]++;
                if(pos[i]>maxpos)
                    maxpos=pos[i];
                if(pos[i]>=p)
