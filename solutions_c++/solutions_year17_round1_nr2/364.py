...
void build()
{
    REP1(i,1,n) sort(ALL(q[i]),greater<int>());
}
...
void sol()
{
    ...
    while(1)
    {
        {
            int min_size=1e9;
            REP1(i,1,n) min_size=min(min_size,SZ(q[i]));
            if(min_size==0) break;
        }
        
        pll lo{0,1},hi{100000000,1};
        REP1(i,1,n)
        {
            pll cl{10*q[i].back(),11*r[i]};
            pll cr{10*q[i].back(),9*r[i]};
            if(cmp(lo,cl)) lo=cl;
            if(cmp(cr,hi)) hi=cr;
        }

        ll up=(lo.F+lo.S-1)/lo.S;
        if(up <= hi.F/hi.S)
        {
            ans++;
            REP1(i,1,n) q[i].pop_back();
        }
        else
        {
            pll mi{100000000,1};
