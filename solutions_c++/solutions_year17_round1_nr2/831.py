...
void input()
{
    ...
    for (int i=1;i<=n;i++)
        for (int j=1;j<=p;j++) read(value[i][j]);
}
void pre()
{
    for (int i=1;i<=n;i++)
...
//            cout<<value[i][j]<<" "<<a[i]<<"   "<<tmpx<<" "<<tmpy<<endl;
            if (tmpy<=tmpx) pack[i][j]=node(tmpy,tmpx);else pack[i][j]=node(-1,-1);
        }
...
void work()
{
    ...
    while (flag)
    {
        ...
        {
            int q=pos[i];
            while (q<=p&&pack[i][q].y<nowk) q++;
            pos[i]=q;
            if (q>p) {flag=false;break;}
        }
        if (!flag) break;
        bool f=true;
        for (int i=1;i<=n;i++)
        {
            if (pack[i][pos[i]].x<=nowk&&pack[i][pos[i]].y>=nowk)continue;
            else f=false;break;
        }
        if (f)
        {
