...
bool operator<(Rec& rec1, Rec& rec2) {
	if (rec1.v[0] < rec2.v[0]) return true;

	if (rec1.v[0] == rec2.v[0] && rec1.v.back() < rec2.v.back())
		return true;
	
	return false;
}
bool operator==(const Rec& rec1, const Rec& rec2) {
	// sort(rec1.v.begin(), rec1.v.end());
	// sort(rec2.v.begin(), rec2.v.end()); 

	if (rec1.v.empty() || rec2.v.empty()) return false;

	if (rec1.v.back() >= rec2.v[0] && rec1.v[0] <= rec2.v.back())
		return true;
...
int main(int argc, char* argv[]) {
	...
	for (int t = 0; t < T; t++) {
		...
		vector< vector<Rec> > packages(n); 
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < p; j++) {
				int num = 0; inf >> num;

				double maxNum = num * 1.0 / 0.9 / used[i]; 
				double minNum = num * 1.0 / 1.1 / used[i]; 

				Rec possible; 
				for (int k = maxNum; k >= minNum; k--) {
					possible.v.push_back(k); 
				}
				sort(possible.v.begin(), possible.v.end());
				if (!possible.v.empty())
					packages[i].push_back(possible);
			}
		...
		for (int i = 0; i < packages[0].size(); i++) {
			...
			for (int j = 1; j < packages.size(); j++) {
				if (std::find(packages[j].begin(), packages[j].end(), packages[0][i]) == packages[j].end()) {
					ok = false;
					break;
				}
			}
			if (ok) {
				result++; 
				for (int j = 1; j < packages.size(); j++) {
					packages[j].erase(std::find(packages[j].begin(), packages[j].end(), packages[0][i]));
				}
			}
		}
