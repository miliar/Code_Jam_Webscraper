...
class Main{
	...
	void run(){
		...
		for(int q:range(T)){
			...
			vector<ll> as(N); cin >> as;
			vector<vector<ll>> ps(N,vector<ll>(P));
			for(int n:range(N))for(int p:range(P)){
				cin >> ps[n][p];
			}
			for(int n:range(N)) sort(ALL(ps[n]));

			vector<vector<ll>> ls(N,vector<ll>(P));
			vector<vector<ll>> rs(N,vector<ll>(P));

			for(int p:range(P))	for(int n:range(N)){
				int l,r;
				{
					ll lc = 0, rc = 2000000;
					while(rc - lc > 1){
						ll mc = (lc+rc)/2;
						if(9 * mc * as[n] <= 10*ps[n][p]){
							lc = mc;
						}else{
							rc = mc;
						}
			...
			for(ll c:cs){
				...
				for(int n:range(N)){
					bool ex = false;
					for(int p:range(P))if(!used[n][p] && ls[n][p] <= c && c  <= rs[n][p]){
						uses[n] = p;
						ex = true;
						break;
					}
