...
LL isValid(LL b, LL a){
    double bb = b;
    double aa = a;
    double avg = (bb*100.0/aa);
    if(avg>=90.0 && avg<=110.0) return 0;
    return 1;
}
...
int solve(){
    ...
    for(int i = 1;i<=P;i++){
        ...
        LL div = gudam[1][i];
        LL cnt = 1;
        while(cnt<=1000000+50){
            int ret = isValid(div, req*cnt);
            if(ret != 0){
                cnt++;
                continue;
            }
