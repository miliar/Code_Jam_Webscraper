...
// linkować z pthreads
#include <cassert>
   bool canWin (ll dwaN, ll P, ll k) {
      assert(k < dwaN);
      if (P >= dwaN) return 1;
      if (k == dwaN-1) return 0;
      if (k % 2 == 0) {
         return canWin(dwaN / 2, P, k / 2);
      } else {
         return canWin(dwaN / 2, P, k / 2 + 1);
      }
   }
   
   ll res (ll dwaN, ll P) {
      ll best = -1, from = 0, to = dwaN - 1;
      while (from <= to) {
         ll mid = (from + to) / 2;
         if (canWin(dwaN, P, mid)) {
            best = mid;
            from = mid+1;
         } else {
            to = mid-1;
...
struct Instance {
   ...
   void compute() { // should produce output and store it, not use IO; will run in parallel
      ll dwaN = 1LL << NN;
      ans = mp(dwaN - 2 - res(dwaN, dwaN - P), res(dwaN, P));
   }
