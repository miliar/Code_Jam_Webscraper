...
long long in_n;
long long in_p;
long long sol_ind_loser;
long long sol_de_winner;
void solve();
void solve_b();
int main(int argc, char **argv)
{
	...
	for(int it_t = 1; it_t <= in_t; it_t++)
	{
		...
		solve();
		solve_b();
		fprintf(fo, "Case #%d: %lld %lld", it_t, sol_ind_loser, sol_de_winner);
		fprintf(fo, "\n");
	}
	closeIOFile();
}
...
void solve()
{
	long long remain_rank = in_p - 1;
	ll t = 0;
	for(long long it_n = in_n - 1; it_n >=0; it_n--)
	{
		if(remain_rank >= (1<<it_n))
		{
			remain_rank -= (1<<it_n);
			t++;
		}
	...
	long long x = 0;
	for(ll cnt_t = 0; cnt_t <=t;cnt_t++)
	{
		x += (1<<cnt_t);
	}
	sol_ind_loser = x - 1;
	if(sol_ind_loser > (1<<in_n) - 1)
	{
		sol_ind_loser = (1<<in_n) - 1;
	}
	//sol_de_winner;
}
void solve_b()
