...
static vector<int> ary, ary1, ary2;
int main()
{
	scanf("%d", &T);
	FOREQ(t,1,T)
	{
		...
		ary.clear(); ary1.clear(); ary2.clear();
		int index = 0;
		int length = 0;
		int cnt = 0;
		while(index <= N)
		{
			FOR(i,0,pow(2,index))
			{
				cnt++;
	//			printf("%d\n", (int)pow(2,N-index)-1);
				ary.push_back((int)pow(2,N-index)-1);
				if (cnt == pow(2,N)) break;
			}
			index++; length++;
		}
		reverse(ALL(ary));
...
*/
		printf("Case #%d: ", t);
		printf("%d %d\n", (upper_bound(ALL(ary2),P-1)-ary2.begin())-1,
			(upper_bound(ALL(ary1),P-1)-ary1.begin())-1);
	}
