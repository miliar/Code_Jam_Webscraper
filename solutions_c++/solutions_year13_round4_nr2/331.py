...
static LL calc_worst(LL better)
{
    LL place = 0;
    LL chunk = teams;
    while (better > 0)
...
void Case::solve(Result &result)
{
    ...
    ::P = P;
    teams = 1LL << N;
    LL x = BINSEARCH(0, teams, calc_worst(_) >= P) - 1;
    LL y = BINSEARCH(0, teams, calc_best(teams - 1 - _) >= P) - 1;
    result.set(STR(x) + " " + STR(y));
}
