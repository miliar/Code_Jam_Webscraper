...
{    
    READCases 
    {
        ...
        int invited = 0, stand_up = 0;
        for (int shyness = 0 ; shyness <= max_shyness ; ++shyness)
        {
            int count = people[shyness] - '0';
            assert (0 <= count && count <= 9);

            if (count && stand_up < shyness)
            {
                int new_invited = shyness - stand_up;
                invited += new_invited;
                stand_up += count + new_invited;
            } else {
                stand_up += count;
            }
        }

        PRINTSolution(invited);
    }
