...
llu scan_llu()    {int ip=getchar_unlocked();llu ret=0;for(;ip<'0'||ip>'9';ip=getchar_unlocked());for(;ip>='0'&&ip<='9';ip=getchar_unlocked())ret=ret*10+ip-'0';return ret;}
 inline void fastRead_string(char *str){register char c=0;register int i = 0;while (c < 33){c = getchar_unlocked();}while (c > 65){str[i] = c;c = getchar_unlocked();i = i + 1;}str[i] = '\0';}
//end of fast input
    ...
    for(i=1;i<=t;i++)
    {
        ...
        for(int k=1;k<=smax;k++)
        {
            if(stand<k)
            {
            	if(s[k]>0)
            	{
            		//printf("%dAdding to Answer %d",k,k-stand);
              		ans+=k-stand;
              		stand+=k-stand+s[k];
            	}
            	else
