 ...
 * make sure she receives a standing ovation -- with every audience member
 * standing up and clapping their hands for her.
 * 
 * Initially, the entire audience is seated. Everyone in the audience has a
 * shyness level. An audience member with shyness level Si will wait until
 ...
 * seated at the beginning, but will stand up to clap later after she sees
 ...
 * audience to ensure that everyone in the crowd stands up and claps in the
 * end. Each of these friends may have any shyness value that you wish, not
 * necessarily the same. What is the minimum number of friends that you
 ...
 * level of the shyest person in the audience, followed by a string of Smax
 * + 1 single digits. The kth digit of this string (counting starting from
 * 0) represents how many people in the audience have shyness level k. For
...
unsigned
calculate(const string &s) {
  unsigned required_pep = 0;
  unsigned total_pep = 0;

  for (unsigned Si = 0; Si < s.length(); ++Si) {
    unsigned pep = s.at(Si) - '0';

    if (total_pep < Si) {
      required_pep += Si - total_pep;
      total_pep += Si - total_pep;
    }
    total_pep += pep;
  }
