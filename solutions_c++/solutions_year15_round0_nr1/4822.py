...
int main(int argc, char** argv)
{
    ...
    for(int eachTest=1; eachTest<=allTests; eachTest++){
        ...
        for(int i=0;i<shn.size();i++){
            peopleRequired = i;
            if(shn[i]==0) continue;
            if(peopleRequired<=peopleStanding){
                peopleStanding += shn[i];
            } else {
                friends += (peopleRequired - peopleStanding);
                peopleStanding += (peopleRequired - peopleStanding) + shn[i];
            }
            //cout<<peopleRequired<<"-----------"<<peopleStanding<<":::::"<<friends<<endl;
        }

        cout<<"Case #"<<eachTest<<": "<<friends<<endl;
