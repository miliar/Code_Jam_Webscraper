...
int main()
{
	...
	FORI(caseNumber, 1, T)
	{
		int SMax, totalFriendsNeeded = 0;
		string s;
		cin >> SMax;
		cin >> s;
		int currentClappersCount = 0;
		FOR(i, SMax + 1)
		{
			int numPeople = s[i] - '0';
			if (currentClappersCount < i)
			{
				int moreNeeded = i - currentClappersCount;
				currentClappersCount += moreNeeded;
				totalFriendsNeeded += moreNeeded;
			}
			currentClappersCount += numPeople;
		}
		cout << "Case #" << caseNumber << ": " << totalFriendsNeeded << endl;
	}
    return 0;
}
