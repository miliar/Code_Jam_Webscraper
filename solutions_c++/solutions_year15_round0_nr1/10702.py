...
}// sort(vt.begin(), vt.end(), compare); //in program vector pair
...
//template for sum of digits of number from 1 to n
...
//template for sum of digits of number from 1 to n over
...
char b[10005];
	...
	for(int i=0;i<l;i++)
				{ if(b[i]!=0)
				vt.push_back(make_pair(b[i]-48,i));
				}
				int count=0;;
				int ans=0;
 for(int i=0;i<vt.size();i++)
		{ if(i==0&&vt[0].second==0)
			{
			count=vt[i].first;
			}else
			{
            //cout<<vt[i].first<<" "<<vt[i].second<<endl;
            if(vt[i].second>count)
            {ans=ans+(vt[i].second-count);
        
            count=count+vt[i].second-count+vt[i].first;
          
            }else
            { count=count+vt[i].first;
