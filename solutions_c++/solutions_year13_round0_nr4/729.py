...
bool brute(vector<pair<int, vi> > &inp, int msk, vi & cur)
{
	if (msk == 0)
		return 1;
	if (mp.count(msk))
	...
	foria(inp)
		if ((msk & (1 << i)) && cur[inp[i].first])
		{
			--cur[inp[i].first];
			forja(inp[i].second)
				++cur[inp[i].second[j]];
			bool ans = brute(inp, msk &~ (1 << i), cur);
			forja(inp[i].second)
				--cur[inp[i].second[j]];
			++cur[inp[i].first];
			if (ans)
...
int main()
{
	...
	fort
	{
		...
		else
		{
			...
			while (msk != 0)
			{
				ans.push_back(mp[msk]);
				msk &= ~(1 << (ans.back() - 1));
			}
			foria(ans) write ans[i] << ' ';
		}
		writeln;
	}
