...
int main(){
	...
	for(int i1 = 0; i1 < ncase; i1++){
		...
		for(int i = 0; i < (1 << n); i++) dp[i].arr.clear();
		lq = rq = 0;
		q[rq++] = 0;
		for(; lq != rq; lq++){
			for(int i = 0; i < n; i++) if(q[lq] & (1 << i)){
				orig[need[i]]--;
				for(int j = 0; j < (int)key[i].size(); j++) orig[key[i][j]]++;
			}
			for(int i = 0; i < n; i++) if(!(q[lq] & (1 << i))) if(orig[need[i]] > 0){
				if(dp[q[lq] | (1 << i)].arr.size() == 0) {
					dp[q[lq] | (1 << i)].arr = dp[q[lq]].arr;
					dp[q[lq] | (1 << i)].arr.push_back(i);
					q[rq++] = q[lq] | (1 << i);
				}else if(dp[q[lq]] < dp[q[lq] | (1 << i)]){
					dp[q[lq] | (1 << i)].arr = dp[q[lq]].arr;
					dp[q[lq] | (1 << i)].arr.push_back(i);
				}
			}
		...
		printf("Case #%d:", i1 + 1);
		if(dp[(1 << n) - 1].arr.size()){
			for(int i = 0; i < n; i++) printf(" %d", dp[(1 << n) - 1].arr[i] + 1);
			printf("\n");
