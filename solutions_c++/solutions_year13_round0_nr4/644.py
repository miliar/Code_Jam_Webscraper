...
void get_keys(const vector<vector<int> > &treasures, const vector<int> &need, int opened, vector<int> &result) {
    REP(i, treasures.size()) {
        if (opened & (1 << i)) {
            --result[need[i]];
            REP(j, treasures[i].size()) {
                ++result[treasures[i][j]];
            }
        }
    }
}
...
int main(void) {
	...
	REP(__number_of_case, __number_of_cases) {
        ...
        REP(i, N) {
            ...
            int Ki;
            cin >> Ki;
            treasures[i].resize(Ki);
            cin >> treasures[i];
            REP(j, Ki) {
                --treasures[i][j];
            }
        }
        vector<vector<int> > keys(1 << N, vector<int> (200, 0));
        REP(i, 1 << N) {
            get_keys(treasures, need, i, keys[i]);
            REP(j, K) {
                ++keys[i][start_keys[j]];
            }
        }
        ...
        if (!ok[(1 << N) - 1]) {
            ...
            vector<bool> ok1(1<<N, false);
            ok1[(1 << N) - 1] = true;
            REPD(i, 1 << N) {
                if (ok1[i]) {
                    REP(j, N) {
                        if (i & (1 << j) && keys[i ^ (1 << j)][need[j]] > 0) {
                            ok1[i ^ (1 << j)] = true;
                        }
                    }
                }
            ...
            while (val != (1 << N) - 1) {
                //cout << val << '\n';
                REP(i, N) {
                    if (!(val & (1 << i)) && ok1[val | (1 << i)] && keys[val][need[i]] > 0) {
                        path.push_back(i);
                        val = val | (1 << i);
                        break;
                    }
