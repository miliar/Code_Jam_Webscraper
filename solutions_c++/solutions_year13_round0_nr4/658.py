...
struct door{
    int T; VI L; void input(){
        RD(T); CLR(L); Rush L.PB(RD());
    }
} A[N];
...
bool dfs(int k = 0, int s = 0){
    ...
    if (k == n){
        ...
        REP(i, n) if (!_1(s, i) && keys[A[i].T]){
            a[k] = i; In(i);
            if (dfs(k+1, s|_1(i))) return 1;
            Out(i);
        }
...
int main(){
    ...
    Rush{
        printf("Case #%d: ", ++Case);
        int k; CLR(vis); CLR(keys); RD(k, n); DO(k) ++keys[RD()];
        REP(i, n) A[i].input();
