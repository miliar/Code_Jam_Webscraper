...
bool isRecycled(int nn, int mm) {
    if (nn == mm) { return false; }
    DOUT("isRecycled? nn,mm="); DOUT(nn); DOUT(",");DLOUT(mm);

    ostringstream nnstr, mmstr;
    nnstr << nn;
    string nns = nnstr.str();
    int len = nns.sz;

    mmstr << mm << mm;
    string mms = mmstr.str();
    string modmms = mms.substr(0, 2*len-1);

    DOUT("isRecycled? len,nns,modmms="); DOUT(len); DOUT(","); DOUT(nns); DOUT(",");DLOUT(modmms);
    size_t loc = modmms.find(nns);
    DOUT(" ====> "); DLOUT(loc != string::npos);
