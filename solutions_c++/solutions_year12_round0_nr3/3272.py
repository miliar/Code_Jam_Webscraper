...
void CCodeJam1Dlg::OnBnClickedOk()
{
	...
	if(f.Open(_T("d:\\A-small-attempt1.in"),CFile::modeRead))
	{
		...
		if(f.Read(chData,f.GetLength()+1))
		{
			...
			CFile fOut(_T("E:\\Kim1.Out"),CFile::modeCreate | CFile::modeReadWrite);
...
void CCodeJam1Dlg::OnBnClickedOk2()
{
	...
	m = "";
	sprintf(g_Rank,"%s",m_strRank);
	if(f.Open(_T("d:\\B-small-attempt6.in"),CFile::modeRead))
	{
		char* chData = new char[f.GetLength()+1];
		if(f.Read(chData,f.GetLength()+1))
		{
			...
			int nCharectorCount[26];
			memset(nCharectorCount,0,sizeof(int)*26);
			CFile fOut(_T("E:\\Kim2_3.Out"),CFile::modeCreate | CFile::modeReadWrite);
...
void CCodeJam1Dlg::OnBnClickedOk3()
{
	...
	m = "";
	sprintf(g_Rank,"%s",m_strRank);
	if(f.Open(_T("d:\\C-small-attempt1.in"),CFile::modeRead))
	{
		char* chData = new char[f.GetLength()+1];
		if(f.Read(chData,f.GetLength()+1))
		{
			...
			for(int iter = 0 ; iter < nTestSize ; iter ++)
			{
				...
				for(int j = nMinData ; j <= nMaxData ; j ++)
				{
					CString strTemp;
					strTemp.Format("%d",j);
					for(int q= 0 ; q<strTemp.GetLength(); q++)
					{
						CString strCompare = strTemp.Mid(q) + strTemp.Left(q);
						int nData = atoi(strCompare);
