...
int main(){
	...
	for(cas = 1;cas<=T;cas++){
		...
		int num[101] = {0};
		for(i=0;i<m;i++){
			scanf("%d %d %d",&node.s,&node.e,&node.p);
			ans1 += (node.e -node.s)* (node.e -node.s -1)*-1 * node.p;
			for(j=node.s+1;j<=node.e;j++){
				num[j]+= node.p;
			}
		}
		for(i=0;i<=100;i++){
			...
			if(mi == 1000001)break;
			int count = 0;
			for(j=0;j<=100 &&k+j<=100;j++){
				if(num[k+j]==0)break;
				num[k+j] -= mi;
				count++;
			}
			for(j=1;j<=100&&k-j>=0;j++){
				if(num[k-j]==0)break;
				num[k-j] -=mi;
				count++;
			}
			ans2 += count * (count-1)*-1 * mi;
		}
		printf("Case #%d: %d\n",cas,(ans1 - ans2)/2);
	}
	return 0;
