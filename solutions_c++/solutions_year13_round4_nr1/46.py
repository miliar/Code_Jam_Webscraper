...
inline DB getTime(){
    ...
    timeval tv;
    gettimeofday(&tv, 0);
    return tv.tv_sec + tv.tv_visc * 1e-6;
#endif
}
...
bool cover(LL m , LL l , LL r){
    return l <= m && m <= r;
}
bool cover(rec P , rec Q){
    return cover(P.x , Q.x , Q.y) || cover(P.y , Q.x , Q.y) || cover(Q.x , P.x , P.y) || cover(Q.y , P.x , P.y);
}

vector<PLL> st, ed;

void dfs(int u){
    vis[u] = 1; st.PB(MP(A[u].x , A[u].p)); ed.PB(MP(A[u].y , A[u].p));
    REP(i, m) if (!vis[i]) if (cover(A[u] , A[i])) dfs(i);
}
...
/*#ifndef ONLINE_JUDGE
    ...
    Rush{
        ...
        Int ans = 0; tot = 0; RD(n, m); REP(i, m){
            A[i].input();
            ans += f(A[i].y - A[i].x) * A[i].p;
        }

        RST(vis); REP(i, m) if (!vis[i]){
            CLR(st, ed); dfs(i); SRT(st), SRT(ed);
            int op = SZ(ed) - 1;
            DWN(j, SZ(st), 0){
                while (op>0 && ed[op-1].fi >= st[j].fi) --op;
                int k = op;
                while (st[j].se){
                    LL t = min(st[j].se, ed[k].se);
                    ans -= f(ed[k].fi - st[j].fi) * t;
                    ed[k++].se -= t, st[j].se -= t;
                }
            }
        }
