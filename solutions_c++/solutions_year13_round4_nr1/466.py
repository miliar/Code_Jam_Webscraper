...
int main()
{
	scanf("%d",&T);
	jum[1] = 0;
	FORN(i,2,100) jum[i] = jum[i-1] + i - 1;
	REPN(cases,T)
	{
		printf("Case #%d: ",cases);
		data.clear();
		...
		REP(i,m)
		{
			FOR(j,i+1,m) 
			{
				if(data[j].fi > data[i].se) break;
				if(data[j].se > data[i].se) swap(data[i].se,data[j].se);
			}
			risan.se += jum[data[i].se - data[i].fi];
		}
		printf("%lld\n",(risan.se - risan.fi) % MOD);
	}
	return 0;
}
