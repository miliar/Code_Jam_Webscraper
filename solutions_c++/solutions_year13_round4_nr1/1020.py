...
int main()
{
    ...
    while(test--)
    {
        ...
        for(i=0;i<M;i++)
        {
            ...
            U_MAP[u] +=c;
            up.pb(u);
            actual_cost += calc(u,v)*c;
        }
        Sort(up); Unique(up);
        Sort(down); Unique(down);
        ...
        for(; ind1<SZ(up) && ind2<SZ(down);)
        {
            while(ind1<SZ(up) && up[ind1]<=down[ind2])
            {
                stk.push(MP(up[ind1],U_MAP[up[ind1]]));
                ind1++;
            }

            ll req = D_MAP[down[ind2]];
            v = down[ind2++];
            while(req)
            {
                pii tmp = stk.top();stk.pop();
                u = tmp.fs; ll val = tmp.sc;
                ll mn = min(req,val);
                total_cost += calc(u,v)*mn;
                req-=mn;
                val-=mn;
                if(val)
                {
