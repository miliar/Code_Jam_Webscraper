...
static const LL MOD = 1000002013LL;
...
{
    assert(n < N);
    return mod(N*n - n*(n - 1)/2);
}

void Case::solve(Result &result)
{
    map<LL, pair<LL, LL> > changes;  // station -> (enter, leave)
    LL total = 0;
    REP(m, M) {
        total = mod(mod(P[m]*cost(B[m] - A[m])) + total);
        changes[A[m]].first  += P[m];
        changes[B[m]].second += P[m];
    }

    vector<pair<LL,LL> > have;  // station, people
    FORIT(it, changes) {
        LL station = it->first;
        LL enter = it->second.first;
        LL leave = it->second.second;
        have.push_back(make_pair(station, enter));
        while (leave > 0) {
            assert(!have.empty());
            LL n = min(leave, have.back().second);
            total = mod(total - mod(mod(n)*cost(station - have.back().first)));
            have.back().second -= n;
            if (have.back().second == 0) have.pop_back();
            leave -= n;
        }
    }
