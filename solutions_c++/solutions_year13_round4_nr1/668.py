...
struct Event{
	long long pos;
	long long people;
	long long flag; // 1 in 0 out
	Event& operator = (const Event& x){
		pos = x.pos;
		people = x.people;
		flag = x.flag;
	}
	//event(long long  p,long long peo, long long f):pos(p),people(peo),flag(f){}
};
Event eve[maxn];
	...
	public:
		class Node{
			public:
				...
				long long  num;//the number of data
				long long  size;
				Node(){}
				Node(long long parData, long long parNum, Node* parParent = NULL)
					{
//						PR(132);
						data = parData;
						parent = parParent;
		...
		long long getSize(Node* now){
			if (now == NULL) return 0;
			else return now->size;
		}

		Node* findKth (long long rank){
			return findKth(rank, head);
		}
		Node* findKth (long long rank, Node* start){///如果rank大于head返回NULL
			if ((start == NULL) || (rank > start->size) || (rank <= 0)) return NULL;

			Node* now = start;
			while (true){
				long long leftSize = ((now->left == NULL)?0:now->left->size);
		...
		Node* findMinKth (long long rank){return findKth(rank, head);}
		Node* findMinKth (long long rank, Node* start){return findKth(rank, start);}
		Node* findMaxKth (long long rank){
			return findMaxKth(rank, head);
//another choice:		return findKth(head->size + 1 -rank, head);
		}
		Node* findMaxKth (long long rank, Node* start){///如果rank大于总大小返回NULL
			if ((start == NULL) || (rank > start->size) || (rank <= 0)) return NULL;
//another choice:		return findKth(start->size + 1 - rank, start);删除下面所有
			Node* now = start;
			while (true){
				long long rightSize = ((now->right == NULL)?0:now->right->size);
		...
		Node* find (long long parData){///if FAILED,return the last visited Node.null means no head
			Node* now = head;
			while (now != NULL){
				if (parData < now->data){
		...
		Node* findLeaf (long long parData){//if FAILED,return the last visited Node.null means no head
			Node* now = head;
			while (now != NULL){
				if (parData < now->data){
					if (now->left == NULL) break;
		...
		Node* insert(long long parData, long long parNum){
//			PR(parData);
//			PR(parNum);
//			PR(1232);
			if (head == NULL) {
...
//			PR(1232);
		...
		long long erase(long long parData, long long parNum){
...
int main(){
	...
	long long testsum;
	in >> testsum;
	for (long long iTest = 1; iTest <= testsum; iTest++){
		splay = new Splay;
		out << "Case #" << iTest << ": ";
		in >> n >> m ;
		ans1 = 0;
