...
#define out(x) {if(DEBUG) {cerr << #x" = " << x << endl;}}
#define debugv(x) {if(DEBUG) {cerr <<#x <<" = "; FOREACH(it, (x)) cerr <<*it <<", "; cerr <<endl; }}
...
void solve(bool skip)
{
  int N, M;
  cin >> N >> M;
  vector<int> in(M), Out(M), person(M);
  REP(i, M) {
    cin >> in[i] >> Out[i] >> person[i];
  }
  if(skip) return;
  
  ll ori = 0;
  REP(i, M) {
    ori += (cost(Out[i]-in[i], N) * person[i]) % MOD;
    ori = ori % MOD;
  }
  out(ori);
  
  vector<tuple<int,bool /* is out */,int > > Q;
  REP(i, M) {
    Q.push_back(make_tuple(in[i],false,person[i]));
    Q.push_back(make_tuple(Out[i],true,person[i]));
  }
  sort(Q.begin(),Q.end());
  ...
  for(auto t: Q) {
    ...
    if(is_in) {
      ...
      while(person>0) {
        while(rest.rbegin()->second==0) {
          auto it = rest.end();
          it--;
          rest.erase(it);
        }
        int p = min((ll)person, rest.rbegin()->second);
        ncost += (cost(station - rest.rbegin()->first, N) * p) % MOD;
        ncost = ncost % MOD;
        rest.rbegin()->second -= p;
        person -= p;
      }
    }
  }
  while(rest.size() && rest.rbegin()->second==0) {
    auto it = rest.end();
    it--;
    rest.erase(it);
  }
  ...
  /*
  while(i<M) {
    int b = min(inp[i].second, outp[j].second);
    ncost += (cost(outp[j].first-inp[i].first, N) * b) % MOD;
    ncost = ncost % MOD;
    inp[i].second -= b;
    outp[j].second -= b;
    if(inp[i].second==0) {
