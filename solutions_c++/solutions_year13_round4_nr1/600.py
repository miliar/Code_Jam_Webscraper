...
#define BIGINT_SIZE 20	// support STEP*SIZE integer
...
struct pi{
	...
	LINT pn;
	int ex;
	bool operator < (const pi &ano) const{
		if(s==ano.s){
			return ex < ano.ex;
		}
		return s < ano.s;
...
int main(){
	...
	for(int csn=1;csn<=cs;csn++){
		...
		L = 0;
		FOR(i, M){
			scanf("%I64d%I64d%I64d", &st[i], &ed[i], &p[i]);
		...
		FOR(i, L){
			if(pis[i].ex==0){
				...
				while(!stk.empty()){
					...
					if(now.pn>=need){
						now.pn -= need;
						LINT num = pis[i].s - now.s;
						bigint cost = need;
						cost *= (2*N-num+1)*num/2;
