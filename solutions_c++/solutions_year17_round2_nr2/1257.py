...
{
t n, d[6];
vector<pair<t,string> > dd;
cin>>n;
for (int i=0;i<6;i++)
{
    cin>>d[i];
//    d(d[i],ff(i));
    dd.push_back({d[i],ff(i)} );
}
std::sort(dd.begin(), dd.end(), [](const std::pair<t,string> &left, const std::pair<t,string> &right) {
    return left.first > right.first;
});

if (dd[0].first>dd[1].first+dd[2].first)
{
    cout<<"IMPOSSIBLE\n";
return;
}
...
list<string> s;
for (int i=0;i<dd[0].first;i++)
{
    s.push_back(dd[0].second);
}
//d(s.size());
...
auto p=s.begin();
bool jo=false;
while (dd[1].first+dd[2].first!=0)
{
    int index;
    if (dd[1].first>dd[2].first) index=1; else index=2;
    s.insert(p,dd[index].second);
    dd[index].first--;
    auto p2=p;
    ++p2;
    if (p2==s.end() ) jo=true;
