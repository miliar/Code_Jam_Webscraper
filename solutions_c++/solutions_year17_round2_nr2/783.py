...
//
    ...
    pair<ll,ll>a[10];
    ll   ts , n , r , o , y , g , b , v , kk = 1;
    cin>>ts; while(ts--){
        cin>>n>>r>>o>>y>>g>>b>>v;

        a[0].xx = r , a[1].xx = y , a[2].xx = b;
        a[0].yy = 0 , a[1].yy = 1 , a[2].yy = 2;
        ...
        if(r < g || y < v || b < o){
            ...
            if(sum - 2*b > 0) printf("IMPOSSIBLE");
            else while(b--) printf("BO");
            puts("");
        }
        else if(r==g && r){
            if(sum - 2*r > 0) printf("IMPOSSIBLE");
            else while(r--) printf("RG");
            puts("");
        }
        else if(y==v && v){
            if(sum - 2*y > 0) printf("IMPOSSIBLE");
            else while(y--) printf("YV");
            puts("");
        }
        else if( a[0].xx + a[1].xx < a[2].xx)
            cout<<"IMPOSSIBLE"<<endl;
        else {

            int ans[4030];
            memset(ans , -1 , sizeof ans);
            for(int i = 1 ; i<=a[2].xx ; i++){
                ans[i + (i-1)*3 ] = a[2].yy;

//                cout<<"#";
            }

            for(int i = 1 ; i<=min( a[1].xx + a[0].xx , a[2].xx) ; i++){
                if(i <= a[1].xx) ans[ 1 + i + (i-1)*3 ] = a[1].yy;
                else {
                    ans[ 1 + i + (i-1)*3 ] = a[0].yy;
                }
            }

            ll lft = a[0].xx + a[1].xx - a[2].xx;

            for(int i = 1 ; i<= lft ; i++){
                ans[2 + i + (i-1)*3 ] = a[0].yy;
            }
