...
string simple(int r, int yy, int b){
    ...
    v.push_back({r,"R"}); v.push_back({yy,"Y"}); v.push_back({b,"B"}); 
    sort(v.begin(),v.end()); 
    string x=v[0].second; string y=v[1].second; string z=v[2].second;
    int t1=v[0].first,t2=v[1].first,t3=v[2].first; 
    string s="";
    if (t1==0){
        if (t3-t2>1) return "IMPOSSIBLE";
        if (t3-t2==1) s+=z;
        for (int i=0;i<t2;i++) s+=y+z; 
        return s;
    }
...
string solve(int r, int o, int y, int g, int b, int v){
    ...
    if ((g!=0)&&(g+1>r)) return "IMPOSSIBLE";
    if ((v!=0)&&(v+1>y)) return "IMPOSSIBLE";
    if ((o!=0)&&(o+1>b)) return "IMPOSSIBLE";

    if (r==0){
        s=simple(r,y-(v+1),b-(o+1)); 
        if (s.size()){
            if (s[0]=='I') return s; 
            if (s[0]==s[s.size()-1]) return "IMPOSSIBLE"; 
        }
        s+='B';
        for (int i=0;i<o;i++) s+="OB"; 
        s+='Y';
        for (int i=0;i<v;i++) s+="VY";
        return s;
    }
    if (y==0){
...
bool check(string s, int n, int r, int o, int y, int g, int b, int v){
    ...
    for (int i=0;i<s.size();i++){
        char x=s[i],y=s[(i+1)%s.size()]; 
        if (x>y) swap(x,y); 
        if (x==y) return false;
        if (allowed.count(make_pair(x,y))==0) return false;
    }
    return true;
}
int main(){
    ...
    for (int ca=1;ca<=t;ca++){
        ...
        //int r=rand()%1000,o=rand()%1000,y=rand()%1000,g=rand()%(r+1),b=r+y,v=rand()%(y+1);
        //int n=r+o+y+g+b+v;
        string s=solve(r,o,y,g,b,v);
        cout<<"Case #"<<ca<<": "<<s<<endl; 
        if (check(s,n,r,o,y,g,b,v)==0) cout<<"AHH"<<" "<<r<<" "<<o<<" "<<y<<" "<<g<<" "<<b<<" "<<v<<endl;
    }
    return 0;
}
