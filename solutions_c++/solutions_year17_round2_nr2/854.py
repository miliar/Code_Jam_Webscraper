...
void init(){
	...
	for(int i = 0; i < 6; i++) fr[i][i] = false;
	for(int i = 0; i < 6; i++){
		int j = (i+1)%6;
		fr[i][j] = false;
		fr[j][i] = false;
...
void solve(){
	...
	if(cc[minT1] < cc[minT2]) swap(minT1, minT2);
	//RBY
	int cRBY = cc[0] + cc[2] + cc[4] - 2*maxcc;		
	for(int t = 0; t< cRBY; t++){
		data p = ans[maxT].back();ans[maxT].pop_back();
		cout<<p.s;
		p = ans[minT1].back();ans[minT1].pop_back();
		cout<<p.s;
		p = ans[minT2].back();ans[minT2].pop_back();
		cout<<p.s;
	}
