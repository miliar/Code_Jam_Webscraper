...
string getOrder(ll R,ll Y,ll B){
    string ordr="";
            ll col[3];
        col[0]=R;
        col[1]=Y;
        ...
        while(col[2]>col[1]){
            ...
            ordr+=seq[2];
            ordr+=seq[0];
            //cout<<seq[2]<<seq[1]<<seq[2]<<seq[0];
            col[0]--;
            col[1]--;
            col[2]-=2;
        }
        for(long i=0;i<col[0];i++){
            ordr+=seq[2];
            ordr+=seq[1];
            ordr+=seq[0];
//            cout<<seq[2]<<seq[1]<<seq[0];
        }
    return ordr;
}
int main(void){
...
//createDummyTestCases();
  ...
  //createDummyTestCase();
    ...
    for(long T=0;T<test_cases;T++){
        ...
        if(G>R||(G==R&&G+R<N&&G>0)||(O>B)||(O==B&&O+B<N&&O>0)||(V>Y)||(V==Y&&V+Y<N&&V>0)){
            cout<<"IMPOSSIBLE\n";
            continue;
        }
        R-=G;
        ...
        string ordr=getOrder(R,Y,B);
        string newOrder="";
        for(long i=0;i<ordr.size();i++){
            newOrder+=ordr[i];
            if(ordr[i]=='R'){
                while(G>0){
                    newOrder+="GR";
