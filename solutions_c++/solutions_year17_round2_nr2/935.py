...
for (int tc = 1; tc <= T; ++tc) {
	...
	if (rc) {
		...
		r -= rc;
		if (r == 0) {for (int i = 0; i < rring.size()-1; ++i) cout << rring[i]; cout << '\n';  continue; }
	}
	if (yc) {
		yring = genring(yc, "YV", "Y");
		y -= yc;
		if (y == 0) {for (int i = 0; i < yring.size()-1; ++i) cout << yring[i]; cout << '\n';  continue; }
	}
	if (bc) {
		bring = genring(bc, "BO", "B");
		b -= bc;
		if (b == 0) {for (int i = 0; i < bring.size()-1; ++i) cout << bring[i]; cout << '\n';  continue; }
	}
