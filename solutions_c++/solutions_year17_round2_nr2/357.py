...
public:
  ...
  void Run(uint32 T) {
    if (R > N / 2 || Y > N / 2 || B > N / 2) {
      print("Case #%0: IMPOSSIBLE", T + 1);
      return;
    }

    std::string result(N, '*');
    std::vector<std::pair<uint32, char>> colors = {
        std::make_pair(R, 'R'),
        std::make_pair(Y, 'Y'),
        std::make_pair(B, 'B'),
    };
    std::sort(colors.begin(), colors.end(), std::greater<std::pair<uint32, char>>());

    uint32 index = 0;
    for (auto color: colors) {
      for (auto i: range<uint32>(0, color.first)) {
        result[index] = color.second;
        index += 2;
        if (index >= N) {
          index = 1;
        }
  ...
  void LoadData() {
    read(std::cin, N, R, O, Y, G, B, V);
  }

private:
  uint32 N, R, O, Y, G, B, V;
};
