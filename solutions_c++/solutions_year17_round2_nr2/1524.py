...
void handle_case(int case_num) {
	...
	int all, red, orange, yellow, green, blue, violet;
	sscanf(
		line, " %d %d %d %d %d %d %d ",
		&all, &red, &orange, &yellow, &green, &blue, &violet);
	// Small: only red, yellow, blue.

	if (red > all / 2 || yellow > all / 2 || blue > all / 2) {
		printf("IMPOSSIBLE");
		return;
	}
