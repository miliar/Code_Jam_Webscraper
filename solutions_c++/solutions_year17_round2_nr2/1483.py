...
void solve() {
	int N; cin >> N;
	vector<pair<int, int> > C(6); REP(i, 6) C[i] = {in(), i+1};
	sort(ALL(C), greater<pii>());

	vector<int> field(C[0].first*4);
	REP(i, C[0].first) {
		field[i * 4] = C[0].second;
	}
	int pos = 2;
	for (; pos < field.size() and C[1].first > 0; pos += 4, C[1].first--) {
		field[pos] = C[1].second;
	}
	for (; pos < field.size() and C[2].first > 0; pos += 4, C[2].first--) {
		field[pos] = C[2].second;
	}
	if (pos < field.size()) {
		cout << "IMPOSSIBLE" << endl;
		return;
	}
	for (pos = 1; pos < field.size() and C[2].first > 0; pos += 2, C[2].first--) {
		field[pos] = C[2].second;
	}
	string Name = "ROYGBV";
	REP(i, field.size()) if(field[i] > 0){
		cout << Name[field[i] - 1];
	}
	cout << endl;
}