...
{
    set<pair<char, char>> s{{'R', 'G'}, {'V', 'Y'}, {'O', 'B'}, {'R', 'B'}, {'B', 'Y'}, {'Y', 'R'}};
    return s.count({c1, c2}) || s.count({c2, c1});
}

set<string> sp{"RYB", "RBY", "YBR", "YRB", "BYR", "BRY"};

int main(int, char *[])
{
    freopen("C:/Users/Infinity/Downloads/B-small-attempt4.in", "r", stdin);
    freopen("C:/Users/Infinity/Downloads/B-small-4.out", "w", stdout);
    for (int ti = 1, tt = getint(); ti <= tt; ++ti) {
        int n, r, o, y, g, b, v, vy = 0, ob = 0, rg = 0;
        read(n, r, o, y, g, b, v);
        try {
            while (v) {
                if (y) {
            ...
            if (t.size() != n || check(t.front(), t.back()) == false) {
                throw string("IMPOSSIBLE");
            }
            for (int i = 0; i < t.size() - 1; i++) {
                if (check(t[i], t[i + 1]) == false) {
                    throw string("IMPOSSIBLE");
                }
            }
            throw t;
