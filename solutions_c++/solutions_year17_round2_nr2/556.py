...
int n , R, O , Y , G , B , V;
int a[6] , c[6];
const string YK = "ROYGBV";
bool check(int x , int y){
    if (x > y) swap(x , y);
    if (x == 0) return y == 2 || y == 3 || y == 4;
    if (x == 1) return y == 4;
    if (x == 2) return y == 0 || y == 4 || y == 5;
    if (x == 3) return y == 0;
...
void solve(){
    RD(n , R , O , Y , G , B , V);
    printf("Case #%d: " , ++Case);
//    if (R > O + Y || O > R + Y || Y > R + O){
//        puts("IMPOSSIBLE");
//        return;
//    }
    a[0] = R , a[1] = O , a[2] = Y , a[3] = G , a[4] = B , a[5] = V;
    int st = 0;
    for (int i = 0 ; i < 6 ; ++i) {
        if (a[st] < a[i]) st = i;
        c[i] = a[i];
    ...
    for (int i = 0; i < n - 1 ; ++i){
        ...
        --c[now];
        ans += YK[now];
        for (int j = 0 ; j < 6 ; ++j) if (check(now , j) && c[j] > 0){
            if (nex == -1 || c[j] > c[nex] || (c[j] == c[nex] && j == st)) nex = j;
        }
//        cerr << ans << endl;
        if (nex == -1){
            puts("IMPOSSIBLE");
