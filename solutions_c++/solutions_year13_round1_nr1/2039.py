...
*/
void eval( unsigned long long r, unsigned long long t, ofstream& outfile)
{
   long long num = 0;
...
/*    
    ...
    */
    ...
    cerr << "A = " << A << "; B = " << B << "; temp = " << temp << endl;

    num = floor(0.25 * (sqrt((2*A-1)*(2*A-1)+8*B) - 2* A +1)); 
    //num = floor(0.25* B * (sqrt((2*temp-1/B)*(2*temp-1/B)+8/B) - 2* temp +1/B)); 
    outfile << num;
    return;
}
...
int main()
{
    ...
    for (int i = 0; i < numTests; i++)
    {
        ...
        //fscanf(fin, "%ll %ll", &temp1, &temp2);

        cerr << "temp1 = " << temp1 << " ;  temp2 = " << temp2 << endl;
        //A = (double) temp1;
        //B = (double) temp2;
        //cerr << "A = " << A << " ;  B = " << B << endl;
        eval(temp1, temp2, outfile);
        if((i+1) < numTests)
        {
            outfile << "\n";
        }
