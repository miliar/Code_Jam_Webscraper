...
#include <math.h>
using namespace std;
  BigInt caluclate(long long r,long long t){
         BigInt rr(r);//4*(r*r)-4*r+1+8*t )-2*r+1)/4
         BigInt tt(r);
         rr*=rr;
         rr*=4;
...
int main (){
    ifstream in ("bullseye.in");
    ofstream out ("bullseye.out");     
//long long a=pow(10,18)*2;
//cout<<a;
int quantity;
in>>quantity;
...
BigInt
BigInt::sqroot(const BigInt& bigNumber,const bool min) {
                   //  cout <<"\n"<<bigNumber<<"\n";
    BigInt low("1");
    BigInt max=bigNumber;
    BigInt subs=BigInt::mod(low-max);
    while (subs>1) {
        low = (low+max)/2;
        max=bigNumber;
        max = max/low;
        subs=BigInt::mod(low-max);
    }
    if (low*low!=bigNumber ){
       if (min==true && low*low<bigNumber){
       low++;               
       }else if(min==false && low*low>bigNumber){
       low--;      
       }                   
    }
    return low;
