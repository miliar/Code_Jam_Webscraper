...
bool check_ok(const vector<string>& a, int i, int j) {
    auto c = a[i][j];
    if (c == '.')
        return true;
    int dx, dy;
    ...
    /*cerr << dx << ' ' << dy << endl;
    cerr << i << ' ' << j << ' ' << a.size() << ' ' << a[0].size() << endl;*/
    for (int i0 = i + dx, j0 = j + dy; 0 <= i0 && i0 < a.size() && 0 <= j0 && j0 < a[0].size(); i0 += dx, j0 += dy) {
        //cerr << i0 << ' ' << j0 << ' ' << a[i0][j0] << endl;
        if (a[i0][j0] != '.') {
            return true;
        }
    }
...
bool check_change(const vector<string>& a, int i, int j) {
    for (int i0 = 0; i0 < a.size(); ++i0) {
        if (i0 != i && a[i0][j] != '.')
            return true;
    }
    for (int j0 = 0; j0 < a[0].size(); ++j0) {
        if (j0 != j && a[i][j0] != '.')
            return true;
    }
    return false;
