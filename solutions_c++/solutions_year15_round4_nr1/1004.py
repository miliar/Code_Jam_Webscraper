...
int CodeJamMain()
{
    ...
    for (int caseNum = 0; caseNum < T; caseNum++)
    {
        ...
        for (auto& row : board) getline(input, row);
        ...
        for (int r = 0; r < R; r++)
        {
            ...
            {
                bool badDir =
                    (board[r][c] == '^' && ((bad[r][c] & up) != 0)) ||
                    (board[r][c] == 'v' && ((bad[r][c] & down) != 0)) ||
                    (board[r][c] == '<' && ((bad[r][c] & left) != 0)) ||
                    (board[r][c] == '>' && ((bad[r][c] & right) != 0));
                if (badDir)
                {
                    bool goodDirExists = (bad[r][c] & (up | down | left | right)) != (up | down | left | right);
                    if (goodDirExists) minChanges++;
