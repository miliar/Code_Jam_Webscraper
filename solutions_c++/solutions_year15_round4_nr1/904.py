...
bool isSafe(long x,long y){
    ...
    while(x>=0&&y>=0&&x<R&&y<C){
        while(x>=0&&y>=0&&x<R&&y<C&&grid[x][y]=='.'){
            x+=dx;
            y+=dy;
        }
        if(!(x>=0&&y>=0&&x<R&&y<C))
...
int main(void){
	...
	for(long T=0;T<test_cases;T++){
        ...
        bool possible=true;
        long modification=0;
        for(long i=0;i<R&&possible;i++){
            for(long j=0;j<C&&possible;j++)
            {
                if(grid[i][j]!='.'&&grid[i][j]!='s')
                {
                    possible=!isFatal(i,j);
                    if(possible&&!isSafe(i,j))
                        modification++;
                }
            }
        }
