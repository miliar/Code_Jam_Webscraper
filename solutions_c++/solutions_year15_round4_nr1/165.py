...
void myCode() {
  ...
  fo (tt, ttt) {
    int m = getint(), n = getint();
      ...
      rowFirst[i] = colFirst[i] = N;
      rowLast[i] = colLast[i] = -1;
    }
    fi(m)
      fj(n)
        if(board[i][j] != '.') {
          rowFirst[i] = min(rowFirst[i], j);
          colFirst[j] = min(colFirst[j], i);
          rowLast[i] = max(rowLast[i], j);
          colLast[j] = max(colLast[j], i);
        }
    int ret = 0;
    fi(m)
      fj(n)
        if(board[i][j] != '.') {
          if ((rowFirst[i] == j && board[i][j] == '<') ||
              (colFirst[j] == i && board[i][j] == '^') ||
              (rowLast[i] == j && board[i][j] == '>') ||
              (colLast[j] == i && board[i][j] == 'v')) {
            if ((rowFirst[i] == j) &&
                (colFirst[j] == i) &&
                (rowLast[i] == j) &&
                (colLast[j] == i)) {
