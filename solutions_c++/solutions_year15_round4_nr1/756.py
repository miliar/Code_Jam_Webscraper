...
bool checkDir(int r, int c, int dR, int dC, const vector<string> &M)
{
	while (true)
	{
		r += dR;
		...
		if (r < 0 || r >= SIZE(M) || c < 0 || c >= SIZE(M[0]))
			return false;
...
bool checkCurDir(int r, int c, const vector<string> &M)
{
	if (M[r][c] == '.')
		return true;

	if (M[r][c] == '^')
		return checkDir(r, c, -1, 0, M);
	if (M[r][c] == 'v')
		return checkDir(r, c, 1, 0, M);
	if (M[r][c] == '<')
		return checkDir(r, c, 0, -1, M);
	//if (M[r][c] == '>')
		return checkDir(r, c, 0, 1, M);
}

bool checkAllDir(int r, int c, const vector<string> &M)
{
	return checkDir(r, c, -1, 0, M) ||
			checkDir(r, c, 1, 0, M) ||
			checkDir(r, c, 0, -1, M) ||
...
void Runner::Run()
{
	...
	FORD(r, 0, R)
	{
		FORD(c, 0, C)
		{
			if (!checkCurDir(r, c, M))
			{
				if (checkAllDir(r, c, M))
				{
					++res;
				}
				else
