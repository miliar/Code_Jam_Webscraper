...
int main()
{
  ...
  rep(test_case, T){
    ...
    cin >> n >> m;
    vector<string> mp(n);
    vector<vector<i64>> cnt(n, vector<i64>(m, 0));
    rep(i, n) cin >> mp[i];
    string outside = "<^>v";
    rep(dir, 4){
      rep(i, n)rep(j, m)if(mp[i][j] != '.'){
        cnt[i][j] += 1;
        if(outside[dir] == mp[i][j]) cnt[i][j] += 4;
        break;
      }
      vector<string> t0 = mp;
      vector<vector<i64>> t1 = cnt;
      mp = vector<string>(m, string(n, '?'));
      cnt = vector<vector<i64>>(m, vector<i64>(n, 0));
      rep(i, n)rep(j, m){
        mp [m - j - 1][i] = t0[i][j];
        cnt[m - j - 1][i] = t1[i][j];
      }
    ...
    i64 ans = 0;
    rep(i, n)rep(j, m){
      if(cnt[i][j] == 8) ans = -bit(50);
      if(4 < cnt[i][j]) ans += 1;
    }
    stringstream ss;
    if(ans < 0) ss << "IMPOSSIBLE";
    else ss << ans;
