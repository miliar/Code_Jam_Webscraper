...
RealCase::RealCase()
{
    ...
    REP(w, W) {
        string token;
        cin >> token;
        int i, j, x = sscanf(token.c_str(), "%d,%d", &i, &j);
        assert(x == 2);
        adj[i].push_back(j);
        adj[j].push_back(i);
    }
...
static vector<char> visited, threatened;
static int best_threatened;
static vector<int> path;

int RealCase::count_threatened()
{
    vector<int> temp;
    FORIT(it, path) {
        FORIT(jt, adj[*it]) if (!visited[*jt] && !threatened[*jt]) {
            threatened[*jt] = true;
            temp.push_back(*jt);
        }
    }
    FORIT(it, temp) threatened[*it] = false;
    return SZ(temp);
}

void RealCase::dfs(int i, int depth)
{
    path.push_back(i);
    visited[i] = true;
    if (depth > 0)
    {
        FORIT(it, adj[i]) if (!visited[*it]) {
            if (*it == 1) {
                best_threatened = max(best_threatened, count_threatened());
            } else {
                dfs(*it, depth - 1);
            }
        }
...
{
    visited = vector<char>(P);
    threatened = vector<char>(P);
    best_threatened = -1;
    path = vector<int>();

    FOR(d, 1, P) {
        dfs(0, d);
        if (best_threatened >= 0) {
            result.set(STR(d - 1) + " " + STR(best_threatened));
            return;
        }
    }
}
