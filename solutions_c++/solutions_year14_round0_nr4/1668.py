...
int playBasic(vector<double> &v,vector<double> &w){
	int n = v.size();
	int count = 0;
	int lastPointer = n-1;
	for(int i=n-1;i>=0;i--){
		if(v[i] > w[lastPointer]) count++;
		else lastPointer--;
	}
	return count;
}

int playDeceived(vector<double> &v,vector<double> &w){
	int n = v.size();
	int firstPointer = 0;
	int count = 0;

	for(int i=0;i<n;i++){
		if(v[i] > w[firstPointer])
			{
				count++;
				firstPointer++;
			}
...
*/
		int count_default = 0;
		int count_deceieved = 0;
		count_default = playBasic(v,w);
		count_deceieved = playDeceived(v,w);

		cout<<"Case #"<<tst+1<<": "<<count_deceieved<<" "<<count_default<<endl;
	}	
}
