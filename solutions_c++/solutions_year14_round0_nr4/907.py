...
void take(vector<double>& v, int idx)
{
	v.erase(v.begin() + idx);
}

int score_a(vector<double> A, vector<double> B)
{
	int s = 0;
...
int score_b(vector<double> A, vector<double> B)
{
	int s = 0;
...
void solve(istream& in, ostream& fout, int caseIdx)
{
	...
	OUTANS( << CaseHead(caseIdx+1) << score_a(A, B) << " " << score_b(A, B) << endl );
}
