...
void readList(int size, list<float>& v)
{
	float temp;
...
int dwar(list<float> naomi, list<float> ken)
{
	int nCount = 0;
...
int war(list<float> naomi, list<float> ken)
{
	...
	while (!naomi.empty())
	{
		list<float>::iterator kIt = ken.begin();
		...
		{
			while (naomi.front() < *kIt && kIt != ken.end())
			{
				++kIt;
			}
...
int main()
{
	...
	for (int i = 1; i <= T; ++i)
	{
		...
		int nCount = war(naomi, ken);
		int mCount = dwar(naomi, ken);

		cout << "Case #" << i << ": ";
		cout << mCount << " " << nCount << endl;
	}
