...
int main(int argc, char *argv[])
{
    ...
    for (int idx = 0; idx < totalCases; ++idx) {
        int num = 0;
        cin >> num;
        vector<float> naomi(num), ken(num);
        for (int i = 0; i < num; ++i) cin >> naomi[i];
        for (int i = 0; i < num; ++i) cin >> ken[i];
        sort(naomi.begin(), naomi.end());
        sort(ken.begin(), ken.end());
        vector<float> naomi2 = naomi;
        ...
        for (int i = 0; i < naomi2.size(); ++i) {
            if (naomi2[i] > ken2.back()) {
                ++winNorm;
                ken2.pop_back();
            } else {
                vector<float>::iterator it = upper_bound(ken2.begin(), ken2.end(), naomi2[i]); 
                if (it != ken2.end()) ken2.erase(it);
            }
        }

        // output results
        cout << "Case #" << idx + 1 << ": " << win << " " << winNorm << endl;
    }
    return 0;
}
