...
////				//if(i+dir[k] > r || j+dir[k+1] > c || j+dir[k+1] <=0 || i+dir[k] <= 0) continue;
...
////	if(r > c){
////		if(r*c - m <= r)
////			for(int i=1; i<=r; i++) for(int j=1; j<=c; j++) { if(stp == m) break; map[i][j] = 1; stp++; }
////			if(r - (m%r) < c - (m%c)) 
////				for(int i=1; i<=r; i++) for(int j=1; j<=c; j++) { if(stp == m) break; map[i][j] = 1; stp++; }
////			else for(int j=1; j<= c; j++) for(int i=r; i>0; i--) { if(stp == m) break; map[i][j] = 1; stp++; }
////	}
////	else {
////		if(r*c - m <= c)
////			for(int i=r; i>0; i--) for(int j=1; j<=c; j++) { if(stp == m) break; map[i][j] = 1; stp++; }
////			if(r - (m%r) < c - (m%c)) 
////				for(int i=1; i<=r; i++) for(int j=1; j<=c; j++) { if(stp == m) break; map[i][j] = 1; stp++; }
////			else for(int j=1; j<= c; j++) for(int i=r; i>0; i--) { if(stp == m) break; map[i][j] = 1; stp++; }
////	}
...
////			continue;
////		}
////		if(r*c - m == 2 && r > 1 && c > 1) { puts("Impossible"); continue; }
////		if(r*c - m == 3 && r > 1 && c > 1) { puts("Impossible"); continue; }
////		if(r*c - m == 4 && r >= 2 && c >= 2 ) {
////			printf("c."); for(int i=3; i<=c; i++) printf("*"); puts("");
////			printf(".."); for(int i=3; i<=c; i++) printf("*"); puts("");
////			for(int i=3; i<=r; i++) { for(int j = 1; j<= c; j++) printf("*"); puts(""); }
////			continue;
////		}
////		//if(r*c - m == 6 && ( r>=2 || c >= 2 )){
////		//	if(r > c){
...
////		//	}
////		//	else {
////		//		printf("c.."); for(int i = 4; i <= c; i++) printf("*"); puts("");
////		//			printf("...");for(int i = 4; i <= c; i++) printf("*"); puts(""); 
////		//		for(int i=3; i<=r; i++) {
////		//			for(int j = 1; j<=c; j++) printf("*");
////		//			puts("");
////		//		}
...
int main () {
	...
	for(int tt = 1; tt <= ti; tt ++){
		...
		sort(k, k+n);
		int ans = 0;
		int tfront = 0, trear = n-1, kfront = 0, krear = n-1;
		while(tfront <= trear){
			if(t[tfront] > k[kfront]){
				score++;
				tfront++;
				kfront++;
			}
			else if(t[tfront] < k[kfront]){
				//score--;
				tfront++;
				krear--;
			}
		...
		for(int i=0; i<n; i++) swap(t[i], k[i]);
		tfront = 0, trear = n-1, kfront = 0, krear = n-1;
		while(tfront <= trear){
			if(t[tfront] > k[kfront]){
				ans++;
				tfront++;
