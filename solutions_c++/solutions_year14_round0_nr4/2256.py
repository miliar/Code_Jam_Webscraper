...
struct Instance {
   ...
   void compute() { // should produce output and store it, not use IO; will run in parallel
      {
         set<double,greater<double> > A(ALL(a));
         set<double > B(ALL(b));
         //FOREACH(x,A) DBG(*x);
         //cerr << endl;
         //FOREACH(x,B) DBG(*x);
         res1 = 0;
         while (!A.empty() && *A.begin() > *B.begin()) { // moje max > jego min
            double jegoMin = *B.begin();
            B.erase(B.begin());
            set<double,greater<double> >::iterator it = A.lower_bound(jegoMin);
            --it;
            A.erase(it);
            ++res1;
         }
      ...
      {
         set<double> A(ALL(a));
         set<double,greater<double> > B(ALL(b));
         while (!A.empty() && *A.begin() < *B.begin()) {
            double jegoMax = *B.begin();
            B.erase(B.begin());
            set<double>::iterator it = A.lower_bound(jegoMax);
            --it;
            A.erase(it);
         }
         res2 = SZ(A);
