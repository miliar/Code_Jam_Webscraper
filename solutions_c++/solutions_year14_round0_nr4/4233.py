...
int OptimalWar(double naomi[], double ken[], int n)
{
	 int naomiCount = 0;
	 for (int i = 0; i < n; i++)
	 {
		 double naomiBlock = naomi[i];
		 int smallestKenIndex = - 1;
		 bool kenWins = false;
		 for (int j = 0; j < n; j++)
		 {
			 double kenBlock = ken[j];
			 if (kenBlock != -1)
			 {
				 if (smallestKenIndex == -1)
				 {
					 smallestKenIndex =  j;
				 }
...
int cheatingNaomi(double naomi[], double ken[], int n)
{
	int wins = 0;
	for (int i = 0; i < n; i ++)
	{
		double targetKen = ken[i];
		for (int j=0; j< n; j++)
		{
			if (naomi[j] > targetKen)
			{
				naomi[j] = -1;
				wins++;
				break;
...
void readBlocks(double * blocks, int n, ifstream &fin)
{
	for(int i = 0; i < n; i++)
	{
		fin>>blocks[i];
...
void problem4()
{
	...
	for(int i=0;i<numcases;i++)
	{
		...
		double naomi[1000];
		double ken[1000];
		double naomiCopy[1000];
		double kenCopy[1000];

		readBlocks(naomi, numBlocks, fin);
		readBlocks(ken, numBlocks, fin);

		qsort (naomi, numBlocks, sizeof(double), compare);
		qsort (ken,  numBlocks, sizeof(double), compare);

		memcpy(naomiCopy, naomi, numBlocks * sizeof(double));
		memcpy(kenCopy, ken, numBlocks * sizeof(double));
		
		int cheating = cheatingNaomi(naomi, ken, numBlocks);
		fout<<"Case #"<<i + 1<<": "<<cheating<<" "<<OptimalWar(naomiCopy, kenCopy, numBlocks)<<endl;
		cout<<"Done Case #"<<i + 1<<endl;
	}
