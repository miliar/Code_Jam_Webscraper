...
int main(){
	...
	for(coso=0;coso<caso;coso++){
		int blocchi,i,j,t1,t2,punti=0,puntifacendolaputtanaschifosa=0;
		in>>blocchi;
		...
		for(i=0;i<blocchi;i++){//punti da brava bambina
			check = true;
			for(j=0;j<blocchi;j++){
		...
		for(i=blocchi-1,t2=0,t1=blocchi-1; i>=0 ; i--){  //punti da big bagascia
				if(ken[i]<nao[t1]){
					puntifacendolaputtanaschifosa++;
					t1--;
					}
				else t2++;
			}
			...
			out<<"case #"<<coso+1<<": "<<puntifacendolaputtanaschifosa<<" "<<punti<<endl;
		}
	}
