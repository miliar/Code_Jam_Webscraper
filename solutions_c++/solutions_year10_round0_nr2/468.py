	...
	if (oper == m.oper) {
		...
		if (abs(*this) >= abs(m)) {
			...
			for (int i = 1, temp = 0; i <= length; i++)
				c.a[i] = ((temp = (-int(temp < 0) + a[i] - m.a[i])) + 10000)
						% 10000;
			c.check();
			return c;
		}
...
bigint sqrt(bigint m) {
	...
	c.a[c.length] = int(sqrt((double) m.a[c.length * 2] * 10000 + m.a[c.length
			* 2 - 1]) + 1e-6);
...
bigint gcdT(bigint * t, int i) {
	if (i == 0)
		return t[0];
	else
		return gcd(t[i], gcdT(t, i - 1));
}
...
int main(int argc, char **argv) {
	...
	//	sort(t, t + N);
		...
		printf("Case #%d: ",caseN);
		if (temp % result != 0)
			writeln((temp / result + 1) * result - temp);
		else
			writeln(0);
		caseN++;
