...
int small(const void *a, const void *b) {
...
void solve1(int c) {
	qsort(nums, n, sizeof(bigint), small);
	bigint g = nums[1] - nums[0];
	for(int i = 2; i < n; ++i) {
		g = gcd(nums[i] - nums[i-1], g);
	}
	bigint m = nums[0] % g;
	bigint res = (g - m) % g;
	printf("Case #%d: ", c); writeln(res);
}
...
void solve2(int c) {
	sort(inums, inums+n);
	int g = inums[1] - inums[0];
	for(int i = 2; i < n; ++i) {
		g = gcd(inums[i] - inums[i-1], g);
	}
	int m = inums[0] % g;
	int res = (g - m) % g;
	printf("Case #%d: %d\n", c, res);
