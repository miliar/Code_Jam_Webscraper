...
void processInput(const char* filePath)
{
    ...
    for (int i = 0; i < T; ++i)
    {
        ...
        string mcd;
        if (ge(times[0].c_str(), times[1].c_str()))
        {
            mcd = sub(times[0].c_str(), times[1].c_str()).c_str();
        }
        else
        {
            mcd = sub(times[1].c_str(), times[0].c_str()).c_str();
        }
        for (int j = 0; j < N; ++j)
            for (int k = j+1; k < N; ++k)
            {
                if (ge(times[j].c_str(), times[k].c_str()))
                {
                    string cha = sub(times[j].c_str(), times[k].c_str()).c_str();
                    if (cha != "0")
                        mcd = calcMaxCommonDivisor(mcd.c_str(), cha.c_str());
                }
                else
                {
                    string cha = sub(times[k].c_str(), times[j].c_str()).c_str();
                    if (cha != "0")
                        mcd = calcMaxCommonDivisor(mcd.c_str(), cha.c_str());
                }
            }
