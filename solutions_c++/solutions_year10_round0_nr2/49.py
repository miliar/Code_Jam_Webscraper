...
struct xnum
{
	...
	bool operator <(const xnum& V){return compare(*this,V)==-1;}
	bool operator >(const xnum& V){return compare(*this,V)==1;}
	bool operator ==(const xnum& V){return compare(*this,V)==0;}
};
...
xnum operator*(const xnum& A, const int B)
{
	...
	{
		if (I < A.Len) Carry += __int64(A[I]) * B;
		R[I] = Carry % Base;
		Carry /= Base;
	}
	R.Len = I;
...
xnum operator*(const xnum& A, const xnum& B)
{
	...
	for (I = 0; I < A.Len; I++)
	{
		...
		{
			if (J < B.Len) Carry += __int64(A[I]) * B[J];
...
int main()
{
	...
	for(int ii=1;ii<=cs;ii++)
	{
		...
		if(num[n-1]%cur==xnum(0))
			cur=0;
		else 
			cur=cur-(num[n-1]%cur);
		cout<<"Case #"<<ii<<": "<<cur<<endl;
	}
}