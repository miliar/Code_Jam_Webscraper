// ARYTMETYKA DUZYCH LICZB
// Autorzy: Arkadiusz Pawlik, Bartosz Walczak

// biguint - klasa duzych liczby nieujemnych
// Definiuje (prawie) wszystkie uzyteczne operatory.
// Odejmowac wolno tylko wtedy, gdy wynik jest liczba nieujemna!
// Konstruktory explicit maja zapobiec przypadkowym konwersjom.

// Dodatkowo zdefiniowane stale: zero, one, two
...
public:
    ...
    biguint& operator/=(const biguint& y) {
        arith::assert_msg(!y.buf.empty(), "Dzielenie przez zero");
        std::vector<uint> tmp;
    ...
    friend biguint operator-(const biguint& x, const biguint& y) {
        arith::assert_msg(x>=y, "Odejmowanie wiekszej liczby");
        biguint z;
    ...
    biguint& operator>>=(int i) {
        arith::assert_msg(i>=0, "Przesuniecie bitowe o ujemna wartosc");
...
//mamy a = b % x, szukam x, wiem, ¿e x | (a-b)
#define MR 1010

int n;
biguint t[MR], r, diff[MR*MR];
...
{	
	...
	for(int c = 0; c < tests; c++)
	{
		...
		for(int i = 1; i < l; i++)
			r = nwd(r, diff[i]);
		biguint rest = t[0] % r;	//mamy r, teraz wylicz ile brakuje kazdej liczbie zeby bylo 0 % r
		if(rest > 0)
			cout << "Case #" << c+1 << ": " << r - rest << "\n"; 
		else
			cout << "Case #" << c+1 << ": 0\n";
	}//petla po testach
	return 0;
