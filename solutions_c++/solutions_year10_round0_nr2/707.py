...
int main()
{
	freopen("fair_large.in","r",stdin);
	freopen("fair_large.out","w",stdout);
	int t,cs,n,i;
	char s[100];
	scanf("%d",&t);
	for(cs=0;cs<t;cs++)
	{
		...
		sort(now,now+n,comp);
		for(i=0;i<n;i++)
			sprintf(s,"%s",now[i].c_str()),num[i]=s;
		BigNum g=0;
		for(i=0;i<n-1;i++)
			dif=num[i]-num[i+1],g=g.gcd(dif);
		if(num[0]%g!=0)
			printf("Case #%d: ",cs+1),(g-(num[0]%g)).print(),printf("\n");
		else
			printf("Case #%d: 0\n",cs+1);
	}
	return 0;
