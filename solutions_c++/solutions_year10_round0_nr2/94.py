...
void gcd(BigInt& a, BigInt& b, BigInt& c)
{
	BigInt cero(0);
	if(a < cero) a = cero-a; if(b < cero) b = cero-b;
	BigInt r;
	while(cero < a)
	{
		r = b%a;
...
int main()
{
	...
	forn(tt,TT)
	{
		...
		forn(i,n)
		{
			...
			cin >> s;
			ns[i] = BigInt(s);
			if(i){ ns[i-1] -= ns[i]; if(ns[i-1]<cero) ns[i-1]=cero-ns[i-1]; }
			//if(i) cout << ns[i-1] << endl;
		}
		
		BigInt g(0);
		forn(i,n-1) gcd(ns[i], g, g);
		//cout << "gcd " << g << endl;
		
		BigInt k, t(1);
		ns[n-1] -= t;
		//cout << "ns[n-1] " << ns[n-1] << endl;
		k = ns[n-1]/g;
		//cout << "k-1 " << k << endl;
		k += t;
		//cout << "k " << k << endl;
