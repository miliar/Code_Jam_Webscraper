...
int isZero(const xnum& A)
{
	return compare(A,xnum(0))==0;
}
...
int main()
{
	...
	freopen("F://result.out","w",stdout);
	int cn;
	int cnt;
	xnum gcdnum;
	xnum ans;
	cin>>cn;
	for(int cnn=1;cnn<=cn;cnn++){
		cin>>n;
		for(int i=0;i<n;i++) cin>>num[i];

		for(int i=0;i<n-1;i++){
			if(compare(num[i],num[i+1])==1)
				dif[i] = num[i]-num[i+1];
			else
				dif[i] = num[i+1]-num[i];
		}
		...
		gcdnum = dif[0];
		for(int i=1;i<n-1;i++){
			if(compare(gcdnum,dif[i])==-1)
				gcdnum = gcd(gcdnum,dif[i]);
			else
				gcdnum = gcd(dif[i],gcdnum);
		}

		
		if(isZero(num[0]%gcdnum)==1) ans = xnum(0);
		else ans = gcdnum-num[0]%gcdnum;

		cout<<"Case #"<<cnn<<": "<<ans<<endl;
