...
Natural NWD(Natural a,Natural b)
{
  ...
  while (a!=b)
  {
    if (a.digs[0]%2==0 && b.digs[0]%2==0)
    {
      a/=2; b/=2; d*=2;
    } else if (a.digs[0]%2==0 && b.digs[0]%2==1)
    {
      a/=2;
    } else if (a.digs[0]%2==1 && b.digs[0]%2==0)
    {
      b/=2;
...
int main()
{
  ...
  FOR(iii,1,ILE)
  {
    ...
    Natural d=(tab[1]-tab[0]);
    FOR(i,2,SIZE(tab)-1) d=NWD(d,tab[i]-tab[i-1]);
    REP(i,SIZE(tab)) tab[i]%=d;
    FOR(i,1,SIZE(tab)-1) if (tab[i]!=tab[0]) { fprintf(stderr,"ZLE!!!\n"); exit(1); }
    if (tab[0]==Natural(0)) tab[0]=d;
    (d-tab[0]).write(); puts("");
  }
  return 0;
}
