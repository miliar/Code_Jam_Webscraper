...
// The following class in written by Igor Naverniouk
class BigInt
{
    private:
        char *digits;
...
bool cmpn(const string &a, const string &b)
{
...
int main()
{
	...
	//freopen("f:/data/B-large.in.txt","r",stdin);
	//freopen("f:/data/br0.txt","r",stdin);
	//freopen("f:/data/bout.txt","w",stdout);
	...
	xrep(ca,1,t)
	{
		...
		set<string> tmp = set<string>(all(nums)); 
		nums = vector<string>(all(tmp));
		
		sort(all(nums),cmpn);
		
		
		
		BigInt b = T[0], cur, fir = T[0], y;
