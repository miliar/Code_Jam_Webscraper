...
string MINUS_INT1(string x, string y)
{
    string y1 = "";
    string ans = "";
    int l = x.length() - y.length();
    for(int i = 1; i <= l; i++) y1 += '0';
    y = y1 + y;
    l = x.length();
    for(int i = l-1; i >= 0; i--)
    {
       if(x[i] >= y[i]) y[i] = '0'+(x[i]-y[i]);
       else
       {
          y[i] = char(48 + (x[i] + 10 -y[i]));
          int tmp = i-1;
          while(x[tmp] == '0') {  x[tmp] = '9'; tmp --; if(tmp < 0) break; }
          if(tmp >= 0) x[tmp] = x[tmp] - 1;
       }
    }
    int i;
    for(i = 0; i <= l-1; i++)
...
freopen("E:/out.txt","w",stdout);
   ...
   {
      int n;
      test++; //cout<<MINUS_INT1("1000000000000000000","1")<<endl;
      scanf("%d",&n);
      for(int i = 1; i <= n; i++)
      {
      ...
      string T;
      if(compare(a[n],a[1]) >= 0) T = MINUS_INT1(a[n],a[1]);
      else T = MINUS_INT1(a[1],a[n]); 
      for(int i = 1; i < n; i++)
      {  
         for(int j = i+1; j <= n; j++)
         {  //cout<<i<<endl;
            if(compare(a[i],a[j]) < 0) { T = gcd(T,MINUS_INT1(a[j],a[i])); }
            else { T = gcd(T,MINUS_INT1(a[i],a[j])); }
         }
      }
      string r = DIVIDE_INT(a[1],T,0);
      //cout<<T<<' '<<r<<endl;
      string ans;
      if(T == "1" || r == "0") ans = "0";
      else ans = MINUS_INT1(T,r);
