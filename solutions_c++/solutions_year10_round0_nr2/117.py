...
Natural silnia(int n){
	if(!n) return Natural(1);
	else return silnia(n-1)*Natural(n);
}

Natural NWD(const Natural& a, const Natural& b){
    if(b == 0) return a;
    if(a == 0) return b;
    if(a%2 == 0 && b%2 == 0) return NWD(a/2,b/2)*2;
    if(a%2 == 0) return NWD(a/2, b);
    if(b%2 == 0) return NWD(b/2, a);
    return NWD(max(a,b)-min(a,b), min(a,b));
}

#define FORE(i,x) FOREACH(i,x)
...
int main(){
    ...
    FOR(cas,1,C){
        D=0;
        cin >> N;
        scanf("%s",c); X = Natural(c);
        REP(i,N-1){
            scanf("%s",c); Y = Natural(c);
            D = NWD(D, max(X,Y)-min(X,Y));
        }
        cout << "Case #" << cas << ": ";
        ((D-(X%D))%D).write();
        printf("\n");
