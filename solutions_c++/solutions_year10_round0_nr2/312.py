...
BigInt lowestCommonMultiple(BigInt &a, BigInt&b){
	return (a*b)/greatestCommonDivisor(a,b);
}
string ifilename = "J:\\B-large.in";
string ofilename = "J:\\B-large.out.txt";
int main(){
	int N;
	ifstream infile;
	ofstream outfile;
	...
	for(int i = 0;i<N;i++){
		...
		for(int j=0;j<vb.size()-1;j++){
			BigInt bi = vb.back()-vb[j];
			vdifs.push_back(bi);
			//cout<< bi.toString() << endl;
		}
		BigInt gcd = vdifs.front();
		for(int j=1;j<vdifs.size();j++){
			gcd = greatestCommonDivisor(gcd,vdifs[j]);
		}
		//cout << "gcd = " << gcd.toString() << endl;
		//cout << "vb.back = " << vb.back().toString()<<endl;
		BigInt ret;
		if (vb.back()%gcd == zero) ret = zero;
		else ret = gcd -(vb.back()%gcd);

		outfile << "Case #" << i+1 << ": " << ret.toString() << endl;
