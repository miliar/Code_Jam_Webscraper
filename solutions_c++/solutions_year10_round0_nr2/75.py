...
int main(){
	...
	while (T--){
		...
		backup = V[0];
		vector<bignum> D;
		for (int i = 0 ; i < N; ++i) for (int j = i+1; j < N; ++j) D.push_back(abs(V[i]-V[j]));
		res = D[0];
		for (int i = 1; i < D.size(); ++i) res = gcd(res, D[i]);
		if (res == 1 || backup%res == 0) printf("0\n");
		else cout << (res - backup%res) << endl;
	}
	return 0;
}
