...
const char* toname(bool impossible)
{
	if (impossible)
		return "RICHARD";
	else
...
// if can impossible then return true
bool solvemain(int x, int r, int c)
{
	if (x == 1)
		return false;
	if (x > c)
...
void solve(istream& in, ostream& fout, int caseIdx)
{
	...
	bool impossible = solvemain(x, r, c);

	OUTANS( << CaseHead(caseIdx+1) << toname(impossible) << endl );
}
