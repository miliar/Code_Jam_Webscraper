...
string ans[] = {"GABRIEL", "RICHARD"};

bool go(int x, int r, int c) {
    if(r < c) swap(r, c);
    int tot = r * c, rem = x - c;
    if( x >= 7 || tot % x != 0 ) return 1;
    if(x <= 2) return 0;
    for(int i = x, j = 1; i >= j; i--, j++) {
        if(i > r || j > c) return 1;
    }
    if(rem) {
        rep(lft, 1, rem - 1) {
            int rgt = rem - lft, a = 0, b = tot - c;
            bool ok = true;
            while(b > 0) {
                if(lft <= a && rgt <= b  && (a - lft) % x == 0 && (b - rgt) % x == 0) {
                    ok = false;
                    break;
                }
                a += c;
    ...
    if( (c == 2 && x >= 4) || (x == 6 && 3 <= x && x <= 4) ) return 1;
    return 0;
}
