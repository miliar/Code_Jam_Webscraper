...
int main(void){
    ...
    for(long T=0;T<test_cases;T++){
        ...
        sort(parties,parties+n);
        cout<<"Case #"<<T+1<<": ";
        while(parties[n-1].first>parties[n-2].first){
            char ch='A'+parties[n-1].second;
            if(parties[n-1].first>parties[n-2].first+1){
                printf("%c%c ",ch,ch);
                parties[n-1].first--;
            }
            else
        ...
        for(long i=0;i+2<n;i++){
            ...
            for(long j=0;j<parties[i].first;j++){
                printf("%c ",ch);
                if((parties[i].first-j)%2==1)
                    cout<<" ";
            }
        }
        long ch1=parties[n-1].second+'A',ch2=parties[n-2].second+'A';
        for(long i=0;i<parties[n-1].first;i++)
            printf("%c%c ",ch1,ch2);
      //cout<<ans<<endl;
        cout<<endl;
