...
class debugger {
    ...
    public:
        ...
        void printall() { int j = 0, l = vars.size(); for( int i = 0; i < ( l - 1 ); i++ ) { if( i )cout << ", "; for( ; j < vars[l - 1].size(); j++ ) { if( vars[l - 1][j] == ',' ) {j++; break;} cout << vars[l - 1][j]; } cout << " = " << vars[i]; } vars.clear(); }
} Dbug;
...
template<class T1, class T2> inline T1 UMAX( T1 & m, T2 other ) { if( m < other ) m = other; return m; }
template<class T1, class T2> inline T1 UMIN( T1 & m, T2 other ) { if( m > other ) m = other; return m; }
    ...
    WRITE("Alargeout.txt");
        ...
        readcase{
                ...
                while( S > 0 ) {
                        ...
                        pQ.pop();
                        pii secBest;
                        if( !pQ.empty() ) secBest = pQ.top();
                        if( best.X >= 2 && S - 2 >= 2 * pQ.top() ) {
                                cout << ( 'A' + best.Y ) << ( 'A' + best.Y );
                                if( best.X - 2 > 0 ) {
                                        pQ.push( mp( best.X - 2, best.Y ) );
                                }
                                S -= 2;
                        } else if( best.X >= 1 && S - 1 >= 2 * pQ.top() ) {
                                cout << ( 'A' + best.Y );
                                if( best.Y - 1 > 0 ) {
                                        pQ.push( mp( best.X - 1, best.Y ) );
                                }
                                S--;
                        } else if( best.X >= 1 && secBest.X >= 1 ) {
                                pQ.pop();
                                if( pQ.empty() == true || pQ.top() )
                                }
