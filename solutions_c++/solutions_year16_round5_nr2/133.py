...
db gao(vi q,int x){
	for(int v:adj[q[x]])q.push_back(v);
	q[x]=q.back();
	q.pop_back();
	int S=0;
	rep(i,sz(q)) S+=sz[q[i]];
	db ans=1;
	rep(i,sz(q)) ans*=C[S][sz[q[i]]]*dp[q[i]],S-=sz[q[i]];
	rtn ans;
}
...
void solve(){
	...
	for (int it=0;it<ITER;it++){
		...
		vi q=adj[0];
		rep(i,n){
			vec<db> pr(sz(q));db sm=0;
			rep(i,sz(q)) pr[i]=gao(q,i),sm+=pr[i];
			rep(i,sz(q)) pr[i]/=sm;
			sm=dis(gen);
//			if (i==0){
//				prt(q);
...
//				prt(sm);
				if (sm<=0){
					s.push_back(S[q[i]-1]);
//					prt(q[i]);
					for(int v:adj[q[i]])q.push_back(v);
					q[i]=q.back();
					q.pop_back();
					break;
				}
