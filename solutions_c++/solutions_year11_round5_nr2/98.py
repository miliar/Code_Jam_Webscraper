...
void solve()
{
  ...
  int min_len = INF;
  while(1) {
    while(count[tip]==lastH && tip <= 10005)
      tip++;
    if(tip > 10005)
      break;
    if(count[tip] > lastH) {
      REP(i, count[tip] - lastH) {
        heads.push_back(tip);
      }
    } else {
      REP(i, lastH - count[tip]) {
        tails.push_back(tip);
      }
    }
    lastH = count[tip];
    if(lastH==0) {
      if(heads.size() != tails.size()) cout << "ERROR1" << endl;
      REP(i, heads.size()) {
        int len = tails[i] - heads[i];
        if(len < min_len)
          min_len = len;
      }
    }
  }
  if(lastH != 0) cout << "ERROR2" << endl;
