...
static double dist2(const asteroid &a, const asteroid &b)
{
    ll ans = 0;
    for (int i = 0; i < 3; i++)
    {
        ll x = a.pos[i] - b.pos[i];
        ans += x * x;
    }
    return ans;
}
...
static void solve_case(int cas, ostream &cout, unique_ptr<promise<void> > &&input_done)
{
    ...
    vector<tuple<double, int, int>> jumps;
    for (int i = 0; i < N; i++)
        for (int j = i + 1; j < N; j++)
        {
            jumps.emplace_back(dist2(ast[i], ast[j]), i, j);
        }
    sort(RA(jumps));
    double ans = HUGE_VAL;
    UnionFind uf(N);
    ...
    {
        uf.merge(get<1>(jump), get<2>(jump));
