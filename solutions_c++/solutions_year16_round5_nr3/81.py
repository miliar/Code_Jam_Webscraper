...
void solve(){
  ...
  FOR(i, n) FORU(j, i+1, n-1) {
    d[i][j] = d[j][i] = A[i].dist(A[j]);
  }
  union_find uf(n);
  viii E; FOR(i,n) FORU(j,i+1, n-1) E.pb(mt(d[i][j], i, j));
  sort(all(E));
  for(auto p : E) {
    DESTRUCT3(p, w, i, j);
    uf.unite(i, j);
    if(uf.find(0)==uf.find(1)){
      cout << setprecision(15) << sqrt(w) << endl;
      return;
    }
  }
}
