...
R dbl(const R &x){ return x * x; }
R dPP2(const P &a, const P &b){
    return dbl(a.x-b.x) + dbl(a.y-b.y) + dbl(a.z-b.z);
}
R dPP(const P &a, const P &b){
    return sqrt(dPP2(a, b));
}
...
void solve(GCJ::Runner &gcj){
    ...
    rep(i, n){
        ...
        cin >> vs[i].x >> vs[i].y >> vs[i].z;
    }
    gcj.end_input();
    vector<tuple<R, int, int>> es;
    rep(i, n) rep(j, i) es.emplace_back(dPP(ps[i], ps[j]), i, j);
    sort(all(es));
    UnionFind uf(n);
    for(const auto &e : es){
        R d; int i, j; tie(d, i, j) = e;
        uf.unite(i, j);
        if(uf.same(0, 1)){
            gcj << d << endl;
            return;
