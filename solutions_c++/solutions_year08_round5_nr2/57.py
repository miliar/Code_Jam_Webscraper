...
{
    unsigned char r, c;
    unsigned char pr, pc, pd;
    unsigned char qr, qc, qd;
};

bool operator< (const State& s, const State &t)
{
    return memcmp(&s, &t, sizeof(State)) < 0;
}

priority_queue<pair<int, State>, vector<pair<int, State> >, greater<pair<int,State> > > Q;

unsigned char D[17][17] [17][17][4] [17][17][4];

inline unsigned char &dist(const State &s)
{
    return D[s.r][s.c][s.pr][s.pc][s.pd][s.qr][s.qc][s.qd];
}

inline void add(int cost, State s)
{
    if (s.pr > s.qr || (s.pr == s.qr && (s.pc > s.qc || (s.pc == s.qc && s.pd > s.qd))))
    {
        swap(s.pr, s.qr);
        swap(s.pc, s.qc);
        swap(s.pd, s.qd);
...
void RealCase::solve(Result &result)
{
    ...
    int Sr = 0, Sc = 0;
    REP(r, 17) REP(c, 17) M[r][c] = 1;
    REP(r, H) REP(c, W) {
        switch (maze[r][c])
        {
        case '.': M[     1 + r][     1 + c] = 0; break;
        case 'O': M[Sr = 1 + r][Sc = 1 + c] = 0; break;
        case 'X': M[Tr = 1 + r][Tc = 1 + c] = 0; break;
        case '#': break;
        default: assert(0);
        }
    }
    ...
    int P[17][17][4][2];
    REP(r, 17) REP(c, 17) if (!M[r][c]) REP(d, 4) {
        int nr = r;
        int nc = c;
        do { nr += DR[d]; nc += DC[d]; } while (!M[nr][nc]);
        P[r][c][d][0] = nr;
        P[r][c][d][1] = nc;
    }

    State init = { Sr, Sc, 0, 0, 0, 0, 0, 0 };
    assert(dist(init) == 255);
    add(0, init);
    ...
    while (!Q.empty())
    {
        ...
        printf("%d: %d,%d  %d,%d-%d  %d,%d-%d\n",
               cost, s.r, s.c, s.pr, s.pc, s.pd, s.qr, s.qc, s.qd);
        assert(!M[s.r][s.c]);
        */
        ...
        REP(d, 4) {
            ...
            if (!M[nr][nc])
            {
                add(cost + 1, (State) { nr, nc, s.pr, s.pc, s.pd, s.qr, s.qc, s.qd });
            }
            else
            if (nr == s.pr && nc == s.pc && d == s.pd && (s.qr != 0 || s.qc != 0))
            {
                int nr = s.qr + DR[(s.qd + 2)%4];
                int nc = s.qc + DC[(s.qd + 2)%4];
                //assert(!M[nr][nc]);
                add( cost + 1, (State) { nr, nc, s.pr, s.pc, s.pd, s.qr, s.qc, s.qd });
            }
            else
            if (nr == s.qr && nc == s.qc && d == s.qd && (s.pr != 0 || s.pc != 0))
            {
                int nr = s.pr + DR[(s.pd + 2)%4];
                int nc = s.pc + DC[(s.pd + 2)%4];
                //assert(!M[nr][nc]);
                add( cost + 1,  (State) { nr, nc, s.pr, s.pc, s.pd, s.qr, s.qc, s.qd });
            }
        }
        ...
        /* Shooting */
        REP(d, 4) {
            int nr = P[s.r][s.c][d][0];
            int nc = P[s.r][s.c][d][1];
            add(cost, (State){ s.r, s.c, nr, nc, d, s.qr, s.qc, s.qd });
            add(cost, (State){ s.r, s.c, s.pr, s.pc, s.pd, nr, nc, d });
        }
    }
    result.set("THE CAKE IS A LIE");
}
