...
//template
int vis[1500],f[1500][1500][2][2],ans,n,m,T;    //总时间 A的时间 当前人 第一个人 
void read(){
	Read(n),Read(m);
	int i,j,a,b;
	...
	for(i=1;i<1440;i++){
		for(j=0;j<=i;j++){
			for(k=0;k<2;k++)
				for(l=0;l<2;l++){
					if(vis[i]!=1)
						f[i][j+1][0][l]=min(f[i][j+1][0][l],f[i-1][j][k][l]+k);
					if(vis[i]!=2)
						f[i][j][1][l]=min(f[i][j][1][l],f[i-1][j][k][l]+(!k));
				}
		}
	}
	for(k=0;k<2;k++)
		for(l=0;l<2;l++)
			ans=min(ans,f[1439][720][k][l]+(k!=l));
}
int main()
{
	Read(T);
