...
typedef ll otype;
otype calcFunction(ll c, ll j, v(p(ll,ll)) ct, v(p(ll,ll)) jt) {
	otype result=0;
	ll C=0;
	ll J=0;
	sort(all(ct));
	sort(all(jt));
	v(p(p(ll,ll),bool)) allt;
	forn(i,c){
		allt.pb(mp(ct[i],true));
	}
	forn(i,j){
		allt.pb(mp(jt[i],false));
	}
	sort(all(allt));
	v(ll) bc;
	v(ll) bj;
	v(ll) bcj;
	ll val = 24*60+allt[0].x.x - allt[c+j-1].x.y;
	if(allt[0].y == allt[c+j-1].y){
		if(allt[0].y){
			bc.pb(val);
		} else
			bj.pb(val);
	...
	forn(i,c+j-1){
		ll curval = allt[i+1].x.x-allt[i].x.y;
		if(allt[i].y == allt[i+1].y){
			if(allt[i].y){
				bc.pb(curval);
			} else
				bj.pb(curval);
...
int main() {
	...
	fore(test, 1, tests){
		...
		//calc result
		otype result=calcFunction(c,j,ct,jt);
		//write output
		outfile << "Case #" << test << ": ";
		cout << "Case #" << test << ": ";
#ifndef IFSTRUCT
