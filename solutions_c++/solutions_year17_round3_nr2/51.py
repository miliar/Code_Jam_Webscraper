...
int solve(int a, int b, int s) {
	...
	auto it = lower_bound(all(V), pair<pii, int>(pii(elt, INF), INF));
	if(it != V.begin()) {
		it--;
		if(inRange(elt, it->f.f, it->f.s-1))
			r = it->s;
	}
	...
	if(r != 1) {
		MIN(res, solve(a - 1, b, 0) + (s != 0));
	}
	if(r != 0) {
		MIN(res, solve(a, b - 1, 1) + (s != 1));
	}
...
int solve() {
	...
	int res = INF;
	alt = 0;
	MIN(res, solve(720, 720, 0));
	MIN(res, solve(720, 720, 1) + 1);
	memset(dp, -1, sizeof dp);
	alt = 1;
	MIN(res, solve(720, 720, 0) + 1);
	MIN(res, solve(720, 720, 1));
