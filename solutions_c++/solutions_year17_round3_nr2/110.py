...
struct activity {
	int start, end ;
	bool type ;
	activity(int ss, int ee, bool tt) : start(ss), end(ee), type(tt) {}
	int len() const {
		return end-start ;
	}
} ;
...
void __main() {
	...
	cin >> n >> m ;
	vector<activity> t ;
	int time[2] = {DAY/2, DAY/2} ;
	REP(i, n) {
		cin >> a >> b ;
		t.PB(activity(a,b, false)) ;
		time[0] -= t.back().len() ;
	}
	REP(j, m) {
		cin >> a >> b ;
		t.PB(activity(a,b, true)) ;
		time[1] -= t.back().len() ;
	}
	sort(ALL(t)) ;
	...
	int ret=0 ;
	REP(i, t.size()) {
		const activity &a = t[i] ;
		const activity &b = t[(i+1)%t.size()] ;
		if(a.type != b.type)
			ret++ ;
		else
			holes[a.type].PB((b.start-a.end+DAY)%DAY) ;
	}
	REP(i, 2) {
		sort(ALL(holes[i])) ;
		FOREACH(it, holes[i]) {
