...
int CodeJamMain()
{
    ...
    for (int caseNum = 0; caseNum < T; caseNum++)
    {
        ...
        for (int i = 0; i < nA; i++) input >> A[i].first >> A[i].second;
        for (int i = 0; i < nB; i++) input >> B[i].first >> B[i].second;

        auto pred = [](const pair<int, int>& a, const pair<int, int>& b)
        {
        ...
        if (nA == 0)
        {
            if (B[nB - 1].second - B[0].first <= 720 || 24 * 60 + B[0].second - B[nB - 1].first <= 720) ans = 2;
            else ans = 4;
        }
        else if (nB == 0)
        {
            if (A[nA - 1].second - A[0].first <= 720 || 24 * 60 + A[0].second - A[nA - 1].first <= 720) ans = 2;
            else ans = 4;
        }
        else
        {
