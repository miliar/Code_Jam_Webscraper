...
ll solve() {
  ...
  dp[0][0][0][0] = dp[0][1][1][0] = 1;

  rep(i, 1440) rep(s, 2) rep(j, 2) rep(k, i+1) {
    int p = T[i];
    int t0 = k;
    int t1 = i - t0;
    ...
    if (p == -1) {
      ...
      if (j == 0) {
        // そのまま
        if (t0+1 <= 720) amin(dp[i+1][s][0][k+1], dp[i][s][j][k]);
        // 交代
        if (t1+1 <= 720) amin(dp[i+1][s][1][k], dp[i][s][j][k] + 1);
      }
      else {
        // そのまま
        if (t1+1 <= 720) amin(dp[i+1][s][1][k], dp[i][s][j][k]);
        // 交代
        if (t0+1 <= 720) amin(dp[i+1][s][0][k+1], dp[i][s][j][k] + 1);
      }
    }
    if (p == 0) {
      if (j == 0) {
