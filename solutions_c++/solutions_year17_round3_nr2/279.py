...
int call(int cur, int camCnt, int f){
    if(camCnt>total/2) return inf;

    if(cur == total){
        if(camCnt != 720) return inf;
        if(START == f) return 0;
        return 1;
    }

    if(DP[cur][camCnt][f] != -1) return DP[cur][camCnt][f];

    int res = inf;

    if(Cameron[cur] == 0){
        int ret = call(cur+1, camCnt+1, 0);
        if(f == 1) ret++;
        res = min(res, ret);
    }
    if(Jabie[cur] == 0){
        int ret = call(cur+1, camCnt, 1);
        if(f == 0) ret++;
        res = min(res, ret);
    }

    return DP[cur][camCnt][f] = res;
}
