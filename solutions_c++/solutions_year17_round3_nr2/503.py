...
int main(int, char *[])
{
    freopen("C:/Users/Infinity/Downloads/B-large.in", "r", stdin);
    freopen("C:/Users/Infinity/Downloads/B-large.out", "w", stdout);
    for (int ti = 1, tt = getint(); ti <= tt; ++ti) {
        int n = getint(), m = getint();
        vector<char> f0(1441, 0), f1(1441, 0);
        for (int i : range(n)) {
            int from = getint(), to = getint();
            for (int j : range(from + 1, to + 1)) {
                f0[j] = 1;
            }
        }
        for (int i : range(m)) {
        ...
        clear(dp);
        for (int i : range(1441)) {
            for (int j : range(721)) {
                for (int k : range(2)) {
                    if (i == 1 && j == k && ((f0[1] == 0 && k == 0) || (f1[1] == 0 && k == 1))) {
                        dp[i][j][k] = 1;
                    } else {
                        dp[i][j][k] = 9999;
                        if ((k == 0 && f0[i]) || (k == 1 && f1[i])) {
                        } else {
                            if (i >= 1 && j >= k) {
                                dp[i][j][k] = min(dp[i - 1][j - k][k], dp[i - 1][j - k][1 - k] + 1);
                            }
                        }
                    }
                }
...
//            for (int j : range(25)) {
//                if (dp[i][j][1] != 9999) {
//                    writeln({i, j, 1, dp[i][j][1]});
//                }
//            }
//            writeln();
//        }
        writeln(caseN(ti), min(dp[1440][720][0], dp[1440][720][1]) / 2 * 2);
    }
    return 0;
}
