...
bool mycmp(const event &ls,const event &rs) {return ((ls.when.x)<(rs.when.x));}
...
{
    ans = 0;
    for(int i=0;i<3;i++) freeTime[i].clear();
    int z = bank.size();
    ...
    for(int i=0;i<z;i++)
    {
        ...
        int timeUsed = tf-ti;
        if(timeUsed<0) timeUsed+=1440;
        if(sumTeam==1) {ans++;}
        //printf("TimeUsed: %d\n",timeUsed);
        if(timeUsed<=0) continue;
...
int solve()
{
    ...
    for(int i=0;i<ac;i++)
    {
        event tst; tst.owner = 0;
        scanf("%d%d",&tst.when.x,&tst.when.y);
        bank.push_back(tst);
    }
    for(int i=0;i<aj;i++)
    {
        event tst; tst.owner = 1;
        scanf("%d%d",&tst.when.x,&tst.when.y);
        bank.push_back(tst);
    }
