...
void print(ll _, ll ret) {
    cout << "Case #" << _+1 << ": " << ret << endl;
}
int main(void) {
    ll t; cin >> t;
    rep(_, t) {
        ...
        vector<P> c(cn); 
        vector<P> d(dn); 
        rep(i, cn) cin >> c[i].fi >> c[i].se;
        rep(i, dn) cin >> d[i].fi >> d[i].se;
        if (!(cn == 2 || dn == 2)) {
            print(_, 2);
            continue;
        }
        if (dn) swap(c, d), swap(cn, dn);
        ll range = 0;
        chmax(range, ((c[0].fi - c[1].se) + 1440) % 1440);
        chmax(range, ((c[1].fi - c[0].se) + 1440) % 1440);
        if (range >= 720) {
            print(_, 2);
        } else {
            print(_, 4);
