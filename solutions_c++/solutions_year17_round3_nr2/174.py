...
int call(int pos,int pre,int a)
{
//    if(a>1440) return inf;
    ...
    if(pos==1440)
    {
        ...
        if(a==b && a==720) return ret;
        else               return inf;
    }

    int &ret = dp[pos][pre][a];
    if(ret!=-1) return ret;
    ret = inf;

    if(ar[pos])
    {
        int add = (ar[pos]==1)?1:0;
        int cost = ( pre!=0 && pre!=ar[pos] ) ? 1:0 ;

        ret = call(pos+1,ar[pos],a+add) + cost;
    }
    else{

        int cost = (pre==2 ? 1:0 );
        int x = call(pos+1,1,a+1) + cost;

        cost = (pre==1 ? 1:0 );
        int y = call(pos+1,2,a)   + cost;
        ret = Min(x,y);
    }
...
int main()
{
    ...
    freopen("B__large_out.txt","w",stdout);
