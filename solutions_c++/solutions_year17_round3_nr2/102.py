...
void open(){
	freopen("D:/OneDrive/Code/GCJ/B-large (2).in","r",stdin);
...
int dp(int minute, int left, int cur, int st_with){
	if(minute == 1440){
		...
		if(cur == st_with)return 0;
		return 1;
	}

	if(memo[minute][left][cur][st_with] != -1)return memo[minute][left][cur][st_with];
...
int main(void){
	...
	for(int ct = 1; ct <= tc; ct++){
		...
		FOR(i,c){
			int st, en;
			sc(st);sc(en);
			for(int j = st;j<en;j++)ac[j] = true;
		}

		FOR(i,j){
			int st, en;
			sc(st);sc(en);
			for(int j = st;j<en;j++)aj[j] = true;
		}
		int ans = INF;
		if(!ac[0]){
