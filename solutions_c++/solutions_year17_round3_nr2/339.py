...
const int tot = 60 * 24;
int a[tot + 5], d[tot + 5][5 + tot / 2][5][5];
...
void input() {
  ...
  while (n--) {
    S(from, to);
    FFR(j, from, to - 1) a[j] = 0;
  }
  while (m--) {
    S(from, to);
    FFR(j, from, to - 1) a[j] = 1;
  }
  FR(i, tot) FR(j, tot / 2 + 1) FR(k, 2) FR(l, 2) d[i][j][k][l] = 2 * tot;
  d[0][0][0][0]=0;
  d[0][1][1][1]=0;
  FOR(i, tot - 1) FR(j, tot / 2 + 1) FR(k, 2) FR(l, 2) {
    if (a[i] == -1 || a[i] == 0) d[i][j][0][k] = min(d[i][j][0][k], d[i - 1][j][l][k] + (l != 0));
    if (j != 0) if (a[i] == -1 || a[i] == 1) d[i][j][1][k] = min(d[i][j][1][k], d[i - 1][j - 1][l][k] + (l != 1));
  }
}

void output() {
  int res=2 * tot;
  FR(i, 2) FR(j, 2) res = min(res, d[tot - 1][tot / 2][i][j] + (i != j));
  PN(res);
}
