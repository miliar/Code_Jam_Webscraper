  ...
  int main() {
    ...
    for (int i = 0; i < t; ++i) {
      ...
      for (int j = 0; j < aj; ++j) {
        cin >> ts >> te;
        tj += te - ts;
        s.insert({{ts, te}, false});
      }
      vector<pair<int, int>> vc, vj;
      if (s.size() > 1) {
        for (auto it = s.begin(); it != s.end(); ++it) {
          ...
          if (it->S == jt->S) {
            kk += 2;
            if (it->S) {
              vc.push_back({(1440 + jt->F.F - it->F.S) % 1440, it->F.S});
            } else {
              vj.push_back({(1440 + jt->F.F - it->F.S) % 1440, it->F.S});
            }
          } else {
            ++kk;
          }
      ...
      sort(ALL(vc));
      sort(ALL(vj));
      for (int j = 0; j < vc.size() && tc + vc[j].F <= 720; ++j) {
        kk -= 2;
        tc += vc[j].F;
      }
      for (int j = 0; j < vj.size() && tj + vj[j].F <= 720; ++j) {
        kk -= 2;
        tj += vj[j].F;
      }
      cout << "Case #" << i + 1 << ": " << kk << '\n';
