...
int fr[2][1443] ; // 0 means cam, 1 means jemi
int visit[1443][723][3] , vv , wh ;
jora_int dp[1443][723][3] ;

jora_int rec(int tme,int spend,int now) // 0 means cam, 1 means jemi
{
    if(tme == 1440)
    {
        if(spend == 720)
    ...
    int &cas = visit[tme][spend][now] ;
    jora_int &ret = dp[tme][spend][now] ;
    ...
    if(fr[wh][tme] == 1 && spend<720)
    {
        temp = rec(tme+1,spend+1,wh) ;
        temp.fs += (now^wh) ;
    ...
    if(fr[wh^1][tme] && tme-spend<720)
    {
        temp = rec(tme+1,spend,wh^1) ;
        temp.fs += (now^(wh^1)) ;
...
int main()
{
    ...
    for(int cas=1 ; cas<=tcase ; cas++)
    {
        ...
        wh = 0 ;
        jora_int ans = rec(0,0,0);

        vv++;
        wh = 1 ;
        jora_int tans = rec(0,0,1);

        int fans = (ans.sc == 1 && tans.sc == 1 ? min(ans.fs,tans.fs) : (ans.sc == 1 ? ans.fs : tans.fs));
