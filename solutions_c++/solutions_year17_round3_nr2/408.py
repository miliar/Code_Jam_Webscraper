...
const int TOTAL_MINUTES = 24 * 60;

char used[2][TOTAL_MINUTES + 10];

int dp[TOTAL_MINUTES + 2][TOTAL_MINUTES / 2 + 2][2];
...
int solve_small() {
  ...
  re (tries, 2) {
    re (i, TOTAL_MINUTES + 1)
      re (j, TOTAL_MINUTES / 2 + 1)
        re (s, 2)
          dp[i][j][s] = (1<<20);
    ...
    fo (m, 1, TOTAL_MINUTES - 1) {
      re (s0, TOTAL_MINUTES / 2 + 1) {
          int s1 = m - s0;
          ...
          if (!used[0][m]) {
            remin (dp[m + 1][s0 + 1][0], dp[m][s0][0]);
            remin (dp[m + 1][s0 + 1][0], dp[m][s0][1] + 1);
          }
          if (!used[1][m]) {
            remin (dp[m + 1][s0][1], dp[m][s0][1]);
            remin (dp[m + 1][s0][1], dp[m][s0][0] + 1);
          }
    ...
    if (tries == 0) {
      cur = min (dp[TOTAL_MINUTES][TOTAL_MINUTES / 2][0],
                 dp[TOTAL_MINUTES][TOTAL_MINUTES / 2][1] + 1);
    } else {
      cur = min (dp[TOTAL_MINUTES][TOTAL_MINUTES / 2][0] + 1,
                dp[TOTAL_MINUTES][TOTAL_MINUTES / 2][1]);
    }
