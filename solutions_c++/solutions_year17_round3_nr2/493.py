        ...
        for (int i = 1; i <= SZ; i++) {
            if (true) { // j = 0
                for (int k = 0; k < 2; k++) {
                    for (int l = 0; l < 2; l++) {
                        f[i][0][k][l] = INF;
                        if (k == a[i - 1] || k == 1) continue; // 'k' can't do slot 'i' as it overlaps with activity, or '1' would have to do the slot, but `minutes_used_by_1` is 0
                        f[i][0][0][l] = min(f[i - 1][0][0][l], f[i - 1][0][1][l] + 1);
                    }
                }
            }
            for (int j = 1; j <= SZ / 2; j++) {
                for (int k = 0; k < 2; k++) {
                    for (int l = 0; l < 2; l++) {
                        f[i][j][k][l] = INF;
                        if (k == a[i - 1]) continue; // 'k' can't do slot 'i' as it overlaps with activity
                        f[i][j][k][l] = min(f[i - 1][j - k][k][l], f[i - 1][j - k][1 - k][l] + 1);
                    }
                }
            }
        }
        int fm = min(f[SZ][SZ / 2][0][0], f[SZ][SZ / 2][1][1]);
        printf("Case #%d: %d\n", t, fm);
    }
    return 0;
}
