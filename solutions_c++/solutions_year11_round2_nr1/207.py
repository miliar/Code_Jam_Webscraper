...
int main( int argc, char* argv[] ) {
	...
	for(int testCase=1;testCase<=testCases;testCase++){
		...
		vd wp(n);
		rep(i,n){
			wp[i]=1.*count(all(a[i]), '1')/(n-count(all(a[i]), '.'));
		}
		...
		rep(i,n){
			...
			rep(j,n){
				if(b[i][j]!='.'){
					double w=1.*count(all(b[j]), '1')/(n-count(all(b[j]), '.'));
					owp[i]+=w/(n-count(all(b[i]), '.'));
				}
			}			
		}
		...
		rep(i,n){
			rep(j,n){
				if(a[i][j]!='.')oowp[i]+=owp[j]/(n-count(all(a[i]), '.'));
			}
		}

		vd res(n);
		rep(i,n)res[i]=0.25*wp[i]+0.5*owp[i]+0.25*oowp[i];
