...
void RPI()
{
  ...
  {
    vector<string> team;
    vector<double> games_won, games_played, match_up;
    vector<double> OWP;
    ...
    for (unsigned j = 0; j < N; j++)
    {
      games_won.push_back(0);
      games_played.push_back(0);
      OWP.push_back(0);
      match_up.push_back(0);
    }
    for (unsigned j = 0; j < N; j++)
    {
      ifile >> input;
    ...
    for (unsigned j = 0; j < N; j++)
    {
      for (unsigned k = 0; k < N; k++)
      {
        ...
        {
          OWP[k] += (games_won[j] - 1)/(games_played[j] - 1);
          match_up[j]++;
        }
        else if (team[j][k] == '0')
        {
          OWP[k] += (games_won[j]) / (games_played[j] - 1);
          match_up[j]++;
        }
      }
    }
    ...
    for (unsigned j = 0; j < N; j++)
    {
      ...
      OOWP /= match_up[j];
      double total = 0.25 * (games_won[j]/games_played[j]) + 0.5 * OWP[j] + 0.25 * OOWP;
      ofile << total << endl;
    }
  }
