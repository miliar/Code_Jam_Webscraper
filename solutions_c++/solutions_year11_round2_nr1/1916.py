...
/* ****************************************************************** */
double wp[101+1],owp[101+1],oowp[101+1];
double win[101+1],loss[101+1];
int ctr;
int main(){
//  time_t seconds;  time(&seconds); srand((unsigned int)seconds);
  freopen ("A-large(2).in", "r", stdin); freopen ("file.out", "w", stdout);
  tst{
    int N;s(N);
    char a[N+1][N+1];
    REP(i,N)ss(a[i]);
    ...
    REP(k,N){
      ...
      REP(i,N){
        ...
        ctr++;
        if(a[k][i]=='1')
          owp[k]=owp[k]+win[i]/(win[i]+loss[i]-1);
        else if(a[k][i]=='0')
          owp[k]=owp[k]+(win[i]-1)/(win[i]+loss[i]-1);
      }
      owp[k]=owp[k]/ctr;
    }
