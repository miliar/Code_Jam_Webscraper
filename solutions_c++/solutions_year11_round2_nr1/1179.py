...
int main (int argc, char* argv[]) {
  ...
  for (int test=0; test<nbTests; test++) {
    ...
    FOR(i,0,n) {
      ...
      FOR(j,0,n) {
	if(t[i][j]=='.') continue;
	if(t[i][j]=='1') sum+= (win[j])/(double)(play[j]-1);
	else sum+= (win[j]-1)/(double)(play[j]-1);
      }
      owp[i] = sum/(double)play[i];
      //cout << "(" << i << ") owp=" << owp[i] << endl;
    }
