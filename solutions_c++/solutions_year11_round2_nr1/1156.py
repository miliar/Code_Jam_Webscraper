...
int main() {
	gcase {
		...
		int N;
		cin >> N;
		vector<row> grid(N, row(N));
		vector< pair<int,int> > wlr(N);
		vector<double> rpi(N);
		forl(i, 0, N) {
			int& win = wlr[i].first;
			int& lose = wlr[i].second;
			win = 0, lose = 0;
			forl(j, 0, N) {
				cin >> grid[i][j];
				if(grid[i][j] == '1') win++;
				else if(grid[i][j] == '0') lose++;
			}
			rpi[i] = 0.25 * win / (win+lose);
			
			//cerr << rpi[i] << endl;
		}
		//cerr << endl;
		cout << "Case #" << gtest << ":" << endl;
		vector<double> owp(N);
		forl(i, 0, N) {
			double sum = 0;
			int count = 0;
