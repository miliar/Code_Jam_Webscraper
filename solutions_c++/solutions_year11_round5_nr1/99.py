...
double area(const vector<point> &U, const vector<point> &L, double line) {
  double ans = 0;
...
double findline(const vector<point> &U, const vector<point> &L, double W, double target) {
  double lower = 0;
  double upper = W;
  while(upper - lower > eps) {
    double line = (upper + lower)/2;
    double a = area(U, L, line);
    if(a < target)
      lower = line;
    else
      upper = line;
...
void solve()
{
  ...
  double a = area(Hs, Ls, W);
  for(int i=1; i<G; i++) {
    double target = a*i/G;
    double line = findline(Hs, Ls, W, target);
    //cout << "line = " << line << endl;
    printf("%.8f\n", line);
  }
  //cout << a << endl;
