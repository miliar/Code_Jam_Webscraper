...
*/
int a[MAT_SIZE][1];
const LLI siz=0x6fffffffLL;
bool sieve[siz];
vector<vector<LLI> > res;

int *primesList,numprimes;

// store pow of 2 to 10 from power 0 to 16.
ULLI pw[11][19];

/*	N - number upto which sieve is to be made. Number below this number would be 
registered in the primesList
*/
int generatePrimesUsingSieve(int N, int maxNumPrimesRequired, int* primesList){
	...
	QL_FOR_DEF(k, N){
		if ( 1 != sieve[k] ){
			primesList[numPrimes] = k;
			numPrimes++;

			if (numPrimes >= maxNumPrimesRequired)
				break;
		}
	}
	//delete sieve;
...
int checkPrimality(LLI sum,int b, int n){
	//int mn=min(sqrt(sum/1.),numprimes/1.);
	int mn=min(sqrt(sum/1.),numprimes/1.);
	QL_FOR_DEF(i,mn){
		
		if(4==b && primesList[i]==31)
			int d=0;
		// contribution of MSB
		LLI tot=sum+computePower(b,n-1,primesList[i]);

		if(0==tot%primesList[i] /*&& tot!=primesList[i]*/)
			return primesList[i];
	}
	return -1;
}

bool bfs(int n,int j,int cnt){

	LLI i=0,k,kc;
	if(1){
		while(i<(1<<(n-2))){
			V_LLI fac;
			// append 1 at both ends
			LLI x=(1LL<<(n-1));
			...
			//if(kc==485432538631LL || kc==548549195581LL)
			//if(kc==1073742145LL || kc==1094713345LL)
			if(kc==32793LL)
				int d=0;
			LLI sum[11]={0};
			for(int b=0;k;k>>=1,++b){
			...
			QL_FOR(p,2,11,1){
				int x=checkPrimality(sum[p],p,n);
				if(-1==x){
					f=0;
					break;
				}else PB(fac,x);
...
int main(){
	...
	while(t--){
		...
		cin>>n>>j;
#endif
		primesList = new int[5800000];
		numprimes = generatePrimesUsingSieve(100000010,100000010,primesList);
		...
		QL_FOR_DEF(i,SZ(res)){
			//cout<<res[i][SZ(res[i])-1]<<" ";
			printBinary(res[i][SZ(res[i])-1]);
			cout<<" ";
			QL_FOR(j,0,SZ(res[i])-1,1){
				cout<<res[i][j]<<" ";
			}
			cout<<endl;
		}
...
#if 01
	FILE *fIn = freopen("G:\\DUMP\\output_C_large.out", "r", stdin);
	FILE *fOut = freopen("G:\\DUMP\\output_verified.out", "w", stdout);
	/*FILE *fLog = freopen("G:\\DUMP\\log.log", "w", stderr);
	assert(fIn&&fOut&&fLog);*/
#endif
	...
	*/
	...
	while(tc--){
		cin>>tmp;
		QL_FOR_DEF(i,9)cin>>factors[i];
		LLI x;
		int l=SZ(tmp);
		string s;
		QL_FOR_DEF(i,SZ(tmp))s+=tmp[l-1-i];
		QL_FOR(p,2,11,1){
			ULLI tot=0;
			QL_FOR_DEF(i,min(n,19)){
				if(s[i]=='1')tot+=pw[p][i];
			}

			int rem=0;
			if(n==32)
				rem=computePower(p,n-1,factors[p-2]);

			if(6==p)
				int d=0;
			tot+=rem;
			if(tot%factors[p-2])
				cout<<"WA - string = "<<tmp<<"  Wrong factor in base "<<p<<"  factor = "<<factors[p-2]<<endl;  
		}
	}
#endif
}
