...
Not every string of 0s and 1s is a jamcoin. For example, 101 is not a jamcoin; its interpretation in base 2 is 5, 
which is prime. But the string 1001 is a jamcoin: in bases 2 through 10, 
...
For example, for the jamcoin 1001 mentioned above, a possible set of nontrivial divisors for 
the base 2 through 10 interpretations of the jamcoin would be: 3, 7, 5, 6, 31, 8, 27, 5, and 77, respectively.
...
where x is the test case number (starting from 1). Each of the last J lines must consist of a jamcoin 
of length N followed by nine integers. The i-th of those nine integers (counting starting from 1) must be 
a nontrivial divisor of the jamcoin when the jamcoin is interpreted in base i+1.
...
This is only one of multiple valid solutions. Other sets of jamcoins could have been used, 
and there are many other possible sets of nontrivial base 10 divisors. Some notes:

110111 could not have been included in the output because, for example, 
it is 337 if interpreted in base 3 (1*243 + 1*81 + 0*27 + 1*9 + 1*3 + 1*1), and 337 is prime.
...
For the first jamcoin in the sample output, the first number after 100011 could not have been either 1 or 35, 
because those are trivial divisors of 35 (100011 in base 2).
*/
...
bool IncrementCoin(string& numstr, int size)
{
	// the first and last digits is 1
	int carry = 1;
	int i;
	for(i = size-2; i > 0 && carry; i--)
	{
		if(numstr[i] == '1')
		{
			numstr[i] = '0';
...
size_t InterpretNumber(string& numstr, int size, int base)
{
	size_t num = 0;
	size_t digit_base = 1;
	for(int i = size-1; i >=0; i--)
	{
		num += (numstr[i] - '0') * digit_base;
		digit_base *= base;
	}
	return num;
}

const long int num32max = 4294967296;
vector<size_t> all_primes;
bool sieve[num32max];

size_t ComputeAllPrimes(const size_t max)
{
...
// return a non-trivial divisor if num is not a prime,
// otherwise return 1
...
int main()
{
 	size_t prime_count = ComputeAllPrimes(100000000);
	...
	while(T--)
	{
		...
		cout << case_str << count++ << colon << endl;
		while(J > 0)
		{
			int jamcoin = true;
			vector<size_t> all_divisors(10);
			for(int b = 2; b <= 10; b++)
			{
				size_t num = InterpretNumber(numstr, N, b);
				all_divisors[b] = IsPrime(num);
				if(all_divisors[b] == 1 || all_divisors[b] == num)
				{
					jamcoin = false;
					break;
				}
			...
			if(!IncrementCoin(numstr, N))
				break;
		}
	}
	/*
	string numstr = "1000000010100011";
	size_t num = InterpretNumber(numstr, 16, 10);
	size_t is_prime = IsPrime(num);
	cout << num << " " << is_prime << endl;
	*/
	return 0;
}
