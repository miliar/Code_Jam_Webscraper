...
template<typename T1, typename T2> inline constexpr pair<T1, T2> mkp(const T1 &a, const T2 &b)
...
constexpr const array<int, 10> pow10({1, 10, 100, 1000, 10000, 100000, 1000000, 10000000, 100000000, 1000000000});
}
} // namespace Infinity

long long basentodec(long long x, int n)
{
    long long p = 0, a = 1;
    while (x)
    {
...
int main(int, char *[])
{
    freopen("C:\\Users\\Infinity\\Downloads\\C-small-attempt1.in", "r", stdin);
    freopen("C:\\Users\\Infinity\\Downloads\\C-small-attempt1.out", "w", stdout);
    for (int t = getint(), i = 1; i <= t; i++)
    {
        int n = getint(), j = getint();
        writeln(caseSharpNoSpace(i));
        int q = 0;
        for (int i = (1 << (n - 1)) + 1; i < (1 << n); i += 2)
        {
            vector<int> a;
            long long b = stoll(bitset<32>(i).to_string());
            bool ok = false;
            for (int k = 2; k <= 10; k++)
            {
                int d = getDivisor(basentodec(b, k));
                if (d == -1)
                {
                    ok = true;
                    break;
