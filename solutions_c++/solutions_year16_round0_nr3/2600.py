...
vector<ll> s;
int N, J;
void convert(int mask) {
    for (int i = 1; i < N-1; i++)
        s[i] = checkbit(mask, i-1);
}
ll check(ll base) {
    ll num = 0;
    ll b = 1;
    for (int i = 0; i < N; i++, b *= base) {
        num += b*s[i];
    }
    for (ll i = 2; sqr(i) <= num; i++) {
        if (num % i == 0) return i;
    }
    return 0;
}
...
//        fastIO();
    ...
    in_range(t, 1, T+1) {
        ...
        s[0] = s[N-1] = 1;
        ll MAXN = (1<<(N-2));
        for(int mask = 0; mask < MAXN && J; mask++) {
            convert(mask);
            vector<ll> divisors;
            ...
            if (sz(divisors) == 9) {
                in_range(i, 0, N) printf("%lld", s[N-i-1]);
                printf(" ");
                for (auto &d : divisors) printf("%lld ", d);
                puts("");
                J--;
            }
        }
