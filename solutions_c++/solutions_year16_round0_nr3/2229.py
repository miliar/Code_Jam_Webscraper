...
InfInt valueInBase(string s, int b)
{
	int len = s.size();
	InfInt result = 0;
	for (int i = 0; i < len; ++i)
...
string binary(unsigned long long n, unsigned int digitCount)
{
	string result;
	...
	while (result.size() < digitCount) result.push_back('0');

	reverse(result.begin(), result.end());
	return result;
}

InfInt isPrime(InfInt n)
{
	InfInt sqrt = 11;//n.intSqrt();
	for (InfInt i = 2; i <= sqrt; ++i)
	{
		if (n % i == 0)
			return i;
	}
...
int main()
{
	...
	for (int tc = 1; tc <= t; ++tc)
	{
		...
		cin >> n >> j;
		cout << "Case #" << tc << ":" << endl;
		unsigned long long int limit = _Pow_int<unsigned long long>(2, n - 2);
		for (int no = 0; no < limit; ++no)
		{
			string nostr = "1" + binary(no, n-2) + "1";
			string result = nostr;
			bool candidate = true;
			for (int base = 2; base <= 10; ++base)
			{
				InfInt v = valueInBase(nostr, base);
				InfInt dividend = isPrime(v);
				if (dividend == -1)
				{
					candidate = false;
					break;
