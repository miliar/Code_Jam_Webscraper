...
BigInt::BigInt( long double d )
{
    ...
    while( d > 0 )
    {
        digits[size++] = min(( int )( ( d - floor( d / 10 ) * 10 ) + 0.5 ), 9);
        d = floor( d / 10 );
    }
}
...
BigInt BigInt::operator*( long double n )
{
    ...
    for( int i = ndigs - 1; i >= -DIGS_AFTER_DOT; i-- )
    {
        ...
        for( int j = 0; j < size || carry; j++ )
        {
            ...
            if( i + j >= 0 && result.digits[i + j] ) result.size = min(result.size,i + j + 1);
            carry = newdig / 10;
        }
    }
...
BigInt baseXtoBase10(string number,BigInt x){
	reverse(number.begin(), number.end());
	BigInt ret = 0;
	for(int i = 0;i<number.size();i++)
		ret+=((BigInt((number[i]-'0')) * powd(x,i)));
	return ret;
}

long long isPrime(BigInt x){

	for(int i = 2;(BigInt(i*i)<=x) && !(BigInt(i)==x) && (i<100);i++){

		if((x%BigInt(i)) == BigInt(0))
			return i;
	}
	return 1;
}
...
int main() {
	...
	while(t--){
		...
		int n,j;
		cin>>n>>j;
		for(long long i = 0;i<(1<<(n-2)) && j>0;i++){
			string s = "1";
			for(int l = 0;l<(n-2);l++)
				s.push_back('0' + ( ( (i&(1<<l))) != 0) );
			s.push_back('1');
			vector<long long> ans;
			for(int x = 2;x <= 10;x++){
				BigInt num = baseXtoBase10(s,BigInt(x))	;
				long long a = isPrime(num);
