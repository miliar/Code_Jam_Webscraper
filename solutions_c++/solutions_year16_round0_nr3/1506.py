...
/*
 * author of Big Integer library in C++: panks
 * the rest of the code: melsclar / msclar
 */
...
BigInteger interpretar (long long int num, int base) {
	BigInteger result = 0;
	BigInteger pot = 1;
	for (int i = 0; i < N; i++) {
		if (num % 2)
...
// max debe ser < 10000
int divisor (BigInteger num, int max) {
	for (int i = 0; i < max; i++) {
		BigInteger t = num % primos[i];
		if (num % primos[i] == 0)
			return primos[i];
	}
	return -1;
...
int main () {
	...
	// 500 jamcoins of length 32
	int divisores[16];
	int cantResults = 0;
	
	cout << "Case #1:" << endl;
	for (long long int num = (1LL << (N-1)) + 1; num < (1LL << N) && cantResults < J; num += 2) {		
		bool isJamCoin = true;
		
		for (int base = 2; base <= 10; base++) {
			BigInteger numEnBase = interpretar(num, base);
			divisores[base] = divisor(numEnBase, 10);
		...
		if (isJamCoin) {
			long long int numAux = num;
			string str;
			for (int i = 0; i < N; i++) {
				str.push_back(numAux%2 + '0');
				numAux /= 2;
			}
