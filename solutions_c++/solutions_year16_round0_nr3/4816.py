...
//	Set(a);
	...
	if(a.size())
	{
		for(i=a.size()-1; !a[i] && i>=0; i--);
		if(i>=0)
		{
			printf("%d",a[i]);
			FORD(j,i-1,0) printf("%09d", a[j]);
			return;
		}
	}
	printf("0");	
...
BigInt operator / (BigInt a, BigInt b)
{
	Set(a);
	Set(b);
	if (b == Integer(0ll)) return Integer("-1"s);
	BigInt ans, cur;
	FORD(i,a.size()-1,0)
	{
...
long long int operator % (BigInt a, long long int b)
{
	Set(a);
	if (b == 0) return -1;
	int ans = 0;
...
{
	if (b == Integer(0ll)) return Integer(1);
