...
using bt2 = uint64_t;
using bt2s = int64_t;
using bt4 = unsigned __int128;
using bt4s = __int128;
constexpr bt2 bsize = 65536LL * 65536;
constexpr bt bmax = 65536LL * 65536 - 1;
...
constexpr bt4 MaximalSizeForFastMul = 1000;
...
std::string binary(uint32_t next) {
    string s;
    do {
        if (next & 1)
            s = '1' + s;
...
int main() {
    ...
    const BN BN2 = BN::bn1()+BN::bn1();
    for (size_t i = 14; i < 33; ++i) {
        size_t count = 0;
        uint32_t test = 1;
        test = test << (i - 1);

        uint32_t next = test - 1;
        while (next != test * 2 + 1 && count < 501) {
            next += 2;
            if (prime[next])
                continue;
            bool ok = true;
            for (size_t base = 3; ok && base < 11; ++base) {
                BN value;
                BN baseBN(base);
                for (size_t mask = test; mask; mask >>= 1) {
                    value = value * baseBN;
                    if (next & mask) {
                        ++value;
                    }
            ...
            vector<BN> divisors;
            for (size_t base = 2; ok && base < 11; ++base) {
                BN value;
                BN baseBN(base);
                for (size_t mask = test; mask; mask >>= 1) {
                    value = value * baseBN;
                ...
                for (BN start(3); start < end; ++start) {
                    if ((value % start).is0()) {
                        divisors.push_back(start);
                        break;
                    }
                }
            }
