...
//#define MAX_PRIME  11111111 //★適当
#define MAX_PRIME  50000 //★適当
...
void CalcPrime(int N)
{
  ...
  for(int i=0;i<=N;i++)
  {
    if(isPrime[i])VPrime.push_back(i);
  }
  return;
}

int is_primeB(BigNum val)
{
  int i = 0;
  int flag = 1;
  while(1){
    BigNum p = convert(VPrime[i]);
    if((val % p) == ZERO){
      flag = 0;
      break;
    }
    i++;
    if(i>=VPrime.size())break;
  }
//cout<<", val="<<val<<", flag="<<flag<<endl;
  return flag;
}

BigNum get_nonprime(BigNum val)
{
 int i = 0;
  while(1){
    BigNum p = convert(VPrime[i]);
  ...
  return val;//ここにはこない想定
}

BigNum get_val(LL bits_all, int base)
{
  BigNum ret = ZERO;
  BigNum m = ONE;
  while(bits_all > 0){
...
int main(void)
{
  ...
  for(t=1;t<=T;t++)
  {
    ...
    int count = 0;
    LL bits_in_num = 0;
    // 最初と最後のbitは必ず1なので、それ以外のbitを全通り試す
    bits_in_num = 1<<(N-2);
    for(LL bits_in = 0; bits_in < bits_in_num; bits_in++){
      vector<BigNum> vec;
      // 値を構成。最初と最後のbitは必ず1。
      LL bits_all = (bits_in << 1) + 1 + (1LL<<(N-1));
//cout<<bits_all<<endl;
      int is_coin = 1;
      //基数を2～10まで計算する
      for(int base = 2 ; base <=10 ; base++){
        // base 進数の値計算
        BigNum val = get_val(bits_all, base);
//cout<<"val="<<val<<endl;
        // 素数かどうかチェック
        int flag = is_primeB(val);
//cout<<"xx"<<endl;
        if (flag != 0){
          is_coin = 0;
          break;
