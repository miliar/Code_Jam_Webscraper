...
ULL Multi(ULL a,ULL b,ULL Mod)
{
    ...
    while (b)
    {
        if (b&1)        ans = Mod-ans>a?ans+a:ans+a-Mod;
...
bool FermatTheorem(LL p,int k){
    while(k--){
        LL a = (rand()%min(p-1,(LL)MOD)) + 1;
        LL x = BigMod(a,p-1,p);
...
set < string > ans;
vector< LL > prime;
int status[(40000000/32)+7];

void Sieve(){
    int i,j,sq = sqrt(40000000);
...
void Check(int n){
    ...
    for (int b=2;b<=10;b++){
        ...
        if (FermatTheorem(num,50))
            return ;
    }
    ans.insert(s);
}

void Gen(int p,int n,int j){
    if (p == n){
        s[p] = 0;
        if (ans.size()<j)   Check(n);
        return ;
    }
