...
ull getNum(int radix, std::string &str) {
  ull N = str.size() + 2;
  ull ans = power(radix, N-1) + 1;
  int pow_int = 1;
  for(int i=str.size()-1; i>=0; i--) {
    ans += ((str[i]-'0')*power(radix, pow_int++));
  }
  return ans;
}

bool isPrime(std::set<ull> &mp, ull pr) {
  if(mp.count(pr)) {
    return true;
  } else {
    return false;
...
void getFactors(std::vector<ull> &prime, std::vector<ull> &res, std::string &str) {
  for(int radix=2; radix<=10; radix++) {
    ull get_num = getNum(radix, str);
    for(int i=0; i<prime.size(); i++) {
      if(get_num%prime[i] == 0) {
    ...
    while(J--) {
      ...
      while(!avail) {
        ...
        for(radix=2; radix<=10; radix++) {
          ull get_num = getNum(radix, str);
          if(is_prime(get_num, ITERATIONS, miller)) {
            break;
          }
        }
        if(radix == 11) {
      ...
      std::vector<ull> res;
      getFactors(result, res, str);
      printf("1%s1 ", str.c_str());
