...
bool divide(string num, i64 base, i64 den)
{
  stringstream res;
  i64 remainder = 0;
  for(char c: num){
    remainder = remainder * base + (c - '0');
    res << remainder / den;
    remainder %= den;
  }
  return remainder == 0;
...
string generate(i64 n)
{
  string s((unsigned int)n, '1');
  for(i64 i = 1; i < n - 1; ++i) s[i] = (char)(rand() % 2 + '0');
  return s;
}
...
int main()
{
  i64 test_case;
  cin >> test_case;
  rep(test_i, test_case){
    cout << "Case #" << test_i + 1 << ":" << endl;
    i64 n, k;
    cin >> n >> k;
    ...
    while(sz(found) < k){
      ...
      bool ok = true;
      for(i64 base = 2; base <= 10; ++base) {
        ok = false;
        for(i64 divisor = 2; divisor <= 13; ++divisor){
          if(divide(s, base, divisor)){
      ...
      cout << s;
      for(i64 divisor: divisors) cout << " " << divisor;
      cout << endl;
      found.insert(s);
    }
  }
