...
map<int , map <int, BigInt > > baseData; 

void fillTestData(){
	for(int base = 2 ; base <= 10; base= base+1 ){
		for(int i = 0 ; i < CJSIZE; i++ ){
			BigInt bigBase = Integer(base);
			baseData[base][i] = pow( bigBase, i );
		}
	}
}


 bool bPrime(BigInt number , BigInt & divisor) {
    for (int i=2;  i <=999; i = i + 1) {//we have too many numbers to choose from !!!
        if (number % i == 0 ) {
		divisor = Integer(i);
		return false;
	}
...
bool test( bitset<CJSIZE>  pcj,  BigInt divisors[]){
	for(int base = 2 ; base <= 10; base = base+1 ){
		BigInt numInBase = Integer(0);
		
		for( int i = 0 ; i < CJSIZE; i++ ) 
		{
			numInBase = numInBase + baseData[base][i]*pcj[i];
		}
		
		BigInt divisor;
		if( bPrime( numInBase, divisor)){
			return false;	
		}
...
int main(){
	...
	for( int t =1 ; t <= T ; t++){
		...
		for( int i = 0 ; j ;i++)
		{
		     bitset<MSIZE>  middle(i);
		     bitset<CJSIZE>  pcj(string("1") + middle.to_string() + string("1"));
		  
		
		     BigInt divisors[9];	
		     if( test( pcj , divisors )){
			cout <<  pcj ;
			for(int d = 0; d < 9; d++)
			{
			     cout << " " ;
