...
int searchfrom(int prim, int low, int high){
    int mid=(low+high)/2;
    if(primes[mid]==prim){
        return mid;
    }
    else if(primes[mid]>prim){
        if((mid>0) && primes[mid-1]>prim){
            return searchfrom(prim,low,mid-1);
        }
        else{
            return mid;
        }
        ...
        return searchfrom(prim,mid+1,high);
    }
}

bool iscomp(long long int n,int base, int it){
    if(n%2==0){
        divs[base]=2;
        return true;
    }
...
int main(){
   ...
   // for(int i=0;i<l;++i){
    ...
    for(int i=32769;i<=65535;i+=2){
        ...
        int f=1;
        for(int b=2;b<=10;++b){
            long long int num=basechange(b,a);
            long long int nn=sqrt(num);
            int aa=nn;
            int it=searchfrom(aa,0,primlen-1);
            if(!iscomp(num,b,it)){
                f=0;
                break;
            }
