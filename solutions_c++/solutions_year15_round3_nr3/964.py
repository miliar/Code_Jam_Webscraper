...
int check (int target, int mask) {
	...
	// LOG (m, v + 1);
	FOR (i, target) if (m [i] == 0) return INF;
	FOR (i, v) if (m [i] == 0) return 1;
	return 0;
}
...
int calc (int mask) {
	...
	int LIM = 1 << (target + 1);
	REP (i, LIM) {
		int extra = __builtin_popcount (i) - (__builtin_popcount (mask & i));
		ret = min (ret, extra + check (target, i | mask));
		// LOG (ret);
		// NL;
	}
