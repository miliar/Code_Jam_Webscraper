...
#define open       freopen("C-small-attempt0.in","r",stdin)
#define close      freopen ("C-small.out","w",stdout)
...
int coin[MX],dp[MX],a[MX],N;

int check(int n)
{
    for (int i=1;i<=n;i++)
        if(dp[i]==INF) return 0;
    ...
    while(t--){
        ...
        for (int i=1;i<=n;i++){
            ...
            for (int k=coin[i];k<=m;k++){
                if(dp[k]!=0){
                    if(dp[k]>=dp[k-coin[i]]&&dp[k-coin[i]]<1)
                    {
                        dp[k]=dp[k-coin[i]]+1;
                    }
                }
