...
void gen(){
	...
	string t = "ourlanguageisimpossibletounderstandtherearetwentysixfactorialpossibilitiessoitisokayifyouwanttojustgiveupq";
	REP(i, sz(s))	m[ s[i] ] = t[i], seen[ t[i] ][0] = 1, seen[ s[i] ][1] = 1;
	char c1 = ' ', c2 = ' ';
	for(char c = 'a'; c <= 'z'; c++){
		if(!seen[c][0])	seen[c][0] = 1, c1 = c;
		if(!seen[c][1])	seen[c][1] = 1, c2 = c;
	}
	if(c2 >= 'a' && c2 <= 'z')	m[c2] = c1;
	for(char c = 'a'; c <= 'z'; c++){
		fout << "	m['" << c << "'] = '" << m[c] << "';" << endl;
	}
	fout << "	m[' '] = ' ';" << endl;
