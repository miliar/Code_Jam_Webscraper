...
int main(int argc,char **args)
{
    ...
    map<char, char> language_map;
    string mar_language = "ejpmysljylckdkxveddknmcrejsicpdrysirbcpcypcrtcsradkhwyfrepkymveddknkmkrkcddekrkdeoyakwaejtysrreujdrlkgcjv";
    string earth_language="ourlanguageisimpossibletounderstandtherearetwentysixfactorialpossibilitiessoitisokayifyouwanttojustgiveup";
    for (size_t i = 0; i < mar_language.length(); ++i)
    {
        language_map.insert(make_pair(mar_language[i], earth_language[i]));
    }
    language_map.insert(make_pair('z', 'q'));
    language_map.insert(make_pair('q', 'z'));
    language_map.insert(make_pair(' ', ' '));
    int T = 0;
    cin>>T;
    for (int i = 0; i < T; ++i)
    {
