...
string googlerese(string str) 
{
	string s1;

	for (int i = 0; i < str.length(); ++i)
	{
		s1.append(1, trans[str[i]]);
	}
...
int main()
{
	...
	for (int i = 0; i1.length() == o1.length() && i < i1.length(); ++i)
	{
		trans[i1[i]] = o1[i];
	}
	...
	for (int i = 1; i <= n; ++i)
	{
		string str = readString_whitespace();
		cout << "Case #" << i << ": " << googlerese(str) << endl;
	}
