...
"so it is okay if you want to just give up"
 ...
 int main()
 {
	 ...
	 FF(i,0,3)
	 {
	 FF(j,0,inputs[i].length())
	 {
	 if(s[inputs[i][j]-97]!=outputs[i][j] && s[inputs[i][j]-97]!=0)
		 cout<<"Ambiguity for mapping of "<<inputs[i][j]<<"Earlier :"<<s[j]<<" Now :"<<outputs[i][j];
	 else
		 s[inputs[i][j]-97]=outputs[i][j];
	 }
	 }
	 int t;
	 cin>>t;
