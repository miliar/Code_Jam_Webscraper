...
static inline bool ReadLine(bool trim = true) {
...
void ZyxGCJ_PreCompute()
{
  char const a[] = "ejpmysljylckdkxveddknmcrejsicpdrysirbcpcypcrtcsradkhwyfrepkymveddknkmkrkcddekrkdeoyakwaejtysrreujdrlkgcjvyeq";
  char const b[] = "ourlanguageisimpossibletounderstandtherearetwentysixfactorialpossibilitiessoitisokayifyouwanttojustgiveupaoz";
  ...
  for (int i = 0; i < N; ++i)
  {
    if (usedA[a[i] - 'a'])
    {
      if (dic[a[i]] != b[i])
      {
        dprintf("Something doesn't add up %c %c [%d]\n", a[i], b[i], dic[a[i]]);
        exit(1);
      }
    }
    else
  ...
  size_t remA = count(ALL(usedA), false);
  size_t remB = count(ALL(usedB), false);
  if (remA != remB || remA > 1)
  {
    dprintf("How can I guess what remains? %d %d\n", remA, remB);
    exit(1);
  }
  if (remA)
    for (int i = 0; i < 26; ++i) if (!usedA[i])
      for (int j = 0; j < 26; ++j) if (!usedB[j])
        dic[i + 'a'] = static_cast<char>(j + 'a');
...
{
  ReadLine(false);
  size_t N = strlen(stringReader);
  for (int i = 0; i < N; ++i)
    stringReader[i] = dic[stringReader[i]];
  printf(stringReader);
}
////////////////////////// Solution ///////////////////////////