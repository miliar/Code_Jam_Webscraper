...
char s[11111];
int nums[2000000*2+10];
#define index(i) (i+2000000+2)
int main(void)
{
	long cc,tt;
	long i;long y,r,c;
	...
	FOR(cc,tt){
		...
		while(!que->empty()){
			if(nums[index(que->front())]>1){
				//cout<<"Index:"<<que->top()<<endl;
				ans+=nums[index(que->front())]/2;
				nums[index(que->front())+1]+=nums[index(que->front())]/2;
				nums[index(que->front())-1]+=nums[index(que->front())]/2;
				nums[index(que->front())]%=2;
				ttt=que->front();
				que->pop();
				que->push(ttt+1);
				que->push(ttt-1);
