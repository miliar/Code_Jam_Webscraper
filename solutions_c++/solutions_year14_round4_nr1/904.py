...
int rec(int st){
  ...
  ret = n;
  rep(i,n)if((1<<i)&~st){
    ret = min(ret,1 + rec(st|(1<<i)));
    rep(j,i)if(((1<<j)&~st) && in[i]+in[j]<=x)
      ret=min(ret,1+rec(st|(1<<i)|(1<<j)));
  }
...
void solve(int ca){
  ...
  for(int i=0;i<=x;++i){
    ...
    ++ans;
    --app[i];
    if(app[i]==0) app.erase(i);
    auto it= app.lower_bound(x-i);
    if((it==app.end() && it!=app.begin()) ||
       (it != app.begin() &&  it->F+i>x ) ) --it;
    
    if(it != app.end()){
      //cout << i << " " << it->F << endl;
      --(it->S);
      if(it->S == 0) app.erase(it->F);
    }//else cout << i << endl;
  }
  ...
  int rans = rec(0);
  //if(rans != ans) cout << rans << " " << ans << endl;
  printf("Case #%d: %d\n",ca,rans);
}
