...
typedef struct zMemo_struct{
	string ind;
	int value;
	bool operator <= (zMemo_struct & t){ return ind <= t.ind; }
}zMemo_type;

class zMemo{
	int unsort_max_len;
	zArray <zMemo_type> z_sort, z_unsort;
	
public:
	zMemo(){ unsort_max_len = 10; }
	void Add(string & ind, int value){
		int i;
		if (z_unsort.Len() == unsort_max_len){
			for (i = 0; i < z_unsort.Len(); i++)z_sort.Add() = z_unsort[i];
			z_sort.Sort();
			z_unsort.Free();
		}
		z_unsort.Add();
		z_unsort[-1].ind = ind;
		z_unsort[-1].value = value;
	}
	bool GetValue(string & ind, int & value){
		int i;
		zMemo_type t;
		t.ind = ind;
		if (z_sort.Len()){
			i = z_sort.BinSearch(t);
			if (z_sort[i].ind == ind){ 
				value = z_sort[i].value; 
				return true; 
			}
		}
		for (i = 0; i < z_unsort.Len(); i++)if (z_unsort[i].ind == ind){
			value = z_unsort[i].value; 
			return true;
		}
		return false;
	}
...
void main(){
	...
	for (n = 0; n < T; n++){
		int N, X, i;
		zIntArray z;
		N = stoi(f.GetLex(2 * n + 1, 0));
		X = stoi(f.GetLex(2 * n + 1, 1));
		for (i = 0; i < N; i++) z.Add() = stoi(f.GetLex(2 * n + 2, i));
		z.Sort();
		...
		for (i = N - 1; i >= 0; i--){
			if (z[i] == -1)continue;
			r++;
			for (j = i - 1; j >= 0; j--)if(z[j] != -1)if ((z[j] + z[i]) <= X){
				z[j] = -1;
				break;
			}
		}
