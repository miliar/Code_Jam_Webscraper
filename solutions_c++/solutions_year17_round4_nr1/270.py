...
int get(int b, int c, int d, int cur) {
  if (dp[cur][b][c][d] >= 0) return dp[cur][b][c][d];
  if (! (b || c || d)) return 0; 
  int ret = 0;
  if (b) ret = max(ret, get(b - 1, c, d, (cur + 1) % p));
  if (c) ret = max(ret, get(b, c - 1, d, (cur + 2) % p));
  if (d) ret = max(ret, get(b, c, d - 1, (cur + 3) % p));
  if (cur % p == 0) ret += 1;
  dp[cur][b][c][d] = ret;
  return ret;
}
...
void solve() {
  ...
  int ans = cnts[0] + get(cnts[1], cnts[2], cnts[3], 0);
  cout << ans << endl;
}
