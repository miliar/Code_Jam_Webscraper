...
int vs[5][105][105][105];
int pq[5][105][105][105];

int dp(int r,int a,int b,int c)
{
	if(a+b+c==0)return 0;
	int &pr=pq[r][a][b][c];
	if(vs[r][a][b][c]==kk)return pr;
	vs[r][a][b][c]=kk;
	
	pr=0;
	if(a)pr=max(pr, dp((r+1)%p,a-1,b,c)+(r==0) );
	if(b)pr=max(pr, dp((r+2)%p,a,b-1,c)+(r==0) );
	if(c)pr=max(pr, dp((r+3)%p,a,b,c-1)+(r==0) );
...
int main()
{
	...
	while(ts--)
	{
		...
		sc2(n,p);
		f(i,n)sc(k),fr[k%p]++;
		int rs=fr[0]+dp(0,fr[1],fr[2],fr[3]);
		//cerr<<rs<<" "<<t<<endl;
		//assert(rs==t);
		printf("Case #%d: %d\n",kk++,rs);
	}
