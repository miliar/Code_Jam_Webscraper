/*
Date: 2017/05/13 21:25:39 Saturday
*/
#include<bits/stdc++.h>
using namespace std;
#define X first
...
int main(){
	...
	scanf("%d",&C);rep(_,1,C){
		...
		scanf("%d%d",&n,&m);
		rep(i,0,m+1)b[i]=0;
		rep(i,1,n)scanf("%d",&a[i]),b[a[i]%m]++;
		int ans=b[0];
		if(m==2){
			while(b[1]>=2)b[1]-=2,++ans;
			ans+=b[1];
		}else if(m==3){
			while(b[1]>=1&&b[2]>=1)b[1]--,b[2]--,++ans;
			while(b[1]>=3)b[1]-=3,++ans;
			while(b[2]>=3)b[2]-=3,++ans;
			ans+=b[1]||b[2];
		}else if(m==4){
			while(b[1]>=1&&b[3]>=1)b[1]--,b[3]--,++ans;
			while(b[2]>=2)b[2]-=2,++ans;
			while(b[1]>=4)b[1]-=4,++ans;
			while(b[3]>=4)b[3]-=4,++ans;
			ans+=b[1]||b[2]||b[3];
		}
		printf("%d\n",ans);
	}
