...
bool check(int x , int y , int z){
    return ((a[1] - x) + (a[2] - y) * 2 + (a[3] - z) * 3) % p == 0;
}
int dfs(int x , int y , int z){
    if (dp[x][y][z] != -1) return dp[x][y][z];
    if (x + y + z == 0) return 0;
    if (x) checkMax(dp[x][y][z] , dfs(x - 1 , y , z));
    if (y) checkMax(dp[x][y][z] , dfs(x , y - 1 , z));
    if (z) checkMax(dp[x][y][z] , dfs(x , y , z - 1));
    dp[x][y][z] += check(x , y , z);
    return dp[x][y][z];
}
void solve(){
    RST(a);
