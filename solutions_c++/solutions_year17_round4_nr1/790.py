...
struct solver
{
    ...
    int rec(int last, vector<int> current) {
        if (mem.count( {last, current} ) == 0 ) {
            int & res = mem[ {last, current} ];
            res = 0;
            for (int i = 0; i < P; i++) {
                if ( current[i] > 0 ) {
                    ...
                    newv[i]--;
                    int newr = ( (last == 0) ? 1 : 0 );
                    newr += rec( (last + i) % P, newv );
                    res = std::max(res, newr);
                }
            }
        }
        return mem[ {last,current} ];
    }
