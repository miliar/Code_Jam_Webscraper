...
int solve()
{
	...
	VI mod_cnt( 4 );
	for_each( ALL( G ), [&]( const int g ){ ++mod_cnt[ g % P ]; } );

	static int dp[ 128 ][ 128 ][ 128 ][ 4 ];
	fill( AALL( dp ), -INF );
	dp[ mod_cnt[1] ][ mod_cnt[2] ][ mod_cnt[3] ][0] = mod_cnt[0];
		...
		for ( int j = 100; 0 <= j; -- j )
		{
			for ( int k = 100; 0 <= k; --k )
			{
				REP( m, P )
				{
					if ( i )
					{
						chmax( dp[ i - 1 ][j][k][ ( m + 1 ) % P ], dp[i][j][k][m] + ( m == 0 ) );
					}
					if ( j )
					{
						chmax( dp[i][ j - 1 ][k][ ( m + 2 ) % P ], dp[i][j][k][m] + ( m == 0 ) );
					}
					if ( k )
					{
						chmax( dp[i][j][ k - 1 ][ ( m + 3 ) % P ], dp[i][j][k][m] + ( m == 0 ) );
					}
				}
			}
		}
	...
	return *max_element( ALL( dp[0][0][0] ) );
}
