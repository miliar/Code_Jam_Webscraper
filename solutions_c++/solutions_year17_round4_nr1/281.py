...
void freshChocolate(int caseNumber)
{
	int N, P; 
	cin >> N >> P; 
	...
	int numGroups = group[0]; 
	switch (P)
	{
	case 2:
	{
		numGroups += (group[1] + 1) / 2; 
		break; 
	}
	case 3:
	{
		int match = min(group[1], group[2]);
		int remain = group[1] + group[2] - 2 * match; 
		numGroups += match + (remain + 2) / 3; 
		break; 
	}
	case 4:
	{
		int match13 = min(group[1], group[3]); 
		int remain13 = group[1] + group[3] - 2 * match13;
		int match2 = group[2] / 2; 
		int remain2 = group[2] % 2; 
		numGroups += match13 + match2; 
		if (remain2 == 1)
		{
			numGroups += 1; 
			remain13 -= 2; 
		...
		if (remain13 > 0)
		{
			numGroups += (remain13 + 3) / 4; 
		}
		break; 
	}
	}

	cout << "Case #" << caseNumber << ": " << numGroups << endl;
}
