...
struct problem_t {
  const int maxn = 128;
  const int maxpow = 7;
  const int mask = (1 << maxpow) - 1;

  int n, p;

  unordered_map<LL, int> d;

  int code_state(const vector<int>& st) {
    int state = 0;
    for (int i = 0; i < p; ++i) {
      state = mset(state, i, st[i]);
    }
    return state;
  }

  int mget(int state, int idx) {
    return (state >> (maxpow * idx)) & mask;
  }
  vector<int> decode_state(int state) {
    vector<int> a(p);
    for (int i = 0; i < p; ++i) {
      a[i] = mget(state, i);
    }
  ...
  int mset(int state, int idx, int val) {
    return state ^ ((mget(state, idx) ^ val) << (maxpow * idx));
  }

  
  int go(int state, int rem) {
    auto it = d.emplace((LL)state * maxn + rem, -1);
    if (!it.second) {
    ...
    // if (mget(state, 1) > 0) {
    //   dbg(rem, decode_state(state));
    // }
    res = -1;

    for (int i = 0; i < p; ++i) {
      const int cur = mget(state, i);
      if (cur > 0) {
        const int leftovers = (p - (2 * p + (i - rem)) % p) % p;
        assert(leftovers >= 0);
        const int fresh = go(mset(state, i, cur - 1), leftovers);
        if (res == -1 || res > fresh) {
          res = fresh;
        }
      }
