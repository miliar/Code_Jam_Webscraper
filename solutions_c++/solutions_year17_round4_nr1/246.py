...
void __main() {
	...
	for(int i=0 ; i<=calc[1] ; i++)
		for(int j=0 ; j<=calc[2] ; j++)
			for(int k=0 ; k<=calc[3] ; k++) {
				int s=i*1+j*2+k*3 ;
				bool ok= (s%p==0) ;
				t[i+1][j][k] = max(t[i+1][j][k], t[i][j][k]+ok) ;
				t[i][j+1][k] = max(t[i][j+1][k], t[i][j][k]+ok) ;
				t[i][j][k+1] = max(t[i][j][k+1], t[i][j][k]+ok) ;
			}
	cout << calc[0] + t[calc[1]][calc[2]][calc[3]] << endl ;
}
