...
public:
  ...
  void Run(uint32 T) {
    auto input = ReadSequence<uint32>(std::cin, N);
    auto groups = map(input, [this](uint32 n) { return n % P; });
    auto groups_filtered = as_vector(filter(groups, [](uint32 n) { return n != 0l; }));

    uint32 result = (N - groups_filtered.size());
    std::map<uint32, uint32> counts;
    for (auto elem: groups_filtered) {
      counts[elem]++;
    }

    if (P == 2) {
      result += ceiling_divide<uint32>(counts[1], 2);
    }
    else if (P == 3) {
      auto pairs = std::min(counts[1], counts[2]);
      result += pairs;
      counts[1] -= pairs;
      counts[2] -= pairs;
      result += ceiling_divide<uint32>(counts[1], 3);
      result += ceiling_divide<uint32>(counts[2], 3);
    }
    else if (P == 4) {
      auto pairs1 = std::min(counts[1], counts[3]);
      auto pairs2 = counts[2] / 2;
      result += pairs1;
      result += pairs2;
      ...
      auto rest_count = std::max(counts[1], counts[3]);
      auto triples = rest_count / 3;
      rest_count -= 3 * triples;
      result += triples;
