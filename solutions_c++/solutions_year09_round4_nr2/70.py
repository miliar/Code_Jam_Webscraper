...
int dp[50][6][3][SHIFT(12)];
bool rock[51][6];
int _T,R,C,F;

enum {RIGHT,LEFT,DOWN};

int solve(int cr, int cc, int last, int mask){
   // cout << "SOLVE: " << cr << " " << cc << " " << last << " " << bitset<12>(mask) << endl;
   if(cr == R - 1) return 0;
   int &ret = dp[cr][cc][last][mask]; if(ret!=-1) return ret; ret = INF;

   if(!rock[cr+1][cc] || mask & SHIFT(cc)){
      int fall = 1, ncr = cr+1;
      while(!rock[ncr+1][cc]) ncr++, fall++;

      if(fall > F) ret = INF;
      else ret = solve(ncr,cc,DOWN,(fall==1)?(mask<<6)&(SHIFT(12)-1):0);
   }
   else {

     if(cc+1<C && (!rock[cr][cc+1] || (mask & SHIFT(6+cc+1)))){
      // cout << "CAN MOVE RIGHT from " << cr << " " << cc << endl;
      if(last != LEFT) ret = min(ret,solve(cr,cc+1,RIGHT,mask));
      if(rock[cr+1][cc+1] && (mask & SHIFT(cc+1))==0) ret = min(ret,1 + solve(cr,cc,DOWN,mask | SHIFT(cc+1)));
     }

     if(cc-1>=0 && (!rock[cr][cc-1] || (mask & SHIFT(6+cc-1)))){
      // cout << "CAN MOVE LEFT from " << cr << " " << cc << endl;
      if(last != RIGHT) ret = min(ret,solve(cr,cc-1,LEFT,mask));
      if(rock[cr+1][cc-1] && (mask & SHIFT(cc-1))==0) ret = min(ret,1 + solve(cr,cc,DOWN,mask | SHIFT(cc-1)));
     }
   }
   // cout << "SOLVE: " << cr << " " << cc << " " << last << " " << bitset<12>(mask) << " -> " << ret << endl;
   return ret;
}
...
int main(){
  ...
  FOR(i,0,_T){
    cout << "Case #" << (i+1) << ": ";
    MEMSET(rock,true); MEMSET(dp,-1);
    cin >> R >> C >> F;
    char ch;
    FOR(j,0,R) FOR(k,0,C){ cin >> ch; if(ch == '.') rock[j][k] = false;}
    if(C <= 6){  
      int ret = solve(0,0,DOWN,0);
      if(ret != INF) cout << "Yes " << ret;
      else cout << "No";
    }
    cout << endl;
  }
