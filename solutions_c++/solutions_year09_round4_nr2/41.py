...
int go(int r, int c, int cur, int below)
{
	if (c<0 || c>=m || (cur&(1<<c)))
		return INF;
	if (!(below&(1<<c)))
	...
	if (r==n-1)
		return 0;
	int &res = d[r][cur][below][c];
	if (res!=-1)
		return res;
	res = INF;
	res = min(res, go(r, c-1, cur, below));
	res = min(res, go(r, c+1, cur, below));
	if (c>0 && !(cur&(1<<(c-1))) && (below&(1<<(c-1))))
		res = min(res, go(r, c, cur, (below&~(1<<(c-1))))+1);
	if (c<m-1 && !(cur&(1<<(c+1))) && (below&(1<<(c+1))))
		res = min(res, go(r, c, cur, (below&~(1<<(c+1))))+1);
	return res;
}
...
/*		REP(i,N) REP(j,N) REP(k,N)
			d[i][j][k]=INF;
		REP(i,n)
			REP(right,m) REP(left,right+1) if (d[i][left][right] < INF)
			{

			}*/
		int res = go(0,0,masks[0],masks[1]);
		printf("Case #%d: ",tst+1);
		if (res>=INF)
			printf("No\n");
		else
