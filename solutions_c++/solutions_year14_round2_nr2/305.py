...
{
    //print3(a,x,y);
    if(a&&x==1&&y==0) return -1;
    int val=x^y;
    return a&(!val);
}

LL dprec(int in,int aa,int bb,int kk)
{
    //debug(in,aa,bb,kk);
    if(in<0) return 1;
    LL &ret=dp[in][aa][bb][kk];
    if(ret!=-1) return ret;
    ret=0;
    int aval=0;
    int bval=0;
    ...
    for(int i=0;i<=1;i++)
    {
        if(valid(aa,i,aval)==-1) continue;
        for(int j=0;j<=1;j++)
        {
            if(valid(bb,j,bval)==-1) continue;
            if(valid(kk,(i&j),kval)==-1) continue;
            ret+=dprec(in-1,valid(aa,i,aval),valid(bb,j,bval),valid(kk,(i&j),kval));
        }
    }
   // debug(in,aa,bb,kk,ret);
    return ret;
}
