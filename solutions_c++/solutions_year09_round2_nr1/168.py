	...
	for(int q = 0; q < cases; q++)
	{
		...
		for(int i = 0; i < A; i++)
		{
			...
			while(true)
			{
				...
				string temporary = "";
				while(vs[curLine][curChar] != ' ' && vs[curLine][curChar] != ')')
				{
					temporary += vs[curLine][curChar];
					curChar++;
				}
				result *= getDouble(temporary);

				while(vs[curLine][curChar] == ' ')
					curChar++;

				temporary = "";
				while(curChar != vs[curLine].size() && vs[curLine][curChar] != ')')
				{
					temporary += vs[curLine][curChar];
					curChar++;
				}
				...
				if(!found)
				{
					...
					while(true)
					{
						for(int j = 0; j < vs[curLine].size(); j++)
							if(vs[curLine][j] == '(')
								numBr++;
							else
							if(vs[curLine][j] == ')')
