...
struct decision{
  ...
  void read(string &str, int &pos){
     // cout << "READ: " << str.substr(pos) << endl;
     while(str[pos]!='(') pos++; pos++; while(str[pos]==' ') pos++;
     string w; while(str[pos]!=' ' && str[pos]!=')') { w += str[pos]; pos++;}
     weight = atof(w.c_str());
     while(str[pos]==' ') pos++; if(str[pos] == ')') { pos++; return;}
     while(str[pos]!=' ') { feature += str[pos]; pos++;}
     while(str[pos]!='(') pos++;
     left = new decision; left->read(str,pos);
     while(str[pos]==' ') pos++;
     right = new decision; right->read(str,pos);
     while(str[pos]!=')') pos++; pos++;

     // cout << "BUILT: " << weight << " " << feature << " -> " << left->weight << " " << right->weight << endl;
  }

  long double solve(long double cur){
     if(left == NULL || right == NULL) return cur * weight;
     else if(features.count(feature)) return left->solve(cur * weight);
     else return right->solve(cur*weight);
  }
...
int _T, L, N, pos; string line, animal, feat;
...
int main(){
  ...
  FOR(i,0,_T){
    ...
    FOR(j,0,L) {getline(cin,line); str = str + " " + line;}
    cin >> N; pos = 0;
    decision *root = new decision; root->read(str,pos);

    FOR(j,0,N){
      cin >> animal; features.clear();
      cin >> L; FOR(k,0,L) { cin >> feat; features.insert(feat);}
//      ITER(it,features) cout << (*it) << " "; cout << endl;
      cout << root->solve(1.0) << endl;	
    }
