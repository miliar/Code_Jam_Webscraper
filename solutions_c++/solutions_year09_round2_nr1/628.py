...
void createTree(int node,vs param)
{
     //outAny(param);
     param.erase(param.begin());
     tree[node]=toDouble(param[0]);
     param.erase(param.begin());
     if(param[0]==")")return;
     if(param[0]!="(")
     {
     ...
     while(k < size(param))
     {
             ...
             if(val==0)
             {
                       vs tmp2(param.begin(),param.begin()+k+1);
                       vs tmp3(param.begin()+k+1 , param.end());
                       createTree(node*2,tmp2);
                       createTree(node*2+1,tmp3);
                       return;
             }
             k++;
     }
...
void searchTree(int node)
{
     ...
     //searchTree(node*2+1);
     prob *= tree[node];
     if(cpresent(feature,data[node]))searchTree(node*2);
     else searchTree(node*2+1);
}
...
int main()
{
    ...
    For(kase,t)
    {
               ...
               string tmp = "";
               gets(str);
               For(i,l){gets(str); tmp += string(str);}
               ...
               For(i,q)
               {
                       gets(str);
                       prob = 1.0;
                       split(string(str) , tmp2);
                       feature = vs(tmp2.begin()+2,tmp2.end());
                       f = size(feature);
                       searchTree(1);
                       printf("%.7f\n",prob);
               }
