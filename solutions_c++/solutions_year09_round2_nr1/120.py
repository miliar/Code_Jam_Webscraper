...
static int max_procs = 4, first_case = 0, last_case = -1;
...
struct Node
{
    Node(double w) : weight(w), feature(), children(0), left(0), right(0) { };
...
string next_token(string &input, int &pos)
{
    if (pos == SZ(input)) { cin >> input; pos = 0; }
    if (input[pos] == '(') { ++pos; return "("; }
    if (input[pos] == ')') { ++pos; return ")"; }
    int sep = pos + 1;
    while (sep < SZ(input) && input[sep] != '(' && input[sep] != ')') ++sep;
    std::string res = string(input.begin() + pos, input.begin() + sep);
    pos = sep;
    return res;
}
...
RealCase::RealCase()
{
    ...
    int L;
    cin >> L; // number of lines -- unused
    vector<Node*> st;
    do {
        string token = next_token(input, pos);
        if (token == "(")
        {
            double w;
            token = next_token(input, pos);
            if (sscanf(token.c_str(), "%lf", &w) != 1) assert(0);
            Node *node = new Node(w);
            if (!st.empty()) {
                Node *parent = st.back();
                assert(!parent->feature.empty());
        ...
        {
            assert(st.back()->feature.empty());
            st.back()->feature = token;
        }
    } while (!st.empty());
...
double calc(Node *node, vector<string> &features)
{
    SORT(features);
    double p = 1;
    for (;;)
    {
        ...
        assert(!node->feature.empty());
        IT(features) it = lower_bound(ALL(features), node->feature);
        bool found = (it != features.end()) && (*it == node->feature);
        node = found ? node->left : node->right;
    }
    return p;
}
...
{
    // print_tree(root);
    FORIT(it, F) result.add(STR(calc(root, *it)));
}
