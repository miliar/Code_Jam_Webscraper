...
#define NAME "A-large"
int p;
#define skip while (buf[p]<=' '&&buf[p]!=0) p++;
...
public:
	...
	Node()
	{
		...
		p++; // ((
		skip;
		sscanf(buf+p, "%lf", &prob);
		while (buf[p]>='0'&&buf[p]<='9'||buf[p]=='.') p++;
		skip;
		if (buf[p]!=')')
		{
			feature = "";
			while (buf[p]>='a'&&buf[p]<='z' || buf[p]>='A'&&buf[p]<='Z')
				feature.push_back(buf[p++]);
			skip;
			a[0] = new Node();
			a[1] = new Node();
		}
	...
	REP(tst,tests)
	{
		...
		REP(i,n)
		{
			...
			while (cur != NULL)
			{
				...
				if (cur->a[0] != NULL)
				{
					cur = cur->a[features.find(cur->feature)!=features.end()?0:1];
				}
				else
					cur=NULL;
			}
