...
long long int Xmods[1000002];
long long int Ymods[1000002];
long long int cumulativemodvals[1000002][11];
...
int main(int argc,char *argv[])
{
	...
	for(long long int i=1;i<=numtestcases;i++)
	{
		...
		for(long long int j=1;j<=n;j++)
			for(long long int k=j+1;k<=n;k++)
			{
				wantedx=(6-Xmods[j]-Xmods[k])%3;
				wantedy=(6-Ymods[j]-Ymods[k])%3;
				count+=cumulativemodvals[k][wantedx*3+wantedy];
