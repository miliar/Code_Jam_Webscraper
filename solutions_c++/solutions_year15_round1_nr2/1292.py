...
int main(void){
    ...
    cin>>m[i];
    lcmVal=getLcm(lcmVal,m[i]);
    busyfor[i]=0;
    }
    long long roundVal=0;
    for(long long i=0;i<B;i++){
        roundVal+=lcmVal/m[i];
    }
    N=N%roundVal;
    if(N==0)
        N=roundVal;
    long long curBarber=0;
    for(long long i=0;i<N;i++){
        long long minTime=100000000000000;
        bool freebarbar=false;
        for(long j=0;j<B;j++){
            if(busyfor[j]==0){
                busyfor[j]=m[j];
                freebarbar=true;
                ...
                minTime=min(minTime,busyfor[j]);
        }
        if(!freebarbar){
            for(long j=0;j<B;j++){
                if(busyfor[j]==minTime&&!freebarbar){
                    busyfor[j]=m[j];
                    curBarber=j+1;
                    freebarbar=true;
                }
