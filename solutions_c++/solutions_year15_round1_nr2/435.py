...
class solver
{
	...
	vector<int> freebarber;

public:
	solver()
	{
		cin >> b >> n;
		B = vector<long long>(b);
		for (int i = 0; i < b; i++)
			cin >> B[i];
		n;
	}

	long long clientsServed(long long time)
	{
		long long clients = 0;
		freebarber.clear();
		for (int i = 0; i < b; i++)
		{
			clients += (time - 1ll) / B[i] + 1ll;
			long long timeago = time % B[i];
			if (timeago == 0) freebarber.push_back(i);
		}
	...
	{
		long long top = 200000000000001ll;
		long long bottom = 0;

		while (top != bottom)
		{
			long long middle = (top + bottom) / 2 + 1;
			if (clientsServed(middle) >= n) top = middle - 1;
			else bottom = middle; 
		}

		long long cs = clientsServed(bottom);
		n -= cs + 1;
		S << *(freebarber.begin() + n) + 1 ;
	};
};
