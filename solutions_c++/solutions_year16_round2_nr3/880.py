...
bool isok(int mask)
{
    set<pair<int,int> > real,fake,nfake;
    pair<int,int> can;
    for(int i=0;i<n;i++)
    {
        if((mask>>i)&1)
    ...
    for(int i=0;i<n;i++)
    {
        ...
        for(int j=0;j<n;j++)
        {
            ...
            if(mat[i][j]) continue;
            can=mp(val[i].fs,val[j].sc);
            if(real.count(can)) continue;
            if(fake.count(can))
                nfake.insert(can);
        }
    }
    return SZ(fake)==SZ(nfake);
}
...
int main()
{
    ...
    while(t--)
    {
        ...
        {
            cin>>a>>b;
            if(mpp.count(a)==0) mpp[a]=id++;
            if(mpp.count(b)==0) mpp[b]=id++;
            val.psb(mp(mpp[a],mpp[b]));
        }
        int lim=(1<<n);
        int res=0;
        clrall(mat,0);
        for(int i=0;i<n;i++)
        {
            for(int j=0;j<n;j++)
            {
                if(val[i].fs==val[i].sc||i==j) mat[i][j]=true;
            }
        }
        for(int i=0;i<(lim);i++)
        {
