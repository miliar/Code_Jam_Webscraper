...
void solve()
{
	...
	for(int i = 1; i <= 2*N-1; i++) C.insert(i), D.insert(i);
	for(int i = 1; i <= M; i++){
		scanf("%s%d%d", buf, &a, &b);
		if( buf[0] == 'x' || buf[0] == 'o'){
			A.erase(a); B.erase(b);
		...
		if( buf[0] == '+' || buf[0] == 'o'){
			C.erase(a+b-1); D.erase(a-b+N);
		}
		res[a][b] = ini[a][b] = buf[0];
	}
	while(A.size()){
		a = *A.begin(), b = *B.begin();
		up_mult(res[a][b]);
	...
	for(int i = 1; i <= N; i++){
		for(int j = 1; j <= N; j++){
			if( res[i][j] == '+' || res[i][j] == 'x' ) ans++;
			else if( res[i][j] == 'o' ) ans += 2;
			if( res[i][j] != ini[i][j] ) P.emplace_back(i, j);
		}
	}
	printf("%d %d\n", ans, P.size());
	for(pii c : P){
		printf("%c %d %d\n", res[c.first][c.second], c.first, c.second);
	}
	for(int i = 1; i <= N*2; i++) L[i].clear(), R[i] = vst[i] = 0;
}
