...
class Matching {
    ...
    bool bfs(const GraphU& graph) {
        dist.assign(graph.size(), -1);
        int* tail = queue.data();
        for (int i = 0; i < graph.size(); ++i)
            if (match1[i] == -1) dist[*tail++ = i] = 1;
        for (int* v = queue.data(); v != tail; ++v)
            for (int u : graph[*v]) {
                int mu = match2[u];
                if (mu == -1) return true;
    ...
    template <class GraphU>
    bool dfs(const GraphU& graph, int v) {
        for (auto u = graph[v].begin() + queue[v]; u != graph[v].end();
             ++u, queue[v] = static_cast<int>(u - graph[v].begin())) {
            int& mu = match2[*u];
    ...
    vector<int> dist, queue;

public:
    template <class GraphU>
    int maxMatching(const GraphU& graph) {
        int n = graph.size();
        match1.assign(n, -1);
        match2.assign(graph.size2(), -1);
        queue.resize(n);
        while (bfs(graph)) {
            queue.assign(n, 0);
            for (int i = 0; i < n; ++i)
                if (match1[i] == -1) dfs(graph, i);
        }
        return n - static_cast<int>(count(match1.begin(), match1.end(), -1));
    }
...
public:
    ...
    void solve() {
        REP(i, n) copy_n(a[i], n, b[i]);
        vector<bool> r(n), c(n);
        vector<bool> dp(2*n), dm(2*n);
        REP(i, n) REP(j, n) {
                if (a[i][j] == 'x' || a[i][j] == 'o') {
                    r[i] = c[j] = true;
                }
                if (a[i][j] == '+' || a[i][j] == 'o') {
                    dp[i + j] = dm[i - j + n-1] = true;
                }
            }
        int x = 0, y = 0;
        while (true) {
        ...
        if (true) {
            GraphUL<true> graph(2 * n, 2 * n);
            REP(i, n) REP(j, n) {
                    int p = i + j;
                    int m = i - j + n - 1;
                    if (!dp[p] && !dm[m]) graph.addEdge(p, m);
                }
            Matching matching;
            matching.maxMatching(graph);
            REP(p, 2 * n) if (matching.match1[p] != -1) {
                    int m = matching.match1[p];
                    int x = (p + m - (n - 1)) / 2;
                    int y = (p - m + (n - 1)) / 2;
                    if (b[x][y] == 'x') b[x][y] = 'o';
                    else b[x][y] = '+';
        ...
        REP(i, n) REP(j, n) {
                if (b[i][j] != '.') ++ans;
                if (b[i][j] == 'o') ++ans;
                if (b[i][j] != a[i][j]) diff.eb(i, j);
            }
        writeln(ans, sz(diff));
        for (auto p : diff) writeln(b[p.X][p.Y], ' ', p.X + 1, p.Y + 1);
        output->flush();
    }
...
int main() {
    //randomTest();
    //maxTest();
    input.reset(new InputFile("d-large.in"));  // -small-attempt0.in
    output.reset(new OutputFile("output.txt"));  // -large
    logs.reset(new OutputFile(stderr));
    logs->write(setPrecision(4));
    testIndex = 0;
