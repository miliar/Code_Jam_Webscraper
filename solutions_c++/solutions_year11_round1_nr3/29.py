...
struct card{
   int c, s, t;
   bool operator<(const card &rhs) const { return s > rhs.s; }
};

vector<card> hand, deck;
void propagate(vector<card> &cur, int &turns, int &score, int &cind){
   if(turns == 0) return;
   FOR(i,0,cur.size()) if(cur[i].t > 0){ 
	turns += cur[i].t-1; score += cur[i].s;
	while(cur[i].c > 0 && cind < deck.size()) { cur.push_back(deck[cind++]), cur[i].c--;}
        cur[i--] = cur.back(); cur.pop_back();
   }
}

int solve(int reach){
   vector<card> cur = hand;
   int turns = 1, cind = 0, score = 0;
   propagate(cur,turns,score,cind);
   sort(BEGEND(cur));

   while(cind < reach && turns > 0){
     // cout << turns << " " << score << " " << cind << endl;
      bool any = false;
      FOR(i,0,cur.size()) if(cur[i].c == 1){
	 any = true;
         turns--; score += cur[i].s; cur[i--] = deck[cind++];
	 propagate(cur,turns,score,cind);
	 sort(BEGEND(cur)); break;
      }
      if(!any) break;
   }
//   cout << turns << " " << score << " " << cind << endl;
   FOR(i,0,min(turns,(int)cur.size())) score += cur[i].s;
   return score;
}
int N, M;
int _T;
int main(){
  ...
  FOR(i,0,_T){
    cout << "Case #" << (i+1) << ": ";
    cin >> N; hand.resize(N);
    FOR(j,0,N) cin >> hand[j].c >> hand[j].s >> hand[j].t;
    cin >> M; deck.resize(M);
    FOR(j,0,M) cin >> deck[j].c >> deck[j].s >> deck[j].t;
  //  cerr << "NM: " << N << " " << M << endl;
    int best = 0; FOR(j,0,deck.size()+1) best = max(best,solve(j));
    cout << best;
    cout << endl;
  }
  return 0;
