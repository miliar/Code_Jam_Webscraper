...
bool like_normal[2048][2048];
int like_malted[2048];
int solution[2048];
...
{ 
  ...
  REP(ctest, ntest)
  {
    ...
    cin >> flavors >> customers;

    MSET0(like_normal);
    MSET(like_malted, -1);
    
    REP(customer, customers)
    {
      int t;
      cin >> t;
      REP(i, t)
      {
        ...
        if (malted)
          like_malted[customer] = flavor;
        else
          like_normal[customer][flavor] = true;
      }
    }
    ...
    while (true)
    {
      ...
      REP(customer, customers)
      {
        // is he already satisfied?
        if (like_malted[customer] != -1 && solution[like_malted[customer]] == 1)
          continue;

        // does he have any normal preference?
        bool likes_some_normal = false;
        REP(flavor, flavors)
          likes_some_normal |= like_normal[customer][flavor];

        if (likes_some_normal)
          continue;

        // does he not have any malted preference?
        if (like_malted[customer] == -1)
        {
          contradiction = true;
          break;
        }

        // he does! do the malting
        changes = true;
        int malt = like_malted[customer];
        solution[malt] = 1;
        REP(customer1, customers)
          like_normal[customer1][malt] = false;
      }

      if (!changes || contradiction)
        break;
    }
