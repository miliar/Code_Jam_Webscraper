...
char *txt[] = { "ZERO", "ONE", "TWO", "THREE", "FOUR", "FIVE", "SIX", "SEVEN", "EIGHT", "NINE" };
char buffer[20000];
int letters[200];
...
int main()
{
	...
	FOR1(test_number, T)
	{
		...
		*/
		...
		while (letters['Z'] > 0)
		{
			int number_len = strlen(txt[0]);
			FOR(j, number_len) --letters[txt[0][j]];
			++numbers[0];
		}
		while (letters['W'] > 0)
		{
			int number_len = strlen(txt[2]);
			FOR(j, number_len) --letters[txt[2][j]];
			++numbers[2];
		}
		while (letters['X'] > 0)
		{
			int number_len = strlen(txt[6]);
			FOR(j, number_len) --letters[txt[6][j]];
			++numbers[6];
		}
		while (letters['G'] > 0)
		{
			int number_len = strlen(txt[8]);
			FOR(j, number_len) --letters[txt[8][j]];
			++numbers[8];
		}
		while (letters['U'] > 0)
		{
			int number_len = strlen(txt[4]);
			FOR(j, number_len) --letters[txt[4][j]];
			++numbers[4];
		}
		while (letters['S'] > 0)
		{
			int number_len = strlen(txt[7]);
			FOR(j, number_len) --letters[txt[7][j]];
			++numbers[7];
		}
		while (letters['T'] > 0)
		{
			int number_len = strlen(txt[3]);
			FOR(j, number_len) --letters[txt[3][j]];
			++numbers[3];
		}
		while (letters['F'] > 0)
		{
			int number_len = strlen(txt[5]);
			FOR(j, number_len) --letters[txt[5][j]];
			++numbers[5];
		}
		while (letters['I'] > 0)
		{
			int number_len = strlen(txt[9]);
			FOR(j, number_len) --letters[txt[9][j]];
			++numbers[9];
		}
		while (letters['O'] > 0)
		{
			int number_len = strlen(txt[1]);
			FOR(j, number_len) --letters[txt[1][j]];
			++numbers[1];
		}
