...
int main(int argc, char** argv)
{
    ...
    int cases;
    cin>>cases;
    string magicString = "ZWUXGOSRVI";
    string magicNum = "0246817359";
    string magicSequence[] = {"ZERO", "TWO", "FOUR", "SIX", "EIGHT", "ONE", "SEVEN", "THREE", "FIVE", "NINE"};
    ...
    for(int eachCase = 0; eachCase<cases; eachCase++){
        ...
        string ans = "";
        for(int i=0;i<10;i++){
            char c = magicString[i];
            //cout<<I(c)<<" ";
            while(arr[I(c)] > 0){
                for(int j=0;j<magicSequence[i].size();j++){
                    arr[I(magicSequence[i][j])]--;
                }
                ans += " ";
                ans[ans.size()-1] = magicNum[i];
            }
        }
