...
string a[] = {"ZERO","TWO", "FOUR", "SIX", "EIGHT", "FIVE", "THREE","SEVEN", "NINE", "ONE",};
int c[] = {0,2,4,6,8,5,3,7,9,1};
int b[12][30];
int cnt[26];
char s[MX];
    ...
    while(t--){
        ...
        //while( len ){
            for (int i=0;i<10;i++){
                ...
                for (int j=0;j<26;j++){
                    if (!b[i][j])   continue;
                    //cout << char('A' + j) << ": " << cnt[j] << "," << b[i][j] << endl;
                    mn = min( cnt[ j ]/b[i][ j ],mn );
                }
                
                for (int j=0;j<26;j++){
                    if (!b[i][j])   continue;
                    
                    //cout << "DEL -> " << mn*b[i][ a[i][j]-'A' ] << ", " << 
                    cnt[j] -= mn*b[i][ j ];
                }
        ...
        P("Case #%d: ",++cs);
        for (int i=0;i<ans.size();i++)  P("%d",ans[i]); NL;
