...
int main()
{
    ...
    Write;
    #endif // MAHDI
    vector<string> val{"ZERO", "TWO", "FOUR", "SIX", "EIGHT", "THREE", "ONE", "FIVE", "SEVEN", "NINE"};
    vector<int> ival{0,2,4,6,8,3,1,5,7,9};
    string s;
    int t,cas=0;
    int cnt[100];
    cin>>t;
    while(t--)
    {
        ...
        for(int i=0;i<10;i++)
        {
            ...
            for(int j=0;j<SZ(val[i]);j++)
            {
                mn=min(mn,lol[val[i][j]]/tmp[val[i][j]]);
//                deb(val[i][j],lol[val[i][j]],tmp[val[i][j]]);
            }
            if(mn==1<<20) continue;
//            deb(ival[i],mn);
            for(int j=0;j<SZ(val[i]);j++)
