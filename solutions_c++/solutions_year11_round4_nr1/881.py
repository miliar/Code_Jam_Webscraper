...
int main() {
    ...
    for (auto TEST: R(1, testCount + 1)) {
        ...
        ws.push_back(make_pair(0., rest));
        sort(ws.begin(), ws.end());
        double res = 0;
        for (int i: R(ws.size())) {
            if (t > 0) {
                double sp = ws[i].first + RR;
                double tt = ws[i].second / sp;
                if (tt < t) {
                    res += tt;
                    t -= tt;
                    continue;
                } else {
                    double rest = ws[i].second - sp * t;
                    res += t;
                    res += rest / (ws[i].first + S);
                    t = -0;
                    continue;
                }
            } else {
                res += ws[i].second / (ws[i].first + S);
            }
        }
