...
double X, S, R, t, N;
int i, j, k;
double B[MAX_W], E[MAX_W], w[MAX_W];
vector<pair<double, int> > corridor;


double run_time(double start, double end, double addi_s, double cur_t)
{
	double t1;
	if (cur_t >= t)
		return (end-start)/(S+addi_s);
	
	t1 = (end-start)/(addi_s+R);
	if (t1+cur_t < t)
		return t1;
	else {
		t1=t-cur_t;
		t1 += (end-start-(addi_s+R)*t1)/(addi_s+S);
		return t1;
	}
}
...
double solve()
{
	...
	{
		cin >> B[i] >> E[i] >> w[i];
		corridor.push_back(make_pair(w[i],(E[i]-B[i])));
		left -= (E[i]-B[i]);
	}
	corridor.push_back(make_pair(0, left));
	sort(corridor.begin(), corridor.end());
	double ts(0), t2;
	fi(N+1)
	{
		if (ts >= t)
			ts += corridor[i].second/(corridor[i].first+S);
		else {
			t2 = corridor[i].second/(corridor[i].first+R);
			if (t2+ts <= t)
				ts+=t2;
			else {
				t2 = t-ts;
				ts = t;
				ts += (corridor[i].second-(corridor[i].first+R)*t2)/(corridor[i].first+S);
			}
		}

		//ts += run_time(corridor[i]);
	}
	return ts;
...
/*
	...
	while (dis < X) {
		if (ci < N)
		{
			addi = w[corridor[ci].second];
			k = corridor[ci].second;
			
			t2 = run_time(dis, B[k], 0, ts);
			ts += t2;
			t2 = run_time(B[k], E[k], addi, ts);
			ts += t2;
			dis = E[k];
			ci++;
		}
		else {
			t2 = run_time(dis, X, 0, ts);
			ts += t2;
			dis = X;
		}
	}
