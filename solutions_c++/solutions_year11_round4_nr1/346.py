...
cout.precision(15);
    ...
    REP(tc, T){
        ...
        W.pb(make_pair(PII(0,W[0].X.X), 0));
        
        REP(i,N-1)
	        W.pb(make_pair(PII(W[i].X.Y, W[i+1].X.X), 0));
        
        W.pb(make_pair(PII(W[N-1].X.Y, X), 0));
        ...
        FORV(i, W){
			double d = min<double>((R+W[i].Y)*t, W[i].X.Y - W[i].X.X);
			t -= d / (R+W[i].Y);
			ans += d / (R+W[i].Y);
			ans += (W[i].X.Y - W[i].X.X - d) / (S+W[i].Y);
		}
