...
int main( int argc, char* argv[] ) {
	...
	for(int testCase=1;testCase<=testCases;testCase++){
		...
		int pos=0;
		rep(i,N){
			segments.pb(mp(0,B[i]-pos));
			segments.pb(mp(w[i],E[i]-B[i]));
			pos=E[i];
		}

		segments.pb(mp(0,X-pos));

		srt(segments);

		double res=0;
		reps(i,segments){
			double len=segments[i].second;
			int w=segments[i].first;
			double time=len/(R+w);
			checkmin(time,t);
			t-=time;
			res+=time;
		...
		cout << "Case #" << testCase << ": " << fixed << setprecision(14) << res << endl;
		cout.flush();
	}
