...
void run() {
    ...
    for (int i = 0; i < N; i++) {
        int B, E, w;
        std::cin >> B >> E >> w;
        vi[i] = make_pair(w + S, E-B);
        total -= (E-B);
    }
    vi.push_back(make_pair(S, total));
    ...
    for (int i = 0; i < vi.size(); i++) {
        if (time <= 0.0) {
            result += (double)vi[i].second / vi[i].first;
        } else {
            double time_required = (double)vi[i].second / (vi[i].first + dif);
            if (time_required <= time) {
                result += time_required;
                time -= time_required;
            } else {
                result += time;
                double dist2 = (double)vi[i].second - (vi[i].first + dif) * time;
                time = 0.0;
                result += dist2 / vi[i].first;
            }
        }
    }
    printf("%.9f", result);
