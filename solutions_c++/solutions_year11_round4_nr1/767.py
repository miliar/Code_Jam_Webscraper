...
bool sort_by_start(const Walk &a, const Walk &b)
{
    return a.B < b.B;
}

bool sort_by_speed(const Walk &a, const Walk &b)
{
    return a.W < b.W;
}
...
RealCase::RealCase()
{
    cin >> X >> S >> R >> T >> N;
    walks.resize(N);
    REP(n, N) cin >> walks[n].B >> walks[n].E >> walks[n].W;
}
...
void RealCase::solve(Result &result)
{
    ...
    {
        double last = 0;
        sort(ALL(walks), sort_by_start);
        FORIT(it, walks) {
            if (it->B > last) all.push_back((Walk){ last, it->B, 0 });
            all.push_back(*it);
            last = it->E;
        }
        if (last != X) all.push_back((Walk){ last, X, 0 });
    }
    sort(ALL(all), sort_by_speed);
    double res = 0.0;
    FORIT(it, all) {
        double length = it->E - it->B, run = min(T, length/(R + it->W));
        res += run + (length - (R + it->W)*run)/(S + it->W);
        T -= run;
    }
    result.set(STR(res));
}
