...
{
	int C;
	typedef MULTIMAP<int, int> SPEED_AND_LENGTH;
	fscanf(fin, "%d", &C);
	for (int i=0; i<C; ++i)
	{
		SPEED_AND_LENGTH Corridors;
		int X, iS, iR, it, N;
		fscanf(fin, "%d %d %d %d %d", &X, &iS, &iR, &it, &N);
		double S=iS;
		double R=iR;
		double t=it;
		int iLen=X;
		for (int j=0; j<N; ++j)
		{
			...
			Corridors.insert(std::make_pair(w, E-B));
			iLen-=(E-B);
		}
		Corridors.insert(std::make_pair(0, iLen));

		double tTotal=0;
		foreach(SPEED_AND_LENGTH, Corridors, itCorridor)
		{
			double len=itCorridor->second;
			double speed=itCorridor->first;
			double tRun = min((double)t, len/(R+speed));
			double tWalk = (len-(tRun*(R+speed)))/(speed+S);
			t -= tRun;
			tTotal += tRun+tWalk;
		}
