...
int main(){
  ...
  FOR(i,0,_T){
	...
	int b, e, s;
	cin >> b >> e >> s;
	walkways.push_back(make_pair(s,e-b));
	X -= (e-b);
    }
    walkways.push_back(make_pair(0,X));
    sort(BEGEND(walkways));
//    printArray(walkways);

    // M  m/s  (R+w) / S
    double ret = 0;
    FOR(w,0,N+1){
//	cout << "BEFORE: " << walkways[w] << " -> " << ret << " " << tS << endl;
	double cX = walkways[w].second;
	if(tS > eps){
	    double d = min(cX,tS * (R+walkways[w].first));
	    cX -= d; ret += d / (R+walkways[w].first);
	    tS -= d / (R + walkways[w].first);
	}
	if(cX > eps) ret += cX / (S+walkways[w].first);
    }
    printf("%.10f",ret);
    cout << endl;
  }
