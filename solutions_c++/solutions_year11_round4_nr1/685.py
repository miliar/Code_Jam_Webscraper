...
int main(){
	...
	while(tt--){
		...
		double ans=0.0;
		REP(i,sz(v)){
			if(double(v[i].d)<=double(v[i].v+R-S)*rem_time){
				ans+=double(v[i].d)/double(v[i].v+R-S);
				rem_time-=double(v[i].d)/double(v[i].v+R-S);
			}
			else{
				double len=v[i].d-(v[i].v+R-S)*rem_time;
				ans+=rem_time;
				rem_time=0;
				ans+=len/double(v[i].v);
			}
		}
		printf("Case #%d: %.9f\n",kase,ans);
	}
