...
////////////////////////// Solution ///////////////////////////
bool ok(const vector<string>& grid, int tl, int tr, int k, int D) {
  int cx = (tl + (k - 1) / 2) * 2;
  int cy = (tr + (k - 1) / 2) * 2;
  if (k % 2 == 0) {
    ++cx;
...
try {
  ...
  WhileTests() {
    ...
    InInt3(R, C , D);
    InVectorString(grid, R);
    int best = 2;
    for (int i = 0; i < R; ++i) for (int j = 0; j < C; ++j) {
      int k = min(R - i, C - j);
      if (k < 3) continue;
      for (; k > best; --k) if (ok(grid, i, j, k, D)) {
        best = k;
      }
    }
    if (best < 3) printf("IMPOSSIBLE\n");
    else printf("%d\n", best);
  }
  ZyxGCJ_BackUpFiles();
  return 0;
