...
int main(){
	...
	while(t--){
		...
		FOR(i,1,R+1){
			cin>>s;
			REP(j,C)	w[i][j+1]=D+s[j]-'0',wx[i][j+1]=w[i][j+1]*i,wy[i][j+1]=w[i][j+1]*(j+1);
		}
		FOR(i,1,R+1)	FOR(j,1,C+1)	WX[i][j]=WX[i-1][j]+WX[i][j-1]-WX[i-1][j-1]+wx[i][j],WY[i][j]=WY[i-1][j]+WY[i][j-1]-WY[i-1][j-1]+wy[i][j],W[i][j]=W[i-1][j]+W[i][j-1]-W[i-1][j-1]+w[i][j];
		int ans=0;
		FOR(i,1,R+1)	FOR(j,1,C+1)	FOR(len,3,min(R,C)+1){
			if(i+len-1>R||j+len-1>C)	continue;
			double ex=i+len/2,ey=j+len/2;
			if((len%2)==0)	ex-=0.5,ey-=0.5;
			double cx=double(WX[i+len-1][j+len-1]-WX[i-1][j+len-1]-WX[i+len-1][j-1]+WX[i-1][j-1]-wx[i][j]-wx[i][j+len-1]-wx[i+len-1][j]-wx[i+len-1][j+len-1])/double(W[i+len-1][j+len-1]-W[i-1][j+len-1]-W[i+len-1][j-1]+W[i-1][j-1]-w[i][j]-w[i][j+len-1]-w[i+len-1][j]-w[i+len-1][j+len-1]);
			double cy=double(WY[i+len-1][j+len-1]-WY[i-1][j+len-1]-WY[i+len-1][j-1]+WY[i-1][j-1]-wy[i][j]-wy[i][j+len-1]-wy[i+len-1][j]-wy[i+len-1][j+len-1])/double(W[i+len-1][j+len-1]-W[i-1][j+len-1]-W[i+len-1][j-1]+W[i-1][j-1]-w[i][j]-w[i][j+len-1]-w[i+len-1][j]-w[i+len-1][j+len-1]);
			if(ex==cx&&ey==cy)	ans=max(ans,len);
		}
		printf("Case #%d: ",kase);
