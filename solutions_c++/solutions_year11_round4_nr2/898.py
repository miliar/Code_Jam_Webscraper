...
double scoreR[501][501], scoreC[501][501];
...
bool canSolveOdd(int K){
   //cerr << " DOING: " << K << " " << K/2 << endl;
   FOR(r,K,R+1) FOR(c,K,C+1){
	int mR = r-K/2-1, mC = c-K/2-1;
	double rs = 0, cs = 0;
	FOR(rr,r-K,r) FOR(cc,c-K,c) if(!(rr==r-K && cc==c-K || rr==r-K && cc==c-1 || rr==r-1 && cc == c-K || rr==r-1 && cc==c-1)){
	    if(rr < mR) rs += abs(mR-rr) * mass[rr][cc];
	    else        rs -= abs(mR-rr) * mass[rr][cc];

	    if(cc < mC) cs += abs(mC-cc) * mass[rr][cc];
	    else        cs -= abs(mC-cc) * mass[rr][cc];
	}
	if(abs(rs) < eps && abs(cs) < eps) return true;
   }
   return false;
}

bool canSolveEven(int K){
   //cerr << " DOING: " << K << " " << K/2 << endl;
   FOR(r,K,R+1) FOR(c,K,C+1){
	int mR = r-K/2, mC = c-K/2;
	double rs = 0, cs = 0;
	FOR(rr,r-K,r) FOR(cc,c-K,c) if(!(rr==r-K && cc==c-K || rr==r-K && cc==c-1 || rr==r-1 && cc == c-K || rr==r-1 && cc==c-1)){
	    if(rr < mR) rs += (mR-rr-0.5) * mass[rr][cc];
	    else        rs -= (rr-mR+0.5) * mass[rr][cc];
	
	    if(cc < mC) cs += (mC-cc-0.5) * mass[rr][cc];
	    else        cs -= (cc-mC+0.5) * mass[rr][cc];
	}
	if(abs(rs) < eps && abs(cs) < eps) return true;
   }
   return false;
...
bool canSolve(int K){
   if(K&1) return canSolveOdd(K);
   return canSolveEven(K);
}
...
int main(){
  ...
  FOR(i,0,_T){
    ...
    //cerr << R << C << D << endl;
    FOR(r,0,R) FOR(c,0,C){
	char w; cin >> w; mass[r][c] = D + w-'0';
    }

    bool found = false;
    for(int k=min(R,C);k>=3;k--) if(canSolve(k)){
	     found = true;
	     //cerr << "KHERE: " << k << endl;
	     cout << k;
	     break;
