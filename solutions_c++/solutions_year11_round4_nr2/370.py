...
bool ok(vector<vector<int> > &data, int K, int left, int top) {
	uint64 x, y, mass;
	x = 0;
	y = 0;
	mass = 0;
	for (int i = left; i < left + K; ++i) {
		for (int j = top; j < top + K; ++j) {
			if ((i != left && i != left+K-1) || (j != top && j != top+K-1)) {
				x += i * data[i][j];
				y += j * data[i][j];
				mass += data[i][j];
			}
	...
	if ((2*left*mass + (K-1)*mass == 2*x) && (2*top*mass + (K-1)*mass == 2*y)) {
		return true;
	}
	return false;
}
...
int main(void) {
	...
	REP(__number_of_case, __number_of_cases) {
		...
		if (best != -1) {
			...
			cout << "Case #" << __number_of_case+1 << ": IMPOSSIBLE" << '\n';;
		}
	}
}
