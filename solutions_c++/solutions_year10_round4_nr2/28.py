	...
	for(cc=0;cc<tt;cc++){
		...
		for(i=1;i<=p;i++){
			...
			for(j=0;j<n;j++){
				...
				for(k=0;k<11;k++){
					if(pay[i-1][j*2][k+1]&&pay[i-1][j*2+1][k+1])
						pay[i][j][k]=pay[i-1][j*2][k+1]+pay[i-1][j*2+1][k+1];
					if(pay[i-1][j*2][k]&&pay[i-1][j*2+1][k])
						if(pay[i][j][k]==0||pay[i][j][k]>pay[i-1][j*2][k]+pay[i-1][j*2+1][k]+co[j][i])
							pay[i][j][k]=pay[i-1][j*2][k]+pay[i-1][j*2+1][k]+co[j][i];
				}
			}
		}
		printf("Case #%d: %d\n",cc+1,pay[p][0][0]-(1<<p));
