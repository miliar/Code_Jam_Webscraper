...
const int maxp=10+5;
const int maxn=two(10)+5;

int p,M[maxn],C[maxp][maxn],f[maxp][maxn][maxp];

int solve(int s,int d,int e)
{
	if (s==0) return (e>M[d])?oo:0;
	int &ret=f[s][d][e];
	if (ret>=0) return ret;
	int r1=solve(s-1,d*2,e+1)+solve(s-1,d*2+1,e+1);
	int r2=solve(s-1,d*2,e)+solve(s-1,d*2+1,e)+C[s][d];
	return ret=min(oo,min(r1,r2));
}
int main()
{
...
//	freopen("B-small-attempt2.in","r",stdin);freopen("B-small-attempt2.out","w",stdout);
	...
	for (int caseId=1;caseId<=testcase;caseId++)
	{
		...
		scanf("%d",&p);
		for (int i=0;i<two(p);i++) scanf("%d",&M[i]);
		for (int i=1;i<=p;i++) for (int j=0;j<two(p-i);j++) scanf("%d",&C[i][j]);
