...
vector<int> need;
int was[1025][11][1025];
int dp[1025][11][1025];
int check = 0;

int solve(int lev, int id, int a, int b, int am) {

	if (a == b) {
		if (need[a] > am) return -1;
		return 0;
	}
	if (was[a][lev][am] == check) return dp[a][lev][am];

	was[a][lev][am] = check;

	//q[lev][id]
	// nu cumpar
	int stg = solve(lev+1, 2 * id, a, (a+b)/2, am);
	int dr = solve(lev+1, 2 * id + 1, (a + b) / 2 + 1, b, am);
	
	int ret = -1;

	if (stg != -1 && dr != -1) ret = stg + dr;

	stg = solve(lev + 1, 2 * id, a, (a + b) / 2, am + 1);
	dr = solve(lev + 1, 2 * id + 1, (a + b) / 2  + 1, b, am + 1);

	if (stg != -1 && dr != -1) {
		stg += dr; stg+= q[lev][id];
		if (ret == -1 || stg < ret) ret = stg;
	}

	return dp[a][lev][am] = ret;
}
void solve() {
	int P, N;
	check++;
	...
	for (int i = 0; i < P; ++i) {
		cat /= 2;
		for (int j = 0; j < cat; ++j) {
			int x; cin >> x; vlad[i].push_back(x);
		}
	}
