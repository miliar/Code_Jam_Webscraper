...
int DP(int v,int n,int have)
{
	if(v>=two(n)-1)
	{
		//debug(v);
		int res=have>=n-miss[v-two(n)+1]?0:inf;
//		cerr<<v<<" "<<have<<"="<<res<<endl;
		return res;
	}
	if(exist[v][have])return dp[v][have];
	int& res=dp[v][have];
	exist[v][have]=true;
	int ll=(v<<1)+1;
	int rr=ll+1;
	res=min(add(DP(ll,n,have+1),DP(rr,n,have+1))+cost[v],
			add(DP(ll,n,have),DP(rr,n,have)));
//	cerr<<v<<" "<<have<<"="<<res<<endl;
	return res;
}
...
int main()
{
	...
	for(cin>>tt;tt--;ca++)
	{
		...
		for(int i=0;i<two(n);i++)cin>>miss[i];
		int top=two(n)-2;
		for(int i=0;i<n;i++)
		{
			vector<int> all(two(n-i-1));
			for(int j=0;j<all.size();j++)cin>>all[j];
			reverse(all.begin(),all.end());
			for(int j=0;j<all.size();j++)cost[top--]=all[j];
		}
		for(int i=0;i<two(n)-1;i++)cerr<<cost[i]<<" ";cerr<<endl;
		cout<<"Case #"<<ca<<": "<<DP(0,n,0)<<endl;
	}
	return 0;
}
