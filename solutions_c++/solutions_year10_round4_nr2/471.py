...
struct RealCase : public Case
{
    ...
    LL calc(int row, int col, int bought);
...
static const long long inf = 999999999999999LL;
...
LL RealCase::calc(int row, int col, int missed)
{
    if (row < 0) return (missed > M[col]) ? inf : 0LL;
    assert(row >= 0 && row <= 10 && col >= 0 && col < (1<<10) && missed >= 0 && missed < 10);
    LL &m = memo[row][col][missed];
    if (m >= 0) return m;
    LL a = calc(row - 1, 2*col + 0, missed + 1) + calc(row - 1, 2*col + 1, missed + 1);
    LL b = calc(row - 1, 2*col + 0, missed + 0) + calc(row - 1, 2*col + 1, missed + 0) + C[row][col];
    return m = min(a, b);
}
