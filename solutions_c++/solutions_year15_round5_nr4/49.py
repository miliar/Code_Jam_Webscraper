...
vector<i64> ans;
void recur(vector<tuple<i64, i64>> &v, i64 t)
{
  //DEBUG(v, t);
  auto dat = v;
  sort(all(v));
  if(t == 0){
    for(auto &p: v){
      if(get<1>(p) % 2){ v = dat; return; }
      get<1>(p) /= 2;
    }
    ans.push_back(t);
    recur(v, t);
  }else{
    if(t < 0) reverse(all(v));
    i64 k = 0;
    i64 total = 0;
    ...
    rep(i, sz(v)){
      //DEBUG(v, cnt);
      total -= cnt[get<0>(v[i])];
      get<1>(v[i]) -= cnt[get<0>(v[i])];
      if(get<1>(v[i]) < 0){ v = dat; return; }
      cnt[get<0>(v[i]) + t] += get<1>(v[i]);
      total += get<1>(v[i]);
    }
    bool found_zero = false;
    rep(i, sz(v))if(get<0>(v[i]) == 0 && get<1>(v[i]) == 0) found_zero = true;
    if(total != 0 || found_zero){ v = dat; return; }
    ans.push_back(t);
    recur(v, t);
  }
}
...
int main()
{
  ...
  rep(test_case, T){
    ...
    i64 p;
    cin >> p;
    vector<tuple<i64, i64>> v(p);
    rep(i, p) cin >> get<0>(v[i]);
    rep(i, p) cin >> get<1>(v[i]);
    //if(test_case == 51) DEBUG(v);
    set<i64> numbers;
    rep(i, p) numbers.insert(get<0>(v[i]));
    for(i64 number: numbers) recur(v, number);
