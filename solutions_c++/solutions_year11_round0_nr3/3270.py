...
void recurse(unsigned int lxor, unsigned int rxor, unsigned int lsum, unsigned int rsum, unsigned int index) {
	unsigned int txor, tsum;
	//toleft
	txor = xor(lxor, arr[index]);
	tsum = lsum + arr[index];
	if (index != N-1) {
		recurse(txor, rxor, tsum, rsum, index+1);
	} else {
		if ( (txor == rxor) && tsum > 0 && rsum > 0) {
			result = true;
			unsigned int t1sum = 0;
			t1sum = (tsum > rsum ? tsum : rsum);
			if (t1sum > sum)
				sum = t1sum;
		}
	...
	// else go right :)
	txor = xor(rxor, arr[index]);
	tsum = rsum + arr[index];
	if (index != N-1) {
		recurse(lxor, txor, lsum, tsum, index+1);
	} else {
		if ( (txor == lxor) && tsum > 0 && lsum > 0) {
			result = true;
			unsigned int t1sum = 0;
			t1sum = (tsum > lsum ? tsum : lsum);
			if (t1sum > sum)
				sum = t1sum;
		}			
	}
