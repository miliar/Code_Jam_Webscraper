...
int main(int argc, char* argv[])
{
	...
	{
		auto cv = readval<size_t>();
		auto vals = readvec<size_t>();
		assert(cv == vals.size());
		str result = "NO";
		if (accumulate(vals.begin(), vals.end(), 0, bit_xor<size_t>()) == 0)
		{
			sort(vals.begin(), vals.end());
			result = cast<str>(accumulate(vals.begin() + 1, vals.end(), 0));
		}

end:
		PRINT("Case #" << current_test <<": " << result << endl);
	}
