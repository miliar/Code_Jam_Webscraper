...
static long long sumv(vector<int> &input, bool is_xor) {
    long long sum = 0;
    
    vector<int>::iterator the_iterator = input.begin();
    while( the_iterator != input.end() ) {
        if (is_xor) {
            sum = sum ^ *the_iterator;
        } else {
            sum = sum + *the_iterator;
        }
        ++the_iterator;
