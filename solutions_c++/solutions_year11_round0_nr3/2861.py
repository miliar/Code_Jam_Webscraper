...
public:
    void Solve(const string& input_file, const string& output_file) {
        ...
        for (int i = 0; i < case_number; ++i) {
            ...
            ReadCase(input, candy_number, values);
            int res = SolveProblem(values);
            WriteCaseResult(i+1, output, res);
        }
        input.close();
        output.close();
...
private:
    int SolveProblem(vector<int>& values) {
    ...
    void ReadCase(ifstream& input_stream, int& candy_number, vector<int>& values) {
        input_stream >> candy_number;
    ...
    void WriteCaseResult(int case_num, ofstream& output_stream, int res) {
        if (res == -1) {
            cout << "Case #" << case_num << ": NO" << std::endl;
            output_stream << "Case #" << case_num << ": NO" << std::endl;
        } else {
            cout << "Case #" << case_num << ": " << res << std::endl;
