...
std::pair< bool, long long int > solve( const std::vector< int >& candies )
{
	long long int summ = 0;
	int xorResult = 0;
	int minCandie = candies.front();

	for ( auto it = candies.begin(); it != candies.end(); ++it )
	{
		xorResult ^= *it;
		summ += *it;
		minCandie = std::min( minCandie, *it );
	}

	if ( xorResult == 0 )
	{
		return std::make_pair( true, summ - minCandie );
	}
	else
	{
		return std::make_pair( false, static_cast< long long int >( 0 ) );
	}
}
...
void solve( std::istream& inp, std::ostream& out )
{
	...
	for ( size_t i = 0; i < tests; ++i )
	{
		...
		auto res = solve( candies );
		if ( res.first )
		{
			std::cout << "Case #" << i + 1 << ": " << res.second << std::endl;
		}
		else
		{
