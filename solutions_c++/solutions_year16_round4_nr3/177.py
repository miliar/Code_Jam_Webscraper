...
map<pair<PII, int>, int> Id;
map<int, pair<PII, int> > Pos;
...
bool ck(int ss){
    ...
    REP(i, n+m){
        ...
        while (true){
            ...
            p.se = trans(p.se, _1(ss, p.fi.fi*m+p.fi.se));

            if (CTN(Id, p)){
                //cout << Id[p] << " "<< t << endl;
                if (t != Id[p]) return false;
                else break;
            }
...
bool ok(){
    Id.clear(); Pos.clear(); RD(n, m);

    int x = 0, y = 0, d = 0;

    REP(i, 2*(n+m)){

        Pos[Id[MP(MP(x, y), d)] = i] = MP(MP(x, y), d);
        --RD(A[i]);
