...
struct pos{
  ...
  pos(){}
  pos(int a,int b,int d):a(a),b(b),d(d){}

  pair<char,pos> go(int i){
    return mp((i+d)%2?'\\':'/',pos(a+(d+1)%2*(((i+d/2)%2)?-1:1),b+d%2*(((i+d/2)%2)?1:-1),(d+(i?1:3))%4));
  }
};

ostream&operator<<(ostream&o,pos p){
  return o<<'('<<p.a<<','<<p.b<<':'<<p.d<<')';
}
...
struct sol{
  ...
  pos getpos(int q){
    ...
    q-=N;
    if(q<M)
      return pos(N-1,M-1-q,1);
    q-=M;
    assert(q<N);
    return pos(N-1-q,0,2);
  }
  int getp(int q){
    forv(i,P){
      if(P[i]==q)
  ...
  int getq(pos p){
    if(p.d==1)
      return p.a==-1?p.b:-1;
    if(p.d==2)
      return p.b==M?M+p.a:-1;
    if(p.d==3)
      return p.a==N?M+N+M-1-p.b:-1;
    assert(p.d==0);
    return p.b==-1?M+N+M+N-1-p.a:-1;
  }
  ...
  bool doit(int a,int b){
    ...
    int c=getp(a);
    //dbg(a,b,A,c);
    if(c<=a || c>=b || !doit(a+1,c) || !doit(c+1,b))
      return false;
    pos p=getpos(a);
    while(getq(p)==-1){
      //dbg(p);
      assert(p.a>=0 && p.a<N && p.b>=0 && p.b<M);
      bool f=false;
      forn(t,2){
	auto pp=p.go(t);
	//dbg(p,t,pp);
	if(A[p.a][p.b]!='.' && pp.X!=A[p.a][p.b])
	  continue;
	A[p.a][p.b]=pp.X;
	p=pp.Y;
	f=true;
