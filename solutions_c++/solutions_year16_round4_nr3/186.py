...
int r,c;
int get_id(pr<int,pii> u)
{
	if (u.first)
	{
		if (u.second.second==0) rtn c+r+c+r-u.second.first;
		else if (u.second.second==c) rtn 1+c+u.second.first;
		else rtn 0;
	}
	else
	{
		if (u.second.first==0) rtn 1+u.second.second;
		else if (u.second.first==r) rtn c+r+c-u.second.second;
		else rtn 0;
	}
}
pr<int,pii> get_node(int u)
{
	if (u<=c) rtn mp(0,mp(0,u-1));
	else if (u<=c+r) rtn mp(1,mp(u-c-1,c));
	else if (u<=c+r+c) rtn mp(0,mp(r,c-(u-c-r)));
	else rtn mp(1,mp(r-(u-c-r-c),0));
}
map<pr<int,pii>,pr<int,pii> > st;
pr<int,pii> F(pr<int,pii> x)
{
	if (st[x]==x) rtn x;
	else
	{
...
void U(pr<int,pii> x,pr<int,pii> y)
{
	st[F(x)]=F(y);
}
...
void solve()
{
	...
	rep(s,1<<(r*c))
	{
		...
		map<pr<int,pii>, int> cnt;
		rep(i,sz(p))
		{
			pair<int,pii> s=get_node(p[i]),t=get_node(p[i+1]);
			cnt[F(s)]++;
			cnt[F(t)]++;
			i++;
		}
...
int main()
{
	int T;
	cin>>T;
	ft(t,1,T) cout<<"Case #"<<t<<":",solve(),cout<<endl;
}
