...
int solve(int v, int mask) {
	...
	for(int i = l; i <= r; i++) {
		...
		ans = min(ans, solve(v + 1, mask ^ (1 << i)));
	}
...
void solve(bool skipThisTest) {
	...
	forn(i, n) {
		if (index[i] != -1) {
			index[i] = int(lower_bound(all(X), index[i]) - X.begin());
		}
	}
	...
	forn(mask, 1 << n) {
		minv = min(minv, solve(0, mask));
	}

	string ans = toStr(minv);
	if (minv > INF/2) ans = "CRIME TIME";
	cout << ans << endl;
