...
struct RealCase : public Case
{
    ...
    int calc(int x, int y, int z, int pos, int off);

    int N;
    int C[300], A[300], B[300];
};
...
RealCase::RealCase()
{
    ...
    REP(n, N)
    {
        ...
        int a, b;
        cin >> col >> a >> b;
        if (colors.find(col) == colors.end()) colors[col] = colnum++;
        data[n].first.first = a - 1;
        data[n].first.second = b;
        data[n].second = colors[col];
    }
    SORT(data);
    REP(n, N) C[n] = data[n].second;
    REP(n, N) A[n] = data[n].first.first;
    REP(n, N) B[n] = data[n].first.second;
}

int memo[301][301][301];

int RealCase::calc(int x, int y, int z, int pos, int q)
{
    int &mem = memo[x+1][y+1][z+1];
    if (mem != -1) return mem;

    int check = 0;
    if (x != -1) check = max(check, B[x]);
    if (y != -1) check = max(check, B[y]);
    if (z != -1) check = max(check, B[z]);
    assert(pos == check);

    if (pos == L) return mem = 0;
    if (q == N) return mem = inf;

    int res = calc(x, y, z, pos, q + 1);
    if (A[q] > pos || B[q] <= pos) return mem = res;
    if ( (x != -1 && C[q] != C[x]) &&
         (y != -1 && C[q] != C[y]) &&
         (z != -1 && C[q] != C[z]) ) return mem = res;
    ...
    if (x == -1) x = q; else
    if (y == -1) y = q; else
    if (z == -1) z = q; else assert(0);

    res = min(res, 1 + calc(x, y, z, B[q], q + 1));
    return mem = res;
}
...
void RealCase::solve(Result &result)
{
    SET(memo, -1);
    ostringstream oss;
    int res = calc(-1, -1, -1, 0, 0);
    if (res < inf) oss << res; else oss << "IMPOSSIBLE";
    result.set(oss.str());
}
