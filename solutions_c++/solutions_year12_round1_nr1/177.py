...
void ZyxGCJ_PreProcess();
void ZyxGCJ_Solve(int current_test_case);
...
int main()
{
  try
  {
    ...
    WhileTests()
    {
      ...
      ZyxGCJ_Solve(current_test_case);
    }
    ZyxGCJ_BackUpFiles();
    return 0;
  }
...
double f(int x)
{
  ...
  double& best = memo[x];
  if (mark[x]) return best;
  mark[x] = true;
  double wrong = 1 - r[x];
  int rem = B - x + 1;
  best = rem * r[x] + wrong * (rem + B + 1);
  best = min(best, f(x - 1) + 1);
  return best;
}
...
{
  InGlobalInt2(A, B);
  InVectorGlobalDouble(p, A);
  r.assign(1, 1.0);
  for (int i = 0; i < A; ++i)
    r.push_back(r.back() * p[i]);
  memset(mark, false, sizeof(mark));
  for (int i = 0; i < A; ++i) f(i);
  double sol = min(B + 2.0, f(A));
  printf("Case #%d: %.9lf\n", current_test_case, sol);
}
////////////////////////// Solution ///////////////////////////