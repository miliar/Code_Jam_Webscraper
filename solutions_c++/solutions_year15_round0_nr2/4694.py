...
inline void foreachTest(uns testNum) {
    ...
    // 2. SOLVE test
    uns timeTaken = 0;
    bool first = true; uns best;
    for (dd=1000; dd>0; --dd) {
        if (counts[dd] == 0) continue;
        if (first) {
            first=false; best=dd;
        }
        // Formula : ((N+1)/2 + cnt(N)) < N
        if ((dd==9) and (counts[9] < 2) and (counts[5] < 1)) {
            timeTaken += counts[dd];
            counts[6] += counts[dd];
            counts[3] += counts[dd];
        } else {
            uns lhs = (dd+1)/2 + counts[dd];
            uns rhs = dd;
            if (lhs >= rhs) {
                timeTaken += dd;
                break;
            ...
            timeTaken += counts[dd];
            if (dd % 2) {
                counts[(dd+1)/2] += counts[dd];
                counts[dd/2] += counts[dd];
            } else {
                counts[dd/2] += 2*counts[dd];
            }
        }
        counts[dd] = 0;
    }
    timeTaken = std::min(timeTaken, best);
