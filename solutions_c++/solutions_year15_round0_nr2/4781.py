...
int main(int argc, char* argv[]) {
  ...
  for (int i=0; i<nCases; ++i) {
    ...
    /*while (true) {
      ...
      int best_portion = 2;
      for (int portion = 2; portion <= maximum / 2; ++portion) {
	int step = max(portion, maximum - portion, secondMax) + nMaximum * 1; //(ceil(double(maximum) / portion) - 1);
#ifdef DEBUG
	printf("%d: step = max(%d, %d) + %d * (ceil(double(%d) / %d) - 1) = %d\n", portion, portion, secondMax, nMaximum, maximum, portion, step);
#endif
	if (step < best_step) {
	  best_step = step;
      ...
      if (best_step >= maximum)
	break;

      steps += nMaximum * 1; //(ceil(double(maximum) / best_portion) - 1);

      for (int j=0; j<arr.size(); ++j) {
	if (arr[j] == maximum) { 
	  while (arr[j] > best_portion) {
	    arr[j] -= best_portion;
	    arr.push_back(best_portion);
	  }
	}
      }
