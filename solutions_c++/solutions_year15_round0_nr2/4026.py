...
void vsort(vector<int>& vals)
{
	std::sort(vals.rbegin(), vals.rend());
}

int vmax(vector<int>& vals)
{
	return *(std::max)(vals.begin(), vals.end());
}
...
void vcut(vector<int>& vals, int cutpos)
{
...
int detectcut(vector<int>& vals)
{
	for (size_t i = 1; i < vals.size(); ++i)
	{
		...
		if (vals[i - 1] - j > 0)
		{
			if (vals[i] <= cut1(vals[0]))
			{
				int improve = cut0(vals[i - 1]);
				if (improve >= i)
					return i;
			}
		}
...
void cuts(Scene& s, int cutpos)
{
	size_t srcsz = s.cakes.size();
	for (size_t i = 0; i < srcsz; ++i)
	{
		if (s.cakes[i] <= cutpos)
			continue;

		int a = s.cakes[i] - cutpos;
		int b = cutpos;
		s.cakes[i] = b;
		s.cakes.push_back(a);
		s.waited++;
...
void solve(istream& in, ostream& fout, int caseIdx)
{
	...
	int val = smin(vals);
	OUTANS( << CaseHead(caseIdx+1) << val << endl );
}
...
void proc_file(const string& f_in, const string& f_out)
{
	ifstream ifs(f_in);
	assert( ifs.is_open() );
...
void proc_file(const string& f_in)
{
	proc_file(f_in, f_in + "_out");
}

void readlist(vector<string>& files, const string& root, const string& listfile)
{
