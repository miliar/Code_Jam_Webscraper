...
long N;
vector<long>pancakes;
long getMax(long specialMin){

    if(specialMin==1)
        return max((1+pancakes[N-1])/2,pancakes[N-2]);
    if(specialMin==2){
        if(2*pancakes[N-2]<pancakes[N-1])
            return max((2+pancakes[N-1])/3,pancakes[N-2]);
        return max((1+pancakes[N-1])/2,pancakes[N-3]);
    }
    if(specialMin==3){
        if(3*pancakes[N-2]<pancakes[N-1])
            return max((3+pancakes[N-1])/4,pancakes[N-2]);
        if(2*pancakes[N-2]<pancakes[N-1]||pancakes[N-1]>=3*pancakes[N-3])
        {
            long m=max(pancakes[N-3],(1+pancakes[N-2])/2);
            return max(m,(2+pancakes[N-1])/3);
        }
        return max((1+pancakes[N-1])/2,pancakes[N-4]);
    }
    if(specialMin==4){
        if(4*pancakes[N-2]<=pancakes[N-1])
            return max(pancakes[N-2],pancakes[N-1]/5);
        if(3*pancakes[N-2]<=pancakes[N-1])
            return max(pancakes[N-2]/2,pancakes[N-1]/4);
        if(2*pancakes[N-2]<=pancakes[N-1]){//prothom 2ta cut borotar 3rd ta 2nd tar
            if(pancakes[N-1]<3*pancakes[N-3]){// shesh cut prothomtar na
                if(2*pancakes[N-3]<pancakes[N-2])
                    return pancakes[N-3];
                else
                    return max(pancakes[N-1]/3,pancakes[N-2]/2);
            }
            if(2*pancakes[N-1]<3*pancakes[N-2])
                return pancakes[N-1]/3;
            return max(pancakes[N-1]/4,pancakes[N-2]/2);
        }
        //prothom cut borotar second cut tarporerTar;
        if(pancakes[N-3]*2<pancakes[N-1]){

        }
        return pancakes[N-1]/2;
    }
    return pancakes[N-1];
}
long long pwr[20];
...
int main(void){
	...
	//	double C,F,X,currentCookies=0,currentRate=2.0,time,curtime=0;
        ...
        long minmin=pancakes[N-1];
        for(long i=1;i<4&&i<minmin;i++){
            long curmin=getMax(i)+i;
            minmin=min(minmin,curmin);
        }
            printf("Case #%ld: %ld\n",T+1,minmin);
	}
	return 0;
