...
int solve(VI P, int time, int depth) {
	int ret;
	VI saveP = P;
	...
	cerr << string(depth, ' ') << "time = " << time << endl;
	cerr << string(depth, ' ');
	FOR (i, 1, 11) {
		cerr << P[i] << ' ';
	}
	...
	P[maxI] = 0;
	ret = solve(P, time + 1, depth + 1);
	P = saveP;

	for (int n = 2; n < maxI; ++n) {
		if (maxI % n != 0) { continue; }
		int m = maxI / n;
		// maxI = n * m
		int tt = (n - 1) * P[maxI];
		P[m] += P[maxI] * n;
		P[maxI] = 0;
		ret = min(ret, solve(P, time + tt, depth + 1));
		P = saveP;
	}

	cerr << string(depth, ' ') << " ret = " << ret << endl;
	return ret;
}
