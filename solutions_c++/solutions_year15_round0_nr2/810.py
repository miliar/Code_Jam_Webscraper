...
int main()
{
	...
	FORI(caseNumber, 1, T)
	{
		int D;
		cin >> D;
		int maxEatingMinutes = 0;
		FOR(i,D)
		{
			cin >> P[i];
			if (P[i] > maxEatingMinutes) maxEatingMinutes = P[i];
		}

		int minMinutes = maxEatingMinutes;
		FORI(dedicatedEatingMinutes, 1, maxEatingMinutes)
		{
			int totalSplitCount = 0;
			FOR(i, D)
			{
				if (P[i] <= dedicatedEatingMinutes)
				{
					continue;  // No split needed.
				}
				else
				{
					int splitsNeeded = P[i]/dedicatedEatingMinutes;
					if (P[i]%dedicatedEatingMinutes == 0)
					{
						--splitsNeeded;
					}
					totalSplitCount += splitsNeeded;
				}
			}
			int totalMinutes = dedicatedEatingMinutes + totalSplitCount;
			if (totalMinutes < minMinutes) minMinutes = totalMinutes;
		}
		
		cout << "Case #" << caseNumber << ": " << minMinutes << endl;
	}
    return 0;
}
