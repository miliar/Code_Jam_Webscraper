...
void go(int test_case) {
	mr(int, n);
	VI d(n);
	cin >> d;
	int best = 99999999;
	sfor(int, eat_minutes, 1, 1005) {
		int tot_move_cost = 0;
		foru(i, n) {
			tot_move_cost += (d[i]-1) / eat_minutes;
		}
		MIN(best, eat_minutes + tot_move_cost);
	}
...
int main(){
	mr(int, num_test_cases);
	foru(tc, num_test_cases) {
		go(tc+1);
	}
}
