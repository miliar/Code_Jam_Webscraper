...
public:
	...
	bool validPermutation(const std::string& order) {
		if (order.size() == 1)
			return true;

		std::string newOrder;
		for (uint32 i = 0 ; i < order.size() ; i += 2) {
			char a = order[i];
			char b = order[i + 1];
			if (a == b)
				return false;

			newOrder += resultOfMatch(a, b);
		}

		return validPermutation(newOrder);
	}

	char resultOfMatch(char a, char b) {
		assert (a != b);
