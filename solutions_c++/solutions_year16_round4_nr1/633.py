...
//      printf("upd with %s\n", x.c_str());
  ...
  void solve() {
    ...
    as[0] = "S";
    REP(i, n) {
      ar[i+1] = combine(ar[i], as[i]);
      ap[i+1] = combine(ap[i], ar[i]);
      as[i+1] = combine(as[i], ap[i]);
    }
...
int main() {
  //randomTest();
  //maxTest();
  freopen("a-small-attempt0.in", "r", stdin);  // -small-attempt0.in
