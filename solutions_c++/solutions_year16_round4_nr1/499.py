...
struct RET{
    ...
    RET(int _r , int _p, int _s, string _ret){
        r = _r , p = _p , s = _s , ret = _ret;
    }
    RET merge(RET A){
        if (r == -1 || A.r == -1) return RET(-1 , -1 , -1 , NO);
        RET b;
        b.r = r + A.r;
        b.s = s + A.s;
        b.p = p + A.p;
        if (ret < A.ret) b.ret = ret + A.ret;
        else b.ret = A.ret + ret;
        return b;
    }
}hash[13][128];
RET dfs(int dep , char res){
    if (hash[dep][res].vis){
        return hash[dep][res];
    }
    hash[dep][res].vis = true;
    if (dep == 0){
        if (res == 'S'){
            return hash[dep][res] = RET(0,0,1,"S");
        }
        else if (res == 'P'){
            return hash[dep][res] = RET(0,1,0,"P");
        }
        else if (res == 'R'){
            return hash[dep][res] = RET(1,0,0,"R");
        }
        else assert(0);
    }
    if (res == 'S'){
        return hash[dep][res] = dfs(dep - 1 , 'P').merge(dfs(dep - 1 , 'S'));
    }
    else if (res == 'P'){
        return hash[dep][res] = dfs(dep - 1 , 'R').merge(dfs(dep - 1, 'P'));
    }
    else if (res == 'R'){
        return hash[dep][res] = dfs(dep - 1 , 'S').merge(dfs(dep - 1 , 'R'));
    }
}
void solve(){
    RD(n , R , P , S);
    ...
    string out = NO;
    ans = dfs(n , 'S');
//    cerr << ans.r << ' ' << ans.p << ' ' << ans.s << endl;
    if (ans.r == R && ans.p == P && ans.s == S){
        if (out == NO || ans.ret < out) out = ans.ret;
    }
    ans = dfs(n , 'P');
//    cerr << ans.r << ' ' << ans.p << ' ' << ans.s << endl;
    if (ans.r == R && ans.p == P && ans.s == S){
