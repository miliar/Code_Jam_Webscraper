...
bool gao(int n,int r,int p,int s,char& state)
{
	rrep(i,n)
	{
		int xps=(1<<i)-r,xrs=(1<<i)-p,xrp=(1<<i)-s;
		if (xps<0||xrs<0||xrp<0) rtn false;
		r=xrs,p=xrp,s=xps;
	}
	if (r) state='R';
	if (p) state='P';
...
void gao(string& s,int l,int r)
{
	...
	else
	{
		...
		rep(i,m-l)
			if (s[l+i]>s[m+i])
			{
				rep(i,m-l) swap(s[l+i],s[m+i]);
				break;
			}
			else if (s[l+i]<s[m+i]) break;
	}
}
void gao(int n,char state)
{
	string s;
	s.push_back(state);
	rep(i,n)
	{
		...
		for (char c:s)
		{
			if (c=='R') ns.push_back('R'),ns.push_back('S');
			if (c=='P') ns.push_back('P'),ns.push_back('R');
			if (c=='S') ns.push_back('S'),ns.push_back('P');
		}
		s.swap(ns);
	}
	gao(s,0,1<<n);
...
void solve()
{
	...
	cin>>n>>r>>p>>s;
	char state;
	if (gao(n,r,p,s,state)) gao(n,state);
