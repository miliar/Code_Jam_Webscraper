...
struct problem {
	...
	string test(int a, int b, int lvl = 0) {
		if (lvl == n - 1) {
			string s = string() + chrs[a] + chrs[b];
			sort(all(s));
			return s;
		}
		else {
			if (arps[(a + 2) % 3]-- <= 0)return "";
			if (arps[(b + 2) % 3]-- <= 0)return "";
			string x = test(a, (a + 2) % 3, lvl + 1);
			string y = test(b, (b + 2) % 3, lvl + 1);
	...
	void solve() {
		ff(i, 0, 3)ff(j, 0, 3) {
			if (i == j)continue;
			arps = rps;
			if (arps[i]-- <= 0 || arps[j]-- <= 0)continue;
			string cur = test(i, j);
			if (cur.size() == (1 << n))ans.push_back(cur);
		}
		sort(all(ans));
		if (ans.size() == 0)prln("IMPOSSIBLE");
		else prln(ans[0]);
	}
