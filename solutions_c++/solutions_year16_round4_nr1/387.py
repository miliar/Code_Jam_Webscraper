...
int rr,pp,ss;
string gen(string str,int depth) {
    string ans = "";
    for(int i  = 0;i < str.size();i++) {
        if(str[i] == 'R') ans.append("RS");
        else if(str[i] == 'P') ans.append("PR");
        else ans.append("SP");
    }
    if(depth == 0) return ans;
    else return gen(ans,depth-1);
}
string genAnswer(string now,int depth) {
    if(depth == 0) return now;
    string toRet = "";
    for(int i = 0;i < now.size();i++) {
        if(now[i] == 'R') {
            string left = genAnswer("R",depth-1);
            string right = genAnswer("S",depth-1);
            if(left > right) {
                right.append(left);
                return right;
            }
            ...
            string left = genAnswer("R",depth-1);
            string right = genAnswer("P",depth-1);
            if(left > right) {
                right.append(left);
                return right;
            }
            ...
            string left = genAnswer("S",depth-1);
            string right = genAnswer("P",depth-1);
            if(left > right) {
                right.append(left);
                return right;
...
void solve()
{
    ...
    if(rr == r && pp == p && ss == s) {
        answer.push_back(genAnswer("R",n));
    }
    // S
    rr = pp = ss = 0;
    str = gen("S",n-1);
    ...
    if(rr == r && pp == p && ss == s) {
        answer.push_back(genAnswer("S",n));
    }
    // P
    rr = pp = ss = 0;
    str = gen("P",n-1);
    ...
    if(rr == r && pp == p && ss == s) {
        answer.push_back(genAnswer("P",n));
    }
    if(answer.size() == 0) {
        printf("IMPOSSIBLE\n");
        return;
