...
const string INPUT_FILE
        = IDE_PATH + "GoogleCodeJam/2016/Round2B/input.txt";
const string OUTPUT_FILE
        = IDE_PATH + "GoogleCodeJam/2016/Round2B/output.txt";
...
void check_ch() {
    ...
    while (csch.length() > 1) {
        ...
        while (i + 1 < clen(csch)) {
            ...
            if ((csch [i] == 'R' && csch [i + 1] == 'S')
                || (csch [i] == 'S' && csch [i + 1] == 'R')) ncsch += "R";
            if ((csch [i] == 'S' && csch [i + 1] == 'P')
                || (csch [i] == 'P' && csch [i + 1] == 'S')) ncsch += "S";
            if ((csch [i] == 'P' && csch [i + 1] == 'R')
                || (csch [i] == 'R' && csch [i + 1] == 'P')) ncsch += "P";
            i += 2;
        }
        csch = ncsch;
    }
...
void calc(int k, int r, int p, int s) {
    if (r + p + s == 0) {
        check_ch();
        return;
    }
