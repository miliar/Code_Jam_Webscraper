...
string solve(int x, int n) {
    ...
    int los = ::los(x);
    int win = x;
    string a = solve(los, n - 1);
    string b = solve(win, n - 1);
    return a < b ? a + b : b + a;
}

bool check(int r, int p, const string & s) {
    int cr = 0, cp = 0;
    for(auto & it : s) {
        if(it == 'R') {
            ++ cr;
...
void gao() {
    ...
    string ansr = solve(0, n);
    string ansp = solve(1, n);
    string anss = solve(2, n);
    string ans = big;
    if(check(r, p, ansr)) ans = ansr;
    if(check(r, p, ansp)) ans = min(ans, ansp);
    if(check(r, p, anss)) ans = min(ans, anss);
    if(ans != big) {
        printf(" %s\n", ans.c_str());
    } else {
        puts(" IMPOSSIBLE");
