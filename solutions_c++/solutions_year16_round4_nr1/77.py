...
static const char prs[] = "PRS";

template<typename T>
static void lexifirst(T first, T last)
{
    int N = last - first;
    if (N > 1)
    {
        ...
        lexifirst(first, mid);
        lexifirst(mid, last);
        if (lexicographical_compare(mid, last, first, mid))
...
static void solve_case(int cas, ostream &cout, unique_ptr<promise<void> > &&input_done)
{
    ...
    for (int last = 0; last < 3; last++)
    {
        ...
        if (count(RA(r), 'R') == R
            ...
            && count(RA(r), 'S') == S)
        {
            lexifirst(r.begin(), r.end());
            if (best.empty() || r < best)
                best = r;
        }
    }

    cout << "Case #" << cas + 1 << ": " << (best.empty() ? "IMPOSSIBLE" : best) << "\n";
}
