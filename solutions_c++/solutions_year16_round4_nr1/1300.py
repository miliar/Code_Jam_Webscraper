...
vector<int> Test(int level,int a)
{
	vector<int> res,res2;
	if (level == 0)
	{
	...
	pair<int, int> prev = Won(a);
	res = Test(level - 1, prev.first);
	res2 = Test(level - 1, prev.second);
/*	if (res[0] > res2[0])
		swap(res, res2);*/
	for (int a : res2)
		res.push_back(a);
...
bool Valid(vector<int> &t)
{
	...
	return co[0] == counts[0] && co[1] == counts[1] && co[2] == counts[2];
}
char letters[3] = { 'P','R','S' };
void Order(int *v, int s)
{
	if (s == 1)
		return;
	int h = s / 2;
	...
	Order(h1, h);
	Order(h2, h);
	vector<int> a1(h1, h1+h), a2(h2, h2+h);
	//if (h1[0] > h2[0])
	if (a1 > a2)
	{
		REP(i, h)
...
void Solve()
{
	in >> N >> counts[1]>> counts[0] >> counts[2];

	vector<vector<int>> pos;
	REP(i, 3)
	{
		vector<int> t = Test(N,i);
		if (Valid(t))
		{
			Order(&t[0], t.size());
			pos.push_back(t);
...
typedef ttmath::Big<2, 2> MyDouble;
