...
string dfs(int n, int r) {
  if (n == 0) return string(1, C[r]);
  else {
    string a = dfs(n - 1, r);
    string b = dfs(n - 1, (r + 2) % 3);
...
void solve() {
  ...
  rep (r, 3) {
    string tmp = dfs(N, r);
    assert((int)tmp.length() == (1 << N));

    rep (i, 3) {
      if (A[i] != count(all(tmp), C[i])) goto dmp;
    }
    ans = min(ans, tmp);
 dmp:;
  }

  if (ans == "X") puts("IMPOSSIBLE");
  else puts(ans.c_str());
}
