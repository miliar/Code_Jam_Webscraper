...
bool findtmpath(ll ln, ll rn, ll i, const vector<vector<bool>> &edges,
        vector<bool> &visited, vector<ll> &l2p, vector<ll> &r2p)
{
    rng(j,0,rn){
        if (edges[i][j] && !visited[j]) {
            visited[j]=true;
            if (r2p[j]==-1||findtmpath(ln, rn, r2p[j], edges, visited, l2p, r2p)){
                l2p[i]=j;
                r2p[j]=i;
                return true;
            }
...
void Hungarian(ll ln, ll rn, const vector<vector<bool>> &edges,
        ll &ans, vector<pair<ll,ll>> &ans_matches)
{
    vector<ll> l2p(ln,-1);
    vector<ll> r2p(rn,-1);
    vector<bool> visited(rn);
    ans=0;
    rng(i,0,ln){
        if (l2p[i]>=0) continue;
        visited.resize(rn,false);
        bool found=findtmpath(ln,rn,i,edges,visited,l2p,r2p);
        if (found) ++ans;
    }
    
    ans_matches.clear();
    rng(i,0,ln){
        if (l2p[i]>=0) ans_matches.emplace_back(i,l2p[i]);
    }
}


string build(char c, ll len) {
...
ll MAX=4096;
map<tuple<ll,ll,ll>,ll> vset;
void func(char winner, ll P, ll R, ll S) {
    vset[make_tuple(P,R,S)]=winner;
    if (P+R+S==MAX) return;
    func(winner, P+S,R+P,S+R);
}
...
int main()
{
    ...
    rng(TI,1,TN+1){
        ...
        scanll(S);
        ll tot=1<<n;
        auto val=make_tuple(P,R,S);
        if (vset.find(val)==vset.end()) {
            cout<<"Case #"<<TI<<": IMPOSSIBLE"<<endl;
        } else {
            cout<<"Case #"<<TI<<": "<<build(vset[val],tot)<<endl;
        }
    }
    return 0;
}
