...
string gen(int lev, char c) {
    if (lev == 0) return string(1, c);
    lev--;

    vector<string> tmp;
    if (c == 'P'){
        tmp.push_back(gen(lev, 'P'));
        tmp.push_back(gen(lev, 'R'));
    }
    if (c == 'S'){
        tmp.push_back(gen(lev, 'S'));
        tmp.push_back(gen(lev, 'P'));
    }
    if (c == 'R'){
        tmp.push_back(gen(lev, 'S'));
    ...
    if(tmp[0] < tmp[1]) return tmp[0] + tmp[1];
...
void submain() {
    ...
    for (char c : {'P', 'R', 'S'}) {
        string str = gen(n, c);
        if(cnt(str) == v) ans.push_back(str);
    }
    if(ans.empty()){
        cout << "IMPOSSIBLE";
    } else {
