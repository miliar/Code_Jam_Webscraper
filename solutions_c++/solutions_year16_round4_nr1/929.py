...
int cnt[3];
char let[4] = "RPS";
char ans[(1 << MAX_N) + 228];
bool calc(int lft, int rgt, int w)
{
	if (rgt - lft == 1)
	{
		if (cnt[w] > 0)
	...
	auto res1 = calc(lft, (lft + rgt) >> 1, w);
	auto res2 = calc((lft + rgt) >> 1, rgt, ((w - 1) + 3) % 3);
	return res1 and res2;
}

string go(int lft, int rgt)
{
	if (rgt - lft == 1)
	{
		return string() + ans[lft];
	}
	auto s1 = go(lft, (lft + rgt) >> 1);
	auto s2 = go((lft + rgt) >> 1, rgt);
