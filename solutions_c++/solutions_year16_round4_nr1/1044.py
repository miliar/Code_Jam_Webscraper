...
// simple mod
template<typename T1, typename T2> constexpr T1 smod(T1 x, T2 m)
{ return x > m ? x - m : x + m > m ? x : x + m; }

// returns the largest value x from [l, r) s.t. check(x)
template<typename T, typename F> T dichotomy(T l, T r, F check, T diff = 1)
{ while (r - l > diff) { T m = l + (r - l) / 2; check(m) ? l = m : r = m; } return l; }
...
int main(int argc, char *argv[])
{
    ...
    [](int k, string s) {
        ...
        for (int len = 1; len < 1 << k; len *= 2)
            for (int j = 0; j < 1 << k; j += len * 2)
                if (lexicographical_compare(&t[j], &t[j + len], &t[j + len], &t[j + len + len]) == false)
                    swap_ranges(&t[j], &t[j + len], &t[j + len]);
        return t;
    };
    for (int te = getint(), ti = 1; ti <= te; ti++)
    {
        int n = getint();
        vector<int> a = getints(3), b(3);
        b[0] = 1;
        for (int i = 0; i < n; i++)
        {
            vector<int> c(3);
        ...
        string s("RPS");
        if (a == b)
            writeln(caseSharpSpace(ti), gen(n, s));
        else
        {
            rotate(&a[0], &a[1], &a[3]);
            rotate(&s[0], &s[1], &s[3]);
            ...
            else
            {
                ...
                if (a == b)
                    ...
                    writeln(caseSharpSpace(ti), "IMPOSSIBLE");
            }
        }
    }
    return 0;
