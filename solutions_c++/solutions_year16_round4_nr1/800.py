...
struct Unit {
	int r, p, s;
	piii toPII() const { return piii(pii(r, p), s); }
	bool operator < (const Unit& o) const { return toPII() < o.toPII(); }
};

map< Unit, string > memo[15];
	...
	for_(i,0,13) {
		for (auto a : memo[i]) {
			...
			for (auto b : memo[i]) {
				...
				Unit nxt{ua.r + ub.r, ua.p + ub.p, ua.s + ub.s};
				
				if (memo[i + 1].count(nxt)) {
					minUpdate(memo[i + 1][nxt], sa + sb);
				} else {
					memo[i + 1][nxt] = sa + sb;
				}
			}
