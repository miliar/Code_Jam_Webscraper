...
const int ND = 15;
string CON = "PRS";
int mv[10] = {0,1,1,2,2,0};

string dp[ND][3];
bool vis[ND][3];
int len,p,q,s;

void func(int lv,int d){
    if (vis[lv][d]) return;
    vis[lv][d] = true;
    if (lv == 0){
        dp[lv][d].pb(CON[d]);
    ...
    string s1 = dp[lv-1][mv[2*d]];
    string s2 = dp[lv-1][mv[2*d+1]];

    dp[lv][d] = min(s1+s2,s2+s1);
    // trace(lv,d,s1,s2,dp[lv][d]);
}
...
bool check(string &str){
    ...
    if (x != p or y != q or z != s) return false;
    return true;
}
...
void solve(int tc){
    ...
    for(int k = 0; k < 3; ++k){
        string temp = dp[len][k];
        if (check(temp)) res.pb(temp);
    }
