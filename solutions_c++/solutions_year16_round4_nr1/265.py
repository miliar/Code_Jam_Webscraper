...
const char* hand = "RPS";
string dfs(int win, int rem_depth)
{
    if (rem_depth == 0)
        return string(1, hand[win]);

    auto left = dfs(win, rem_depth - 1);
    auto right = dfs((win + 2) % 3, rem_depth - 1);
...
int main()
{
    ...
    for (int case_i = 1; case_i <= T; ++case_i)
    {
        ...
        rep(winner, 3)
        {
            auto order = dfs(winner, round);

            bool ok = true;
            rep(i, 3)
                ok &= count(all(order), hand[i]) == num[i];
            if (ok)
            {
                res = "";
                for (int i : order)
