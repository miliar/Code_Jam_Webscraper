...
void processCase()
{
    ...
    std::vector<std::vector<std::string> > a(3, std::vector<std::string> (N+1));
    std::vector<std::vector<int> > rc(3, std::vector<int> (N+1));
    std::vector<std::vector<int> > pc(3, std::vector<int> (N+1));
    std::vector<std::vector<int> > sc(3, std::vector<int> (N+1));
    
    a[0][0]="P";
    a[1][0]="S";
    a[2][0]="R";
    
    pc[0][0]=1;  sc[1][0]=1;  rc[2][0]=1;
    ...
    for (int i=1; i<=N; ++i) {
        ...
        for (int j=0; j<3; ++j) {
            pc[j][i]=pc[j][i-1]+sc[j][i-1];
            sc[j][i]=sc[j][i-1]+rc[j][i-1];
            rc[j][i]=rc[j][i-1]+pc[j][i-1];
        }
    }
    
    for (int j=0; j<3; ++j) {
        if (P==pc[j][N] && R==rc[j][N] && S==sc[j][N]) {
            pr(a[j][N]);
            return;
        }
    }
