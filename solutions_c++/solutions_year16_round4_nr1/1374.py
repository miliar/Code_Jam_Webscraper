...
string order(string s){
    if(s.length()==2){
        ...
        string l = order(s.substr(0,s.length()/2));
        string r = order(s.substr(s.length()/2,s.length()/2));
        if(l > r)swap(l,r);
        string ret = "";
...
string level(int n, string last){
    assert(last.length() == 2);
    if(n == 1)return last;
    else{
        for(int i=1; i<n; i++){
            string s="";
            forall(i, 0, last.length()){
                if(last[i] == 'P'){
                    s.pb('P');s.pb('R');
                }else if(last[i] == 'S'){
                    s.pb('P');s.pb('S');
...
int main(){
    ...
    forall(t, 1, T+1){
        ...
        string pr = level(n, "PR");
        string ps = level(n, "PS");
        string rs = level(n, "RS");
        int R,P,S;
        cout<<"Case #"<<t<<": ";
        bool found=false;
        R=P=S=0;
//        trace1("PR");
        forall(i, 0, pr.length()){if(pr[i] == 'P')P++;else if(pr[i] == 'R')R++;else S++;}
        if(R == r && s == S && p == P){cout<<pr<<endl;found = true;}
//        trace1("PS");
        R=P=S=0;
        forall(i, 0, ps.length()){if(ps[i] == 'P')P++;else if(ps[i] == 'R')R++;else S++;}
        if(R == r && s == S && p == P){cout<<ps<<endl;found = true;}
//        trace1("RS");
        R=P=S=0;
        forall(i, 0, rs.length()){if(rs[i] == 'P')P++;else if(rs[i] == 'R')R++;else S++;}
        if(R == r && s == S && p == P){cout<<rs<<endl;found = true;}
