...
double horseDist[110];
double horseSpeed[110];
double timeMatrix[110][110];
double distMatrix[110][110];
double cumDista[110];
ll U[110],V[110];
double getMinTime(ll town,ll horse){
    if(timeMatrix[town][horse]<-0.1){
        if(town+1==N)
            timeMatrix[town][horse]=0;
        else{
            timeMatrix[town][horse]=INF;
            if(distMatrix[town][town+1]<horseDist[town]+0.00001)
                timeMatrix[town][horse]=(distMatrix[town][town+1]/horseSpeed[town])+getMinTime(town+1,town);
            ll coveredDist=cumDista[town]-cumDista[horse];
            ll remainingDist=horseDist[horse]-coveredDist;
            if(remainingDist+0.00001>distMatrix[town][town+1]&&timeMatrix[town][horse]>getMinTime(town+1,horse)+(distMatrix[town][town+1]/horseSpeed[horse]))
                timeMatrix[town][horse]=getMinTime(town+1,horse)+(distMatrix[town][town+1]/horseSpeed[horse]);
        }

    }
    return timeMatrix[town][horse];
}
int main(void){
...
//createDummyTestCases();
  ...
  //createDummyTestCase();
    ...
    for(long T=0;T<test_cases;T++){
        ...
        cumDista[0]=0;
        for(long i=1;i<N;i++){
            cumDista[i]=cumDista[i-1]+distMatrix[i-1][i];
        }
        for(long i=0;i<Q;i++)
            cin>>U[i]>>V[i];
        cout<<getMinTime(0,0);
