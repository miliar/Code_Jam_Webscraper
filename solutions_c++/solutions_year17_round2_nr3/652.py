...
// vector<lld> V[lim];
// string S[lim];
double D[lim],B[lim],C[lim],E[lim],S[lim];
...
void dijkstra(lld x,lld n,lld tot){
    lld i,j,k;
    PA p,t;
    for(i=1;i<=n;i++){
        if(i==x) Q.push(make_pair(D[x]=0,x));
        else Q.push(make_pair(D[i]=tot,i));
    }
    while(!Q.empty()){
        p=Q.top();
        Q.pop();
        ...
        if(A[k]==0){
            ...
            for(j=0;j<V[k].size();j++){
                t=V[k][j];
                if(D[k]+t.first < D[t.second]){
                    D[t.second]=D[k]+t.first;
                    Q.push(make_pair(D[t.second],t.second));
                }
            }
        }
    }
...
int main(){
    ...
    rep(testC,1,T){
        ...
        rep(i,1,n) V[i].clear();
        rep(i,1,n) rep(j,1,n) if(E[i]>=Dis[i][j] && i!=j) V[i].pb(mp(Dis[i][j]/S[i],j));
        while(q--){
            sc2(u,v);
            rep(i,1,n) A[i] = 0;
            dijkstra(u,n,mod*1000000);
            printf("%lf ",D[v]);
        }
        printf("\n");
