...
double horseTime[maxn+1][maxn+1];

void optimize (double d[maxn+1][maxn+1]) {
    bool change = true;
    while (change) {
        change = false;
        for (int i=1; i<=N; i++) for (int j=1; j<=N; j++) for (int k=1; k<=N; k++) {
            if (d[i][j] == -1 || d[j][k] == -1) continue;
            double cand = d[i][j] + d[j][k];
            if (d[i][k] == -1 || cand < d[i][k]) {
                change = true;
                d[i][k] = cand;
            }
        }
