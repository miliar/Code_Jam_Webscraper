...
//	freopen("out", "w", stdout);
	...
	int tc, cs, i, j, k, a, b, Q, n;

	scanf("%d", &tc);
	for(cs = 1; cs <= tc; cs ++) {

		priority_queue< pair< pair<double, int>, pair<int, int> > > q;
		pair< pair<double, int>, pair<int, int> > u;
		...
		for(k = 1; k <= Q; k ++) {
			scanf("%d %d", &a, &b);
			for(i = 1; i <= n; i ++) for(j = 1; j <= n; j ++) key[i][j] = 1e12;
			key[a][a] = 0;
			q.push( {{0, e[a]}, {a, a}} );
			while(q.empty() == 0) {
				u = q.top();
				...
				for(i = 1; i <= n; i ++) {
					...
					if(d[ct][i] == -1) continue;

					if(u.F.S >= d[ct][i]) {
						if(key[i][u.S.S] > key[ct][u.S.S] + d[ct][i]/s[u.S.S]) {
							key[i][u.S.S] = key[ct][u.S.S] + d[ct][i]/s[u.S.S];
							q.push( {{key[i][u.S.S], u.F.S - d[ct][i]}, {i, u.S.S}} );
						}
					}
					if(e[ct] >= d[ct][i]) {
						if(key[i][ct] > key[ct][u.S.S] + d[ct][i]/s[ct]) {
							key[i][ct] = key[ct][u.S.S] + d[ct][i]/s[ct];
							q.push( {{key[i][ct], e[ct] - d[ct][i]}, {i, ct}} );
						}
					}
				}
			}
			double mx = 1e12;
			for(i = 1; i <= n; i ++) mx = min(mx, key[b][i]);
			printf(" %.10lf", mx);
		}
		nl;
	}
