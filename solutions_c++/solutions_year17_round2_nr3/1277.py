...
for (int i=0;i<n;i++)
{
    t maxdist=horses[i][0],speed=horses[i][1];
    double ellapsed_time=time[i];
    //d(i,time[i]);
    int i2=i;
    while (maxdist>0 && i2<n)
    {
        ellapsed_time+= (double)dist[i2]/speed;
        maxdist-=dist[i2];
        if (maxdist>=0)
        {
            //time[i+1]=min(ellapsed_time,time[i+1]);
            i2++;
            time[i2]=min(ellapsed_time,time[i2]);
        }
    }
}
d("alma");
