...
int n;
ll sum[101];
double solve(int i, int j)//(int curCity, int horseCity)
{
    if(i == n-1) return 0.0;
    ...
    if(dp[i][j] < -0.5)
    {
        dp[i][j] = 1e100;
        // keep the horse
        ll traveled = (sum[i] - dist[i]) - (sum[j] - dist[j]);
        if(horseLen[j] - traveled - dist[i] >= 0)
        {
            dp[i][j] = min(dp[i][j], solve(i+1, j) + dist[i] / horseSpeed[j]);
        }
        
        //change the horse
        if(horseLen[i] - dist[i] >= 0)
        {
            dp[i][j] = min(dp[i][j], solve(i+1, i) + dist[i] / horseSpeed[i]);
        }
    }
...
int main()
{
    freopen("/Users/ibra/Downloads/C-small-attempt0 (2).in", "r", stdin);
