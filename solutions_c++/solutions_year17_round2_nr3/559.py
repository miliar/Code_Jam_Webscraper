...
namespace{
	...
	#define		ForRA( i, s, e )	for(int i= s -1; i>= e; i--)
...
int main(){
	...
	For( testLoop, testCnt ){
#ifndef		INF_DEBUG
		cerr<< testLoop<< endl;
#endif
		printf( "Case #%d: ", testLoop +1 );
		answer();
	}
...
void init(House uma, double* cost, int now, double sum){
	For( i, N ) if( Dist[now][i] >= 0 && uma.K >= Dist[now][i] ){
		double c= sum +Dist[now][i] /uma.S;
		if( cost[i]== -1 || cost[i] > c ){
			cost[i]= c;
			uma.K-= Dist[now][i];
			init( uma, cost, i, c );
			uma.K+= Dist[now][i];
...
double ans(int st, int gl){
	struct Data{
		double cost;
		int now;

		Data(){}
		Data(double c, int n) : cost(c), now(n){}
	...
	while( !que.empty() ){
		dt= que.top();
		que.pop();
		if( isVisit[dt.now] ) continue;

		isVisit[dt.now]= true;
		cost[dt.now]= dt.cost;
		if( dt.now== gl ) break;

		For( i, N ) if( !isVisit[i] && Cost[dt.now][i] >= 0 ){
			que.push( Data( dt.cost +Cost[dt.now][i], i ) );
		}
	}
...
void answer(){
	...
	For( i, N ) cin>> H[i].K>> H[i].S;
	For( i, N ) For( k, N ){
		cin>> Dist[i][k];
		Cost[i][k]= -1;
	}

	For( i, N ) init( H[i], Cost[i], i, 0 );

	int Qa, Qb;
	For( q, Q ){
		cin>> Qa>> Qb;
		if( q ) printf( " " );
		printf( "%.12f", ans( Qa -1, Qb -1 ) );
	}
	puts("");
}
