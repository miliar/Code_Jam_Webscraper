...
void loveYK(){
    ...
    while(!q.empty()){
        ST now = q.top() , go;
        q.pop();
        if (dblcmp(vis[now.p] - now.t) < 0) continue;
//        cerr << now.p << ' ' << now.t << endl;
        for (int i = 0 ; i < n ; ++i) if (now.p != i && D[now.p][i] > 0 && c[now.p].fi >= D[now.p][i]){
            if (dblcmp(vis[i]) < 0 || dblcmp(now.t + D[now.p][i] / c[now.p].se - vis[i]) < 0){
                vis[i] = now.t + D[now.p][i] / c[now.p].se;
                go.p = i;
                go.t = vis[i];
                q.push(go);
            }
