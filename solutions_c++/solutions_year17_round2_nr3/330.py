	...
	for (int cn = 1; cn <= T; ++cn)
	{
		...
		for (int i = 0; i < Q; ++i)
		{
			...
			while (!PQ.empty())
			{
				...
				if (arrive_time[now.second] < MAX) continue;
				...
				for (int i = 0; i < N; ++i)
				{
					if (graph[now.second][i] == -1 || i == now.second) continue;
					if (graph[now.second][i] > horse_dist[now.second]) continue;

					double time_i = -arrive_time[now.second] - graph[now.second][i] * 1.0 / horse_speed[now.second];
					PQ.push({ time_i, i });
				}
			}
		}
