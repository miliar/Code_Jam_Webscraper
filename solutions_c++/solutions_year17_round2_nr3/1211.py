...
int main(void){
	...
	REP(t, T){
		...
		long double dd[101];
		fill(dd, dd+101, INF_LL);
		REP(i, 101) fill(dp[i], dp[i]+101, INF_LL);
		dp[0][0] = 0;
		dd[0] = 0;
		REP(i, N){
			REP(j, i){
				if(e[j] >= dis[i+1]-dis[j]){
					dp[i+1][j] = dp[i][j]+(long double)(dis[i+1]-dis[i])/(s[j]);
					dd[i+1] = min(dd[i+1], dp[i+1][j]);
				}
			}
			if(e[i] >= dis[i+1]-dis[i]){
				dp[i+1][i] = dd[i]+(long double)(dis[i+1]-dis[i])/s[i];
				dd[i+1] = min(dd[i+1],dp[i+1][i]);
			}
		}
		long double res = INF_LL;
		cout << "Case #" << t+1 << ": " << fixed << setprecision(10) << dd[N-1] << endl;
	}
}
