...
//{
	...
	//{
		//if( ds[cr][j]<=mx[cr] )
		//{
			//pr=min( pr, slve(j)+(ds[cr][j]*1.0)/s[cr] );
		//}
	//}
	//return pr;
//}
...
double slv(int st,int e)
{
	swap(e,st);
	if(qs[e]==kk)return rs[e][st];
	qs[e]=kk;
	...
	while(!qu.empty())
	{
		...
		for(int j=1;j<=n;j++)
		{
			if(j!=u&& ds[u][j]<=mx[u] )
			{
				double qx=rs[e][u]+(ds[u][j]*1.0)/s[u];
				if( qx<rs[e][j] )
				{
					rs[e][j]=qx;
					if(!on[j])
...
int main()
{
	...
	{
		sc2(n,m);
		for(i=1;i<=n;i++)sc2(mx[i],s[i]);
		for(i=1;i<=n;i++)
		{
			for(j=1;j<=n;j++)
			{
				sc(k);
				if(k==-1)ds[i][j]=(1LL<<61);
				else ds[i][j]=k;
				ds[i][i]=0;
				rs[i][j]=(1LL<<61);
			}
