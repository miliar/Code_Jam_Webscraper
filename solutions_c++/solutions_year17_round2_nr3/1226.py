...
vector<double> small(int64_t N, int64_t Q, const vector<int64_t> &Es,
                     const vector<int64_t> &Ss,
                     const vector<vector<int64_t>> &D,
                     const vector<pair<int, int>> &uv) {
    assert(Q == 1);

    auto dp = make_vector<double, 2>(N + 1, N + 1, 1e12);
    ...
    for (int i : rep(0, N)) {
        ...
        for (int j : rep(i + 1, N)) {
            ...
            dp[i][j] = min(dp[i][j], dp[i][j - 1] + 1.0 * D[j - 1][j] / Ss[i]);
...
int main() {
    ...
    for (int testcase : rep(1, T + 1)) {
        int64_t N, Q;
        in(N, Q);

        vector<int64_t> Es(N), Ss(N);
        for (int i : rep(0, N)) {
            in(Es[i], Ss[i]);
        }

        auto D = vin<int64_t>(N, N);

        vector<pair<int, int>> uv(Q);
        for (int i : rep(0, Q)) {
            in(uv[i].first, uv[i].second);
        }

        auto ans = small(N, Q, Es, Ss, D, uv);
