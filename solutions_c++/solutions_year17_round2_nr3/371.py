...
public:
  ...
  void Run(uint32 T) {
    auto qs = ReadSequence<uint32_pair>(std::cin, Q);
    auto results = map(qs, [this](const uint32_pair& pair) {
      logger.debug("Calculating distance between %0 and %1.", pair.first, pair.second);
      return Distance(pair.first - 1, pair.second - 1);
    });
    print("%2%3Case #%0: %1", T + 1, iterate(results), std::fixed, std::setprecision(20));
  }

  float_type Distance(uint32 begin, uint32 end) {
    using vertex_type = uint32_pair;
    std::set<vertex_type> visited;
    using queue_value_type = std::pair<std::pair<float_type, int64>, vertex_type>;
    std::priority_queue<queue_value_type, std::vector<queue_value_type>, std::greater<queue_value_type>> Q;

    Q.emplace(std::make_pair(0.0, 0), std::make_pair(begin, begin));

    while (!Q.empty()) {
      const auto top = Q.top();
      Q.pop();
      const auto vertex = top.second;
      const auto horse = vertex.second;
      const auto horse_range = horses[horse].first;
      const auto horse_speed = horses[horse].second;
      const auto city = vertex.first;
      const auto time = top.first.first;
      const auto travelled = top.first.second;
      ...
      for (auto v: range<uint32>(0, N)) {
        auto d = edges[city][v];
        if (d == -1)
          continue;

        if (d + travelled <= horse_range) {
          auto time_delta = float_type(d) / horse_speed;
          Q.emplace(std::make_pair(time + time_delta, d + travelled), std::make_pair(v, horse));
        }
      }

      
      for (auto v: range<uint32>(0, N)) {
        const auto horse_range = horses[city].first;
        const auto horse_speed = horses[city].second;
        ...
        if (d <= horse_range) {
          auto time_delta = float_type(d) / horse_speed;
          Q.emplace(std::make_pair(time + time_delta, d), std::make_pair(v, city));
        }
      }
    }
  ...
  void LoadData() {
    read(std::cin, N, Q);
    edges.assign(N, std::vector<int64>(N));
    horses = as_vector(ReadSequence<uint32_pair>(std::cin, N));
    for (auto i: range<uint32>(0, N)) {
      for (auto j: range<uint32>(0, N)) {
        int64 d;
        read(std::cin, d);
        edges[i][j] = (d > -1)? d : int64_infinity;
      }
    }
  }
...
private:
  uint32 N, Q;
  std::vector<uint32_pair> horses;
  std::vector<std::vector<int64>> edges;
};

int main(int argc, const char* argv[]) {
  std::ios::sync_with_stdio(false);
  logging::log_master().subscribe("main", logging::log_level::info, "", &std::cerr);
  uint32 T;
  read(std::cin, T);
