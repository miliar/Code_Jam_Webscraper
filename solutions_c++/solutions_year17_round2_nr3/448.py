...
slong dist[MAXN+5][MAXN+5];

double timedist[MAXN+5][MAXN+5];
...
int main()
{
    ...
    for (t = 1; t <= T; t++)
	{
		...
		for (i = 1; i <= n; i++)
		{
			for (j = 1; j <= n; j++)
			{
				if (dist[i][j] != LONG_LIMIT)
				{
					//go from i -> j
					if (e[i] >= dist[i][j])
					{
						double curr = dist[i][j]*1.0/s[i];
						timedist[i][j] = min(timedist[i][j], curr);
						//debug(i, j, dist[i][j], s[i], curr);
					}
				}
			}
		}
		...
		for (int k = 1; k <= n; k++)
		{
			for (i = 1; i <= n; i++)
			{
				...
				{
					//go from i->j directly
					if (timedist[i][k] + timedist[k][j] < timedist[i][j])
						timedist[i][j] = timedist[i][k] + timedist[k][j];
				}
			}
		}
		...
		while (q--)
		{
			...
			double ans = timedist[u][v];
			
			assert(ans < DOUBLE_INFINITY/100);
