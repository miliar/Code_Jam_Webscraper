...
/* Convert a string number to a given base */
int to_base10(char c){ return isdigit(c) ? c-'0' : 10+c-'A'; }

int64 to_base10(string num,int base)
{
     int64 ans=0,pow=1,len=size(num)-1;
     for(int64 i=len;i>=0;i--,pow*=base)ans+=to_base10(num[i])*pow;
     return ans;
}

/* Convert a given number to given base */
char to_baseN(int c){ return c < 10 ? c+'0' : 'A'+c-10; }

string to_baseN(int64 temp,int base)
{
       string ans="";
       int64 high=base*base;
       while(high<=temp)high*=base;high/=base;
       while(high>1)
       {
                  ans+=to_baseN(temp/high);
                  temp%=high;
                  high/=base;
       }
       return ans + to_baseN(temp);
}
...
int main()
{
    ...
    For(kase,t)
    {
               ...
               {
                               //cout << M[str[i]] << " ";
                               str[i]=to_baseN(M[str[i]]);
               }
               //cout << str << endl;
               int64 val = to_base10(str,k);
               cout << val << endl;
    }
    return 0;
}
