...
template<class T> inline string toString(T n){ostringstream ost;ost<<n;ost.flush();return ost.str();}//NOTES:toString(
template<class T> inline T toNum(string s, T &n){istringstream sin(s);sin>>n;return n;}//NOTES:toNum(
inline int toInt(string s){int r=0;istringstream sin(s);sin>>r;return r;}//NOTES:toInt(
inline int64 toInt64(string s){int64 r=0;istringstream sin(s);sin>>r;return r;}//NOTES:toInt64(
inline double toDouble(string s){double r=0;istringstream sin(s);sin>>r;return r;}//NOTES:toDouble(
template<class T> inline void stoa(string &s, int &n, T A[]){n=0;istringstream sin(s);for(T v;sin>>v;A[n++]=v);}//NOTES:stoa(
template<class T> inline void atos(int n, T A[], string &s){ostringstream sout;for(int i=0;i<n;i++){if(i>0)sout<<' ';sout<<A[i];}s=sout.str();}//NOTES:atos(
template<class T> inline void atov(int n, T A[], vector<T> &vi){vi.clear();for (int i=0;i<n;i++) vi.push_back(A[i]);}//NOTES:atov(
template<class T> inline void vtoa(vector<T> &vi, int &n, T A[]){n=vi.size();for (int i=0;i<n;i++)A[i]=vi[i];}//NOTES:vtoa(
template<class T> inline void stov(string &s, vector<T> &vi){vi.clear();istringstream sin(s);for(T v;sin>>v;vi.push_back(v));}//NOTES:stov(
template<class T> inline void vtos(vector<T> &vi, string &s){ostringstream sout;int len=vi.size();for(int i=0;i<len;i++){sout<<vi[i]<<' ';}s=sout.str();}//NOTES:vtos(
//Fraction
