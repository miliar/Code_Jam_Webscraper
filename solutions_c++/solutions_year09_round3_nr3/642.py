...
void GetInt(int n, int a[]) {
	for (int i = 0; i < n; ++i)
		scanf("%d", &a[i]);
}
int **CreateMatrix(int mr, int nc) {
	int **ma = (int**) malloc(mr * sizeof(int*));
	for (int m = 0; m < mr; ++m)
		ma[m] = (int*) malloc(nc * sizeof(int));
	return ma;
}
void GetMatrix(int mr, int nc, int **ma) {
	for (int m = 0; m < mr; ++m)
		for (int n = 0; n < nc; ++n)
			scanf("%d", &ma[m][n]);
}
void PrintMatrix(int mr, int nc, int **ma) {
	for (int m = 0; m < mr; ++m) {
		for (int n = 0; n < nc; ++n)
			printf("%d ", ma[m][n]);
		printf("\n");
...
void Solve()
{
	...
	do
	{
		...
		cells = (char*) malloc(P * sizeof(char));
		for (int i = 0; i < P; ++i)
			cells[i] = 'o';
		...
		{
			cells[qs[i] - 1] = 'e';
			for (int j = qs[i]; j < P; ++j)
			{
				if (cells[j] == 'e') break;
				++acost;
			}
			for (int j = qs[i] - 2; j >= 0; --j)
			{
				if (cells[j] == 'e') break;
				++acost;
			}
	...
	while (next_permutation(qs, qs + Q));
...
int main()
{
	freopen("c:\\in_c", "rt", stdin); freopen("c:\\out_c", "wt", stdout);
	
	int numOfTestCase; scanf("%d\n", &numOfTestCase); 
