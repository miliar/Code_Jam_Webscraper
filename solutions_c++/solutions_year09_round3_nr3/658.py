...
int main()
{
	...
	int testcase;int a[5],i,p,j,q,n,d[100],k;
	int b[120][5];
	scanf("%d",&testcase);
	for (int caseId=1;caseId<=testcase;caseId++)
	{   
        for(i=0;i<120;i++)for(j=0;j<5;j++)b[i][j]=j+1;
		cin>>p>>q;
        for(i=0;i<5;i++)a[i]=0;
		for(i=0;i<q;i++)cin>>a[i];
		printf("Case #%d: ",caseId);
		if(q==1){cout<<p-1;if(caseId!=testcase)cout<<endl;continue;}
		for(i=1;i<120;i++)
		  { next_permutation(b[i],b[i]+q);
		    if(i!=119){for(j=0;j<5;j++)b[i+1][j]=b[i][j];}
           }
        int c=0,ans=1000;
        if(q==2)n=2;if(q==3)n=6;if(q==4)n=24;if(q==5)n=120;
        for(i=0;i<n;i++)
        { c=0;
          for(j=0;j<100;j++)d[j]=1;
          for(j=0;j<q;j++)
           { int tp=b[i][j];int tp1=a[tp-1];
             d[tp1-1]=0;//cout<<tp1-2;
             for(k=tp1-2;k>=0;k--)if(d[k]==1)c++;else break;
             //cout<<c;
             for(k=tp1;k<p;k++)if(d[k]==1)c++;else break;
             //cout<<c<<"   ";
           }
           //cout<<endl<<" asdasda"<<c<<endl;
          if(c<ans)ans=c;
        }
        cout<<ans;if(caseId!=testcase)cout<<endl;   
       //for(i=0;i<120;i++){for(j=0;j<5;j++)cout<<b[i][j]<<" ";cout<<endl;}
     }      
...
//			if (isdigit(G[i][j]))
//				addnode(i,j,G[i][j]-'0',1);
//		for (int cl=0;cl<sizeQ;cl++)
//		{
//			int x=Q[cl]%size;
//			int y=Q[cl]/size%size;
//			int value=Q[cl]/size/size-M;
...
//							int newvalue=value;
//							int delta=G[x3][y3]-'0';
//							if (G[x2][y2]=='+') newvalue+=delta;
//							else newvalue-=delta;
//							addnode(x3,y3,newvalue,D[x][y][value+M]+2);
//						}
//					}
//			}
//		}
