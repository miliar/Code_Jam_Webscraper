...
int R;
int Inp[200][200], tmp[200][200];

bool Check() {
   REP(i,200) REP(j,200) if(Inp[i][j]) return true;
   return false;
}
...
void run() {
   ...
   FOR(i,1,105) FOR(j,1,105) {
      if(Inp[i][j] && (Inp[i-1][j] || Inp[i][j-1]));
      else tmp[i][j] = 0;
      if(!Inp[i][j] && (Inp[i-1][j] && Inp[i][j-1]))
	 tmp[i][j] = 1;
   }
   memcpy(Inp,tmp,sizeof tmp);
}
int main() {
	...
	REP(kase,kases) {
		...
		int r, x1, x2, y1, y2; scanf("%d",&r);
		REP(i,r) {

		   scanf("%d%d%d%d",&x1,&y1,&x2,&y2);
		   FOR(j,x1,x2+1) FOR(l,y1,y2+1) Inp[j][l] = 1;	
		}
		int score = 0;
		while(Check()) {
			run();
