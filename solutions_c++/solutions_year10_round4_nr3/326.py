// Time-stamp: <2010-04-28 14:22:48 errge>
...
void step(const board &act, board &next) {
    coun = 0;
    next = board(MS, vector<int>(MS, 0));
    for (int y=1; y<MS; ++y)
        for (int x=1; x<MS; ++x) {
            if (act[y][x] == 1) {
                if (act[y-1][x] == 0 && act[y][x-1] == 0) {
                    // died
                } else {
                    ++coun;
                    next[y][x] = 1;
