...
RealCase::RealCase()
{
    ...
    REP(r, R) {
        ...
        cin >> x1 >> y1 >> x2 >> y2;
        --x1, --y1, --x2, --y2;
        rects.push_back(make_pair(  make_pair(x1,y1), make_pair(x2,y2) ));
    }
}
...
bool dead()
{
    REP(x, 100) REP(y, 100) if (grid[x][y]) return false;
    return true;
}

void advance()
{
    bool next_grid[100][100];
    REP(i, 100) REP(j, 100) {
        next_grid[i][j] = grid[i][j];
        if ( (i == 0 || !grid[i-1][j]) && (j == 0 || !grid[i][j-1]) ) next_grid[i][j] = 0;
        if ( (i > 0 && grid[i-1][j]) && (j > 0 && grid[i][j-1]) ) next_grid[i][j] = 1;
    }
...
{
    FORIT(it, rects) {
        FORE(x, it->first.first, it->second.first) {
            FORE(y, it->first.second, it->second.second) {
                grid[x][y] = true;
            }
        }
    }
    int res = 0;
    while (!dead()) advance(), ++res;
    result.set(STR(res));
}
