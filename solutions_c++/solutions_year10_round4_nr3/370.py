...
bool bacteria[101][101];
bool tbacteria[101][101];

int _T;
int R;

bool anyleft(){
   FOR(i,0,101) FOR(j,0,101) if(bacteria[i][j]) return true;
   return false;
}
...
int main(){
  ...
  FOR(i,0,_T){
    cout << "Case #" << (i+1) << ": ";
    MEMSET(bacteria,false);
    cin >> R;
    FOR(j,0,R){
       int x1, x2, y1, y2;
       cin >> x1 >> y1 >> x2 >> y2;
       FOR(k,x1,x2+1) FOR(k2,y1,y2+1) bacteria[k][k2] = true;
    }
    int ret = 0;
    while(anyleft()){
	MEMSET(tbacteria,false);
        FOR(r,0,101) FOR(c,0,101){
	   if(!bacteria[r][c] && r!=0 && c!=0 && bacteria[r-1][c] && bacteria[r][c-1]) tbacteria[r][c] = true;
	   if(bacteria[r][c] && (r!=0 && bacteria[r-1][c] || c!=0 && bacteria[r][c-1])) tbacteria[r][c] = true;
	}
        memcpy(bacteria,tbacteria,sizeof(bacteria));
        ret++;
    }
    cout << ret;
    cout << endl;
