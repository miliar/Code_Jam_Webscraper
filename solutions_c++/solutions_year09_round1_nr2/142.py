...
// declare global variable for each case
int N, M;
int S[22][22], W[22][22], T[22][22];
int t[50][50];
int d1[4]={0,-1,0,1}, d2[4]={1,0,-1,0};
// for init purpose for each case
void init()
{
	scanf("%d %d",&N, &M);
	for (int i=0;i<N;++i)
...
void start(int x, int y)
{
	...
	for (int i=0;i<4;++i)
	{
		xe=x+d1[i];
		ye=y+d2[i];
		if (xe>=0&&xe<=2*N-1&&ye>=0&&ye<=2*M-1)
		{
			if (((i==1) && (x%2==0)) ||
				((i==3) && x%2==1) ||
				((i==2) && (y%2==0)) ||
			...
			{
				tmp_t=(t[x][y]-T[xg][yg])%(S[xg][yg]+W[xg][yg]);
				if (tmp_t < 0)
					tmp_t += (S[xg][yg]+W[xg][yg]);
				if (i%2)
				{
					if (tmp_t<S[xg][yg])
					{
						...
						tmp_t1 = t[x][y]+W[xg][yg]+S[xg][yg]-tmp_t+1;
				}
				else
				{
					if (tmp_t<S[xg][yg])
					{
						tmp_t1 = t[x][y]+S[xg][yg]-tmp_t+1;
					}
					else
						tmp_t1 = t[x][y]+1;
				}
				if (t[xe][ye] < 0 || t[xe][ye] > tmp_t1)
				{
					t[xe][ye] = tmp_t1;
					start(xe,ye);
				}
