...
clock_t t;
/////////////////////////////////////////////////// Function
int function(const LD &N, const LD &M);
bool palin(const LD &x);
///////////////////////////////////////////////////
  ...
  if(in_file.is_open()){
    ...
    while(!in_file.eof()){
      ...
      /// Reading DATA 
        ...
        if(out_file.is_open()){
          out_file << "Case #" << lineN << ": " << function(N,M) << endl;  
        }else{
...
bool palin(const LD &x)
{
  ...
  ss << x;
  string str = ss.str();  
  for(LD i=0; i<floor(str.size()/2);++i){
