#include <iostream>
#include <cstdio>
using namespace std;
long long int palin[39] = { 1, 4, 9, 121, 484, 10201, 12321, 14641, 40804, 44944, 1002001, 1234321, 4008004, 100020001, 102030201, 104060401, 121242121, 123454321,
                          125686521, 400080004, 404090404, 10000200001, 10221412201, 12102420121, 12345654321, 40000800004, 1000002000001, 1002003002001, 1004006004001,
                          1020304030201, 1022325232201, 1024348434201, 1210024200121, 1212225222121, 1214428244121, 1232346432321, 1234567654321, 4000008000004,
                          4004009004004};

void algo() {
    static int caseNo = 0;
    long long int a, b;
    int i=0,j=0;
    int count = 0;
    cin >> a >> b;
    while(a > palin[i] && i < 39){
        i++;
    }
    j=i;
    while(b >= palin[j] && j < 39){
        j++;
    }
    if(i == 39 || j == 39)
        count = 0;
    else
        count = j - i;

    cout << "Case #" << ++caseNo <<": "<< count << endl;
}

int main() {
    int nCases;
    cin >> nCases;
    for(int i=0; i < nCases; i++)
        algo();
    return 0;
}
