	...
	int main()
	{	
		unsigned long long cases, A,B,ans;
		string a;
		cin >> cases;
		getline(cin,a); //for extra space
		
		bool palindrome(unsigned long long);
		bool isPerfectSquare(unsigned long long);
		...
		{	
			...
			{
				//if(palindrome(num) == 1 && isPerfectSquare(num))
				if(palindrome(num) == 1 && sqrt(num) == floor(sqrt(num)) && palindrome(sqrt(num)) == 1)
				{
					ans++;
				}
	...
	{
		int digit[15], numModified, index = 0, noOfChecks;
		if(num < 10)
			{
			digit[0] = 0;
			return 1;
		...
		{	
			...
			while(numModified > 0)
			{
				digit[index] = numModified%10;
				numModified = floor(numModified/10);
				index++;
			}
		}
		noOfChecks = floor(index/2);
		for(int check = 0; check< noOfChecks; check++)
		{			
			if(digit[check] != digit[index-1-check])			
				return 0;
		}
		return 1; //will reach here only when it comes out of for loop which means it is a pal
	}
	



bool isPerfectSquare(unsigned long long int n)
{/*
 bool bad255[] =
{
 ...
 int start[] =
{
  ...
  if( bad255[(unsigned long long int)y] )
      return false;
  ...
  unsigned long long int r, t, z;
  r = start[(unsigned long long int)((n >> 3) & 0x3ffL)];
  do {
    z = n - r * r;
    if( z == 0 )
      return true;
