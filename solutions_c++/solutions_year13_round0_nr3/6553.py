...
int main()
{
	...
	ifstream in("/home/sandip/Downloads/C-small-attempt0.in",ifstream::in);
	ofstream out("/home/sandip/Downloads/C-small-attempt0.out",ofstream::out);
	...
	for(unsigned long long i=begin;i<=end;i++)
	{
		if(isPerfectSquare(i)|| i==1)
		{
			...
			if( equal(s.begin(), s.begin() + s.size()/2, s.rbegin()) )
			{
				...
				if( equal(s1.begin(), s1.begin() + s1.size()/2, s1.rbegin()) )
					nFairSquare++;
