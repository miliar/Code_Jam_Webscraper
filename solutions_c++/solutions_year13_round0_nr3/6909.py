...
#define forit(c,itr) for(itr=(c).begin();itr!=(c).end();++itr)
#define all(v) (v).begin(), (v).end()

#define Fill(a,c) memset(&a, c, sizeof(a))

int nDigit(int t){ if (t<10) return 1; return 1 + nDigit(t/10); }
template<class T> inline int countbit(T n){return (n==0)?0:(1+countbit(n&(n-1)));}
template<class T> inline bool isPrime(T n){if(n<=1)return false;for (T i=2;i*i<=n;i++) if (n%i==0) return false;return true;}
int power(int n,int k){ int r =n; if((n==0)||(k==1)) return n; if(k==0) return 1; while(k>1) { n = r*n; k--;} return n; }

char* stoa(string s){char *str=new char(sizeof(s.length()+1)); strcpy(str,s.c_str());return str;} //free the mem after use
int toInt(string s){int r=0;istringstream sin(s);sin>>r;return r;}
__int64 toInt64(string s){__int64 r=0;istringstream sin(s);sin>>r;return r;}
template<class T> string toString(T n){ostringstream ost;ost<<n;ost.flush();return ost.str();}

bool funobj (int i,int j){ return (i>j); } // Useful for sort algorithm
...
static bool isPerfectSquare(ll n)
{
	// Quickfail
...
bool palindrom(ll number) {
	// PAL or Not
	ll rev = 0,temp = number;
    while (number) {
        rev *=10;
        rev += number % 10;
        number /= 10;
...
void sp(ll A)
{
	...
	s = toString(A);
	sLen = s.length();
	cN = s[sLen-1]-'0'; // toInt(s[sLen-1].c_str()); // character to number
	int pFlag = 5;
	// Checking the no tail is 1, 4, 5, 6 or 9.
	if((cN == 1) || (cN == 4) || (cN == 5) ||
		(cN == 6) || (cN == 9)){
			...
			for (int n = 0; n < sLen/2; n++) {
				m = sLen - n - 1;
				if (s[n] != s[m]) {
					pFlag = 7; // fail not palindrome
				}
			}

			if(pFlag == 5) { // PAL

				if(true == isPerfectSquare(A)) {
					//temp = std::sqrt(A);
					if(palindrom(std::sqrt((long double)A)))
					 pCnt = pCnt +1;
				}
			}
	} //if
...
int main(){
	...
	for(int tc=0; tc<tCount; ++tc) 
	{
		...
		int aDigit,bDigit,diffDigit,acDigit,bcDigit;
		ll A,B;
		//bool temp = true;
		cin>>A>>B;
		...
		aDigit = nDigit(A); // 2
		bDigit = nDigit(B); // 4
		diffDigit = bDigit - aDigit;

		// Count by lookup table
		if(diffDigit > 1) {
			for(int i= aDigit+1; i<bDigit; ++i )
				pCnt = pCnt + pal[i];
		}
