...
void init()
{
	for (int i = 0; ; i++)
	{
		...
		string s3 = s + string(s2.begin() + 1, s2.end());
		string s4 = s + s2;
		int n1 = atoi(s3.c_str());
		int n2 = atoi(s4.c_str());
		if (ll(n1) * n1 > 1000000000000000LL)
			break;
		if (check(n1))
			ans.push_back(ll(n1) * n1);
		if (check(n2))
			ans.push_back(ll(n2) * n2);
	}
}
...
int main()
{
	...
	fort
	{
		...
		read b;
		ll cnt = 0;
		foria(ans) if (a <= ans[i] && ans[i] <= b) ++cnt;
		write cnt;
		writeln;
	}
