...
#define MAX_UNIT_VALUE  (SIZE_MAX >> 2)
#define BASE1      10

#ifdef _WIN64
#define BASE2      1000000000000000000ULL   // BASE1 ** (BASE1 - 1)
#define subBASE2   999999999999999999ULL    // subBASE2 == BASE2 - 1
#define overBASE2  0xf000000000000000ULL    // overBASE2 > subBASE2
#else
#define BASE2      1000000000UL   // BASE1 ** (BASE1 - 1)
#define subBASE2   999999999UL    // subBASE2 == BASE2 - 1
#define overBASE2  0xc0000000UL   // overBASE2 > subBASE2
#endif
...
const size_t ULONG_MSB = static_cast<size_t>(1) << (sizeof(size_t) * CHAR_BIT - 1);
...
#define VIN_CREATE_EMPTY_VTEST_BIG_ULONG vector<pair<VinBigInt, size_t> > vtest
...
#define VIN_ADD_TEST_CASE_ULONG_for_BIG_ULONG(a, b) \
	  vtest.push_back(pair<VinBigInt, size_t> (VinBigInt(a), b))

#define VIN_ADD_TEST_CASE_STRING_DEC_for_BIG_ULONG(a, b) \
	  vtest.push_back(pair<VinBigInt, size_t> (VinBigInt(a, DEC_DIGIT), b)))

#define VIN_ADD_TEST_CASE_STRING_HEX_for_BIG_ULONG(a, b) \
	  vtest.push_back(pair<VinBigInt, size_t> (VinBigInt(a, HEX_DIGIT), b))
...
void ShowVersion()
{
  ...
  cerr << "Compiled in " << (sizeof(size_t) * CHAR_BIT) << "-bit mode" << endl;
...
static const size_t hex_digit_size_cns (4);
static const size_t digits_in_ulong_s ((sizeof (size_t) * CHAR_BIT)/hex_digit_size_cns);
...
{
  private:
    map<char, size_t> hex_to_dec_digits_convertor_;
  ...
  public:
    ...
    map<char, size_t> get_hex_to_dec_digits_convertor ()
    {
...
map<char, size_t> BaseBigInt::hex_to_dec_digits_convertor_s (aux1_ins.get_hex_to_dec_digits_convertor());
...
// ------------------
size_t BaseBigInt::get_units_size() const
{
  return units_.size();
}
...
// ------------------
static size_t carry_s = 0;
size_t add_units(size_t n1, size_t n2)
{
  n1 += (n2 + carry_s);
...
// ------------------
// Constructor-1
VinBigInt::VinBigInt (size_t unit_i) 
{
  if (!(unit_i < BASE2))
  {
    FATAL_ERROR ("size_t value too big");
...
// ------------------
// Constructor-2
VinBigInt::VinBigInt (const string& arg_i, size_t digit_base_i)
{
...
// ------------------
size_t VinBigInt::get_pure_ulong () const
{
  ASSERTION (!units_.empty());
  if (units_.size() > 1)
  {
    FATAL_ERROR ("size_t can't be obtained: units_.size() too big");
...
// ------------------
// VinBigInt::operator size_t () const
// {
...
// ------------------
bool VinBigInt::init_via_string (const string& arg_i, size_t digit_base_i)
{
  ASSERTION (units_.empty());
...
{
VinBigInt ret;
const size_t max_size (MAX_VAL (units_.size (), arg_i.units_.size ()));

vector<size_t> u1 (units_);
vector<size_t> u2 (arg_i.units_);
...
// ------------------
VinBigInt VinBigInt::operator* (size_t arg_i) const
{
VinBigInt ret (0);
  for (size_t i = 0; i < arg_i; i++) ret = ret + (*this);
  return ret;
}
...
bool VinBigInt::operator< (const VinBigInt& arg_i) const
{
  ...
  for (size_t i = (units_.size() - 1); i > 0; i--)
  {
...
ostream& operator<< (ostream& os, const VinBigInt& ins_i)
{
  ...
  for (size_t i = (ins_i.units_.size () - 1); i; --i) 
  {
...
vector<string> hexstr_nums;
  ...
  for (size_t i = 0; i < ulong_nums.size(); i++)
  {
    for (size_t k = 0; k < ulong_nums.size(); k++)
    {
  ...
  for (size_t i = 0; i < hexstr_nums.size(); i++)
  {
    for (size_t k = 0; k < hexstr_nums.size(); k++)
    {
...
// ------------------
// Constructor-1
RossiBigInt::RossiBigInt (size_t unit_i) 
{
  ASSERTION (units_.empty());
  units_.push_back (unit_i);
}
...
// ------------------
// Constructor-2
RossiBigInt::RossiBigInt (const string& arg_i, size_t digit_base_i) 
{
...
RossiBigInt& RossiBigInt::operator= (size_t unit_i)
{
	units_.resize(1);
	units_.front() = unit_i;
	return *this;
...
// ------------------
size_t RossiBigInt::get_pure_ulong () const
{
  ASSERTION (!units_.empty());
  if (units_.size() > 1)
  {
...
bool RossiBigInt::is_zero() const
{
	if ( units_.size() > 1 )
		return false;
	return units_.empty() || !units_.front();
}

// ------------------
// RossiBigInt::operator size_t () const
// {
//  return get_pure_ulong();
// }
...
// ------------------
bool RossiBigInt::init_via_string (const string& arg_i, size_t digit_base_i)
{
  ASSERTION (units_.empty());
  ASSERTION ((digit_base_i ==  16) || (digit_base_i ==  10));
...
// ------------------
void RossiBigInt::resize_units(size_t size_i)
{
...
// ------------------
// RossiBigInt& RossiBigInt::operator= (size_t arg_i)
// {
...
const size_t max_size (MAX_VAL (get_units_size(), arg.get_units_size()));
...
// ------------------
RossiBigInt RossiBigInt::operator>> (size_t shift_i) const
{
...
// ------------------
RossiBigInt& RossiBigInt::operator>>= (size_t shift_i)
{
size_t carry;
  units_.push_back(0);

  for (size_t i = 0; i < shift_i; i++)
  {
    carry = units_.back() & 1;
    units_.back() >>= 1;

    for (size_t j = (units_.size() - 1); (j + 1) > 0; --j)   // j must be of signed type
    {
      if (carry)
      {
        carry = units_[j] & 1;
...
const size_t push_back_size (shift_i/(sizeof (size_t) * CHAR_BIT) + 1);

  for (size_t i = 0; i < (push_back_size + 1); i++)
  {
    units_.push_back(0);
  }
  ...
  for (size_t i = 0; i < shift_i; i++)
  {
    ...
    for (size_t j = 1; j < units_.size(); j++)
    {
      if (carry)
      {
...
RossiBigInt RossiBigInt::operator& (const RossiBigInt& arg_i)
{
const size_t max_size (MAX_VAL (get_units_size (), arg_i.get_units_size ()));
...
RossiBigInt arg (arg_i);
  ...
  for (size_t i = (units_.size() - 1); (i + 1 ) > 0; --i)   // i must be of signed type
  {
...
// ------------------
RossiBigInt RossiBigInt::operator* (size_t arg_i) const
{
  return ((*this) * RossiBigInt (arg_i));
}
...
size_t i = 0;
  ...
  ret = tmp; 
  for (size_t j = 0; j < size_t(i >> 1); ret >>= 1, j++)
  {
    // Do nothing
  }
  ...
  do
  {
	...
	RossiBigInt tmp2((tmp >> size_t(1)) / ret);
	RossiBigInt ret2(ret >> size_t(1));
	if ( tmp2 <= ret2 )
		break;
    delta = tmp2 - ret2;
    ret = ret + delta;
...
{
const std::size_t HEX_SETW = sizeof(size_t) * 2;
ostringstream oss;
  ASSERTION (!units_.empty ());
  ...
  oss << hex;
  oss << units_.back();
  for (size_t i = (units_.size () - 2); (i + 1) > 0; --i) 
  {
    oss << setw (HEX_SETW) 
        << setfill ('0') 
        << units_ [i];
