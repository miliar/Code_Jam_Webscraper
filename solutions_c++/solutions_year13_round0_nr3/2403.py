...
bool isParandromic(const long x)
{
  ostringstream os;
  os << x;
  string s = os.str();
...
void listParandrome(vector<long>& ret, const long max_value)
{
...
void solve(const vector<long>& ans, long A, long B)
{
  long count = 0;
  FOREACH(it, ans) {
    if ((A <= *it) && (*it <= B))
