...
//This code below is the UPC's big integer class for programming contests
...
// }
// 
// void rec(string s, bool one, bool two) {
//   if (s.size() > 0) {
//     string r = s;
//     reverse(r.begin(), r.end());
//     check(s + r);
//     check(s.substr(0, s.size() - 1) + s[s.size() - 1] + r.substr(1, r.size() - 1));
//   }
//   if (one and two) return;
//   if (s.size() < 25) {
//     for (int i = 0; i < 3; ++i) {
//       if ((s.size() == 0 and i > 0) or s.size() > 0) {
//         if (i == 2 and s.size() > 0 and s[s.size() - 1] == '2') continue;
//         if (i == 1 and s.size() >= 5 and s.substr(s.size() - 5, 5) == "11111") continue;
//         rec(s + char('0' + i), one or (i == 1), two or (i == 2));
//       }
//     }
//   }
// }
...
/**
 * I used the commented code to precalculate all the numbers and write them sorted into a file called "tots.out"
 */

#include <fstream>

int main() {
  ifstream fin("tots.out");
  vector<BI> numbers;
  BI x;
  while (fin >> x) numbers.push_back(x);
  ...
  for (int cas = 1; cas <= casos; ++cas) {
    BI a, b;
    cin >> a >> b;
    vector<BI>::iterator it = upper_bound(numbers.begin(), numbers.end(), b);
    vector<BI>::iterator it2 = lower_bound(numbers.begin(), numbers.end(), a);
    cout << "Case #" << cas << ": " << int(it - it2) << endl;
  }
}