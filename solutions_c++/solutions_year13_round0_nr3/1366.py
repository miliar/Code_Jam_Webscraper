...
void makecandi(LL num)
{
    if(ispal(toString(num)))
        candi.pb(num);
}

void makepalin(int n,int in,LL num)
{
    if(in>n-in+1) return makecandi(num*num);
    int i;
    int st=0;
    if(in==1) st=1;
    if(in!=n-in+1)
    for(i=st;i<=9;i++)
        makepalin(n,in+1,num+pow10[in-1]*i+pow10[n-in]*i);
    else
    for(i=st;i<=9;i++)
        makepalin(n,in+1,num+pow10[in-1]*i);
}

int bs(int st,int end,LL tar)
{
    if(tar==0) return -1;
    if(st==end) return st;
    int mid=(st+end)/2;
    if(candi[mid]<=tar&&candi[mid+1]>tar) return mid;
    if(candi[mid]>tar) return bs(st,mid-1,tar);
    return bs(mid+1,end,tar);
}
...
int main()
{
    ...
    {
        cin>>a>>b;
        int x=bs(0,SZ(candi)-1,b)-bs(0,SZ(candi)-1,a-1);
        csprnt;
        print1(x);
    }
    return 0;
