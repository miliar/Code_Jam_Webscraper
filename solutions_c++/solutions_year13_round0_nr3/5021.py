...
int main (int argc, char *argv[])
{
	...
	for (int i = 1; i <= T; i++)
	{
		...
		tmp[0] = '\0';
		call_sqrt(A, sroot, tmp);
		tmp[0] = '\0';
		call_sqrt(B, eroot, tmp);

		while((strlen(sroot) < strlen(eroot)) || (strcmp(sroot, eroot) < 0))
		{
			if(is_palindrome(eroot))
			{
				result[0] = '\0';

				call_mult(eroot, eroot, result);
			...
			strcpy(tmp, eroot);
			call_minus(tmp, "1", eroot);
		}

		if(strcmp(sroot, eroot) == 0)
		{
			call_mult(sroot, sroot, result);
