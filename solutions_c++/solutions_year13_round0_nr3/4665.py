...
bool palindrome(llu n)
{
	...
	bool result = true;
	for (int i = 0; result && i < size/2; i++)
	{
		result = number[i] == number[size - i - 1];
	}
	return result;
}
...
int main(int argc, char** argv)
{
	...
	for (int testcase = 1; testcase <= t; testcase++)
	{
		...
		result = 0;
		for (llu i = min; i <= max; i++)
		{
			debug cout << "n: " << i
					   << ", palindrome: " << palindrome(i)
					   << ", raiz: " << sqrt(i)
					   << ", floor(raiz): " << floor(sqrt(i))
					   << ", palindrome(raiz): " << palindrome(sqrt(i))
					   << endl;
			if (palindrome(i) && (raiz = sqrt(i)) == floor(raiz) && palindrome(raiz))
			{
				result++;
			}
		}
