...
const ll MAX_N = 1000000000000000030ll;
...
void pre_compute(){
  ...
  for(int len=2;len<MAX_LEN;len++){
    ...
    for(set<string>::iterator it=ans.begin();it!=ans.end();++it){
      string val = *it;
      if(len + val.size() > MAX_LEN/2) continue;
      if((len + val.size()) % 2) continue;
      while(val.size() < len-2)
        val = "0" + val + "0";
      for(char c='1';c<='9';c++)
        if(check(string(1,c) + val + string(1,c)))
          ans.insert(string(1,c) + val + string(1,c));
    }
