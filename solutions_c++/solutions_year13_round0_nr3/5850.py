...
void CGoogleCodeJamSolver::FairAndSquare()
{
	CStdioFile oFile("c:\\temp.txt", CFile::modeRead|CFile::shareDenyNone);
	CStdioFile oFileOut("c:\\temp_out.txt", CFile::modeReadWrite|CFile::shareDenyNone|CFile::modeCreate);
	CString sLine;
	...
	{
		oFile.ReadString(sLine);
		int iWhiteSpace = sLine.Find(" ", 0);
		int iStart = atoi(sLine.Left(iWhiteSpace));
		int iEnd = atoi(sLine.Mid(iWhiteSpace+1, sLine.GetLength()-iWhiteSpace-1));
		int iFairSquare = 0;
		...
		CString sResponse;
		sResponse.Format("Case #%d: %d\n", iCase, iFairSquare);
		oFileOut.WriteString(sResponse);
	}
...
bool CGoogleCodeJamSolver::IsFair(int i)
{
	if(i<10)
	{
		return true;
...
bool CGoogleCodeJamSolver::IsSquare(int i)
{
	double sqrt_i = sqrt((double)i);
	int int_sqrt_i = (int) sqrt_i;
	if(int_sqrt_i*int_sqrt_i==i)
	{
		if(IsFair(int_sqrt_i))
			return true;
	}
...
void CGoogleCodeJamSolver::TicTac()
{
