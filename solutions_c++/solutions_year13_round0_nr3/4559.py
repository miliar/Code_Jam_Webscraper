...
public:
	...
	void Solve(){
		result = 0;
		for(int64 i=A, max=B; i<=max; ++i){
			if(isPalindrome(i) && isSquareOfPalindrome(i)){
				++result;
			}
		}
	}

	bool isPalindrome(int64 i){
		stringstream st;
		st << i;
		string str = st.str();
	...
	bool isSquareOfPalindrome(int64 i){
		int64 x = (int64)sqrt((double)i);
		return x * x == i && isPalindrome(x);
	}
