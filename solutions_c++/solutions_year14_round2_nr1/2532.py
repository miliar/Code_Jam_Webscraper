...
void gcjmain() {
	...
	forl(n, 0, N) cin >> lines[n];
	int ans = 0;
	vector<int> pos(N);
	int complete = 0;
	while(complete == 0) {
		char cur = lines[0][pos[0]];
		//cerr << cur << endl;
		vector<int> count(N);
		forl(n, 0, N) {
			while(cur == lines[n][pos[n]]) {
				pos[n]++;
				count[n]++;
				if(pos[n] == lines[n].size()) {
					complete++;
					break;
				}
			}
		...
		if(ans != -1) {
			int sum = 0;
			forl(n, 0, N) sum += count[n];
			int x = sum / N;
			int r = sum % N;
			if(r > N/2) x++;
			forl(n, 0, N) ans += abs(x-count[n]);
		}
	}
	if(ans < 0 || complete != N) cout << "Fegla Won" << endl;
	else cout << ans << endl;
}
