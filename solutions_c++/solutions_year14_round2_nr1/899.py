...
void Readelem(istream& in, Word& w)
{
	string s;
	in >> s;
...
bool eq(Word& w0, Word& w1)
{
	if (w0.elems.size() != w1.elems.size())
		return false;

	for (size_t i = 0; i < w0.elems.size(); ++i)
	{
		if (w0.elems[i].c != w1.elems[i].c)
			return false;
	}
	return true;
}
...
void solve(istream& in, ostream& fout, int caseIdx)
{
	...
	for (size_t i = 0; i < words.size(); ++i)
	{
		for (size_t j = i + 1; j < words.size(); ++j)
		{
			...
			if (!eq(w0, w1))
			{
				OUTANS( << CaseHead(caseIdx+1) << "Fegla Won" << endl );
				return;
			}
		}
	}

	for (size_t i = 0; i < words.front().elems.size(); ++i)
	{
		int minmove = 1000000000;
		for (size_t j = 0; j < words.size(); ++j)
		{
			int pivot = words[j].elems[i].len;
			int move = 0;
			for (size_t k = 0; k < words.size(); ++k)
			{
				move += abs(words[k].elems[i].len - pivot);
			}
	...
	OUTANS( << CaseHead(caseIdx+1) << cnt << endl );
}
