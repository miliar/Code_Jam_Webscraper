...
struct AVLPair{
	...
	T1 x; T2 y;
	AVLPair(){}
	AVLPair(const T1&xx, const T2&yy):x(xx),y(yy){}
};
template<class T>
int depth(T*){return 0;}
template<class T1, class T2>
int depth(AVLPair<T1,T2>*p){return 1+max(depth(&p->x),depth(&p->y));}
template<class T1, class T2>
const int AVLPair<T1,T2>::d = depth((AVLPair<T1,T2>*)0);

template<class T1, class T2>
AVLPair<T1,T2> operator,(const T1&x, const T2&y){
	return AVLPair<T1,T2>(x,y);
}
template<class T1, class T2, class T3>
AVLPair<T1,T2> operator,(const AVLPair<T1,T2>&x, const T3&y){
...
//#define cforeach(x, cont) for(cont::const_iterator __it = cont.begin())
...
int solve(int n, string *s){
	...
	for (i = 1; i < s[0].length(); i++){
		if (s[0][i] != s[0][i - 1])s0.push_back(s[0][i]);
	}
	vector<vector<int> > m(s0.length());
	for (i = 0; i < m.size(); i++)
		m[i].resize(n);
	for (j = 0; j < n; j++){
		s1.clear();
...
int main(){
	...
	for(i=1; i<=T; i++){
		int n,m,h;
		stringarr f;
		if (!fin.read("d>n as[n]", &n, &f)){
			printf("cannot read case #%d\n", i);
			break;
		}
		int r = solve(n, f);
		if (r>=0)fprintf(fout.f, "case #%d: %d\n", i, r);
		else fprintf(fout.f, "case #%d: Fegla Won\n", i);
	}
//	system("pause");
	return 0;
}
