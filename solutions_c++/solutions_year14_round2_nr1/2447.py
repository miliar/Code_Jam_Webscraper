...
void run(istream & fin, ostream & fout)
{
	...
	for(int t:Range(cases)) {
		vector<intv> all_cnts;
		string single;
		int strings;
		fin >> strings;
		fout << "Case #" << (t+1) << ": ";
		for( int i: Range(strings) ) {
			string in = rdline(fin), out;
			intv cnts;
			char l = 0;
			int cnt = 0;
			for( char c: in ) {
		...
		if( single.empty() )
			...
			int ans = 0;
			for( int i: Range(single.size()) ) {
				int tot = 0;
				for( intv & cnts: all_cnts )
					tot += cnts[i];
				tot = int(round(tot / double(strings)));
				for( intv & cnts: all_cnts )
					ans += abs(cnts[i]-tot);
			}
			fout << ans << endl;
