...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	FRE(e,1,casecnt) {
		...
		while (1) {
			...
			FR(i,0,N) {
				if (start[i] == l[i])
					...
					ss.insert(s[i][start[i]]);
				}
			}
			if (ss.size()>1 || (cnt>0 && cnt<N)) {
				pos = false;
				break;
			}
			if (cnt==N)
			...
			FR(i,0,N) {
				int j=0;
				for(j=start[i];j<l[i] && s[i][j]==s[i][start[i]];j++);
				v.push_back(j-start[i]);
				mx = max(mx, j-start[i]);
				start[i] = j;
			}
		...
		if(pos) {
			...
			fout<<"Case #"<<e<<": Fegla Won"<<endl;
			cout<<"Case #"<<e<<": Fegla Won"<<endl;
		}
	}
	return 0;
}