...
public:
	...
	bool operator<(const Point<T>& b) const { return (x < b.x); }
};

bool binarySearch(vector<Point<double> >& base, const double& time, const double& D)
{
	bool can = true;
	double start = -INF;
	
	for(int i=0; i<base.size() && can; ++i)
	{
		for(int k=0; k<base[i].y && can; ++k)
		{
			if ( start + D <= base[i].x + time)
			{
				start = max(start + D, base[i].x - time);
			}
			else can = false;
		}
	}
...
void init()
{
	...
	for(int q=1; q<=t; ++q)
	{
		int C, D;
		cin >> C >> D;
		vector<Point<double> >base;
		for(int i=0; i<C; ++i)
		{
			double a,b;
			cin >> a >> b;
			base.push_back( Point<double>(a,b) );
		}
		sort(base.begin(), base.end());
		
		double left = 0-eps, right = 1e11;
		while(left+eps < right)
		{
			double mid = (right-left)/2 + left;
			bool can = binarySearch(base, mid, D);
			
			if (can) right = mid;
			else left = mid;
		}
		
		cout << "Case #" << q << ": " << fixed << setprecision(7) << right +eps<< endl;
	}
}
