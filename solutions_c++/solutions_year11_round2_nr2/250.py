...
try {
  ...
  {
    map<int, int> C;
    for (int i = 0; i < 1000000; ++i) ++C[rand() % 200001 - 100000];
    printf("%u 1000000\n", C.size());
    for (map<int, int>::const_iterator it = C.begin(), end = C.end(); it != end; ++it) {
      printf("%d %d\n", it->first, it->second);
    }
  }
  return 0;
  ...
  WhileTests() {
    ...
    sort(pos, pos + N);
    int D2 = D * 2;
    i64 low = 0, high = 3000000000000LL;
    while (low < high) {
      i64 mid = (low + high) / 2;
      i64 last = pos[0] - mid;
      bool ok = true;
      for (int i = 1; ok && i < N; ++i) {
        i64 next = last + D2;
        if (abs(pos[i] - next) > mid) {
          int dir = pos[i] < next ? 1 : -1;
          next = pos[i] + dir * mid;
        }
        ok = next - last >= D2;
        last = next;
