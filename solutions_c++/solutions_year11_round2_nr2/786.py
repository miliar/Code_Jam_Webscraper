...
int CodeJamMain()
{
    ...
    for(int caseNum=0;caseNum<T;caseNum++)
    {
        ...
        // for small, simulation bruteforce
        ...
        for(int c=0;c<C;c++)
        {
            ...
            input>>P;
            input>>V;
            for(int i=0;i<V;i++) positionsx2[v++] = 2*P;
        }
        ...
        bool satisfied = true;
        for(int i=1;i<numVendors;i++) if(positionsx2[i]-positionsx2[i-1]<D*2) satisfied = false;

        while(!satisfied)
        {
            for(int i=0;i<numVendors;i++) positionsx2old[i] = positionsx2[i];

            for(int i=0;i<numVendors;i++)
            {
                if(i==0 || positionsx2old[i] - positionsx2[i-1] >= 2*D) positionsx2[i]--;
                else positionsx2[i]++;
            }
        ...
        double totalTime = (double)totalTimex2/2.0;
        out<<totalTime;
