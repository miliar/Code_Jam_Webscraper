...
long double testcase_itr(ll D, ll N, vector<ll>&Ks, vector<ll>&Ss)
{
    // solution start
    long double maxt = 0;
    for(ll i=0;i<N;i++){
        long double t = (long double)(D-Ks[i])/(long double)Ss[i];
        if(t > maxt){
            maxt = t;
        }
    }
    
    long double sp = (long double)D / maxt;
...
void testcase(istream &in)
{
    ...
    for(int i=0;i<T;i++){
        ...
        auto ret = testcase_itr(D, N, Ks, Ss);
