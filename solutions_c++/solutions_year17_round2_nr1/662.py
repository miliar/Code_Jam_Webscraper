...
string problem = "/Users/alanpierce/codejam17/Round1B/A-large";
FILE *infile;
FILE *outfile;
...
string handle_case() {
    lld dest = next_lld();
    int num_horses = next_int();

    double max_time = 0;
    for (int i = 0; i < num_horses; i++) {
        lld position = next_lld();
        lld speed = next_lld();
        max_time = max(max_time, double(dest - position) / double(speed));
    }

    stringstream ss;
    ss.precision(15);
    ss << double(dest) / max_time;
    return ss.str();
}
