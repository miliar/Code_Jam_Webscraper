...
double getT(double d,double s){
    return d/s;
}

double getD(double k1,double s1,double k2,double s2)
{
    return (s1*k2-s2*k1)/(s1-s2);
}
...
int main()
{
    ...
    while(t--){
        ...
        for(int i=n-1;i>=0;i--)
        {
            double t=0,p=val[i].fs;
            ll s=val[i].sc;
            mt=max(mt,getT(D-p,s));
            for(int j=i+1;j<n;j++)
            {
                if(s>val[j].sc)
                {
                    double ND=getD(p,s,val[j].fs,val[j].sc);
                    if(ND>D) continue;
                    t+=getT(ND-p,s);
                    p=ND;
                    s=val[j].sc;
