...
void solve(int qqq)
{
    t d,n;
    cin>>d>>n;
    double last_beerkezo=0;
for (int i=0;i<n;i++)
{
    t id,is;
    cin>>id>>is;
    double beerkezes=(double)(d-id)/(double)is;
    if (qqq==67) d(d,id,is,beerkezes);
    last_beerkezo=max(last_beerkezo,beerkezes);
}
double speed=(double)d/(double)last_beerkezo;
if (qqq==67) d(d,last_beerkezo);
//cout<<speed<<"\n";
//printf("%.6lf\n",speed);
printf("%.8lf\n",speed);
