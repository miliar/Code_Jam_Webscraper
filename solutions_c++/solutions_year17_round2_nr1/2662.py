...
int T, N;
int maxim_speed[SIZE];
double max_Ttime, answer;
LL D;
LL start_pos[SIZE];
...
int main()
{
    ...
    FOR(t, 1, T)
    {
        ...
        REP(i, 0, N)
        {
            scanf("%lld%d", start_pos+i, maxim_speed+i);
            max_Ttime = max(max_Ttime, ((double)D - (double)start_pos[i]) / (double)maxim_speed[i]);
        }
        
        answer = (double)D / max_Ttime;
        printf("Case #%d: %.10lf\n", t, answer);
    }
