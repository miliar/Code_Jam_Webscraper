...
public:
  ...
  void Run(uint32 T) {
    auto horses = ReadSequence<uint32_pair>(std::cin, N);
    auto times = as_vector(map(horses, [this](const uint32_pair& pair) {
      return float_type(D - pair.first) / float_type(pair.second);
    }));
    auto maximum = *std::max_element(times.begin(), times.end());

    print("%2%3Case #%0: %1", T + 1, D / maximum, std::fixed, std::setprecision(20));
  }
