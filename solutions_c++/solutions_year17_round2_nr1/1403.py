...
void solve(istream &cin, ostream &cout, int test_number = -1) {
    ...
    forn(i, n - 1) {
        cin >> x[i] >> v[i];
        a.emplace_back(x[i], v[i]);
    }
    x[n - 1] = 0;
    v[n - 1] = INFLL;
    a.emplace_back(x[n - 1], v[n - 1]);
    ...
    V[0] = v[0];
    FOR(i, 1, n) {
        V[i] = min((ld)v[i], (ld)V[i - 1] * (d - x[i]) / (d - x[i - 1]));
    }

    cout.precision(30);
    cout << "Case #" << test_number << ": " << V[n - 1] << endl;
...
//
//        t[i] = (dest - x[i]) /
//        ld pos1 = (ld) (x[j] - x[i]) / (v[i] - v[j]) * v[i] + x[i];
//        ll pos = (x[j] - x[i]) * v[i] + x[i] * (v[i] - v[j]);
//        if (pos < dest * (v[i] - v[j])) {
//            t[i] =
//        }
//    }
