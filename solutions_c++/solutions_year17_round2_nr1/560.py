...
bool check(DB sp){
    for (int i = 0 ; i < n ; ++i){
        // (D - horse[i].fi) / horse[i].se <= D / sp
        if (dblcmp(D * horse[i].se - sp * (D - horse[i].fi)) < 0) return false;
    }
    return true;
}
void solve(){
    ...
    for (int i = 0 ; i < n ; ++i) {
            horse[i].fi = RD();
            horse[i].se = RD();
            //cerr << horse[i].fi << ' ' << horse[i].se << endl;
    }
    DB low = 0 , high = 1e14 , ans = 0 , mid;
    do{
        mid = (low + high) / 2;
        if (check(mid)){
            low = mid + eps;
    ...
    }while((high - low >= eps) && (high - low >= low * eps));
