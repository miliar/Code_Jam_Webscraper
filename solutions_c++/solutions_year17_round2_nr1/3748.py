...
int main()
{
    ...
    while(t--)
    {
    	...
    	ifs>>a[i].F>>a[i].S;
    	sort(a,a+n);
    	double time=(double)(d-a[n-1].F)/(a[n-1].S*1.0);
    	ll speed=a[n-1].S,dist=a[n-1].F;
    	//dbg(time);
		double pret=time;
    	reff(i,n-2,0)
    	{
    		...
    		{
    			ll tm=a[i].S-speed;
    			double tmtime=(double)(dist-a[i].F)/(tm*1.0);
    			double ndist=(double)tmtime*a[i].S;
    			ndist+=a[i].F;
    			if(ndist>d)
    			{
    			tmtime=(double)(d-a[i].F)/(a[i].S*1.0);
				time=max(tmtime,time);
    			speed=a[i].S;
    			dist=a[i].F;
    			pret=tmtime;
				}
				...
				{
    			//dbg1(tmtime,ndist);
    			tmtime+=(double)(d-ndist)/(speed*1.0);
    			time=max(tmtime,time);
    			speed=a[i].S;
    			dist=a[i].F;
    			pret=tmtime;
			...
			{
				double tmtime=(double)(d-a[i].F)/(a[i].S*1.0);
				time=max(tmtime,time);
    			speed=a[i].S;
    			dist=a[i].F;
    			pret=tmtime;
