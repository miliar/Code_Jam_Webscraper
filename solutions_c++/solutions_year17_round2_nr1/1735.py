...
int main() {
  ...
  FOR (test, 1, T+1) {
    ll D, N;
    cin >> D >> N;
    vector<pair<ll,ll> > vec(N, make_pair(0,0));
    FOR(i,0,N) {
      ll ki, si;
      cin >> ki >> si;
      vec[i] = make_pair(ki,si);
    }
    sort (all(vec));
    int next = N-1;
    for (int i = N-2; i >= 0; i--) {
      if ((D - vec[i].first) * vec[next].second > (D - vec[next].first) * vec[i].second) { // si i ne rattrape pas next
	next = i;
      }
      
    }
    double sol = (double) (D * vec[next].second) / (double) (D - vec[next].first);
