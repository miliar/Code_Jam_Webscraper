...
ld calcTime(ll D, ll K, ll S) {
	ld time = (D-K)/(ld)S;
	return time;
}

typedef ld otype;
otype calcFunction(ll D, ll N, v(ll) Ks, v(ll) Ss) {
	otype result=0;
	ld maxTime = 0;
	for(int i=0; i<N; i++) {
		ld curTime = calcTime(D, Ks[i], Ss[i]);
		if(curTime > maxTime) {
			maxTime = curTime;
		}
	}
...
int main() {
	...
	fore(test, 1, tests){
		...
		otype result=calcFunction(D, N, horsesK, horsesS);
		outfile << "Case #" << test << ": ";
		cout << "Case #" << test << ": ";
#ifndef IFSTRUCT
		outfile << result << endl;
