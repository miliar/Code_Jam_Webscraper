...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	FRE(e,1,casecnt)
	{
		...
		if(t/sum > it)
		{
			...
			for(int i=0; i<n%c; i++)
				sum += 2*C[i];
			fout<<"Case #"<<e<<": "<<sum<<endl;
			cout<<"Case #"<<e<<": "<<sum<<endl;
			continue;
		}
		...
		for(int i=0; i<(n%c); i++)
			cnt[i]++;
		for(int i=0; i<c; i++)
			cnt[i] += (n/c - (it+1)) > 0 ? (n/c - (it+1)) : 0;

		vector<pair<I, I> > v;
		FR(i,0,c)
		{
			v.push_back(pair<I,I>(C[i],cnt[i]));
		}
		if(su!=-1)
			v.push_back(pair<int,int>(su,1));
		sort(v.begin(), v.end());
		reverse(v.begin(), v.end());
		for(int i=0; i<v.size() && l!=0; i++)
		{
			if(v[i].second<=l)
			{
...
{
	int casecnt;
	fin.open("F:\\Preparation\\GCJ\\C-small-attempt1.in",ios::in);
	fout.open("F:\\Preparation\\GCJ\\C-small-attempt1.out",ios::out);
	if(!fin.good())
	{
		cout<<"Input file not opened"<<endl;
		exit(-1);
