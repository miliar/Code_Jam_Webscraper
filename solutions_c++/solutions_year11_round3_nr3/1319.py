...
#define PRINT1D(x,n){cout<<endl; for(int kk=0;kk<n;kk++) cout<<setw(7)<<x[kk]; cout<<endl;}
...
{
        int i=0;
        for(int j=0; j<s.length(); j++) if(s[j] == c) i++;
        return i;
}
vector<string> ex2v(string s, char c)
{
        vector<string> vs;
        string tmp="";
