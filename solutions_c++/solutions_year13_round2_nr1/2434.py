...
void sfileIO(){freopen("A-small-attempt3.in","r",stdin); freopen("A-small-attempt3.out","w",stdout);}
...
/***********************************main*****************************************/
int MAX_MOTE = 100;
vector<int> numOfMotes(int curr, int req){
    if(curr == 1)
    {
        vector<int> r;
        r.PB(-1);
...
int main(int argc, char** argv)
{
   sfileIO();
    ...
    for(int eachCase = 0; eachCase < totalCases; eachCase++){
        
        int myVal, totalMote;
        cin>>myVal>>totalMote;
        vector<int> motes(totalMote);
        vector<int> ans(totalMote, 0);
        for(int i=0;i<totalMote;i++){
            cin>>motes[i];
        }
        SORT(motes);
        int ret = 0;
        bool DONE = false;
        for(int i=0;i<totalMote && !DONE;i++){
           // cout<<endl<<"--"<<motes[i]<<"  "<<myVal<<endl;
            if(myVal>motes[i]){
                myVal += motes[i];
            } else {
                vector<int> q = numOfMotes(myVal, motes[i]);
                //cout<<endl<<"-!-"<<q[i]<<" "<<totalMote-i<<endl;
                if(q[1] > totalMote - i){
                    ret = ret + totalMote - i;
                    DONE = true;
                }
