/************************************************************************************************************
	...
	CREATION DATE : Sat 04 May 2013 09:39:07 PM IST

	LAST MODIFIED : Sat 04 May 2013 11:31:15 PM IST
...
#define LS(a,x) ({int z=(lower_bound(ALL(a),x)-(a).begin());z;})
...
int solve(int s,int pos,int acost){
	if(pos==SZ(mote))
		return acost;
	int &f=dp[acost][pos];
	if(f!=-1)
		return f;
	if(s>mote[pos])
		return solve(s+mote[pos],pos+1,acost);
	else{
		int c=0;
		if(s!=1){
			while(s<=mote[pos]){
			...
			f=min(solve(s+mote[pos],pos+1,acost+c),acost+SZ(mote)-pos);
			return f;
		}
		f=acost+SZ(mote)-pos;
		return f;
