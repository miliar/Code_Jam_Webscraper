...
int approach				(task& t, int idx, long long A, int count)
{
	if (count > t.list.size())	return t.list.size();
	if (idx == t.list.size())	return count;

	if (A > t.list[idx]) {
		A					+= t.list[idx];
		return				approach(t, idx+1, A, count);
	}

	int f1					= approach(t, idx+1, A, count+1);
	int f2					= approach(t, idx, 2 * A -1, count+1);
	return min(f1, f2);
}

void execute_task			(task& t)
{
	::sort					(t.list.begin(), t.list.end());
	int count				= approach(t, 0, t.A, 0);
	stringstream ss;
	ss << count;
	t.result				= ss.str();
}
...
bool read_task				(task& t, ifstream& file)
{
	...
	{
		stringstream ss		(buffer);
		for (int i=0; i<(int)t.list.size(); ++i) {
			ss >> t.list[i];
		}
	}
