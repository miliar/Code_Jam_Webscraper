...
void ProcessFile(FILE* fin)
{
	...
	for (int i=0; i<T; ++i)
	{
		...
		int iSteps=0;
		for (int j=0, iStepsToFinish = N; j<N; ++j, --iStepsToFinish)
		{
			int iStepsRequired=0;
			for (;O[j]>=A && iStepsRequired<iStepsToFinish; ++iStepsRequired)
				A += A-1;
			iSteps += iStepsRequired;
			if (iStepsRequired==iStepsToFinish)
				break;
			A += O[j];
		}
		delete []O;
