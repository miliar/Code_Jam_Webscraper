...
int A, N, a[ 1000005 ], dp[ 105 ][ 1000010 ];

const int MAX_SIZE = 1000005;

int DP ( int id, int size )
{
	if ( id == N ) return 0;
	if ( dp[ id ][ size ] != -1 ) return dp[ id ][ size ];

	int best = INF;
	if ( size > a[ id ] )
	{
		best = DP( id + 1, min( MAX_SIZE, size + a[ id ] ) );
	}
	else
	{
		best = min( best, 1 + DP( id + 1, size ) );
		if ( size - 1 > 0 )
			best = min( best, 1 + DP( id, min( MAX_SIZE, size + (size-1) ) ) );
	}

	return dp[ id ][ size ] = best;
}
