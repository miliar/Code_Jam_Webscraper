...
struct C {
    ...
    friend int intersection(const C& c1, const C& c2, P& p1, P& p2) { //圆与圆相交(相切)
        double d = dist(c1.o, c2.o);
    ...
    friend int intersection(const C& c, line& l, P& p1, P& p2) { //圆与直线是否相交(相切)
...
int qiu()
{
	...
	while(!que.empty())
	{
		...
		e=que.top();
		que.pop();
		//cout<<e.i<<"  "<<e.l<<"  "<<d[e.i]<<"  "<<m<<endl;
		if(e.l+d[e.i]>=m)return 1;
		for(int i=e.i+1;i<n;i++)
		{

		
			if(d[i]-d[e.i]>e.l)break;
			t.l=min(d[i]-d[e.i],l[i]);//	cout<<t.l<<endl;
			if(t.l>l[i]||t.l<=lx[i])continue;
			lx[i]=t.l;
			t.i=i;
			que.push(t);
		}
