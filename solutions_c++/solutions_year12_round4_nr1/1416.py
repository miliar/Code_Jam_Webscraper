...
{	
	...
	for(int q=1; q<=T; ++q)
	{
		...
		for(int i=0; i<N; ++i) 
		{
			scanf("%d %d", &vec[i].x, &vec[i].y); //d, l
		}
		
		int TG; cin >> TG;
		
		queue<Point<int>> que;
		que.push(Point<int>(0, vec[0].x)); 	// pos, R
		
		bool finish = false;
		while(que.empty() == false && !finish)
		{
			int p = que.front().x;
			int r = que.front().y;
			que.pop();
			...
			for(int i=p+1; i<vec.size() && vec[i].x <= vec[p].x+r && !finish; ++i)
			{
				que.push(Point<int>(i, min(vec[i].x - vec[p].x, vec[i].y)));
			}
		}
		
		printf("Case #%d: %s%s", q, finish ? "YES":"NO", q == T ? "":"\n");		
	}
}
