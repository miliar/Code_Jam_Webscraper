...
int main(void) {
	...
	REP(__number_of_case, __number_of_cases) {
        ...
        FOR(i, 1, N-1) {
            //cout << best << '\n';
            REP(j, i) {
                if (data[i].first - data[j].first <= best[j]) {
                    remax(best[i], min(data[i].first - data[j].first, data[i].second));
                }
            }
        }
        //cout << best << '\n';
        bool ok(false);
        REP(i, N) {
            if (best[i] >= 0 && best[i]+data[i].first >= D) {
                ok = true;
                break;
            }
        }
		cout << "Case #" << __number_of_case+1 << ": " << (ok ? "YES" : "NO") << '\n';
	}
}
