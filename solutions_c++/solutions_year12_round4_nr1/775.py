...
void solve(int CaseNum){
	...
	while(!q.empty()){
		...
		LET(lbd,lower_bound(BE(d),d[pos]-m[pos]));
		LET(ubd,upper_bound(BE(d),d[pos]+m[pos]));


		for(TYPE(ubd) it=lbd;it!=ubd;++it){

			int idx=it-BG(d);

			int v=min(abs(d[idx]-d[pos]),l[idx]);
