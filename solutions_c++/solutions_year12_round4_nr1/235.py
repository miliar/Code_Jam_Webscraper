...
void solve(){
        ...
        while (true){
            ...
            int j, p = -1, t = -1; FOR_N(j, i+1, n) if (d[j] - d[i] <= len){

                //cout << j << ":" << d[j] << endl;

                //if (d[j] - d[i] <= l[j]){

                    if (d[j] + min(l[j], d[j] - d[i]) >= t){
                        p = j;
                        t = d[j] + min(l[j], d[j] - d[i]);
                    }
                //}
            }
...
bool solve2(){
    ...
    REP(i, n){
        ...
        if (d[i] + dp[i] >= L) return true;

        FOR(j, i+1, n) if (d[i] + dp[i] >= d[j]){
...
int main(){
    ...
    Rush{
        ...
        RD(n); REP(i, n) RD(d[i], l[i]); RD(L);

        printf("Case #%d: ", ++____Case);

        cout << (solve2() ? "YES" : "NO") << endl;
