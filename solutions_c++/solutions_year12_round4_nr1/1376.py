...
bool tryIt(int cRope, ll cLen) {
	if(cRope == nRopes - 1) return true;
	int mxMove = upper_bound(dist, dist + nRopes, dist[cRope] + cLen) - dist - 1;
	ROFba(nxtRope, mxMove, cRope + 1) {
		ll nxtLen = min( dist[nxtRope] - dist[cRope] , length[nxtRope] );
		if(tryIt(nxtRope, nxtLen)) return true;
	}
	return false;
}
