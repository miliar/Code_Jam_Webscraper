...
//END TEMPLATE_BY_ACRUSH_TOPCODER

#include<fstream>
#include<stdio.h>
#include<string.h>

int N,D,d[10000],l[10000];
int r;
char b[10000];
...
void main()
{
	...
	out=fopen("A.out","w+");
	in=fopen("A.in","r");
	int nTestCase,nTestCaseCounter;
//	char line[32];

//	fgets(line,32,in);
//	sscanf(line,"%d",&nTestCase);
	fscanf(in,"%d",&nTestCase);

	int i,j,flag;

	for(nTestCaseCounter=1;nTestCaseCounter<=nTestCase;nTestCaseCounter++)
	{
		cout<<nTestCaseCounter<<':'<<endl;
...
//		fgets(line,32,in);
		...
		for(i=0,r=d[0]+l[0];i<N && d[i]<r && r<D;i++)
