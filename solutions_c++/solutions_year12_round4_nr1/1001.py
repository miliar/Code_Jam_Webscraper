...
int main()
{
  ...
  for (int i = 0; i < 30; ++i)
  {
    set<int> vals;
    int const N = 10000;
    int const D = 1000000000;
    int const F = 1000000000 - 10000 - 1;
    printf("%d\n", N);
    for(int i = 0; i < N; ++i)
      printf("%d %d\n", F + i, D - 1);
    printf("%d\n", F + N);
...
void ZyxGCJ_Solve(int current_test_case)
{
  ...
  lowest[0] = d[0];
  bool done = false;
  for (int i = 0; i < N; ++i) if (lowest[i] >= 0)
  {
    for (int j = i + 1; j <= N; ++j)
    {
      if (d[i] + lowest[i] < d[j])
        break;
      int size = min(d[j] - d[i], s[j]);
      lowest[j] = max(lowest[j], size);
    }
  }
  printf("%s\n", lowest.back() == -1 ? "NO" : "YES");
}
////////////////////////// Solution ///////////////////////////