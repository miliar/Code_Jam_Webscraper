...
int n;
pair<long long ,long long> p[20000];
long long f[20000];
long long D;
void solve() {
    FR(i,n) f[i] = -1;
    ...
    for(int i = 1; i <= n-1; i++) {
        for(int j = 0; j < i; j++) {
            //
            long long dist = p[i].first - p[j].first;
            if (f[j] >= dist) {
                f[i] = max(f[i], min(dist, p[i].second));
            }
        }
    }
    FR(i,n) {
        if (f[i] != -1 && f[i] + p[i].first >= D) {
            cout << "YES" << endl;
            return;
        }
    }
...
int main() {
    ...
    FOR(test,1,ntest) {
        cout << "Case #" << test << ": ";
        cin >> n;
        FR(i,n) cin >> p[i].first >> p[i].second;
        cin >> D;
        solve();
    }
    return 0;
