...
int main(){
	...
	while(t--){
		...
		n=GI;
		REP(i,n)	cin>>W[i]>>V[i];
		int ans=0,curA=1,curB=1,resA=0,resB=0;
		REP(i,n){
			if(W[i]=='O'){
				if(resA>=abs(V[i]-curA))	resA=abs(V[i]-curA);
				ans+=1+abs(V[i]-curA)-resA;
				resB+=1+abs(V[i]-curA)-resA;
				resA=0;
				curA=V[i];
			}
			else{
				if(resB>=abs(V[i]-curB))	resB=abs(V[i]-curB);
				ans+=1+abs(V[i]-curB)-resB;
				resA+=1+abs(V[i]-curB)-resB;
				resB=0;
				curB=V[i];	
			}
		}
