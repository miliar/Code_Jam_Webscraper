...
int main(void){
	...
	forn(cas,t){
		...
		II(n){
			...
			int turn;
			if(s[0]=='O'){
				if(abs(place-a)<=aa)turn=1;
				else turn=abs(place-a)-aa+1;
				a=place;aa=0;bb+=turn;
			}else if(s[0]=='B'){
				if(abs(place-b)<=bb)turn=1;
				else turn=abs(place-b)-bb+1;
				b=place;bb=0;aa+=turn;
			}
			cnt+=turn;
		}
		printf("Case #%d: %d\n",cas+1,cnt);
