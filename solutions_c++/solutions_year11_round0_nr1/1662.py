...
// 	#define sal cout
	...
	forn( ncase, maxcase ) {
		...
		tint sol = 0, n, aux, poso = 1, posb = 1;
		char c;
		...
		forn( i, n ) {
			int d;
			if( which[i] ){
				sol += d = abs( poso - o[i] ) + 1;
				poso = o[i];
				if ( abs( posb - b[i] ) <= d ) posb = b[i];
				else posb += ( posb < b[i] ) ? d : -d;
			} else {
				sol += d = abs( posb - b[i] ) + 1;
				posb = b[i];
				if ( abs( poso - o[i]) <= d ) poso = o[i];
				else poso += ( poso < o[i] ) ? d : -d;
			}
		}
