...
void solve(const vector<int>& vo, const vector<int> &vb, const vector<char>& vc)
{
  ...
  int po = 1;
  int pb = 1;
  vector<int>::const_iterator io = vo.begin();
  vector<int>::const_iterator ib = vb.begin();
  
  for(vector<char>::const_iterator ic = vc.begin(); ic != vc.end(); ++ic)
  {
    if(*ic == 'O')
    {
      for(; po != *io; po < *io ? ++po : --po)//move
      {
        ++seconds;
        if(ib != vb.end() && pb != *ib)
          pb <* ib? ++pb : --pb;
