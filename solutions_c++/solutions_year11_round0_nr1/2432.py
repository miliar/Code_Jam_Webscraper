...
ostream &operator<<(ostream & out, const pair<T,S> &pr) { return out << "(" << pr.first << "," << pr.second << ")";}
//END CUT - Print Array
...
queue<int> nextO, nextB;
queue<pair<int,char> > next;

int solve(){
   int ret = 0, cO = 1, cB = 1;
   while(next.size() > 0){
//      cerr << ret << ": " << cO << " " << cB << " " << next.front() << endl;
      if(nextO.size() == 0){
	while(nextB.size() > 0){
	   ret += abs(nextB.front()-cB)+1;
	   cB = nextB.front(); nextB.pop(); next.pop();
	}
	break;
      }
      else if(nextB.size() == 0){
	while(nextO.size() > 0){
	   ret += abs(nextO.front()-cO)+1;
	   cO = nextO.front(); nextO.pop(); next.pop();
	}
	break;
      }
      else if(next.front().second == 'O'){
	  int dO = abs(nextO.front() - cO) + 1;
	  cO = nextO.front(); nextO.pop(); next.pop();
	  if(abs(nextB.front() - cB) <= dO) cB = nextB.front();
	  else if(nextB.front() > cB) cB += dO;
	  else                        cB -= dO;
	  ret += dO;
      }
      else{
	  int dB = abs(nextB.front() - cB) + 1;
	  cB = nextB.front(); nextB.pop(); next.pop();
	  if(abs(nextO.front() - cO) <= dB) cO = nextO.front();
	  else if(nextO.front() > cO) cO += dB;
	  else                        cO -= dB;
	  ret += dB;
      }
   }
...
int main(){
  ...
  FOR(i,0,_T){
    ...
    while(!nextB.empty()) nextB.pop();
    while(!next.empty())  next.pop();

    int cnt, num; char which;
    cin >> cnt;
    FOR(t,0,cnt){
	cin >> which >> num;
	next.push(make_pair(num,which));
	if(which == 'O') nextO.push(num);
	else             nextB.push(num);
    }
    cout << solve();
    cout << endl;
