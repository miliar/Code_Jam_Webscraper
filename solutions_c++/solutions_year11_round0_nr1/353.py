...
// Analyse/conversion d'un caract̬re/chaine
...
int main (int argc, char* argv[]) {
  ...
  for (int test=0; test<nbTests; test++) {
    int n, nb, no, m; char c;
    int blue[102], orange[102];
    char tour[102];
    nb=0;
    no=0;
    in >> n;
    ...
    int sec=0; int io=0, ib=0;
    int poso=1; int posb=1;
    bool av; int i=0;

    while(i < n) {
      av = false;
      if(tour[i] == 'O' && poso == orange[io]) {
	av = true;
        c = 'O';
      }
      if(tour[i] == 'B' && posb == blue[ib]) {
	av = true;
        c = 'B';
      }
      ...
      if(poso > orange[io])
	poso--;
      else if (poso < orange[io])
	poso++;
      
      if(posb > blue[ib])
	posb--;
      else if (posb < blue[ib])
	posb++;
