...
public:
	...
	void GetInput(istream& in){
		...
		for(int i=0; i<n; ++i){
			in >> c >> num;
			input.push_back(pair<char, int>(c, num));
		}
	}
	//--------------------------------------------------------------------//
	//     Output for Debug
	...
	void DebugOutput(ostream& out) const{
		for(uint i=0; i<input.size(); ++i){
			out << input[i].first << ": " << input[i].second << endl;
		}
	}
	//--------------------------------------------------------------------//
	//     Solve
	...
	void Solve(){
		...
		for(uint i=0; i<input.size(); ++i){
			if(input[i].first == 'O'){
				int time = abs(input[i].second - pos_o);
				result += max(time - (result - time_o), 0) + 1;
				time_o = result;
				pos_o = input[i].second;
			}
			else{
				int time = abs(input[i].second - pos_b);
				result += max(time - (result - time_b), 0) + 1;
				time_b = result;
				pos_b = input[i].second; 
			}
		}
	}
