...
#define SWAP(a,b,Type) {Type _t;_t=(a);a=(b);b=_t;} //Add by Luo
void CStrReverse(char* src,int nLen){for (int i=0;i<nLen/2;i++)SWAP(src[i],src[nLen-i-1],char);}//Add by Luo
...
int main()
{
	...
	//freopen("..\\A-small-attempt4.in","r",stdin);freopen("..\\A-small4.out","w",stdout);
	...
	for (i=0;i<N;i++)
	{
		...
		for(j=0;j<K;j++)
		{
			...
			case 'O':
				...
				dt =  abs(pos-CurPosO)+1;
				if( preMove == 0) //上一个移动的是O
				{
					StepO += dt;
					t += dt;	//直接累加时间
				}
				...
				{
					t += max( abs(pos-CurPosO)-StepB, 0)+1;
					StepO =max( abs(pos-CurPosO)-StepB, 0)+1;
				}
				CurPosO = pos;
			...
			case 'B':
				...
				dt =  abs(pos-CurPosB)+1;
				if( preMove == 1) //上一个移动的是B
				{
					StepB += dt;
					t += dt;	//直接累加时间
				}
				...
				{
					t += max( abs(pos-CurPosB)-StepO, 0)+1;
					StepB = max( abs(pos-CurPosB)-StepO, 0)+1;
				}
				CurPosB = pos;
