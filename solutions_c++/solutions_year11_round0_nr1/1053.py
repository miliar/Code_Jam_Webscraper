...
int main()
{
	...
	int c;
	cin>>c;
	pair<long long ,long double> arr[200];
	long long maxl=0;
	for(int cc=0;cc<c;cc++)
	{
		int n,no;
		char c;
		int b[200],o[200],seq[200];
		b[0]=1,o[0]=1;
		int bno=0,ono=0,tot=0;
		cin>>n;
		rep(i,n)
		{
			cin>>c>>no;
		...
		rep(tc,tot)
		{
			if(seq[tc]==0)
			{
				int th=abs(b[bc]-bpos)+1;
				t+=(th);
				int movDis=min( abs(o[oc]-opos),th);
				if(o[oc]<opos)
					opos-=movDis;
				else
					opos+=movDis;
			...
			{	
				int th=abs(o[oc]-opos)+1;
				t+=(th);
				int movDis=min( abs(b[bc]-bpos),th);
				if(b[bc]<bpos)
					bpos-=movDis;
				else
					bpos+=movDis;
