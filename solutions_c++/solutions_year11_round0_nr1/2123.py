...
int main() {
    ...
    for (int qq = 0; qq < t; qq++) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            int val; std::cin >> val;
            int rob = (ch == 'O') ? 0 : 1;
            vp.push_back(make_pair(rob, val));
        }
        int res = 0;
        int cur_pos1 = 1;
        int cur_pos2 = 1;
        ...
        for (int i = 0; i < vp.size(); i++) {
            if (!vp[i].first) {
                int time = max((abs(vp[i].second - cur_pos1) - sec_na1), 0) + 1;
                res += time;
                cur_pos1 = vp[i].second;
                sec_na1 = 0;
                sec_na2 += time;
            } else {
                int time = max((abs(vp[i].second - cur_pos2) - sec_na2), 0) + 1;
                res += time;
                cur_pos2 = vp[i].second;
                sec_na1 += time;
                sec_na2 = 0;
            }
        }
        std::cout << "Case #" << qq + 1 << ": " << res << std::endl;
    }
    return 0;
}
