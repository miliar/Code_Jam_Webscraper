...
# define repin(i, m, n)     for( int i = m; i < n; i++)
...
int main()
{
	...
	repin ( testi, 1, test + 1)
	{
		...
		rep ( i, n )
		{
			...
			{
				int one = abs(p - boletop);
				int two = ( totalt - btime );
				if ( one <= two )
				{
					totalt += 1;
					boletop = p;
		...
		out << "Case #" << testi << ": " << totalt << endl;
	}
	printf("I think I have solved it now!!!! Yipeee\n");
	system("pause");
	out . close();
	return 0;
}
