...
int main()
{
	...
	REP (tt, t) {
		...
		int bPos = 1, oPos = 1, bi = 0, oi = 0;
		
		int res = 0;
		EACH (l, L) {
			int move;
			if (*l == 'B') {
				move = abs(bPos - B[bi]) + 1;
				bPos = B[bi++];
				if (oPos < O[oi]) {
					oPos += min(move, O[oi] - oPos);
				} else {
					oPos -= min(move, oPos - O[oi]);
				}
			} else {
				move = abs(oPos - O[oi]) + 1;
				oPos = O[oi++];
				if (bPos < B[bi]) {
					bPos += min(move, B[bi] - bPos);
				} else {
					bPos -= min(move, bPos - B[bi]);
				}
			}
			res += move;
		}
