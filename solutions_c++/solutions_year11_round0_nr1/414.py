...
void solveCase() {
  ...
  char col[200][10];
  int pos[200];
  for(int a=0; a<N; a++) scanf("%s%d", &col[a], &pos[a]);
  
  int co = 1, cb = 1, tt = 0;
  
  int ap = 0;
  for(int a=0; a<N; a++) { 
    // if(cnum == 3) printf("%d, %d <%s%d>", co, cb, col[a], pos[a]);
    while(col[ap][0] == col[a][0] && ap < N) ap++;
    int crt;
    if(col[a][0] == 'O') {
      crt = abs(pos[a] - co) + 1; co = pos[a];
      if(ap<N) moveother(cb, pos[ap], crt);
      }
    else {
      crt = abs(pos[a] - cb) + 1; cb = pos[a];
      if(ap<N) moveother(co, pos[ap], crt);
      }
    if(0) if(cnum == 3) {
      printf(" -> %d %d (%d)\n", co, cb, crt);
      }
