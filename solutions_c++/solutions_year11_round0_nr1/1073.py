...
__int64 doit( filein& fin )
{
   ...
   for ( int i = 0; i< N; i++ )
   {
      player.push_back( fin.readStr() == "O" ? 1 : 0 );
      pos.push_back( fin.readInt() );
   }
   ...
   for ( int i = 0; i < N; i++ )
   {  
      t[player[i]] += abs(pos[i]-p[player[i]])+1;
      t[player[i]] = max(t[player[i]], t[1-player[i]]+1);
      p[player[i]] = pos[i];
   }
   
   return max(t[0],t[1])-1;
}

void main()
{
   filein fin( "A-large (2).in" );
   fileout fout( "out.txt" );
   trace.setFile( fout );
