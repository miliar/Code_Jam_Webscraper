...
try {
  ...
  WhileTests() {
    ...
    for (int i = 0; i < N; ++i) {
      InString1(who);
      InInt1(b);
      int index = who[0] == 'O' ? 0 : 1;
      events.push(index);
      buttons[index].push_back(b);
    }
    int position[2] = { 1, 1 };
    i64 time = 0;
    while (!events.empty()) {
      ...
      for ( int i = 0; i < 2; ++i ) {
        if (!buttons[i].empty()) {
          t[i] = abs(buttons[i].front() - position[i]);
          d[i] = buttons[i].front() >= position[i] ? +1 : -1;
          if ( t[i] == 0 ) d[i] = 0;
        }
      }
      int index = events.front();
      int other = 1 - index;
      ++t[index];
      if ( t[index] <= t[other] || t[other] == 0 ) {
        position[index] = buttons[index].front();
        position[other] += d[other] * t[index];
        time += t[index];
        buttons[index].pop_front();
        events.pop();
      } else if ( t[other] ) {
        position[index] += d[index] * t[other];
        position[other] += d[other] * t[other];
        time += t[other];
      }
    }
    printf("%lld\n", time);
