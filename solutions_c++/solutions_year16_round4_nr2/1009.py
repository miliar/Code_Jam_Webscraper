...
#define RAP(n) for (int i=n-1; i>=0; i--)
...
double a[N];
double d[N][N];
double brutal(int i, int k, int mk, int n) {
	if (k > mk) return 0;
	if (i == n) {
		if (k != mk) return 0;
		return d[mk][mk / 2];
	...
	double f = brutal(i + 1, k, mk, n);


	k++;
	for (int i = 0; i < n; i++) {
		d[k][i] = d[k-1][i] * x + (i ? d[k-1][i - 1] * (1 - x) : 0);
	}
	double s = brutal(i + 1, k, mk, n);
	return max(s, f);
}
