...
void rec2(int i,int j,int msk)
{
    ...
    if(i==m)return ;
    rec2(i+1,j,msk);
    rec2(i+1,j+1,msk|(1<<i));
}

void rec(int i,int j,int msk)
{
    if(j==m)
    {
        j=0;
        f(i,n)if(msk&(1<<i))b[j++]=a[i];
        qrs=0.0;
        rec2(0,0,0);
        rs=max(rs,qrs);
        return ;
...
int main()
{
    ...
    while(ts--)
    {
        cin>>n>>m;
//        sc2(n,m);
        f(i,n)cin>>a[i];//scanf("%lf",&a[i]);
        rs=0.0;
        rec(0,0,0);
        printf("Case #%d: ",kk);
        cout<<setprecision(10)<<fixed<<rs<<endl;
        cerr<<setprecision(10)<<fixed<<rs<<endl;
        kk++;
    }
