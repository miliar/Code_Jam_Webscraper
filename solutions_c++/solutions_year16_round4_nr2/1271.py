...
bool vis[20][20][20];
vector<double> pp;
double f(int idx,int yes,int no,int n)
{
	if(idx == n)
	{
		if(!yes and !no)
	...
	double ret=0;
	if(vis[idx][yes][no])
		return dp[idx][yes][no];
	vis[idx][yes][no]=1;
	// put in yes
	if(yes)
		ret+=f(idx+1,yes-1,no,n)*pp[idx];
	if(no)
		ret+=f(idx+1,yes,no-1,n)*(1-pp[idx]);
	return dp[idx][yes][no]=ret;
}
