			...
			for (unsigned mask = 0; mask < (1U << k); ++mask)
			{
				if (popcount(mask) == k / 2)
				{
					...
					FI(k)
					{
						...
						{
							current *= 1.0L - chosenP[i];
						}
					}
					tie += current;
				}
			}
			answer = max(answer, tie);
		}
	}
	return answer;
}
...
int main()
{
	/*freopen("B-state.in", "r", stdin);
	freopen("B-state.out", "w", stdout);*/
	...
	for (int testcase = 1; testcase <= t; ++testcase)
	{
		...
		printf("Case #%d: %.9lf\n", testcase, (double)solveSmall(n, k));
	}
	return 0;
}
