...
double solve(i64 n, i64 k, vector<double> v)
{
  double best = 0;
  rep(mask, bit(n)){
    if(__builtin_popcountll(mask) != k) continue;
    vector<double> cur(k + 1, 0);
    cur[0] = 1.0;
    rep(i, n){
      if(bit(i) & ~mask) continue;
      vector<double> next(k + 1, 0);
      rep(j, k) next[j + 0] += cur[j] * (1.0 - v[i]);
      rep(j, k) next[j + 1] += cur[j] * v[i];
      cur = next;
    }
    best = max(best, cur[k / 2]);
  }
  return best;
}
