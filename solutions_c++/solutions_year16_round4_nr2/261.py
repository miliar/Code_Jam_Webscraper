...
double dpru(const vector<double>& p)
{
    const int k = p.size();
    assert(k % 2 == 0);
    ...
    rep(i, p.size())
    {
        erep(yes, i)
        {
            ...
            dp[yes + 1][no] += dp[yes][no] * p[i];
            dp[yes][no + 1] += dp[yes][no] * (1 - p[i]);
        }
    }
    return dp[k / 2][k / 2];
}
int main()
{
    ...
    for (int case_i = 1; case_i <= T; ++case_i)
    {
        ...
        rep(i, k - front)
        choice.push_back(p[n - 1 - i]);
        assert(choice.size() == k);

        upmax(max_tie_p, dpru(choice));
        }
        printf("Case #%d: %.10f\n", case_i, max_tie_p);
    }
}
