...
#define x first
#define y second
#define prio(q) priority_queue<int,vector<int>,greater<int>> q;
#define priot(q,t) priority_queue<t,vector<t>,greater<t>> q;
#define opl(T) bool operator<(const T & a, const T & b)
#define opg(T) bool operator>(const T & a, const T & b)
...
int msb(int x){ asm ("bsrl %0, %0" : "=r" (x) : "0" (x)); return x; }
void bitprint(int x) {
	rep(i,10) {
		cerr << checkbit(x,i);
	}
...
int K;
double P[2*e(2)];
int mm(int a,int b) { return (a<<4)+b; }
unordered_map<int,int> MEM;

double prob(int m, int y) {
	if (m==0) return 1;
	// if (has(MEM, mm(m,y))) return MEM[mm(m,y)];
	// dbg(y);bitprint(m);
	int pos = msb(m);
	double ret=0;
	if (y < K/2) ret += P[pos] * prob(m-(1<<pos),y+1); // yes
	if (K-y-popcnt(m) < K/2) ret += (1-P[pos]) * prob(m-(1<<pos),y);
	// MEM[mm(m,y)]=ret;
	return ret;
}
