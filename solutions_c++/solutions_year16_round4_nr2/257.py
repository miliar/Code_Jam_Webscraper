...
class Main{
	...
	void run(){
		...
		for(int q:range(T)){
			...
			for(int l:range(K+1)){
				vector<double> uses;
				for(int i:range(l)) uses.push_back(ps[i]);
				for(int i=n-1;i>=n-(K-l);i--) uses.push_back(ps[i]);
				vector<double> dp(K+1);dp[0] = 1;
				for(int i:range(K)){
					vector<double> ndp(K+1);
					for(int k:range(0,K+1)){
						ndp[k] += dp[k] * (1-uses[i]);
						if(k>=0) ndp[k] += dp[k-1] * uses[i];
					}
					dp = ndp;
				}
				Mv2 = max(Mv2,dp[K/2]);
			...
			// cerr << Mv <<" " << Mv2 << endl;
			// assert(abs(Mv - Mv2) < 1e-6);
			cout << make_tuple("Case","#"+to_string(q+1)+":",Mv2)<<endl;
