...
double Solve(int pos,int x,int y) {
    if(pos==len) {
        return 1.0;
    }

    double &res=dp[pos][x][y];
    if(vis[pos][x][y]) return res;
    vis[pos][x][y]=1;

    res=0.0;

    if(x) res+=Solve(pos+1,x-1,y)*b[pos];
    if(y) res+=Solve(pos+1,x,y-1)*(1.0-b[pos]);
    return res;
}

double Dekhi(int n,int k) {
    double ans=0.0;
    for(int i=0;i<(1<<n);i++) {
        if(__builtin_popcount(i)==k) {
            ...
            double tp=Solve(0,len/2,len/2);
            if(tp>ans) {
                ans=tp;
//                for(int l=0;l<len;l++) cout<<b[l]<< " "; printf("\n");
            }
        }
    }
    return ans;
