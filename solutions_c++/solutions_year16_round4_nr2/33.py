...
double probability_of_tie(vector<double> P) {
  assert((int)P.size() == K);

  vector<double> prv(K + 1), nxt;
  prv[0] = 1.0;
  rep (i, K) {
    double p = P[i];
    nxt.assign(K + 1, 0.0);
    ...
    // Yes
    rep (j, K) nxt[j + 1] += prv[j] * p;

    // No
    rep (j, K + 1) nxt[j] += prv[j] * (1.0 - p);

    // rep (j, K + 1) printf("%.4f ", nxt[j]); puts("");
...
void solve() {
  ...
  for (int k = 0; k <= K; ++k) {
    vector<double> p;
    rep (i, k) p.pb(P[i]);
    rep (i, K - k) p.pb(P[N - i - 1]);
    ans = max(ans, probability_of_tie(p));
  }
  printf("%.10f\n", ans);
}
