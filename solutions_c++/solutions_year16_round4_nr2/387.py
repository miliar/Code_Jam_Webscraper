...
double getProp(vector<double> V) {
    int n = V.size();
    /*for(int i = 0;i < n;i++) printf("%.2f ",V[i]);
    printf("\n============\n");*/
    for(int i = 0;i < n+1;i++) for(int j = 0;j < n+1;j++) dyn[i][j] = 0;
    dyn[0][0] = 1;
    for(int i = 0;i < n+1;i++) {
        for(int j = 0;j < n+1;j++) {
            if(i+j >= n) continue;
            dyn[i+1][j] += dyn[i][j]*V[i+j];
            dyn[i][j+1] += dyn[i][j]*(1-V[i+j]);
        }
    }
    /*for(int i = n;i >= 0;i--) {
        for(int j = 0;j < n+1;j++) {
...
void solve()
{
    ...
    /*for(int i = 0;i <= n-k;i++) {
        vector<double> V;
        for(int j = 0;j < k;j++) V += arr[i+j];
        ans = max(ans,getProp(V));
        printf("%.3f ",getProp(V));
    }*/
    for(int i = 0;i <= k;i++) {
        vector<double> V;
        for(int j = 0;j < i;j++) V += arr[j];
        for(int j = 0;V.size() != k;j++) V+=arr[n-j-1];
        ans = max(ans,getProp(V));
    }
    /*if(n%2 == 0) {
        // good , 1 middle
        vector<double> V;
        for(int i = n/2-k/2;i < n/2+k/2;i++) V += arr[i];
        ans = getProp(V);
    }
    else {
        // 2 middle
        vector<double> V1,V2;
        for(int i = n/2-k/2;i < n/2+k/2;i++) V1 += arr[i];
        for(int i = n/2-k/2 + 1;i <= n/2+k/2;i++) V2 += arr[i];
        ans = max(getProp(V1),getProp(V2));
    }*/
    printf("%.10f\n",ans);
    // bruteforce here
    /*for(int i = 0;i < (1<<n);i++) {
        ...
        if(V.size() == k) ans = max(ans,getProp(V));
    }
    printf("%.10f\n",ans);*/
}
