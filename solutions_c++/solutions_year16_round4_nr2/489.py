...
VV(double) func(vector<double> ps){
	int n = ps.size();
	VV(double) ret;
	initvv(ret, n + 1, n + 1);
	ret[0][0] = 1;
	for(int i = 1; i <= n; ++i){
		double p = ps[i - 1];
		ret[i][0] = ret[i - 1][0] * (1 - p);
		for(int j = 1; j <= n; ++j){
			ret[i][j] = ret[i - 1][j] * (1 - p) + ret[i - 1][j - 1] * p;
		}
	}
	return ret;
}
...
double solve(){
	...
	VV(double) tbl1 = func(ps);
	reverse(ALL(ps));
	VV(double) tbl2 = func(ps);
	
	int halfk = k / 2;
	double ans = 0;
	for(int i = 0; i <= k; ++i){
		double s = 0;
		for(int j = 0; j <= i && j <= halfk; ++j){
			s += tbl1[i][j] * tbl2[k - i][halfk - j];
		}
		ans = max(ans, s);
	}
	return ans;
