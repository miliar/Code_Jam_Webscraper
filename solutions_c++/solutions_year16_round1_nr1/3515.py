...
ll Power(ll base, ll power){ if (power < 0)return 0; ll ret = 1; while (power){ if (power & 1)ret *= base; power >>= 1; base *= base; } return ret; }
...
inline void run(){
	...
	for (int tt = 1; tt <= t; ++tt){
		...
		sz = strlen(ch);
		for (int i = 0; i < sz; ++i)
		if (!i || ch[i] < q.front())q.emplace_back(ch[i]);
		else q.emplace_front(ch[i]);
		for (int i = 0; i < sz; ++i){
			ch[i] = q.front();
			q.pop_front();
		}
