...
resultType	solve(instanceType&	instance)
{
	...
	aux_S[i] = instance.S[0];
	for(int k = 1; k < instance.S.size(); ++k)
	{
		if(instance.S[k] >= aux_S[i]) aux_S[--i] = instance.S[k];
		else aux_S[j++] = instance.S[k];
	}
	solution.result = aux_S.substr(i, instance.S.size());
