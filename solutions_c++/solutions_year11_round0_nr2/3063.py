...
int main() {
  ...
  scanf("%d", &stest);
  rep(test,1,stest) {
    scanf("%d", &comn); fori(i, comn) scanf("%s", com[i]);
    scanf("%d", &oppn); fori(i, oppn) scanf("%s", opp[i]);
    scanf("%d", &n); scanf("%s", w);
    res = "";
    fori(i, n) {
      if(res.size() > 0) {
        ...
        while (res.size()>=2) {
          ...
          fori(k, comn) if(
              (com[k][0] == c1 && com[k][1] == c2) ||
              (com[k][1] == c1 && com[k][0] == c2)) {
        ...
        if(res.size() >= 2) {
          char c1=res[res.size()-1];
          for(size_t it=0; it < res.size()-1; ++it) {
            char c2 = res[it];
            fori(k, oppn) if(
                (opp[k][0] == c1 && opp[k][1] == c2) ||
                (opp[k][1] == c1 && opp[k][0] == c2)) {
              res = "";
              break;
            }
            if(res=="")break;
    ...
    printf("Case #%d: [", test);
    forv(i, res) if(i==res.size()-1) printf("%c]\n", res[i]); else printf("%c, ", res[i]);
