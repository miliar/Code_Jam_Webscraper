...
try {
  ...
  WhileTests() {
    ...
    vector<int> opposite[26];
    memset(combine, 0, sizeof(combine));
    InInt1(C);
    for (int i = 0; i < C; ++i) {
      InString1(comb);
      combine[comb[0] - 'A'][comb[1] - 'A'] = comb[2] - 'A';
      combine[comb[1] - 'A'][comb[0] - 'A'] = comb[2] - 'A';
    }
    InInt1(D);
    for (int i = 0; i < D; ++i) {
      InString1(op);
      opposite[op[0] - 'A'].push_back(op[1] - 'A');
      opposite[op[1] - 'A'].push_back(op[0] - 'A');
    }
    InInt1(N);
    InString1(invoke);
    vector<int> result;
    ...
    for (int i = 0; i < N; ++i) {
      int c = invoke[i] - 'A';
      if (!result.empty() && combine[result.back()][c]) {
        --count[result.back()];
        c = combine[result.back()][c];
        result.pop_back();
      }
      bool clear = false;
      for (vector<int>::const_iterator it = opposite[c].begin(), end = opposite[c].end(); !clear && it != end; ++it) {
        clear = count[*it] != 0;
      }
      if (clear) {
        memset(count, 0, sizeof(count));
    ...
    printf("[");
    for (vector<int>::const_iterator it = result.begin(), end = result.end(); it != end; ++it) {
