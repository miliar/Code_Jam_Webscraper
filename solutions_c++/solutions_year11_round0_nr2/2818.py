...
int main() {
	gcase {
		...
		foreach(e, invk) {
			...
			if(ans.size()) {
				char a = *e, b = ans.back();
				pair<char,char> tp = mp(min(a, b),max(a, b));
				if(comb.find(tp) != comb.end()) {
					ans.pop_back();
					ans.pb(comb[tp]);
					cerr << "Combine: " << a << '&' << b << '=' << ans.back() << endl;
				}
				else ans.pb(a);
				b = ans.back();
				if(opp.find(b) != opp.end()) {
					if(find(ans.begin(), ans.end(), opp[b]) != ans.end()) ans.clear();
					cerr << "Clear." << endl;
				}
			}
			else ans.pb(*e);
