...
class Combine {
    ...
    public:
        char _c[3];
        bool combined(string& s) {
            int len = s.length();
            if (len > 1) {
                char c1 = s[len-1];
                char c2 = s[len-2];
                if ((c1 == _c[0] and c2 == _c[1]) or
                    (c2 == _c[0] and c1 == _c[1])) {
                    s[len-2] = _c[2];
                    s.resize(len-1);
                    return true;
...
public:
    Combine() {
        ...
        for(int Ci = 0; Ci < C; ++Ci) {
            scanf(" %c%c%c",_vc[Ci]._c,_vc[Ci]._c+1,_vc[Ci]._c+2);
        }
    }
    void check(string& s) {
        int i = 0;
        do {
            for( i=0; i<_vc.size(); ++i) {
                if (_vc[i].combined(s)) break;
            }
        } while ( i < _vc.size() );
    }
};
...
public:
    Oppose() {
        ...
        _vc.resize(C);
        for(int Ci = 0; Ci < C; ++Ci) {
            scanf(" %c%c",_vc[Ci]._c,_vc[Ci]._c+1);
        }
    }
    void check(string& s) {
...
class ElementList {
    string _s;
public:
    ElementList(Combine& comb, Oppose& opp, Series& series) : _s() {
        for(int i=0; i<series._s.size(); ++i) {
            _s += series._s[i];
            comb.check( _s );
            opp.check( _s );
        }
...
int main() {
  ...
  for (int Ti = 1; Ti <= T; ++Ti) {
      ...
      Oppose opp;
      Series series;
      ElementList final(comb,opp,series);
