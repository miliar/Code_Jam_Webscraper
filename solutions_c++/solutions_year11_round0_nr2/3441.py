...
#define active_bits(x) __builtin_popcount( x )
...
template<class T> T bits(T x) {if(x==0) return 1; return (T)ceil(log(x+1)/log(2));}
...
int main()
{
    ...
    while(test--)
    {
        ...
        printf("Case #%d: ",++cas);
        if(SZ(v)==0) cout<<"[]"<<endl;
        else if(SZ(v)==1) printf("[%c]\n",v[0]);
        else{
        printf("[%c",v[0]);
        FOR(i,1,SZ(v)-1) cout<<", "<<v[i];
        cout<<"]"<<endl;
        }
