...
int main() {
    ...
    for (int qq = 0; qq < t; qq++) {
        ...
        for (int i = 0; i < C; i++) {
            string s1;
            std::cin >> s1;
            string key = s1.substr(0, 2);
            dict[key] = s1.substr(2, 1);
        ...
        for (int i = 0; i < D; i++) {
            string s1;
            std::cin >> s1;
            opp[s1[0]].insert(s1[1]);
            opp[s1[1]].insert(s1[0]);
        }
        std::cin >> N;
        string magic; std::cin >> magic;
        string res = "";
        for (int i = 0; i < magic.size(); i++) {
            res += magic[i];
            while (res.size() > 1 && dict.count(res.substr(res.size()-2, 2))) {
                res = res.substr(0, res.size() - 2) + dict[res.substr(res.size()-2, 2)];
            }
            for (int i = 0; i < res.size() - 1; i++) {
                if (opp.count(res[i]) && opp[res[i]].count(res[res.size()-1])) {
                    res = "";
                    break;
                }
            }
