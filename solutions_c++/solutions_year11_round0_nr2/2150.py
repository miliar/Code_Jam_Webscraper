...
int SolveIt()
{
	...
	memset(mapC,0,sizeof(char)*26*26);
	memset(mapD,0,sizeof(char)*26*26);

	for(i=0;i<C;i++)
	{
		index0 = szC[i][0]-'A';
		index1 = szC[i][1]-'A';
		mapC[index0][index1] = szC[i][2];
		mapC[index1][index0] = szC[i][2];
	}
	...
	for(i=0;i<D;i++)
	{
		index0 = szD[i][0]-'A';
		index1 = szD[i][1]-'A';
		mapD[index0][index1] = 1;
		mapD[index1][index0] = 1;
	}
	...
	while(flag)
	{
		...
		for(i=1;i<n;i++)
		{
			index1=szN[i]-'A';
			index2=szN[i-1]-'A';
			if(mapC[index1][index2] != 0) //合并
			{
				memmove(szN+i-1,szN+i,n-i);
				szN[i-1] = mapC[index1][index2];
				szN[n-1]=0;
				flag = true;
				break;
			}
			...
			{
				index0 = szN[j]-'A';
				if( mapD[index0][index1] != 0) //删除
				{
					memmove(szN,szN+i+1,n-i-1);
					szN[n-i-1]=0;
					//memmove(szN+j,szN+i+1,n-i-1);
					//szN[n-(i-j+1)]=0;
					flag = true;
					goto JMP;
				}
