...
int main()
{
	...
	for(int I=0;I<T;I++)
	{
		...
		{
			fin>>temp;
			opp.insert(make_pair(temp[0],temp[1]));
		}
		fin>>N>>temp;
		string tp;tp+=temp[0];
		for(int i=1;i<temp.size();i++)
		{
			if(tp.size()==0) {tp+=temp[i];continue;}
			if(comb.find(make_pair(temp[i],tp[tp.size()-1]))!=comb.end())
			{
				tp=tp.substr(0,tp.size()-1)+comb[make_pair(temp[i],tp[tp.size()-1])];continue;
			}
			else if(comb.find(make_pair(tp[tp.size()-1],temp[i]))!=comb.end())
			{
				tp=tp.substr(0,tp.size()-1)+comb[make_pair(tp[tp.size()-1],temp[i])];continue;
			}
			bool f=false;
			for(int j=0;j<tp.size();j++)
			{
				if(opp.find(make_pair(temp[i],tp[j]))!=opp.end())
				{
					tp="";f=true;break;
				}
				else if(opp.find(make_pair(tp[j],temp[i]))!=opp.end())
				{
                    tp="";f=true;break;
				}
			}
...
//	for(int I=0;I<T;I++)
//	{
//        int values[1000],s=0,sum=0,Min=INF;
//		fin>>N;
//		for(int i=0;i<N;i++)
//		{
//			fin>>values[i];
...
//		}
//        if(s!=0) fou<<"Case #"<<I+1<<": "<<"NO"<<endl;
//		else fou<<"Case #"<<I+1<<": "<<sum-Min<<endl;
//	}
//}
//int main()
//{
