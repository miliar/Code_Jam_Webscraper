...
const int MAXCH = 260;
int c, d, n;
char combine[MAXCH][MAXCH];
bool opposed[MAXCH][MAXCH], inList[MAXCH];
/* END DECLARATION */
...
inline void run() {
	...
	for (test = 1; test <= ntest; ++test) {
		...
		FOR(i, 1, d) {
			cin >> s;
			opposed[(int)s[0]][(int)s[1]] = true;
			opposed[(int)s[1]][(int)s[0]] = true;
		}
		cin >> n;
		cin >> s;
		SET(inList, false);
		...
		REP(i, n) {
			char ch = s[i];
			if (combine[(int)ch][(int)last] != 0) {
				eleList.erase(eleList.end() - 1);
				bool found = false;
				REP(i, SIZE(eleList) - 1) {
					if (eleList[i] == last) {
						found = true;
						break;
					}
				}
				inList[(int)last] = found;
				last = combine[(int)ch][(int)last];
				eleList.PB(last);
				inList[(int)last] = true;
			} else {
				bool conflicted = false;
				for (char ccc = 'A'; ccc <= 'Z'; ccc++) {
					if (inList[(int)ccc] && opposed[(int)ccc][(int)ch]) {
						SET(inList, false);
						eleList.clear();
						last = 0;
						conflicted = true;
