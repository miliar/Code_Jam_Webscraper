...
int main(){
	...
	while(t--){
		kase++;
		string s;
		map<pair<char,char>,char> tr;
		map<pair<char,char>,int> nok;
		int c=GI;
		REP(i,c){
			cin>>s;
			tr[pair<char,char>(s[0],s[1])]=s[2];
			tr[pair<char,char>(s[1],s[0])]=s[2];
		}
		int d=GI;
		REP(i,d){
			cin>>s;
			nok[pair<char,char>(s[0],s[1])]=1;
			nok[pair<char,char>(s[1],s[0])]=1;
		}
		int n=GI;
		cin>>s;
		string ans="";
		REP(i,n){
			if(ans=="")	ans+=s[i];
				...
				if(tr[pair<char,char>(ans[sz(ans)-1],s[i])]>='A'&&tr[pair<char,char>(ans[sz(ans)-1],s[i])]<='Z'){
					char tmp=ans[sz(ans)-1];
					ans.resize(sz(ans)-1);
					ans+=tr[pair<char,char>(tmp,s[i])];
				}
				else{
					int m=sz(ans),found=false;
					REP(j,m){
						if(nok[pair<char,char>(ans[j],s[i])]==1){
							found=true;
							ans="";
							break;
						}
