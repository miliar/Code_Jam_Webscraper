...
{
   int C = fin.readInt();
   vector<string> combs = fin.readStrings( C );
   int D = fin.readInt();
   vector<string> opps = fin.readStrings( D );
   int N = fin.readInt();
   string inv = fin.readStr();

   string s;

   for ( int k = 0; k < N; k++ )
   {
      char a = s.empty() ? 0 : s[s.length()-1];
      char b = inv[k];
      int i;
      for ( i = 0; i < combs.size(); i++ )
         if ( combs[i][0] == a && combs[i][1] == b
           || combs[i][1] == a && combs[i][0] == b )
         {
            s = s.substr( 0, s.length()-1 ) + combs[i][2];
            goto next;
         }

      for ( i = 0; i < opps.size(); i++ )
         if ( b == opps[i][0] && (int) s.find( opps[i][1] ) != -1
           || b == opps[i][1] && (int) s.find( opps[i][0] ) != -1 )
         {
            s = "";
            goto next;
         }
