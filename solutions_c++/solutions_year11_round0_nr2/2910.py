...
int main()
{
    int i,j,Case=1,test,ch1,ch2,C,D,N,f,v;
    string output;
    char str[110],chng[5],ele[5];
    ...
    while(test--)
    {
        ...
        {
            scanf("%s",chng);
            M1[MP(chng[0],chng[1])]=chng[2];
            M1[MP(chng[1],chng[0])]=chng[2];
        }
        scanf("%d",&D);
        for(i=0;i<D;i++)
        {
        ...
        for(i=0;i<N;i++)
        {
           ...
           {
               ch1=str[i];
               ch2=output[output.size()-1];
               if((v=M1[MP(ch1,ch2)]))
                    output[output.size()-1]=v;
               else
               {
                   for(j=0;j<output.size();j++)
                        if(M2[MP(ch1,output[j])])
                        {
                            f=0;
                            output="";
                            break;
