...
char* stoa(string s){char *str=new char(sizeof(s.length()+1));	rep(i,s.length()) str[i]=s[i];str[s.length()]=0;return str;}
...
int main()
{
	...
	for(int cc=0;cc<c;cc++)
	{
		...
		rep(i,c)
		{
			char str[6];
			cin>>str;
			combine[ str[0]-64 ][ str[1]-64 ] = str[2]-64;
			combine[ str[1]-64 ][ str[0]-64 ] = str[2]-64;
		}
		cin>>d;
		rep(i,d)
		{
			char str[6];
			cin>>str;
			destroy[ str[0]-64 ][ str[1]-64 ] = 1;
			destroy[ str[1]-64 ][ str[0]-64 ] = 1;
		...
		{
			int n=str[i]-64;
			if(val.size()>0 && combine[n][ val[val.size()-1] ]!=0)
			{
				int nn=val[val.size()-1];
				nn=combine[n][nn];
				val.pop_back();
				val.push_back(nn);
			}
