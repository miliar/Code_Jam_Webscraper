...
string change(string s, vector<string> combs, vector<string> oppos) {
	REP(i, s.size()) {
		if (i > 0) {
			REP(j, combs.size()) {
				if (combs[j][0] == s[i-1] && combs[j][1] == s[i]) {
					//cout << "s: " << s << ", replace (" << i << ", " << j << ") with " << combs[j][2] << '\n';
					s.replace(i-1, 2, 1, combs[j][2]);
					return s;
				}
			}
		}
		REP(j, i) {
			REP(k, oppos.size()) {
				if (oppos[k][0] == s[i] && oppos[k][1] == s[j]) {
					//cout << "s: " << s << " erase (" << i << ", " << j << ")\n";
					//s.erase(j, i - j + 1);
					s.erase(0, i+1);
					return s;
...
int main(void) {
	...
	REP(__number_of_case, __number_of_cases) {
		int combs_count;
		cin >> combs_count;
		vector<string> combs(combs_count * 2);
		string s;
		REP(i, combs_count) {
			cin >> s;
			combs[i*2] = s;
		...
		int opp_count;
		cin >> opp_count;
		vector<string> opps(opp_count * 2);
		REP(i, opp_count) {
			cin >> s;
			opps[i*2] = s;
			swap(s[0], s[1]);
		...
		string s1;
		while (1) {
			s1 = change(s, combs, opps);
			if (s1 == s) {
				break;
			}
			s = s1;
		}
		cout << "Case #" << __number_of_case+1 << ": [";// << s << '\n';
		REP(i, s.size()) { 
			cout << s[i];
			if (i != s.size() - 1) {
				cout << ", ";
