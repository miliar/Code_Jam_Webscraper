...
int main(){
  ...
  FOR(i,0,_T){
    cout << "Case #" << (i+1) << ": ";
    combine.clear(), destroy.clear();

    int C, D, N;
    string input, tmp, result;
    cin >> C;
    FOR(j,0,C){
	 cin >> tmp;
	 combine[tmp.substr(0,1) + tmp.substr(1,1)] = tmp.substr(2);	
	 combine[tmp.substr(1,1) + tmp.substr(0,1)] = tmp.substr(2);
    }
    ...
    FOR(j,0,D){
	 cin >> tmp;
	 destroy.insert(tmp);
	 destroy.insert(tmp.substr(1,1) + tmp.substr(0,1));
    }
    cin >> N >> input;

    FOR(j,0,N){
	result = result + input[j];
	while(result.length() > 1 && combine.count(result.substr(result.length()-2))){
	    result = result.substr(0,result.length()-2) + combine[result.substr(result.length()-2)];
	}
	FOR(i,0,result.length()-1) if(destroy.count(result.substr(i,1) + result[result.length()-1])){
	   result = ""; break;
	}
    }
    cout << "[";
    if(result.length() > 0) cout << result[0];
    FOR(i,1,result.length()) cout << ", " << result[i];
    cout << "]";
    cout << endl;
  }
  return 0;
