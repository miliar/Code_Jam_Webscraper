...
public:
	void solve(std::istream& in, std::ostream& out) {
		...
		vector<vector<int>> datas(k, vector<int>(1, 0));
		for(int i: range(n - k)) {
			datas[i % k].push_back(datas[i % k].back() + sums[i + 1] - sums[i]);

		}

		for (int i: range(k)) {
			int z = *min_element(datas[i]);
			for (int& x: datas[i]) {
				x -= z;
			}
		}
		...
		int needAdd = ((sums[0] - sumsFirst) % k + k) % k;

		int globalMax = 0;
		for (int i: range(k)) {
			globalMax = max(globalMax, *max_element(datas[i]));
		}
		int canAdd = 0;
		for (int i: range(k)) {
			canAdd += globalMax - *max_element(datas[i]);
		}

		int ans =  globalMax + (needAdd > canAdd);
