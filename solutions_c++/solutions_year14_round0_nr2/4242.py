...
double cookieSim(double farmcost, double rateincrease, double goal )
{
    int cookies = 0;
    long double rate=2;
    ...
    //temptime=farmcost/ratepersec;
    //time+=temptime;
    //cookies +=temptime*ratepersec;



    while ((farmcost)/(rate)+ (goal)/(rate+rateincrease) <(goal)/(rate)) //if worth it to get to farm cost, buy, and then get to goal
    {
        //printf("farmcost: %f, rate: %f, goal: %f, rateincrease: %f, temptime: %f\n", farmcost, rate, goal, rateincrease, farmcost/rate);

        temptime=farmcost/rate;
        time+=temptime;
        cookies +=farmcost;
    ...
    temptime=goal/rate;
    time+=temptime;
    cookies +=temptime*rate; //should be goal
...
int main()
{
    ...
    else if (currentstep ==1)
    {
        ...
        double cost=atof((input_line.substr(0, space[0])).c_str());
        double farmrate=atof((input_line.substr(space[0]+1, space[1]-space[0])).c_str());
        double goal=atof((input_line.substr(space[1])).c_str());

        //printf("%f, %f, %f\n", cost, farmrate, goal);
        double result = cookieSim(cost, farmrate, goal);
        //magic
