...
{
	t.Times.reserve			(10000);
	double rate				= 2.0;
	double amount			= t.WinAmount;
	double time				= amount / rate;
	double farm_time		= t.FarmCost / rate;
	double next_rate		= rate + t.RatePerFarm;
	double next_time		= farm_time + amount / next_rate;

	while (time > next_time) {
		time				= next_time;
		farm_time			+= t.FarmCost / next_rate;
		next_rate			+= t.RatePerFarm;
		next_time			= farm_time + amount / next_rate;
		if (fabs(time - next_time) < 0.00000001) {
			break;
		}
	}
	char buffer[16];
	sprintf					(buffer, "%.7f", time);
	t.result				= buffer;
}
