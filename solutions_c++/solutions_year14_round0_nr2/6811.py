/**
 * @file Problem_B.cpp
 * @brief Problem B. Cookie Clicker Alpha 
 * @details 
 * Introduction
 * 
...
//	cout << T << endl;
	...
	for (int k = 0; k < T; ++k)
	{
		...
		double remaining_time, time_to_factory, ETA;
		...
		while(!min) 
		{
		    remaining_time = X/cps;
		    time_to_factory = C/cps;
		    ETA = time_to_factory + X/(cps+F);

		    if(ETA < remaining_time)
		    {
		    	t += time_to_factory;
		    	cps += F;
		    }
		...
		outFile << "Case #" << k+1 << ": " << std::fixed << std::setprecision(7) << t + remaining_time << endl;
//		cout << "Case #" << k+1 << ": " << std::fixed << std::setprecision(7) << t + remaining_time << endl;;
	}
