...
int taskB(int argc, char* argv[])
{
	...
	while(N > 0)
	{
		...
		char *lineData = new char[data.length() + 1];
		strcpy(lineData, data.c_str());

		char** splittedData = splitString(lineData);

		double C, F, X;

		double cookiesNow = 0;
		double currentCookieRate = 2.0;
		double spentSecs = 0.0;

		C = atof(splittedData[0]);
		F = atof(splittedData[1]);
		X = atof(splittedData[2]);

		while (cookiesNow < X)
		{
			double secsToComplete = (X-cookiesNow)/currentCookieRate;
			double nextFactory = C/currentCookieRate;
			double nextFactoryToComplete = (X-cookiesNow)/(currentCookieRate+F) + nextFactory;

			if(secsToComplete < nextFactory || nextFactoryToComplete > secsToComplete)
			{
				spentSecs += secsToComplete;
				cookiesNow = X;
			}
			else
			{
				cookiesNow = 0;
				spentSecs += nextFactory;
				currentCookieRate += F;
			}
		}
		
		char outLine[100];
		sprintf(outLine,"Case #%d: %1.7f \n", testCases-N+1, spentSecs);//Case #3: 
