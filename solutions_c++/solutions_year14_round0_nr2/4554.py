...
int main(int argc, char *argv[])
{
    ...
    if (argc != 3)
    {
        qDebug() << "Usage:  ./cc in.txt out.txt";
        return 0;
    }
    ...
    for(int caseNum = 1; caseNum <= totalCases; caseNum++)
    {
        ...
        QList<double> problemVariables = readDoubles(inputFile);

        double costOfCookieFarm = problemVariables.at(0);
        double benefitOfCookieFarm = problemVariables.at(1);
        double cookiesToWin = problemVariables.at(2);

        double cookieProductionRate = 2.0;
        double cookiesProduced = 0.0;
        double timeElapsed = 0.0;
        double allowedError = 0.0000001;

        while (cookiesProduced < (cookiesToWin - allowedError))
        {
            //qDebug() << "Case" << caseNum << "  cookies made" << cookiesProduced << "  rate" << cookieProductionRate;

            double timeToCreateFarm = costOfCookieFarm / cookieProductionRate;
            double timeToWinOneMoreFarm = timeToCreateFarm + ( (cookiesToWin - cookiesProduced) / (cookieProductionRate + benefitOfCookieFarm));
            double timeToWinNoMoreFarms = (cookiesToWin - cookiesProduced) / cookieProductionRate;

            //qDebug() << "Time till farm" << timeToCreateFarm << "  time_with_no_more_farms" << timeToWinNoMoreFarms << "  time_with_1_more_farm" << timeToWinOneMoreFarm;

            if(timeToWinNoMoreFarms < timeToWinOneMoreFarm)
            {
                // Produce enough cookies to win
                cookiesProduced += timeToWinNoMoreFarms * cookieProductionRate;
                timeElapsed += timeToWinNoMoreFarms;
                //qDebug() << "going for the win, time" << timeElapsed;
            }
            else
            {
                // Build another farm
                timeElapsed += timeToCreateFarm;
                cookieProductionRate += benefitOfCookieFarm;
                //qDebug() << "building a farm, time" << QString::number(timeElapsed, 'G', 15);
            }
        }


        QString outputText = QString("Case #%1: %2\n").arg(caseNum).arg(QString::number(timeElapsed, 'G', 15));
