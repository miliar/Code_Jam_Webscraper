...
int main()
{
    ...
    int cases,ccount;
    fastread(&cases);
    double farmBonus,totamount,inputRate,toWin,farmRate;
    double timeSpent;
    for(ccount=1;ccount<=cases;++ccount)
    {
        ...
        cin>>toWin;
        //reset vals
        totamount=0; inputRate=2.0;
        timeSpent=0.0;

        double nextwait=-1,totwait=0;
        while(nextwait<totwait)
        {
            nextwait=((double)farmRate/(double)inputRate)+((double)toWin/(double)(inputRate+farmBonus));
            totwait=((double)toWin/(double)inputRate);
            if(nextwait<totwait)
            {
                timeSpent += ((double)farmRate/(double)inputRate);
                inputRate += farmBonus;
            }
            else
            {
