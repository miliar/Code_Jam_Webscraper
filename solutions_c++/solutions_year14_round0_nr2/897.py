...
double gettime(double C, double F, double X, int max_farm, int cur_farm)
{
	double speed = 2.0 + F * (double)cur_farm;

	if (cur_farm == max_farm)
	{
		return X / speed;
	}

	double timeto_nextfarm = C / speed;

	return timeto_nextfarm + gettime(C, F, X, max_farm, cur_farm + 1);
}
...
void solve(istream& in, ostream& fout, int caseIdx)
{
	...
	OUTANS( << CaseHead(caseIdx+1) << setprecision(8) << t << endl );
}
