...
int main()
{
	...
	for (int i = 1; i <= T; ++i)
	{
		...
		double costToBuyFarm = C / currRate;
		double costToWin = X / (currRate + F);
		double costToWinDirect = X / currRate;
		
		// cout << "costToBuyFarm " << costToBuyFarm << " costToWin " << costToWin << " costToWinDirect " << costToWinDirect << endl;

		while (costToBuyFarm + costToWin < costToWinDirect)
		{
			ans += costToBuyFarm;

			currRate += F;

			costToBuyFarm = C / currRate;
			costToWin = X / (currRate + F);
			costToWinDirect = X / currRate;
