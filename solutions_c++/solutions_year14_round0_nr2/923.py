...
{
    if( x < 0.0L )
        return digammal(1.0L-x)+M_PIl/tanl(M_PIl*(1.0L-x)) ;
    else if( x < 1.0L )
    ...
    {
        register long double Tn_1 = 1.0L ;
        register long double Tn = x-2.0L ; 
...
int main() {
    int tc,t;
    long double time, c,f,x, a,b,fx, i;
    ...
    for(tc=1; tc<=t; ++tc) {
        ...
        b  = 2 + f;
        fx = f*x;
        while(( (fx - b*c)/(a*b) ) >= 0.0000001) {
            ++ i;

            a+= f;
            b+= f;
        }
        if(i)   time = (digammal(i + 2/f) - digammal(2/f))/f;
        else    time = 0;
        time = c*(time) + x/a;
