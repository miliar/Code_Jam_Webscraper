...
{
    const double INITSPEED = 2;
    int totalCases = 0;
    cin >> totalCases;
    for (int idx = 0; idx < totalCases; ++idx) {
        double C, F, X;      // farm cost, farm produce speed, target num
        cin >> C >> F >> X;
        // cout << C << ", " << F << ", " << X << endl;

        double currSpeed = INITSPEED;
        double maxSeconds = X / currSpeed; 
        double usedSeconds = 0;
        while (usedSeconds < maxSeconds) {
            usedSeconds += C / currSpeed;
            currSpeed += F;
            double seconds = usedSeconds + X / currSpeed;
            if (seconds < maxSeconds) maxSeconds = seconds;
        }

        // output results
        // cout << "Case #" << idx + 1 << ": " << maxSeconds << endl;
        printf("Case #%d: %.7f\n", idx + 1, maxSeconds);
    }
    return 0;
}
