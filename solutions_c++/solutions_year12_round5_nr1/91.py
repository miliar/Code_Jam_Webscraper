...
//	__filename="A-small-attempt0";
	...
	for (int caseId=1;caseId<=testcase;caseId++)
	{
		...
		do{
			...
			{
				int key=y[i];
				double p0=p[key]/100.0;
				double new_alpha=p0+(1-p0)*alpha;
				double new_beta=L[key]+(1-p0)*beta;
				alpha=new_alpha;
				beta=new_beta;
			}
			double x=beta/(1.0-alpha);
			if (x<min_x-1e-11)
			{
				min_x=x;
				for (int i=0;i<n;i++) r[i]=y[i];
			}
		}while (next_permutation(y,y+n)!=0);
		for (int i=0;i<n;i++)
			if (x[i]!=r[i])
			{
				printf("ERROR\n");
