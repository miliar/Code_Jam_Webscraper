...
#define _F_in       freopen("D-small-attempt1.in","r",stdin)
#define _F_out      freopen("D-small.out","w",stdout)
...
int main()
{
    ...
    for(ll cs=1; cs<=t; cs++)
    {
        ...
        else
        {
            ...
            ll count=0;
            ll count1=K;
            for(ll i=2; count+i<=temp && count+i<=count1 && count1<=temp; i++)
            {
                res.pb(count+i);
                count+=K;
                count1+=K;
