...
long long n, bitmask, uno;

void BFS(int level, int num0, int num1)
{
    if(num0==0 && num1==0)
    {
        for(int i = 0; i<k; i++)
        {
            if((bitmask & (uno<<pos[i]))!=0) bitmask -= (uno<<pos[i]);
            bitmask += (news[i]<<pos[i]);
        }
        if(bitmask%7==0)
        {
            if(solution == -1) solution = bitmask;
            else solution = max(solution, bitmask);
        }
    }
    else
    {
        ...
        {
            news[level] = 0;
            BFS(level+1, num0-1, num1);
        }
        if(num1!=0)
        {
            news[level] = 1;
            BFS(level+1, num0, num1-1);
        }
    }
}
...
*/
/*
int solution[3000100];
int triang[2000];
int m_size = 0;
void init()
{
    int j = 0;
    for(int i = 1; j+i<=3000000; i++)
    {
        j+=i;
        triang[i-1] = j;
        m_size = i;
    ...
    {
        for(int j = 0; triang[i]+triang[j]<=3000000; j++)
        {
            if(solution[triang[i]+triang[j]]==0) solution[triang[i]+triang[j]] = 2;
        }
    }
}
int main()
