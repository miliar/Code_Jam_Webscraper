...
typedef v(ll) otype;
otype calcFunction(ll K, ll C, ll S) {
	otype result;
	if(C*S<K)
		return result;
	ll index =0;
	forn(i, S) {
		ll resultcur = 0;
		fornn(locali, index, index+C) {
			resultcur += powll(K, locali-index)*(locali%K);
		}
		result.pb(resultcur+1);
		index+=C;
		if(index>=K)
			break;
	}
...
int main() {
	...
	fore(test, 1, tests){
		...
		cin >> K >> C >> S;
		//write output
		otype result=calcFunction(K,C,S);
		outfile << "Case #" << test << ": ";
		cout << "Case #" << test << ": ";
#ifndef IFSTRUCT
		outfile << result << endl;
