...
#define fs first
#define sc second
#define Read freopen("DS.in","r",stdin)
#define Write freopen("DS.out","w",stdout)
...
ll power(ll B,ll P) {     ll R=1; while(P>0)      { if(P%2==1) { R=(R*B); } P/=2; B=(B*B); } return R; } /// (B^P)
...
void rec(ll n,ll org,int k,int c,int lvl)
{
    if(c==0)
    {
        printBits(n,power(k,lvl));
        return ;
    }
    ll nl=power(k,lvl);
    ll nn=0;
...
int main()
{
    ...
    while(t--)
    {
        ...
        for(ll i=0;i<s;i++)
        {
            pf(" %lld",i*power(k,c-1)+1);
        }
        pf("\n");
    }
//    while(sf("%lld %lld",&k,&c)==2)
//    {
//        ll n=1ll<<k;
//        for(ll i=0;i<n;i++)
//        {
...
/**
LLLLGLLLLLLGLLLLLLGLLLLLLGLLGGGGGGGLLLLGLLLLLLGLL
