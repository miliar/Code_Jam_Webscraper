...
int main(int, char *[])
{
    freopen("C:\\Users\\Infinity\\Downloads\\D-small-attempt0.in", "r", stdin);
    freopen("C:\\Users\\Infinity\\Downloads\\D-small-attempt0.out", "w", stdout);
    for (int t = getint(), i = 1; i <= t; i++)
    {
        int k = getint(), c = getint(), s = getint();
        long long b = qpow(k, c - 1, maxll);
        writeSP(caseSharpSpace(i));
        // impossible ?
        vector<long long> a;
        long long p = 1;
        while (s--)
