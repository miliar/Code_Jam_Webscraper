...
__int64 Calc_i_pos_After_Next_Gen(__int64 K, __int64 first_pos, __int64 i){
	return (i - 1)*K + first_pos;
}

__int64 Calc_i_pos_After_C_Gen(__int64 K, __int64 C, __int64 first_pos){
	__int64 q, r = first_pos;
	for (q = 1; q < C; q++)r = Calc_i_pos_After_Next_Gen(K, first_pos, r);
	return r;
}

 // C == 1 => S cleaning is required
 // C > 1:
 //   tile i generating sequence (i) of length K after C oeration
 //   at sequence (1) cleaning tile 2 => check tiles 1 & 2 at original sequence 
 //   at sequence (3) cleaning tile 4 => check tiles 3 & 4 at original sequence 
 //   ...
 //   at sequence (K) cleaning tile K, K = 2*p => check tile K at original sequence 
 //   at sequence (K-1) cleaning tile K, K = 2*p+1 => check tiles (K-1) & K at original sequence 
 //   => min (S) : 2*S >= K
...
void main(){
	...
	for (n = 0; n < T; n++){
		...
		if (C == 1){
			...
			if (2 * S < K)r = "IMPOSSIBLE";
			else{
				for (i = 1; i <= K; i += 2){
					if (i > 1)r += " ";
					if (i < K)r += to_string(Calc_i_pos_After_C_Gen(K, C, i) + 1);
					else r += to_string(Calc_i_pos_After_C_Gen(K, C, i));
				}
			}
		}		
