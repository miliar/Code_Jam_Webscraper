...
ZS mul(ZS a, ZS b)
{
    ...
    ZS t=MUL[a.second][b.second];
    return {s*t.first, t.second};
}
ZS pa(char x)
{
    static map<char, Z> H {{'i', I}, {'j', J}, {'k', K}};
    ass(H.count(x));
    return {1, H[x]};
}
const char* go(string s)
{
    int n=s.length();
    vector<ZS> ca(n);
    REP(i, n) ca[i]=pa(s[i]);
    vector<vector<ZS>> m(n, vector<ZS>(n+1, {0, O}));
    REP(i, n)
    {
        m[i][i]={1, O};
        FOR(j, i, n) m[i][j+1]=mul(m[i][j], ca[j]);
    }
    ZS L={1, I}, M={1, J}, R={1, K};
    REP(m2, n) REP(m1, m2) if (m[0][m1]==L&&m[m1][m2]==M&&m[m2][n]==R)
    {
        return "YES";
    }
    return "NO";
