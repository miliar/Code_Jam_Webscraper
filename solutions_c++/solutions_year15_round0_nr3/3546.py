...
//            char test = string[0];
//            if(test == 'i') i = true;
//            else if(test == 'j') j = true;
//            else if(test == 'k') k = true;
...
//                if(i && j && k) break;
//                if(i == initialString.length()) test = getMult(test, initialString[0]);
//                else test = getMult(test, initialString[i]);
//            }

//            if(!i || !j || !k) goto endBad;
...
endGood:
    ...
    std::cout << "Time: " << std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now()-initTime).count() << " secs" << std::endl;
