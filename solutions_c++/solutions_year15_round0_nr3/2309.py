...
char quaternion(char a, char b) 
{
  bool negative = 0;
  char c;
  if (a > 'k') negative = 1 - negative, a -= 6;
  if (b > 'k') negative = 1 - negative, b -= 6;
  if (a == 'h') c = b;
  else if (b == 'h') c = a;
  else if (a == b) c = 'h', negative = 1 - negative;
  else if (a == 'i' && b == 'j') c = 'k';
  else if (a == 'i' && b == 'k') c = 'j', negative = 1 - negative;
  else if (a == 'j' && b == 'i') c = 'k', negative = 1 - negative;
  else if (a == 'j' && b == 'k') c = 'i';
  else if (a == 'k' && b == 'i') c = 'j';
  else if (a == 'k' && b == 'j') c = 'i', negative = 1 - negative;
  if (negative) c += 6;
  return c;
}

bool solve(string str, LL l, LL x) 
{
  LL i1, j1 = -1, i2, j2 = -1, i3, j3 = -1;
  LL p = (x > 4) ? 4 : x;
  char now = 'h';
  for(LL i = 0; i < p; ++i) {
    for(LL j = 0; j < l; ++j) {
      now = quaternion(now, str[j]);
      if (now == 'i') {
        i1 = i, j1 = j;
        break;
      }
  ...
  // cout<<i3<<' '<<j3<<endl;
  if (j3 == -1) return false;
  if (i1 + i3 >= x) return false;
  now = 'h';
  if (i1 + i3 + 1 == x) {   
    for(LL j = j1 + 1; j < j3; ++j)
      now = quaternion(now, str[j]);    
  } else {
    for(LL j = j1 + 1; j < l; ++j)
      now = quaternion(now, str[j]);
