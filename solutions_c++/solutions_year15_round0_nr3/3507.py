...
bool super(int indx, int part, int sign, int x);
void preprocess();
int convert(char x);
int mul(int x, int y);
int check(int left);
...
bool super(int indx, int part, int sign, int x)
{
    ...
    if(part == 0)
    {
        ret = ret | super(indx + 1, part, sign, x);
        int s = 0;
        if(first[indx] < 0)
            s = 1;

        if(first[indx] == 2)
            ret = ret | super(indx + 1, 1, 0, 1);
    }
    else if(part == 1)
    {
        ...
        if(tmp < 0)
            s = 1;
        ret = ret | super(indx + 1, part, s,  abs(tmp));
    }
