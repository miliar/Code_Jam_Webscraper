...
int main()
{
	...
	vector<string> vec;
	ifstream fichier("A-small-practice.in", ios::in);  // on ouvre le fichier en lecture
	int Ncases = 0;
	...
	if (fichier)  // si l'ouverture a réussi
	{
		...
		while (getline(fichier, ligne))  // tant que l'on peut mettre la ligne dans "contenu"
		{
			...
			{
				vector<string> ch_split = split(ligne);
				int L = atoi(ch_split[0].c_str());
				int X = atoi(ch_split[1].c_str());
				// cout<<"L = "<<L<<endl;
				// cout<<"X = "<<X<<endl;
				//cout<<"nb dinners "<<nb_dinners<<endl;
				getline(fichier, ligne);
				//   cerr<<"ligne chaine "<<ligne<<endl;
				string line = dijkstra(ligne, L, X);
				bool ok = find_i(line);
				cout << "ok = " << ok << endl;
				if (ok)
					resultat.push_back("YES");
				else
					resultat.push_back("NO");
				//cerr<<"ligne chaine "<<nb_min<<endl;
	...
	ofstream fichier1("dijkstra.out", ios::out | ios::trunc);  // ouverture en écriture avec effacement du fichier ouvert
