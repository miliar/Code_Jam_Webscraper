...
const char prod(const char& a, const char& b, bool& sign)
{
	if (a == b)
	{
		sign= 1;
...
const char prod2(const char& a, const char& b, bool& sign)
{
	if (a == b)
	{
		sign= 1;
...
bool IsSubRes(const char sr, const char*& input)
{
	const char* i= input;
	char c_v= *i;
	bool c_s= false;
	bool possible = sr == c_v;
...
int program::my_program()
{
	...
	vector <unsigned int> a= u_convert(argv[vi]);

	unsigned int _i= a[0];
	unsigned int j= a[1];
	unsigned int k= a[2];
	
	string r= argv[vi +1][0];
	string str= ""; 
	...
	if (str.length() >2)
	{
		...
		const char* org_index= str.c_str();
		const char* index= org_index;

		possible= possible && IsSubRes('i', index);
		possible= possible && IsSubRes('j', index);
		possible= possible && IsSubRes('k', index);

		while (possible && *index)
			possible= possible && IsSubRes('1', index);
