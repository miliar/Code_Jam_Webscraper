...
int main()
{
    ...
    for(int yyy=1;yyy<=t;yyy++)
    {
        ...
        if(ch[1]=='i') inceput.push_back(1);
        char val=ch[1];
        for(int i=2;i<=n-2;i++)
        {
            val=calc(val,ch[i]);
            if(val=='i') inceput.push_back(i);
        }
        if(ch[n]=='k') sfarsit.push_back(n);
        val=ch[n];
        for(int i=n-1;i>=3;i--)
        {
            val=calc(ch[i],val);
            if(val=='k') sfarsit.push_back(i);
        }
        int ss1=inceput.size(),ss2=sfarsit.size();
        for(int it1=0;it1<ss1;++it1)
        {
            for(int it2=0;it2<ss2;++it2)
            {
                if(inceput[it1]+1>=sfarsit[it2]) break ;
                val=query(1,n,inceput[it1]+1,sfarsit[it2]-1,1);
                if(val=='j')
                {
                    ast=1;
                    break;
