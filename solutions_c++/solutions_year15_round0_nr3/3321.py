...
//.016-.040-.900-2.48
/***************************************************************************************************************************************/
char inp[1000001],CC[1000010];
pair<int,int>NextState(int agr1Sign,int arg1val,int arg2Sign, int arg2val)
{
    PII ret;
    ret.first=agr1Sign*arg2Sign;
    if(arg1val=='1')
    {
        ret.second = arg2val;
    }
    ...
    {
        if(arg2val=='1'){ret.snd='i';}
        if(arg2val=='i'){ret.snd='1';ret.first*=-1;}
        if(arg2val=='j'){ret.snd='k';}
        if(arg2val=='k'){ret.snd='j';ret.first*=-1;}
    }
    else if(arg1val=='j')
    {
        if(arg2val=='1'){ret.snd='j';}
        if(arg2val=='i'){ret.snd='k';ret.first*=-1;}
        if(arg2val=='j'){ret.snd='1';ret.first*=-1;}
        if(arg2val=='k'){ret.snd='i';}
    }
    else if(arg1val=='k')
    {
        if(arg2val=='1'){ret.snd='k';}
        if(arg2val=='i'){ret.snd='j';}
        if(arg2val=='j'){ret.snd='i';ret.first*=-1;}
        if(arg2val=='k'){ret.snd='1';ret.first*=-1;}
    }
    return ret;
}
int main()
{
    filein("C-small-attempt1.in");
    fileout("C-small-out.txt");
    int kase,ks=0;
    asd(kase);
    int L,X;
    while(kase--)
    {
        ...
        for(int i=1;i<=X;i++)
        {
            for(int j=0;j<L;j++)CC[idx++]=inp[j];
        }
        CC[idx]='\0';
        bool seen_I=false;
        bool seen_K=false;
        ...
        for(int i=0;i<idx;i++)
        {
            PII temp = NextState(signa,vala,1,CC[i]);
            signa=temp.fst;
            vala=temp.snd;
            if(signa==1 && vala=='i')seen_I=true;
            if(signa==1 && vala=='k' && seen_I==true)seen_K=true;
            //cout << (signa==1?"+":"-") << (char)vala  << endl;
        }
        //cout << signa <<  (char)vala << seen_I << seen_MinusK << endl;
        printf("Case #%d: ",++ks);
        //if(signa==-1 && vala=='1' && seen_I && seen_MinusK)
        if(signa==-1 && vala=='1' && seen_I && seen_K)
        {
            puts("YES");
        }
        else
