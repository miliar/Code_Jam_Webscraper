...
bool solve(const string& tok, ll repeat) {
	...
	bool neg = false, new_neg = false, hasAll = false;
	ll cycle_len = LLONG_MAX;
	for (ll i = 0; i < size; ++i) {
		//if (cycle_len != LLONG_MAX && i >= cycle_len + tok.size()) break;
		...
		//if (!neg && ch == '1' && (i + 1) % tok.size() == 0 && cycle_len == LLONG_MAX)
		//	cycle_len = i + 1;
	}
