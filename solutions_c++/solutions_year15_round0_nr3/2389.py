...
char quat[4] = { '1', 'i', 'j', 'k' };
typedef long long intlong;
std::vector<intlong> ConvertSpacedInts(std::string& line)
{
	std::vector<intlong> integers;
...
std::string solve(std::vector<char>& LV, int L, intlong X)
{
	assert(LV.size() == L);
		...
		for (unsigned int xB = 0; (xB <4); xB++)
		{
			...
			for (int l = 0; l < L; l++)
			{
				...
				if (PLC[l] == ISC&&PLS[l] == ISS)
				{
					...
					if ((intlong)xF + 1 + (intlong)xB + 1 - X>0) continue;
					JSC = JSRC;
					JSS = JSRS;
...
void parsecase(std::ifstream& inputfile, int& L, intlong& X, std::string& ijks)
{
	std::string line;
	std::getline(inputfile, line);
	std::vector<intlong> LX = ConvertSpacedInts(line);
	L = LX[0];
	X = LX[1];
	assert(LX.size() == 2);
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	std::ifstream inputfile("C-large.in");
	...
	intlong X;
	std::string ijks;
	std::ofstream output_test("C-largeoutput.txt");
