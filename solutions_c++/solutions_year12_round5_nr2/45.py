...
pii root(pii p) {
  if (par.count(p)) return par[p] = root(par[p]);
  else return p;
}
...
void pre(int x, int y, int e) {
  bool xc = x == 1 || x == S || x == S + S - 1;
  bool yc = y == 1 || y == S || y == S + S - 1;
  if (xc && yc) corn[mp(x, y)] = 1;
  else edge[mp(x, y)] = 1 << e;
  // printf("%d %d: %d\n", x, y, xc & yc);
}
...
void solve() {
  ...
  rep (i, S - 1) pre(1            , 1 + i,         0);
  rep (i, S - 1) pre(1 + i        , S + i,         1);
  rep (i, S - 1) pre(S + i        , S + S - 1,     2);
  rep (i, S - 1) pre(S + S - 1    , S + S - 1 - i, 3);
  rep (i, S - 1) pre(S + S - 1 - i, S - i,         4);
  rep (i, S - 1) pre(S - i        , 1,             5);
  ...
  rep (i, M) {
    ...
    rep (d, 6) {
      int tx = x + dx[d];
      int ty = y + dy[d];
      if (pos.count(mp(tx, ty))) {
        ++E;
        pii p1 = root(mp(x, y)), p2 = root(mp(tx, ty));
        if (p1 != p2) {
          --cc;
          pii p = par[p1] = p2;
          edge[p] = edge[p1] | edge[p2];
          corn[p] = corn[p1] + corn[p2];
        }
      }
    }
    ...
    rep (d, 6) {
      pii p = mp(x                  , y);
      pii q = mp(x + dx[d          ], y + dy[d          ]);
      pii r = mp(x + dx[(d + 1) % 6], y + dy[(d + 1) % 6]);
      if (pos.count(p) && pos.count(q) && pos.count(r)) {
        ++F;
      }
    }
    ...
    pii p = root(mp(x, y));
    ring   = cc - V + E - F;
    fork   = __builtin_popcount(edge[p]) >= 3;
    bridge = corn[p] >= 2;

    if (ring || bridge || fork) {
      // printf("%d %d\n", S, M);
      if (bridge) printf("bridge");
      if (fork)   printf("%sfork", bridge ? "-" : "");
      if (ring)   printf("%sring", bridge || fork ? "-" : "");
      printf(" in move %d\n", i + 1);
      return;
    }
  }
