...
public:
	...
	friend ostream& operator <<(ostream& out, const Answer &a){
		for (int i = 0; i < a.ans.size(); ++i) out << "\n" << a.ans[i];
...
void* solve(void *_id){
	...
	for (int i = 0; i < n; ++i)
	{
		...
		for (int j = 0; j < m; ++j)
		{
			if (ss[i][j] != '?') s += ss[i][j], v.push_back(j);
		}
		if (!s.size()) continue ;
		for (int ii = cur; ii <= i; ++ii)
		{
			for (int j = 0; j < (v.size() > 1 ? v[1] : m); ++j)
			{
				ss[ii][j] = s[0];
			}
		}
		for (int k = 1; k < s.size(); ++k)
		{
			for (int ii = cur; ii <= i; ++ii)
			{
				for (int j = v[k]; j < (v.size() > k+1 ? v[k+1] : m); ++j)
				{
					ss[ii][j] = s[k];
				}
			}
