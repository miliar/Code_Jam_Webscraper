...
void solve(int casenum, outboth rout) {
  ...
  int R, C; cin >> R >> C;
  vector<string> cake(R); read(cake);
  dout << db(R) << db(C) << show(cake) << endl;
  ...
  for (int r = 1; r < R; ++r) {
    if (cake[r][0] == '?') copy(All(cake[r-1]), cake[r].begin());
  }
  for (int r = R-2; r >= 0; --r) {
    if (cake[r][0] == '?') copy(All(cake[r+1]), cake[r].begin());
  }
...
// clang-format off
// // compile-command: "/opt/brew/opt/llvm/bin/clang++ -fmodules -std=c++11 -Wall -Wextra -g -O3 -o A.O A.cc && time ./A.O A.test.in"
...
// Local variables:
// compile-command: "/opt/brew/opt/llvm/bin/clang++ -fmodules -fsanitize={undefined,address} -std=c++11 -Wall -Wextra -g -o A A.cc && time ./A A.test.in"
