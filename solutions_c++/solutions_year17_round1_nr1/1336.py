...
int main()
{
	...
	for (int test_i = 0; test_i < test_n; test_i++) {
		...
		for (int k = 0; k < r; k++) {
			for (int i = 0; i < c; i++) {
				if (v[k][i] != '?') {
					for (int ri = i + 1; ri < c && v[k][ri] == '?'; ri++) v[k][ri] = v[k][i];
					for (int li = i - 1; li >= 0 && v[k][li] == '?'; li--) v[k][li] = v[k][i];
				}
			}
		}
		...
		for (int k = 0; k < c; k++) {
			for (int i = 0; i < r; i++) {
				if (v[i][k] != '?') {
					for (int ru = i - 1; ru >= 0 && v[ru][k] == '?'; ru--) v[ru][k] = v[i][k];
					for (int rd = i + 1; rd < r && v[rd][k] == '?'; rd--) v[rd][k] = v[i][k];
				}
			}
		}

		fout << "Case #" << test_i + 1 << ":" << endl;
		for (auto e : v) {
			fout << e << endl;
		}
