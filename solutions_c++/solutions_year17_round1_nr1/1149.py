...
void Runner::Run()
{
	...
	vector<bool> EmptyRow(R);
	FORS(r, EmptyRow)
	{
		int cnt = (int)count(M[r].begin(), M[r].end(), '?');
		EmptyRow[r] = (cnt == C);
	}

	map<int, vector<int>> RowsToFill; // not empty, what to fill
	FORS(r, EmptyRow)
	{
		if (!EmptyRow[r])
			RowsToFill[r].push_back(r);
	}
	FORS(r, EmptyRow)
	{
		if (EmptyRow[r])
		{
			auto it = RowsToFill.lower_bound(r);
	...
	for (auto it = RowsToFill.begin(); it != RowsToFill.end(); ++it)
	{
		int r = it->first;
		...
		FORS(c, M[r])
		{
			...
			FORS(rowToFill, it->second)
			{
				M[it->second[rowToFill]][c] = curChar;
			}
		}
	}
