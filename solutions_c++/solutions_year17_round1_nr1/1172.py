...
int CodeJamMain()
{
    ...
    for (int caseNum = 0; caseNum < T; caseNum++)
    {
        ...
        char cake[26][26];
        for (int i = 0; i < R; i++) input >> cake[i];

        // general strategy: extrude right, extrude left, extrude down, extrude up...
        ...
        for (int r = 0; r < R; r++)
        {
            ...
            for (int c = 0; c < C; c++)
            {
                if (cake[r][c] == '?')
                {
                    if (rows[r] != '?') cake[r][c] = rows[r];
                }
                else rows[r] = cake[r][c];
            }
            memset(rows, '?', 26);
            for (int c = C - 1; c >= 0; c--)
            {
        ...
        for (int c = 0; c < C; c++)
        {
            ...
            for (int r = 0; r < R; r++)
            {
                if (cake[r][c] == '?')
                {
                    if (cols[c] != '?') cake[r][c] = cols[c];
                }
                else cols[c] = cake[r][c];
            }
            memset(cols, '?', 26);
            for (int r = R - 1; r >= 0; r--)
            {
        ...
        for (int r = 0; r < R; r++)
        {
            for (int c = 0; c < C; c++) answer << cake[r][c];
            if(r != R - 1) answer << endl;
        }
