...
void fillRow(int i) {
    int j = 0;
    while (grid[i][j] == '?') j++;
    char toClone = grid[i][j];
    for (int k=0; k<j; k++) grid[i][k] = toClone;
    for (++j; j<c; ++j) {
        if (grid[i][j] == '?') grid[i][j] = toClone;
        else toClone = grid[i][j];
    }
}

void cloneRow(int i, int ii) {
    for (int j=0; j<c; j++) grid[ii][j] = grid[i][j];
}
...
void doCase(int case_number) {
    ...
    for (int i=0; i<r; i++) {
        scanf("%s",grid[i]);
        empty[i] = true;
        for (int j=0; j<c; j++) if (grid[i][j] != '?') empty[i] = false;
    }

    int i = 0;
    while (empty[i]) i++;
    fillRow(i);
    for (int ii=0; ii<i; ii++) cloneRow(i,ii);

    for (++i; i<r; ++i) {
        if (empty[i]) cloneRow(i-1,i);
        else fillRow(i);
    }
