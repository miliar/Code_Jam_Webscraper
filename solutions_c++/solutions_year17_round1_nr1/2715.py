...
bool np[pSz + 10]; vi prime; int prime_size;void sieve(){np[0] = np[1] = 0;prime.pb(2);for(LL i = 4; i <= pSz; i+=2) np[i] = 1;for(LL i = 3; i <= pSz; i+=2){if(!np[i]){prime.pb(i);for(LL j = i * i; j <= pSz; j += (i << 1)) np[j] = 1;}}prime_size = prime.size();}
#endif
...
void ans_me(vector <string> vv, set <char> st)
{
    int notDone = 0;
    if(done) return;
    bool noUpdate = true;
    Rep(i1, row) Rep(j1, col) {
            if(vv[i1][j1] == '?') notDone = 1;
            For(i2, i1, row) For(j2, j1, col) {
            int cnt, typ, bad;
            cnt = typ = bad = 0;
            char who = '?';
//            cout << "before : " << i1 << ' ' << i2 << ' ' << j1 << ' ' << j2 << '\n';
//            Rep(i, row) cout << vv[i] << '\n';
            For(x, i1, i2 + 1) {
                For(y, j1, j2 + 1) {
                    if(vv[x][y] == '?') continue;
            ...
            if(bad) continue;
            if(cnt == (i2 - i1 + 1) * (j2 - j1 + 1)) continue;
            if(st.find(who) != st.end()) continue;
            noUpdate = false;
            vector <string> in = vv;
            For(x, i1, i2 + 1) For(y, j1, j2 + 1) in[x][y] = who;
//            cout << "after\n";
//            Rep(i, row) cout << in[i] << '\n';
            set <char> nw(all(st));
            nw.insert(who);
