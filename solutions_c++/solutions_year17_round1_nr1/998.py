...
class Main{
	...
	void run(){
		...
		for(int q:range(T)){
            int Y,X;cin >> Y >> X;
            vector<string> board(Y);cin >> board;
            vector<string> iboard = board;

            map<char,tuple<int,int,int,int>> rmap;
            for(char c:range('A','Z'+1)){
                int yl = Y,yr = 0,xl = X,xr = 0;
                for(int y:range(Y))for(int x:range(X))if(board[y][x] == c){
                    yl = min(yl,y), yr = max(yr,y+1);
                    xl = min(xl,x), xr = max(xr,x+1);
                }
                rmap[c] = make_tuple(yl,yr,xl,xr);
                for(int y:range(yl,yr))for(int x:range(xl,xr)){
                    board[y][x] = c;
                }
            }
            ...
            while(true){
                bool change = false;
                for(char c:range('A','Z'+1)){
                    int yl,yr,xl,xr; tie(yl,yr,xl,xr) = rmap[c];
                    if(yl == Y)continue;
                    if(yl-1 >= 0){
                        bool ok = true;
                        for(int x:range(xl,xr)) ok &= board[yl-1][x] == '?';
                        if(ok){
                            change = true;
                            for(int x:range(xl,xr)) board[yl-1][x] = c;
                            tie(yl,yr,xl,xr) = rmap[c] = make_tuple(yl-1,yr,xl,xr);
                        }
                    }
                    if(yr < Y){
                        bool ok = true;
                        for(int x:range(xl,xr)) ok &= board[yr][x] == '?';
                        if(ok){
                            change = true;
                            for(int x:range(xl,xr)) board[yr][x] = c;
                            tie(yl,yr,xl,xr) = rmap[c] = make_tuple(yl,yr+1,xl,xr);
                        }
                    }
                    if(xl-1 >= 0){
                        bool ok = true;
                        for(int y:range(yl,yr)) ok &= board[y][xl-1] == '?';
                        if(ok){
                            change = true;
                            for(int y:range(yl,yr)) board[y][xl-1] = c;
                            tie(yl,yr,xl,xr) = rmap[c] = make_tuple(yl,yr,xl-1,xr);
                        }
                    }
                    if(xr < X){
                        bool ok = true;
                        for(int y:range(yl,yr)) ok &= board[y][xr] == '?';
                        if(ok){
                            change = true;
                            for(int y:range(yl,yr)) board[y][xr] = c;
                            tie(yl,yr,xl,xr) = rmap[c] = make_tuple(yl,yr,xl,xr+1);
                        }
                    }
                }
                if(!change)break;
            ...
            for(int y:range(Y))for(int x:range(X))if(iboard[y][x] != '?'){
                if(board[y][x] != iboard[y][x]){
                    cerr <<"!" <<endl;
                }
            }
