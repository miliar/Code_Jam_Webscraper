...
int main()
{
  ...
  freopen("a__large_out.txt","w",stdout);
    ...
    rep(cas,1,tc)
    {
        ...
        rep(i,1,n)
        {
            double tot = pi*ar[i].r*ar[i].r;

//            vector<data>v;
//            rep(j,i+1,n) v.push_back( ar[j] );
//
//            sort( all(v),cmp );
//            v.push_back( ar[i] );
//
...
//            }
            ...
            rep(j,i+1,n)
            {
                vv.PB( 2.0*pi*ar[j].h * ar[j].r );
            }

            sort( all(vv) );

            vv.PB( 2.0*pi*ar[i].h*ar[i].r );
