...
//  GoogleCodeJam
//
//  Created by Rumman Mahmud on 4/30/17.
...
int vp[1050][1050][2],n,k;
double dp[1050][1050][2];

int loop=0;

double getCirc(int i)
{
    return PI*(val[i].fs*val[i].fs);
}

double getCyl(int i)
{
    return 2.0*PI*(val[i].fs*val[i].sc);
}

double getArea(int i)
{
    return getCyl(i) + getCirc(i);
}
double rec(int i,int j,int f)
{
    if(i==n)
    {
        if(j==0) return 0;
    ...
    if(j)
    {
        double add=getCirc(i);
        if(f) add=0;
        ret=max(ret,rec(i+1,j-1,1)+getCyl(i)+add);
    }
    return ret;
}
