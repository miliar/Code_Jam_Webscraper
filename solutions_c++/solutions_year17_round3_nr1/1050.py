...
//	freopen("out", "w", stdout);
	...
	for(cs = 1; cs <= tc; cs ++) {
		...
		for(i = 0; i < n; i ++) {
			...
			v.PB({2 * PI * (double)r[i].F * (double)r[i].S, i});
		}
		vsort(v);
		double sum = 0;
		ans = 0;
		...
		for(i = n - 1, j = 1; j <= k; j ++, i --) {
			ans = max(ans, sum + PI * SQR( (double) r[ v[i].S ].F));
		}
		sum -= v[i + 1].F;
		for( ; i >= 0; i --) {
			ans = max(ans, sum + 2 * PI * (double)r[ v[i].S ].F * (double)r[ v[i].S ].S + PI * SQR( (double) r[ v[i].S ].F));
		}
		printf("Case #%d: %.10lf\n", cs, ans);
		v.clear();
	}
