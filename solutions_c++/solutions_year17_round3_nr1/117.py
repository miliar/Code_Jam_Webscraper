...
double dp(int id, int Ki, bool first) {
	if(id == cakes.size()-1 && Ki > 0) {
		return memo[id][Ki][first] = (double) 2*PI*cakes[id].first*cakes[id].second + first * PI * cakes[id].first * cakes[id].first ;
	}
	if(id == cakes.size()-1 && Ki == 0) {
	...
	double maxHeight = 0;
	//cout << id << " " << Ki << " " << dp(id+1, Ki, first) << " " << dp(id+1, Ki-1, false) + 2*PI*cakes[id].first*cakes[id].second + first * PI * cakes[id].first * cakes[id].first << endl;
	maxHeight = max(dp(id+1, Ki, first), dp(id+1, Ki-1, false) + 2*PI*cakes[id].first*cakes[id].second + first * PI * cakes[id].first * cakes[id].first);
