...
int main() {
    ...
    for (int C = 1; C <= T; ++C) {
        ...
        rep(bottom_i, n) {
            ...
            rep(i, n) {
                if (i != bottom_i && r[i] <= r[bottom_i]) {
                    surface.push_back(2 * PI * r[i] * h[i]);
                }
            }
            if (surface.size() >= k - 1) {
                sort(rall(surface));
                const double sum_surface = PI * r[bottom_i] * r[bottom_i]
                    + 2 * PI * r[bottom_i] * h[bottom_i]
                    + accumulate(surface.begin(), surface.begin() + (k - 1), 0.0);
                upmax(opt, sum_surface);
            }
        }
        printf("Case #%d: %.9f\n", C, opt);
    }
