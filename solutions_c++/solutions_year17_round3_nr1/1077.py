...
int mark[MXN+3][MXN+3];

double rec(int pos, int rem){
    if( rem == 0 )  return 0.0;
    if( pos<0 ) return (-INF);
    double &ret = dp[pos][rem];
    if( mark[pos][rem] == cas ) return ret;
    mark[pos][rem] = cas;
    double res1 = (double)2.0*PI*double(arr[pos].fs)*double(arr[pos].sc);
    if( rem == k )
        res1 += (PI*double(arr[pos].fs)*double(arr[pos].fs));
    res1 += rec(pos-1, rem-1);
    double res2 = rec(pos-1, rem);
    ret = max(res1, res2);
    return ret;
}

int main(){
//    freopen("D:\\Solutions\\00.txt", "r", stdin);
