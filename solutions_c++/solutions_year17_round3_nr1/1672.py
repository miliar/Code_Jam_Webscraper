...
pii a[maxn+2];
double res;
double mem[maxn+2][maxn+2];
bool u[maxn+2][maxn+2];


double dp(int kk, int p) {
    if (p>=n && kk > 0) return -1.0;
    if (p>=n && kk == 0) return 0.0;
    ...
    if (kk==0) {
        ...
        double add1 = 2 * M_PI * a[p].x * a[p].y;
        if (kk == k) {
            add1 += M_PI * a[p].x * a[p].x;
        }
        //printf("add1 %d %lf\n", p, add1);
        double c = dp(kk - 1, p + 1);
        if (c != -1.0) {
            r = max(r, add1 + c);
        }
