...
int main(int, char *[])
{
    freopen("C:/Users/Infinity/Downloads/A-large.in", "r", stdin);
    freopen("C:/Users/Infinity/Downloads/A-large.out", "w", stdout);
    dbl_prec = 9;
    ldbl_prec = 9;
    for (int ti = 1, tt = getint(); ti <= tt; ++ti) {
        int n = getint(), k = getint();
        vector<pair<long double, long double>> rs;
        for (int i : range(n)) {
            int r = getint();
            rs.emplace_back(pi * r * r, 2 * pi * r * getint());
        }
        sort(rs, [](ConstRef<pair<long double, long double>> p1, ConstRef<pair<long double, long double>> p2){
            return p1.second > p2.second;
        });
        long double ans = 0, psum = 0;
        for (int i : range(k - 1)) {
            psum += rs[i].second;
        }
        for (int i : range(n)) {
            long double cans = rs[i].first + psum;
            cans += (i < k - 1 ? rs[k - 1].second : rs[i].second);
            ans = max(ans, cans);
        }
        writeln(caseN(ti), ans);
    }
    return 0;
}
