...
void Solver::read(std::istream &in) {
    ...
    for (int i = 0; i < n_; ++i) {
        r_.push_back(nextValue<int>(in));
        h_.push_back(nextValue<int>(in));
    }
}

void Solver::saw(std::ostream &out) {
    std::vector<int> ix(n_);
    std::iota(ix.begin(), ix.end(), 0);
    std::sort(ix.begin(), ix.end(), [this](int a, int b) { return r_[a] > r_[b];});
    vector<double> r(n_);
    vector<double> h(n_);
    vector<double> s(n_);
    for (int i = 0; i < n_; ++i) {
    ...
    double best = 0;
    for (int b = 0; b < n_ - k_ + 1; ++b) {
        vector<double> t = s;
        t[b] += r[b] * r[b] * pi;
        std::sort(t.begin() + b, t.end(), std::greater<double>());
        double sum = std::accumulate(t.begin() + b, t.begin() + b + k_, 0.0);
        if (sum > best)
            best = sum;
    }
