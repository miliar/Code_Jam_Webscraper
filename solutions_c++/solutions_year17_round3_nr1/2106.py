...
long long f(int n, int s, int k) {
	...
	long long mx = 0;
	for (int i = s; i <= n - k; i++) {
		mx = max(mx, v[i].first * v[i].second + f(n, i + 1, k - 1));
	}
	d[s][k] = mx;
	return mx;
}
...
int main() {
	...
	for (int cs = 1; cs <= T; cs++) {
		...
		double ret1 = 0.0;
		for (int i = 0; i<=n-k; i++) {
			double ret = 1.0L*PI*(double)v[i].first*(double)v[i].first + 2.0L * PI*(double)v[i].first*(double)v[i].second;
			memset(d, 0, sizeof(d));
			ret += 2.0L*PI*f(n, i+1, k-1);
			ret1 = max(ret1, ret);
		}
