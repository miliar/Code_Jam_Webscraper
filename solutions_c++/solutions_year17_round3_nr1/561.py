...
typedef ld otype;
otype calcFunction(ll N, ll K, v(ll) pancakesR, v(ll) pancakesH) {
	otype result=0;
	ll resultNoPi = 0;
	ll count = 0;
	deque<pair<ll, ll>> RHandR;
	for(int i=0; i<N; i++) {
		RHandR.pb(make_pair(2*pancakesR[i]*pancakesH[i], powll(pancakesR[i], 2)));
	}
	sort(RHandR.begin(), RHandR.end());
	reverse(RHandR.begin(), RHandR.end());
	
	ll maxR = 0;
	for(int i=0; i<K-1; i++) {
		resultNoPi += RHandR[i].first;
		if(RHandR[i].second > maxR) maxR = RHandR[i].second;
	}
	
	ll maxAdd = 0;
	for(int i=K-1; i<N; i++) {
		ll addR = max(maxR, RHandR[i].second);
		ll curAdd = addR + RHandR[i].first;
		//cout << RHandR[i].first << " " << RHandR[i].second << endl;
		if(curAdd > maxAdd) maxAdd = curAdd;
	}
	resultNoPi += maxAdd;
	//cout << resultNoPi << endl;
	result = resultNoPi*pi;
	return result;
}
...
int main() {
	...
	fore(test, 1, tests){
		...
		otype result=calcFunction(N, K, pancakesR, pancakesH);
		outfile << "Case #" << test << ": ";
		cout << "Case #" << test << ": ";
#ifndef IFSTRUCT
		outfile << result << endl;
