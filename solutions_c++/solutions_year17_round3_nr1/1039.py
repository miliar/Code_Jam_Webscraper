...
int main(){
    ...
    while(ts--){
        ...
        for(int i=1;i<=n;++i){
            cin>>p[i].r>>p[i].h;
            p[i].area=p[i].r*p[i].r*PI;
            p[i].side=2.0*p[i].r*PI*p[i].h;
        }
        sort(p+1,p+1+n,[](const pie &a,const pie &b){return a.r>b.r;});
        for(int i=1;i<=n;++i){
            for(int j=1;j<=i;++j){
                // dp[i][j]=front i take j
                if(j==1){
                    dp[i][j]=max(dp[i-1][j],p[i].area+p[i].side);
                }
                else{
                    dp[i][j]=max({dp[i-1][j],dp[i-1][j-1]+p[i].side});
                }
            }
        }
        // for(int i=1;i<=n;++i){
