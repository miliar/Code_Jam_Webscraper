...
void open(){
	freopen("D:/OneDrive/Code/GCJ/A-large (3).in","r",stdin);
...
double tinggiarea(int idx){
	// cout << idx << " " << 2.0 * PI * pancake[idx].first * pancake[idx].second << endl; 
...
double dp(int idx, int sisa){
	if(sisa == 0)return 0;
	if(idx < 0)return -INF;
	if(memo[idx][sisa] >= 0)return memo[idx][sisa];

	double ans = 0;
	if(sisa == k){
		ans = max(dp(idx - 1, sisa), dp(idx - 1, sisa - 1) + tinggiarea(idx) + base(idx));
	}else if(sisa > 0){
