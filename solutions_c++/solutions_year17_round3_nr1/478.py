...
istream& operator >>(istream& istr, InputData& inputData) {
	istr >> inputData.n >> inputData.k;
	inputData.h.resize(inputData.n);
	inputData.r.resize(inputData.n);
	for (int i = 0; i < inputData.n; ++i) {
		istr >> inputData.r[i] >> inputData.h[i];
	}
	return istr;
}

ostream& operator <<(ostream& ostr, const OutputData& outputData) {
	ostr << setprecision(10) << fixed;
	ostr << outputData.square;
	return ostr;
}
...
OutputData solve(InputData inputData) {
	...
	for (int i = 0; i < n; ++i) {
		a[i] = {inputData.r[i], inputData.h[i]};
	}
	sort(all(a));
	long double res = 0;
	vector<double> tmp;
	for (int i = 0; i < k - 1; ++i) {
		tmp.push_back(2.0 * a[i].first * a[i].second);
	}
	sort(tmp.rbegin(), tmp.rend());
	for (int i = k - 1; i < n; ++i) {
		long double qw = accumulate(all(tmp), 0.0) + 1.0 * a[i].first * a[i].first + 2.0 * a[i].first * a[i].second;
		tmp.push_back(2.0 * a[i].first * a[i].second);
		sort(tmp.rbegin(), tmp.rend());
		tmp.pop_back();
		res = max(res, qw);
