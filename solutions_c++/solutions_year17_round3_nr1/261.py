...
bool pancakeCompare(const P &ls,const P &rs)
{
    return ls.x>rs.x;
}

bool largerFirst(const LL &ls,const LL &rs)
{
    return ls>rs;
}
...
LL getMe(int baseId)
{
    ...
    for(int i=0;i<selectLeft;i++) ret+=bank[i];
    return ret;
}
...
double solve()
{
    ...
    LL best = 0;
    for(int i=0;i<n;i++) best = max(best,getMe(i));
    double ans = PI;
    ans*= best;
