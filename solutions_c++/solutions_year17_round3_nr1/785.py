...
int main(void) {
    ...
    rep(_, t) {
        double ret = 0;
        ll n, k; cin >> n >> k;
        vector<tuple<double, double>> rh(n);
        rep(i, n) {
...
//        cout << rh << "#RH" << endl;
        rep(i, n) {
            ...
            vector<double> areas;
            repi(j, i+1, n) {
                areas.pb(M_PI*2.0*get<0>(rh[j])*get<1>(rh[j]));
            }
            if (areas.size() < k-1) break;
            sort(all(areas), greater<double>());
//            cout << i << " " <<areas << endl;
            rep(j, k-1) {
                cand += areas[j];
            }
            cand += M_PI * get<0>(rh[i]) * get<0>(rh[i]) + 2.0 * M_PI * get<0>(rh[i]) * get<1>(rh[i]);
            chmax(ret, cand);
        }
