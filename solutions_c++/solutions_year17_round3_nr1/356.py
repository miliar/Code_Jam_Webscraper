...
int visited[MAXX][MAXX], cc = 0;
double dp[MAXX][MAXX];

double rec(int pos, int k)
{
        if(pos >= N)
        {
                return -INF;
        ...
        double tmp = (2.0 * PI) * cakes[pos].fr * cakes[pos].sc;

        if(k == 1)
        {
                tmp += PI * cakes[pos].fr * cakes[pos].fr;
        }
        else
        {
                tmp += rec(pos + 1, k - 1);
...
int main ()
{
    ...
    while(kases--)
    {
            ...
            loop(i, N)
            {
                    sf("%lf %lf", &cakes[i].fr, &cakes[i].sc);
            }
