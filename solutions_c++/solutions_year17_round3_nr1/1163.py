...
int vis[MX][MX]; double dp[MX][MX]; int cases;
pll a[MX];
int n;


double f(int i, int rem) {
	if(rem == 0) return 0;
	if(i > n) return -inf;

	double &ret = dp[i][rem];
	if(vis[i][rem] == cases) return ret;
	vis[i][rem] = cases;

	ret = f(i+1, rem);
	ret = max(ret, 2.0*PI*a[i].F*a[i].S + f(i+1, rem-1));
...
int main () {
	...
	REP(cs, t) {
		...
		double res = 0;
		REP(i, n) res = max(res, PI*a[i].F*a[i].F + 2.0*PI*a[i].F*a[i].S + f(i+1, k-1));
