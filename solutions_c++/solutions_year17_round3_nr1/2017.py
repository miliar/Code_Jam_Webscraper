...
pair<ll,int> arr[1001];
ll ans;

int sz,k;

void rec(int n, bool usd[], ll cur, int r){
    /* whatis(n); */
    /* whatis(r); */
    /* whatis(cur); */
    if(dp[mp(n,r)] < cur)
        dp[mp(n,r)] = cur;
    if(n == k){
        ans = max(ans,dp[mp(n,r)]);
        return;
    }
    FOR(i,0,sz){
        if(usd[i] || arr[i].e1 > r) continue;
        ll sth = 2*arr[i].e1*arr[i].e2;
        if(cur + sth > dp[mp(n+1,arr[i].e1)])
            dp[mp(n+1,arr[i].e1)] = cur + sth;
        usd[i] = 1;
        rec(n+1,usd,cur+sth,arr[i].e1);
        usd[i] = 0;
    }
}
...
int main(){
    ...
    FORE(t,1,N){
        ...
        sc(sz,k);
        FOR(i,0,sz)
            sc(arr[i].e1,arr[i].e2);
        bool usd[sz];
        memset(usd,0,sz);
        FOR(i,0,sz){
            ll cur = arr[i].e1*arr[i].e1 + 2*arr[i].e1*arr[i].e2;
            if(cur > dp[mp(1,arr[i].e1)]){
                dp[mp(1,arr[i].e1)] = cur;
                usd[i] = 1;
                rec(1,usd,cur,arr[i].e1);
                usd[i] = 0;
            }
        }
        cout << (long double)M_PI*ans << '\n';
        dp.clear();
        ans = 0;
    }
}
