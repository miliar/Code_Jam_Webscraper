...
LL go(int a, int b, int f) {
    ...
    if(ret != -1) return ret;
    if(f) {
        if(b) ret = max(ret, d[a].r * d[a].h * 2 + go(a - 1, b - 1, f));
        ret = max(ret, go(a - 1, b, f));
    } else {
        if(b) {
            ret = max(ret, d[a].r * d[a].h * 2 + go(a - 1, b - 1, f));
            ret = max(ret, d[a].r * (d[a].r + d[a].h * 2) + go(a - 1, b - 1, 1));
        }
        ret = max(ret, go(a - 1, b, f));
    }
    return ret;
...
int main() {
    ...
    CASE() {
        cin >> n >> k;
        FOR(i, 0, n) scanf("%lld%lld", &d[i].r, &d[i].h);
        SET(dp, -1);
        LL ans = go(n - 1, k, 0);
        printf("Case #%d: %.10lf\n", ++cs, PI * ans);
    }
