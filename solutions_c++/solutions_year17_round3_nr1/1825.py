...
int n,k;
pii a[1005];
long double inf=1e16,pi=acos(-1);
...
int main()
{
	...
	REP(l,1,t)
	{
		...
		REP(i,1,n)
		{
			multiset<long double>aux;
			aux.clear();
			long double pre=2*pi*a[i].F*a[i].S+pi*a[i].F*a[i].F;
			//debug2(a[i].F,pre);
			REP(j,1,n)
			{
				if(i==j)
				...
				if(a[i].F>=a[j].F)
				{
					//debug2(a[j].F,2*pi*a[j].F*a[j].S);
					aux.insert(-2*pi*a[j].F*a[j].S);
				}
			}
			int cnt=1;
			//debug(pre);
