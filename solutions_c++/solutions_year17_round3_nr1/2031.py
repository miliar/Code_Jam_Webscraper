...
void solve()
{
    ...
    REP(i, 0, possible.size())
    {
        ...
        val = PI * possible[i].fir * possible[i].fir;
        
        if(i+1 < possible.size()  &&  i+1 < K)
            val -= PI * possible[i+1].fir * possible[i+1].fir;
        
        val += 2 * PI * possible[i].fir * possible[i].sec;
        curr += val;
    }
...
int main()
{
    ...
    FOR(t, 1, T)
    {
        ...
        REP(i, 0, N)
            scanf("%d%d", &panc[i].fir, &panc[i].sec);
