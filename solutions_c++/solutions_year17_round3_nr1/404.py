...
double solve() {
  ...
  fo (i, K - 1, N - 1) {
    ...
    cur_result = M_PI * sqr(1.0*pancakes[i].fi);

    vd H (i + 1);
    re (j, i) {
      H[j] = 1.0 * pancakes[j].fi * pancakes[j].se;
    }
    sort (all(H));
    reverse (all(H));
  
    // H[K-1] = 1.0 * pancakes[i].fi * pancakes[i].se;

    re (j, K - 1) {
      cur_result += 2 * M_PI * H[j];
    }
    cur_result += 2 * M_PI * pancakes[i].fi * pancakes[i].se;

    remax(result, cur_result);
  }
