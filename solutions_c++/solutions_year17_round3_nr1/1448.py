...
double rec(int pos,int rem,int flag)
{
    if(rem==0) return 0;
    if(pos>=n+1){
       // deb(rem,flag);
    ...
    double &ret = dp[pos][rem][flag];
    bool &vv = vis[pos][rem][flag];
    if(vv==1) return ret;
    vv = 1;

    ret = -1e200;

    ret = max(ret,rec(pos+1,rem,flag));

    if(flag==0){
        ret = max(ret,rec(pos+1,rem-1,1) +
                  (2.0*PI*ara[pos].rad*ara[pos].hgt) +
                  (PI*ara[pos].rad*ara[pos].rad));
