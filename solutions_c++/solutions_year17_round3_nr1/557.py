...
int main()
{
    ...
    while(tc--)
    {
        ...
        for(int i=n;i>=1;i--)
        {
            res[i][1]=2.0*po*a[i].x*(a[i].x+a[i].y)-po*a[i].x*a[i].x;
            double xo=res[i][1]-po*a[i].x*a[i].x;


            for(int j=2;j<=m;j++)
            {
                    res[i][j]=max(res[i][j],res[i+1][j-1]+xo);
            }

            for(int j=1;j<=m;j++)res[i][j]=max(res[i][j],res[i+1][j]);
        }
        cout<<fixed<<setprecision(10);
        cout<<"Case #"<<++cs<<": "<<res[1][m]<<endl;
    }
