...
double call(int cur, int cnt){
    if(cnt == K) return 0;
    if(cur == N) return 0;
    if(vis[cur][cnt] != -1) return DP[cur][cnt];

    double res = call(cur+1, cnt);
    double ret = call(cur+1, cnt+1) + A[cur].R*A[cur].H*2.0;
    res = max(res, ret);
...
int main() {
    ...
    for(int cs = 1;cs<=nCase;cs++){
        ...
        double res = 0;
        for(int i = 0;i<N;i++){
            double ret = A[i].R*A[i].R + A[i].R*A[i].H*2.0;
            ret += call(i+1, 1);
            res = max(res, ret);
        }
        //debug(res);
        res  = res*pi;
        cout << "Case #" << cs << ": " << setprecision(9) << res << endl;
    }
    return 0;
}
