...
bool exam_all_consant(string names) {
	cerr << "Examing " << names <<endl;
	REP(i, names.length()) {
		if (
			names.substr(i).find('a') != string::npos ||
			names.substr(i).find('i') != string::npos ||
			names.substr(i).find('o') != string::npos ||
			names.substr(i).find('u') != string::npos ||
			names.substr(i).find('e') != string::npos 
			) {
			return false;
		}
	}
...
void find_consec_pos()
{
	REP(i, tribname.length()-nvalue+1) {
		if (exam_all_consant(tribname.substr(i,nvalue))) {
			consec.push_back(i);
		}
	}
}

void print_consec_str(){
	REP(i, consec.size()) {
		cerr << tribname.substr(consec.at(i),nvalue) << endl;
	}
}

long comb(long n,long m){
	return (m==0||n==m)?1:comb(n-1,m)+comb(n-1,m-1);
}
...
//	print_consec_str();
	if (consec.size()==0) {
		...
		REP(i, consec.size()) {
			long posbegin = consec.at(i);
			long posend = consec.at(i)+nvalue-1;
			long comblocal = (i==0)? 
				(posbegin+1) * (tribname.length()-posend+1-1):
				(posbegin+1-(consec.at(i-1)+1)) * (tribname.length()-posend+1-1)	;
			comball+=comblocal;
		}
		if(consec.size()>1) {
		  //comball-=comb(consec.size(),2);
		}
	}
	cout << comball << endl;
}
...
int main(int argc, char **argv)
{
	...
	freopen ("outA-small-attempt0 (3).in.txt", "w", stdout);
