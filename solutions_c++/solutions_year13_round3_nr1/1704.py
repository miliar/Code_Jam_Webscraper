...
void solve(istream& in, ostream& fout, int caseIdx)
{
	...
	{
		int subs_len = i;
		for(int head = 0; head + subs_len < src.size() + 1; ++head)
		{
			string subs = src.substr(head, subs_len);
			...
			//exist.insert(subs);
			exist.push_back(subs);
		}
	}

	int c = exist.size();
	OUT( << CaseHead(caseIdx+1) << c << endl );
}
