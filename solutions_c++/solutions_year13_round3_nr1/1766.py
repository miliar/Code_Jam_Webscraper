...
int main(void){
  ...
  for(int p=0; p<cases; p++){
     ...
     for(int i=0;i<(int)name.size();i++){
       bool extent = false;
       for(int j=i;j<(int)name.size();j++){
	 if(extent == true) {sum++; continue;}
	 if(j >= (int) name.size()) { break;}
	 int flag = 0;
	 bool ok = false;
	 for(int k=i;k<=j;k++){
	   if(name[k]=='a'||name[k]=='i'||name[k]=='u'||name[k]=='e'||name[k]=='o'){
	     flag = 0;
	   }else{
	     flag++;	     
	   }
