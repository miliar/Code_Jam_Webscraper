...
int main(void){
	...
	long long mushrooms[1010];
	long long total=0,total2=0,maxEaten=0;
	long long L,X;
		for(long long i=0;i<N;i++){
            cin>>mushrooms[i];
            if(i>0&&mushrooms[i]<mushrooms[i-1]){
                total+=(mushrooms[i-1]-mushrooms[i]);
                maxEaten=max(maxEaten,mushrooms[i-1]-mushrooms[i]);
            }
		}
		for(long long i=0;i<N-1;i++){
            total2+=min(mushrooms[i],maxEaten);
		}
