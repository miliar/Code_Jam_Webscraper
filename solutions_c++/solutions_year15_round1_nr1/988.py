...
int main()
{
	...
	FORI(caseNum, 1, T)
	{
		...
		FORI(i, 1, N-1)
		{
			...
			if (current < last)
			{
				int eaten = last - current;
				totalEaten += eaten;
				if (eaten > maxMushRoomsIn10Seconds)
				{
					maxMushRoomsIn10Seconds = eaten;
				}
			}
			last = current;
		}
		//cout << "MaxMushroomIn10seconds:"<<maxMushRoomsIn10Seconds<<endl;
		int totalEaten2 = 0;
		int left = M[0];
		FORI(i, 1, N-1)
		{
			int eaten = 0;
			if (M[i-1] > maxMushRoomsIn10Seconds)
			{
				eaten = maxMushRoomsIn10Seconds;
			}
			else
			{
				eaten = M[i-1];
		...
		cout << "Case #" << caseNum << ": " << totalEaten << " " << totalEaten2 << endl;
	}
    return 0;
}
