...
typedef pair<long long,long long>toy;
typedef pair<long long,long long>box;
typedef pair<sum,long long>updat;
typedef vector<long>adjList;
adjList graph[1002];
long visitCount[1002];
long long N;
long dp[1000005];
long long mostToy(vector<toy>toys,vector<box>boxes){
    if(toys.size()==0||boxes.size()==0)
        return 0;
    long long result=0;
    while(toys[0].second==boxes[0].second){
        long long cur=min(toys[0].first,boxes[0].first);
        toys[0].first-=cur;
        boxes[0].first-=cur;
        result+=cur;
        if(toys[0].first==0)
    ...
    vector<toy>newToys=toys;
    vector<box>newBoxes=boxes;
    long long pos=0,bLen;
    while(!newBoxes.empty()&&newBoxes[pos].second!=toys[pos].second){
        newBoxes.erase(newBoxes.begin());
        bLen=newBoxes.size();
    }
    newToys.erase(newToys.begin());
    long long res1=0,res2=0;
    if(!newBoxes.empty())
        res1=mostToy(toys,newBoxes);
    if(!newToys.empty())
        res2=mostToy(newToys,boxes);
    return result+max(res1,res2);
}
long long flipNum(long long num){
    long long fn=0;
    while(num>0){
        fn=fn*10+num%10;
        num/=10;
...
void buildDP(){
    for(long i=0;i<21;i++)
        dp[i]=i;
    for(long i=21;i<1000002;i++){
        long long fn=flipNum(i);
        if(fn<i&&i%10)
            dp[i]=1+min(dp[fn],dp[i-1]);
        else
            dp[i]=1+dp[i-1];
    }
}
...
long long getLastFlip(long long num){
    long len=getlen(num);
    long halflen=len/2,temp=num,ans=1;

    for(long i=halflen;i<len;i++)
        temp/=10;
    temp=flipNum(temp);
    return pow10[len-1]+temp;
}
bool bfs(long source){
    queue<long>waitingNodes;
    waitingNodes.push(source);
    bool inQueue[1002];
    for(long i=0;i<=N;i++)
        inQueue[i]=false;
    while(!waitingNodes.empty()){
        long node=waitingNodes.front();
        waitingNodes.pop();
        long len=graph[node].size();
        for(long i=0;i<len;i++){
            if(inQueue[graph[node][i]])
                return true;
            inQueue[graph[node][i]]=true;
            waitingNodes.push(graph[node][i]);
            visitCount[graph[node][i]]=1;
        }
    }
    return false;
}
long long occCount(string cur,string target){
    long len1=cur.size();
    long len2=target.size();
    long long result=0;
    for(long i=0;i+len2<=len1;i++){
        long match=0;
        for(long j=i;j<i+len2;j++){
            if(cur[j]==target[j-i])
                match++;
            else
                break;
...
double getProb(string kBoard,string Target){
    double prob=1.0;
    long len1=kBoard.size();
    long len2=Target.size();
    for(long i=0;i<len2;i++){
        long occ=0;
        for(long j=0;j<len1;j++){
            if(kBoard[j]==Target[i])
                occ++;
        }
        if(occ==0)
            return -1.0;
        prob*=((double)occ)/((double)len1);
    }
    return prob;
}
long recurrencePosition(string s){
    long len=s.size();
    for(long i=1;i<len;i++){
        long match=0;
        for(long j=0;j+i<len;j++){
            if(s[j]==s[j+i])
                match++;
        }
        if(match+i==len)
...
int main(void){
    ...
    freopen ("E:/outputSmallnew.txt","w",stdout);
	long test_cases;
	cin>>test_cases;
	string s;
//	buildDP();
...
//	long long X,Y;
    ...
    toy t;
    box b;
    string curword,targetWord,kBoard;
    long long K,L,S,totalCombination=1,maxCount=0,totalCount=0;
    long long values[110];
	cin>>K>>L>>S;
	cin>>kBoard;
	cin>>targetWord;
	if(S<L){
    ...
    double matchProb=getProb(kBoard,targetWord);
    if(matchProb<-0.1){
        cout<<"0.0000000000"<<endl;
        continue;
    }
	long maxC=1+(S-L)/recurrencePosition(targetWord);
	double prob=((double)maxC)-(matchProb*((double)(1+S-L)));
    cout<<prob<<endl;
	/*
	if(N<21){
        cout<<N<<endl;
