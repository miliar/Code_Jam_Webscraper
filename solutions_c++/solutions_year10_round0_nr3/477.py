...
llint get_ans(int n,int r)
{
	...
	debug(pos);
	debug(visited[pos]);
	llint cycle_sum=earn[pos];
	int cycle_len=1;
	for(int p=go[pos];p!=pos;p=go[p],cycle_len++)
		cycle_sum+=earn[p];
	debug(cycle_len);
	assert(cycle_len==tm-visited[pos]);
	res+=cycle_sum*(llint)(r/cycle_len);
	r%=cycle_len;
	while(r)
	{
		r--;
...
int main()
{
	...
	for(cin>>tt;tt--;ca++)
	{
		...
		llint up=min(k,accumulate(group,group+n,llint(0)));
		for(int i=0;i<n;i++)
		{
			int len=0;
			llint sum=0;
			while(sum+group[(len+i)%n]<=up)
				sum+=group[(len+i)%n],len++;
			go[i]=(i+len)%n;
			earn[i]=sum;
			//cerr<<i<<"="<<go[i]<<" "<<earn[i]<<endl;
		}
		llint ans=get_ans(n,r);
