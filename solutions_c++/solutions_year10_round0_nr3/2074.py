...
unsigned long long solve() {
    ...
    while(current < r) {
        ...
        if (node.count == 0) {
            node.count++;
            node.lastTime = current;
            node.lastCost = cost;
        } else  {
            if (node.loopLength == 0) {
                node.loopLength = current - node.lastTime;
                node.loopCost = cost - node.lastCost;
            }

            int loops = (r - current) / node.loopLength;
            if (loops) {
                current += node.loopLength;
                cost += node.loopCost;
            }
        }
