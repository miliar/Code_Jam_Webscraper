...
void ProcessFile(FILE* fin)
{
	...
	for (int i=0; i<T; ++i)
	{
		...
		INTPAIR *h = new INTPAIR[N];
		memset(h, 0xFF, sizeof(INTPAIR)*N);
		INTPAIR *c = new INTPAIR[N];
		memset(c, 0, sizeof(INTPAIR)*N);
		for (int i=0;i<N;++i)
			fscanf(fin, "%d", &g[i]);
		for (int i=0; i<N;++i)
		{
			for (int j=0; j<N && c[i].first+g[(i+j)%N]<=k; ++j, ++c[i].second )
				c[i].first+=g[(i+j)%N];
		}
		__int64 s=0;
