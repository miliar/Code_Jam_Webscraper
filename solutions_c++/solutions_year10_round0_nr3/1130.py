...
int main() {
  ...
  FORU(testcase,1,T) {      
    ...
    int cost = 0, seats = k;
    FORU(i,0,R-1) {
      cost = 0; seats = k; ride.clear();
      while (!Q.empty() && (seats - Q.front() >= 0)) {
        int g = Q.front(); Q.pop(); seats -= g; 
        ride.PB(g); cost += g;
      } res += cost;         
      FORU(i,0,ride.size()-1) Q.push(ride[i]);         
    } 
    
    printf("Case #%d: ",testcase); cout << res << endl;
  }
