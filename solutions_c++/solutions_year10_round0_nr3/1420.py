...
lint cnt(size_t r, size_t k, vecsz& g)
{
	vecsz first;
	veclint len;
	...
	for (; i < r; ++i)
	{
		if (find(first.begin(), first.end(), cur) != first.end())
			break;
	...
	if (i == r)
		return accumulate(len.begin(), len.end(), lint(0));

	sz cyc_begin = find(first.begin(), first.end(), cur) - first.begin();

	lint result = accumulate(len.begin(), len.begin() + cyc_begin, lint(0));
	lint cyc_len = accumulate(len.begin() + cyc_begin, len.end(), lint(0));

	result += ((r - cyc_begin) / (first.size() - cyc_begin)) * cyc_len;

	sz tail = (r - cyc_begin) % (first.size() - cyc_begin);
	result += accumulate(len.begin() + cyc_begin, len.begin() + cyc_begin + tail, lint(0));
...
int main(int argc, char* argv[])
{
	...
	for (size_t current_test = 1; current_test <= count_tests; ++current_test)
	{
		...
		PRINT("Case #" << current_test <<": " << cnt(R, k, g) << endl);
	}
