...
bool ProcessFile(char *pszInputFile, char * pszOutputFile)
{
	...
	for (int iEntry = 1; iEntry <= iT; iEntry ++)
	{
		...
		// retrieve input - first line
		getline(f_in, strQuery);
		sscanf(strQuery.c_str(), "%d %d %d", &R, &k, &N);
		
		// retrieve intput - second line
		getline(f_in, strQuery);
		string strTemp;
		for (std::string::const_iterator cit = strQuery.begin(); true; ++cit)
		{
			if (cit == strQuery.end() || *cit == ' ')
			{
				int iDigit = atoi(strTemp.c_str());
				vqueue.push_back(iDigit);
				strTemp.clear();
			}
			else
			{
