...
readIn( ifstream& ifs ){
   ...
   for ( unsigned i = 0 ; i < nCases ; ++i ){
      char c1[1048576];
      ifs.getline(c1,1048576);
      string s1 = c1;
      vector<string> firstline;
      getTokens(s1,' ',firstline);
      R.push_back(str2Int(firstline[0]));
      k.push_back(str2Int(firstline[1]));
      N.push_back(str2Int(firstline[2]));

      char c2[1048576];
      ifs.getline(c2,1048576);
      string s2 = c2;
      vector<string> secondline;
      getTokens(s2,' ',secondline);
      for ( unsigned j = 0 ; j < secondline.size() ; ++j )
         qs[i].push_back( str2Int(secondline[j]));
   }
}
...
solve( ofstream& ofs ){
   ...
   for ( int i = 0 ; i < nCases; ++i ){
      unsigned long long ith = 0;
      ul cost = 0;
      map<unsigned long long, Par>  mapping;
      for ( ul r = 0 ; r < R[i] ; ++r ){
         map<ul,Par>::iterator it = mapping.find(ith);
         if ( it != mapping.end()){
            ith = it->second.pos;
            cost += it->second.cost;
            continue;
         } 
         ul taking_tmp = 0, taking=0;
         ul preith = ith;
         while(1){
            taking = taking_tmp;
            taking_tmp += qs[i][ith];
