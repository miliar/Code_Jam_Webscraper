...
template<class T>
bool bellmanFord(const vector<vector<pair<int,T> > > &adj, vector<T> &dist, vector<int > &prev, int start){ int N = adj.size(); dist = 
vector<T>(N,INF), prev = vector<int>(N,-1); dist[start] = 0; FOR(k,0,N-1) { bool any =false; FOR(i,0,N) FOR(j,0,adj[i].size()) if(dist[i] + adj[i][j].second < dist[adj[i][j].first]){ any=true; dist[adj[i][j].first] = dist[i] + adj[i][j].second; prev[adj[i][j].first] = i;} if(!any) break;} FOR(i,0,N) FOR(j,0,adj[i].size()) if(dist[adj[i][j].first] > dist[i] + adj[i][j].second) return false; return true;}
template<class T>
...
int main(){
  ...
  FOR(i,0,_T){
    cout << "Case #" << (i+1) << ": ";
    string str; cin >> str;
    vector<int> v(1,0); FOR(i,0,str.length()) v.push_back(str[i]-'0');
    next_permutation(BEGEND(v)); while(v[0]==0) v.erase(v.begin());
