...
//	freopen("input.txt","r",stdin);freopen("output.txt","w",stdout);
    ...
    for(int Cas=1;Cas<=CAS;++Cas){
        ...
        if(i==len){
            ...
            for(i=1;i<len;++i){
                if(a[i]!=0){
                    if(min_i<0||a[i]<a[min_i])min_i=i;
                }
            }
            if(min_i<0){
                //printf("len:%d\n",len);
            ...
            if(i==len-1){
                ...
                for(j=0;j<i;++j){
                    if(a[j]!=0){
                        if(a[j]>a[i]){
                            if(min_i<0||a[j]<a[min_i])min_i=j;
                        }
                    }
                }
            }else{
            ...
            int s=a[min_i];a[min_i]=a[i];a[i]=s;
