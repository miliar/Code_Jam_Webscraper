...
int n,m,a[MAXV][MAXV],b[MAXV][MAXV],cnt[MAXV*MAXV];
...
//    #endif
    ...
    {
    	IN(n);
    	FOR(i,0,3) FOR(j,0,3) IN(a[i][j]);
    	IN(m);
    	FOR(i,0,3) FOR(j,0,3) IN(b[i][j]);
    	MEM(cnt,0);
    	FOR(i,0,3) cnt[a[n-1][i]-1]++;
    	FOR(i,0,3) cnt[b[m-1][i]-1]++;
    	int two=0,ans;
    	FOR(i,0,15) if(cnt[i]==2) two++,ans=i+1;
    	if(two==1) printf("Case #%d: %d\n",times,ans);
    	else if(two>1) printf("Case #%d: Bad magician!\n",times);
