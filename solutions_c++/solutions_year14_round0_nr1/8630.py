...
string solve(int r1, VVI m1, int r2, VVI m2){
	VI res(100);
	tr(m1, it) sort(all(*it));
	tr(m2, it) sort(all(*it));
	VI::iterator it = set_intersection(all(m1[r1-1]), all(m2[r2-1]), res.begin());
...
int main(){
	...
	sfor(int, testCase, 1, numTestCases+1) {
		...
		VVI m2(4, VI(4));
		cin >> m2;
		cout << "Case #" << testCase << ": " << solve(r1, m1, r2, m2) << endl;
	}
}
