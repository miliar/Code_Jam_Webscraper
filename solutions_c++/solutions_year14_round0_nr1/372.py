...
void sfileIO(){freopen("A-small-attempt0.in","r",stdin); freopen("A-small.out","w",stdout);}
...
int main(int argc, char** argv)
{
    ...
    while(totalCases--){
        ...
        if(si.size()>7){
            cout<<"Case #"<<zi++<<": "<<"Volunteer cheated!"<<endl;
            continue;
        }
        
        if(si.size()<7){
            cout<<"Case #"<<zi++<<": "<<"Bad magician!"<<endl;
            continue;
        }
        bool done=false;
        FOR(i,0,4) 
            FOR(j,0,4)
                if(arr1[r1][i]==arr2[r2][j] && !done)
                {
                    cout<<"Case #"<<zi++<<": "<<arr1[r1][i]<<endl;
                    done=true;
                }
