...
int main()
{
    ...
    int i,j,test,Case=1;
    int matA[5][5],matB[5][5],A,B,cnt,ans;
    ...
    while(test--) {
        ...
        scanf("%d",&A);
        REP(i,4) REP(j,4) scanf("%d",&matA[i][j]);
        scanf("%d",&B);
        REP(i,4) REP(j,4) scanf("%d",&matB[i][j]);
        cnt = 0;
        REP(i,4) REP(j,4) if(matA[A-1][i]==matB[B-1][j]) cnt++;
        REP(i,4) REP(j,4) if(matA[A-1][i]==matB[B-1][j]) ans = matA[A-1][i];
