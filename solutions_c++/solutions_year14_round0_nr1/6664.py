...
string solve() {
  VI ret(4);
  set <int> A = get(), B = get();
  ret.resize(set_intersection(ALL(A), ALL(B), ret.begin()) - ret.begin());
  if(ret.size() > 1) return "Bad magician!";
  else if(ret.size() == 1) return to_string(ret[0]);
