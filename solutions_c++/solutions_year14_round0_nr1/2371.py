...
int main() {
    ...
    while (t--) {
        ...
        printf("Case #%d: ", kase);
        int r1 = GI-1;
        REP(i, 4)   REP(j, 4)   a[i][j] = GI;
        int r2 = GI-1;
        REP(i, 4)   REP(j, 4)   b[i][j] = GI;
        si s;
        REP(j, 4)   s.insert(a[r1][j]), s.insert(b[r2][j]);
        if (sz(s) == 8) printf("Volunteer cheated!\n");
        else if (sz(s) < 7) printf("Bad magician!\n");
        else {
            int ans = -1;
            REP(j, 4)   REP(k, 4) {
                if (a[r1][j] == b[r2][k]) {
