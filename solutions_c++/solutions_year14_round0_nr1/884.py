...
void readmat(vector< vector<int> >& mat, istream& in)
{
	mat.resize(4);
...
bool existin(const vector<int>& v, int val)
{
	for (int i : v)
	{
		if (i == val)
...
vector<int> getunion(const vector<int>& l0, const vector<int>& l1)
{
	vector<int> ret;
	for (int i : l0)
	{
...
void solve(istream& in, ostream& fout, int caseIdx)
{
	...
	vector<int> l0 = mat0[line0 - 1];
	vector<int> l1 = mat1[line1 - 1];

	vector<int> uni = getunion(l0, l1);

	if (uni.size() == 1)
	{
		OUTANS( << CaseHead(caseIdx+1) << uni.front() << endl );
	}
	else if (uni.size() == 0)
	{
		OUTANS( << CaseHead(caseIdx+1) << "Volunteer cheated!" << endl );
	}
	else
	{
		OUTANS( << CaseHead(caseIdx+1) << "Bad magician!" << endl );
	}
}
...
int main(int argc, char* argv[])
{
	...
	proc_file( "../CJ/A-small-attempt0.in" , "../CJ/out.txt" );
