#pragma comment(linker, "/STACK:512000000")
#define _CRT_SECURE_NO_DEPRECATE
#define _USE_MATH_DEFINES
#include <iostream>
#include <fstream>
#include <cstdio>
#include <cstdlib>
#include <cassert>
#include <climits>
#include <ctime>
#include <numeric>
#include <vector>
#include <algorithm>
#include <bitset>
#include <cmath>
#include <cstring>
#include <iomanip>
#include <complex>
#include <deque>
#include <functional>
#include <list>
#include <map>
#include <string>
#include <sstream>
#include <set>
#include <stack>
#include <queue>
#include <unistd.h>
using namespace std;
typedef long long lng;
typedef unsigned long long ulng;
typedef unsigned int uint;
typedef unsigned char uchar;
typedef long double ld;
typedef pair<int, int> PII;
typedef pair<short, short> PSS;
typedef pair<PII, int> PIII;
typedef pair<lng, lng> PLL;
typedef pair<ulng, ulng> PUU;
typedef pair<lng, int> PLI;
typedef pair<int, lng> PIL;
typedef pair<ld, ld> PDD;
template<class T> inline T sqr(T x) { return x * x; }
template<class T> inline string tostr(const T & x) { stringstream ss; ss << x; return ss.str(); }
inline lng parse(const string & s) { stringstream ss(s); lng x; ss >> x; return x; }
#define left asdleft
#define right asdright
#define link asdlink
//#define unlink asdunlink
#define next asdnext
#define prev asdprev
#define y0 asdy0
#define y1 asdy1
#define mp make_pair
#define pb push_back
#define sz(x) ((int)(x).size())
#define all(x) (x).begin(), (x).end()
#define clr(ar,val) memset(ar, val, sizeof(ar))
#define istr stringstream
#define forn(i,n) for(int i=0;i<(n);++i)
#define forv(i,v) forn(i,sz(v))
#define X first
#define Y second
#define hash asdhash
#define move asdmove
const ld EPS = 1e-12;
const int INF = 1000*1000*1000;
const char CINF = 102;
const lng LINF = INF * 1ll * INF;
const ld DINF = 1e200;
const ld PI = 3.1415926535897932384626433832795l;
int gcd(int a,int b){return a?gcd(b%a,a):b;}
lng gcd(lng a,lng b){return a?gcd(b%a,a):b;}
lng powmod(lng a,lng p,lng m){lng r=1;while(p){if(p&1)r=r*a%m;p>>=1;a=a*a%m;}return r;}
#define TASKA "iota"

int N,M,K;
int ans;

void print(vector<PII> P){
	vector<vector<int> > A(N,vector<int>(M,-1));
	forv(i,P){
		A[P[i].X][P[i].Y]=i;
	}
	cout<<endl;
	forn(i,N){
		forn(j,M){
			cout.width(2);
			cout<<A[i][j]<<' ';
		}
		cout<<endl;
	}
	cout<<endl;
}

void solve(){
	if(M<N)
		swap(M,N);
	vector<PII> P;
	int x0=M/2;
	int y0=N/2;
	int ind=0;
	for(int k=0;;++k){
		int s0=sz(P);
		for(int y=max(0,y0-k+1);y<min(N,y0+k);++y){
			int x=max(x0+y0-k-y,-(y0-x0+k-y));
			if(x>=0)
				P.pb(mp(y,x));
		}
		for(int y=max(0,y0-k);y<min(N,y0+k+1);++y){
			int x=min(-(y0-x0-k-y),x0+y0+k-y);
			if(x<M)
				P.pb(mp(y,x));
		}
		if(sz(P)==s0)
			break;
	}
	
	if(sz(P)!=N*M)
		exit(123);
	
	//print(P);
	
	ans=INF;
	vector<vector<char> > A(N,vector<char>(M,0));
	forv(q,P){
		A[P[q].X][P[q].Y]=1;
		if(q+1<K)
			continue;
		int c=0;
		forn(i,N){
			forn(j,M){
				if(!A[i][j])
					continue;
				const int di[]={0,0,-1,1};
				const int dj[]={-1,1,0,0};
				forn(d,4){
					int ii=i+di[d];
					int jj=j+dj[d];
					if(ii<0||ii>=N||jj<0||jj>=M||!A[ii][jj]){
						++c;
						break;
					}
				}
			}
		}
		ans=min(ans,c);
	}
	
	if(ans==INF)
		exit(122);
}

void input(){
	cin>>N>>M>>K;
}

void output(){
	cout<<ans<<endl;
}

vector<string> W;
bool was[25][25];

int dfs(int a,int b){
	if(a<0||a>=N||b<0||b>=M||was[a][b]||W[a][b]!='.')
		return 0;
	was[a][b]=true;
	int r=1;
	const int da[]={0,0,-1,1};
	const int db[]={-1,1,0,0};
	forn(d,4){
		r+=dfs(a+da[d],b+db[d]);
	}
	return r;
}

int bitcnt(int a){
	int r=0;
	while(a){
		r+=a&1;
		a>>=1;
	}
	return r;
}

void brute(){
	W=vector<string>(N,string(M,'.'));
	PII res(INF,0);
	forn(m,1<<(N*M)){
		clr(was,0);
		forn(i,N){
			forn(j,M){
				W[i][j]=(m&(1<<(i*M+j)))?'x':'.';
			}
		}
		int c=N*M;
		forn(i,N){
			c-=dfs(i,0);
			c-=dfs(i,M-1);
		}
		for(int j=1;j+1<M;++j){
			c-=dfs(0,j);
			c-=dfs(N-1,j);
		}
		if(c>=K)
			res=min(res,mp(bitcnt(m),m));
	}
	ans=res.X;
	forn(i,N){
		forn(j,M){
			W[i][j]=(res.Y&(1<<(i*M+j)))?'x':'.';
		}
	}
}

void test(){
	int tc;
	cin>>tc;
	forn(qqq,tc){
		cerr<<qqq<<endl;
		input();
		solve();
		int r=ans;
		brute();
		int b=ans;
		if(b!=r){
			cout<<"failed "<<qqq<<": "<<b<<' '<<r<<endl;
			forv(i,W){
				cout<<W[i]<<endl;
			}
			cout<<endl;
			return;
		}
	}
	cout<<"passed"<<endl;
}

int main(){
#ifdef __ASD__
	freopen("input.txt", "r", stdin); freopen("output.txt", "w", stdout);
#else
	//freopen(TASKA".in", "r", stdin); freopen(TASKA".out", "w", stdout);
#endif
	
	//test();return 0;
	
	int tc;
	cin>>tc;
	forn(qqq,tc){
		cerr<<qqq<<"/"<<tc<<endl;
		printf("Case #%d: ", qqq+1);
		input();
		//solve();
		brute();
		output();
	}
	
	return 0;
}
