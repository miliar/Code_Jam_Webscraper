...
int get(int aa, int bb, int cc)
{
	int res=0;
	REP(i,n)
		if (a[i].X 	<= cc && a[i].Y.X <= aa && a[i].Y.Y <= bb)
			res++;
	return res;
}
...
int stupid()
{
	...
	UNIQUE(cc);
	int res=0;
	REP(ia,SZ(aa)) REP(ib,SZ(bb))
	{
		res=max(res,get(aa[ia],bb[ib],10000-aa[ia]-bb[ib]));
	}
	return res;
}
...
REP(i,n)
{
	...
	a[i]=MP(10000-q-w,MP(q,w));
		...
		{ //fprintf(stderr,"%d\n",i);
			int curc = cc[i];
			int start=0;
			while (start<n&&a[start].X<=curc) start++;
			VPII q;
			q.clear();
			REP(i,start)
				q.PB(a[i].Y);
			res=max(res,solve(q,10000-curc));
		}
