...
public:
    ...
    inline int readString(char* arg, int limit, UnaryPredicate isTerminator, bool skip) {
        ...
        for (--limit; head < tail; fillInput()) {
            ptrdiff_t chunkSize =
                find_if(head, min(tail, head + limit - charsRead), isTerminator) - head;
    ...
    inline bool read(string& arg, Terminator terminator, Ts&&... args) {
        int length = max(16, static_cast<int>(arg.capacity())), last = 0;
        for (;; length = last += length) {
...
class InputFile final : public InputDevice {
    FILE* file;
    bool lineBuffered;
    bool owner;
...
class InputString final : public InputDevice {
...
protected:
    ...
    OutputDevice() : output(buffer), end(buffer + BUFFER_SIZE + MAX_LENGTH), separate(false)
        ...
        , precision(setPrecision().value), delimiter(setDelimiter().value) { computeBasePower(); }
    ...
    template <class F> char* writeFloatingPoint(F arg, char* last) {
        ...
        if (negative) arg = -arg;
        if (std::isnan(arg)) for (int i = 0; i < 3; ++i) *--last = i["NaN"];
        else if (std::isinf(arg)) for (int i = 0; i < 3; ++i) *--last = i["fnI"];
        else {
            auto integerPart = static_cast<unsigned long long>(arg);
            auto fractionalPart =
                static_cast<unsigned long long>((arg - integerPart) * basePower + F(0.5));
    ...
    inline int writeFill(uint count) {
        ...
        if (__builtin_expect(output + count + MAX_LENGTH < end, true)) {
            ...
            uint chunkSize = static_cast<uint>(buffer + BUFFER_SIZE - output);
            for (;; chunkSize = BUFFER_SIZE) {
...
public:
    ...
    inline int writeString(const char* arg, size_t length, bool checkWidth = true) {
        ...
        int charsWritten = static_cast<int>(count);
        if (checkWidth && count < width) charsWritten += writeFill(width - count);
        if (__builtin_expect(output + count + MAX_LENGTH < end, true)) {
...
class OutputFile final : public OutputDevice {
    FILE* file;
    bool owner;
...
class OutputString final : public OutputDevice {
    string& str;
...
unique_ptr<OutputFile> logs;

template <class... Ts> inline void read(Ts&&... args) { assert(input->read(forward<Ts>(args)...)); }
template <class... Ts> inline void writeln(Ts&&... args) {
    logs->write(forward<Ts>(args)..., '\n');
    logs->flush();
}
...
class StrongComponents {
    int tGlobal;
    vector<int> tLow, stack;

    template <class GraphU>
    void dfs(const GraphU& graph, int v, int parent) {
        bool root = true;
        tLow[v] = ++tGlobal;
        stack.push_back(v);
        bool sawParent = graph.oriented();
        for (int u : graph[v]) {
            if (sawParent || u != parent) {
                if (tLow[u] == 0) dfs(graph, u, v);
                if (tLow[v] > tLow[u]) {
                    tLow[v] = tLow[u];
                    root = false;
                }
...
public:
    ...
    template <class GraphU>
    void process(const GraphU& graph) {
        int n = graph.size();
        tGlobal = componentCount = 0;
        component.resize(n);
        tLow.assign(n, 0);
        stack.reserve(n);
        for (int i = 0; i < n; ++i)
            if (tLow[i] == 0) dfs(graph, i, -1);
    }
};
...
//        write("\n", setDelimiter("\n"), s, r);
        ...
        REP(i, sz(beams)) {
            ...
            REP(dir0, 4) {
                ...
                while (true) {
                    ...
                    if (0 > x || x >= r || 0 > y || y >= c || s[x][y] == '#') break;
                    if (s[x][y] == '-' || s[x][y] == '|') {
                        ok[dir0] = false;
                        break;
                    }
                    if (s[x][y] == '.') e[dir0].eb(x, y);
                    if (s[x][y] == '/') dir ^= 3;
                    if (s[x][y] == '\\') dir ^= 1;
                }
            }
            ...
            REP(dir, 4) {
                int var = 2 * i;
                if ((dir & 1) == (s[beams[i].X][beams[i].Y] == '|')) ++var;
                if (ok[dir] && ok[dir ^ 2]) {
                    for (pii p : e[dir]) a[p.X][p.Y].pb(var);
                } else if (dir < 2) {
                    sat2.addOr(var ^ 1, var ^ 1);
                }
            }
        }
        ...
        REP(i, r) REP(j, c) if (s[i][j] == '.') {
                    ...
                    if (sz(a[i][j]) == 1) a[i][j].pb(a[i][j][0]);
                    if (sz(a[i][j]) > 2) while (true);
                    sat2.addOr(a[i][j][0], a[i][j][1]);
                }
        if (possible) {
            if (!sat2.solve()) possible = false;
        }
        if (possible) {
            writeln("POSSIBLE");
            REP(i, sz(beams)) if (sat2.answer[i]) {
                    s[beams[i].X][beams[i].Y] ^= ('-' ^ '|');
                }
            write(setDelimiter("\n"), s, r);
        } else {
            write("IMPOSSIBLE");
    ...
    string outputStr;
    TestCase() : output(new OutputString(outputStr)) {};
private:
    unique_ptr<OutputDevice> output;

    template <class... Ts> int write(Ts&&... args) { return output->write(forward<Ts>(args)...); }
    template <class... Ts> int writeln(Ts&&... args) { return write(forward<Ts>(args)..., '\n'); }
};
...
void work() {
    while (true) {
        ...
        writeln("Case ", current + 1, " started");
        unique_ptr<TestCase> test(new TestCase());
        test->input();
        inputMutex.unlock();
        test->solve();
        answer[current].swap(test->outputStr);
    }
}

void writeAnswer(int test, const string& answer) {
    output->write("Case #", test + 1, ": ", answer, "\n");
    output->flush();
}
...
void randomTest() {
    for (int i = 0; i < 100; ++i) {
        ...
        test->solve();
        string output;
        output.swap(test->outputStr);
        test->solveSlow();
        if (test->outputStr != output) {
            writeln("Error on test #", i + 1);
            writeln("Expected:\n", test->outputStr);
            writeln("Received:\n", output);
        }
        //writeln(output);
    }
    writeln("\n\nTest time: ", static_cast<double>(clock()) / CLOCKS_PER_SEC);
    exit(0);
}
...
int main() {
    //randomTest();
    //maxTest();
    input.reset(new InputFile("c-large.in"));  // -small-attempt0.in
    output.reset(new OutputFile("output.txt"));  // -large
    logs.reset(new OutputFile(stderr));
    logs->write(setPrecision(6));
    testIndex = 0;
    input->read(testCount);
    ...
    for (int i = 0; i < testCount; ++i) writeAnswer(i, answer[i]);
#else
    for (int i = 0; i < testCount; ++i) {
        writeln("Case ", i + 1, " started");
        unique_ptr<TestCase> test(new TestCase());
        test->input();
        test->solve();
        writeAnswer(i, test->outputStr);
...
#endif
    writeln("\n\nTotal time: ", static_cast<double>(clock()) / CLOCKS_PER_SEC);
    return 0;
}
