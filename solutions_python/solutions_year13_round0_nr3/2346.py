#Based on http://jamftw.blogspot.com/2012/05/python-code-jam-wrapper.html
    ...
    def __init__(self, reader, solver, directory, question):
        ...
        for o, a in opts:
            if o in ("-s", "--small"):
                ...
                mid = int(a)
        filepref = directory + "/" + "{problem}-{input}-{id}".format(problem=question,input=datatype,id=mid)
        self.infile = filepref + ".in"
        self.outfile = filepref + ".out"
        self.errfile = filepref + "-attempt:{0}".format(attempt) + ".err"
        if (self.job == "run"):
            print >> stderr, "Creating " + \
                self.outfile + " from " + self.infile
        else:
            print >> stderr, "Testing that " + \
                self.outfile + " would create " + self.infile
