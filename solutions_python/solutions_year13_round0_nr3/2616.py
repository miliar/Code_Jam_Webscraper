...
LOOKUP_MAX = 10200
FP_THRESHOLD = 0.0000000001

def check_fns(square):
    root = math.sqrt(square)
    if (root - int(root)) < FP_THRESHOLD:
        num2 = str(int(root))
        if num2 == num2[::-1]:
            num1 = str(square)
            if num1 == num1[::-1]:
...
def check_range(rng_str):
    acc = 0
    a,b = map(int, rng_str.split())
    if b < LOOKUP_MAX:
        for case in FNS_NUMS:
            if a <= case <= b:
                acc += 1
        return acc
    else:
        if a < LOOKUP_MAX:
            for case in FNS_NUMS:
                if a <= case <= b:
                    acc += 1
        nums = np.arange(max(LOOKUP_MAX, a),b+1)
        v = np.vectorize(check_fns)
        acc += len(np.where(v(nums))[0]+1)
    return acc

if __name__ == '__main__':
    num_cases = int(raw_input())
    for case_num in xrange(num_cases):
        print 'Case #%d: %s' % (case_num+1, check_range(raw_input()))
