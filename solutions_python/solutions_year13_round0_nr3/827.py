#PalindromeQ = ((# // IntegerDigits // Reverse // 
#      FromDigits) == #) &; Select[
# Table[n^2, {n, 0, 10000000}], PalindromeQ]

import sys
psquare=[
0, 1, 4, 9, 121, 484, 10201, 12321, 14641, 40804, 44944, 1002001, \
1234321, 4008004, 100020001, 102030201, 104060401, 121242121, \
123454321, 125686521, 400080004, 404090404, 10000200001, 10221412201, \
12102420121, 12345654321, 40000800004, 1000002000001, 1002003002001, \
1004006004001, 1020304030201, 1022325232201, 1024348434201, \
1210024200121, 1212225222121, 1214428244121, 1232346432321, \
1234567654321, 4000008000004, 4004009004004, 100000020000001, \
100220141022001, 102012040210201, 102234363432201, 121000242000121, \
121242363242121, 123212464212321, 123456787654321
];
n=int(raw_input())
for i in range(n):
    a=raw_input().split()
    cnt=0;
    b=[int(a[0]),int(a[1])];
    for t in psquare:
        #print ( t >=b[0] and t<=b[1] )
        if ( t >=b[0] and t<=b[1] ):
            cnt+=1;
    sys.stdout.write("Case #")
    sys.stdout.flush()
    print i+1,
    sys.stdout.write(": ")
    sys.stdout.flush()
    print cnt
