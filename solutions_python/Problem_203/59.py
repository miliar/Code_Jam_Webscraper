...
def solve(f, skip=False):
    r, c = f.read_int_list()
    a = [list(f.read_str()) for _ in xrange(r)]
    if skip: return 0
    ...
    for ri in xrange(r):
        for ci in xrange(c):
            ...
            for si in s.keys():
                ...
                cmi = min(s[si][1], ci)
                cma = max(s[si][3], ci)
                if all([a[rii][cii] == '?' or a[rii][cii] == si for rii in xrange(rmi, rma+1) for cii in xrange(cmi, cma+1)]):
                    for rii in xrange(rmi, rma+1):
                        for cii in xrange(cmi, cma+1):
                            a[rii][cii] = si
                    s[si] = (rmi, cmi, rma, cma)
                    break
