...
class Solver(ProblemSolver):
    def precalculate(self):
        ...
        self.groups = set()
        for i in range(4):
            self.groups.add(tuple((i, j) for j in range(4)))
            self.groups.add(tuple((j, i) for j in range(4)))
        self.groups.add(tuple((i, i) for i in range(4)))
        self.groups.add(tuple((i, 3 - i) for i in range(4)))

    def process(self, infile, ncase):
        ## User code here
        lines = [infile.readline().strip() for i in range(4)]
        infile.readline()
        for group in self.groups:
            cells = [lines[i][j] for (i, j) in group]
            if set(cells) <= set('XT'):
                return "X won"
            elif set(cells) <= set('OT'):
                return "O won"
