...
def calculate_input(inpt):
    add_people = 0
    accum_stand = 0;
    for s in range(0, len(inpt)):
        if inpt[s][1] > 0 and accum_stand < inpt[s][0]:
            add_people += (inpt[s][0]-accum_stand)
            accum_stand += (inpt[s][0]-accum_stand)
        accum_stand = accum_stand + inpt[s][1]
    return add_people
