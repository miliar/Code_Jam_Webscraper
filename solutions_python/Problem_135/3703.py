...
# The program will be given the two arrangements of the cards, and the volunteer's answers to the two questions: the row number of the selected card in the first arrangement, and the row number of the selected card in the second arrangement. The rows are numbered 1 to 4 from top to bottom.
...
for n in range(1, T+1):
	...
	rows_1 = []
	for i in range(4):
		rows_1.append(set(map(int, input().split())))

	# card arrangement 2
	ans_2 = int(input())
	rows_2 = []
	for i in range(4):
		rows_2.append(set(map(int, input().split())))

	# which one?
	# such indexing, many 0
	...
	cards_no = len(cards)
	if cards_no == 1:
		print_case(list(cards)[0])
	elif cards_no == 0:
		print_case('Volunteer cheated!')
	else:
		print_case('Bad magician!')
