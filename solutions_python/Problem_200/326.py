...
class ProblemB(CodeJamBase):
    ...
    def solve_case(self, last_counted: str):
        last_digit = 0
        for i, x in enumerate(last_counted):
            d = int(x)
            if d < last_digit:
                earlier = last_counted[:i][::-1]
                for j, y in enumerate(earlier):
                    e = int(y)
                    if j + 1 == len(earlier):
                        if e <= 1:
                            continue
                        ...
                        if e == f:
                            continue
                    return last_counted[:i - j - 1] + str(e - 1) + ('9' * (len(last_counted) - i + j))
                return '9' * (len(last_counted) - 1)
            last_digit = d
        return last_counted

    def self_test(self, n=10 ** 6):
        l = str(1)
        for i in range(1, n):
            s = str(i)
            if self.test_nr(s):
            ...
            a = self.solve_case(s)
            if a != l:
                raise Exception('Expected {} to be {} but got {}'.format(s, l, a))
        print('all ok up to {}'.format(n))

    def test_nr(self, last_counted: str):
        last_digit = 0
        for i, x in enumerate(last_counted):
            d = int(x)
            if d < last_digit:
