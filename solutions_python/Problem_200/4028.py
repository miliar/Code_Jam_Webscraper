...
'
' Use at own risk. Use the code for whatever you likeðﾟﾙﾂ
'''
...
def solve(n,s):
    ...
    if flag==1:
        ...
        result=result*10+int(s[index])-1
...
def main():
    ...
    for testCaseNumber in range(1, testCases+1):
        ...
        n = int(s)#(inputFile.readline().strip())
        ...
        # Read a string
        #s = inputFile.readline().strip()
        #n_list = map(np.uint64, inputFile.readline().strip().split())
        ...
        '''
        string = 'Case #%d: %d' % (testCaseNumber, solve(n,s))
