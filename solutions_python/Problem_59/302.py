...
class Solver(object):
    ...
    def read_case_input(self, input):
        num_existing, num_wanted = read_line_of_ints(input)
        existing = set()
        wanted = set()
        for i in range(num_existing):
            existing.add(input.readline().strip())
        for i in range(num_wanted):
            wanted.add(input.readline().strip())
        return existing, wanted

    def add_parent(self, path, wanted):
        parent, name = os.path.split(path)
        if parent != "/":
            wanted.add(parent)
            self.add_parent(parent, wanted)

    def solve_case(self, existing, wanted):
        new = set()
        for path in wanted:
            self.add_parent(path, new)
        wanted = wanted.union(new)
        needed = wanted - existing
        return len(needed)
