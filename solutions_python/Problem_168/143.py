...
def try_to_fix(R, C, board, i, j, dir):
	good = False
	for new_dir in ARROWS:
		if new_dir == dir:
			continue
		found = False
		if new_dir == '^':
			for k in xrange(i-1, -1, -1):
				if board[k][j] in ARROWS:
					found = True
					board[i][j] = new_dir
					break
			if found:
...
def solve(R, C, board):
	...
	for i in xrange(R):
		...
		if dir != '<':
			continue
		good = try_to_fix(R, C, board, i_change, j_change, dir)
		if not good:
			return 'IMPOSSIBLE'
		else:
			count += 1
