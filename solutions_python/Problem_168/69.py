...
""" functions """
def solve(R, C, grid):
    def check_arrow_validity(direct, pt):
        pt += direct
        r, c = list(pt)
    ...
    for r in range(R):
        for c in range(C):
            if grid[r][c] != None:
                if not check_arrow_validity(grid[r][c], np.array([r, c])):
                    valid = False
                    for dirstr in "^v<>":
                        direct = parse_arrow(dirstr)
                        if check_arrow_validity(direct, np.array([r, c])):
                            valid = True
                    if not valid:
                        return "IMPOSSIBLE"
                    counter += 1
...
def main():
    ...
    for questionindex in xrange(numquestions):
        ...
        grid = []
        for i in range(R):
            grid.append([parse_arrow(x) for x in read_line()])
