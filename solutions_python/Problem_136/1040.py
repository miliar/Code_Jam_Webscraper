...
def solver(case):
    ...
    while finish == 0:
        #k=k+1
        strat1 = Decimal(x)/Decimal(v)
        strat2 = Decimal(c)/Decimal(v) + Decimal(x)/Decimal(v+f)
        if strat1 == min(strat1, strat2):
            finish = 1
            t=t+strat1
        else:
            t=t+Decimal(c)/Decimal(v)
            v=v+f
...
if __name__ == "__main__":
    #from GCJ import GCJ
    GCJ(reader, solver, "C:/Users/Robin/PycharmProjects/GCJ2014/", "b").run()
