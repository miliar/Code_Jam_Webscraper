...
after the flip, the stack is i, i-1, ..., 2, 1, i+1, i+2, ..., N. Pancakes 1, 2,
..., i now have the opposite side up, whereas pancakes i+1, i+2, ..., N have the
same side up that they had up before.
...
case number (starting from 1) and y is the minimum number of times you will need
...
bottom pancake individually to get a one-move solution; every time you
execute the maneuver, you must select a stack starting from the top.

In Case #4, all of the pancakes are already happy side up, so there is no
need to do anything.
...
def solve_case(S):
  ...
  for c in reversed(S):
    if c == target:
      flips += 1
      target = ('-','+')[target == '-'] # where's ma x?y:x duh
  return str(flips)

T = int(raw_input())
for i in range(T):
  S = raw_input()
  print "Case #" + str(i + 1) + ": " + solve_case(S)
