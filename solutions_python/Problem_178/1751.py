...
def flip_stack(stack, index):
    return ''.join(reversed(string.translate(stack[:index], table))) + stack[index:]
...
def get_min_flips(stack):
    ...
    if '-' not in stack:
        return 0
    first_char = stack[0]
    first_diff = stack.find(flip[first_char])
    if first_diff == -1:
        return 1  # flip whole stack
    return 1 + get_min_flips(flip_stack(stack, first_diff))
...
for i, line in enumerate(fileinput.input()):
    if fileinput.isfirstline():
        ...
        print 'Case #{0}: {1}'.format(i, get_min_flips(line))