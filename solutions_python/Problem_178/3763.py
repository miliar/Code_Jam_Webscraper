...
def flip_top_n(stack, n):
    if n == 1:
        stack[0] = flip_pancake(stack[0])
        return
    for i in xrange(n / 2):
        stack[i] = flip_pancake(stack[i])
        stack[n-i-1] = flip_pancake(stack[n-i-1])
        stack[i], stack[n-i-1] = stack[n-i-1], stack[i]
    if n % 2 == 1:
        stack[n/2] = flip_pancake(stack[n/2])
    return
...
def counter(t, n):
    for i in xrange(t):
        print('Case #{}: {}'.format(i+1, all_happy_up(n[i])))
