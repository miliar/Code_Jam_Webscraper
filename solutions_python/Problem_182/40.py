...
#   python -c 'from setuptools import setup; from Cython.Build import cythonize; import Cython.Compiler.Options; from sage.env import sage_include_directories; Cython.Compiler.Options.embed="main"; setup(ext_modules=cythonize("R1A2016B.pyx"), include_dirs=sage_include_directories())' build_ext --inplace
#   ./R1A2016B.so input.in
# 
...
class Problem(CodejamProblem):
    ...
    def solvecase(self, case):
        ...
        for r in case:
            for v in r:
                count[v] = count[v] + 1
        ans = [k for k,v in count.items() if v % 2 == 1]
        return sorted(ans)
