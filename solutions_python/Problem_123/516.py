...
def get_min_op(sizes, A):
    LOG.debug('A: %d' % A)
    LOG.debug('sizes: %s' % sizes)
    A = absorb(A, sizes)
    LOG.debug('after absorb A: %d' % A)
    LOG.debug('after absorb sizes: %s' % sizes)
    if not sizes or len(sizes) <= 1:
        return len(sizes)
    sizes_no_max = sizes[1:]
    LOG.debug('>> sizes_no_max: %s' % sizes_no_max)
    op_no_max = 1 + get_min_op(sizes_no_max, A)
    LOG.debug('<< op_no_max: %d' % op_no_max)
    if A > 1:
        sizes_with_a = copy.copy(sizes)
        sizes_with_a.append(A - 1)
        LOG.debug('>> sizes_with_a: %s' % sizes_with_a)
        op_with_a = 1 + get_min_op(sizes_with_a, A)
        LOG.debug('<< op_with_a: %d' % op_with_a)
    else:
        op_with_a = len(sizes)
    LOG.debug('op_with_a: %d' % op_with_a)
    return min(op_no_max, op_with_a)
...
def do_job(in_file, out_file=sys.stdout):
    ...
    for testcase in xrange(T):
        ...
        assert len(sizes) == N
        result = get_min_op(sorted(sizes, reverse=True), A)
