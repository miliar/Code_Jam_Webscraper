...
class pancaces_stack:
    def __init__(self, caseN, pstack):
        self.pstack = []
        self.count = 0
        self.caseN = caseN
        for ch in pstack:
            ...
            else: raise Exception("error input "+pstack)
            self.pstack.append(k)

    def __repr__(self):
        return "Case #{0}: {1}".format(self.caseN, self.count)

    def flip(self, n, debug):
        pstack_new = self.pstack[:n]
        pstack_new.reverse()
        for i in range(len(pstack_new)):
            pstack_new[i] = 1 - pstack_new[i]
        pstack_new.extend(self.pstack[n:])
        if debug: print ("flip ", n, self.pstack, pstack_new)
        self.pstack = pstack_new

    def crop(self, debug):
        while len(self.pstack)>0 and self.pstack[-1]==1:
            self.pstack.pop()
        if debug: print ("crop ", self.pstack)

    def get_info(self, debug):
        first = self.pstack[0]
        ind = 1
        while ind <len(self.pstack) and self.pstack[ind]==first:
            ind += 1
        return [first, ind]
    ...
    def solve(self, debug = False):
        while True:
            self.crop(debug)
            if len(self.pstack) == 0: return
            self.count += 1
            if self.pstack[0] == 1:
                first, ind = self.get_info(debug)
                self.flip(ind, debug)
            else:
                self.flip(len(self.pstack), debug)
