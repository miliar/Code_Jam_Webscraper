...
2, ..., i now have the opposite side up, whereas pancakes i+1, i+2, ..., N have
the same side up that they had up before.
...
when read left to right, represents the stack when viewed from top to bottom.
Output
...
def flip(s, n):
    return s[:n].translate(flip_trans) + s[n:]

@lru_cache(maxsize=None)
def solve(stack):
    s = ''.join(k for k,g in groupby(stack))
    s = s.rstrip('+')
    if not s:
        return 0
    ...
    subs = [flip(s, len(s)).rstrip('+')]
    for i in range(len(s) - 1):
        if s[i] != s[i+1]:
            subs.append(flip(s, i+1).rstrip('+'))

    return(1 + min(solve(sub) for sub in subs))

# import random
# r = ''.join(random.choice(('+', '-')) for i in range(100))
# t = '----+-+-+---+---+-+-++-+----+-+++-++----'
# t1 = '+++-----++---+----+--+-+--++-+-+-+---+--'
