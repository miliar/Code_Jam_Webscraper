...
def code_here(_cc, _DEBUG, _data):
    ...
    a = sorted(dd.p, reverse=True)
    for i in range(dd.N-dd.K+1):
        cur = a[i][0]*a[i][0]

        cur += 2*a[i][0]*a[i][1]
        cur += sum(sorted([ 2*a[j][0]*a[j][1] for j in range(i+1,dd.N) ], reverse=True)[:dd.K-1])

        res = max(res, cur)

    return "{0:.8f}".format(math.pi * res)
