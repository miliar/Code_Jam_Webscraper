...
def gen(cachefile,ts):
	...
	if os.path.exists(cachefile):
		with open(cachefile,'rb') as f: param,cache = (pickle.load(f))
		if param == PARAM_:
			CACHE_ = cache
			ts('g')
			return
		ts('x')
	CACHE_ = sorted(answers(PARAM_))
	with open(cachefile,'wb') as f: pickle.dump((PARAM_,CACHE_),f,-1)
	ts('c')
...
def answers_plen(n,i=0,p=0):
	...
	while 1:
		...
		bottom = 10**i
		if top < bottom: break
		v = (top if top == bottom else top+bottom)
		for r in answers_plen(n,i,p+v):
			yield r
		i += 1

def answers(n):
	for plen in xrange(1,n+1):
		print "Generating %i"%plen
		for pp in answers_plen(plen):
			yield pp
...
def process(f_, out):
	...
	for X_ in range(1,T_+1):
		...
		output = len(list(n for n in CACHE_ if A <= n <= B))
