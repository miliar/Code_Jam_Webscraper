...
def split_string(S):
    return set([S[:i+1] for i in range(len(S))])

def solve_case(case):
    N, strings = case
    prefixes = [split_string(S) for S in strings]
    
    maxval = len(reduce(lambda x, y: x.union(y), prefixes))
    count = 1
    if N == 1: #Single server, we don't *need* to iterate
        '%i %i' % (maxval, count % 1000000007)
    
    for i in range(pow(N, len(prefixes))):
        #Initialize the servers
        servers = [set() for j in range(N)]
        #Assign each prefix to a server
        for j in range(len(prefixes)):
            idx = i % N
            i = (i - idx) // N
            servers[idx] = servers[idx].union(prefixes[j])
        #Servers must be non-empty
        minlen = min(map(lambda x: len(x), servers))
        if minlen == 0:
            continue
        #Make a count
        total = sum(map(lambda x: len(x) + 1, servers))
        #print servers, total
        if total > maxval:
            maxval = total
            count = 1
    ...
    return '%i %i' % (maxval, count % 1000000007)
