...
def readFile(path = 'd:\small-practice.in'):
		with open(path, 'rb+') as f:
			...
			for i in range(n):
				buf = f.readline()[:-1]
				buf = buf.split(" ")
				buf = [float(elem) for elem in buf]
				inputs.append(buf)
		return n, inputs
...
def process_aux(input):
	...
	goal = input[2]
	while True:
		currentGoal = goal / denominator
		nextFarm = farmCost / denominator
		nextGoal = goal / (denominator + increase)
		if currentGoal <= nextFarm + nextGoal:
			time += currentGoal
			break
		time += nextFarm
