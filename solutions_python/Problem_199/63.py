...
def flip(row, n, K):
    row[n:n+K]=[not c for c in row[n:n+K]]
def solve(case):
    row,K=case
    flips=0
    for n in xrange(len(row)-K+1):
        if not row[n]:
            flip(row,n,K)
            flips+=1
    if all(row[-K:]):
...
def outcome_string(outcome):
    return "{}".format(outcome) if outcome is not None else "IMPOSSIBLE"
