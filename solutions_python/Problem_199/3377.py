...
def no_flips(str_to_flip, k, count):
    # print(str_to_flip, count)
    f_pos = str_to_flip.find('-')
    if f_pos == -1:
        return count
    if f_pos + k <= len(str_to_flip):
        new_str_to_flip = filp(str_to_flip, k, f_pos)
        return no_flips(new_str_to_flip, k, count + 1)
    else:
        return("IMPOSSIBLE")


def filp(str_to_flip1, k, f_pos):
    str_to_flip = list(str_to_flip1)
    for i in range(k):
        if str_to_flip[i + f_pos] == "+":
            str_to_flip[i + f_pos] = "-"
        elif str_to_flip[i + f_pos] == "-":
            str_to_flip[i + f_pos] = "+"
    return "".join(str_to_flip)
...
with open('ans.txt', 'w') as f:
    for i, s in enumerate(q):
        f.write("Case #{}: {}\n".format((i+1), str(no_flips(s[0], int(s[1]), 0))))
