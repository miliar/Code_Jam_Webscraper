...
class ProblemA(CodeJamBase):
    ...
    def _read_input_for_case(self, i):
        layout, k_str = self._read_line(split_lines=True)
        return layout, int(k_str)

    def solve_case(self, layout: str, k: int):
        pancakes = [x == '+' for x in layout]
        turns = 0
        ...
        if not all(x for x in pancakes[-k:]):
