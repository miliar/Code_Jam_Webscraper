        ...
        for j in range(0, h):
            for k in range(0, w):
                if mapaSalida[j][k].capitalize() == mapaSalida[j][k]:
                    watersheds.arreglar(mapaSalida, j, k)
        if mapaSalida[0][0] != 'a':
            for j in range(0, h):
                for k in range(0, w):
                    if j != 0 or k != 0:
                        if mapaSalida[j][k] == 'a':
                            mapaSalida[j][k] = mapaSalida[0][0]
                        elif mapaSalida[j][k] == mapaSalida[0][0]:
                            mapaSalida[j][k] = 'a'
            mapaSalida[0][0] = 'a'      
...
def calcular(north, west, east, south, actual):
    ...
    if south < menor:
        ...
        menores = 'S'
    if actual <= menor:
        return letras[letraActual]
    else:
        return menores

def arreglar(mapaSalida, i, j):
    if mapaSalida[i][j].capitalize() != mapaSalida[i][j]:
        return mapaSalida[i][j]
    else:
        if mapaSalida[i][j] == 'N':
            mapaSalida[i][j] = arreglar(mapaSalida, i - 1, j)
        elif mapaSalida[i][j] == 'W':
            mapaSalida[i][j] = arreglar(mapaSalida, i, j - 1)
