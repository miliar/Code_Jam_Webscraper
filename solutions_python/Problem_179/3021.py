...
def gen_number(num_len, no_of_cases):
    numbers = set('1' + ''.join(x) + '1' for x in itertools.product(['0', '1'], repeat=num_len - 2))
    prime_nos = {}
    values = {}
    for num in numbers:
        prime_nos[num] = [int(num, i) for i in range(2, 11)]

    for string, val in prime_nos.items():
        if len(values) < no_of_cases and not any(is_prime(i) for i in val):
            values[string] = [smallestdivisor(i) for i in val]
    return values
#
# C:\Users\prakaran\Downloads\a_sample.txt
input = (open(r'C:\Users\prakaran\Downloads\C-small-attempt0.in', 'r').read().strip()).split('\n')
with open('c_small_output.out', 'w') as f:
    for i in xrange(1, int(input[0]) + 1):
        f.write("Case #%s:\n" % i)
        n, j = map(int, input[i].strip().split())
        for item, val in gen_number(num_len=n, no_of_cases=j).items():
            f.write(item + ' ' +  ' '.join(str(x) for x in val) + '\n')
