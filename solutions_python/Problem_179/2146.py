...
class Solver:
    ...
    def _mocking_io(self, lines=None, out=None):
        ...
        try:
            if lines is not None:
                input = lambda *a: iter(lines).__next__()
            if out is not None:
    ...
    def _select_input_file(self, keyword):
        ...
        for cont in content:
            ...
            if (
                os.path.isfile(cont_path) and
                keyword.lower() in cont.lower() and cont.endswith(".in")
            ):
                files.append(cont)
    ...
    def _divisor(self, n):
        for i in range(3, min(int(math.sqrt(n)) + 1, 100000), 2):
            if n % i == 0:
                return i
        return None
