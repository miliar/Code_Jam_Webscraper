File=open("A-small-attempt0.in","r")
data=File.read().splitlines()
data1=[]
result=""
res=""
ans=0
listOfRaws=[1, 6, 11, 16, 21, 26, 31, 36, 41, 46, 51, 56, 61, 66, 71, 76, 81, 86, 91, 96, 101, 106, 111, 116, 121, 126, 131, 136, 141, 146, 151, 156, 161, 166, 171, 176, 181, 186, 191, 196, 201, 206, 211, 216, 221, 226, 231, 236, 241, 246, 251, 256, 261, 266, 271, 276, 281, 286, 291, 296, 301, 306, 311, 316, 321, 326, 331, 336, 341, 346, 351, 356, 361, 366, 371, 376, 381, 386, 391, 396, 401, 406, 411, 416, 421, 426, 431, 436, 441, 446, 451, 456, 461, 466, 471, 476, 481, 486, 491, 496, 501, 506, 511, 516, 521, 526, 531, 536, 541, 546, 551, 556, 561, 566, 571, 576, 581, 586, 591, 596, 601, 606, 611, 616, 621, 626, 631, 636, 641, 646, 651, 656, 661, 666, 671, 676, 681, 686, 691, 696, 701, 706, 711, 716, 721, 726, 731, 736, 741, 746, 751, 756, 761, 766, 771, 776, 781, 786, 791, 796, 801, 806, 811, 816, 821, 826, 831, 836, 841, 846, 851, 856, 861, 866, 871, 876, 881, 886, 891, 896, 901, 906, 911, 916, 921, 926, 931, 936, 941, 946, 951, 956, 961, 966, 971, 976, 981, 986, 991, 996]
#listOfRaws=[1, 6, 11, 16, 21, 26]
for i in listOfRaws:
    row=int(data[i])
    data1.append(data[i+row])
num=len(data1)
ite=1
while ite <= num:
    temp1=data1[ite-1].split(" ")
    temp2=data1[ite].split(" ")
    #print temp1, temp2
    for y in temp1:
        if y in temp2:
            ans +=1
            res=y
    if ans==0 :
        result=result+"Case #"+str((ite+1)/2)+": Volunteer cheated!"+" \n"
        #print ans
    elif ans==1 :
        result=result+"Case #"+str((ite+1)/2)+": "+str(res)+" \n"
        #print ans
    else :
        result=result+"Case #"+str((ite+1)/2)+": Bad magician!"+" \n"
        #print ans 
    ite=ite+2
    ans=0
print result
