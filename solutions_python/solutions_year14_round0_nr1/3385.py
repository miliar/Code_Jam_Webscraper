...
1 ≤ T ≤ 100. 1 ≤ both answers ≤ 4. Each number from 1 to 16 will appear exactly
...
with open('A-small-attempt0.in') as f:
    lines = [{int(element) for element in line.split()} for line in f]

outcomes = []
numinputs ,= lines[0]
for i, doublechunk in enumerate(chunks(list(chunks(lines[1:], 5)), 2)):
    possibilities = set(range(1, 17))
    for chunk in doublechunk:
        choice, *rows = chunk
        choice ,= choice
        print(choice, rows)
        possibilities &= rows[choice-1]
        print(' ', possibilities)
    ...
    message = 'Case #{}: {}'.format(i+1, outcome)
    outcomes.append(message)
    print(message)

with open('output_sm.txt', 'w') as f:
    f.write('\n'.join(outcomes))
