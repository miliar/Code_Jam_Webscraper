...
def solver(case):
    ...
    # Evaluate the cost for an increasing number of cuts
    # With zero cuts, cost is the largest pile
    costs = [case[0]]
    for cuts in range(1,max(case)):
        # At most the 'cuts' largest piles are involved
        # Copy them with some metadata - (present,orig,pieces)
        logging.debug("Cuts: {}".format(cuts))
        newpiles = [(p,p,1) for p in case[:cuts]]
        for _ in range(cuts):
            # spend each cut on the current largest pile
            p,o,c = newpiles[0]
            newpiles[0] = (math.ceil(o/(c+1)), o, c+1)
            newpiles.sort(reverse=True, key=operator.itemgetter(0)) # resort on the present size
            logging.debug("Largest piles: {}".format(newpiles))

        # The new largest pile is either the largest of the original largest set, or the largest of the remainder
        largest = newpiles[0][0]
        if cuts < len(case):
            largest = max(largest, case[cuts])

        costs.append(largest + cuts) # largest remaining pile + number of minutes spent cutting

    logging.debug("costs: {}".format(costs))
    return min(costs)
