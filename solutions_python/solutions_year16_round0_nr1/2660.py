...
def check_input(data, m=1):
    ndata = str(int(data) * m)
    for val in ndata:
        cset.add(val)
    if len(cset) == 10:
        return int(ndata)
    else:
        return check_input(data, m + 1)


input = (open(r'C:\Users\prakaran\Downloads\A-large.in', 'r').read().strip()).split('\n')
cset = set()
with open('large_output.out', 'w') as f:
    for i in xrange(1, int(input[0]) + 1):
        cset.clear()
        if int(input[i]):
            val = check_input(input[i].strip())
            f.write("Case #%s: %s\n" % (i, val))
        else:
