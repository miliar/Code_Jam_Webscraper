...
"""
    ...
    def run(self):
        ...
        if (self.multi == 1):
            ...
            answers = 0
            #self.multiprocess(data, self.multi)
...
def reader(inFile):
    N = inFile.getInt()
    nblocks = inFile.getFloats()
    kblocks = inFile.getFloats()

    return [N, nblocks, kblocks]

def solver(case):
    N = case[0]

    nblocksW = case[1]
    nblocksW = sorted(nblocksW)
    #nblocksW = sorted(nblocksW, reverse=True)
    kblocksW = case[2]
    kblocksW = sorted(kblocksW)

    nblocksDW = case[1]
    nblocksDW = sorted(nblocksDW)
    kblocksDW = case[2]
    kblocksDW = sorted(kblocksDW)

    #War game part
    scoreW = 0
    for i in xrange(N):
        chosenNaomi = nblocksW.pop()

        lchosenKen = []
        for j in xrange(len(kblocksW)):
            if kblocksW[j] > chosenNaomi:
                lchosenKen.append(kblocksW[j])
        if not lchosenKen:
            chosenKen = min(kblocksW)
        else:
            chosenKen = min(lchosenKen)
        kblocksW.remove(chosenKen)
    ...
    scoreDW = 0
    for i in xrange(N):
        toldNaomi = min(kblocksDW) - 0.0000000001 #Naomi choses min of Ken blocks left minus a little

        #Naomi's choice
        lchosenNaomi = []
        for j in xrange(len(nblocksDW)):
            if nblocksDW[j] > toldNaomi:
                lchosenNaomi.append(nblocksDW[j])
        if not lchosenNaomi:
            chosenNaomi = min(nblocksDW)
        else:
            chosenNaomi = min(lchosenNaomi)
        nblocksDW.remove(chosenNaomi)

        #Ken's choice
        lchosenKen = []
        for j in xrange(len(kblocksDW)):
            if kblocksDW[j] > toldNaomi:
                lchosenKen.append(kblocksDW[j])
        if not lchosenKen:
            chosenKen = min(kblocksDW)
        else:
            chosenKen = min(lchosenKen)
        kblocksDW.remove(chosenKen)
    ...
    score = " %s %s" %(scoreDW, scoreW)
    return score

if __name__ == "__main__":
    #from GCJ import GCJ
    GCJ(reader, solver, "C:/Users/Robin/PycharmProjects/GCJ2014/", "d").run()
