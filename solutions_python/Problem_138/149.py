...
""" functions """
def best_score(mine, other):
    mine_left = set(mine)
    win_count = 0
    for block in other:
        bigger_cands = [b for b in mine_left if b > block]
        if len(bigger_cands) > 0:
            chosen = min(bigger_cands)
            win_count += 1
        else:
            chosen = min(mine_left)
        mine_left.remove(chosen)
    return win_count
...
def solve():
    ...
    blocksA = read_floats()
    blocksB = read_floats()
    return "{} {}".format(best_score(blocksA, blocksB), N - best_score(blocksB, blocksA))
