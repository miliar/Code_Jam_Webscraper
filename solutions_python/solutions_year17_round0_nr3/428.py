...
def aux(N, K):
    if K == 1:
        ...
        digits = bin(K)[2:]
        offset_K = int(digits[2:], base=2) if len(digits) > 2 else 0
        new_K = 2**(len(digits)-2) + offset_K
        new_N = aux(N, new_K)[int(digits[1])]
        return new_N//2, (new_N-1)//2


def main_func(N, K):
    return " ".join(str(n) for n in aux(N, K))
...
with open("input.txt", 'r') as inputFile:
    ...
    formattedInput = \
        [
            [
                int(param) for param in inputLines[caseStartIndex[caseID]].split(colSep)
            ]
