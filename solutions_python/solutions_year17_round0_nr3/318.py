...
class CodeJamBase:
    def __init__(self):
        ...
        self._multiple_lines = False
        self._decimal_precision = 7
    ...
    def _find_input_and_output_files():
        ...
        patterns = ['^[A-Z]-large(-(attempt\d?|practice(-\d)?))?\.in$',
                    '^[A-Z]-small(-\d)?(-(attempt\d?|practice(-\d)?))?\.in$',
                    '^sample\.in$']
    ...
    def _format_case_result(self, result):
        ...
        if isinstance(result, Decimal):
            if result == Decimal(0):
                return '0.' + ''.zfill(self._decimal_precision)
            to_return = '{}'.format(result.quantize(Decimal(10) ** (-self._decimal_precision)))
            if to_return == '0E-' + str(self._decimal_precision):
                return '0.' + ''.zfill(self._decimal_precision)
            return to_return
        if isinstance(result, float):
            return ("{0:." + str(self._decimal_precision) + "f}").format(result)
        if isinstance(result, list) or isinstance(result, tuple):
            return (' ' if not self._multiple_lines else '\n').join((self._format_case_result(x) for x in result))
    ...
    def _write_output_for_case(self, i, result):
        ...
        :param result: case answer
        """
        to_output = "Case #{0}:{1}{2}".format(i, ' ' if not self._multiple_lines else '\n',
                                              self._format_case_result(result))
...
class ProblemC(CodeJamBase):
    ...
    def _read_input_for_case(self, i):
        ...
        # return self._read_n_and_x_lines(split_lines=True, map_func=int)

    def solve_case_slow(self, n: int, k: int):
        if n == k:
            return (0, 0)
        ...
        for i in range(k):
            ...
            while left_stall < n + 1:
                for j in range(left_stall + 1, n + 2):
                    if stalls[j]:
                        right_stall = j
                        break
                else:
                    raise AssertionError()
                distance = right_stall - left_stall
                if distance > 1:
                    location = left_stall + (distance // 2)
                    free_space = (right_stall - location - 1, location - left_stall - 1)
                    if current is None or (max(free_space) > max(current) or (
                                    max(free_space) == max(current) and min(free_space) > min(current))):
                        current = free_space
                        current_location = location
                left_stall = right_stall
            assert current_location is not None
            assert not stalls[current_location]
            stalls[current_location] = True
        return (max(current), min(current))

    def solve_case_quick(self, n: int, k: int):
        bin_k = bin(k)[2:]
        small_n = None
        # n -= k
        start_n = n
        for x in reversed(bin_k):
            n -= 1
            small_n = n // 2
            big_n = n - small_n
            n = big_n if x == '0' else small_n
            # print(big_n, small_n, n, x)
        return big_n, small_n

    def solve_case(self, n: int, k: int):
        return self.solve_case_quick(n, k)

    def test(self):
        for i in range(1, 100):
            for n in (100, 1000, 5000, 100000):
                correct = self.solve_case_slow(n, i)
                bin_answer = self.solve_case_quick(n, i)
                print('{} / {}: {}'.format(i, bin_answer, correct))
                assert bin_answer == correct
