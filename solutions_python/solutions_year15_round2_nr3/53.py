...
def mainCase():
    ...
    hikers = []
    for _ in range(N):
        Di, Hi, Mi = map(int, raw_input().strip().split())
        for h in range(Hi):
            hikers.append((Di, Mi+h))
    hikers.sort(key=lambda (a, b): b)
    if 1 == len(hikers):
        return 0
    elif 2 == len(hikers):
        Dfast, Mfast = hikers[0]
        Dslow, Mslow = hikers[1]
        willLap = ((720-Dfast)*Mfast <= (360-Dslow)*Mslow)
        return int(willLap)
    else:
        return 1
