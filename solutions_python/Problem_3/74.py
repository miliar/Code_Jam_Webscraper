...
for case in xrange(input()):
    ...
    # Simplification 2: Because of symmetry, we can consider the whole tennis
    # racquet, half of it, a quadrant, or an octant. We choose the top right
    # quadrant.
    ...
    for column in xrange(maxint):
        ...
        for row in xrange(maxint):
            ...
            if right ** 2 + top ** 2 <= inner_radius ** 2:
                ...
                miss_area += hole_area(left, bottom, right, top, inner_radius)
    miss_area += hole_count * gap ** 2
    miss_area *= 4

    # Print result.
    fly_area = pi * outer_radius ** 2
