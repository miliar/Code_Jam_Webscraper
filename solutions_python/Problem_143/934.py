...
"""
    ...
    def __init__(self, reader, solver, directory, question, aopts = None):
        self.reader = reader
        self.solver = solver
        ...
        if aopts is not None:
            opts = [(aopt, True) for aopt in aopts]
        for o,a in opts:
            if o in ("-s", "--small", "small"):
                datatype = "small-attempt" + str(int(a))
            elif o in ("-l", "--large", "large"):
                datatype = "large"
            elif o in ("-v", "--validate", "val", "validate"):
                self.job = "validate"
            elif o in ("-m", "--multi"):
                self.multi = int(a)
