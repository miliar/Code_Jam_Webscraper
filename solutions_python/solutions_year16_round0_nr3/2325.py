...
def calc(nb_of_jamcode, size):
    #print("size[%s], nb: %s" % (size, nb_of_jamcode))
    res = []
    for jam in range((size - 2) ** 2):
        ...
        #print(str(o))
        # tester en base 2 => 10
        # pour chaque base check si il est premier ou non
        # si il est premier dans un => continue
        #  trouver un diviseur commun
        #print("%s %s" % (jam, "{0:>{width}}".format(str_jam, width=size)))
        if flag:
            res.append(o)
