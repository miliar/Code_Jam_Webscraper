...
#    undone = False
    for slc in slice_board(board):
        if np.all(xwins == slc, 1).any():
            return 'X won'
        if np.all(owins == slc, 1).any():
            return 'O won'
#        if not undone:
#            undone = np.all(incomplete == slc, 1).any()
    if (board == '.').any():
...
if __name__ == '__main__':
    ...
    for case_num in xrange(num_cases):
        print 'Case #%d: %s' % (case_num+1, eval_board(read_board()))
