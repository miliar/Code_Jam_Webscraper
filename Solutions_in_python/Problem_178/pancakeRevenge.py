def pancakeFlipper(inputString):
	
	inputString.strip()
	#print (inputString)
	rawPancakeStack = list(inputString)
	#print (rawPancakeStack)
	if len(rawPancakeStack) > 1:
		rawPancakeStack = list(reversed(rawPancakeStack))
	print (rawPancakeStack)
	
	for n,i in enumerate(rawPancakeStack):
		if i=='+':
			rawPancakeStack[n] = 1
		if i=='-':
			rawPancakeStack[n] = -1
	...
	cookedPancakeStack = [int(pancake) for pancake in rawPancakeStack]
	numberOfFlips = 0


	notTimeToServe = True
	while notTimeToServe:

		if all(pancake > 0 for pancake in cookedPancakeStack):
			notTimeToServe = False
			pass
			...
			for x in range(cookedPancakeStack.index(-1), len(cookedPancakeStack)):
				print (x)
				cookedPancakeStack[x] = (cookedPancakeStack[x] * -1)

			print (cookedPancakeStack)
...
def googleCodeReporter():
	...
	with open(inputFileName) as inputFile:
		...
		for x in sampleIndex:
			...
			value = pancakeFlipper(inputFile[x])
