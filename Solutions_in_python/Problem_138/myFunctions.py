...
# def tree_nodes(C, F, X, current_production, time_passed):

# 	direct_time = X/(current_production)
# 	time_buy_farm = C/(current_production)
# 	indirect_time = time_buy_farm + X/(current_production + F)
# 	# print "new times: ", current_production, direct_time, time_buy_farm

# 	if indirect_time >= direct_time:
# 		return time_passed + direct_time, time_passed + direct_time
# 	else:
# 		# sleep(2)
# 		# print direct_time, indirect_time
# 		# return tree_nodes(C, F, X, 2.0 + F, time_passed + time_buy_farm)
# 		return time_passed + direct_time, time_passed + time_buy_farm



def cookie_clicker(l):
	...
	# a, b = tree_nodes(l[0], l[1], l[2], 2.0, 0.0)
	# print a, b
	...
	# while (a != b):
	# 	sleep(1)
	# 	current_production += l[1]
	# 	a, b = tree_nodes(l[0], l[1], l[2], current_production, b)
	# 	print a, b
	# 	# res = min(a, b)
...
def deceitful_war(l):
	...
	for i in range(n):
		if w4[-1] < w3[i]:
			...
			for j in range(i, n):
				if w4[j] > w3[i]:
					w4 = w4[:i] + [w4[j]] + w4[i:j] + w4[j+1:] 
					break 
		
	return str(naomi_wins) + " " + str(naomi_wins_war)		
